...
이제 모든 Real-Time CDP 및 Adobe Journey Optimizer 고객이 [!UICONTROL Use Case Playbooks] 기능을 일반적으로 사용할 수 있습니다. [!UICONTROL Use Case Playbooks]은(는) Real-Time Customer Data Platform 또는 Adobe Journey Optimizer으로 시작할 때 사용자가 문제를 극복할 수 있도록 설계되었습니다. 어디서부터 시작해야 할지, 원하는 사용 사례에 적합한 자산을 어떻게 생성해야 할지 불분명한 경우, 사용 사례 플레이북이 영감을 제공하고 테스트할 수 있는 다양한 자산을 만들어 준비가 되면 프로덕션 환경으로 가져올 수 있도록 도와줍니다.

[!UICONTROL Use Case Playbooks]을(를) 시작하려면 다음 설명서 페이지를 참조하세요.
