...
| 쿼리 실행 지연 | 쿼리 실행 지연에 대한 알림을 설정하여 계산 시간을 제어할 수 있습니다. 특정 기간이 지나도 쿼리 상태가 변경되지 않는 경우 경고를 받도록 선택할 수 있습니다. 쿼리 진행 상황에 대한 최신 정보를 계속 제공하려면 Experience Platform UI에서 원하는 지연 시간을 설정하기만 하면 됩니다. UI에서 이 경고 설정 방법을 알아보려면 [쿼리 예약 설명서](../../query-service/ui/query-schedules.md#alerts-for-query-status) 또는 [인라인 쿼리 액션 안내서](../../query-service/ui/monitor-queries.md#query-run-delay)를 참조하십시오. |
| 간소화된 쿼리 로그 인벤토리 | 이제 [간소화된 쿼리 로그 UI](../../query-service/ui/query-logs.md#filter-logs)를 통해 개선된 문제 해결 효율성과 작업 모니터링을 사용할 수 있습니다. <ul><li> 이제 Experience Platform UI는 기본적으로 모든 &quot;시스템 쿼리&quot;를 로그 탭에서 제외합니다. </li><li> **시스템 쿼리 제외**&#x200B;를 선택 취소하여 시스템 쿼리를 확인합니다. </li></ul> <br> ![쿼리 UI 작업 영역의 로그 탭.](../2024/assets/may/query-log.png "쿼리 UI 작업 영역의 로그 탭."){width="100" zoomable="yes"} <br> 간소화된 쿼리 로그 UI를 사용하여 관련 로그를 신속하게 식별하고 분석하는 데 도움이 되는 보다 집중해서 볼 수 있습니다. |
| 데이터베이스 선택기 | 새로운 데이터베이스 선택기 드롭다운 메뉴를 사용하여 [Power BI 또는 Tableau에서 Customer Journey Analytics 데이터 보기에 편리하게 액세스](../../query-service/ui/credentials.md#connect-to-customer-journey-analytics)할 수 있습니다. 이제 Experience Platform UI에서 원하는 데이터베이스를 직접 선택하여 BI 도구를 보다 원활하게 통합할 수 있습니다. <br> ![쿼리 UI 작업 영역의 자격 증명 탭.](../2024/assets/may/database-selector.png "쿼리 UI 작업 영역의 자격 증명 탭."){width="100" zoomable="yes"} <br> |
