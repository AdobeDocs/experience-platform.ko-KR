...
사용 가능한 B2B 기능에 대한 자세한 내용은 [Real-Time CDP, B2B 에디션 개요](../../rtcdp/b2b-overview.md)를 참조하고, B2B 워크플로의 일부로 계정 프로필을 만드는 방법에 대한 자세한 내용은 [전체 자습서](../../rtcdp/b2b-tutorial.md)를 참조하십시오.
...
| 기능 | 설명 |
| --- | --- |
| [!DNL Data Prep] Recommendations 개선 사항 | [!DNL Data Prep] Recommendations이 더 똑똑하고 빨라졌습니다. 새 유효성 검사 를 통해 가장 일반적인 매핑 오류가 크게 줄어 값 생성 시간이 더욱 단축됩니다. |
...
| 예측 리드 및 계정 점수 | 예측 리드 및 계정 점수는 기회 단계 전환 이벤트를 학습하고 예측하며, 계정 점수를 생성하기 위해 개인 활동을 계정 수준으로 집계하는 트리 기반(랜덤 포레스트/그래디언트 부스팅) 머신 러닝 방법을 사용합니다. B2B 마케터가 점수를 유도한 요소를 더 잘 이해하는 데 도움이 되는 가장 영향력 있는 요소는 집계와 단위 수준 모두에서 사용할 수도 있습니다. <br><br>자세한 내용은 [예측 리드 및 계정 점수](../../rtcdp/b2b-ai-ml-services/predictive-lead-and-account-scoring.md)에 대한 문서를 참조하십시오. 점수를 관리하는 방법에 대한 자세한 내용은 [Real-time Customer Data Platform, B2B Edition에서 예측 리드 및 계정 점수 관리에 대한 안내서를 참조하십시오.](../../rtcdp/b2b-ai-ml-services/manage-predictive-lead-and-account-scoring.md) |

프로필 데이터 보강 모니터링 방법에 대한 지침은 [UI에서 프로필 데이터 보강 모니터링](../../dataflows/ui/b2b/monitor-profile-enrichment.md)에 대한 문서를 참조하십시오. Real-Time CDP B2B 에디션에 대한 자세한 내용은 [Real-Time CDP B2B 개요](../../rtcdp/overview.md)를 참조하세요.
...
| 기능 | 설명 |
| --- | --- |
| [!DNL Azure Data Explorer] 소스의 일반 가용성 | Azure Data Explorer 소스를 사용하여 [!DNL Azure] 인스턴스의 데이터를 Experience Platform 상태로 가져옵니다. 자세한 내용은 [[!DNL Azure Data Explorer] 소스 개요](../../sources/connectors/databases/data-explorer.md)를 참조하십시오. |
| [!DNL Generic OData] 소스의 일반 가용성 | [!DNL Generic OData] 원본을 사용하여 개방형 데이터 프로토콜을 지원하는 시스템의 리소스를 Experience Platform 상태로 만듭니다. 자세한 내용은 [[!DNL Generic OData] 소스 개요](../../sources/connectors/protocols/odata.md)를 참조하십시오. |
| Experience Platform UI에서 [!DNL Data Landing Zone]에 대한 소스 파일 속성의 자동 검색 지원 | 이제 [!DNL Data Landing Zone] 원본은 Experience Platform UI를 사용할 때 파일 속성의 자동 검색을 지원합니다. 자세한 내용은 [소스 연결 만들기 [!DNL Data Landing Zone] 에 대한 설명서를 참조하십시오.](../../sources/tutorials/ui/create/cloud-storage/data-landing-zone.md) |
