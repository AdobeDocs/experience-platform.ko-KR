...
Adobe은 더 이상 Adobe Experience Platform의 태그에 대해 [Internet Explorer 10(IE10)](https://docs.microsoft.com/en-us/lifecycle/announcements/internet-explorer-10-end-of-support) 및 [Internet Explorer 11(IE11)](https://docs.microsoft.com/en-us/lifecycle/announcements/internet-explorer-11-end-of-support)을 지원하는 데 투자하지 않습니다. Microsoft은 이러한 브라우저에 대한 지원을 종료했으며, 최신 웹 기술은 점점 더 이들 브라우저와 호환되지 않으며 더 광범위한 산업이 이러한 브라우저로부터 멀어졌습니다. 이러한 요인들은 이전 버전과의 호환성을 유지하는 것을 어렵고 비용이 많이 듭니다. Adobe에서 IE10 및 IE11 지원을 적극적으로 제거하지 않지만 새 태그 기능은 더 이상 이러한 브라우저에 맞게 테스트되거나 최적화되지 않습니다. 이 변경 사항을 통해 Adobe은 최신 기능을 더 빠르게 제공할 수 있습니다.
...
| 기능 | 설명 |
| --- | --- |
| 의료 산업 데이터 모델 | 디지털 획득 증가, 프로그램 등록 개선 및 약물 정보 홍보와 관련된 5가지 일반적인 산업 사용 사례를 지원하기 위해 표준 헬스케어 데이터 모델이 도입되었습니다. 이러한 사용 사례와 이를 지원하는 표준 XDM 구성 요소에 대한 자세한 내용은 [의료 데이터 모델](../../xdm/schema/industries/healthcare.md)에 대한 개요를 참조하십시오.<br><br>사용자 지정 스키마를 작성할 때 의료 관련 구성 요소를 탐색할 수 있도록 [!UICONTROL Schemas] UI에 새로운 업계 필터가 추가되었습니다. |
...
| 구성 요소 유형 | 이름 | 설명 |
| --- | --- | --- |
| 클래스 | [[!UICONTROL Weather]](https://github.com/adobe/xdm/blob/master/components/classes/weather.schema.json) | 날씨 데이터를 캡처하는 데 사용되는 레코드 기반 클래스입니다. |
| 필드 그룹 | [[!UICONTROL Current Weather]](https://github.com/adobe/xdm/blob/master/components/classes/weather.schema.json) | 우편 번호에 대한 현재 기상 상태를 캡처하는 데 사용되는 [!UICONTROL XDM ExperienceEvent] 및 [!UICONTROL Weather] 클래스의 필드 그룹입니다. |
| 필드 그룹 | [[!UICONTROL Forecasted Weather]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/forecasted-weather.schema.json) | 우편 번호에 대해 예측된 기상 조건을 캡처하는 데 사용되는 [!UICONTROL XDM ExperienceEvent] 및 [!UICONTROL Weather] 클래스의 필드 그룹입니다. |
| 필드 그룹 | [[!UICONTROL Product Triggers]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/product-triggers.schema.json) | 소비자 행동을 유도하는 것으로 알려진 기상 조건을 활용하는 제품별 트리거를 캡처하는 데 사용되는 [!UICONTROL XDM ExperienceEvent] 및 [!UICONTROL Weather] 클래스의 필드 그룹입니다. |
| 필드 그룹 | [[!UICONTROL Relative Triggers]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/relative-triggers.schema.json) | 소비자 행동을 유도하는 것으로 알려진 기상 조건을 활용하는 상대 트리거를 캡처하는 데 사용되는 [!UICONTROL XDM ExperienceEvent] 및 [!UICONTROL Weather] 클래스의 필드 그룹입니다. |
| 필드 그룹 | [[!UICONTROL Severe Triggers]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/severe-triggers.schema.json) | 소비자 행동을 유도하는 것으로 알려진 심각한 날씨 조건을 활용하는 트리거를 캡처하는 데 사용되는 [!UICONTROL XDM ExperienceEvent] 및 [!UICONTROL Weather] 클래스의 필드 그룹입니다. |
| 필드 그룹 | [[!UICONTROL Weather Triggers]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/weather-triggers.schema.json) | 소비자 행동을 유도하는 것으로 알려진 기상 조건을 활용하는 일반 트리거를 캡처하는 데 사용되는 [!UICONTROL XDM ExperienceEvent] 및 [!UICONTROL Weather] 클래스의 필드 그룹입니다. |
| 필드 그룹 | [[!UICONTROL Media Interaction Details]](https://github.com/adobe/xdm/blob/master/docs/reference/fieldgroups/experience-event/experienceevent-media.schema.json) | 미디어 인터랙션에 대한 세부 정보를 캡처하는 [!UICONTROL XDM ExperienceEvent] 클래스의 필드 그룹입니다. |
| 필드 그룹 | [[!UICONTROL MediaAnalytics Interaction Details]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/experience-event/experienceevent-media-analytics.schema.json) | 미디어 보고와의 인터랙션에 대한 세부 정보를 캡처하는 [!UICONTROL XDM ExperienceEvent] 클래스의 필드 그룹입니다. |
| 데이터 유형 | [[!UICONTROL Advertising details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/advertisingdetails.schema.json) | 광고 자산에 대한 세부 정보를 캡처합니다. |
| 데이터 유형 | [[!UICONTROL Advertising Pod details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/advertisingpoddetails.schema.json) | 광고 pod에 대한 세부 정보를 캡처합니다. 이 pod는 단일 광고 브레이크 내에서 연속으로 재생되는 여러 광고의 시퀀스입니다. |
| 데이터 유형 | [[!UICONTROL Chapter details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/chapterdetails.schema.json) | 비디오 컨텐츠의 챕터 또는 세그먼트에 대한 세부 정보를 캡처합니다. |
| 데이터 유형 | [[!UICONTROL Error details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/errordetails.schema.json) | 비디오 재생 오류에 대한 세부 정보를 캡처합니다. |
| 데이터 유형 | [[!UICONTROL Player state data information]](https://github.com/adobe/xdm/blob/master/components/datatypes/playerstatedata.schema.json) | 비디오 플레이어에 대한 상태 관련 세부 정보를 캡처합니다. |
| 데이터 유형 | [[!UICONTROL Qoe Data details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/qoedatadetails.schema.json) | 비디오 재생 이벤트에 대한 QoE(체감 품질) 세부 정보를 캡처합니다. |
| 데이터 유형 | [[!UICONTROL Session details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/sessiondetails.schema.json) | 비디오 재생 이벤트에 대한 세션 세부 정보를 캡처합니다. |
