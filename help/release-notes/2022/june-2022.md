...
Data Science Workspace은 머신 러닝 및 인공 지능을 사용하여 데이터에서 통찰력을 제공합니다. Adobe Experience Platform에 통합된 Data Science Workspace은 Adobe 솔루션 전반에서 컨텐츠 및 데이터 자산을 사용하여 예측을 수행할 수 있도록 합니다. Data Science Workspace이 이를 달성하는 방법 중 하나는 JupyterLab을 사용하는 것입니다. JupyterLab은 <a href="https://jupyter.org/" target="_blank">Project Jupyter</a>용 웹 기반 사용자 인터페이스이며 Adobe Experience Platform에 긴밀하게 통합됩니다. 데이터 과학자가 Jupyter 노트북, 코드 및 데이터를 사용할 수 있는 대화형 개발 환경을 제공합니다.
...
| 기능 | 설명 |
| --- | --- |
| 데이터스트림에 대한 [액세스 유형 구성](../../datastreams/overview.md#create) | 이제 새 데이터 스트림을 만들 때 Edge Network이 수락할 요청 유형을 선택할 수 있습니다. <ul><li>**[!UICONTROL 혼합 인증]**: 이 옵션을 선택하는 경우 Edge Network는 인증된 요청과 인증되지 않은 요청을 모두 허용합니다. Web SDK 또는 [Mobile SDK](https://developer.adobe.com/client-sdks/documentation/)를 사용하려는 경우 [Server API](../../server-api/overview.md)와 함께 이 옵션을 선택합니다. </li><li>**[!UICONTROL 인증된 요청만]**: 이 옵션을 선택하는 경우 Edge Network는 인증된 요청만 허용합니다. 서버 API만 사용하고 인증되지 않은 요청이 [!DNL Edge Network]에 의해 처리되지 않도록 하려면 이 옵션을 선택하십시오. </li></ul> |
...
| [Mobile-to-Web 및 도메인 간 ID 공유](../../web-sdk/identity/id-sharing.md) | 이제 Adobe Experience Platform Web SDK가 방문자 ID 공유 기능을 지원하므로 모바일 앱과 모바일 웹 콘텐츠 간에 그리고 도메인 간에 보다 정확하게 개인화된 경험을 제공할 수 있습니다. |
