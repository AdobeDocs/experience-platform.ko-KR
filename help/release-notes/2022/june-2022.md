...
| 기능 | 설명 |
| --- | --- |
| 데이터스트림에 대한 [액세스 유형 구성](../../datastreams/overview.md#create) | 이제 새 데이터 스트림을 만들 때 Edge Network에서 허용할 요청 유형을 선택할 수 있습니다. <ul><li>**[!UICONTROL 혼합 인증]**: 이 옵션을 선택하는 경우 Edge Network는 인증된 요청과 인증되지 않은 요청을 모두 허용합니다. Web SDK 또는 [Mobile SDK](https://developer.adobe.com/client-sdks/documentation/)를 사용하려는 경우 [Server API](../../server-api/overview.md)와 함께 이 옵션을 선택합니다. </li><li>**[!UICONTROL 인증된 요청만]**: 이 옵션을 선택하는 경우 Edge Network는 인증된 요청만 허용합니다. 서버 API만 사용하고 인증되지 않은 요청이 [!DNL Edge Network]에 의해 처리되지 않도록 하려면 이 옵션을 선택하십시오. </li></ul> |
