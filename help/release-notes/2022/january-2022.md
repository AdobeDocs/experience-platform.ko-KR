...
| 기능 | 설명 |
| --- | --- |
| 익명 블록 | 익명 블록 SQL 구성을 사용하면 쿼리 서비스의 대규모 데이터 준비 작업을 더 작은 작업으로 분류한 다음 증분 데이터 로드를 위해 순서대로 재사용하고 실행할 수 있습니다. 자세한 내용은 [익명 블록 설명서에 대한 샘플 쿼리](../../query-service/key-concepts/anonymous-block.md). |
