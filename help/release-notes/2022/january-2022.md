...
| 기능 | 설명 |
| --- | --- |
| 통합 매핑 경험 | Platform UI의 새 매핑 인터페이스는 일관된 매핑 환경을 제공하여 지능형 매핑 권장 사항을 활용하고, 매핑 규칙을 수동으로 구성하며, 매핑 세트에 발생하는 모든 오류를 디버깅합니다. 자세한 내용은 [[!DNL Data Prep] UI 안내서](../../data-prep/ui/mapping.md)를 참조하십시오. |
...
| 기능 | 설명 |
| ----------- | ----------- |
| 동일 페이지 및 다음 페이지 개인화 | [동일 페이지 및 다음 페이지 개인화 기능](../../destinations/ui/activate-edge-personalization-destinations.md)은(는) 마케팅과 고객 채널 간의 일관성을 위해 Edge Network의 애플리케이션에 대한 사용자를 대상으로 공유 가능한 보기를 제공합니다. 이 개인화는 [Adobe Target 연결](../../destinations/catalog/personalization/adobe-target-connection.md) 및 [사용자 지정 개인화 연결](../../destinations/catalog/personalization/custom-personalization.md)을 통해 가능합니다. 같은 페이지 또는 다음 페이지 개인화 캠페인을 구성하려면 [전용 자습서](../../destinations/ui/activate-edge-personalization-destinations.md)를 참조하십시오. |
...
| Marketo 대상 개선 사항 | Marketo Engage을 사용하는 Experience Platform 고객은 [Marketo 대상 커넥터](/help/destinations/catalog/adobe/marketo-engage.md)를 통해 새로운 개인 레코드를 Experience Platform에서 Marketo Engage으로 푸시하는 새로운 기능을 통해 Marketo 데이터베이스를 극대화할 수 있습니다. <br> 대상 세그먼트를 Experience Platform에서 Marketo Engage으로 보낼 때 Marketo Engage 데이터베이스에 아직 없는 세그먼트 내의 사람은 자동으로 추가할 수 있습니다. 자세한 내용은 [Adobe Experience Platform 세그먼트를 Marketo 정적 목록으로 푸시](https://experienceleague.adobe.com/docs/marketo/using/product-docs/core-marketo-concepts/smart-lists-and-static-lists/static-lists/push-an-adobe-experience-platform-segment-to-a-marketo-static-list.html)를 참조하십시오(자습서의 9단계는 새로운 사용자 레코드를 Marketo으로 푸시하는 방법을 나타냄). |
...
| 기능 | 설명 |
| --- | --- |
| 샌드박스 UI 개선 사항 | 이제 샌드박스 표시기가 모든 Platform UI 애플리케이션의 헤더에 통합되었습니다. 샌드박스 표시기에는 샌드박스 이름, 영역 및 유형이 표시되며 드롭다운 메뉴에 액세스하여 샌드박스 간을 전환할 수 있습니다. 자세한 내용은 [샌드박스 UI 안내서](../../sandboxes/ui/user-guide.md)를 참조하십시오. |
...
| 기능 | 설명 |
| --- | --- |
| 세그먼트 일치 | 세그먼트 일치는 두 명 이상의 플랫폼 사용자가 안전하고 관리되며 개인정보 보호에 친화적인 방식으로 공통 식별자를 기반으로 데이터를 교환할 수 있는 데이터 공동 작업 서비스입니다. 세그먼트 일치는 Platform 개인 정보 보호 표준과 해시된 이메일, 해시된 전화번호 및 IDFA 및 GAID와 같은 장치 식별자와 같은 개인 식별자를 사용합니다. 자세한 내용은 [세그먼트 일치 개요](../../segmentation/ui/segment-match/overview.md)를 참조하세요. |
