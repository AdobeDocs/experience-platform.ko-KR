...
| 선택적 데이터 스트림 ID [Adobe Target 대상](../../destinations/catalog/personalization/adobe-target-connection.md) | Experience Platform Web SDK를 구현할 수 없는 사용자를 위해 Adobe Target 개인화를 활성화하기 위해 이제 Adobe Target 대상을 구성할 때 데이터 스트림 ID 선택은 선택 사항입니다. 데이터 스트림을 사용하지 않는 경우 Experience Platform에서 Target으로 내보낸 세그먼트는 다음 세션 개인화만 지원하고 에지 세분화는 비활성화됩니다 [사용 사례](../../destinations/ui/configure-personalization-destinations.md) 이는 에지 세그멘테이션에 따라 다릅니다. |
