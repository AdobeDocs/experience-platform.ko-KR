...
| 기능 | 설명 |
| --- | --- |
| ADLS 데이터 세트에 대한 열 수준의 통계 계산 | 다음 `ANALYZE TABLE` 명령을 확장했습니다. `COMPUTE STATISTICS` SQL 명령. 이제 ADLS 데이터 세트의 하위 집합 또는 해당 데이터 세트 내의 특정 열에 대한 통계를 계산할 수 있습니다. 자세한 내용은 [데이터 세트 통계 계산 안내서](../../query-service/essential-concepts/dataset-statistics.md)를 참조하십시오. |
