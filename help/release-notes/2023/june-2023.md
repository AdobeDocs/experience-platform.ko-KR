...
| 유형 | 기능 | 설명 |
| --- | --- | --- |
| 확장 | [!DNL Google Cloud Platform] 이벤트 전달 확장 | 다음 [[!DNL Google Cloud Platform]](../../tags/extensions/server/google-cloud-platform/overview.md) 이벤트 전달 확장을 사용하면 Google pub/sub를 통해 활성화를 위해 이벤트 데이터를 Google에 전달할 수 있습니다. |
...
| 확장 | [!DNL TikTok] 전환 API 확장 | 다음 [!DNL TikTok] 전환 API 이벤트 전달 확장을 사용하면 Adobe Experience Platform Edge Network에서 캡처한 데이터를 활용하여 로 전송할 수 있습니다. [!DNL Pinterest] 를 사용하는 서버측 이벤트 형식으로 [!DNL TikTok] 전환 API입니다. |
| 암호 | OAuth 2 JWT 암호 | 다음 [OAuth 2 JWT 암호](https://experienceleague.adobe.com/docs/experience-platform/tags/event-forwarding/secrets.html?lang=en) 을(를) 통해 Adobe 및 [!DNL Google] 이벤트 전달에서 서버 간 상호 작용을 지원하는 서비스 토큰입니다. |
| 태그 및 이벤트 전달 | 사용자 속성 | 사용자 속성은 다음을 통해 주요 활동 데이터를 제공합니다 [태그](https://experienceleague.adobe.com/docs/experience-platform/tags/home.html?lang=ko-KR) 및 [이벤트 전달](https://experienceleague.adobe.com/docs/experience-platform/tags/event-forwarding/overview.html?lang=en) UI.<br><br>데이터에는 누가 언제 어떤 변화를 주었는지 포함됩니다. 이 데이터는 다음 화면의 태그 및 이벤트 전달 UI에 표시됩니다.<br><br>* 속성 개요<br><br>* 설치된 확장<br><br>* 규칙 비교 및 검토<br><br>* 데이터 요소 비교 검토<br><br>* 확장 비교 검토<br><br>* 라이브러리 리소스 변경 사항<br><br>* 라이브러리 마지막 빌드 및 게시됨 |
