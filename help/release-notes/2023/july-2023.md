...
| ----------- |----------------|----------- |
| [[!DNL LiveRamp - Onboarding]](../../destinations/catalog/advertising/liveramp-onboarding.md) | 신규 | Adobe Experience Platform의 ID를 [!DNL LiveRamp Connect]로 온보딩하여 [!DNL Ramp ID] 식별자를 사용해 모바일, 오픈 웹, 소셜 및 [!DNL CTV] 플랫폼의 사용자를 타겟팅할 수 있습니다. |
| [[!DNL Azure Data Lake Storage Gen2]](../../destinations/catalog/cloud-storage/adls-gen2.md) | 신규 | [!DNL Azure Data Lake Storage Gen2]에 대한 실시간 아웃바운드 연결을 만들어 주기적으로 Adobe Experience Platform의 데이터 파일을 자체 스토리지 위치로 내보냅니다. 이 새 대상은 향상된 파일 내보내기 기능을 제공하며 [!BADGE Beta]{type=Informative} 데이터 세트 내보내기를 지원합니다. |
| [[!DNL Data Landing Zone]](../../destinations/catalog/cloud-storage/data-landing-zone.md) | 신규 | [!DNL Data Landing Zone]은(는) Adobe Experience Platform에서 프로비저닝한 [!DNL Azure Blob] 저장소 인터페이스로서, Experience Platform에서 파일을 내보낼 수 있는 안전한 클라우드 기반 파일 저장소 기능에 대한 액세스 권한을 부여합니다. 이 새 대상은 향상된 파일 내보내기 기능을 제공하며 [!BADGE Beta]{type=Informative} 데이터 세트 내보내기를 지원합니다. |
| [[!DNL Google Cloud Storage]](../../destinations/catalog/cloud-storage/google-cloud-storage.md) | 신규 | [!DNL Google Cloud Storage]에 대한 실시간 아웃바운드 연결을 만들어 주기적으로 Adobe Experience Platform의 데이터 파일을 자체 버킷으로 내보냅니다. 이 새 대상은 향상된 파일 내보내기 기능을 제공하며 [!BADGE Beta]{type=Informative} 데이터 세트 내보내기를 지원합니다. |
...
| 구성 요소 유형 | 이름 | 설명 |
| --- | --- | --- |
| 클래스 | [[!UICONTROL XDM Individual Prospect Profile]](https://github.com/adobe/xdm/pull/1758/files) | 이 클래스를 사용하여 데이터 공급업체의 단계 상단 고객 확보 사용 사례에서 소싱된 잠재 고객 프로필을 가져옵니다. |
| 필드 그룹 | [[!UICONTROL Enriched Event Segment Details]](https://github.com/adobe/xdm/pull/1754/files) | 이벤트 수집 시점에 프로필이 자격을 갖춘 대상자 목록입니다. |
...
| 구성 요소 유형 | 이름 | 설명 업데이트 |
| --- | --- | --- |
| 필드 그룹 | [[!UICONTROL MediaAnalytics Interaction Details]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status`가 `experimental`에서 `stable`로 업데이트되었습니다. |
| 필드 그룹 | [[!UICONTROL Media Interaction Details]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status`가 `stable`에서 `deprecated`로 업데이트되었습니다. |
| 데이터 유형 | [[!UICONTROL Session details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status`가 `experimental`에서 `stable`로 업데이트되었습니다. |
| 데이터 유형 | [[!UICONTROL Qoe Data details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status`가 `experimental`에서 `stable`로 업데이트되었습니다. |
| 데이터 유형 | [[!UICONTROL Player state data information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status`가 `experimental`에서 `stable`로 업데이트되었습니다. |
| 데이터 유형 | [[!UICONTROL Media event information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status`가 `experimental`에서 `stable`로 업데이트되었습니다. |
| 데이터 유형 | [[!UICONTROL Media details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status`가 `experimental`에서 `stable`로 업데이트되었습니다. |
| 데이터 유형 | [[!UICONTROL Error details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status`가 `experimental`에서 `stable`로 업데이트되었습니다. |
| 데이터 유형 | [[!UICONTROL Error details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status`가 `stable`에서 `deprecated`로 업데이트되었습니다. |
| 데이터 유형 | [[!UICONTROL Custom metadata details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status`가 `experimental`에서 `stable`로 업데이트되었습니다. |
| 데이터 유형 | [[!UICONTROL Chapter details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status`가 `experimental`에서 `stable`로 업데이트되었습니다. |
| 데이터 유형 | [[!UICONTROL Advertising Pod details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status`가 `experimental`에서 `stable`로 업데이트되었습니다. |
| 데이터 유형 | [[!UICONTROL Advertising details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status`가 `experimental`에서 `stable`로 업데이트되었습니다. |
| 확장 기능(고객 여정 관리) | [[!UICONTROL Domain]](https://github.com/adobe/xdm/pull/1756/files) | 받는 사람 전자 메일 주소의 도메인을 기록하기 위해 `Domain` 필드를 [!UICONTROL Adobe CJM ExperienceEvent - Message Profile Details]에 추가했습니다. |
| 확장 기능(고객 여정 관리) | [[!UICONTROL Channel's variant Name]](https://github.com/adobe/xdm/pull/1753/files) | 이 필드는 채널 변형 이름을 나타내기 위해 [!UICONTROL AJO Entity Fields]에 추가되었습니다. |
| 확장 기능(Adobe Analytics) | [[!UICONTROL Context value]](https://github.com/adobe/xdm/pull/1761/files) | `Context value`가 [!UICONTROL `Adobe Analytics ExperienceEvent Full Extension`]에 추가되었습니다. |
