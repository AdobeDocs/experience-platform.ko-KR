...
| 기능 | 설명 |
| --- | --- |
| 신규 매퍼 기능 | 이제 데이터 준비에서 오브젝트를 매핑할 때 다음 기능을 사용할 수 있습니다. <ul><li>`map_get_values`</li><li>`map_has_keys`</li><li>`add_to_map`</li></ul> 이러한 기능에 대한 자세한 내용은 [데이터 준비 기능 안내서](../../data-prep/functions.md#hierarchies---objects)를 참조하십시오. |
...
| 구성 요소 유형 | 이름 | 설명 |
| --- | --- | --- |
| 클래스 | [[!UICONTROL XDM 개별 잠재 고객 프로필]](https://github.com/adobe/xdm/pull/1758/files) | 이 클래스를 사용하여 데이터 공급업체의 주요 고객 확보 사용 사례에서 가져온 잠재 고객 프로필을 가져올 수 있습니다. |
| 필드 그룹 | [[!UICONTROL 보강된 이벤트 세그먼트 세부 정보]](https://github.com/adobe/xdm/pull/1754/files) | 이벤트 수집 시 프로필에서 사용할 수 있는 대상 목록입니다. |
...
| 구성 요소 유형 | 이름 | 설명 업데이트 |
| --- | --- | --- |
| 필드 그룹 | [[!UICONTROL MediaAnalytics 상호 작용 세부 정보]](https://github.com/adobe/xdm/pull/1756/files) | 다음 `meta:status` 실험에서 (으)로 업데이트됨 `stable`. |
| 필드 그룹 | [[!UICONTROL 미디어 인터랙션 세부 정보]](https://github.com/adobe/xdm/pull/1756/files) | 다음 `meta:status` 다음에서 업데이트됨: `stable` 끝 `deprecated`. |
| 데이터 유형 | [[!UICONTROL 세션 세부 정보]](https://github.com/adobe/xdm/pull/1756/files) | 다음 `meta:status` 다음에서 업데이트됨: `experimental` 끝 `stable`. |
| 데이터 유형 | [[!UICONTROL Qoe 데이터 세부 정보]](https://github.com/adobe/xdm/pull/1756/files) | 다음 `meta:status` 다음에서 업데이트됨: `experimental` 끝 `stable`. |
| 데이터 유형 | [[!UICONTROL 플레이어 상태 데이터 정보]](https://github.com/adobe/xdm/pull/1756/files) | 다음 `meta:status` 다음에서 업데이트됨: `experimental`끝 `stable`. |
| 데이터 유형 | [[!UICONTROL 미디어 이벤트 정보]](https://github.com/adobe/xdm/pull/1756/files) | 다음 `meta:status` 다음에서 업데이트됨: `experimental` 끝 `stable`. |
| 데이터 유형 | [[!UICONTROL 미디어 세부 정보]](https://github.com/adobe/xdm/pull/1756/files) | 다음 `meta:status` 다음에서 업데이트됨: `experimental` 끝 `stable`. |
| 데이터 유형 | [[!UICONTROL 오류 세부 정보]](https://github.com/adobe/xdm/pull/1756/files) | 다음 `meta:status` 다음에서 업데이트됨: `experimental` 끝 `stable`. |
| 데이터 유형 | [[!UICONTROL 오류 세부 정보]](https://github.com/adobe/xdm/pull/1756/files) | 다음 `meta:status` 다음에서 업데이트됨: `stable` 끝 `deprecated`. |
| 데이터 유형 | [[!UICONTROL 사용자 지정 메타데이터 세부 정보]](https://github.com/adobe/xdm/pull/1756/files) | 다음 `meta:status` 다음에서 업데이트됨: `experimental` 끝 `stable`. |
| 데이터 유형 | [[!UICONTROL 챕터 세부 정보]](https://github.com/adobe/xdm/pull/1756/files) | 다음 `meta:status` 다음에서 업데이트됨: `experimental` 끝 `stable`. |
| 데이터 유형 | [[!UICONTROL Advertising Pod 세부 정보]](https://github.com/adobe/xdm/pull/1756/files) | 다음 `meta:status` 다음에서 업데이트됨: `experimental` 끝 `stable`. |
| 데이터 유형 | [[!UICONTROL 광고 상세 정보]](https://github.com/adobe/xdm/pull/1756/files) | 다음 `meta:status` 다음에서 업데이트됨: `experimental` 끝 `stable`. |
| 확장(고객 여정 관리) | [[!UICONTROL 도메인]](https://github.com/adobe/xdm/pull/1756/files) | `Domain` 필드 추가됨 [!UICONTROL Adobe CJM ExperienceEvent - 메시지 프로필 세부 정보] 받는 사람 이메일 주소의 도메인을 기록합니다. |
| 확장(고객 여정 관리) | [[!UICONTROL 채널 변형 이름]](https://github.com/adobe/xdm/pull/1753/files) | 이 필드가에 추가되었습니다. [!UICONTROL AJO 엔티티 필드] 채널 변형 이름을 나타냅니다. |
| 확장(Adobe Analytics) | [[!UICONTROL 컨텍스트 값]](https://github.com/adobe/xdm/pull/1761/files) | `Context value` 이(가)에 추가되었습니다. [!UICONTROL `Adobe Analytics ExperienceEvent Full Extension`]. |
