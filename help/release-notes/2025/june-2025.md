...
| 기능 | 설명 |
| ------- | ----------- |
| 대시보드 데이터 내보내기 권한 | 이제 대시보드의 **[!UICONTROL Download CSV]** 및 **[!UICONTROL Send as email]** 옵션에 **[!UICONTROL Export Dashboard Data]** 권한이 필요합니다. 이 권한은 인가된 사용자만 집계된 인사이트 데이터를 내보낼 수 있도록 하여 더 엄격한 거버넌스 및 데이터 액세스 제어 정책을 지원합니다. 자세한 내용은 액세스 제어 안내서의 [권한 섹션](../../access-control/home.md#permissions)을 참조하십시오. |
...
| 기능 | 설명 |
| ------- | ----------- |
| 이메일로 보내기 내보내기 옵션 | 이제 **[!UICONTROL Send as email]** 메뉴에서 **[!UICONTROL View more]**&#x200B;을(를) 선택하여 Query Pro 모드 대시보드에서 최대 10,000개의 레코드를 내보낼 수 있습니다. 이 옵션을 사용하면 대용량 파일을 내보낼 때 Adobe에 연결된 이메일로 다운로드 링크를 안전하게 전송할 수 있습니다. 자세한 내용은 [더 보기 안내서](../../dashboards/sql-insights-query-pro-mode/view-more.md#export)를 참조하십시오. |
...
| [Facebook](../../destinations/catalog/social/facebook.md#supported-identities) 대상에 대한 추가 식별자 지원 | 이제 [!DNL Facebook] 대상은 새로운 주소 관련 필드 매핑을 지원하여 Facebook 속성의 프로필과 타기팅 및 매칭을 개선합니다. 새로운 주소 관련 필드에 대한 자세한 내용은 [지원되는 ID](../../destinations/catalog/social/facebook.md#supported-identities) 섹션을 참조하십시오. <br> ![Facebook에 대한 추가 필드를 보여 주는 플랫폼 UI 이미지입니다.](assets/june/facebook-destination-fields.png "Facebook에 대한 추가 필드를 보여 주는 플랫폼 UI 이미지입니다."){width="200" align="center" zoomable="yes"} |
| [[!DNL Braze]](../../destinations/catalog/mobile-engagement/braze.md) 대상 업그레이드 | 2025년 6월 19일부터 대상 카탈로그에 두 개의 **[!DNL Braze]** 카드가 나란히 표시됩니다. 이는 대상 서비스의 내부 업그레이드로 인한 변경 사항입니다. 기존 [!DNL Braze] 대상 커넥터의 이름이 **[!UICONTROL (Deprecated) Braze]**(으)로 변경되었으며 이름이 **[!UICONTROL Braze]**&#x200B;인 새 카드를 사용할 수 있습니다. <br> 카탈로그의 **[!UICONTROL Braze]** 연결을 사용하여 새 활성화 데이터 흐름을 만드십시오. **[!UICONTROL (Deprecated) Braze]** 대상으로의 활성 데이터 흐름이 있는 경우 해당 데이터 흐름은 자동으로 업데이트되므로 다른 조치가 필요하지 않습니다. <br> [Flow Service API](https://developer.adobe.com/experience-platform-apis/references/destinations/)를 통해 데이터 흐름을 생성하는 경우 [!DNL flow spec ID] 및 [!DNL connection spec ID]를 다음 값으로 업데이트해야 합니다. <ul><li>흐름 사양 ID: `cb7919bd-69aa-462d-bcc0-db7cdc7fdf51`</li><li>연결 사양 ID: `ab957205-5a78-4393-b901-b930ed548220`</li></ul> |
