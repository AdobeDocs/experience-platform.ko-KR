...
매핑 인터페이스에는 수집 워크플로우 컨텍스트 내의 매핑 세트 상태에 대한 정보를 제공하는 대시보드가 포함되어 있습니다. 대시보드에는 매핑 세트에 대한 다음 세부 정보가 표시됩니다.

| 속성 | 설명 |
| --- | --- |
| [!UICONTROL 매핑된 필드] | 오류에 관계없이 대상 XDM 필드에 매핑된 총 소스 필드 수를 표시합니다. |
| [!UICONTROL 필수 필드] | 필요한 매핑 필드 수를 표시합니다. |
| [!UICONTROL ID 필드] | ID로 정의된 총 매핑 세트 수를 표시합니다. 이러한 매핑 세트는 지문 아이콘으로 표시됩니다. |
| [!UICONTROL 오류] | 잘못된 매핑 집합 수를 표시합니다. |

![위쪽 패널](../images/ui/mapping/top-panel.png)

매핑 인터페이스에서는 매핑 세트를 통해 보다 효과적으로 상호 작용하거나 필터링할 수 있는 옵션 패널도 제공합니다.
...
선택 **[!UICONTROL 오류가 있는 필드]** 오류가 있는 모든 매핑 세트를 보려면 다음을 수행하십시오.

![filter](../images/ui/mapping/filter.png)

지능형 매핑 권장 사항을 통해 또는 수동 매핑 트리를 통해 오류를 처리할 수 있도록 잘못된 매핑 세트에 대한 분리된 보기가 나타납니다.
...
모든 매핑 세트를 제거하려면 **[!UICONTROL 모든 매핑 지우기]**.

![모두 지우기](../images/ui/mapping/clear-all.png)

### 매핑 인터페이스 사용

Platform은 선택한 대상 스키마나 데이터 세트를 기반으로 자동 매핑 필드에 대한 지능형 권장 사항을 자동으로 제공합니다. 매핑 규칙을 수동으로 조정하여 사용 사례에 맞게 조정하거나 중복된 매핑 세트를 수정하여 오류를 지울 수 있습니다.
...
대상 스키마 매핑 인터페이스가 매핑 세트와 동일한 보기에 나타나므로 동일한 화면 내에서 매핑 쌍을 수정할 수 있습니다. 사용 사례에 맞는 타겟 필드를 선택하거나 오류를 수정합니다.
