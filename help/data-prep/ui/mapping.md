...
>[!TIP]
>
>소스 카탈로그에서 소스를 선택하여 데이터 흐름 세부 정보에 액세스할 수 있습니다. 자세한 내용은 [소스 개요](../../sources/home.md)를 참조하십시오.

CSV 데이터를 XDM 스키마에 매핑하려면 먼저 데이터 흐름의 세부 사항을 설정해야 합니다.

[!UICONTROL 데이터 흐름 세부 정보] 페이지에서 CSV 데이터를 기존 대상 데이터 세트로 수집할지 또는 새 대상 데이터 세트로 수집할지 여부를 선택할 수 있습니다. 기존 데이터 세트에는 데이터를 매핑할 사전 빌드된 대상 스키마가 함께 제공되는 반면, 새 데이터 세트에는 데이터를 매핑할 기존 스키마를 선택하거나 새 스키마를 만들어야 합니다.
...
![파일 선택](../images/ui/mapping/choose-files.png)
...
**[!UICONTROL 매핑]** 인터페이스는 원본 스키마의 원본 필드를 대상 스키마의 해당 대상 XDM 필드에 매핑하는 포괄적인 도구를 제공합니다.
...
특정 매핑 집합을 검색하려면 **[!UICONTROL 원본 필드 검색]**&#x200B;을 선택하고 분리할 원본 데이터의 이름을 입력하십시오.

![검색](../images/ui/mapping/search.png)

매핑 인터페이스 보기 범위를 더 좁힐 수 있도록 필터링 옵션의 드롭다운 메뉴를 보려면 **[!UICONTROL 모든 소스 필드]**&#x200B;를 선택하십시오.

필터링 옵션은 다음과 같습니다.

| Source 필드 | 설명 |
| --- | --- |
| [!UICONTROL 모든 원본 필드] | 이 옵션은 소스 스키마의 모든 소스 필드를 표시합니다. 이 옵션은 기본적으로 표시됩니다. |
...
오류가 있는 모든 매핑 필드를 보려면 **[!UICONTROL 오류가 있는 필드]**&#x200B;를 선택하십시오.

![필터](../images/ui/mapping/filter.png)
...
기존 데이터 흐름의 매핑을 재사용하여 데이터 수집의 수동 구성 시간을 줄이고 실수를 제한할 수 있습니다. 기존 매핑을 재사용하려면 **[!UICONTROL 매핑 가져오기]**&#x200B;를 선택하십시오.

![가져오기 매핑](../images/ui/mapping/import-mapping.png)

선택할 데이터 흐름 목록을 제공하는 [!UICONTROL 매핑 가져오기] 창이 나타납니다.

미리 보기 아이콘을 선택하여 선택한 데이터 흐름의 매핑을 미리 봅니다.

![목록 매핑](../images/ui/mapping/list-mapping.png)

미리보기 창을 사용하면 데이터 흐름으로 가져오기 전에 기존 매핑을 검사할 수 있습니다. 매핑을 확인하면 **[!UICONTROL 뒤로]**&#x200B;를 선택하여 데이터 흐름 목록으로 돌아가서 다른 매핑 집합을 검사하거나 **[!UICONTROL 선택]**&#x200B;을 선택하여 계속할 수 있습니다.

![미리 보기 매핑](../images/ui/mapping/preview-mapping.png)

또는 데이터 흐름 목록 창에서 가져올 매핑을 선택할 수 있습니다. 가져올 매핑이 포함된 데이터 흐름을 선택한 다음 **[!UICONTROL 선택]**&#x200B;을 선택하여 계속하십시오.

![매핑 선택](../images/ui/mapping/select-mapping.png)

가져온 매핑으로 인터페이스가 업데이트됩니다.
...
선택한 데이터 집합에서 최대 100개의 샘플 데이터 행에 대한 매핑 결과를 보려면 **[!UICONTROL 데이터 미리 보기]**&#x200B;를 선택하십시오.

![미리 보기 데이터](../images/ui/mapping/preview-data.png)

미리보기 중에 ID 열은 매핑 결과의 유효성을 검사할 때 필요한 주요 정보이므로 첫 번째 필드로 우선 지정됩니다. 완료되면 **[!UICONTROL 닫기]**&#x200B;를 선택합니다.

![미리 보기 화면](../images/ui/mapping/preview-screen.png)

모든 매핑 필드를 제거하려면 **[!UICONTROL 모든 매핑 지우기]**&#x200B;를 선택하십시오.
...
[!UICONTROL 매핑 권장 사항] 팝업 패널이 나타나고 특정 소스 필드에 매핑할 수 있는 권장 대상 필드 목록이 표시됩니다. 기본적으로 첫 번째 권장 사항이 자동으로 적용됩니다.
...
또는 대화형 대상 스키마 매핑 트리를 수동으로 사용하려면 **[!UICONTROL 수동으로 선택]**&#x200B;을 선택할 수 있습니다.
...
![타겟 필드 선택](../images/ui/mapping/select-target-field.png)

완료되면 **[!UICONTROL 완료]**&#x200B;를 선택하여 계속 진행하십시오.
