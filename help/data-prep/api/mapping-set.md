...
에 GET 요청을 수행하여 조직에 대한 모든 매핑 세트 목록을 검색할 수 있습니다 `/mappingSets` 엔드포인트.
...
다음 요청은 조직 내에서 마지막 두 개의 매핑 세트를 검색합니다.
...
성공적인 응답은 매핑된 데이터의 미리 보기를 사용하여 HTTP 상태 200을 반환합니다.
...
| 매개 변수 | 설명 |
| --------- | ----------- |
| `{MAPPING_SET_ID}` | 매핑 정보를 조회하려는 매핑 세트의 ID입니다. |
