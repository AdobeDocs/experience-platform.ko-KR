...
>[!NOTE]
>
>모든 Privacy Service 및 [!UICONTROL 판매 거부] 권한은 기능상 겹치지 않으며 서로 구별되고 별개입니다. 이는 Privacy Service API가 멱등 문자로 간주되기 때문에 가능합니다.
...
이전에는 기술 계정에서 통합 및 권한을 위해 제품 프로필이 필요하지 않았습니다. 그러나 최근 Privacy Service 권한의 개선으로 인해 이제 레거시 API 자격 증명을 제품 프로필로 마이그레이션해야 합니다. 이 업데이트를 통해 기술 계정 소유자에게 세분화된 권한을 부여할 수 있습니다. 아래 제공된 단계에 따라 Privacy Service에 대한 기술 계정 권한을 업데이트합니다.

#### 기술 계정 권한 업데이트 {#update-tech-account-permissions}

기술 계정에 대한 권한 집합을 할당하는 첫 번째 단계는 [Adobe Admin Console](https://adminconsole.adobe.com/) Privacy Service을 위한 새 제품 프로필을 만들 수 있습니다.

Admin Console UI에서 를 선택합니다. **제품** 탐색 모음 뒤에 오는 **[!UICONTROL Experience Cloud]** 및 **[!UICONTROL Adobe Experience Platform Privacy Service]** 왼쪽 사이드바에서 다음 [!UICONTROL 제품 프로필] 탭이 나타납니다. 선택 **새 프로필** 을 클릭하여 Privacy Service에 대한 새 제품 프로필을 만듭니다.

![Adobe Admin Console의 Experience Platform Privacy Service 제품 프로필 탭과 새 프로필 이 강조 표시됩니다.](./images/permissions/create-product-profile.png)

다음 [!UICONTROL 새 제품 프로필 만들기] 대화 상자가 나타납니다. 제품 프로필을 만드는 방법에 대한 전체 지침은 [프로필 만들기에 대한 UI 안내서](../access-control/ui/create-profile.md).

새 제품 프로필을 저장한 후 로 이동합니다. [Adobe Developer 콘솔](https://developer.adobe.com/console/home) 해당 제품이나 프로젝트에 로그인합니다. 선택 **[!UICONTROL 프로젝트]** 맨 위 탐색에서 프로젝트 카드가 뒤에 옵니다.

>[!NOTE]
>
>캐시를 지우거나 새 프로젝트가 Developer Console 프로젝트 목록에 나타날 때까지 잠시 기다려야 할 수 있습니다.

프로젝트에 로그인한 후 다음을 선택합니다. **[!UICONTROL PRIVACY SERVICE API]** 왼쪽 사이드바에서 통합.
...
![제품 프로필 편집이 강조 표시된 Adobe Developer 콘솔의 Privacy Service API 통합 대시보드](./images/permissions/edit-product-profiles.png)

다음 [!UICONTROL API 구성] 대화 상자에는 현재 서비스에 있는 사용 가능한 제품 프로필이 표시됩니다. Admin Console에서 만든 제품 프로필과 상호 연관성이 있습니다. 사용 가능한 제품 프로필 목록에서 admin console의 기술 계정에 대해 만든 새 제품 프로필에 대한 확인란을 선택합니다. 이렇게 하면 이 기술 계정이 선택한 제품 프로필의 권한과 자동으로 연결됩니다. 선택 **[!UICONTROL 구성된 API 저장]** 설정을 확인합니다.

>[!NOTE]
>
>기술 계정이 이미 제품 프로필과 연결되어 있는 경우 사용 가능한 제품 프로필 목록의 확인란 중 하나가 이미 선택됩니다.

![Adobe Developer 콘솔의 API 구성 대화 상자에서 제품 프로필 확인란과 구성된 API 저장 이 강조 표시됩니다.](./images/permissions/select-profile-for-tech-account.png)

#### 설정이 적용되었는지 확인 {#confirm-applied-settings}

설정이 계정에 적용되었는지 확인합니다. (으)로 돌아가기 [Admin Console](https://adminconsole.adobe.com/) 새로 만든 제품 프로필로 이동합니다. 다음 항목 선택 **[!UICONTROL API 자격 증명]** 탭으로 이동하여 연결된 프로젝트 목록을 확인하십시오. 기술 계정에 제품 프로필을 할당한 Developer Console에서 사용된 프로젝트가 자격 증명 목록에 표시됩니다. 각 API 자격 증명의 이름은 임의로 생성된 번호가 끝에 붙은 프로젝트 이름으로 구성됩니다. 자격 증명을 선택하여 열기 [!UICONTROL 세부 사항] 패널.

![API 자격 증명 탭과 프로젝트 자격 증명 행이 강조 표시된 Admin Console의 제품 프로필입니다.](./images/permissions/confirm-credentials-in-admin-console.png)

다음 [!UICONTROL 세부 사항] 패널에는 관련 기술 ID, API 키, 생성 및 마지막 수정 날짜, 관련 Adobe 제품을 포함한 API 자격 증명에 대한 정보가 포함되어 있습니다.

![Admin Console 내의 API 자격 증명에 대해 강조 표시된 세부 정보 패널입니다.](./images/permissions/admin-console-details-panel.png)
