...
다음은 [!DNL Experience Cloud]과(와) 통합된 [!DNL Privacy Service] 응용 프로그램 목록이며, 호환되는 [!DNL Privacy Service] 기능, 삭제 요청 처리를 위한 프로토콜 및 자세한 내용을 보려면 설명서에 대한 링크를 포함합니다.
...
| --- | :---: | :---: | --- | --- |
| Adobe Advertising | ✓ | ✓ | 데이터 주체의 쿠키 ID 또는 장치 ID는 쿠키와 연결된 모든 비용, 클릭 및 매출 데이터와 함께 시스템에서 삭제됩니다. | <ul><li>[GDPR에 대한 액세스/삭제 설명서](https://experienceleague.adobe.com/ko/docs/advertising/privacy/gdpr)</li><li>[CCPA에 대한 액세스/삭제 설명서](https://experienceleague.adobe.com/ko/docs/advertising/privacy/ccpa/ccpa-access-delete)</li><li>[CCPA에 대한 판매 중지 설명서](https://experienceleague.adobe.com/ko/docs/advertising/privacy/ccpa/ccpa-opt-out-of-sale)</li></ul> |
| Adobe Analytics | ✓ | ✓ | Adobe Analytics은 민감도 및 계약 제한 사항에 따라 데이터에 레이블을 지정하는 도구를 제공합니다. 레이블은 다음에 대한 중요한 단계입니다.<ol><li>데이터 주체 식별.</li><li>액세스 요청의 일부로 반환할 데이터 결정</li><li>삭제 요청의 일부로 삭제해야 하는 데이터 필드 식별.</li></ol> | <ul><li>[개인 정보 보호 워크플로](https://experienceleague.adobe.com/ko/docs/analytics/technotes/privacy/gdpr)</li><li>[Analytics 레이블 지정](https://experienceleague.adobe.com/ko/docs/analytics/admin/admin-tools/privacy-labeling/labels)</li><li>[Analytics 옵트아웃](https://experienceleague.adobe.com/ko/docs/analytics/components/dimensions/cm-opt-out)</li></ul> |
| Adobe Audience Manager | ✓ | ✓ | 요청에 포함된 Audience Manager 식별자와 연관된 모든 트레이트 및 세그먼트가 삭제됩니다. 또한 개인에 대한 각 식별자가 추가 데이터 수집에서 옵트아웃되고 각 ID 매핑이 제거됩니다. | <ul><li>[액세스](https://experienceleague.adobe.com/ko/docs/audience-manager/user-guide/overview/data-privacy/data-privacy-requests#access-data) / [삭제](https://experienceleague.adobe.com/ko/docs/audience-manager/user-guide/overview/data-privacy/data-privacy-requests#delete-data) 설명서</li><li>[옵트아웃 설명서](https://experienceleague.adobe.com/ko/docs/audience-manager/user-guide/features/declared-ids#opt-out-calls)</li><li>[옵트아웃 요청](https://experienceleague.adobe.com/ko/docs/audience-manager/user-guide/overview/data-privacy/data-privacy-requests#opt-out-requests)</li></ul> |
| Adobe Campaign Classic | ✓ | ✓ | 데이터 주체의 저장된 데이터는 시스템에서 삭제됩니다. | <ul><li>[개인 정보 관리](https://experienceleague.adobe.com/ko/docs/campaign-classic/using/getting-started/privacy/privacy-management)</li></ul> |
| Adobe Campaign Standard | ✓ | ✓ | 데이터 주체의 저장된 데이터는 시스템에서 삭제됩니다. | <ul><li>[설명서 액세스/삭제](https://experienceleague.adobe.com/ko/docs/campaign-standard/using/getting-started/privacy/privacy-management#right-access-forgotten)</li><li>[옵트아웃 설명서](https://experienceleague.adobe.com/ko/docs/campaign-standard/using/profiles-and-audiences/understanding-opt-in-and-opt-out-processes/about-opt-in-and-opt-out-in-campaign)</li></ul> |
| Adobe 고객 속성(CRS) | ✓ | N/A | 데이터 주체의 속성이 시스템에서 삭제됩니다. | <ul><li>[GDPR에 대한 액세스/삭제 설명서](https://experienceleague.adobe.com/ko/docs/core-services/interface/services/customer-attributes/gdpr)</li><li>[CCPA에 대한 액세스/삭제 설명서](https://experienceleague.adobe.com/ko/docs/core-services/interface/services/customer-attributes/ccpa)</li><li>고객 속성에는 데이터를 전송할 수 있는 기능이 없으므로 판매 중지 요청을 적용할 수 없습니다.</li></ul> |
...
| Adobe Target | ✓ | N/A | 데이터 주체의 ID와 관련된 모든 데이터가 해당 방문자 프로필에서 삭제됩니다. 개인을 식별하지 않거나 관련이 없는 집계 또는 익명화 데이터(예: 콘텐츠 데이터)는 삭제 요청에 적용되지 않습니다. | <ul><li>[설명서 액세스/삭제](https://experienceleague.adobe.com/ko/docs/target-dev/developer/implementation/privacy/cmp-privacy-and-general-data-protection-regulation)</li><li>[!DNL Target]에 데이터를 전송할 수 있는 기능이 없으므로 판매 중지 요청을 적용할 수 없습니다.</li></ul> |
| Commerce(Personalization) | ✓ | N/A | Privacy Service은 마케팅 목적으로 Commerce SaaS 서비스에 저장된 [!DNL Commerce]개의 데이터를 삭제합니다. 즉, 데이터 주체의 프로필 및 순서가 더 이상 캠페인 및 고객 여정에 사용하기 위해 Adobe 마케팅 애플리케이션으로 전송되지 않습니다. 그러나 Privacy Service은 [!DNL Commerce] 응용 프로그램의 데이터를 삭제하지 않습니다. 상인 트랜잭션 요구 사항에 계속 필요할 수 있기 때문입니다. 판매자는 [!DNL Commerce] 응용 프로그램에서 데이터 삭제/액세스 요청을 담당합니다. | <ul><li>[Commerce에 대한 액세스/삭제 설명서](https://experienceleague.adobe.com/ko/docs/commerce-merchant-services/data-connection/handle-privacy-request)</li></ul> |
| Marketo Engage | ✓ | N/A | 데이터 주체의 저장된 데이터는 시스템에서 삭제됩니다. | <ul><li>[설명서 액세스/삭제](https://experienceleague.adobe.com/ko/docs/marketo/using/product-docs/core-marketo-concepts/miscellaneous/privacy-requests)</li><li>[!DNL Marketo]에 데이터를 전송할 수 있는 기능이 없으므로 판매 중지 요청을 적용할 수 없습니다.</li></ul> |

## 셀프서비스 애플리케이션 {#self-serve}

다음은 [!DNL Experience Cloud]과(와) 통합되지 않은 [!DNL Privacy Service] 응용 프로그램 목록이며 내부적으로 해당 응용 프로그램의 개인 정보 문제를 관리해야 합니다. 설명서 내용에 대한 설명과 함께 각 애플리케이션의 설명서에 대한 링크가 제공됩니다.

| 애플리케이션 | 설명서 설명 |
| ------- | ----------- |
| [Adobe Experience Manager](https://experienceleague.adobe.com/ko/docs/experience-manager-64/managing/data-protection/data-protection-and-privacy) | 고객 개인 정보 보호 관리자 또는 AEM 관리자가 GDPR 요청을 처리할 수 있는 방법에 대한 개요입니다. |
| [Adobe Commerce](https://experienceleague.adobe.com/ko/docs/commerce-operations/security-and-compliance/overview) | Adobe Commerce 설치가 특정 개인 정보 보호 규정의 요구 사항을 준수하는지 확인합니다. |
