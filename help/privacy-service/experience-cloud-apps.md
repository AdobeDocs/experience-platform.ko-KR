...
| Adobe Analytics | ✓ | ✓ | If `analyticsDeleteMethod` 생략되거나 로 설정됨 `anonymize` 개인 정보 요청을 만들 때 지정된 사용자 ID 컬렉션에서 참조하는 모든 데이터는 익명으로 수행됩니다. If `analyticsDeleteMethod` 가 로 설정되어 있습니다. `purge`이면, 모든 데이터가 완전히 제거됩니다. | <ul><li>[설명서 액세스/삭제](https://experienceleague.adobe.com/docs/analytics/admin/data-governance/an-gdpr-overview.html?lang=ko-KR)</li><li>[!DNL Analytics] 를 사용하여 옵트아웃 요청 처리 [개인 정보 보고 변수](https://experienceleague.adobe.com/docs/analytics/admin/data-governance/consent-variables.html)</li></ul> |
...
| Adobe Primetime 인증 | ✓ | 해당 없음 | 데이터 주체의 저장된 데이터가 시스템에서 삭제됩니다. | <ul><li>[설명서 액세스/삭제](http://tve.helpdocsonline.com/how-to-make-a-privacy-request)</li><li>[!DNL Primetime] 에는 데이터를 전송할 기능이 없으므로 판매 중지 요청을 적용할 수 없습니다.</li></ul> |
