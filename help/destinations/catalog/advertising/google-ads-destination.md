...
[!DNL Google Ads]이전에는 [!DNL Google AdWords], 은 텍스트 기반 검색, 그래픽 디스플레이, [!DNL YouTube] 비디오 및 인앱 모바일 디스플레이.

## 대상 세부 사항 {#specifics}

에 해당하는 다음 세부 사항을 참고하십시오 [!DNL Google Ads] 대상:
...
### 기존 [!DNL Google Ads] account

>[!IMPORTANT]
>
> [!DNL Google] 새로 사용되지 않음 [!DNL Google Ads] 타사 공급업체와의 쿠키 통합. 다음 섹션에서 허용 목록 단계를 수행하려면 와 기존 통합이 있어야 합니다. [!DNL Google Ads]. 그 결과, 사용에 대한 권장 접근 방법이 사용됩니다 [!DNL Google Ads] 설정 중 [!DNL Google Customer Match] 통합. 만들기 [!DNL Google Customer Match] 통합, 만들기 관련 자습서를 참조하십시오 [[!DNL Google Customer Match]](./google-customer-match.md) 연결.
...
* 계정 유형: **AdWords**
* **Google AdWords ID**: ID가 [!DNL Google]. ID 형식은 일반적으로 123-456-7890.
...
데이터를 로 성공적으로 내보냈는지 확인하려면 [!DNL Google Ads] 대상, [!DNL Google Ads] 계정이 필요합니다. 활성화가 성공하면 계정에 대상이 채워집니다.
...
이 오류는 고객 계정이 [전제 조건](#prerequisites) 또는 고객이 기존 [!DNL Google Ads] 계정이 필요합니다.

[!DNL Google] 새로 사용되지 않음 [!DNL Google Ads] 타사 공급업체와의 쿠키 통합. 다음을 수행하려면 다음을 수행하십시오. [허용 목록](#allow-listing) 단계에는 [!DNL Google Ads].

사용에 대한 권장 접근 방법 [!DNL Google Ads] 설정 중 [[!DNL Google Customer Match]](google-customer-match.md) 통합.
