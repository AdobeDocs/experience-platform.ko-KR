...
이전에 DFP([!DNL Google Ad Manager]) 또는 [!DNL DoubleClick for Publishers]&#x200B;(으)로 알려졌던 [!DNL DoubleClick AdX]은(는) 게시자에게 웹 사이트, 비디오 및 모바일 앱에서 광고 표시를 관리할 수 있는 수단을 제공하는 [!DNL Google]의 광고 제공 플랫폼입니다.
...
* [!DNL Experience Platform]은(는) 현재 성공적인 활성화를 확인하기 위한 측정 지표를 포함하지 않습니다. 통합의 유효성을 검사하고 대상 타깃팅 크기를 이해하려면 Google의 대상자 규모 를 참조하십시오.
* 대상을 [!DNL Google Ad Manager] 대상에 매핑하면 대상 이름이 [!DNL Google Ad Manager] 사용자 인터페이스에 바로 나타납니다.
* [!DNL Google Ad Manager]에 세그먼트 모집단을 표시하려면 24-48시간이 필요합니다. 또한 대상자는 [!DNL Google Ad Manager]에 표시할 대상자 크기가 최소 50개 이상이어야 합니다. 크기가 50개 프로필보다 작은 대상은 [!DNL Google Ad Manager]에서 채워지지 않습니다.
...
| 항목 | 유형 | 참고 |
|---------|----------|---------|
| 내보내기 유형 | **[!UICONTROL Audience export]** | 대상의 모든 구성원을 Google 대상으로 내보내고 있습니다. |
...
* **[!UICONTROL Account Type]**: Google 계정에 따라 옵션을 선택하십시오.
   * 게시자에 대해 `DFP by Google`에 [!DNL DoubleClick] 사용
   * `AdX buyer`에 [!DNL Google AdX] 사용
* **[!UICONTROL Append audience ID to audience name]**: Google Ad Manager의 대상 이름에 다음과 같이 Experience Platform의 대상 ID가 포함되도록 하려면 이 옵션을 선택하십시오. `Audience Name (Audience ID)`.
