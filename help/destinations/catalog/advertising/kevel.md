...
[!DNL Kevel]의 [!DNL Adobe Experience Platform] 스트리밍 대상을 사용하면 고객이 [!DNL Kevel]의 UserDB 및 세그먼트 관리 API로 직접 Adobe 대상을 활성화하여 광고 결정 시 실시간 타깃팅을 지원할 수 있습니다.
...
### 사용자 정의 ID 네임스페이스 지원 {#custom-identity-namespaces}
...
### ID 매핑 동작 {#identity-mapping-behavior}
...
대상자를 [!DNL Kevel]에 보내려면 [스트리밍 대상으로 대상자 활성화](/help/destinations/ui/activate-segment-streaming-destinations.md)의 워크플로를 따르십시오.
...
### [!DNL Kevel] UserDB에서 받은 샘플 페이로드 {#sample-payload}
...
#### [!DNL Kevel]에서 이 프로필을 해석하는 방법 {#kevel-profile-interpretation}
