...
description: 이 커넥터를 사용하여 마케팅 채널 전반에서 데이터 보강 및 사용을 위해 Real-Time CDP에서 Acxiom으로 자사 Adobe 프로필을 활성화합니다.
...
분석, 세그먼테이션 및 타기팅 애플리케이션에서 사용할 수 있도록 Acxiom 데이터 개선 커넥터를 사용하여 Adobe 프로필에 추가 설명 데이터를 제공하십시오. 수백 개의 요소를 사용할 수 있으므로 사용자는 데이터를 더 잘 세그먼트화하고 모델링할 수 있으므로 더 정확한 타겟팅 및 예측 모델링을 수행할 수 있습니다.
...
Acxiom 데이터 개선 사항 대상을 사용하는 방법과 시기를 더 잘 이해할 수 있도록 Adobe Experience Platform 고객이 이 대상을 사용하여 해결할 수 있는 샘플 사용 사례를 소개합니다.

### 고객 데이터 향상 {#enhance-customer-data}

이 커넥터는 선택한 설명 요소를 Adobe 프로필에 추가하여 도달 전략의 효과를 높이고 이를 사용하여 캠페인을 더 잘 타기팅하는 것을 목표로 하는 마케팅 전문가가 사용해야 합니다.
...
그런 다음 해당 Acxiom 데이터 섭취 소스 카드를 사용하여 하이드레이션된 고객 프로필을 다시 Adobe Real-Time CDP으로 수집합니다.
...
Acxiom 데이터 개선 카드를 사용하여 이미 정의된 계정은 목록 팝업에 표시되며, 선택되면 계정에 대한 세부 정보가 제공됩니다.  이 아이콘은 로 이동하면 UI의 아래 예제에 표시됩니다. **대상** > **계정**;
