...
[](https://www.oracle.com/cx/marketing/campaign-management/) 응답이란 이메일, 모바일, 디스플레이 및 소셜 [!DNL Oracle] 에서 상호 작용을 개인화하기 위해에서 제공하는 크로스 채널 마케팅 캠페인을 위한 엔터프라이즈 이메일 마케팅 도구입니다.

세그먼트 데이터를 [!DNL Oracle Responsys]에 보내려면 먼저 [을 Adobe Experience Platform의 대상](#connect-destination)에 연결한 다음, [저장소 위치에서 ](#import-data-into-responsys)로 데이터 가져오기[!DNL Oracle Responsys]를 설정해야 합니다.
...
[!DNL Platform]을 [!DNL SFTP] 저장소에 연결한 후 저장소 위치에서 [!DNL Oracle Responsys]로 데이터 가져오기를 설정해야 합니다. 이를 수행하는 방법에 대한 자세한 내용은 [!DNL Oracle Responsys Help Center]에서 [연락처 또는 계정 가져오기](https://docs.oracle.com/cloud/latest/marketingcs_gs/OMCEA/Connect_WizardUpload.htm)를 참조하십시오.
