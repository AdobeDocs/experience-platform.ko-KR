...
# [!DNL Adobe Campaign Managed Cloud Services] 연결 {#adobe-campaign-managed-services}

>[!IMPORTANT]
>
>이 통합은 [[!DNL Adobe Campaign] 버전 8.4 이상](https://experienceleague.adobe.com/docs/campaign/campaign-v8/new/release-notes.html?lang=ko#release-8-4-1)에서 작동합니다.

## 개요 {#overview}

[!DNL Adobe Campaign Managed Cloud Services]은(는) 크로스 채널 고객 경험을 디자인할 수 있는 플랫폼을 제공하며 시각적 캠페인 오케스트레이션, 실시간 상호 작용 관리 및 크로스 채널 실행을 위한 환경을 제공합니다. [Campaign 시작](https://experienceleague.adobe.com/docs/campaign/campaign-v8/start/get-started.html?lang=ko)
...
[!DNL Adobe Campaign Managed Cloud Services] 연결을 사용할 때는 다음 보호 기능에 유의하십시오.
...
* 각 대상자에 대해 [맵](#map)에서 [!DNL Adobe Campaign]까지 최대 20개의 필드를 추가할 수 있습니다.

  Campaign 탐색기의 **>** > **[!UICONTROL Administration]** 폴더에서 **[!UICONTROL Platform]** NmsCdp_Aep_Destinations_Max_Columns **[!UICONTROL Options]** 옵션의 값을 업데이트하여 이 제한을 변경할 수 있습니다.
...
* 대상을 `&`(으)로 내보낼 때 특수 문자(예: [!DNL Adobe Campaign])가 포함된 세그먼트/대상은 지원되지 않습니다.

## 사용 사례 {#use-cases}

[!DNL Adobe Campaign] 서비스 관리 대상을 사용하는 방법과 시기를 더 잘 이해할 수 있도록 [!DNL Adobe Experience Platform] 고객이 이 대상을 사용하여 해결할 수 있는 사용 사례 예제를 소개합니다.

* [!DNL Adobe Experience Platform]은(는) id 그래프, analytics의 동작 데이터, 오프라인 및 온라인 데이터 병합 등과 같은 정보를 통합하는 고객 프로필을 만듭니다. 이 통합을 통해 [!DNL Adobe Campaign] 기반 대상을 사용하여 [!DNL Adobe Experience Platform] 내에 이미 존재하는 세분화 기능을 늘릴 수 있으므로 Campaign에서 해당 데이터를 활성화할 수 있습니다.

  예를 들어 스포츠 의류 회사는 [!DNL Adobe Experience Platform]에서 지원하는 다양한 채널에서 고객 기반에 연락하기 위해 [!DNL Adobe Campaign]명의 자율 대상자를 활용하고 [!DNL Adobe Campaign]을(를) 사용하여 활성화하려고 합니다. 메시지를 보낸 후에는 전송, 열기 및 클릭과 같은 [!DNL Adobe Experience Platform]의 경험 데이터로 [!DNL Adobe Campaign]의 고객 프로필을 개선하려고 합니다.

  그 결과, Adobe Experience Cloud 생태계 전반에서 보다 일관적인 크로스 채널 캠페인과 다양한 고객 프로필이 적용되어 빠르게 학습할 수 있습니다.


* Campaign에서 대상을 활성화할 수 있을 뿐만 아니라, [!DNL Adobe Campaign Managed Services] 대상을 활용하여 [!DNL Adobe Experience Platform]의 프로필에 연결된 추가 프로필 특성을 가져올 수 있고 동기화 프로세스를 통해 [!DNL Adobe Campaign] 데이터베이스에서 업데이트되도록 할 수 있습니다.

  예를 들어 [!DNL Adobe Experience Platform]에서 옵트인 및 옵트아웃 값을 캡처하고 있다고 가정해 보겠습니다. 이 연결을 통해 이러한 값을 [!DNL Adobe Campaign]&#x200B;(으)로 가져와서 정기적으로 업데이트되도록 동기화 프로세스를 수행할 수 있습니다.

  >[!NOTE]
  >
  >[!DNL Adobe Campaign] 데이터베이스에 이미 있는 프로필에 대해 프로필 특성 동기화를 사용할 수 있습니다.

[자세히 알아보기 [!DNL Adobe Campaign] 통합 [!DNL Adobe Experience Platform]](https://experienceleague.adobe.com/docs/campaign/campaign-v8/connect/ac-aep.html?lang=ko)

## 지원되는 ID {#supported-identities}

*[!DNL Adobe Campaign Managed Cloud Services]*&#x200B;은(는) 아래 표에 설명된 ID 활성화를 지원합니다. [ID](/help/identity-service/features/namespaces.md)에 대해 자세히 알아보세요.
...
| 항목 | 유형 | 참고 |
|---------|----------|---------|
| 내보내기 유형 | **[!UICONTROL Profile-based]** | [대상 활성화 워크플로](/help/destinations/ui/activate-batch-profile-destinations.md#select-attributes)의 프로필 특성 선택 화면에서 선택한 대로 원하는 스키마 필드(예: 이메일 주소, 전화번호, 성)와 함께 대상자의 모든 구성원을 내보냅니다. |
...
![[!DNL Adobe Campaign Managed Cloud Services] 대상 세부 정보 양식에서 이름, 설명, 인스턴스 선택, 대상 매핑 및 동기화 유형에 대한 필드를 표시합니다.](../../assets/catalog/email-marketing/adobe-campaign-managed-services/destination-details.png)
...
* **[!UICONTROL Select sync type]**:
   ...
   * **[!UICONTROL Audience sync]**: 이 옵션을 사용하여 [!DNL Adobe Experience Platform] 대상자를 [!DNL Adobe Campaign]&#x200B;(으)로 보냅니다.
   * **[!UICONTROL Profile sync (Update only)]**: 이 옵션을 사용하여 [!DNL Adobe Experience Platform] 프로필 특성을 [!DNL Adobe Campaign]&#x200B;(으)로 가져오고 정기적으로 업데이트할 수 있도록 동기화 프로세스를 준비하십시오.
...
대상으로 내보낼 데이터에 적용할 수 있는 마케팅 작업을 선택합니다. [!DNL Adobe Campaign]의 경우 **[!UICONTROL Email Targeting]** 마케팅 작업을 선택하는 것이 좋습니다.
...
프로필로 내보낼 XDM 필드를 선택하고 해당 [!DNL Adobe Campaign] 필드에 매핑합니다.[이메일 마케팅 대상의 ID 및 특성 선택에 대해 자세히 알아보기](overview.md)

1. 소스 필드 선택:

   * **및**&#x200B;에서 프로필을 고유하게 식별하는 원본 ID로 [!DNL Adobe Experience Platform]식별자[!DNL Adobe Campaign]&#x200B;(예: 전자 메일 필드)를 선택하십시오.

   * **(으)로 내보내야 하는 다른 모든** XDM 소스 프로필 특성[!DNL Adobe Campaign]을(를) 선택하십시오.

   >[!NOTE]
   >
   >segmentMembershipStatus 필드는 segmentMembership 상태를 반영하는 필수 매핑입니다. 이 필드는 기본적으로 추가되며 수정하거나 제거할 수 없습니다.

1. 각 필드를 [!DNL Adobe Campaign]의 대상 필드와 매핑합니다. 사용 가능한 대상 필드는 [대상을 만들 때](#destination-details) 선택한 대상 매핑에 의해 결정됩니다.
...
**[!UICONTROL Administration]**&#x200B;에서 활성화된 모든 내보내기 작업을 모니터링하려면 **[!UICONTROL Audit]** > **[!UICONTROL Audience load jobs]** > [!DNL Adobe Experience Platform] 메뉴로 이동합니다.

![[!DNL Adobe Campaign]에서 활성화된 내보내기 작업을 보여 주는 [!DNL Adobe Experience Platform]개의 대상 로드 작업 화면입니다.](../../assets/catalog/email-marketing/adobe-campaign-managed-services/campaign-jobs.png)
...
프로필 및 타겟에서 사용 가능한 Experience Platform의 내보낸 대상을 표시하는 ![[!DNL Adobe Campaign] AEP 대상 목록 보기.](../../assets/catalog/email-marketing/adobe-campaign-managed-services/campaign-audiences.png)
