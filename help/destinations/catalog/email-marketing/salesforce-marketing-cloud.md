...
세그먼트 데이터를 (으)로 보내기 [!DNL Salesforce Marketing Cloud], 먼저 다음을 수행해야 합니다. [대상 연결](#connect-destination) Platform에서 [데이터 가져오기 설정](#import-data-into-salesforce) 스토리지 위치에서 로 [!DNL Salesforce Marketing Cloud].
...
대상 [!DNL Salesforce Marketing Cloud] 대상, 플랫폼에서 다음을 생성합니다. `.csv` 파일을 제공한 저장소 위치에 있습니다. 파일에 대한 자세한 내용은 [세그먼트 활성화 확인](../../ui/activate-batch-profile-destinations.md#verify) 세그먼트 활성화 자습서에서 참조하십시오.
