...
[[!DNL Oracle Eloqua]](https://www.oracle.com/marketingcloud/products/marketing-automation/) 은 B2B 마케터 및 조직이 마케팅 캠페인 및 영업 리드 생성을 관리하는 데 도움이 되는 마케팅 자동화를 위한 서비스(SaaS) 플랫폼 [!DNL Oracle] 의 소프트웨어입니다.

세그먼트 데이터 [!DNL Oracle Eloqua]를 전송하려면 먼저 실시간 고객 데이터 [플랫폼에서 대상을](#connect-destination) [연결한 다음 스토리지 위치에서 (으)로 데이터 가져오기](#import-data-into-eloqua) 를 [!DNL Oracle Eloqua]설정해야 합니다.
...
[ **[!UICONTROL 연결]** ] > [대상 ****]에서 [!DNL Oracle Eloqua]선택한 다음 **[!UICONTROL Connect 대상을 선택합니다]**.

[Furnar에 연결](../../assets/catalog/email-marketing/oracle-eloqua/catalog.png)

이전에 클라우드 스토리지 대상에 대한 연결을 **[!UICONTROL 설정한 경우 인증]** 단계에서 **[!UICONTROL 기존 계정을]** 선택하고 기존 연결 중 하나를 선택합니다. 또는 새 계정 **[!UICONTROL 을]** 선택하여 새 연결을 설정할 수 있습니다. 계정 인증 자격 증명을 입력하고 대상에 **[!UICONTROL 연결을 선택합니다]**. 예를 [!DNL Oracle Eloqua]들어 암호를 사용하는 **[!UICONTROL SFTP와 SSH 키를 사용하는]** SFTP 중에서 선택할 수 있습니다 ****. 연결 유형에 따라 아래 정보를 입력하고 대상에 **[!UICONTROL 연결을 선택합니다]**.

암호 **[!UICONTROL 가]** 연결된 SFTP의 경우 도메인, 포트, 사용자 이름 및 암호를 제공해야 합니다.
SSH 키 **** 연결이 있는 SFTP의 경우 도메인, 포트, 사용자 이름 및 SSH 키를 제공해야 합니다.

![Furnar 마법사 설정](../../assets/catalog/email-marketing/oracle-eloqua/account-info.png)
...
위 필드 **[!UICONTROL 를 채운 후 대상]** 만들기를 클릭합니다. 이제 대상이 만들어지고 대상에 대한 세그먼트를 [활성화할](../../ui/activate-destinations.md) 수 있습니다.
...
세그먼트를 대상에 [활성화할](../../ui/activate-destinations.md) 때 [!DNL Oracle Eloqua] 조합 스키마에서 고유 식별자를 선택하는 것이 [좋습니다](../../../profile/home.md#profile-fragments-and-union-schemas). 대상으로 내보낼 고유 식별자 및 기타 XDM 필드를 선택합니다. 자세한 내용은 이메일 [마케팅 대상의 내보낸 파일에서](./overview.md#destination-attributes) 대상 속성으로 사용할 스키마 필드 선택을 참조하십시오.
