...
>[!IMPORTANT]
>
>**[!UICONTROL Marketo Engage Person Sync]** 대상 카드는 **2025년 10월**&#x200B;에 사용되지 않습니다.
>
...
| 대상 ID | 설명 |
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 이메일 | 이메일 주소를 나타내는 네임스페이스입니다. 이 유형의 네임스페이스는 종종 단일 사용자와 연결되므로 서로 다른 채널에서 해당 사용자를 식별합니다. |
...
| 대상자 원본 | 지원됨 | 설명 |
| -------------------- | :-------: | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Segmentation Service | 예 | Experience Platform [세그먼테이션 서비스](https://experienceleague.adobe.com/ko/docs/experience-platform/segmentation/home)를 통해 생성된 대상입니다. |
...
회사에 여러 조직에 대한 액세스 권한이 있는 경우 Marketo Engage 및 [!DNL Real-Time CDP] 모두에서 동일한 조직을 사용하는지 확인하십시오. 여기서 대상 커넥터를 Marketo에 설정합니다.  이미 대상을 구성한 경우 새 구성에 사용할 기존 Marketo 계정을 선택할 수 있습니다.  그렇지 않은 경우 대상 커넥터 프롬프트를 클릭하여 원하는 대상의 이름, 설명 및 Marketo Munchkin ID를 설정합니다.  Marketo 인스턴스의 Munchkin ID는 관리->Munchkin 메뉴에서 찾을 수 있습니다.
...
* **[!UICONTROL Partition]**: 비즈니스 관심사별로 잠재 고객 레코드를 구분하는 데 사용되는 Marketo Engage의 개념
* **[!UICONTROL First searchable field]**: 중복을 제거할 필드입니다. 필드는 입력의 각 리드 레코드에 있어야 합니다. 기본값은 이메일입니다
* **[!UICONTROL First searchable field]**: 중복을 제거할 보조 필드입니다. 필드는 입력의 각 리드 레코드에 있어야 합니다. 선택 사항입니다
...
특정 사용자 특성에 대한 변경 내용을 Marketo Engage으로 보내려면 필드를 [!DNL Real-Time CDP] 필드에서 Marketo 필드로 매핑해야 합니다.
...
어떤 경우에는 통합에서 값이 이미 있는 필드에 대한 업데이트를 금지하면서 값이 없는 경우 필드 값을 설정하도록 허용하는 것이 좋습니다.  대상 커넥터가 Marketo Engage 인스턴스의 기존 값을 덮어쓰지 않도록 해야 하는 경우 Marketo 인스턴스의 관리->필드 관리 섹션에서 업데이트를 차단하고 [!DNL Adobe Experience Platform] 소스 유형을 전환하도록 필드를 구성할 수 있습니다.
