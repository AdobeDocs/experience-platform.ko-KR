...
프로필 데이터를 HTTP 끝점으로 보내려면 먼저 Experience Platform에서 [대상에 연결](#connect-destination)해야 합니다.

## 사용 사례 {#use-cases}

HTTP API 대상을 사용하여 XDM 프로필 데이터 및 대상을 일반 HTTP 종단점으로 내보냅니다. 여기에서 자체 분석을 실행하거나 Experience Platform에서 내보낸 프로필 데이터에 대해 필요할 수 있는 다른 작업을 수행할 수 있습니다.
...
| 항목 | 유형 | 참고 |
| ---------|----------|---------|
| 내보내기 유형 | **[!UICONTROL Profile-based]** | [대상 활성화 워크플로](../../ui/activate-segment-streaming-destinations.md#mapping)의 매핑 화면에서 선택한 대로 원하는 스키마 필드(예: 이메일 주소, 전화번호, 성)와 함께 대상자의 모든 구성원을 내보냅니다. |
...
* mTLS를 사용하려는 경우: 데이터 수신 끝점에 TLS가 비활성화되어 있고 mTLS만 활성화되어야 합니다.
...
[!DNL Mutual Transport Layer Security]&#x200B;(mTLS)를 사용하여 HTTP API 대상 연결에 대한 아웃바운드 연결에서 보안을 강화할 수 있습니다.

mTLS는 데이터를 공유하기 전에 정보를 공유하는 양쪽이 자신의 소유자임을 보장하는 상호 인증 프로토콜입니다. mTLS에는 표준 TLS와 비교하여 추가 단계가 포함되어 있습니다. 서버에서 클라이언트의 인증서를 요청하고 확인하는 반면 클라이언트는 서버의 인증서를 확인합니다.
...
추가 타사 유효성 검사를 위해 CN(일반 이름) 및 SAN(주체 대체 이름)과 같은 인증서 세부 정보를 검사하려면 API를 사용하여 인증서를 검색하고 응답에서 해당 필드를 추출하십시오.

자세한 내용은 [공개 인증서 끝점 설명서](../../../data-governance/mtls-api/public-certificate-endpoint.md)를 참조하세요.

## IP 주소 {#ip-address-allowlist}

고객 보안 및 규정 준수 요구 사항을 충족하기 위해 Experience Platform에서는 HTTP API 대상에 대해 허용 목록에 추가하다할 수 있는 정적 IP 목록을 제공합니다. 스트리밍 대상에 대한 [IP 주소](/help/destinations/catalog/streaming/ip-address-allow-list.md)의 전체 IP 목록(허용 목록에 추가하다)을 참조하십시오.
...
HTTP 끝점에 연결할 **[!UICONTROL Bearer token]** 인증 유형을 선택한 경우 아래에 정보를 입력하고 **[!UICONTROL Connect to destination]**&#x200B;을(를) 선택하십시오.

![필드 [!UICONTROL Bearer token]이(가) 있는 HTTP API 인증 화면입니다.](../../assets/catalog/http/http-api-authentication-bearer.png)

* **[!UICONTROL Bearer token]**: HTTP 위치에 인증할 전달자 토큰을 입력하십시오.

#### 인증 없음 {#no-authentication}

HTTP 끝점에 연결할 **[!UICONTROL None]** 인증 유형을 선택하는 경우:

![[!UICONTROL None] 인증 유형을 선택한 HTTP API 인증 화면입니다.](../../assets/catalog/http/http-api-authentication-none.png)

이 인증 옵션을 선택하면 **[!UICONTROL Connect to destination]**&#x200B;만 선택하면 끝점에 연결됩니다.

#### OAuth 2 암호 인증 {#oauth-2-password-authentication}

HTTP 끝점에 연결할 **[!UICONTROL OAuth 2 Password]** 인증 유형을 선택한 경우 아래에 정보를 입력하고 **[!UICONTROL Connect to destination]**&#x200B;을(를) 선택하십시오.

![필드가 [!UICONTROL OAuth 2 Password]인 HTTP API 인증 화면입니다.](../../assets/catalog/http/http-api-authentication-oauth2-password.png)

* **[!UICONTROL Access Token URL]**: 액세스 토큰 및 선택적으로 새로 고침 토큰을 발급하는 사용자 측의 URL입니다.
* **[!UICONTROL Client ID]**: 시스템이 Adobe Experience Platform에 할당하는 `client ID`입니다.
* **[!UICONTROL Client Secret]**: 시스템이 Adobe Experience Platform에 할당하는 `client secret`입니다.
* **[!UICONTROL Username]**: HTTP 끝점에 액세스하기 위한 사용자 이름.
* **[!UICONTROL Password]**: HTTP 끝점에 액세스하기 위한 암호입니다.

#### OAuth 2 클라이언트 자격 증명 인증 {#oauth-2-client-credentials-authentication}

HTTP 끝점에 연결할 **[!UICONTROL OAuth 2 Client Credentials]** 인증 유형을 선택한 경우 아래에 정보를 입력하고 **[!UICONTROL Connect to destination]**&#x200B;을(를) 선택하십시오.

![필드가 [!UICONTROL OAuth 2 Client Credentials]인 HTTP API 인증 화면입니다.](../../assets/catalog/http/http-api-authentication-oauth2-client-credentials.png)

>[!WARNING]
>
>[!UICONTROL OAuth 2 Client Credentials] 인증을 사용할 때 [!UICONTROL Access Token URL]에는 최대 하나의 쿼리 매개 변수를 사용할 수 있습니다. 쿼리 매개 변수가 더 많은 [!UICONTROL Access Token URL]을(를) 추가하면 끝점에 연결할 때 문제가 발생할 수 있습니다.

* **[!UICONTROL Access Token URL]**: 액세스 토큰 및 선택적으로 새로 고침 토큰을 발급하는 사용자 측의 URL입니다.
* **[!UICONTROL Client ID]**: 시스템이 Adobe Experience Platform에 할당하는 `client ID`입니다.
* **[!UICONTROL Client Secret]**: 시스템이 Adobe Experience Platform에 할당하는 `client secret`입니다.
* **[!UICONTROL Client Credentials Type]**: 끝점에서 지원하는 OAuth 2 클라이언트 자격 증명 부여 유형을 선택하십시오.
   * **[!UICONTROL Body Form Encoded]**: 이 경우 `client ID` 및 `client secret`은(는) 대상으로 전송된 요청의 본문에 *포함됩니다*. 예를들어 [지원되는 인증 형식](#supported-authentication-types) 섹션을 참조하십시오.
   * **[!UICONTROL Basic Authorization]**: 이 경우 `client ID` 및 `client secret`은(는) base64로 인코딩되어 대상으로 전송된 후 *헤더에 `Authorization`포함됩니다*. 예를들어 [지원되는 인증 형식](#supported-authentication-types) 섹션을 참조하십시오.
...
>title="HTTP 엔드포인트"
>abstract="프로필 데이터를 전송할 HTTP 엔드포인트의 URL입니다. 데이터 수신 엔드포인트이며 구성된 경우 mTLS를 지원합니다."
...
![완료된 필드가 있는 HTTP API 대상 세부 정보 화면입니다.](../../assets/catalog/http/http-api-destination-details.png)
...
* **[!UICONTROL Include Segment Names]**: 내보내는 대상의 이름을 데이터 내보내기에 포함하려면 전환합니다. **참고**: 대상 이름은 대상에 매핑된 대상에 대해서만 포함됩니다. 내보내기에 나타나는 매핑되지 않은 대상에는 `name` 필드가 포함되지 않습니다. 이 옵션을 선택한 데이터 내보내기의 예는 아래의 [내보낸 데이터](#exported-data) 섹션을 참조하십시오.
...
[[!UICONTROL Select attributes]](../../ui/activate-streaming-profile-destinations.md#select-attributes) 단계에서는 [유니온 스키마](../../../profile/home.md#profile-fragments-and-union-schemas)에서 고유 식별자를 선택하는 것이 좋습니다. 대상으로 내보낼 고유 식별자 및 기타 XDM 필드를 선택합니다.
...
* 프로필 업데이트는 [ID 맵](/help/xdm/field-groups/profile/identitymap.md)의 변경 내용으로 결정됩니다. 예를 들어 대상에 매핑된 대상자 중 하나에 대해 이미 자격이 있는 프로필의 ID 맵 속성에 새 ID가 추가되었습니다.
* 프로필 업데이트는 대상에 매핑된 속성 중 하나 이상에 대한 속성 변경에 의해 결정되었습니다. 예를 들어 매핑 단계에서 대상에 매핑된 속성 중 하나가 프로필에 추가됩니다.

위에서 설명한 모든 경우에 관련 업데이트가 발생한 프로필만 대상으로 내보냅니다. 예를 들어 대상 플로우에 매핑된 대상자에 100명의 멤버가 있고 5개의 새 프로필이 대상자에 적합한 경우 대상으로의 내보내기는 증분 것이며 5개의 새 프로필만 포함합니다.

>[!NOTE]
>
>변경 내용이 있는 위치에 관계없이 프로필에 대해 매핑된 모든 속성을 내보냅니다. 따라서 위의 예에서는 속성 자체가 변경되지 않았더라도 이러한 5개의 새 프로필에 대해 매핑된 모든 속성을 내보냅니다.
...
|---------|----------|
| <ul><li>매핑된 속성 및 대상은 대상 내보내기에 대한 큐 역할을 합니다. 즉, 프로필의 `segmentMembership` 상태가 `realized` 또는 `exiting`(으)로 변경되거나 매핑된 특성이 업데이트되면 대상 내보내기가 시작됩니다.</li><li>ID는 현재 HTTP API 대상에 매핑될 수 없으므로 주어진 프로필의 ID를 변경하면 대상 내보내기도 결정됩니다.</li><li>속성에 대한 변경 사항은 동일한 값인지 여부에 관계없이 속성에 대한 모든 업데이트로 정의됩니다. 즉, 값 자체가 변경되지 않았더라도 속성에 대한 덮어쓰기를 변경 사항으로 간주합니다.</li></ul> | <ul><li>`segmentMembership` 개체에는 활성화 데이터 흐름에서 매핑된 대상이 포함되어 있습니다. 이 경우 자격 또는 대상 종료 이벤트 후 프로필의 상태가 변경되었습니다. 정규화된 프로필의 매핑되지 않은 다른 대상자가 활성화 데이터 흐름에서 매핑된 대상자와 동일한 [병합 정책](/help/profile/merge-policies/overview.md)에 속하는 경우 대상 내보내기의 일부가 될 수 있습니다. <br> **중요**: **[!UICONTROL Include Segment Names]** 옵션을 활성화하면 대상에 매핑된 대상에 대해서만 세그먼트 이름이 포함됩니다. 옵션이 활성화된 경우에도 내보내기에 나타나는 매핑되지 않은 대상에는 `name` 필드가 포함되지 않습니다. </li><li>`identityMap` 개체의 모든 ID도 포함됩니다. 현재 Experience Platform에서는 HTTP API 대상에서 ID 매핑을 지원하지 않습니다.</li><li>매핑된 속성만 대상 내보내기에 포함됩니다.</li></ul> |
...
프로필이 *3개의 매핑된 대상자 중 하나를 사용하거나 종료하면 대상으로 프로필 내보내기가 트리거됩니다*. 데이터 내보내기에서 `segmentMembership` 개체(아래 [내보낸 데이터](#exported-data) 참조)에 매핑되지 않은 대상자도 포함될 수 있습니다. 해당 프로필이 해당 대상자의 구성원이고 이 대상자가 내보내기를 트리거한 대상자와 동일한 병합 정책을 공유하는 경우 마찬가지입니다. 예를 들어, 프로필이 **Customer with DeLorean Cars** 대상자를 대상으로 하지만 **Watched &quot;Back to the Future&quot;** 영화 및 **Science fans** 대상자의 멤버인 경우, 해당 두 대상자는 `segmentMembership`Customer with DeLorean Cars **대상자와 동일한 병합 정책을 공유하는 경우** 개체에도 표시됩니다.
...
내보낸 Experience Platform 데이터는 HTTP 대상에 JSON 형식으로 도착합니다. 예를 들어, 아래 내보내기에는 특정 대상에 대해 자격이 있고 다른 두 대상의 멤버이며 다른 대상을 종료한 프로필이 포함됩니다. 내보내기에는 프로필 속성 이름, 성, 생년월일 및 개인 이메일 주소도 포함됩니다. 이 프로필의 ID는 ECID와 이메일입니다.
...
>[!NOTE]
>
>이 예제에서 첫 번째 대상(`5b998cb9-9488-4ec3-8d95-fa8338ced490`)은 대상에 매핑되고 `name` 필드를 포함합니다. 두 번째 대상(`354e086f-2e11-49a2-9e39-e5d9a76be683`)은 대상에 매핑되지 않으며 `name` 옵션이 활성화되어 있더라도 **[!UICONTROL Include Segment Names]** 필드를 포함하지 않습니다.
...
시간의 95%를 차지하는 Experience Platform은 HTTP 대상에 대한 각 데이터 흐름의 초당 요청 수가 10,000개 미만인 상태로 성공적으로 전송된 메시지에 대해 10분 미만의 처리량 지연 시간을 제공하려고 합니다.

HTTP API 대상에 대한 요청이 실패하면 Experience Platform은 이 요청을 저장하고 두 번 다시 시도합니다.
