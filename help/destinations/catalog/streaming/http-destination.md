...
| 항목 | 유형 | 참고 |
| ---------|----------|---------|
| 내보내기 유형 | **[!UICONTROL Profile-based]** | [대상 활성화 워크플로](../../ui/activate-segment-streaming-destinations.md#mapping)의 매핑 화면에서 선택한 대로 원하는 스키마 필드(예: 이메일 주소, 전화번호, 성)와 함께 세그먼트의 모든 구성원을 내보냅니다. |
...
[!DNL mTLS]은(는) 데이터를 공유하기 전에 정보를 공유하는 양쪽이 자신의 소유자임을 확인하는 상호 인증 프로토콜입니다. [!DNL mTLS]은(는) 표준 [!DNL TLS]과(와) 비교하여 추가 단계를 포함합니다. 이 단계에서 서버는 클라이언트의 인증서를 요청하고 확인하는 반면 클라이언트는 서버의 인증서를 확인합니다.
...
HTTP 끝점에 연결할 **[!UICONTROL Bearer token]** 인증 유형을 선택한 경우 아래 필드를 입력하고 **[!UICONTROL Connect to destination]**&#x200B;을(를) 선택하십시오.

![전달자 토큰 인증을 사용하여 HTTP API 대상에 연결할 수 있는 UI 화면의 이미지](../../assets/catalog/http/http-api-authentication-bearer.png)

* **[!UICONTROL Bearer token]**: HTTP 위치에 인증할 전달자 토큰을 삽입합니다.
...
이 인증 열기를 선택하면 **[!UICONTROL Connect to destination]**&#x200B;만 선택하면 끝점에 연결됩니다.

#### OAuth 2 암호 인증 {#oauth-2-password-authentication}

HTTP 끝점에 연결할 **[!UICONTROL OAuth 2 Password]** 인증 유형을 선택한 경우 아래 필드를 입력하고 **[!UICONTROL Connect to destination]**&#x200B;을(를) 선택하십시오.
...
* **[!UICONTROL Client ID]**: 시스템이 [!DNL client ID]에 할당하는 [!DNL Adobe Experience Platform]입니다.
* **[!UICONTROL Client Secret]**: 시스템이 [!DNL client secret]에 할당하는 [!DNL Adobe Experience Platform]입니다.
...
HTTP 끝점에 연결할 **[!UICONTROL OAuth 2 Client Credentials]** 인증 유형을 선택한 경우 아래 필드를 입력하고 **[!UICONTROL Connect to destination]**&#x200B;을(를) 선택하십시오.
...
* **[!UICONTROL Client Credentials Type]**: 끝점에서 지원하는 OAuth2 클라이언트 자격 증명 부여 유형을 선택하십시오.
   * **[!UICONTROL Body Form Encoded]**: 이 경우 [!DNL client ID] 및 [!DNL client secret]은(는) 대상으로 전송된 요청의 본문에 *포함됩니다*. 예를들어 [지원되는 인증 형식](#supported-authentication-types) 섹션을 참조하십시오.
   * **[!UICONTROL Basic Authorization]**: 이 경우 [!DNL client ID] 및 [!DNL client secret]은(는) base64로 인코딩되어 대상으로 전송된 후 *헤더에 `Authorization`포함됩니다*. 예를들어 [지원되는 인증 형식](#supported-authentication-types) 섹션을 참조하십시오.
...
>title="HTTP 엔드포인트"
>abstract="프로필 데이터를 전송할 HTTP 엔드포인트의 URL입니다. 이는 데이터 수신 엔드포인트이며 구성된 경우 mTLS를 지원합니다(OAuth 2 암호 또는 클라이언트 자격 증명 인증에서는 사용할 수 없음)."
...
>[!NOTE]
>
>이 예제에서 첫 번째 세그먼트(`5b998cb9-9488-4ec3-8d95-fa8338ced490`)는 대상에 매핑되고 `name` 필드를 포함합니다. 두 번째 세그먼트(`354e086f-2e11-49a2-9e39-e5d9a76be683`)는 대상에 매핑되지 않으며 `name` 옵션이 활성화되어 있더라도 **[!UICONTROL Include Segment Names]** 필드를 포함하지 않습니다.
