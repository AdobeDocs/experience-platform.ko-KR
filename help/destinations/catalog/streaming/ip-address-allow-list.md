...
> * 여기에 문서화된 IP 목록 *다음이 아님* 을 사용하여 빌드하는 대상에 적용 [[!DNL Destination SDK]](/help/destinations/destination-sdk/overview.md).
...
허용 목록에 추가하다 이 페이지에서는 데이터를 HTTP 엔드포인트로 안전하게 내보내기 위해 Experience Platform에 추가할 수 있는 IP 범위를 제공합니다. [!DNL Amazon Kinesis], 또는 [!DNL Azure Event Hubs] 인스턴스. 이 기능은 HTTP 끝점이 엔터프라이즈 방화벽 뒤에 있거나 회사 보안 및 규정 준수 표준에서 IP 범위 목록을 허용 목록에추가된으로 유지해야 하는 경우에 특히 유용합니다.
