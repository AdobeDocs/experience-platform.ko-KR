...
아래 표에는 구현 유형에 따라 Web SDK 또는 Edge Network API를 사용하거나 사용하지 않고, [에지 세분화](/help/segmentation/home.md#edge)를 사용하거나 사용하지 않고 Adobe Target 대상에 대해 지원되는 사용 사례가 나와 있습니다.
...
[데이터 스트림 사용](#parameters)에 대한 [!DNL Adobe Target] 연결을 구성할 때는 [Adobe Experience Platform 데이터 수집](/help/collection/home.md)을 구현해야 합니다.
...
Adobe Target 연결을 구현할 때는 다음 사항에 유의하십시오.

* **같은 IMS 조직**: Adobe Experience Platform과 Adobe Target이 같은 IMS 조직에 속해 있어야 합니다.
* **첫 번째 로그인 개인화를 위한 ID 관리**: 첫 번째 로그인 개인화에는 Real-Time CDP ID 그래프의 알려진 고객 ID와 일치하도록 개인화 요청의 기본 ID가 필요합니다. 기본 ID가 ECID 또는 연결되지 않은 ID인 경우 ID 조정은 에지 개인화가 알려진 고객의 데이터를 반영하기까지 몇 분이 걸릴 수 있습니다.
* **사용자 지정 ID 네임스페이스 조회**: 사용자 지정 ID 네임스페이스를 조회하려면 관련 ID가 적절하게 매핑된 웹 SDK 또는 Edge Network API 배포가 필요합니다.
* **Edge 프로필 TTL(time-to-live)**: Edge 프로필에는 14일 TTL이 있습니다. 14일 동안 활동이 없으면 Edge 프로필이 만료되며 다음 페이지 방문 시 허브에서 검색해야 합니다. 즉, 만료 후 첫 번째 방문에서는 실시간 데이터가 아닌 기록 프로필 보기를 사용할 수 있습니다.
...
* **데이터스트림**: 대상자가 포함될 데이터 수집 데이터스트림을 결정합니다. 드롭다운 메뉴에는 Target 및 [!DNL Adobe Experience Platform] 서비스가 활성화된 데이터스트림만 표시됩니다. [!DNL Adobe Experience Platform] 및 [!DNL Adobe Target]에 대한 데이터 스트림을 구성하는 방법에 대한 자세한 내용은 [데이터 스트림 구성](../../../datastreams/configure.md#aep)을 참조하십시오.
...
[!DNL Adobe Experience Platform] Edge Network에서 [!DNL Adobe Target] *읽기* 프로필 데이터를 가져오므로 데이터를 내보내지 않습니다.
