...
Adobe Audience Manager Data Integration Library 확장(클라이언트측 구현)입니다. 참고: 이 확장은 Adobe Analytics 데이터의 서버측 전달(SSF)에 사용하기 위한 것이 아닙니다. SSF의 경우 Adobe Analytics 확장을 사용합니다. 중요: 버전 8.0부터 DIL은 [!DNL Experience Cloud] ID 서비스, 버전 3.3 이상에 종속되어 있습니다. 전체 [!DNL Audience Manager] 데이터 통합 기능에 대해 [!DNL Experience Cloud] ID 서비스와 DIL을 모두 구현하십시오.

[!DNL Audience Manager] DIL은 Adobe Experience Platform의 DMP(데이터 관리 플랫폼) 확장입니다. 확장 기능에 대한 자세한 내용은 태그 설명서의 [Audience Manager 확장 페이지](../../../tags/extensions/client/audience-manager/overview.md)를 참조하십시오.
