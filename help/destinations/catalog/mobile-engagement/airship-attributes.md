...
| 항목 | 유형 | 참고 |
|---------|----------|---------|
| 내보내기 유형 | **[!UICONTROL Profile-based]** | 필드 매핑에 따라 원하는 스키마 필드(예: 이메일 주소, 전화번호, 성) 및/또는 ID와 함께 세그먼트의 모든 멤버를 내보냅니다. |
...
[!DNL Airship] 특성은 장치 인스턴스(예: iPhone)를 나타내는 채널 또는 사용자의 모든 장치를 고객 ID와 같은 공통 식별자에 매핑하는 명명된 사용자에 대해 설정할 수 있습니다. 스키마에 기본 ID로 일반 텍스트(해시되지 않은) 이메일 주소가 있는 경우 **[!UICONTROL Source Attributes]**&#x200B;의 이메일 필드를 선택하고 아래와 같이 [!DNL Airship]의 오른쪽 열에 있는 **[!UICONTROL Target Identities]** 명명된 사용자에 매핑합니다.
...
* `fullName` &quot;Full Name&quot; 특성에 대한 Adobe [!DNL Airship] 특성

>[!NOTE]
>
>특성 매핑에 대한 대상 필드를 선택할 때 `attribute_id` 대시보드의 특성에 해당하는 [!DNL Airship]을(를) 사용하십시오.
