...
Experience Platform은 두 가지 유형의 Snowflake 대상을 제공합니다. [Snowflake 스트리밍](/help/destinations/catalog/cloud-storage/snowflake.md) 및 [Snowflake 일괄 처리](snowflake-batch.md).
...
|  | 필요한 경우 [Snowflake 일괄 처리](snowflake-batch.md) 선택 | 필요할 때 [Snowflake 스트리밍](/help/destinations/catalog/cloud-storage/snowflake.md)을 선택하세요. |
|--------|-------------------|----------------------|
...
스트리밍 데이터 공유에 대한 자세한 내용은 [Snowflake 스트리밍 연결](../cloud-storage/snowflake.md) 설명서를 참조하십시오.
...
| 내보내기 빈도 | **[!UICONTROL 일괄 처리]** | 이 대상은 Snowflake 데이터 공유를 통해 전체 대상 멤버십에 대한 정기적인 스냅샷을 제공합니다. 각 스냅샷은 이전 데이터를 대체하므로 항상 대상에 대한 최신 전체 보기를 확인할 수 있습니다. |
...
대상에 인증하려면 **[!UICONTROL 대상에 연결]**&#x200B;을 선택하고 계정 이름과 필요한 경우 계정 설명을 제공하십시오.
...
여러 병합 정책이 있는 대상은 단일 데이터 흐름에서 지원되지 않습니다. 서로 다른 병합 정책으로 인해 서로 다른 스냅샷이 생성되며, 실제로 한 대상과 관련된 데이터는 두 대상의 데이터를 모두 예상대로 내보내는 대신 다른 대상의 데이터로 덮어쓰여집니다.
