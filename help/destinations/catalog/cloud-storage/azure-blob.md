...
[!DNL Azure Blob]&#x200B;(이하 [!DNL Blob])은(는) 클라우드에 대한 Microsoft의 개체 저장소 솔루션입니다. 이 자습서에서는 [!DNL Experience Platform] 사용자 인터페이스를 사용하여 [!DNL Blob] 대상을 만드는 단계를 제공합니다.

## API 또는 UI를 통해 [!UICONTROL Azure Blob] 저장소에 연결합니다. {#connect-api-or-ui}

* Experience Platform 사용자 인터페이스를 사용하여 [!UICONTROL Azure Blob] 저장소 위치에 연결하려면 아래의 [대상에 연결](#connect) 및 [이 대상에 대상 활성화](#activate) 섹션을 참조하세요.
* 프로그래밍 방식으로 [!UICONTROL Azure Blob] 저장소 위치에 연결하려면 흐름 서비스 API 자습서를 사용하여 [파일 기반 대상에 대상 활성화](../../api/activate-segments-file-based-destinations.md)를 읽어 보세요.
...
* **[!UICONTROL 컨테이너]**: 이 대상에서 사용할 [!DNL Azure Blob Storage] 컨테이너의 이름을 입력하십시오.
