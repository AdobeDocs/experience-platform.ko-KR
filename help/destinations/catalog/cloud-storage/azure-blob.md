...
[!DNL Azure Blob](이하 [!DNL Blob])은(는) 클라우드에 대한 Microsoft의 개체 저장소 솔루션입니다. 이 자습서에서는 [!DNL Platform] 사용자 인터페이스를 사용하여 [!DNL Blob] 대상을 만드는 단계를 제공합니다.

## API 또는 UI를 통해 [!UICONTROL Azure Blob] 저장소에 연결합니다. {#connect-api-or-ui}

* 플랫폼 사용자 인터페이스를 사용하여 [!UICONTROL Azure Blob] 저장소 위치에 연결하려면 아래의 [대상에 연결](#connect) 및 [이 대상에 대상 활성화](#activate) 섹션을 참조하세요.
