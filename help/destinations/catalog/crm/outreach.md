...
title: 지원 연결
description: 아웃리치 대상을 사용하여 계정 데이터를 내보내고 비즈니스 요구 사항에 맞게 아웃리치 내에서 활성화합니다.
...
이 [!DNL Adobe Experience Platform] [대상](/help/destinations/home.md)은(는) [Outreach Update Resource API](https://api.outreach.io/api/v2/docs#update-an-existing-resource)를 활용하여 [!DNL Outreach]의 잠재 고객에 해당하는 대상 내의 ID를 업데이트합니다.
...
마케터는 [!DNL Adobe Experience Platform] 프로필의 특성에 따라 잠재 고객에게 개인화된 경험을 제공할 수 있습니다. 대상 및 프로필이 [!DNL Outreach]에서 업데이트되는 즉시 잠재 고객의 피드에 표시하기 위해 오프라인 데이터에서 대상을 작성하고 이 대상을 [!DNL Adobe Experience Platform]&#x200B;(으)로 보낼 수 있습니다.
...
[!DNL Outreach]사용자에게 친숙한&#x200B;*레이블이 대상 이름과 일치하는 사용자 지정 필드를 표시하는 잠재 고객의* 설정 페이지입니다. 잠재 고객 페이지에서 이러한 레이블에 대한 대상 상태를 볼 수 있습니다.
...
**[!UICONTROL Destinations]** > **[!UICONTROL Catalog]**&#x200B;에서 [!DNL Outreach]을(를) 검색합니다. 또는 CRM 범주 아래에서 찾을 수 있습니다.
...
* **[!UICONTROL Username]**: [!DNL Outreach] 계정 전자 메일입니다.
* **[!UICONTROL Password]**: [!DNL Outreach] 계정 암호입니다.
...
대상 데이터를 [!DNL Adobe Experience Platform]에서 [!DNL Outreach] 대상으로 올바르게 보내려면 필드 매핑 단계를 거쳐야 합니다. 매핑은 Experience Platform 계정의 XDM(Experience Data Model) 스키마 필드와 대상 대상의 해당 스키마 필드 간에 링크를 작성하는 것으로 구성됩니다. XDM 필드를 [!DNL Outreach] 대상 필드에 올바르게 매핑하려면 다음 단계를 따르십시오.
...
1. [!UICONTROL Select target field] 창에서 소스 필드를 매핑할 대상 필드의 유형을 선택합니다.
   ...
   * **[!UICONTROL Select custom attributes]**: 소스 필드를 [!UICONTROL Attribute name] 필드에 정의한 사용자 지정 특성에 매핑하려면 이 옵션을 선택하십시오. 지원되는 속성의 전체 목록은 [[!DNL Outreach] 잠재 고객 설명서](https://api.outreach.io/api/v2/docs#prospect)를 참조하십시오.
...
* 이렇게 하려면 각 세그먼트를 선택한 다음 *필드의 `N`에서*&#x200B;사용자 지정 필드 [!DNL Outreach] 레이블&#x200B;**[!UICONTROL Mapping ID]** 필드에 해당하는 숫자 값을 입력하십시오.
...
1. [!DNL Outreach] 웹 사이트로 이동한 다음 [!DNL Apps] > [!DNL Contacts] 페이지로 이동하여 대상자의 프로필이 추가되었는지 확인합니다. [!DNL Outreach]대상 예약[!UICONTROL Mapping ID] 단계 동안 제공된 [&#x200B; 값을 기반으로 &#x200B;](#schedule-segment-export-example)의 각 대상 상태가 Experience Platform의 해당 대상 상태로 업데이트되었음을 확인할 수 있습니다.
...
이 오류를 해결하려면 [!UICONTROL Mapping ID] 대상에 대해 Experience Platform에서 제공한 [!DNL Outreach]이(가) 유효하고 [!DNL Outreach]에 있는지 확인하십시오.
