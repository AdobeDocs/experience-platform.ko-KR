---
title: Zendesk 연결
description: Zendesk 대상을 사용하여 계정 데이터를 내보내고 비즈니스 요구 사항에 맞게 Zendesk 내에서 활성화합니다.
...
멀티채널 B2C 플랫폼의 고객 서비스 부서는 고객을 위해 매끄러운 개인화된 경험을 보장하기를 원합니다. 부서는 자체 오프라인 데이터에서 대상을 만들어 새 사용자 프로필을 만들거나 다른 상호 작용(예: 구매, 반품 등)에서 기존 프로필 정보를 업데이트하여 [!DNL Adobe Experience Platform]에서 [!DNL Zendesk]&#x200B;(으)로 해당 대상을 보낼 수 있습니다. [!DNL Zendesk]에 업데이트된 정보를 제공하면 고객 서비스 상담원이 즉시 고객의 최신 정보를 사용할 수 있으므로 더 빠른 응답과 해결이 가능합니다.
...
Experience Platform에서 [!DNL Zendesk] 계정으로 데이터를 내보내려면 [!DNL Zendesk] 계정이 있어야 합니다.
...
대상 데이터를 [!DNL Adobe Experience Platform]에서 [!DNL Zendesk] 대상으로 올바르게 보내려면 필드 매핑 단계를 거쳐야 합니다. 매핑은 Experience Platform 계정의 XDM(Experience Data Model) 스키마 필드와 대상 대상의 해당 스키마 필드 간에 링크를 작성하는 것으로 구성됩니다.
...
1. [!DNL Zendesk] 웹 사이트로 이동한 다음 **[!UICONTROL Contacts]** 페이지로 이동하여 대상자의 프로필이 추가되었는지 확인합니다. 대상&#x200B;**[!UICONTROL Mapping ID]** 및 대상 상태로 만든 추가 필드에 대한 열을 표시하도록 이 목록을 구성할 수 있습니다.
