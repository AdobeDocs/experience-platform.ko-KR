...
이 문서에서는 [!DNL Adobe Experience Platform]에서 대상을 활성화하여 클라우드 저장소 및 이메일 마케팅 대상과 같은 프로필 파일 기반 대상을 일괄 처리하는 데 필요한 워크플로에 대해 설명합니다.
...
고정된 시간에 활성화 작업을 실행하려면 **[!UICONTROL Scheduled]** 옵션을 사용하십시오. 이 옵션을 사용하면 Experience Platform 프로필 데이터를 매일 같은 시간에 내보낼 수 있습니다. 그러나 활성화 작업이 시작되기 전에 배치 세분화 작업이 완료되었는지 여부에 따라 내보내는 프로필이 최신 상태가 아닐 수 있습니다.
   ...
   내보내기 일정을 구성할 때 활성화 흐름을 완료한 후 시작 시간을 적어도 **1시간**&#x200B;으로 설정하십시오. 대상자 활성화는 시스템을 통해 전파되는 데 최대 1시간이 걸릴 수 있습니다. 활성화 후 1시간 이내에 내보내기를 실행하도록 예약하는 경우 예약된 내보내기가 누락될 수 있습니다.
...
### 예약된 내보내기 동작 이해 {#export-behavior}

예약된 내보내기에는 대상 스냅샷 데이터와 스냅샷 생성과 내보내기 시간 사이에 발생하는 증분 프로필 또는 ID 변경 사항이 포함됩니다. 스냅숏 데이터만 사용하는 [주문형 내보내기](export-file-now.md)와(과) 다릅니다.

다음 표에서 특히 데이터 신선도 및 사용 의도 측면에서 예약된 내보내기가 온디맨드 내보내기와 어떻게 다른지 조명합니다.

|  | 예약된 내보내기 | 지금 파일 내보내기 |
|--------|-------------------|-----------------|
| **데이터 원본** | 스냅샷 + 증분 변경 사항 | 스냅샷만 |
| **프로필 속성** | 내보내기 시 현재 값 | 스냅샷 시간 값 |

대상 평가 후 프로필이 업데이트되면 대상 멤버십이 평가 시간에 결정되더라도 예약된 내보내기에 업데이트된 속성 값이 포함됩니다.

**예**: &quot;retailID가 null인 프로필&quot;에 대한 대상은 해당 필드가 *후* 평가에서 *이전* 예약된 내보내기로 업데이트된 경우 retailID가 채워진 프로필을 내보낼 수 있습니다.

**Recommendations**

* 중복 레코드를 방지하려면 [중복 제거 키](#deduplication-keys)를 구성하십시오.
* 정확한 스냅샷 기반 데이터를 위해 온디맨드 내보내기 사용
* 배치 수집을 평가 일정에 맞춰 불일치를 최소화합니다.

주문형 내보내기에 대해서는 [주문형 파일 내보내기](/help/destinations/ui/export-file-now.md#scheduled-vs-ondemand)에 대한 설명서를 참조하십시오.
...
2. 내보내기를 수행할 시간을 [!DNL UTC] 형식으로 선택하려면 **[!UICONTROL Time]** 선택기를 사용하십시오.
...
>[!IMPORTANT]
>
>예약된 내보내기에 대해 항상 중복 제거 키를 구성합니다. 중복 제거를 사용하지 않으면 예약된 내보내기가 스냅샷 및 증분 데이터를 모두 처리하므로 동일한 프로필에 대해 중복 행 또는 충돌하는 세그먼트 멤버십이 표시될 수 있습니다.

중복 제거 키는 프로필의 중복 제거 방법을 결정하는 사용자 정의 기본 키입니다. 동일한 개인에 대해 여러 레코드가 존재하는 경우 중복 제거를 통해 최신 레코드만 내보냅니다.
...
#### 대상 멤버십 속성은 매핑 워크플로를 통해 선택할 수 없습니다. {#audience-membership-attribute-mapping}
...
#### 내보내기를 위해 현재 ID 네임스페이스를 선택할 수 없습니다. {#identity-namespaces-export-limitation}
...
>[!IMPORTANT]
>
>카탈로그의 모든 클라우드 저장소 대상은 이 섹션에서 설명한 **[!UICONTROL Select attributes]** 단계를 대체하는 향상된 [[!UICONTROL Mapping] 단계](#mapping)을(를) 볼 수 있습니다.
>
>이 **[!UICONTROL Select attributes]** 단계는 [!DNL Adobe Campaign], Oracle Responsys, Oracle Eloqua 및 Salesforce Marketing Cloud 이메일 마케팅 대상에 대해 계속 표시됩니다.
...
>[!NOTE]
>
> [!DNL Adobe Experience Platform]은(는) 스키마에서 일반적으로 사용되는 네 가지 권장 특성으로 선택을 미리 채웁니다. `person.name.firstName`, `person.name.lastName`, `personalEmail.address`, `segmentMembership.seg_namespace.seg_id.status`.
...
>title="계층 구조 출력 활성화됨"
>abstract="이 대상은 배열, 맵 및 개체의 내보내기 활성화 토글이 켜져 있으므로 계층적 출력을 지원합니다. 하나의 매핑에서 동일한 배열의 최상위 배열, 배열 요소 또는 여러 필드를 내보낼 수 있습니다. 자세한 내용은 이 문서를 참조하십시오."

>[!CONTEXTUALHELP]
>id="platform_destinations_enrichment_attributes_source_field"
>title="소스 필드"
>abstract="내보낼 데이터 보강 속성을 선택합니다. 배열 내의 필드의 경우 소스는 변형 식으로 자동 채워집니다. 여러 필드를 하나의 매핑으로 내보내려면 먼저 한 필드를 추가한 다음 소스 표현식을 편집합니다. 자세한 내용은 이 문서를 참조하십시오."

>[!CONTEXTUALHELP]
>id="platform_destinations_enrichment_attributes_target_field"
>title="대상 필드"
>abstract="대상 필드가 소스 필드 이름으로 자동 채워집니다. 내보낸 파일에 다른 이름을 두려면 다른 별칭을 사용하도록 편집합니다."

>[!IMPORTANT]
>
>이 단계는 [대상자 선택](#select-audiences) 단계 동안 **[!UICONTROL Custom upload]**&#x200B;대상자를 선택한 경우에만 표시됩니다.
...
[검토](#review) 단계로 이동하려면 **[!UICONTROL Next]**&#x200B;을(를) 선택하십시오.
...
대상을 클라우드 저장소 대상으로 내보낼 때 [!DNL Adobe Experience Platform]은(는) 제공한 저장소 위치에 `.csv`, `.json` 또는 `.parquet` 파일을 만듭니다. 워크플로우에서 설정한 일정에 따라 스토리지 위치에 새 파일이 생성됩니다. 기본 파일 형식은 아래에 표시되어 있지만 [파일 이름의 구성 요소를 편집](#configure-file-names)할 수 있습니다.
