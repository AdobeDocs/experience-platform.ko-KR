...
대상에 데이터를 활성화하려면 대상을 [연결했어야 합니다](./connect-destination.md). 아직 수행하지 않은 경우 [대상 카탈로그로](../catalog/overview.md)이동하여 지원되는 대상을 탐색하고 하나 이상의 대상을 설정합니다.
...
![대상 검색](../assets/ui/activate-destinations/connect.png)

정품 인증 워크플로로 이동할 대상 이름을 선택합니다.

![정품 인증](../assets/ui/activate-destinations/activate-flow.png)
...
![ID로 이메일 주소](../assets/ui/activate-destinations/email-as-identity.gif)
...
![필드 채우기 후 ID 매핑](../assets/common/identity-mapping.png)
...
![데이터 원본 선택](../assets/ui/activate-destinations/choose-data-origin.png)
...
![권장 속성](../assets/ui/activate-destinations/mark-mandatory.png)
...
>
> 
예를 들어, 필드에 대상의 마케팅 사용 사례와 충돌하는 특정 데이터 사용 레이블이 `person.name.firstName` 있는 경우 검토 단계에서 데이터 사용 정책 위반이 표시됩니다. 자세한 내용은 실시간 CDP의 [데이터 거버넌스를 참조하십시오](../../rtcdp/privacy/data-governance-overview.md#destinations).
...
파일 포맷을 편집할 수 있습니다. 자세한 내용은 클라우드 스토리지 [대상 및 이메일](#configure) 마케팅 대상에 대한 구성 단계로 이동합니다.
...
스토리지 위치에 이러한 파일이 있는지 확인한 후 정품 인증을 완료했습니다. 내보낸 파일의 구조를 이해하려면 샘플 .csv 파일을 [다운로드할 수 있습니다](../assets/common/sample_export_file_segment12341e18-abcd-49c2-836d-123c88e76c39_20200408061804.csv). 이 샘플 파일에는 프로필 속성 `person.firstname`, `person.lastname`, `person.gender`및 `person.birthyear`파일이 포함되어 `personalEmail.address`있습니다.
