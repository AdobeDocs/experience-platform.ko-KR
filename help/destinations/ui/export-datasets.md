...
<tbody>
  ...
  <tr>
    <td>Ultimate</td>
    <td>Adobe Systems Journey Optimizer</a> 설명서를 참조하십시오<a href="https://experienceleague.adobe.com/docs/journey-optimizer/using/data-management/datasets/export-datasets.html#datasets">.</td>
  </tr>
  <tr>
...
이 페이지 페이지에 설명된 작업 과정, 데이터 세트 내보내기 기능 사용의 이점 및 몇 가지 제안된 사용 사례에 대한 엔드 투 엔드 설명을 보려면 아래 비디오를 시청하십시오.

>[!VIDEO](https://video.tv.adobe.com/v/3424392/)

## 지원되는 대상 {#supported-destinations}

현재 데이터 세트를 스크린샷 아래에 강조 표시된 클라우드 스토리지 대상으로 내보낼 수 있습니다.
...
## 목적지를 선택하세요 {#select-destination}

지침에 따라 데이터 세트를 내보낼 수 있는 대상을 선택합니다.

1. Connections > Destinations(**[!UICONTROL 연결 대상)]**&#x200B;으로 이동하고 카탈로그&#x200B;]**탭 선택합니다**[!UICONTROL .
...
1. **[!UICONTROL 빈도]** 선택기를 사용하여 내보내기 빈도를 선택하십시오.
   ...
   * **[!UICONTROL 매일]**: 증분 파일 내보내기를 하루에 한 번, 매일 지정한 시간에 예약합니다.
...
원하는 매크로를 선택하면 저장소 위치에 생성되는 폴더 구조의 미리보기가 표시됩니다. 폴더 구조의 첫 번째 수준은 데이터 세트를 내보내기 위해 대상](/help/destinations/ui/connect-destination.md##set-up-connection-parameters)에 연결할 때 [표시한 폴더 경로를&#x200B;]**나타냅니다**[!UICONTROL .
...
데이터 세트를 내보낼 때 Experience Platform 은 사용자가 제공한 저장소 위치에 하나 이상의 `.json` or `.parquet` 파일을 만듭니다. 제공한 내보내기 일정에 따라 새 파일이 저장 위치에 저장될 것으로 예상합니다.

Experience Platform 은 지정한 저장 위치에 폴더 구조를 만들어 내보낸 데이터 세트 파일을 보관합니다. 기본 폴더 내보내기 패턴은 아래와 같지만 원하는 매크로를 사용하여 폴더 구조를 사용자 지정할 수 [있습니다](#edit-folder-path).

>[!TIP]
> 
>이 폴더 구조의 첫 번째 수준인 - `folder-name-you-provided` 는 데이터 세트를 내보내기 위해 대상](/help/destinations/ui/connect-destination.md##set-up-connection-parameters)에 연결할 때 [지정한 폴더 경로를&#x200B;]**나타냅니다**[!UICONTROL .
...
**매니페스트 파일에 추가 세부 정보(예: 레코드 수)를 추가할 수 있습니까? 만약 그렇다면, 어떻게?**

+++답변
매니페스트 파일에 추가 정보를 추가할 수 없습니다. 레코드 카운트는 엔티티를 통해 사용할 수 있습니다(API를 `flowRun` 통해 쿼리 가능). 자세한 내용은 목적지 모니터링을 참조하세요.
+++

**데이터 파일은 어떻게 분할됩니까? 파일당 레코드 수는 몇 개입니까?**

+++답변
데이터 파일은 Experience Platform 데이터 레이크의 기본 파티셔닝에 따라 분할됩니다. 더 큰 데이터 세트에는 더 많은 수의 파티션이 있습니다. 기본 분할은 읽기에 최적화되었기 때문에 사용자가 구성할 수 없습니다.
+++
