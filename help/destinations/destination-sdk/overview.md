---
description: Adobe Experience Platform Destination SDK은 선택한 데이터 및 인증 형식을 기반으로 대상 및 프로필 데이터를 엔드포인트 또는 스토리지 위치에 제공하기 위해 Experience Platform에 대한 대상 통합 패턴을 구성하기 위한 구성 API 세트입니다. 구성은 Experience Platform에 저장되며 추가 업데이트를 위해 API를 통해 검색할 수 있습니다.
...
[!DNL Adobe Experience Platform] Destination SDK은 선택한 데이터 및 인증 형식을 기반으로 대상과 프로필 데이터를 끝점 또는 저장소 위치에 제공하기 위해 Experience Platform에 대한 대상 통합 패턴을 구성하기 위한 구성 API 세트입니다. 구성은 Experience Platform에 저장되며 추가 업데이트를 위해 API를 통해 검색할 수 있습니다.

Destination SDK 설명서는 [!DNL Adobe Experience Platform] Destination SDK을 사용하여 [!DNL Adobe Experience Platform]과(와) 제품화된 대상 통합을 구성, 테스트 및 릴리스하고 대상을 지속적으로 성장하는 대상 카탈로그의 일부로 만드는 지침을 제공합니다. Destination SDK을 사용하여 사용자 정의 개인 대상을 만들어 요구 사항에 맞게 데이터를 내보낼 수도 있습니다.
...
3. [!DNL Adobe Experience Platform] 및 Adobe [!DNL Real-Time Customer Data Platform]과(와) 제품화된 대상 통합으로 포함됩니다.
...
[!DNL Adobe Experience Platform]은(는) Destination SDK을 통해 REST API 끝점이 있는 대상과의 실시간(스트리밍이라고도 함) 통합을 지원합니다. Experience Platform과의 실시간 통합은 다음과 같은 기능을 지원합니다.
...
* 다운스트림 요구 사항을 충족하도록 내보낸 파일의 형식을 구조화하기 위한 구성 가능한 파일 형식 옵션
...
Destination SDK 액세스는 파트너 또는 Experience Platform, [!DNL Real-Time CDP] 고객 상태에 따라 다릅니다. 자세한 내용은 아래 표를 참조하십시오.
