...
다음 `/authoring/testing/template/render` 엔드포인트는 템플릿화 방법을 시각화하는 데 도움이 됩니다 [고객 데이터 필드](file-based-destination-configuration.md#customer-data-fields) 대상 구성에 정의되어 있으면 다음과 같은 모습입니다.
...
| 매개 변수 | 설명 |
| -------- | ----------- |
| `destinationId` | 의 ID [대상 구성](file-based-destination-configuration.md) 테스트하고 있습니다. |
| `templates` | 에 정의된 템플릿화된 필드 이름 [대상 서버 구성](server-and-file-configuration.md). |
...
이 문서를 읽고 나면 이제에서 정의한 고객 데이터 필드 구성을 확인하는 방법을 이해할 수 있습니다. [대상 서버](server-and-file-configuration.md).
