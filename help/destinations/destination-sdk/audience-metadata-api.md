...
이 페이지에서는 `/authoring/audience-templates` API 종단점을 사용하여 수행할 수 있는 모든 API 작업을 나열하고 설명합니다. 이 종단점을 사용할 시점에 대한 설명은 [대상 메타데이터 관리](./audience-metadata-management.md)를 참조하십시오.
...
`/authoring/audience-templates` 종단점에 대한 POST 요청을 만들어 새 대상 템플릿을 만들 수 있습니다.
...
다음 요청은 페이로드에 제공된 매개 변수로 구성된 새로운 대상 메타데이터 템플릿을 만듭니다. 아래 페이로드에는 `/authoring/audience-templates` 종단점에서 허용하는 모든 매개 변수가 포함되어 있습니다. 호출에 모든 매개 변수를 추가할 필요는 없으며 API 요구 사항에 따라 템플릿을 사용자 지정할 수 있습니다.
...
| 속성 | 유형 | 설명 |
| -------- | ----------- | ----------- |
| `name` | 문자열 | 대상에 대한 대상 메타데이터 템플릿의 이름입니다. 이 이름은 Experience Platform 사용자 인터페이스의 모든 파트너별 오류 메시지에 나타나고 `metadataTemplate.create.errorSchemaMap`에서 구문 분석된 오류 메시지가 표시됩니다. |
...
| `requestBody` | 문자열 | API로 전송해야 하는 메시지 본문의 콘텐츠를 지정합니다. `requestBody` 개체에 추가해야 하는 매개 변수는 API에서 허용하는 필드에 따라 다릅니다. 예를 들어 대상 메타데이터 기능 문서에서 [첫 번째 템플릿 예제](./audience-metadata-management.md#example-1)를 참조하십시오. |
| `responseFields.name` | 문자열 | API가 호출될 때 반환하는 응답 필드를 지정합니다. 예를 들어 대상 메타데이터 기능 문서에서 [템플릿 예제](./audience-metadata-management.md#examples)를 참조하십시오. |
| `responseFields.value` | 문자열 | API가 호출될 때 반환하는 응답 필드의 값을 지정합니다. |
| `responseErrorFields.name` | 문자열 | API가 호출될 때 반환하는 응답 필드를 지정합니다. 예를 들어 대상 메타데이터 기능 문서에서 [ 템플릿 예제](./audience-metadata-management.md#examples)를 참조하십시오. |
...
| `validations.field` | 문자열 | 대상에 API 호출을 수행하기 전에 모든 필드에 대한 유효성 검사를 실행해야 하는지 여부를 나타냅니다. 예를 들어 `{{validations.accountId}}` 을 사용하여 사용자의 계정 ID를 확인할 수 있습니다. |
...
`/authoring/audience-templates` 종단점에 PUT 요청을 만들고 업데이트할 대상 템플릿의 인스턴스 ID를 제공하여 기존 대상 템플릿을 업데이트할 수 있습니다. 호출 본문에서 업데이트된 템플릿을 제공합니다.
...
`/authoring/audience-templates` 종단점에 GET 요청을 하여 IMS 조직에 대한 모든 대상 템플릿 목록을 검색할 수 있습니다.
...
다음 응답은 사용한 IMS 조직 ID 및 샌드박스 이름을 기반으로 액세스 권한이 있는 대상 메타데이터 템플릿 목록과 함께 HTTP 상태 200을 반환합니다. 하나의 `instanceId` 은 하나의 대상에 대한 템플릿에 해당합니다. 간결성을 위해 응답이 잘립니다.
...
`/authoring/audience-templates` 종단점에 GET 요청을 하고 검색할 대상 템플릿의 인스턴스 ID를 제공하여 특정 대상 템플릿에 대한 자세한 정보를 검색할 수 있습니다.
...
`/authoring/audience-templates` 종단점에 DELETE 요청을 하고 요청 경로에서 삭제하려는 대상 템플릿의 ID를 제공하여 지정된 대상 템플릿을 삭제할 수 있습니다.
...
이 문서를 읽은 후에는 대상 메타데이터 템플릿을 사용할 시점과 `/authoring/audience-templates` API 종단점을 사용하여 대상 메타데이터 템플릿을 구성하는 방법을 알 수 있습니다. [대상 SDK를 사용하여 대상](./configure-destination-instructions.md)을 구성하는 방법을 읽어 이 단계가 대상 구성 프로세스에 맞는 위치를 파악합니다.
