...
Destination SDK을 사용하여 Adobe Experience Platform이 대상에 연결할 수 있도록 합니다. [OAuth 2 인증 프레임워크](https://tools.ietf.org/html/rfc6749).
...
Experience Platform에서 대상에 대한 OAuth 2 인증을 설정하려면 OAuth 2 세부 사항을 [대상 구성](./destination-configuration.md)아래에 있는 `customerAuthenticationConfigurations` 매개 변수( `platform.adobe.io/data/core/activation/authoring/destinations` [API 엔드포인트](./destination-configuration-api.md). 자세한 내용은 [예제 구성](./destination-configuration.md#example-configuration). OAuth 2 인증 부여 유형에 따라 구성 템플릿에 추가해야 하는 필드에 대한 특정 지침은 이 페이지에서 더 아래에 나와 있습니다.
...
>* 테이블에서 굵게 강조 표시된 입력 매개 변수는 OAuth 2 인증 흐름의 필수 매개 변수입니다. 다른 매개 변수는 선택 사항입니다. 여기에 표시되지 않지만 섹션에서 길이에 설명된 다른 사용자 지정 입력 매개 변수가 있습니다 [OAuth 2 구성 사용자 지정](./oauth2-authentication.md#customize-configuration) 및 [액세스 토큰 새로 고침](./oauth2-authentication.md#access-token-refresh).
...
대상에 대해 이 인증 방법을 설정하려면 의 `/destinations` [엔드포인트](./destination-configuration.md):
...
OAuth 2 클라이언트 자격 증명을 구성할 수 있습니다( [RFC 표준 사양](https://tools.ietf.org/html/rfc6749#section-4.4)) 대상. 아래에 나열된 표준 입력 및 출력을 지원합니다. 값을 사용자 지정할 수 있습니다. 자세한 내용은 [OAuth 2 구성 사용자 지정](./oauth2-authentication.md#customize-configuration) 자세한 내용
...
이 예에서는 섹션에 표시된 대로 글로벌 클라이언트 ID 및 클라이언트 암호를 만드는 대신 [시스템의 사전 요구 사항](./oauth2-authentication.md#prerequisites)를 지정하는 경우 고객이 클라이언트 ID, 클라이언트 암호 및 계정 ID(고객이 대상에 로그인하는 데 사용하는 ID)를 입력해야 합니다
...
이 문서를 읽은 후에는 Adobe Experience Platform에서 지원하는 OAuth 2 인증 패턴을 이해하고 OAuth 2 인증 지원을 사용하여 대상을 구성하는 방법을 알아봅니다. 다음으로, Destination SDK을 사용하여 OAuth 2 지원 대상을 설정할 수 있습니다. 읽기 [Destination SDK을 사용하여 대상을 구성합니다](./configure-destination-instructions.md) 을 참조하십시오.
