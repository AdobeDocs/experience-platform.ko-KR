...
조직에 대한 GET 요청을 수행하여 조직의 모든 대상 서버 구성 목록을 검색할 수 있습니다 `/authoring/destination-servers` 엔드포인트.
...
다음 요청은 조직 및 샌드박스 구성에 따라 액세스 권한이 있는 대상 서버 구성 목록을 검색합니다.
...
다음 응답은 사용한 조직 ID 및 샌드박스 이름을 기반으로 액세스 권한이 있는 대상 서버 구성 목록과 함께 HTTP 상태 200을 반환합니다. 1개 `instanceId` 는 하나의 대상 서버에 대한 템플릿에 해당합니다. 간결성을 위해 응답이 잘립니다.
