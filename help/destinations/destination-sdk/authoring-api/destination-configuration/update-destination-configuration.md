...
이 페이지는 기존 대상 구성을 업데이트하는 데 사용할 수 있는 API 요청 및 페이로드를 다음과 같이 예시합니다. `/authoring/destinations` API 엔드포인트.
...
다음을 업데이트할 수 있습니다. [기존](create-destination-configuration.md) 다음을 수행하여 대상 구성: `PUT` 에 대한 요청 `/authoring/destinations` 업데이트된 페이로드가 있는 엔드포인트.
...
| 매개변수 | 설명 |
| -------- | ----------- |
| `{INSTANCE_ID}` | 업데이트할 대상 구성의 ID입니다. 기존 대상 구성 및 해당 구성 가져오기 `{INSTANCE_ID}`, 참조 [대상 구성 검색](retrieve-destination-configuration.md). |

+++요청

다음 요청은에서 만든 대상을 업데이트합니다. [이 예](create-destination-configuration.md#create) 다름 `filenameConfig` 옵션.
...
이제 이 문서를 읽고 Destination SDK을 통해 대상 구성을 업데이트하는 방법을 배웁니다 `/authoring/destinations` API 엔드포인트.
