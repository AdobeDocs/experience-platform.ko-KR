---
description: 이 페이지에서는 Adobe Experience Platform Destination SDK을 통해 대상 구성을 검색하는 데 사용되는 API 호출을 보여줍니다.
title: 대상 구성 검색
...
이 페이지는 를 사용하여 기존 대상 구성에 대한 정보를 검색하는 데 사용할 수 있는 API 요청 및 페이로드를 보여줍니다 `/authoring/destination` API 엔드포인트.
...
를 검색할 수 있습니다 [기존](create-destination-configuration.md) 대상 구성을 `GET` 에 요청 `/authoring/destination` 엔드포인트.
...
계정에 대한 모든 대상 구성을 검색하려면 다음 API 형식을 사용하십시오.
...
다음 두 요청은 전달 여부에 따라 IMS 조직 또는 특정 대상 구성에 대한 모든 대상 구성을 검색합니다 `INSTANCE_ID` 매개 변수를 채우는 방법을 설명합니다.
...
성공적인 응답은 다음을 기반으로 액세스 권한이 있는 대상 구성 목록과 함께 HTTP 상태 200을 반환합니다 [!DNL IMS Org ID] 및 사용한 샌드박스 이름을 지정합니다. 1개 `instanceId` 는 하나의 대상 구성에 해당합니다.
...
성공적인 응답은 다음에 해당하는 대상 구성의 세부 정보와 함께 HTTP 상태 200을 반환합니다 `{INSTANCE_ID}` 호출에 제공됩니다.
...
이 문서를 읽은 후에는 Destination SDK을 통해 대상 구성을 검색하는 방법을 알 수 있습니다 `/authoring/destinations` API 엔드포인트.
