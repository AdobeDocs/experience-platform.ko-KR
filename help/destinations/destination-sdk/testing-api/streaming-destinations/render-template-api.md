...
이 페이지에서는 다음을 사용하여 수행할 수 있는 모든 API 작업을 나열하고 설명합니다. `/authoring/testing/template/render` API 엔드포인트, 대상에 따라 예상되는 형식과 일치하는 내보낸 프로필 렌더링 [메시지 변환 템플릿](../../functionality/destination-server/message-format.md#using-templating). 이 끝점에서 지원하는 기능에 대한 설명은 다음을 참조하십시오. [템플릿 만들기](create-template.md).
...
에 POST 요청을 하여 내보낸 프로필을 렌더링할 수 있습니다. `authoring/testing/template/render` 대상 구성의 대상 ID 및 를 사용하여 만든 템플릿을 제공하는 엔드포인트 [샘플 템플릿 API 끝점](sample-template-api.md).

변형을 적용하지 않고 원시 프로필을 내보내는 간단한 템플릿을 사용하여 시작한 다음 프로필에 변형을 적용하는 보다 복잡한 템플릿으로 이동할 수 있습니다. 단순 템플릿의 구문은 다음과 같습니다. <br> `"template": "{% for profile in input.profiles %}{{profile|raw}}{% endfor %}}"`
...
| `profiles` | *선택 사항입니다*. 요청 본문에 프로필을 추가할 수 있습니다. 프로필을 추가하지 않으면 Experience Platform이 프로필을 자동으로 생성하고 요청에 추가합니다. <br> 호출 본문에 프로필을 추가하려면 다음을 사용하여 프로필을 생성할 수 있습니다. [샘플 프로필 생성 API](sample-profile-generation-api.md). |

{style="table-layout:auto"}

렌더링 템플릿 API 끝점에서 반환되는 응답은 대상 집계 정책에 따라 다릅니다. 대상에 구성 가능한 집계 정책이 있는 경우 프로필이 집계되는 방법을 결정하는 집계 키도 응답에 반환됩니다. 읽어보기 [집계 정책](../../functionality/destination-configuration/aggregation-policy.md) 을 참조하십시오.

| 응답 매개 변수 | 설명 |
| -------- | ----------- |
| `aggregationKey` | 대상으로 내보내기에서 프로필이 집계되는 정책을 나타냅니다. 이 매개 변수는 선택 사항이며, 대상 집계 정책이 로 설정된 경우에만 표시됩니다. `CONFIGURABLE_AGGREGATION`. |
...
성공한 응답은 내보낸 데이터의 세부 정보와 함께 HTTP 상태 200을 반환합니다. 에서 내보낸 프로필 찾기 `output` 매개 변수(이스케이프 처리된 문자열)입니다.
...
다음 요청은 대상에 필요한 형식과 일치하는 내보낸 여러 프로필을 렌더링합니다. 이 예에서 대상 ID는 구성 가능한 집계가 있는 대상 구성에 해당합니다. 요청 본문에 두 개의 프로필이 포함되어 있으며 각각 세 개의 대상 자격과 다섯 개의 ID를 보유합니다. 를 사용하여 호출 시 보낼 프로필을 생성할 수 있습니다. [샘플 프로필 생성 API](sample-profile-generation-api.md).
...
응답은 템플릿 렌더링 결과 또는 발생한 오류를 반환합니다.
성공한 응답은 내보낸 데이터의 세부 정보와 함께 HTTP 상태 200을 반환합니다. 응답에서 대상자 멤버십 및 ID를 기반으로 프로필을 집계하는 방법을 확인합니다. 에서 내보낸 프로필 찾기 `output` 매개 변수(이스케이프 처리된 문자열)입니다.
실패한 응답은 발생한 오류에 대한 설명과 함께 HTTP 상태 400을 반환합니다.
...
이 문서를 읽은 후에는 메시지 변환 템플릿을 사용하여 대상의 예상 데이터 형식과 일치하는 내보낸 프로필을 생성하는 방법을 이해할 수 있습니다. 읽기 [Destination SDK을 사용하여 대상을 구성하는 방법](../../guides/configure-destination-instructions.md) 대상을 구성하는 프로세스에 이 단계가 어디에 적합한지 이해할 수 있습니다.
