...
이 페이지에서는 다음을 사용하여 수행할 수 있는 모든 API 작업을 나열하고 설명합니다. `/authoring/testing/template/sample` API 엔드포인트, API 엔드포인트 [메시지 변환 템플릿](../../functionality/destination-server/message-format.md#using-templating) 목적지로요 이 끝점에서 지원하는 기능에 대한 설명은 다음을 참조하십시오. [템플릿 만들기](create-template.md).
...
제공한 대상 ID가 의 대상 구성에 해당하는 경우 [최적 작업 집계](../../functionality/destination-configuration/aggregation-policy.md) 및 `maxUsersPerRequest=1` 집계 정책에서 요청은 다음과 유사한 샘플 템플릿을 반환합니다.
