...
**의 대상 스키마와 대상에서 지원하는 메시지 형식 간에**&#x200B;메시지 변환 템플릿을 만들고 테스트하려면[!DNL Adobe Experience Platform]아래 설명된 *템플릿 작성 도구*&#x200B;를 사용하십시오.  [메시지 형식 문서](../../functionality/destination-server/message-format.md#using-templating)에서 원본 스키마와 대상 스키마 간의 데이터 변환에 대해 자세히 알아보세요.
...
Destination SDK에서 대상을 만드는 첫 번째 단계 중 하나는 대상 멤버십, ID 및 프로필 특성에 대한 데이터 형식을 [!DNL Adobe Experience Platform]에서 대상으로 내보낼 때 변환하는 방법을 고려하는 것입니다. [메시지 형식 문서](../../functionality/destination-server/message-format.md#using-templating)에서 Adobe XDM 스키마와 대상 스키마 간의 변환에 대한 정보를 찾으십시오.
...
* [본문으로 전송된 프로필이 없는 템플릿 렌더링](render-template-api.md#best-effort)
* [본문으로 전송된 프로필로 템플릿 렌더링](render-template-api.md#configurable-aggregation)
