...
Destination SDK을 사용하여 만든 일부 대상의 경우 대상에서 세그먼트 메타데이터를 프로그래밍 방식으로 생성, 업데이트 또는 삭제하려면 대상 메타데이터 구성을 만들어야 합니다. 이 페이지에서는 사용 방법을 보여 줍니다. `/authoring/audience-templates` 구성을 만들기 위한 API 엔드포인트.
