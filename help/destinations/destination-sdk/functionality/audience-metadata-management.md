...
### 사용 사례 1 - 서드파티 API가 있으며 사용자는 매핑 ID를 입력할 필요가 없습니다. {#use-case-1}
...
### 사용 사례 2 - 사용자는 먼저 대상에 대상을 만들어야 하며 매핑 ID를 수동으로 입력해야 합니다. {#use-case-2}
...
### 사용 사례 3 - 대상이 Experience Platform 대상 ID를 수락하므로 사용자가 매핑 ID를 수동으로 입력할 필요가 없습니다. {#use-case-3}
...
| 템플릿 섹션 | 설명 |
|--- |--- |
| `create` | API를 HTTP 호출하고 프로그래밍 방식으로 플랫폼에서 세그먼트/대상을 만들고 정보를 다시 [!DNL Adobe Experience Platform]에 동기화하는 데 필요한 모든 구성 요소(URL, HTTP 메서드, 헤더, 요청 및 응답 본문)를 포함합니다. |
| `update` | API를 HTTP 호출하고, 프로그래밍 방식으로 플랫폼에서 세그먼트/대상을 업데이트하고, 정보를 [!DNL Adobe Experience Platform]에 다시 동기화하는 데 필요한 모든 구성 요소(URL, HTTP 메서드, 헤더, 요청 및 응답 본문)를 포함합니다. |
...
| `createDestination` | API에 대한 HTTP 호출을 만들고 프로그래밍 방식으로 플랫폼에서 데이터 흐름을 만들고 정보를 다시 [!DNL Adobe Experience Platform]에 동기화하는 데 필요한 모든 구성 요소(URL, HTTP 메서드, 헤더, 요청 및 응답 본문)를 포함합니다. |
| `updateDestination` | API에 대한 HTTP 호출을 만들고, 플랫폼의 데이터 흐름을 프로그래밍 방식으로 업데이트하고, 정보를 [!DNL Adobe Experience Platform]에 다시 동기화하는 데 필요한 모든 구성 요소(URL, HTTP 메서드, 헤더, 요청 및 응답 본문)를 포함합니다. |
| `deleteDestination` | API를 HTTP 호출하고, 플랫폼에서 데이터 흐름을 프로그래밍 방식으로 삭제하는 데 필요한 모든 구성 요소(URL, HTTP 메서드, 헤더, 요청 및 응답 본문)를 포함합니다. |
...
일부 예제에서는 `{{authData.accessToken}}` 또는 `{{segment.name}}`과(와) 같은 매크로 필드가 URL에 사용되고, 다른 예제에서는 헤더 또는 요청 본문에 사용됩니다. 사용은 마케팅 API 사양에 따라 다릅니다.
...
| 매크로 | 설명 |
|--- |--- |
| `{{segment.alias}}` | Experience Platform의 대상 별칭에 액세스합니다. |
| `{{segment.name}}` | Experience Platform의 대상 이름에 액세스합니다. |
| `{{segment.id}}` | Experience Platform의 대상 ID에 액세스합니다. |
| `{{customerData.accountId}}` | 대상 구성에서 설정한 계정 ID 필드에 액세스합니다. |
| `{{oauth2ServiceAccessToken}}` | OAuth 2 구성을 기반으로 액세스 토큰을 동적으로 생성합니다. |
| `{{authData.accessToken}}` | 액세스 토큰을 API 엔드포인트에 전달합니다. Experience Platform에서 만료되지 않는 토큰을 사용하여 대상에 연결하는 경우 `{{authData.accessToken}}`을(를) 사용하고, 그렇지 않으면 `{{oauth2ServiceAccessToken}}`을(를) 사용하여 액세스 토큰을 생성합니다. |
...
| `{{destination.enrichmentAttributes}}` | 대상에 매핑된 모든 대상에 대한 모든 데이터 보강 속성에 액세스합니다. 이 매크로는 `createDestination`, `updateDestination` 및 `deleteDestination` 이벤트에서 지원됩니다. 강화 속성은 [사용자 정의 업로드 대상자](destination-configuration/schema-configuration.md#external-audiences)에 대해서만 사용할 수 있습니다. 데이터 보강 특성 선택이 어떻게 작동하는지 확인하려면 [일괄 대상자 활성화 가이드](../../ui/activate-batch-profile-destinations.md#select-enrichment-attributes)를 참조하세요. |
| `{{destination.enrichmentAttributes.<namespace>.<segmentId>}}` | 대상에 매핑된 특정 외부 대상자에 대한 데이터 보강 속성에 액세스합니다. 강화 속성은 [사용자 정의 업로드 대상자](destination-configuration/schema-configuration.md#external-audiences)에 대해서만 사용할 수 있습니다. 데이터 보강 특성 선택이 어떻게 작동하는지 확인하려면 [일괄 대상자 활성화 가이드](../../ui/activate-batch-profile-destinations.md#select-enrichment-attributes)를 참조하세요. |
