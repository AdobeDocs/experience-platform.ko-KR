...
대상에 대한 기본 인증을 설정하려면 아래와 같이 `customerAuthenticationConfigurations` 끝점을 통해 `/destinations` 섹션을 구성하십시오.
...
대상에 대한 전달자 유형 인증을 설정하려면 아래와 같이 `customerAuthenticationConfigurations` 끝점을 통해 `/destinations` 섹션을 구성하십시오.
...
사용자는 Twitter 사용자 지정 대상 대상에 대해 아래 예와 같이 대상에 대한 OAuth 2 인증 흐름을 트리거하려면 **[!UICONTROL Connect to destination]**&#x200B;을(를) 선택합니다. 대상 끝점에 대한 OAuth 2 인증 구성에 대한 자세한 내용은 전용 [Destination SDK OAuth 2 인증 페이지](oauth2-authorization.md)를 참조하십시오.

![OAuth 2 인증을 사용하여 UI 렌더링](../../assets/functionality/destination-configuration/oauth2-authentication-ui.png)

대상에 대해 [!DNL OAuth2] 인증을 설정하려면 아래와 같이 `customerAuthenticationConfigurations` 끝점을 통해 `/destinations` 섹션을 구성하십시오.
...
대상에 대해 [!DNL Amazon S3] 인증을 설정하려면 아래와 같이 `customerAuthenticationConfigurations` 끝점을 통해 `/destinations` 섹션을 구성하십시오.
...
대상에 대한 [!DNL Azure Blob] 인증을 설정하려면 아래와 같이 `customerAuthenticationConfigurations` 끝점에서 `/destinations` 매개 변수를 구성하십시오.
...
## 암호 인증이 있는 SFTP {#sftp-password-auth}
...
대상에 대해 암호를 사용하여 SFTP 인증을 설정하려면 아래와 같이 `customerAuthenticationConfigurations` 끝점에서 `/destinations` 매개 변수를 구성하십시오.
...
## SSH 키 인증이 있는 SFTP {#sftp-ssh-key-auth}

Experience Platform의 파일 기반 대상에 대해 [!DNL SFTP] 키를 사용하는 [!DNL SSH] 인증이 지원됩니다.
...
대상에 대해 SSH 키를 사용하여 SFTP 인증을 설정하려면 아래와 같이 `customerAuthenticationConfigurations` 끝점에서 `/destinations` 매개 변수를 구성하십시오.
...
[!DNL Google Cloud Storage] 인증 유형을 구성할 때 사용자는 [!DNL Google Cloud Storage] [!UICONTROL access key ID] 및 [!UICONTROL secret access key]을(를) 입력해야 합니다.

![Google 클라우드 저장소 인증을 사용하여 UI 렌더링](../../assets/functionality/destination-configuration/google-cloud-storage-ui.png)

대상에 대한 [!DNL Google Cloud Storage] 인증을 설정하려면 아래와 같이 `customerAuthenticationConfigurations` 끝점에서 `/destinations` 매개 변수를 구성하십시오.
