...
다음을 통해 파일 이름 지정 및 내보내기 예약 설정을 구성할 수 있습니다. `/authoring/destinations` 엔드포인트. 이 페이지에 표시된 구성 요소를 구성할 수 있는 자세한 API 호출 예는 다음 API 참조 페이지를 참조하십시오.
...
![매크로가 미리 선택된 파일 이름 구성 화면을 보여 주는 UI 이미지](../../assets/functionality/destination-configuration/file-name-configuration.png)
