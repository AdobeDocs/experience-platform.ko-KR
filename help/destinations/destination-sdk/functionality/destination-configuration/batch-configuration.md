...
| `defaultFrequency` | 열거형 | 기본 파일 내보내기 빈도를 정의합니다.지원되는 값:<ul><li>`ONCE`</li><li>`EVERY_3_HOURS`</li><li>`EVERY_6_HOURS`</li><li>`EVERY_8_HOURS`</li><li>`EVERY_12_HOURS`</li><li>`DAILY`</li><li>`WEEKLY`</li><li>`MONTHLY`</li></ul> 기본값은 `DAILY`입니다. |
...
파일 이름 구성 매크로를 사용하여 내보낸 파일 이름에 포함되어야 하는 내용을 정의합니다. 아래 표의 매크로는 [파일 이름 구성](../../../ui/activate-batch-profile-destinations.md#configure-file-names) 화면의 UI에 있는 요소를 설명합니다.
...
| 매크로 | UI 레이블 | 설명 | 예 |
|---|---|---|---|
| `DESTINATION` | [!UICONTROL Destination] | UI의 대상 이름입니다. | Amazon S3 |
| `SEGMENT_ID` | [!UICONTROL Segment ID] | Experience Platform에서 생성한 고유 대상 ID | ce5c5482-2813-4a80-99bc-57113f6acde2 |
| `SEGMENT_NAME` | [!UICONTROL Segment Name] | 사용자 정의 대상 이름 | VIP 구독자 |
| `DESTINATION_INSTANCE_ID` | [!UICONTROL Destination ID] | 대상 인스턴스의 Experience Platform 생성 고유 ID | 7b891e5f-025a-4f0d-9e73-1919e71da3b0 |
| `DESTINATION_INSTANCE_NAME` | [!UICONTROL Destination Name] | 대상 인스턴스의 사용자 정의 이름입니다. | 내 2022 Advertising 대상 |
| `ORGANIZATION_NAME` | [!UICONTROL Organization Name] | [!DNL Adobe Experience Platform]의 고객 조직 이름. | 내 조직 이름 |
| `SANDBOX_NAME` | [!UICONTROL Sandbox Name] | 고객이 사용하는 샌드박스의 이름. | prod |
| `DATETIME` / `TIMESTAMP` | [!UICONTROL Date and time] | `DATETIME`과(와) `TIMESTAMP`은(는) 모두 파일 생성 시기를 정의하지만 형식이 다릅니다. <br><br><ul><li>`DATETIME`은(는) YYYYMMDD_HHMMSS 형식을 사용합니다.</li><li>`TIMESTAMP`은(는) 10자리 Unix 형식을 사용합니다. </li></ul> `DATETIME`과(와) `TIMESTAMP`은(는) 함께 사용할 수 없으므로 동시에 사용할 수 없습니다. | <ul><li>`DATETIME`: 20220509_210543</li><li>`TIMESTAMP`: 1652131584</li></ul> |
| `CUSTOM_TEXT` | [!UICONTROL Custom text] | 파일 이름에 포함할 사용자 정의 사용자 정의 텍스트입니다. `defaultFilename`에서 사용할 수 없습니다. | My_Custom_Text |
| `TIMESTAMP` | [!UICONTROL Date and time] | 파일이 생성된 시간의 10자리 타임스탬프(Unix 형식)입니다. | 1652131584 |
| `MERGE_POLICY_ID` | [!UICONTROL Merge Policy ID] | 내보낸 대상을 생성하는 데 사용되는 [병합 정책](../../../../profile/merge-policies/overview.md)의 ID입니다. 내보낸 대상을 병합 정책에 따라 파일로 그룹화할 때 이 매크로를 사용합니다. 이 매크로를 `segmentGroupingEnabled:true`과(와) 함께 사용합니다. | e8591fdb-2873-4b12-b63e-15275b1c1439 |
| `MERGE_POLICY_NAME` | [!UICONTROL Merge Policy Name] | 내보낸 대상을 생성하는 데 사용되는 [병합 정책](../../../../profile/merge-policies/overview.md)의 이름입니다. 내보낸 대상을 병합 정책에 따라 파일로 그룹화할 때 이 매크로를 사용합니다. 이 매크로를 `segmentGroupingEnabled:true`과(와) 함께 사용합니다. | 내 사용자 지정 병합 정책 |

{style="table-layout:auto"}

### 파일 이름 구성 예 {#file-name-configuration-example}
