...
첫 번째 단계로 [!DNL Adobe Experience Platform]에 대한 시스템에서 앱을 만들거나 시스템에 Experience Platform을 등록해야 합니다. 목표는 대상에 Experience Platform을 인증하는 데 필요한 클라이언트 ID와 클라이언트 암호를 생성하는 것입니다.

시스템에서 이 구성의 일부로 아래 목록에서 가져올 수 있는 [!DNL Adobe Experience Platform] OAuth 2 리디렉션/콜백 URL이 필요합니다.
...
>[!IMPORTANT]
>
>시스템에서 [!DNL Adobe Experience Platform]에 대한 리디렉션/콜백 URL을 등록하는 단계는 인증 코드[ 부여 유형의 ](#authorization-code)OAuth2에만 필요합니다. 지원되는 다른 두 권한 유형(암호 및 클라이언트 자격 증명)의 경우 이 단계를 건너뛸 수 있습니다.
...
| `authorizationUrl` | 문자열 | 응용 프로그램으로 이동하도록 사용자를 리디렉션하는 인증 서버의 URL. |
| `refreshTokenUrl` | 문자열 | *선택 사항입니다.* 새로 고침 토큰을 발급하는 사용자 측의 URL입니다. 종종 `refreshTokenUrl`은(는) `accessTokenUrl`과(와) 같습니다. |
| `clientId` | 문자열 | 시스템이 [!DNL Adobe Experience Platform]에 할당하는 클라이언트 ID입니다. |
| `clientSecret` | 문자열 | 시스템이 [!DNL Adobe Experience Platform]에 할당하는 클라이언트 암호입니다. |
...
## 암호 부여가 있는 OAuth 2 {#oauth2-password-grant}
...
## 클라이언트 자격 증명 부여가 있는 OAuth 2 {#oauth2-client-credentials-grant}
...
이제 [!DNL Adobe Experience Platform]에서 지원하는 OAuth 2 권한 부여 패턴에 대해 이해하고 OAuth 2 권한 부여 지원을 통해 대상을 구성하는 방법에 대해 알아봅니다. 그런 다음 Destination SDK을 사용하여 OAuth 2 지원 대상을 설정할 수 있습니다. 다음 단계를 위해 [Destination SDK을 사용하여 대상 구성](../../guides/configure-destination-instructions.md)을 참조하십시오.
