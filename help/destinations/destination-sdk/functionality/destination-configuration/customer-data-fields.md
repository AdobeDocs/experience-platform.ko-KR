...
| `unique` | 부울 | 선택 사항입니다 | 사용자 조직에서 설정한 모든 대상 데이터 흐름에서 값이 고유해야 하는 고객 데이터 필드를 만들어야 할 때 이 매개 변수를 사용합니다. 예를 들어 **[!UICONTROL Integration alias]**&#x200B;사용자 지정 Personalization[ 대상의 ](../../../catalog/personalization/custom-personalization.md) 필드는 고유해야 합니다. 즉, 이 대상에 대한 두 개의 개별 데이터 흐름은 이 필드에 대해 동일한 값을 가질 수 없습니다. |
...
새 대상을 만들 때 Destination SDK은 Experience Platform UI의 대상 연결 화면에 **[!UICONTROL Name]** 및 **[!UICONTROL Description]** 필드를 자동으로 추가합니다. 위의 예에서 볼 수 있듯이 **[!UICONTROL Name]** 및 **[!UICONTROL Description]** 필드는 고객 데이터 필드 구성에 포함되지 않고 UI에서 렌더링됩니다.

>[!IMPORTANT]
>
>고객 데이터 필드 구성에 **[!UICONTROL Name]** 및 **[!UICONTROL Description]** 필드를 추가하면 사용자가 UI에서 중복되어 표시됩니다.

## 고객 데이터 필드 주문 {#ordering}

대상 구성에서 고객 데이터 필드를 추가하는 순서는 Experience Platform UI에 반영됩니다.

예를 들어 아래 구성은 UI에 따라 반영되며, 옵션은 **[!UICONTROL Name]**, **[!UICONTROL Description]**, **[!UICONTROL Bucket name]**, **[!UICONTROL Folder path]**, **[!UICONTROL File Type]**, **[!UICONTROL Compression format]** 순서로 표시됩니다.
...
이렇게 하려면 `"type": "object"`을(를) 사용하여 그룹을 만들고 아래 이미지에 표시된 대로 `properties` 그룹화가 강조 표시된 **[!UICONTROL CSV Options]** 개체 내에서 원하는 고객 데이터 필드를 수집하십시오.
...
**1단계.아래와 같이 동적 API 호출에 대한** 템플릿을 사용하여 [ ](../../authoring-api/destination-server/create-destination-server.md#dynamic-dropdown-servers)대상 서버를 만듭니다`responseFields`.
...
복잡한 통합 패턴에 대해 중첩된 고객 데이터 필드를 만들 수 있습니다. 이렇게 하면 고객에 대한 일련의 선택 항목을 연결할 수 있습니다.
...
보다 넓은 컨텍스트에서는 `conditional` 문자열 및 필드가 정의된 `fileType` 개체와 함께 아래 대상 구성에서 사용되는 `csvOptions` 필드를 볼 수 있습니다. 조건부 필드는 `properties` 매개 변수에 정의되어 있습니다.
