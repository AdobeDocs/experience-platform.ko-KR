...
## 필수 구성 요소 - [!DNL Adobe Experience Platform] 개념 {#prerequisites}

Adobe 측의 메시지 포맷, 프로필 구성 및 변환 프로세스를 이해하려면 다음 Experience Platform 개념을 숙지하십시오.
...
* **IdentityMap**. ID 맵은 [!DNL Adobe Experience Platform]의 모든 최종 사용자 ID의 맵을 나타냅니다. `xdm:identityMap`XDM 필드 사전[의 ](../../../../xdm/schema/field-dictionary.md)을(를) 참조하세요.
...
이 페이지에서는 [!DNL Adobe Experience Platform]에서 대상으로 내보낸 데이터의 메시지 형식 및 프로필 변형을 다룹니다.

[!DNL Adobe Experience Platform]은(는) 데이터를 다양한 데이터 형식의 상당한 수의 대상으로 내보냅니다. 대상 유형의 몇 가지 예로는 광고 플랫폼(Google), 소셜 네트워크(Facebook) 및 클라우드 저장소 위치(Amazon S3, Azure 이벤트 허브)가 있습니다.

Experience Platform은 측면에서 예상되는 포맷과 일치하도록 내보낸 프로필의 메시지 포맷을 조정할 수 있습니다. 이 사용자 지정을 이해하기 위해서는 다음 개념이 중요합니다.

* [!DNL Adobe Experience Platform]의 원본(1) 및 대상(2) XDM 스키마
* 파트너측의 예상 메시지 형식(3) 및
...
**Source XDM 스키마(1)**: 이 항목은 고객이 Experience Platform에서 사용하는 스키마를 참조합니다. Experience Platform에서 대상 활성화 워크플로의 [매핑 단계](../../../ui/activate-segment-streaming-destinations.md#mapping)에서 고객은 XDM 스키마의 필드를 대상의 대상 스키마에 매핑합니다(2).
...
프로필 변환 프로세스를 보여주기 위해 아래 예제에서는 [!DNL Adobe Experience Platform]에 있는 세 가지 공통 프로필 특성(**이름**, **성** 및 **이메일 주소**)을 사용합니다.

>[!NOTE]
>
>고객은 [!DNL Adobe Experience Platform]대상 워크플로우 활성화&#x200B;**의**&#x200B;매핑[ 단계에서 ](../../../ui/activate-segment-streaming-destinations.md#mapping) UI의 소스 XDM 스키마의 특성을 파트너 XDM 스키마에 매핑합니다.
...
페이지의 아래 예를 더 이해하려면 Experience Platform의 프로필 구조를 아는 것이 중요합니다.
...
Adobe은 [Jinja](https://pebbletemplates.io/)와(과) 유사한 템플릿 언어인 [페블 템플릿](https://jinja.palletsprojects.com/en/2.11.x/)을(를) 사용하여 Experience Platform XDM 스키마의 필드를 대상에서 지원하는 형식으로 변환합니다.
...
1. 간단한 변형 예. [프로필 특성](#attributes), [대상자 멤버십](#audience-membership) 및 [ID](#identities) 필드에 대한 간단한 변형을 사용하여 템플릿화가 작동하는 방식을 알아봅니다.
...
3. 집계 키를 포함하는 템플릿. 대상 구성에서 [구성 가능한 집계](../../functionality/destination-configuration/aggregation-policy.md#configurable-aggregation)를 사용하는 경우 Experience Platform은 대상 ID, 대상 네임스페이스, 대상 상태 또는 ID 네임스페이스와 같은 기준을 기반으로 대상으로 내보낸 프로필을 그룹화합니다.
...
>[!IMPORTANT]
>
>[!DNL Adobe Experience Platform]에서 사용 가능한 모든 프로필 특성 목록을 보려면 [XDM 필드 사전](../../../../xdm/schema/field-dictionary.md)을 참조하세요.
...
아래의 `json`은(는) [!DNL Adobe Experience Platform]에서 내보낸 데이터를 나타냅니다.
...
대상 구성에서 [구성 가능한 집계](../../functionality/destination-configuration/aggregation-policy.md#configurable-aggregation)를 사용하는 경우 대상 ID, 대상 네임스페이스, 대상 별칭, 대상 멤버십 또는 ID 네임스페이스와 같은 기준을 기반으로 대상으로 내보낸 프로필을 그룹화할 수 있습니다.
...
[구성 가능한 집계](../../functionality/destination-configuration/aggregation-policy.md#configurable-aggregation)를 사용하고 `includeSegmentId`을(를) true로 설정하면 대상으로 내보낸 HTTP 메시지의 프로필이 대상 ID별로 그룹화됩니다. 템플릿의 대상 ID 및 대상 네임스페이스에 액세스하는 방법은 아래를 참조하십시오.

**입력**

아래의 네 가지 프로필을 고려하십시오. 여기서

* 처음 두 개는 `788d8874-8007-4253-92b7-ee6b6c20c6f3` 네임스페이스 아래에 대상 ID가 `ups`인 대상에 속합니다
* 세 번째 프로필은 `8f812592-3f06-416b-bd50-e7831848a31a` 네임스페이스 아래에 대상 ID가 `CustomerAudienceUpload`인 대상의 일부입니다
* 네 번째 프로필은 각각 해당 네임스페이스 아래에 있는 위의 두 대상 모두에 속합니다.
...
`audienceId` 및 `audienceNamespace`이(가) 대상 ID 및 네임스페이스에 액세스하는 데 템플릿에서 어떻게 사용되는지 아래에 알아보십시오. 이 예제에서는 대상 분류에서 대상 멤버십에 `audienceId`을(를) 사용한다고 가정합니다. 자체 분류법에 따라 다른 필드 이름을 대신 사용할 수 있습니다.
...
프로필은 대상으로 내보낼 때 대상 ID 및 네임스페이스에 따라 두 개의 그룹으로 분할됩니다.
...
템플릿에 제공된 컨텍스트에는 `input`(이 호출에서 내보낸 프로필/데이터)과 `destination`(모든 프로필에 유효한 Adobe에서 데이터를 보내는 대상에 대한 데이터)이 포함되어 있습니다.
...
| `destination.namespaceSegmentAliases` | 특정 [!DNL Adobe Experience Platform] 네임스페이스의 대상 ID에서 파트너 시스템의 대상 별칭으로 매핑합니다. | `destination.namespaceSegmentAliases["ups"]["seg-id-1"]` |
| `destination.namespaceSegmentNames` | 특정 [!DNL Adobe Experience Platform] 네임스페이스의 대상 이름에서 파트너 시스템의 대상 이름으로 매핑합니다. | `destination.namespaceSegmentNames["ups"]["seg-name-1"]` |
| `destination.namespaceSegmentTimestamps` | 대상이 생성, 업데이트 또는 활성화된 시간을 UNIX 타임스탬프 형식으로 반환합니다. | <ul><li>`destination.namespaceSegmentTimestamps["ups"]["seg-id-1"].createdAt`: `seg-id-1` 네임스페이스에서 ID가 `ups`인 세그먼트를 만든 시간을 UNIX 타임스탬프 형식으로 반환합니다.</li><li>`destination.namespaceSegmentTimestamps["ups"]["seg-id-1"].updatedAt`: `seg-id-1` 네임스페이스에서 ID가 `ups`인 대상이 업데이트된 시간을 UNIX 타임스탬프 형식으로 반환합니다.</li><li>`destination.namespaceSegmentTimestamps["ups"]["seg-id-1"].mappingCreatedAt`: ID가 `seg-id-1`인 대상이 `ups` 네임스페이스에서 대상으로 활성화된 시간을 UNIX 타임스탬프 형식으로 반환합니다.</li><li>`destination.namespaceSegmentTimestamps["ups"]["seg-id-1"].mappingUpdatedAt`: 대상에서 대상 활성화가 업데이트된 시간을 UNIX 타임스탬프 형식으로 반환합니다.</li></ul> |
...
| `destination.segmentAliases` | **사용되지 않습니다.`destination.namespaceSegmentAliases`** 네임스페이스의 대상 ID에서 파트너 시스템의 대상 별칭에 대한 <br><br> [!DNL Adobe Experience Platform] 맵으로 대체되었습니다. | `destination.segmentAliases["seg-id-1"]` |
| `destination.segmentNames` | **사용되지 않습니다.`destination.namespaceSegmentNames`** 네임스페이스의 대상 이름에서 파트너 시스템의 대상 이름에 대한 <br><br> [!DNL Adobe Experience Platform] 맵으로 대체되었습니다. | `destination.segmentNames["seg-name-1"]` |
