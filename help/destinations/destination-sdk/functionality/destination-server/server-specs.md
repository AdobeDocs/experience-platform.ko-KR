...
대상 서버 사양은 [!DNL Adobe Experience Platform]에서 데이터를 받을 대상 플랫폼의 유형 및 Experience Platform과 대상 간의 통신 매개 변수를 정의합니다. 예:
...
### 하드 코딩된 필드와 템플릿화된 필드를 사용해야 하는 경우 {#when-to-use-hard-coded-vs-templatized}
...
이를 위해 서버 사양에서 대상 플랫폼 연결 매개변수를 하드 코딩할 수 있습니다. 대상 서버 구성에서 하드 코딩된 매개 변수 값을 사용하면 사용자의 입력 없이 [!DNL Adobe Experience Platform]과(와) 대상 플랫폼 간의 연결이 처리됩니다.
...
이 대상 서버 유형을 사용하면 HTTP 요청을 통해 [!DNL Adobe Experience Platform]의 데이터를 대상으로 내보낼 수 있습니다. 서버 구성에는 메시지를 받는 서버(사용자 측 서버)에 대한 정보가 포함되어 있습니다.
...
| `templatingStrategy` | 문자열 | *필수.* <ul><li>`PEBBLE_V1` 필드에서 하드 코딩된 값 대신 템플릿화된 필드를 사용하는 경우 `value`을(를) 사용합니다. 사용자가 Experience Platform UI에서 끝점 영역을 선택해야 하는 `https://api.moviestar.com/data/{{customerData.region}}/items`과(와) 같은 끝점이 있는 경우 이 옵션을 사용합니다. </li><li> Adobe 측에 템플릿 변환이 필요하지 않은 경우(예: `NONE`과 같은 끝점이 있는 경우) `https://api.moviestar.com/data/items`을(를) 사용하십시오. </li></ul> |
