...
이 페이지에서는 Destination SDK을 사용하여 [!DNL Azure Blob Storage] 사용자 지정 대상 [파일 서식 옵션](configure-file-formatting-options.md) 및 사용자 지정 [파일 이름 구성](../../functionality/destination-configuration/batch-configuration.md#file-name-configuration).
...
아래 페이로드에는 사용자 지정 Azure Blob 구성을 포함하는 일반 Azure Blob 구성이 포함되어 있습니다 [CSV 파일 형식](../../functionality/destination-server/file-formatting.md) 사용자가 Experience Platform UI에서 정의할 수 있는 구성 매개 변수입니다.
...
![선택한 대상 카드가 있는 대상 카탈로그 페이지를 표시하는 화면 기록](../../assets/guides/batch/blob-destination-card.gif)
...
이 문서를 읽으면 이제 사용자 정의 작성 방법을 알 수 있습니다 [!DNL Azure Blob Storage] Destination SDK 사용 대상. 다음으로, 팀이 [파일 기반 대상에 대한 활성화 워크플로우](../../../ui/activate-batch-profile-destinations.md) 데이터를 대상으로 내보내려면 다음을 수행하십시오.
