...
이 페이지에서는 Destination SDK을 사용하여 [!DNL Azure Blob Storage] 사용자 지정 대상 [파일 서식 옵션](../../server-and-file-configuration.md#file-configuration) 및 사용자 지정 [파일 이름 구성](../../file-based-destination-configuration.md#file-name-configuration).
...
을 사용하여 시작합니다 `/destination-server` 서버 및 파일 구성을 만드는 끝점입니다. HTTP 요청의 매개 변수에 대한 자세한 내용은 [파일 기반 대상에 대한 서버 및 파일 구성 사양](../../server-and-file-configuration.md#blob-example) 그리고 [파일 형식 구성](../../server-and-file-configuration.md#file-configuration).
...
아래 페이로드에는 사용자 지정 Azure Blob 구성을 포함하는 일반 Azure Blob 구성이 포함되어 있습니다 [CSV 파일 형식](../../server-and-file-configuration.md#file-configuration) 사용자가 Experience Platform UI에서 정의할 수 있는 구성 매개 변수입니다.
...
![선택한 대상 카드가 있는 대상 카탈로그 페이지를 표시하는 화면 기록](../../assets/blob-destination-card.gif)
