---
description: Destination SDK을 사용하여 사용자 지정 파일 이름 및 서식 옵션을 사용하여 Amazon S3 대상을 구성하는 방법에 대해 알아봅니다.
...
이 페이지에서는 Destination SDK을 사용하여 사용자 정의로 Amazon S3 대상을 구성하는 방법을 설명합니다 [파일 서식 옵션](/help/destinations/destination-sdk/server-and-file-configuration.md#file-configuration) 및 사용자 지정 [파일 이름 구성](/help/destinations/destination-sdk/file-based-destination-configuration.md#file-name-configuration).

이 페이지에는 Amazon S3 대상에 사용할 수 있는 모든 구성 옵션이 표시됩니다. 아래 단계에 표시된 구성을 편집하거나 필요에 따라 구성의 특정 부분을 삭제할 수 있습니다.

## 전제 조건 {#prerequisites}

아래 설명된 단계로 진행하기 전에 다음을 읽어 보십시오. [Destination SDK 시작](/help/destinations/destination-sdk/getting-started.md) Destination SDK API를 사용하여 작업하는 데 필요한 Adobe I/O 인증 자격 증명 및 기타 전제 조건을 얻는 방법에 대한 자세한 내용을 보려면 페이지 를 참조하십시오.

## 1단계: 서버 및 파일 구성 만들기 {#create-server-file-configuration}

를 사용하여 시작 `/destination-server` 서버 및 파일 구성을 만드는 끝점입니다. HTTP 요청의 매개 변수에 대한 자세한 설명은 [파일 기반 대상에 대한 서버 및 파일 구성 사양](/help/destinations/destination-sdk/server-and-file-configuration.md#s3-example) 및 관련 항목 [파일 형식 지정 구성](/help/destinations/destination-sdk/server-and-file-configuration.md#file-configuration).
...
아래 페이로드에는 사용자 지정 기능이 있는 일반 Amazon S3 구성이 포함됩니다 [CSV 파일 형식 지정](/help/destinations/destination-sdk/server-and-file-configuration.md#file-configuration) 사용자가 Experience Platform UI에서 정의할 수 있는 구성 매개 변수입니다.
...
* [일괄 처리 대상 구성](/help/destinations/destination-sdk/file-based-destination-configuration.md#batch-configuration)
...
![선택한 대상 카드가 있는 대상 카탈로그 페이지를 보여 주는 화면 레코딩입니다.](../../assets/destination-card.gif)
...
대상을 구성한 후 [대상 게시 API](/help/destinations/destination-sdk/destination-publish-api.md) Adobe에 구성을 제출하여 검토합니다.
...
ISV(Independent Software Vendor) 또는 SI(System Integrator)가 [제품화 통합](/help/destinations/destination-sdk/overview.md#productized-custom-integrations), 사용 [셀프서비스 설명서 프로세스](/help/destinations/destination-sdk/docs-framework/documentation-instructions.md) 에서 대상에 대한 제품 설명서 페이지를 만들려면 [Experience Platform 대상 카탈로그](/help/destinations/catalog/overview.md).

## 다음 단계 {#next-steps}

이제 이 문서를 읽고 사용자 지정을 작성하는 방법을 알 수 있습니다 [!DNL Amazon S3] 대상(Destination SDK 사용) 다음으로, 팀은 [파일 기반 대상에 대한 활성화 워크플로](/help/destinations/ui/activate-batch-profile-destinations.md) 데이터를 대상으로 내보냅니다.
