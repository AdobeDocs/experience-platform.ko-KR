...
이 페이지에서는 Destination SDK을 사용하여 사용자 정의로 파일 기반 대상을 구성하는 방법에 대해 설명합니다 [파일 서식 옵션](configure-file-formatting-options.md) 및 사용자 지정 [파일 이름 구성](../../functionality/destination-configuration/batch-configuration.md#file-name-configuration) 내보내기 [잠재 고객](/help/destinations/ui/activate-prospect-audiences.md). 이 안내서의 예제에서는 잠재 고객 프로필 대상을 Amazon S3 위치로 내보내는 방법을 설명합니다.

STFP 또는 기타 스토리지 위치를 설정하여 잠재 고객을 내보낼 수도 있습니다. 기억해야 할 중요한 부분은 아래의 코드 조각을 의 대상 구성에 추가하는 것입니다. [2단계](#create-destination-configuration) 을(를) 활성화하려면 [잠재 고객을 내보내는 워크플로우](/help/destinations/ui/activate-prospect-audiences.md) 대상에.
...
아래 설명된 단계로 진행하기 전에 다음을 읽어 보십시오. [Destination SDK 시작](../../getting-started.md) Destination SDK API를 사용하여 작업하는 데 필요한 인증 자격 증명 및 기타 전제 조건을 얻는 방법에 대한 자세한 내용은 페이지를 참조하십시오.
...
아래 페이로드에는 사용자 지정 기능이 있는 일반 Amazon S3 구성이 포함됩니다 [CSV 파일 형식 지정](../../functionality/destination-server/file-formatting.md) 사용자가 Experience Platform UI에서 정의할 수 있는 구성 매개 변수입니다.
...
![대상 세부 사항 입력](../../assets/guides/batch/file-configuration-options.gif)

내보내기 간격을 예약할 때 에서 설정한 필드가 어떻게 표시되는지 확인합니다. `batchConfig` 구성.
![내보내기 예약 옵션](../../assets/guides/batch/ui-view-scheduling-prospect-destination.png)
...
이제 이 문서를 읽고 Destination SDK을 사용하여 사용자 지정을 작성하는 방법을 알 수 있습니다 [!DNL Amazon S3] 잠재 고객을 내보낼 대상.