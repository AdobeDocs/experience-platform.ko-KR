...
# 미리 정의된 파일 형식 옵션 및 사용자 지정 파일 이름 구성을 사용하여 [!DNL Amazon S3] 대상 구성

## 개요 {#overview}

이 페이지에서는 Destination SDK을 사용하여 사전 정의된 기본 [파일 형식 옵션](configure-file-formatting-options.md) 및 사용자 지정 [파일 이름 구성](../../functionality/destination-configuration/batch-configuration.md#file-name-configuration)으로 Amazon S3 대상을 구성하는 방법에 대해 설명합니다.

이 페이지에는 [!DNL Amazon S3] 대상에 사용할 수 있는 모든 구성 옵션이 표시됩니다. 아래 단계에 표시된 구성을 편집하거나 필요에 따라 구성의 특정 부분을 삭제할 수 있습니다.
...
아래 페이로드에는 사용자가 Experience Platform UI에서 정의할 수 있는 사전 정의된 기본 [CSV 파일 형식 지정](../../functionality/destination-server/file-formatting.md) 구성 매개 변수와 함께 일반 [!DNL Amazon S3] 구성이 포함되어 있습니다.
...
[단계 1](#create-server-file-configuration)의 서버 구성을 이 대상 구성에 연결하려면 아래 API 요청의 `destinationServerId` 값을 [단계 1](#create-server-file-configuration)에서 대상 서버를 만들 때 얻은 `instanceId` 값으로 바꾸십시오.
...
![구성에 정의된 고객 데이터 필드를 보여 주는 화면 기록입니다.](../../assets/guides/batch/file-configuration-options.gif)
...
이 문서를 읽으면 이제 Destination SDK을 사용하여 사용자 지정 [!DNL Amazon S3] 대상을 작성하는 방법을 알 수 있습니다. 그런 다음 팀은 [파일 기반 대상에 대한 활성화 워크플로](../../../ui/activate-batch-profile-destinations.md)를 사용하여 데이터를 대상으로 내보낼 수 있습니다.
