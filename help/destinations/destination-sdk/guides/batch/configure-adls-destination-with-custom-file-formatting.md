---
description: Destination SDK을 사용하여 사용자 지정 파일 형식 옵션 및 사용자 지정 파일 이름 구성으로 Azure Data Lake 저장소 대상을 구성하는 방법을 알아봅니다.
title: 사용자 지정 파일 형식 옵션 및 사용자 지정 파일 이름 구성으로 Azure Data Lake 저장소 대상을 구성합니다.
...
# 구성 [!DNL Azure Data Lake Storage] 사용자 지정 파일 형식 옵션 및 사용자 지정 파일 이름 구성이 있는 대상

## 개요 {#overview}

이 페이지에서는 Destination SDK을 사용하여 [!DNL Azure Data Lake Storage] 사용자 지정 대상 [파일 서식 옵션](configure-file-formatting-options.md) 및 사용자 지정 [파일 이름 구성](../../functionality/destination-configuration/batch-configuration.md#file-name-configuration).

이 페이지에는 Azure Data Lake 저장소 대상에 사용할 수 있는 모든 구성 옵션이 표시됩니다. 아래 단계에 표시된 구성을 편집하거나 필요에 따라 구성의 특정 부분을 삭제할 수 있습니다.

아래에 사용되는 매개 변수에 대한 자세한 설명은 를 참조하십시오 [대상 SDK의 구성 옵션](../../functionality/configuration-options.md).
...
을 사용하여 시작합니다 `/destination-server` 엔드포인트 [서버 및 파일 구성 만들기](../../authoring-api/destination-server/create-destination-server.md).
...
아래 페이로드에는 일반 항목이 포함되어 있습니다 [!DNL Azure Data Lake Storage] 구성, 사용자 지정 사용 [CSV 파일 형식](../../functionality/destination-server/file-formatting.md) 사용자가 Experience Platform UI에서 정의할 수 있는 구성 매개 변수입니다.
...
![선택한 대상 카드가 있는 대상 카탈로그 페이지를 표시하는 화면 기록](../../assets/guides/batch/adls-destination-card.gif)

아래 이미지 및 레코딩에서 [파일 기반 대상에 대한 활성화 워크플로우](../../../ui/activate-batch-profile-destinations.md) 대상 구성에서 선택한 옵션과 일치시킵니다.
...
![대상 세부 사항 채우기](../../assets/guides/batch/file-configuration-options.gif)

내보내기 간격을 예약할 때 표시되는 필드가 `batchConfig` 구성.
![예약 옵션 내보내기](../../assets/guides/batch/file-export-scheduling.png)

파일 이름 구성 옵션을 볼 때 표시되는 필드가 `filenameConfig` 구성에 설정한 옵션입니다.
![파일 구성 옵션](../../assets/guides/batch/file-naming-options.gif)
...
대상을 구성한 후 [대상 게시 API](../../publishing-api/create-publishing-request.md) 검토를 위해 Adobe에 구성을 제출합니다.
...
이 문서를 읽으면 이제 사용자 정의 작성 방법을 알 수 있습니다 [!DNL Azure Data Lake Storage] Destination SDK 사용 대상. 다음으로, 팀이 [파일 기반 대상에 대한 활성화 워크플로우](../../../ui/activate-batch-profile-destinations.md) 데이터를 대상으로 내보내려면 다음을 수행하십시오.
