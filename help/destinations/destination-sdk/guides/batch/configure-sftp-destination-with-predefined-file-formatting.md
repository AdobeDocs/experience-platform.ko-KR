...
이 페이지에서는 Destination SDK을 사용하여 사전 정의된 기본값으로 SFTP 대상을 구성하는 방법에 대해 설명합니다 [파일 서식 옵션](configure-file-formatting-options.md) 및 사용자 지정 [파일 이름 구성](../../functionality/destination-configuration/batch-configuration.md#file-name-configuration).
...
아래 페이로드에는 사전 정의된 기본값이 있는 일반 SFTP 구성이 포함됩니다 [CSV 파일 형식 지정](../../functionality/destination-server/file-formatting.md) 사용자가 Experience Platform UI에서 정의할 수 있는 구성 매개 변수입니다.
...
이 문서를 읽으면 이제 Destination SDK을 사용하여 사용자 지정 SFTP 대상을 작성하는 방법을 알 수 있습니다. 다음으로, 팀은 [파일 기반 대상에 대한 활성화 워크플로](../../../ui/activate-batch-profile-destinations.md) 데이터를 대상으로 내보냅니다.
