...
이 페이지에서는 [대상 SDK](../functionality/configuration-options.md)의 구성 옵션 및 기타 Destination SDK 기능 및 API 참조 문서의 정보를 사용하여 [스트리밍 대상](../../destination-types.md#streaming-destinations)을 구성하는 방법에 대해 설명합니다. 단계는 아래에 순서대로 나열되어 있습니다.
...
![Destination SDK 엔드포인트를 사용하는 단계](../assets/guides/destination-sdk-steps.png)
...
>[!IMPORTANT]
>
>올바르게 구성된 실시간(스트리밍) 대상을 만들려면 *아래 그림과 같이*&#x200B;에 하나 이상의 대상 ID를 추가해야`identityNamespaces`합니다. 대상 ID가 구성되어 있지 않으면 사용자는 활성화 워크플로의 [매핑 단계](../../ui/activate-segment-streaming-destinations.md#mapping)을(를) 지나서 진행할 수 없습니다.
...
대상이 지원하는 페이로드를 기반으로, Adobe XDM 형식에서 내보낸 데이터의 형식을 대상에서 지원하는 형식으로 변환하는 템플릿을 만들어야 합니다. [ID, 특성 및 대상 멤버십 변환에 템플릿 언어 사용](../functionality/destination-server/message-format.md#using-templating) 섹션의 템플릿 예제를 참조하고 Adobe에서 제공하는 [템플릿 작성 도구](../testing-api/streaming-destinations/create-template.md)를 사용하십시오.
