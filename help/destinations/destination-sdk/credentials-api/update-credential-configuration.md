---
description: 이 페이지에서는 Adobe Experience Platform Destination SDK을 통해 기존 자격 증명 구성을 업데이트하는 데 사용되는 API 호출을 보여줍니다.
title: 자격 증명 구성 업데이트
...
이 페이지에서는 기존 자격 증명 구성을 업데이트하는 데 사용할 수 있는 API 요청 및 페이로드를 `/authoring/credentials` API 엔드포인트.
...
을(를) 업데이트할 수 있습니다 [기존](create-credential-configuration.md) 자격 증명 구성 `PUT` 에 요청 `/authoring/credentials` 업데이트된 페이로드가 있는 종단점입니다.

기존 자격 증명 구성 및 해당 자격 증명을 가져오려면 `{INSTANCE_ID}`에 대한 자세한 내용은 [자격 증명 구성 검색](retrieve-credential-configuration.md).
...
| 매개변수 | 설명 |
| -------- | ----------- |
| `{INSTANCE_ID}` | 업데이트할 자격 증명 구성의 ID입니다. 기존 자격 증명 구성 및 해당 자격 증명을 가져오려면 `{INSTANCE_ID}`를 참조하십시오. [자격 증명 구성 검색](retrieve-credential-configuration.md). |

다음 요청은 페이로드에 제공된 매개 변수로 정의된 기존 자격 증명 구성을 업데이트합니다.
...
성공한 응답은 업데이트된 자격 증명 구성의 세부 정보와 함께 HTTP 상태 200을 반환합니다.
...
| -------- | ----------- | ----------- |
| `username` | 문자열 | 자격 증명 구성 로그인 사용자 이름 |
| `sshKey` | 문자열 | [!DNL SSH] 키 [!DNL SFTP] with [!DNL SSH] 인증 |
...
| `servicePrincipalId` | 문자열 | [!DNL Azure Service Principal] 용 ID [!DNL Azure Data Lake Storage] |
