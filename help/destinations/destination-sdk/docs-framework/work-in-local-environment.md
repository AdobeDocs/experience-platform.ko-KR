...
1. 설명서 페이지는 [셀프서비스 대상 템플릿](./self-service-template.md). 다운로드 [대상 템플릿](assets/yourdestination-template.zip). 압축 풀기 및 파일 추출 `yourdestination-template.md` 위의 4단계에서 언급된 디렉터리로 이동합니다.  파일 이름 바꾸기 `YOURDESTINATION.md`여기서 YOURDESTINATION은 Adobe Experience Platform의 대상 이름입니다. 예를 들어 회사의 이름이 Moviestar인 경우 파일 이름을 지정합니다 `moviestar.md`.
...
![새 로컬 분기](./assets/new-branch-local.gif)
