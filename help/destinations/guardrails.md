...
| **시스템 적용 보호 기능(하드 제한)** | [!DNL Real-Time CDP] UI 또는 API에 의해 시스템 적용 가드레일이 적용됩니다. 이는 UI 및 API가 귀하를 차단하거나 오류를 반환하므로 초과할 수 없는 제한입니다. |
...
| 지정된 시간에 내보낼 수 있는 최대 대상 수 | 100 | 성능 보호 | 최대 100개의 대상을 대상 인스턴스를 일괄 처리하는 것이 좋습니다. |
...
| 대상 인스턴스에서 활성화할 수 있는 최대 외부 대상 수(예: FAC, 사용자 지정 업로드, 대상 구성) | 20 | 시스템 강제 보호 | 파일 기반 대상을 일괄 처리하는 외부 대상(예: [Federated Audience Composition](/help/segmentation/ui/audience-portal.md#fac), [사용자 지정 업로드](/help/segmentation/ui/audience-portal.md#import-audience), [Audience Composition](/help/segmentation/ui/audience-portal.md#audience-composition))을 활성화할 때 대상 인스턴스에서 활성화할 수 있는 대상은 20명으로 제한됩니다. 이러한 대상 유형에 대한 자세한 내용은 [대상 유형 및 사용자 지정](/help/segmentation/ui/audience-portal.md#customize)을 참조하십시오. [파일 기반 대상을 일괄 처리할 외부 대상을 활성화](/help/destinations/ui/activate-batch-profile-destinations.md#select-audiences)하는 워크플로에 대해 자세히 알아보십시오. |
...
| 가드레일 | 제한 | 제한 유형 | 설명 |
| --- | --- | --- | --- |
| 최대 [사용자 지정 개인화](/help/destinations/catalog/personalization/custom-personalization.md) 대상 수 | 10 | 성능 보호 | 샌드박스당 최대 10개의 사용자 지정 개인화 대상 인스턴스를 설정할 수 있습니다. |
| 샌드박스당 개인화 대상에 매핑된 최대 속성 수 | 30 | 성능 보호 | 샌드박스당 개인화 대상 인스턴스에 최대 30개의 속성을 매핑할 수 있습니다. |
...
#### 예약된 데이터 세트 내보내기 보호 {#scheduled-dataset-exports}
...
| **XDM 경험 이벤트 스키마 외의 모든 스키마**&#x200B;를 기반으로 하는 데이터 세트 | 대상 인스턴스의 내보낸 모든 파일에 대해 100억 개의 레코드 | 시스템 강제 보호 | 데이터 세트의 기록 수는 압축된 JSON 또는 Parquet 파일의 경우 100억 개 미만이어야 하고, 압축되지 않은 Parquet 파일의 경우 100만 개 미만이어야 합니다. 그렇지 않으면 내보내기에 실패합니다. 허용된 임계값보다 큰 경우 내보내려는 데이터 세트의 크기를 줄이십시오. |
...
| 가드레일 | 제한 | 제한 유형 | 설명 |
| --- | --- | --- | --- |
| 최대 [개인 사용자 지정 대상 수](/help/destinations/destination-sdk/overview.md#productized-custom-integrations) | 샌드박스당 5개 | 성능 보호 | Destination SDK을 사용하여 샌드박스당 최대 5개의 개인 사용자 지정 스트리밍 또는 배치 대상을 만들 수 있습니다. 이러한 대상을 5개 이상 만들어야 하는 경우 사용자 정의 지원 담당자에게 문의하십시오. |
...
| 대상 유형 | 설명 |
| --- | --- |
| 엔터프라이즈 대상(HTTP API, Amazon Kinesis, Azure EventHubs) | 그 중 95% 동안 Experience Platform은 각 엔터프라이즈 대상 인스턴스에 대해 초당 1만 개 미만의 요청으로 성공적으로 전송된 메시지에 대해 10분 미만의 처리량 지연 시간을 제공하려고 시도합니다. <br> Enterprise 대상에 대한 요청이 실패한 경우 Experience Platform은 실패한 요청을 저장하고 두 번 다시 시도하여 끝점에 요청을 보냅니다. |
