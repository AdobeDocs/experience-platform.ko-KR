...
이 자습서에서는 API 호출을 사용하여 [!DNL Adobe Experience Platform] 데이터에 연결하고, 스트리밍 클라우드 스토리지 대상([Amazon Kinesis](../catalog/cloud-storage/amazon-kinesis.md) 또는 [Azure Event Hubs](../catalog/cloud-storage/azure-event-hubs.md))에 연결하고, 새로 만든 대상에 대한 데이터 흐름을 만들고, 새로 만든 대상에 데이터를 활성화하는 방법을 보여 줍니다.
...
### Experience Platform에서 데이터에 대한 액세스 권한 인증 {#authorize-access-experience-platform}
...
### 스트리밍 대상에 대한 액세스 권한 부여 {#authorize-access-streaming-destination}
...
## 데이터 흐름 유효성 검사 {#validate-data-flow}
...
* 이 문서의 지침에 따라 [!DNL API]에서 [!DNL Postman] 요청을 실행합니다.
...
선호하는 스트리밍 대상 중 하나에 Experience Platform을 연결하고 각 대상에 대한 데이터 흐름을 설정했습니다. 이제 나가는 데이터를 고객 분석 또는 수행하려는 기타 데이터 작업을 위한 대상에서 사용할 수 있습니다. 자세한 내용은 다음 페이지를 참조하십시오.
