...
* 임시 활성화 각 작업은 최대 20개의 세그먼트를 활성화할 수 있습니다. 작업당 20개 이상의 세그먼트를 활성화하려고 하면 작업이 실패합니다.
...
| 속성 | 설명 |
| -------- | ----------- |
| `code` | API 응답 코드. 성공적인 호출이 반환됩니다 `DEST-ADH-200` (상태 코드 200), 형식이 잘못 지정된 항목이 반환됩니다 `DEST-ADH-400` (상태 코드 400). |
| `message` | API에서 반환된 성공 또는 오류 메시지입니다. |
| `statusURLs` | 활성화 흐름의 상태 URL입니다. 를 사용하여 흐름 진행률을 추적할 수 있습니다 [Flow Service API](../../sources/tutorials/api/monitor.md). |
