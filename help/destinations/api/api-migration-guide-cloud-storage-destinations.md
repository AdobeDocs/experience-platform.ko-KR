...
세 대상 모두의 프로필 선택기 단계는 원하는 경우 내보낸 파일의 열 헤더 이름을 바꾸는 매핑 단계로 대체됩니다. 이전 속성 선택기 단계 왼쪽과 새 매핑 단계 오른쪽이 있는 아래 나란히 표시되는 이미지를 참조하십시오.
...
이제 클라우드 스토리지 대상 마이그레이션을 준비하기 위해 어떤 조치도 취해야 하는지 알 수 있습니다. Experience Platform에서 원하는 클라우드 스토리지 대상으로 파일을 내보내도록 API 기반 워크플로를 설정할 때 참조할 설명서 페이지를 알 수도 있습니다. 다음으로 API 자습서를 보고 [클라우드 저장소 대상으로 데이터 내보내기](/help/destinations/api/activate-segments-file-based-destinations.md)를 할 수 있습니다.
