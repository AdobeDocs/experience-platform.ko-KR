...
>[!IMPORTANT]
>
>* 이 페이지에 설명된 기능은 [!DNL Real-Time CDP] Prime 및 Ultimate 패키지를 구입한 고객에게 제공됩니다. 더 많은 내용은 Adobe 담당자에게 문의하십시오.

## 마이그레이션 컨텍스트 {#migration-context}

[2022년 10월](/help/release-notes/2022/october-2022.md#new-or-updated-destinations)부터 새 파일 내보내기 기능을 사용하여 Experience Platform에서 파일을 내보낼 때 향상된 사용자 지정 기능에 액세스할 수 있습니다.
...
기능이 향상된 이 대상은 처음에 베타 버전으로 제공되었지만 *Adobe은 이제 모든 [!DNL Real-Time CDP] 고객을 새 클라우드 저장소 대상으로 이동*&#x200B;합니다. 이미 [!DNL Amazon S3], [!DNL Azure Blob] 또는 SFTP를 사용 중인 고객의 경우 기존 데이터 흐름이 새 카드로 마이그레이션됩니다. 마이그레이션의 일부인 특정 변경 내용에 대한 자세한 내용은 계속 읽어보십시오.
...
Experience Platform에서 내보낸 파일 위에 [!DNL Amazon S3], [!DNL Azure Blob] 또는 SFTP 클라우드 저장소 위치에서 스크립트가 실행 중인 경우 매핑 단계와 관련하여 새 카드의 연결 및 흐름 사양에 대한 일부 매개 변수가 변경되고 있음을 알아두십시오.
...
+++[!DNL base connection]에 대한 레거시 [!DNL Amazon S3] 보기
...
+++[!DNL target connection]에 대한 레거시 [!DNL Amazon S3] 보기
...
+++[!DNL base connection]에 대한 새 [!DNL Amazon S3] 보기
...
+++[!DNL target connection]에 대한 새 [!DNL Amazon S3] 보기
...
+++[!DNL base connection]에 대한 레거시 [!DNL Azure Blob] 보기
...
+++[!DNL target connection]에 대한 레거시 [!DNL Azure Blob] 보기
...
+++[!DNL base connection]에 대한 새 [!DNL Azure Blob] 보기
...
+++[!DNL target connection]에 대한 새 [!DNL Azure Blob] 보기
...
+++[!DNL base connection] 인증에 대한 레거시 [!DNL SFTP - SSH key] 보기
...
+++[!DNL base connection]에 대한 새 [!DNL SFTP - password authentication] 보기
...
+++[!DNL base connection] 인증에 대한 새 [!DNL SFTP - SSH key] 보기
...
세 대상 모두의 프로필 선택기 단계는 매핑 단계로 대체되며, 원할 경우 내보낸 파일의 열 헤더 이름을 변경할 수 있습니다. 이전 속성 선택기 단계 왼쪽과 새 매핑 단계 오른쪽이 있는 아래 나란히 표시되는 이미지를 참조하십시오.
...
데이터를 클라우드 저장소 대상으로 내보내기`profileMapping`하기 위한 [API 자습서에서 &#x200B;](/help/destinations/api/activate-segments-file-based-destinations.md#attribute-and-identity-mapping) 개체 설정에 대한 전체 정보를 확인하십시오.
...
이제 이 페이지를 읽고 클라우드 스토리지 대상의 마이그레이션을 준비하기 위해 어떤 조치도 취해야 하는지 알 수 있습니다. Experience Platform에서 원하는 클라우드 스토리지 대상으로 파일을 내보내도록 API 기반 워크플로를 설정할 때 참조할 설명서 페이지를 알 수도 있습니다. 다음으로 API 자습서를 보고 [클라우드 저장소 대상으로 데이터 내보내기](/help/destinations/api/activate-segments-file-based-destinations.md)를 할 수 있습니다.
