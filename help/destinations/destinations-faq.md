...
* **[!DNL Adobe Experience Cloud]** 비즈니스 계정을 [!DNL Facebook Ad Account]의 광고 파트너로 추가해야 합니다. `business ID=206617933627973` 사용. 자세한 내용은 Facebook 설명서의 [비즈니스 관리자에 파트너 추가](https://www.facebook.com/business/help/1717412048538897)를 참조하십시오.

  >[!IMPORTANT]
  >
  > [!DNL Adobe Experience Cloud]에 대한 권한을 구성할 때는 **캠페인 관리** 권한을 활성화해야 합니다. [!DNL Adobe Experience Platform] 통합에 필요합니다.
...
### Facebook에서 [!DNL Adobe Experience Platform]의 정보를 처리하는 데 시간이 얼마나 걸립니까? {#facebook-processing-time}
...
### [!DNL Adobe Experience Platform]과(와) [!DNL Facebook Custom Audiences] 통합은 더 이상 자격이 없는 사용자가 대상에서 자격을 박탈하는 것을 지원합니까? {#facebook-disqualify-users}
...
## [!DNL Adobe Target] 및 사용자 지정 Personalization 대상을 통한 동일 페이지 및 다음 페이지 개인화 {#same-next-page-personalization}

### Experience Platform Web SDK을 사용하여 대상자와 특성을 [!DNL Adobe Target]&#x200B;(으)로 보내야 합니까? {#target-web-sdk}
...
### Real-time Customer Data Platform에서 [!DNL Adobe Target] 또는 사용자 지정 Personalization 대상으로 보낼 수 있는 특성 수에 제한이 있습니까? {#target-attributes-limit}

+++답변
예. 동일 페이지 및 다음 페이지 개인화 사용 사례는 대상을 [!DNL Adobe Target] 또는 사용자 지정 Personalization 대상으로 활성화할 때 샌드박스당 최대 30개의 특성을 지원합니다. 활성화 보호 기능에 대한 자세한 내용은 [보호 기능 설명서](guardrails.md#edge-destinations-activation)를 참조하세요.
+++
...
### [!DNL Adobe Target] 대상을 삭제하면 어떻게 됩니까? {#delete-target-destination}

+++답변
대상을 삭제하면 대상 아래에 매핑된 모든 대상 및 특성이 [!DNL Adobe Target]에서 삭제되며, Edge Network에서도 제거됩니다.
+++
...
### 에지 상에서 활성 상태인 병합 정책을 하나만 가질 수 있습니다. 다른 병합 정책을 사용하는 대상을 빌드하고 [!DNL Adobe Target]에 스트리밍 대상으로 보낼 수 있습니까? {#edge-merge-policy}

+++답변
아니요. [!DNL Adobe Target]에 활성화하려는 모든 대상은 Active-On-Edge [병합 정책](../profile/merge-policies/ui-guide.md)을 사용해야 합니다.
+++
