...
기본적으로 [!DNL Adobe Experience Platform Edge Network]은(는) 데이터 스트림에 도달하는 모든 이벤트를 데이터 스트림에 대해 활성화한 모든 [!DNL Experience Cloud] [서비스](/help/datastreams/configure.md#add-services)에 보냅니다. 사용 사례에 따라 이상적인 워크플로가 아닐 수도 있습니다.

동적 데이터 스트림 구성은 데이터 스트림에 대해 활성화된 각 서비스에 대해 정의하는 규칙 집합을 통해 이 문제를 해결합니다. 이 규칙은 각 데이터 형식을 받는 [!DNL Experience Cloud] 솔루션을 제어합니다.

## 전제 조건 {#prerequisites}

데이터 스트림에 대한 동적 구성을 만들려면 다음 두 가지 조건을 충족해야 합니다.

* 작업할 데이터 스트림을 *최소*&#x200B;개 만들어야 합니다. 자세한 내용은 [데이터 스트림을 만드는 방법](/help/datastreams/configure.md)에 대한 설명서를 참조하십시오.
* 데이터 스트림에 *최소*&#x200B;개의 [!DNL Experience Cloud] 서비스가 추가되어 있어야 합니다. 자세한 내용은 데이터스트림에 [서비스를 추가](/help/datastreams/configure.md#add-services)하는 방법에 대한 설명서를 참조하십시오.
...
| [!DNL Adobe Analytics]에 대한 데이터 스트림당 최대 동적 데이터 스트림 구성 수 | 5 | 성능 보호 |
| [!DNL Adobe Target]에 대한 데이터 스트림당 최대 동적 데이터 스트림 구성 수 | 5 | 성능 보호 |
| [!DNL Adobe Audience Manager]에 대한 데이터 스트림당 최대 동적 데이터 스트림 구성 수 | 5 | 성능 보호 |
...
동적 데이터 스트림 구성 및 [데이터 스트림 구성 재정의](/help/datastreams/overrides.md)는 함께 사용할 수 없는 기능입니다.

데이터 스트림 구성 재정의와 함께 동적 데이터 스트림 구성을 사용할 수 없습니다. 둘 중 하나를 선택해야 합니다.

둘 다 활성화하면 구성 무시가 우선하며 시스템은 동적 데이터스트림 구성 규칙을 무시합니다.
...
1. 데이터를 각 서비스로 전송할지 여부에 따라 **[!UICONTROL Configuration]** 섹션에서 각 규칙에 대한 서비스를 활성화하거나 비활성화합니다. 서비스를 사용하지 않도록 설정하면 라우팅이 사용되지 않고 *데이터가 다운스트림 서비스로 전송되지 않습니다*.
...
![끌어서 놓기를 사용하여 동적 데이터스트림 규칙을 다시 정렬합니다.](assets/configure-dynamic-datastream/move-rules.gif)
...
| **논리** | `INCLUDE`, `ANY/ALL`([!DNL AND]/[!DNL OR]에 해당) |

>[!NOTE]
>
>**[!UICONTROL EXCLUDE]** 연산자는 직접 지원되지 않지만, **[!UICONTROL INCLUDE]**&#x200B;을(를) 사용하여 무시된 비교 연산자(예: &quot;같지 않음&quot;)와 동등한 논리를 얻을 수 있습니다.
...
**단순 식만 사용**. 규칙을 단순 논리 표현식으로 정의해야 합니다. 중첩된 논리 식([!DNL AND]/[!DNL OR]의 컨테이너 또는 여러 수준 사용)은 지원되지 않습니다. 복잡한 논리가 필요한 경우 여러 플랫 규칙으로 나눕니다.

예를 들어 다음 복잡한 규칙을 생각해 보십시오.

![여러 AND/OR 조건을 가진 중첩 복합 규칙의 예입니다.](assets/configure-dynamic-datastream/complex-rule.png)

이 규칙을 다음과 같은 간단한 규칙으로 나눌 수 있습니다.

![중첩된 복합 규칙을 바꾸는 첫 번째 간소화된 규칙입니다.](assets/configure-dynamic-datastream/simple-rule-1.png)

![중첩된 복합 규칙을 바꾸는 두 번째 간소화된 규칙입니다.](assets/configure-dynamic-datastream/simple-rule-2.png)
...
* **규칙을 단순하고 평평하게 유지하십시오.** 복잡한 논리를 표현해야 하는 경우 중첩하지 않고 여러 규칙을 사용합니다.
* **[지원되는 데이터 형식](#supported-data-types) 및 [연산자](#supported-operators)만 사용**
* **성능을 위한 규칙을 테스트합니다.** 규칙이 너무 복잡하거나 지원되지 않으면 시스템이 해당 규칙을 거부하거나 시스템 성능에 영향을 줄 수 있습니다.
