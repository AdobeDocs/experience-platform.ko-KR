...
2. 그런 다음 다음 방법 중 하나로 Edge Network에 재정의를 전송해야 합니다.
   ...
   * 웹 SDK [태그 확장](../tags/extensions/client/web-sdk/configure/configuration-overrides.md)을 통해.
...
>[!IMPORTANT]
>
>[Edge Network API](https://developer.adobe.com/data-collection-apis/docs/api/) 통합은 현재 데이터 스트림 재정의를 지원하지 않습니다.
><br>
...
데이터 스트림을 만들었으면 추가한 [Adobe Target](configure.md#target) 서비스를 편집하고 **[!UICONTROL Property Token Overrides]** 섹션을 사용하여 아래 그림과 같이 원하는 데이터 스트림 재정의를 추가하십시오. 한 줄에 하나의 속성 토큰을 추가합니다.
...
데이터 스트림을 만들었으면 추가한 [Adobe Experience Platform](configure.md#aep) 서비스를 편집하고 **[!UICONTROL Add Event Dataset]** 옵션을 선택하여 아래 이미지에 표시된 대로 하나 이상의 재정의 이벤트 데이터 세트를 추가합니다.
...
데이터 스트림을 만들었으면 **[!UICONTROL Advanced Options]**(으)로 이동하여 **[!UICONTROL Third Party ID Sync]** 옵션을 활성화하십시오.

그런 다음 **[!UICONTROL Container ID Overrides]** 섹션을 사용하여 아래 그림과 같이 기본 설정을 재정의할 컨테이너 ID를 추가합니다.
