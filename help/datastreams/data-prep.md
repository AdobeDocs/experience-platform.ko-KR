...
데이터 계층에서 바로 속성을 캡처하려면 JSON 오브젝트에는 단일 루트 속성이 있어야 합니다`data`. `data` 오브젝트의 하위 속성은 캡처하려는 데이터 계층 속성에 매핑하는 방식으로 구성해야 합니다. 아래 섹션을 선택하여 `data` 루트로 형식이 제대로 지정된 JSON 오브젝트의 예제를 확인합니다.
...
* 수동 프로세스를 통해 이 데이터스트림에 대한 [매핑 규칙을 새로 만듭니다](#create-mapping).
* 기존 데이터스트림에서 [매핑 규칙을 가져옵니다](#import-mapping).

### 새 매핑 만들기 {#create-mapping}

시작하려면 **[!UICONTROL 새 매핑 추가]**&#x200B;를 선택하여 새 매핑 행을 만듭니다.
...
이전에 만든 데이터스트림이 있다면 새 데이터스트림에 구성된 매핑 규칙을 다시 사용할 수 있습니다.

>[!WARNING]
>
>다른 데이터스트림에서 매핑 규칙을 가져오는 경우 가져오기 전에 추가될 수 있는 모든 필드 매핑을 덮어씁니다.
...
위 단계를 따라 나머지 필드를 대상 스키마에 계속 매핑합니다. 사용 가능한 소스 필드를 모두 매핑할 수는 없지만 이 단계를 완료하려면 필요에 따라 설정된 대상 스키마의 모든 필드를 매핑해야 합니다. **[!UICONTROL 필수 필드]** 카운터는 아직 현재 구성에 매핑되지 않은 필수 필드의 수를 표시합니다.

필수 필드 수가 0에 도달하고 매핑에 만족스러우면 **[!UICONTROL 저장]**&#x200B;을 선택하여 변경을 완료합니다.
