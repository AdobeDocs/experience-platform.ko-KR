...
데이터스트림은 Adobe Experience Platform Web 및 Mobile SDK 구현 시 서버측 구성을 나타냅니다. SDK의 [구성 명령](../edge/fundamentals/configuring-the-sdk.md)이 클라이언트에서 처리해야 하는 사항(예: `edgeDomain`)을 제어하는 동안 데이터스트림은 SDK에 대한 다른 모든 구성을 처리합니다. Adobe Experience Platform Edge Network로 요청이 전송되면 `edgeConfigId`는 데이터스트림을 참조하는 데 사용됩니다. 이를 통해 웹 사이트에서 코드를 변경하지 않고도 서버측 구성을 업데이트할 수 있습니다.
...
>[!IMPORTANT]
>
>이 문서의 콘텐츠는 법률적인 조언이 아니며, 법률적인 조언을 대체하지 않습니다. 민감한 데이터 처리에 대한 자세한 내용은 회사의 법무 부서에 문의하십시오.
...
데이터스트림은 Experience Platform의 기본 제공 데이터 거버넌스 기능을 활용하여 민감한 데이터가 HIPAA 미지원 서비스로 전송되지 않도록 합니다. 데이터스트림 스키마의 민감한 데이터에 포함된 특정 필드 레이블을 지정하여 특정 용도에 사용할 수 있는 데이터 필드를 세밀하게 제어할 수 있습니다.
...
새 데이터스트림 생성 시 선택한 스키마에 민감한 데이터 사용 레이블이 포함된 경우 데이터스트림만 구성하여 해당 데이터를 HIPAA 지원 대상으로 전송할 수 있습니다. 현재 데이터스트림에서 지원하는 유일한 HIPAA 지원 대상은 Adobe Experience Platform입니다. Adobe Target, Adobe Analytics, Adobe Audience Manager, 이벤트 전달 및 에지 대상 등 기타 대상 서비스는 민감한 데이터 사용 레이블이 포함된 데이터스트림에 대해 비활성화되어 있습니다.

HIPAA 지원 서비스가 아닌 기존 데이터스트림에서 스키마를 사용 중인 경우 민감한 데이터 사용 레이블을 스키마에 추가하게 되면 정책 위반 메시지가 표시되고 작업이 차단됩니다. 메시지는 위반을 트리거한 데이터스트림을 지정하고, 문제 해결을 위해 HIPAA 미지원 서비스를 데이터스트림에서 제거할 것을 제안합니다.

### 감사 로그

Experience Platform에서 감사 로그 형식으로 데이터스트림 활동을 모니터링할 수 있습니다. 감사 로그는 관련 데이터스트림의 문제 해결에 도움이 되며, 기업 데이터 관리 정책 및 규제 요구 사항을 준수하는 데 도움이 되는 다른 컨텍스트 데이터와 함께 **어떤 사람**&#x200B;이 **어떤** 작업을 **언제** 수행했는지 알려 줍니다.
