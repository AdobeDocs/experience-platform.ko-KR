...
**[!UICONTROL 데이터스트림]** 탭에는 친숙한 이름, ID와 마지막 수정 날짜를 포함하여 기존 데이터스트림 목록이 표시됩니다. 종료 [세부 정보 보기 및 서비스 구성](#view-details)를 클릭하고 데이터 스트림의 이름을 선택합니다.

특정 데이터 스트림에 대한 추가 옵션을 표시하려면 &quot;기타&quot; 아이콘(**...**). 를 업데이트하려면 [기본 구성](#configure) 데이터 스트림에 대해 을 선택합니다. **[!UICONTROL 편집]**. 데이터 스트림을 제거하려면 다음을 선택합니다. **[!UICONTROL 삭제]**.
...
Experience Platform에서 사용할 데이터 스트림을 구성하고 Web SDK도 사용하는 경우 [이벤트 기반 XDM(경험 데이터 모델) 스키마](../xdm/classes/experienceevent.md) 수집 예정인 데이터를 나타냅니다.
...
>[!IMPORTANT]
>
>아래 표에 표시된 설정은 함께 사용할 수 없습니다. 사용자 에이전트 정보를 둘 다 선택할 수는 없습니다. *및* 동시에 디바이스 조회 데이터.
...
고급 구성 옵션을 표시하려면 을 선택합니다. **[!UICONTROL 고급 옵션]**. 여기에서 IP 난독화, 자사 ID 쿠키 등과 같은 추가 데이터스트림 설정을 구성할 수 있습니다.
...
| [!UICONTROL Media Analytics] | Experience Platform SDK 또는 를 통해 Edge Network 통합을 위한 스트리밍 추적 데이터 처리를 활성화합니다. [Media Edge API](https://developer.adobe.com/cja-apis/docs/endpoints/media-edge/getting-started/). 에서 Media Analytics에 대해 알아봅니다. [설명서](https://experienceleague.adobe.com/docs/media-analytics/using/media-overview.html?lang=ko-KR). |
