...
| [!UICONTROL 유형에 액세스] | Edge Network가 데이터스트림에 대해 허용하는 인증 유형을 정의합니다. <ul><li>**[!UICONTROL 혼합 인증]**: 이 옵션을 선택하는 경우 Edge Network는 인증된 요청과 인증되지 않은 요청을 모두 허용합니다. [Edge Network API](https://developer.adobe.com/client-sdks/home/)와 함께 웹 SDK 또는 [모바일 SDK](https://developer.adobe.com/data-collection-apis/docs/api/)을(를) 사용하려면 이 옵션을 선택하십시오. </li><li>**[!UICONTROL 인증된 요청만]**: 이 옵션을 선택하는 경우 Edge Network는 인증된 요청만 허용합니다. Edge Network API만 사용하고 인증되지 않은 요청이 Edge Network에 의해 처리되지 않도록 하려는 경우 이 옵션을 선택합니다.</li></ul> |
