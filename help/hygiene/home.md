...
플랫폼의 데이터 위생 기능을 사용하면 다음을 통해 저장된 데이터를 관리할 수 있습니다.

* 자동화된 데이터 세트 만료 예약
* 하나 또는 모든 데이터 세트에서 개별 레코드 삭제
...
레코드 삭제 및 데이터 세트 만료 요청에는 각각 고유한 처리 타임라인이 있으며 각 워크플로우의 주요 지점에서 투명도 업데이트를 제공합니다. 각 작업 유형에 대한 자세한 내용은 아래 섹션을 참조하십시오.
...
### 레코드 삭제 {#record-delete-transparency}

>[!IMPORTANT]
>
>Adobe Healthcare Shield를 구입한 조직에서만 레코드 삭제를 사용할 수 있습니다.

다음 작업은 [레코드 삭제 요청](./ui/record-delete.md) 이(가) 생성되었습니다.

| 스테이징 | 제출 요청 후 시간 | 설명 |
| --- | --- | --- |
| 요청이 제출됨 | 0시간 | 데이터 관리자 또는 개인 정보 분석가가 레코드 삭제 요청을 제출합니다. 요청이 [!UICONTROL 데이터 위생 UI] 제출한 후. |
