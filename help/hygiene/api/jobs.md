...
| 속성 | 설명 |
| --- | --- |
| `companyContexts` | 조직에 대한 인증 정보가 포함된 배열입니다. 여기에는 다음 속성을 갖는 단일 개체가 포함되어야 합니다. <ul><li>`namespace`: 을(를) 로 설정해야 합니다. `imsOrgID`.</li><li>`value`: 자신의 조직 ID. 이 값은 `x-gw-ims-org-id` 헤더.</li></ul> |
