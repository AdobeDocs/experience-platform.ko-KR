...
데이터 위생 API를 호출하려면 먼저 인증 자격 증명을 수집해야 합니다. Privacy Service API에 액세스하는 데 사용되는 자격 증명과 동일한 자격 증명입니다. 다음을 참조하십시오. [API 개요](./overview.md#getting-started) 아래와 같이 데이터 위생 API의 필수 각 헤더에 대한 값을 생성하려면 다음을 수행합니다.
...
요청 페이로드는 의 구조와 유사합니다. [Privacy Service API에서 요청 삭제](../../privacy-service/api/privacy-jobs.md#access-delete). 여기에는 다음이 포함됩니다 `users` 데이터가 삭제될 사용자를 나타내는 객체를 가진 배열입니다.
...
| 속성 | 설명 |
| --- | --- |
| `companyContexts` | 조직의 인증 정보가 포함된 배열입니다. 다음 속성을 가진 단일 개체가 있어야 합니다. <ul><li>`namespace`: 다음으로 설정되어야 합니다. `imsOrgID`.</li><li>`value`: IMS 조직 ID입니다. 에 제공된 것과 동일한 값입니다 `x-gw-ims-org-id` 머리글입니다.</li></ul> |
