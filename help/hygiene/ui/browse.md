...
>title="작업 주문 ID"
>abstract="데이터 위생 요청이 시스템으로 전송되면 요청된 작업을 실행하기 위한 작업 순서가 만들어집니다. 즉, 작업 명령은 현재 상태 및 기타 관련 세부 사항을 포함하는 특정 데이터 위생 프로세스를 나타냅니다. 각 작업 주문은 생성 시 자동으로 고유한 자체 ID가 할당됩니다."
...
데이터 위생 요청이 시스템으로 전송되면 요청된 작업을 실행하기 위한 작업 순서가 만들어집니다. 작업 명령은 현재 상태 및 기타 관련 세부 사항이 포함된 예약된 데이터 세트 만료와 같은 특정 데이터 위생 프로세스를 나타냅니다.
...
>title="서비스별 상태"
>abstract="데이터 위생 요청은 여러 Experience Platform 서비스에서 독립적으로 처리됩니다. 이 섹션에서는 각 서비스에 대한 요청의 현재 처리 상태에 대해 간략히 설명합니다. 자세한 내용은 데이터 위생 UI 안내서 를 참조하십시오."

>[!CONTEXTUALHELP]
>id="platform_hygiene_numberofidentities"
>title="ID 수"
>abstract="이 작업 주문의 일부로 레코드 업데이트 또는 삭제가 요청된 ID의 수입니다. 카운트에 포함된 ID가 영향을 받는 데이터 세트에 반드시 존재할 필요는 없습니다. 자세한 내용은 데이터 위생 UI 안내서 를 참조하십시오."

>[!CONTEXTUALHELP]
>id="platform_hygiene_responsemessages"
>title="삭제 응답 기록"
>abstract="레코드 삭제 프로세스에서 시스템의 응답을 받으면 다음 메시지가 아래에 표시됩니다 **[!UICONTROL 결과]** 섹션. 작업 지시를 처리하는 동안 문제가 발생하면 이 섹션에 관련 오류 메시지가 표시되어 문제를 해결하는 데 도움이 됩니다. 자세한 내용은 데이터 위생 UI 안내서 를 참조하십시오."
