...
[[!UICONTROL Data Lifecycle] 작업 영역](./overview.md)을(를) 사용하여 기본 ID를 기반으로 Adobe Experience Platform에서 레코드를 삭제합니다. 이러한 레코드는 개별 소비자 또는 ID 그래프에 포함된 다른 엔티티에 연결할 수 있습니다.
...
레코드를 삭제하려면 Experience Platform에서 ID 필드가 작동하는 방식을 이해해야 합니다. 특히 삭제할 데이터 세트(또는 데이터 세트)에 따라 레코드를 삭제할 엔터티의 기본 ID 네임스페이스와 값을 알고 있어야 합니다.
...
>[!IMPORTANT]
>
>레코드 삭제는 데이터 집합 스키마에 정의된 **기본 ID** 필드에서만 작동합니다. 다음 제한 사항이 적용됩니다.
>
...
>* **ID 구성 전에 수집된 데이터는 사용할 수 없습니다.** 데이터 수집 후 기본 ID 필드가 스키마에 추가된 경우 이전에 수집된 레코드는 이 워크플로우를 통해 삭제할 수 없습니다.

## 요청 만들기 {#create-request}

프로세스를 시작하려면 Experience Platform UI의 왼쪽 탐색에서 **[!UICONTROL Data Lifecycle]**&#x200B;을(를) 선택합니다. [!UICONTROL Data lifecycle requests] 작업 영역이 나타납니다. 작업 영역의 기본 페이지에서 **[!UICONTROL Create request]**&#x200B;을(를) 선택합니다.

![[!UICONTROL Data lifecycle requests]이(가) 있는 [!UICONTROL Create request] 작업 영역을 선택했습니다.](../images/ui/record-delete/create-request-button.png)

요청 생성 워크플로가 나타납니다. 기본적으로 **[!UICONTROL Delete record]** 섹션에서 **[!UICONTROL Requested Action]** 옵션이 선택됩니다. 이 옵션을 선택된 상태로 두십시오.
...
![선택한 [!UICONTROL Delete record] 옵션이 강조 표시된 요청 생성 워크플로우입니다.](../images/ui/record-delete/delete-record.png)
...
**[!UICONTROL Record Details]** 섹션에서 라디오 단추를 선택하여 특정 데이터 세트 또는 모든 데이터 세트를 선택합니다.

특정 데이터 집합에서 삭제하려면 **[!UICONTROL Select dataset]**&#x200B;을(를) 선택한 다음 데이터베이스 아이콘(![데이터베이스 아이콘](/help/images/icons/database.png))을 선택하십시오. 표시되는 대화 상자에서 데이터 세트를 선택하고 **[!UICONTROL Done]**&#x200B;을(를) 선택하여 확인합니다.

![데이터 집합이 선택되고 [!UICONTROL Select dataset]이 강조 표시된 [!UICONTROL Done] 대화 상자.](../images/ui/record-delete/select-dataset.png)

모든 데이터 세트에서 삭제하려면 **[!UICONTROL All datasets]**&#x200B;을(를) 선택하십시오. 이 옵션은 작업 범위를 늘리며 타깃팅하려는 각 데이터 세트에 대한 기본 ID 유형을 제공해야 합니다.

![선택한 [!UICONTROL Select dataset] 옵션이 있는 [!UICONTROL All datasets] 대화 상자입니다.](../images/ui/record-delete/all-datasets.png)

>[!WARNING]
>
>**[!UICONTROL All datasets]**&#x200B;을(를) 선택하면 조직의 모든 데이터 세트로 작업이 확장됩니다. 각 데이터 세트는 다른 기본 ID 유형을 사용할 수 있습니다. 정확한 일치를 위해 **각 데이터 집합에 대한 기본 ID 형식을 제공**&#x200B;해야 합니다.
>
...
레코드를 삭제할 때 시스템에서 삭제할 레코드를 결정할 수 있도록 ID 정보를 제공해야 합니다. Experience Platform에 있는 모든 데이터 세트의 경우 데이터 세트의 스키마에 의해 정의된 **기본 ID** 필드를 기반으로 레코드가 삭제됩니다.

>[!NOTE]
>
>UI를 통해 ID 네임스페이스를 선택할 수 있지만 실행 시 데이터 세트의 스키마에 구성된 **기본 ID**&#x200B;만 사용됩니다. 제공한 ID 값이 데이터 세트의 기본 ID 필드에 해당하는지 확인합니다.

Experience Platform의 모든 ID 필드와 마찬가지로 기본 ID는 **type**(ID 네임스페이스)과 **value**, 이렇게 두 가지로 구성됩니다. ID 유형은 필드가 레코드(예: 이메일 주소)를 식별하는 방법에 대한 컨텍스트를 제공합니다. 값은 해당 형식에 대한 레코드의 특정 ID를 나타냅니다(예: `jdoe@example.com` ID 형식의 경우 `email`). 기본 ID로 사용되는 일반적인 필드에는 계정 정보, 장치 ID 및 쿠키 ID가 포함됩니다.

>[!TIP]
>
>특정 데이터 세트에 대한 ID 네임스페이스를 모를 경우 Experience Platform UI에서 찾을 수 있습니다. **[!UICONTROL Datasets]** 작업 영역의 목록에서 해당 데이터 세트를 선택합니다. 데이터 세트에 대한 세부 정보 페이지에서 오른쪽 레일의 데이터 세트 스키마 이름 위로 마우스를 가져갑니다. 스키마 이름 및 설명과 함께 ID 네임스페이스가 표시됩니다.
>
...
JSON 파일을 업로드하려면 파일을 제공된 영역으로 끌어다 놓거나 **[!UICONTROL Choose files]**&#x200B;을(를) 선택하여 로컬 디렉터리에서 찾아 선택할 수 있습니다.

![JSON 파일을 업로드하기 위한 파일 선택 및 드래그 앤 드롭 인터페이스가 강조 표시된 요청 생성 워크플로우입니다.](../images/ui/record-delete/upload-json.png)

JSON 파일은 개체 배열로 포맷되어야 합니다. 각 개체는 대상 데이터 세트에 대한 기본 ID 값을 나타냅니다.
...
| 속성 | 설명 |
| --- | --- |
| `namespaceCode` | 대상 데이터 세트에 대한 기본 ID 네임스페이스. |
...
ID를 수동으로 입력하려면 **[!UICONTROL Add identity]**&#x200B;을(를) 선택하십시오.

![[!UICONTROL Add identity] 옵션이 강조 표시된 요청 생성 워크플로우입니다.](../images/ui/record-delete/add-identity.png)

ID를 한 번에 하나씩 입력할 수 있는 컨트롤이 나타납니다. **[!UICONTROL identity namespace]**&#x200B;에서 드롭다운 메뉴를 사용하여 ID 유형을 선택합니다. **[!UICONTROL Primary Identity Value]**&#x200B;에서 레코드에 대한 ID 네임스페이스 값을 제공합니다.

![ID 필드가 있는 요청 만들기 워크플로를 수동으로 추가했습니다.](../images/ui/record-delete/identity-added.png)

더 많은 ID를 추가하려면 더하기 아이콘(![더하기 아이콘)을 선택하십시오.](/help/images/icons/tree-expand-all.png)) 행 중 하나 옆에 있거나 **[!UICONTROL Add identity]**&#x200B;을(를) 선택합니다.
...
요청에 ID를 추가했으면 **[!UICONTROL Request settings]**&#x200B;에서 **[!UICONTROL Submit]**&#x200B;을(를) 선택하기 전에 요청에 대한 이름과 선택적 설명을 제공하십시오.

>[!TIP]
>
>UI를 통해 요청당 최대 10,000개의 ID를 제출할 수 있습니다. 더 큰 볼륨(요청당 최대 100,000개의 ID)을 제출하려면 [API 메서드](../api/workorder.md#create)를 사용하십시오.

![강조 표시된 [!UICONTROL Name]이(가) 있는 요청 설정의 [!UICONTROL Description] 및 [!UICONTROL Submit] 필드](../images/ui/record-delete/submit.png)

ID를 삭제한 후에는 복구할 수 없음을 나타내는 [!UICONTROL Confirm request] 대화 상자가 나타납니다. 데이터를 삭제할 ID 목록을 확인하려면 **[!UICONTROL Submit]**&#x200B;을(를) 선택하십시오.

![대화 상자 [!UICONTROL Confirm request]개](../images/ui/record-delete/confirm-request.png)

요청이 제출되면 작업 순서가 만들어지고 [!UICONTROL Record] 작업 영역의 [!UICONTROL Data Lifecycle] 탭에 나타납니다. 여기에서 요청을 처리할 때 작업 주문의 상태를 모니터링할 수 있습니다.

>[!NOTE]
>
>레코드 삭제가 실행되면 처리되는 방법에 대한 자세한 내용은 [타임라인 및 투명도](../home.md#record-delete-transparency)의 개요 섹션을 참조하십시오.

![새 요청이 강조 표시된 [!UICONTROL Record] 작업 영역의 [!UICONTROL Data Lifecycle] 탭입니다.](../images/ui/record-delete/request-log.png)
...
변경 데이터 캡처와 함께 소스를 사용하는 관계형 스키마는 업스트림에서 삭제를 구분할 때 `_change_request_type` 컨트롤 열을 사용할 수 있습니다. 수집 중에 `d`(으)로 플래그가 지정된 레코드는 데이터 집합에서 삭제되지만 `u`(으)로 또는 열 없이 플래그가 지정된 레코드는 업데이트로 처리됩니다. `_change_request_type` 열은 수집 시에만 읽으며 대상 스키마에 저장되거나 XDM 필드에 매핑되지 않습니다.

>[!NOTE]
>
>데이터 수명 주기 UI를 통해 레코드를 삭제해도 소스 시스템에는 영향을 주지 않습니다. 두 위치에서 데이터를 제거하려면 Experience Platform과 소스 모두에서 데이터를 삭제합니다.

### 관계형 스키마에 대한 추가 삭제 방법

표준 레코드 삭제 워크플로우 외에도 관계형 스키마는 특정 사용 사례에 대해 추가 메서드를 지원합니다.
...
관계형 스키마 설명자는 정확한 위생 작업을 위한 필수 메타데이터를 제공합니다.
...
스키마 요구 사항에 대한 자세한 내용은 [관계형 스키마 설명자 요구 사항](../../xdm/schema/relational.md#relational-schemas)을 참조하십시오.

변경 데이터 캡처가 소스에서 작동하는 방식에 대해 알아보려면 [소스에서 변경 데이터 캡처 사용](../../sources/tutorials/api/change-data-capture.md#using-change-data-capture-with-relational-schemas)을 참조하세요.
