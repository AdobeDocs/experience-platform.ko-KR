...
## XDM 개별 프로필 클래스를 기반으로 스키마 구성

데이터 집합을 만들려면 먼저 XDM 개별 프로필 클래스를 구현하는 새 스키마를 만들어야 합니다. 스키마 생성 방법에 대한 자세한 내용은 스키마 레지스트리 API [개발자 안내서를](../../xdm/api/getting-started.md)참조하십시오.
...
성공적으로 응답하면 HTTP 상태 201이 반환되고 새로 생성된 스키마 기본 ID 설명자에 대한 정보가 반환됩니다.
...
>[!NOTE] 이 데이터 집합은 실시간 고객 프로필 **및 ID 서비스에** 대해 **활성화됩니다**.
...
데이터 세트 및 스트리밍 연결을 통해 XDM 형식의 JSON 레코드를 인제스트하여 기록 데이터를 플랫폼에 인제스트할 수 있습니다.
...
>[!NOTE] 다음 API 호출에는 인증 헤더가 필요하지 **않습니다** .
...
다음 GET 요청을 사용하여 이전에 인제스트한 레코드 데이터를 검토할 수 있습니다.
...
성공적인 응답은 요청된 엔티티의 세부 정보와 함께 HTTP 상태 200을 반환합니다. 보시는 바와 같이, 이것은 일찍이 성공적으로 수집되었던 기록과 같습니다.
