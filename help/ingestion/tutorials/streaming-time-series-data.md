...
데이터 세트를 만들려면 먼저 XDM ExperienceEvent 클래스를 구현하는 새 스키마를 만들어야 합니다. 스키마 생성 방법에 대한 자세한 내용은 스키마 [레지스트리 API 개발자 안내서를 참조하십시오](../../xdm/api/getting-started.md).
...
>[!NOTE]
>
>이 데이터 집합은 적절한 태그를 설정하여 **실시간 고객 프로필** 및 **ID에** 대해 활성화됩니다.
...
데이터 세트 및 스트리밍 연결을 적절히 사용하면 XDM 형식의 JSON 레코드를 인제스트하여 Platform 내의 시간 시리즈 데이터를 인제스트할 수 있습니다.
...
이전에 인제스트한 레코드의 유효성을 확인하려면 프로필 액세스 API를 [사용하여 시간](../../profile/api/entities.md) 시리즈 데이터를 검색할 수 있습니다. 종단점에 대한 GET 요청을 사용하고 선택적 쿼리 매개 변수를 사용하여 `/access/entities` 수행할 수 있습니다. 여러 매개 변수를 앰퍼샌드(&amp;)로 구분하여 사용할 수 있습니다.&quot;
