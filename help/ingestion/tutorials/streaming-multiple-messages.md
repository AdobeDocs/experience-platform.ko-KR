...
헤더에서 사용 중인 잘못된 IMS 조직 ID로 인해 세 번째 메시지가 실패했습니다. IMS 조직은 게시하려는 {CONNECTION_ID}와 일치해야 합니다. 사용 중인 스트리밍 연결과 일치하는 IMS 조직 ID를 확인하려면 다음을 수행할 수 있습니다 `GET inlet` 를 사용하여 요청 [[!DNL Data Ingestion API]](https://www.adobe.io/experience-platform-apis/references/data-ingestion/). 자세한 내용은 [스트리밍 연결 검색](./create-streaming-connection.md#get-data-collection-url) 앞에서 만든 스트리밍 연결을 검색하는 방법의 예입니다.
