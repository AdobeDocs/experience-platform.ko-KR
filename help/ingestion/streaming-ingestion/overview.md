...
Adobe Experience Platform용 스트리밍 수집은 클라이언트 및 서버측 장치에서 실시간으로 Experience Platform으로 데이터를 전송하는 방법을 사용자에게 제공합니다.

## 스트리밍 수집으로 무엇을 할 수 있습니까?

Adobe Experience Platform을 사용하면 각 개별 고객에 대한 실시간 고객 프로필을 생성하여 통합되고 일관되며 관련 있는 경험을 추진할 수 있습니다. 스트리밍 수집은 프로필 데이터를 가능한 한 짧은 지연 시간으로 데이터 레이크에 전달할 수 있도록 함으로써 이러한 프로필을 빌드하는 데 중요한 역할을 합니다.
...
스트리밍 수집을 통해 사용자는 프로필 레코드와 [!DNL ExperienceEvents]을(를) Experience Platform에 초 단위로 스트리밍하여 실시간 개인화를 촉진할 수 있습니다. 스트리밍 수집 API로 전송된 모든 데이터는 데이터 레이크에서 자동으로 유지됩니다.

자세한 내용은 [스트리밍 연결 가이드 만들기](../tutorials/create-streaming-connection.md)를 참조하십시오.

### 데이터 세트로 스트리밍

데이터가 깨끗하다고 확신하면 실시간 고객 프로필 및 [!DNL Identity Service]에 데이터 세트를 사용하도록 설정할 수 있습니다.

프로필 및 [!DNL Identity Service]에 대한 데이터 집합 활성화에 대한 자세한 내용은 [데이터 집합 구성 가이드](/help/profile/tutorials/dataset-configuration.md)를 참조하십시오.
