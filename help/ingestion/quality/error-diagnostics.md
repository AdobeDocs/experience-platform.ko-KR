...
| 속성 | 설명 |
| -------- | ----------- |
| `metrics.failedRecordCount` | 구문 분석, 전환 또는 유효성 검사로 인해 처리할 수 없는 행의 수입니다. 이 값은 `inputRecordCount`에서 `outputRecordCount`을(를) 빼서 파생할 수 있습니다. 이 값은 `errorDiagnostics`이(가) 사용되는지 여부에 관계없이 모든 일괄 처리에서 생성됩니다. |
...
| -------- | ----------- |
| `metrics.failedRecordCount` | 구문 분석, 전환 또는 유효성 검사로 인해 처리할 수 없는 행의 수입니다. 이 값은 `inputRecordCount`에서 `outputRecordCount`을(를) 빼서 파생할 수 있습니다. 이 값은 `errorDiagnostics`이(가) 사용되는지 여부에 관계없이 모든 일괄 처리에서 생성됩니다. |
| `errors.recordCount` | 지정된 오류 코드에 대해 실패한 행 수입니다. 이 값은 **이(가) 활성화된 경우** only`errorDiagnostics`입니다. |
