...
데이터를 Adobe Experience Platform에 수집하는 프로세스는 여러 단계로 구성됩니다. 수집해야 하는 데이터 파일을 식별하면 [!DNL Platform]: 수집 프로세스가 시작되고 데이터가 성공적으로 수집되거나 실패할 때까지 각 단계가 연속적으로 발생합니다. 수집 프로세스는 [Adobe Experience Platform 배치 수집 API](https://developer.adobe.com/experience-platform-apis/references/batch-ingestion/) 또는 [!DNL Experience Platform] 사용자 인터페이스.
