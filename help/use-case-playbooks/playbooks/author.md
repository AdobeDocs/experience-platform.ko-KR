...
하나의 샌드박스에서 플레이북을 만들고 게시하면 조직 내의 모든 샌드박스에서 자동으로 사용할 수 있게 됩니다. 이렇게 하면 수동으로 공유할 필요가 없고 다른 샌드박스에서 플레이북의 인스턴스를 원활하게 만들 수 있습니다.

>[!TIP]
>
>플레이북이 대상 샌드박스의 유니온 스키마에서 사용할 수 없거나 필수 권한이 없는 필드를 참조하는 경우 인스턴스를 생성하려고 하면 오류 메시지가 표시될 수 있습니다. 해당 메시지는 누락된 필드 및/또는 누락된 권한을 호출합니다.
