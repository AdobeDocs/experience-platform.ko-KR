...
처음부터 새로 시작하거나 기존 레서피를 구성하는 경우 조직의 중앙 집중화되고 표준화된 데이터 카탈로그에서 데이터 검색을 시작하여 추적을 상당히 간소화할 수 있습니다. 조직에서 다른 데이터 과학자가 이미 유사한 데이터 세트를 식별했으며 처음부터 새로 시작하지 않고 해당 데이터 세트를 미세 조정하도록 선택할 수도 있습니다.
