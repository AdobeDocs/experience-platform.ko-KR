...
다음 안내서에서는 실시간 기계 학습 응용 프로그램을 만드는 데 필요한 단계를 설명합니다. 이 안내서에서는 Adobe에 제공된 **[!UICONTROL 실시간 ML]** Python 전자 필기장 템플릿을 사용하여 모델 교육, DSL 만들기, Edge에 DSL을 게시하고 요청 점수를 매깁니다. 실시간 머신 러닝 모델을 구현하는 작업이 진행되면 데이터세트의 요구 사항에 맞게 템플릿을 수정할 수 있습니다.

## 실시간 기계 학습 노트북 만들기

Adobe Experience Platform UI의 **데이터 과학** 내에서 **[!UICONTROL 노트북]**&#x200B;을 선택합니다. 그런 다음 **[!UICONTROL JupiterLab]**&#x200B;을 선택하고 환경을 로드할 시간을 허용합니다.

![open JupiterLab](../images/rtml/open-jupyterlab.png)

[!DNL JupyterLab] 실행 프로그램이 나타납니다. *실시간 기계 학습*&#x200B;으로 스크롤하고 **[!UICONTROL 실시간 ML]** 전자 필기장을 선택합니다. 예제 데이터세트의 노트북 셀이 들어 있는 템플릿이 열립니다.
...
[!DNL JupyterLab] 전자 필기장의 데이터 세트에 액세스하려면 [!DNL JupyterLab]의 왼쪽 탐색에서 **데이터** 탭을 선택합니다. **[!UICONTROL 데이터 집합]** 및 **[!UICONTROL 스키마]** 디렉토리가 표시됩니다. **[!UICONTROL 데이터 집합]**&#x200B;을 선택하고 마우스 오른쪽 단추를 클릭한 다음 사용할 데이터 집합의 드롭다운 메뉴에서 **[!UICONTROL 노트북의 데이터 탐색]** 옵션을 선택합니다. 실행 가능한 코드 항목이 전자 필기장 하단에 표시됩니다. 이 셀에는 `dataset_id`이 있습니다.
...
**[!UICONTROL 실시간 ML]** 템플릿을 사용하여 ML 모델을 분석, 사전 처리, 트레이닝 및 평가해야 합니다. 이는 데이터 변형을 적용하고 교육 파이프라인을 구축하는 방식으로 이루어집니다.

**데이터 변형**

데이터 세트 작업을 수행하려면 **[!UICONTROL 실시간 ML]** 템플릿 **데이터 변환** 셀을 수정해야 합니다. 일반적으로 열 이름 바꾸기, 데이터 롤업 및 데이터 준비/기능 엔지니어링이 포함됩니다.
...
[!DNL Edge]에 게시한 후에는 클라이언트의 POST 요청에 의해 점수를 매깁니다. 일반적으로 ML 점수가 필요한 클라이언트 애플리케이션에서 이 작업을 수행할 수 있습니다. 포스트맨에서도 가능합니다. **[!UICONTROL 실시간 ML]** 템플릿은 EdgeUtils를 사용하여 이 프로세스를 보여 줍니다.
