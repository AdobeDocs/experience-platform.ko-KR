...
| 하이퍼매개 변수 | 설명 | 권장 범위 |
| --- | --- | --- |
| learning_rate | 학습 비율은 learning_rate로 각 트리의 기여도를 축소합니다. learning_rate와 n_estimators 사이에는 상쇄 조건이 있습니다. | 0.1 |
| n_견적 도구 | 수행할 부흥 단계 수입니다. 그레이디언트 증대는 지나치게 맞추기 때문에 일반적으로 많은 수가 더 나은 성능을 얻을 수 있습니다. | 100 |
| max_depth | 개별 회귀 견적 도구의 최대 깊이입니다. 최대 깊이는 트리의 노드 수를 제한합니다. 최상의 성능을 위해 이 매개 변수를 조정하십시오.가장 좋은 값은 입력 변수의 상호 작용에 따라 달라집니다. | 3 |
