...
단일 GET 요청을 수행하고 요청 경로에서 유효한 모델 ID를 제공하여 특정 모델에 속하는 모델 세부 사항 목록을 검색할 수 있습니다. 결과를 필터링하는 데 도움이 되도록 요청 경로에서 쿼리 매개 변수를 지정할 수 있습니다. 사용 가능한 질의 목록은 자산 검색을 위한 [쿼리 매개 변수의 부록 섹션을 참조하십시오](./appendix.md#query).
...
종단점에 POST 요청을 수행하여 미리 생성된 모델을 등록할 수 `/models` 있습니다. 모델을 등록하려면 `modelArtifact` 파일 및 `model` 속성 값을 요청 본문에 포함해야 합니다.
...
다음 POST에는 필요한 `modelArtifact` 파일 및 `model` 속성 값이 포함됩니다. 이러한 값에 대한 자세한 내용은 아래 표를 참조하십시오.
...
사용자 및 트랜스코딩된 모델의 ID로 GET 요청을 수행하여 특정 트랜스코딩 모델 `{MODEL_ID}` 을 검색할 수 있습니다.
