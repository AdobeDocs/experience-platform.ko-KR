...
성공적인 응답은 각 모델의 고유 식별자(`id`)를 포함하여 모델의 세부 정보가 포함된 페이로드를 반환합니다.
...
성공적인 응답은 모델 고유 식별자(`id`)를 포함하여 모델의 세부 정보가 포함된 페이로드를 반환합니다.
...
`/models` 끝점에 대한 POST 요청을 수행하여 미리 생성된 모델을 등록할 수 있습니다. 모델을 등록하려면 `modelArtifact` 파일 및 `model` 속성 값을 요청 본문에 포함해야 합니다.
...
다음 POST에 필요한 `modelArtifact` 파일과 `model` 속성 값이 포함되어 있습니다. 이러한 값에 대한 자세한 내용은 아래 표를 참조하십시오.
...
| --- | --- |
| `id` | 모델에 해당하는 ID. |
| `modelArtifactUri` | 모델이 저장되는 위치를 나타내는 URI입니다. URI는 모델에 대한 `id` 값으로 끝납니다. |
...
트랜스코딩은 한 인코딩을 다른 인코딩으로 직접 디지털 대 디지털 변환하는 것입니다. 새 출력을 가져올 `{MODEL_ID}` 및 `targetFormat`을(를) 제공하여 모델에 대한 새 코드 변환 작업을 만듭니다.
...
성공적인 응답은 트랜스코딩의 정보와 함께 JSON 개체가 포함된 페이로드를 반환합니다. 여기에는 [특정 코드 변환된 모델 검색](#retrieve-transcoded-model)에 사용되는 코드 변환 고유 식별자(`id`)가 포함됩니다.
...
`{MODEL_ID}`(으)로 GET 요청을 수행하여 모델에서 수행된 전환 목록을 검색할 수 있습니다.
...
성공적인 응답은 모델에서 수행된 각 코드 변환 목록과 함께 json 개체가 포함된 페이로드를 반환합니다. 코드 변환된 각 모델은 고유 식별자(`id`)를 받습니다.
...
`{MODEL_ID}` 및 트랜스코딩된 모델의 ID로 GET 요청을 수행하여 특정 트랜스코딩된 모델을 검색할 수 있습니다.
