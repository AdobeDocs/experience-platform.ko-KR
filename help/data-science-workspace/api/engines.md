...
>[!NOTE] Docker 암호는 업데이트될 때마다 `{ACCESS_TOKEN}` 변경됩니다.
...
## 이진 가공물을 사용하여 피쳐 파이프라인 엔진 생성

메타 데이터와 객체의 경로를 여러 부분으로 구성된 형태로 제공하는 동안 POST 요청을 수행하여 로컬 `.jar` 또는 `.egg` 이진 객체를 사용하여 피쳐 파이프라인 엔진을 생성할 수 있습니다. PySpark 또는 Spark Engine은 코어 수 또는 메모리 양과 같은 컴퓨팅 리소스를 지정할 수 있습니다. 자세한 내용은 PySpark 및 [Spark 리소스 구성의](./appendix.md#resource-config) 부록 섹션을 참조하십시오.
