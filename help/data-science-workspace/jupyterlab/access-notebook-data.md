...
Scala에서 `clientContext`을(를) 가져와서 Experience Platform 값을 가져와서 반환할 수 있으므로 `var userToken`과(와) 같은 변수를 정의할 필요가 없습니다. 아래 Scala 예제에서 `clientContext`은(는) 데이터 집합을 읽는 데 필요한 모든 필수 값을 가져오고 반환하는 데 사용됩니다.
...
Scala에서 `clientContext`을(를) 가져와서 Experience Platform 값을 가져와서 반환할 수 있으므로 `var userToken`과(와) 같은 변수를 정의할 필요가 없습니다. 아래 Scala 예제에서 `clientContext`은(는) 데이터 집합에 쓰는 데 필요한 모든 필수 값을 정의하고 반환하는 데 사용됩니다.
