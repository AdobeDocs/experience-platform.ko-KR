...
title: 탐색적 데이터 분석(EDA) 노트북
type: Tutorial
description: 이 안내서에서는 탐색적 데이터 분석(EDA) Notebook을 사용하여 웹 데이터에서 패턴을 검색하고, 예측 목표가 있는 이벤트를 합계 집계하고, 집계된 데이터를 정리하고, 예측 변수와 목표 간의 관계를 이해하는 방법을 중점적으로 설명합니다.
...
# 탐색적 데이터 분석(EDA) 노트북을 사용하여 예측 모델에 대한 웹 기반 데이터 탐색
...
아래 제공된 셀을 사용하여 표에 포함된 날짜 범위 를 볼 수 있습니다. 일 수, 첫 번째 날짜 및 마지막 날짜를 탐색하는 목적은 추가 분석을 위해 날짜 범위를 선택하는 데 도움이 됩니다.
...
모든 매개 변수를 구성하고, 를 시작 [!DNL Query Service]하고, 날짜 범위 가 있으면 데이터 행 읽기를 시작할 준비가 된 것입니다. 읽는 행 수를 제한해야 합니다.
...
데이터 세트 목록에서 사용할 수 있는 열 수를 보려면 다음 셀을 사용합니다.

```python
print("\nNumber of columns:",len(Table.columns))
```

데이터 세트 행을 보려면 다음 셀을 사용합니다. 이 예에서는 행 수가 5개로 제한됩니다.
...
데이터 세트에 어떤 데이터가 포함되어 있는지 파악한 후에는 데이터 세트를 추가로 분류하는 것이 유용할 수 있습니다. 이 예제에서는 각 열의 열 이름과 데이터 형식이 나열되고 출력은 데이터 형식이 올바른지 여부를 확인하는 데 사용됩니다.
...
다음 섹션에는 데이터의 추세와 패턴을 탐색하는 데 사용되는 4개의 예제 쿼리가 포함되어 있습니다. 아래에 제공된 예는 완전하지는 않지만 일반적으로 볼 수 있는 몇 가지 기능을 다룹니다.
...
쿼리가 작동하는지 확인한 후 시각적 명확성을 위해 데이터를 일변량 플롯 막대 그래프로 표시할 수 있습니다.
...
이 쿼리는 가장 많이 본 10개 제품 목록을 제공합니다. 아래 `Explode()` 예에서 함수는 개체의 각 제품을 `productlistitems` 자체 행으로 반환하는 데 사용됩니다. 이를 통해 중첩된 쿼리를 수행하여 다양한 SKU에 대한 제품 보기를 집계할 수 있습니다.
...
탐색적 분석을 시작하려면 프로필 수준에서 목표를 만든 다음 데이터 세트 집계를 수행해야 합니다. 이 예에서는 두 개의 쿼리가 제공됩니다. 첫 번째 쿼리에는 목표 만들기가 포함됩니다. 두 번째 쿼리는 첫 번째 쿼리에 포함되지 않은 변수를 포함하도록 업데이트해야 합니다. 쿼리에 대해 `limit`을(를) 업데이트할 수 있습니다. 다음 쿼리를 수행하면 이제 집계된 데이터를 탐색할 수 있습니다.
...
이 프로세스에는 결측값과 이상치 식별이 포함됩니다. 문제가 식별되면 다음 작업은 문제를 처리하기 위한 구체적인 전략을 마련하는 것입니다.

>[!NOTE]
>
>이 단계에서 데이터 로깅 프로세스의 오류를 알릴 수 있는 값의 손상을 발견할 수 있습니다.
...
![결측값](../images/jupyterlab/eda/missing-values.png)

결측값을 감지한 후 이상치 식별하는 것이 중요. 평균, 표준편차, 상관관계와 같은 모수적 통계는 이상치에 매우 민감하다. 또한 선형 회귀와 같은 일반적인 통계 절차의 가정도 이러한 통계에 기반한다. 이것은 이상치가 분석을 정말 엉망으로 만들 수 있다는 것을 의미합니다.
...
>[!TIP]
>
>이상치 수정을 위해서는 자신이 속한 비즈니스와 산업에 대한 이해가 필요합니다. 이상형이라는 이유만으로 관측을 포기할 수 없는 경우도 있다. 이상치는 합법적인 관찰이 될 수 있으며 종종 가장 흥미로운 관찰입니다. 이상값 삭제에 대한 자세한 내용은 [선택적 데이터 정리 단계](#optional-data-clean)를 참조하세요.
...
누락 값 및 이상치에 대한 데이터가 수정되면 분석을 시작할 수 있습니다. 분석에는 일변량, 이변량, 다변량 분석의 세 가지 유형이 있다. 일변량 분석은 데이터를 가져오고, 요약하고, 단일 변수 관계를 사용하여 데이터에서 패턴을 찾습니다. 이변량 분석은 한 번에 두 개 이상의 변수 확인하는 반면, 다변량 분석은 한 번에 세 개 이상의 변수를 확인합니다.

다음 예에서는 기능의 분포를 시각화하는 테이블을 생성합니다.
...
![기능 분포](../images/jupyterlab/eda/distribution-of-features.PNG)

기능을 배포한 후에는 배열을 사용하여 시각화된 데이터 차트를 만들 수 있습니다. 다음 셀은 위의 표를 숫자 데이터로 시각화하는 데 사용됩니다.
...
- **상관 관계**: 상관 계수는 두 기능 간의 관계의 강도를 측정한 것입니다. 상관 관계의 값은 -1과 1 사이이며, 여기서 1은 강한 양의 관계, -1은 강한 음의 관계, 결과가 0이면 관계가 전혀 없음을 나타냅니다.
- **쌍체 그림**: 쌍체 그림은 각 변수 간의 관계를 시각화하는 간단한 방법입니다. 데이터의 각 변수 간의 관계 행렬을 생성합니다.
- **히트맵**: 히트맵은 데이터 세트의 모든 변수에 대한 상관 계수입니다.
- **상자 그림**: 상자 그림은 5개의 숫자 요약(최소값, 첫 번째 사분위수(Q1), 중앙값, 세 번째 사분위수(Q3) 및 최대값)를 기반으로 데이터 분포를 표시하는 표준화된 방법입니다.
- **개수 그림**: 개수 그림은 일부 범주형 특징에 대해 막대 그래프 또는 막대 그래프 좋아요됩니다. 특정 유형의 카테고리를 기준으로 항목의 발생 횟수를 보여줍니다.

&#39;목표&#39; 변수 및 예측 변수/기능 간의 관계를 이해하기 위해 차트는 데이터 유형을 기반으로 사용됩니다. 수치 특징의 경우 &#39;목표&#39; 변수가 범주형이면 상자 그림을 사용해야 하고, &#39;목표&#39; 변수가 숫자이면 쌍그림과 히트맵을 사용해야 합니다.

범주형 특징의 경우 &#39;goal&#39; 변수가 범주형이면 개수구를 사용해야 하고, &#39;goal&#39; 변수가 숫자형이면 상자 그림을 사용해야 합니다. 이러한 방법을 사용하면 관계를 이해하는 데 도움이 됩니다. 이러한 관계는 기능 또는 예측 변수 및 목표의 형태일 수 있습니다.
...
데이터를 분석하는 동안 인사이트를 발견하는 것은 드문 일이 아닙니다. 다음 예는 타겟 이벤트에 대한 최신성 및 통화 값을 매핑하는 인사이트 예제입니다.
...
![예제 인사이트](../images/jupyterlab/eda/insight.PNG)
...
탐색적 데이터 분석을 마쳤으면 모델 생성을 시작할 준비가 된 것입니다. 또는 파생한 데이터 및 인사이트를 사용하여 Power BI과 같은 도구를 사용하여 대시보드를 만들 수 있습니다.
