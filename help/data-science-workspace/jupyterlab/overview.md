...
JupiterLab은 Project Jupiter를 위한 웹 기반의 <a href="https://jupyter.org/" target="_blank">유저</a> 인터페이스로, 긴밀하게 통합되어 [!DNL Adobe Experience Platform]있습니다. 데이터 과학자들이 Jupiter 노트북, 코드 및 데이터를 사용하여 작업할 수 있는 인터랙티브한 개발 환경을 제공합니다.
...
JupiterLab의 각 활성 노트북 또는 활동은 커널 세션을 활용합니다. 왼쪽 사이드바에서 **실행 단말기 및 커널** 탭을 확장하여 모든 활성 세션을 찾을 수 있습니다. 노트북 인터페이스의 맨 위 오른쪽에 있는 커널의 유형과 상태를 관찰하여 확인할 수 있습니다. 아래 다이어그램에서 노트북 관련 커널은 **Python 3** 이고 현재 상태는 오른쪽에 회색 원으로 표시됩니다. 빈 원은 유휴 커널을 의미하고 단색 원은 사용 중인 커널을 의미합니다.
...
다음 정보는 읽을 수 있는 최대 데이터 양, 사용된 데이터 유형 및 데이터를 읽는 예상 기간을 정의합니다. Python 및 R의 경우 벤치마크에 40GB RAM으로 구성된 노트북 서버가 사용되었습니다. PySpark 및 Scala의 경우 64GB RAM, 8개 코어, 2개의 DBU로 구성된 데이터베이스 클러스터는 아래 나와 있는 벤치마크에 최대 4명의 작업자를 사용했습니다.

사용한 ExperienceEvent 스키마 데이터는 최대 10억 개(1B) 행에 이르는 1,000개 행에서 다양한 크기로 사용됩니다. PySpark 및 Spark 지표의 경우 XDM 데이터에 10일의 날짜 범위가 사용되었습니다.

애드혹 스키마 데이터가 CTAS로 쿼리 서비스 테이블 만들기를 사용하여 미리 처리되었습니다. 또한 이 데이터는 최대 10억(1B) 행에 이르는 1,000개 행부터 크기가 달라집니다.
