...
## [!DNL JupyterLab]의 [!DNL Experience Platform]
...
| **[!DNL Experience Platform]서비스 통합** | 기본 제공 통합을 사용하면 [!DNL Experience Platform] 내에서 직접 다른 [!DNL JupyterLab] 서비스를 활용할 수 있습니다. 지원되는 통합의 전체 목록은 [다른 Experience Platform 서비스와 통합](#service-integration) 섹션에 나와 있습니다. |
...
표준화 및 상호 운용성은 [!DNL Experience Platform]의 핵심 개념입니다. [!DNL JupyterLab]에 포함된 IDE로 [!DNL Experience Platform]을(를) 통합하면 다른 [!DNL Experience Platform] 서비스와 상호 작용할 수 있으므로 [!DNL Experience Platform]을(를) 최대한 활용할 수 있습니다. [!DNL Experience Platform]에서 다음 [!DNL JupyterLab]개의 서비스를 사용할 수 있습니다.
...
>[!NOTE]
>
>[!DNL Experience Platform]의 일부 [!DNL JupyterLab] 서비스 통합이 특정 커널로 제한됩니다. 자세한 내용은 [커널](#kernels)의 섹션을 참조하십시오.
...
[Adobe Experience Platform](https://platform.adobe.com)의 왼쪽 탐색 열에서 **[!UICONTROL Notebooks]**&#x200B;을(를) 선택합니다. [!DNL JupyterLab]이(가) 완전히 초기화될 때까지 잠시 기다립니다.
...
* **의 모양을 변경하는**&#x200B;보기:[!DNL JupyterLab] 동작
...
[!DNL JupyterLab]에서 오른쪽 상단 모서리의 톱니바퀴 아이콘을 선택하여 *Notebook 서버 구성*&#x200B;을 엽니다. 슬라이더를 사용하여 GPU를 켜고 필요한 메모리 양을 할당할 수 있습니다. 할당할 수 있는 메모리 양은 조직이 프로비저닝한 양에 따라 다릅니다. **[!UICONTROL Update configs]**&#x200B;을(를) 선택하여 저장합니다.
...
[!DNL JupyterLab]에서 세션을 종료하여 추가 리소스가 사용되지 않도록 할 수 있습니다. 먼저 **전원 아이콘** ![전원 아이콘](/help/images/icons/power.png)을 선택한 다음 세션을 종료하는 것으로 표시되는 팝오버에서 **[!UICONTROL Shut Down]**&#x200B;을(를) 선택하십시오. 12시간 동안 아무 활동이 없으면 노트북 세션이 자동으로 종료됩니다.

[!DNL JupyterLab]을(를) 다시 시작하려면 전원 아이콘 바로 왼쪽에 있는 **다시 시작 아이콘** ![다시 시작 아이콘](/help/images/icons/restart.png)을(를) 선택한 다음 표시되는 팝오버에서 **[!UICONTROL Restart]**&#x200B;을(를) 선택하십시오.
...
노트북 커널은 노트북 셀을 처리하는 언어별 컴퓨팅 엔진입니다. [!DNL Python]은(는) [!DNL JupyterLab] 외에도 R, PySpark 및 [!DNL Spark]&#x200B;(Scala)에서 추가 언어 지원을 제공합니다. 전자 필기장 문서를 열면 연결된 커널이 실행됩니다. 노트북 셀이 실행될 때, 커널은 계산을 수행하고 상당한 CPU 및 메모리 리소스를 소비할 수 있는 결과를 생성한다. 할당된 메모리는 커널을 종료할 때까지 해제되지 않습니다.
...
| [!DNL Query Service] | 규모에 맞게 데이터를 분석하는 샘플 워크플로우를 제공하는 [!DNL Query Service]에서 직접 [!DNL JupyterLab]의 사용을 보여 주는 미리 채워진 전자 필기장입니다. |
