---
keywords: Experience Platform;홈;인기 항목;네임스페이스;네임스페이스;네임스페이스;ID 네임스페이스;ID 네임스페이스;ID 네임스페이스;ID;ID 서비스;ID 서비스
...
ID 네임스페이스를 사용하여 작업하려면 관련된 다양한 Adobe Experience Platform 서비스에 대한 이해가 필요합니다. 네임스페이스로 작업을 시작하기 전에 다음 서비스에 대한 설명서를 검토하십시오.
...
>title="ID 유형 지정"
>abstract="ID 유형은 ID 그래프에 데이터가 저장되는지 여부를 제어합니다. 사람이 아닌 식별자는 저장되지 않으며 다른 모든 ID 유형은 저장됩니다."
...
>title="통합 ID 보기"
>abstract="통합 ID는 다른 시스템과 연결하는 데 사용되는 네임스페이스이며 ID 해결이나 ID 결합에는 사용되지 않습니다. <br> 이러한 ID는 기본적으로 숨겨져 있습니다. 토글을 사용하여 통합 네임스페이스를 봅니다."
