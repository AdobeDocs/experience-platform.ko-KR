...
* 이 시나리오에서 수신 경험 이벤트는 2개의 CRMID와 1개의 GAID를 포함합니다. 이러한 네임스페이스 사이에서 CRMID는 네임스페이스 우선 순위가 가장 높은 네임스페이스로 구성됩니다. 그러나 CRMID가 2개이므로 **프로필은 이 경험 이벤트를 저장하지** 않습니다.

**시나리오 3: 1개의 CRMID, 2개의 GAID**

* 이 시나리오에서 수신 경험 이벤트는 1개의 CRMID와 2개의 GAID를 포함합니다. 이러한 네임스페이스 사이에서 CRMID는 네임스페이스 우선 순위가 가장 높은 네임스페이스로 구성됩니다. CRMID가 하나만 있으므로 네임스페이스 우선 순위가 가장 높은 네임스페이스의 ID가 하나만 있으므로 프로필이 경험 이벤트를 수집합니다.
