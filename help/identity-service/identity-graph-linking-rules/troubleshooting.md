...
네임스페이스 우선 순위의 컨텍스트에서 프로필은 **지정된 수신 이벤트**&#x200B;에서 네임스페이스 우선 순위가 가장 높은 두 개 이상의 ID가 포함된 모든 이벤트를 거부합니다. 예를 들어 ID 설정이 다음과 같이 구성되어 있다고 가정해 보겠습니다.

| 네임스페이스 | 그래프별로 고유 | 우선 순위 |
| --- | --- | --- |
| CRMID | ✔️ | 1 |
| GAID | | 2 |
| ECID | | 3 |

각 시나리오에 대해 경험 이벤트에 다음 이벤트가 포함되어 있다고 가정합니다.

**시나리오 1: 2개의 GAID, 1개의 ECID**

* 이 시나리오에서 수신 경험 이벤트는 2개의 GAID와 1개의 ECID를 포함합니다. 이러한 네임스페이스 사이에서 GAID는 네임스페이스 우선 순위가 가장 높은 네임스페이스로 구성됩니다. 그러나 GAID가 2개이므로 **프로필은 이 경험 이벤트를 저장하지** 않습니다.

**시나리오 2: CRMID 2개, GAID 1개**

* 이 시나리오에서 수신 경험 이벤트는 2개의 CRMID와 1개의 GAID를 포함합니다. 이러한 네임스페이스 사이에서 CRMID는 네임스페이스 우선 순위가 가장 높은 네임스페이스로 구성됩니다. 그러나 GAID가 2개이므로 **프로필은 이 경험 이벤트를 저장하지** 않습니다.

**시나리오 3: 1개의 CRMID, 2개의 GAID**

* 이 시나리오에서 수신 경험 이벤트는 1개의 CRMID와 2개의 GAID를 포함합니다. 이러한 네임스페이스 사이에서 CRMID는 네임스페이스 우선 순위가 가장 높은 네임스페이스로 구성됩니다. CRMID가 하나만 있으므로 네임스페이스 우선 순위가 가장 높은 네임스페이스의 인스턴스가 하나만 있으므로 프로필이 경험 이벤트를 수집합니다.
...
ID 최적화 알고리즘이 먼저 적용되어 개인 엔티티 표현을 보장합니다. 그런 다음 그래프가 [ID 그래프 보호](../guardrails.md)(그래프당 50ID)을(를) 초과하려고 하면 이 논리가 적용됩니다. 네임스페이스 우선 순위는 50 ID/그래프 가드레일의 삭제 논리에 영향을 주지 않습니다.
...
| 세분화 | 4개의 세그먼트 정의를 만듭니다(**참고**: 각 세그먼트 정의 쌍에는 일괄 처리 및 다른 스트리밍을 사용하여 평가된 한 쌍이 있어야 합니다.) <ul><li>세그먼트 정의 A: John의 인증된 이벤트 및/또는 속성에 따른 세그먼트 자격.</li><li>세그먼트 정의 B: Jane의 인증된 이벤트 및/또는 속성에 따른 세그먼트 자격.</li></ul> | John과 Jane은 공유 장치 시나리오에 관계없이 항상 해당 세그먼트에 대한 자격이 있어야 합니다. |
