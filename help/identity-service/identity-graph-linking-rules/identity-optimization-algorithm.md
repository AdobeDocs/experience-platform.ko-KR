...
네임스페이스 우선 순위와 전체 기능 및 사용에 대한 자세한 내용은 다음을 참조하십시오. [네임스페이스 우선 순위 안내서](./namespace-priority.md).
...
이 예에서는 CRM ID와 이메일이 모두 고유한 네임스페이스로 지정됩니다. 위치 `timestamp=0`: 고유한 네임스페이스 구성 때문에 CRM 레코드 데이터 세트가 수집되고 두 개의 다른 그래프를 만듭니다. 각 그래프에는 CRM ID 및 이메일 네임스페이스가 포함되어 있습니다.
