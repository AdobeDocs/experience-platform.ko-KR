...
* **네임스페이스 우선 순위**: 네임스페이스의 우선 순위는 특정 ID 그래프의 다른 네임스페이스와 비교하여 상대적 중요도를 결정합니다. 예를 들어 ID 그래프에 CRM ID, ECID, 이메일 및 Apple IDFA의 4가지 네임스페이스가 있는 경우 우선순위를 구성하여 4가지 네임스페이스에 대한 중요도 순서를 결정할 수 있습니다.
