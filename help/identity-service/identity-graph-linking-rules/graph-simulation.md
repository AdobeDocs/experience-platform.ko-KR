...
Adobe Experience Platform UI에서 [!DNL Graph Simulation]에 액세스할 수 있습니다. 왼쪽 탐색에서 **[!UICONTROL Identities]**&#x200B;을(를) 선택한 다음 상단 헤더에서 **[!UICONTROL Graph Simulation]**&#x200B;을(를) 선택합니다.
...
이벤트: **[!UICONTROL Events]** 패널을 사용하여 그래프를 시뮬레이션할 ID를 추가합니다. 정규화된 ID에는 ID 네임스페이스 및 해당 ID 값이 있어야 합니다. 그래프를 시뮬레이션하려면 적어도 두 개의 ID를 추가해야 합니다. **[!UICONTROL Load Example]**&#x200B;을(를) 선택하여 사전 구성된 이벤트 및 알고리즘 설정을 입력할 수도 있습니다.

![그래프 시뮬레이션 도구의 이벤트 패널입니다.](../images/graph-simulation/events.png)

>[!TAB 알고리즘 구성]

알고리즘 구성: **[!UICONTROL Algorithm configuration]** 패널을 사용하여 네임스페이스에 대한 최적화 알고리즘을 추가하고 구성합니다. 네임스페이스를 드래그 앤 드롭하여 해당 우선 순위 순위를 수정할 수 있습니다. **[!UICONTROL Unique Per Graph]**&#x200B;을(를) 선택하여 네임스페이스가 고유한지 확인할 수도 있습니다.
...
시작하려면 **[!UICONTROL Add events]**&#x200B;을(를) 선택하십시오.

![이벤트 추가 단추를 선택했습니다.](../images/graph-simulation/add-events.png)

[!UICONTROL Event #1]에 대한 팝업 창이 나타납니다. 여기에서 ID 네임스페이스 및 ID 값 조합을 입력합니다. 드롭다운 메뉴를 사용하여 ID 네임스페이스를 선택할 수 있습니다. 또는 네임스페이스의 처음 몇 글자를 입력한 다음 드롭다운 메뉴에 제공된 옵션을 선택할 수 있습니다. 네임스페이스를 선택하면 네임스페이스에 해당하는 ID 값을 제공합니다.
...
그런 다음 동일한 단계를 반복하고 두 번째 ID를 추가합니다. ID 그래프를 생성하려면 2개의 정규화된 ID가 필요합니다. 아래 예에서는 ECID가 네임스페이스로 추가되고 값 `111`이(가) 제공됩니다. 완료되면 **[!UICONTROL Save]**&#x200B;을(를) 선택합니다.

![{ECID: 111}의 두 번째 ID가 이벤트 #1에 추가되었습니다.](../images/graph-simulation/first-event.png)

[!UICONTROL Events] 인터페이스가 업데이트되어 첫 번째 이벤트가 표시됩니다(이 경우: `{Email: tom@acme.com, ECID: 111}`).
...
미리 설정된 알고리즘과 이벤트 구성을 사용하여 예제 그래프를 설정하려면 **[!UICONTROL Load example]**&#x200B;을(를) 선택하십시오.
...
완료되면 **[!UICONTROL Simulate]**&#x200B;을(를) 선택합니다.

![잘못된 휴대폰에 대한 예제 그래프가 시뮬레이트되었습니다.](../images/graph-simulation/example-simulated.png)

### 텍스트 버전 사용 {#use-text-version}

텍스트 모드를 사용하여 이벤트를 구성할 수도 있습니다. 텍스트 모드를 사용하려면 설정 아이콘을 선택한 다음 **[!UICONTROL Text (Advanced users)]**&#x200B;을(를) 선택합니다.
...
이벤트를 편집하려면 지정된 이벤트 옆의 생략 부호(`...`)를 선택한 다음 **[!UICONTROL Edit]**&#x200B;을(를) 선택합니다.

![이벤트 편집 아이콘을 선택했습니다.](../images/graph-simulation/edit.png)

### 이벤트 삭제 {#delete-event}

이벤트를 삭제하려면 지정된 이벤트 옆의 생략 부호(`...`)를 선택한 다음 **[!UICONTROL Delete]**&#x200B;을(를) 선택합니다.
...
이벤트를 추가한 후에는 그래프를 시뮬레이션하는 데 사용할 알고리즘을 구성할 수 있습니다. 시작하려면 **[!UICONTROL Add config]**&#x200B;을(를) 선택하십시오.

![알고리즘 구성 패널입니다.](../images/graph-simulation/add-config.png)

빈 구성 행이 나타납니다. 먼저 이벤트에 사용한 것과 동일한 네임스페이스를 입력합니다. 이 경우 이메일 입력부터 시작합니다. 네임스페이스를 입력하면 [!UICONTROL Identity Symbol] 및 [!UICONTROL Identity Type]의 열이 자동으로 채워집니다.
...
네임스페이스 우선 순위를 구성하려면 네임스페이스 행을 선택하고 원하는 우선 순위 순서로 드래그합니다. 맨 위 행은 더 높은 우선 순위를 나타내고 맨 아래 행은 더 낮은 우선 순위를 나타냅니다. 네임스페이스를 고유하게 지정하려면 **[!UICONTROL Unique Per Graph]** 확인란을 선택하십시오.
...
[!UICONTROL Simulated Graph] 섹션에는 추가한 이벤트와 구성한 알고리즘을 기반으로 생성된 ID 그래프가 표시됩니다.
