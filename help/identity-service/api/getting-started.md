...
다음 [!DNL Identity Service] API는 `{REGION}` 를 요청 경로의 일부로 포함했습니다. 조직 프로비저닝 중에 영역이 조직 프로필 내에서 결정되고 저장됩니다. 각 종단점에 올바른 영역을 사용하면 [!DNL Identity Service] API는 해당 영역으로 라우팅됩니다.
...
조직 프로필 내에서 해당 지역을 찾을 수 없는 경우 시스템 관리자에게 문의하여 지원을 받으십시오.
