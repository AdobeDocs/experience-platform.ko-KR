...
오늘날 조직과 비즈니스는 대량의 서로 다른 데이터 세트에 직면해 있습니다. 개별 고객은 다양한 서로 다른 식별자로 표시됩니다. 고객은 다양한 웹 브라우저(Safari, Google Chrome), 하드웨어 장치(휴대폰, 노트북) 및 기타 개인 식별자(CRMID, 이메일 계정)에 연결할 수 있습니다. 이렇게 하면 고객에 대한 분리된 보기가 만들어집니다.
...
* 사용자 이름과 암호 조합으로 랩톱을 사용하여 전자 상거래 웹 사이트에 로그인합니다. 이 이벤트는 사용자를 인증된 사용자로 인증하므로 ID 서비스에서 CRMID를 인식합니다.
