...
**장치를 사용하는 실제 사용자를 식별하지 않고도 알 수 없거나 익명** ID가 분리됩니다. 이 카테고리에는 방문자의 IP 주소 및 쿠키 ID와 같은 정보가 포함됩니다. 알 수 없는 ID를 사용하여 장치에서 행동 데이터를 수집할 수 있지만, 장치 또는 미디어에 이러한 ID를 연결하는 것은 고객이 여행 중에 PII를 제공할 때까지 제한됩니다.
