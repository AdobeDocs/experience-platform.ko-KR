...
* **표시 이름**: 해당 네임스페이스에 대한 알기 쉬운 이름.
* **ID 심볼**: 네임스페이스를 나타내기 위해 ID 서비스에서 내부적으로 사용되는 코드입니다.
* **ID 유형**: 해당 네임스페이스의 분류.
* **설명**: (선택 사항) 주어진 네임스페이스와 관련하여 제공할 수 있는 모든 추가 정보입니다.
...
| TNTID | Adobe Target을 나타내는 네임스페이스입니다. 에 대한 다음 문서를 참조하십시오. [Target](https://experienceleague.adobe.com/docs/target/using/target-home.html) 추가 정보. |
...
![조직의 사용자 정의 ID 네임스페이스 디렉터리입니다.](../images/namespace/browse.png)
...
다음 [!UICONTROL ID 네임스페이스 만들기] 창이 나타납니다. 먼저 만들려는 사용자 지정 네임스페이스에 대한 표시 이름과 ID 기호를 제공해야 합니다. 필요한 경우 만들고 있는 사용자 지정 네임스페이스에 더 많은 컨텍스트를 추가하는 설명을 제공할 수도 있습니다.

![사용자 지정 ID 네임스페이스와 관련된 정보를 입력할 수 있는 팝업 창입니다.](../images/namespace/name-and-symbol.png)

그런 다음 사용자 지정 네임스페이스에 할당할 ID 유형을 선택합니다. 완료되면 다음을 선택합니다. **[!UICONTROL 만들기]**.

![사용자 정의 ID 네임스페이스에서 선택하고 할당할 수 있는 ID 유형 선택.](../images/namespace/select-identity-type.png)
...
ID에 대한 네임스페이스를 제공하는 것은 ID 데이터를 제공하는 데 사용하는 메서드에 따라 다릅니다. 데이터 ID 데이터 제공에 대한 자세한 내용은 [[!DNL Identity Service] 구현 안내서](../implementation.md).

## 다음 단계

이제 ID 네임스페이스의 주요 개념을 이해했으므로 다음을 사용하여 ID 그래프로 작업하는 방법을 배울 수 있습니다. [id 그래프 뷰어](../features/identity-graph-viewer.md).
