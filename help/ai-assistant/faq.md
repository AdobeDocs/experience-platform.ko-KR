...
description: AI Assistant(이전)와 관련하여 자주 묻는 질문에 대한 답변에 대해 알아봅니다.
...
다음은 AI Assistant(기존)에 대한 FAQ 응답 목록입니다.

## AI 비서 정보가 실시간으로 제공됩니까?

AI Assistant(이전) 응답에 제공된 데이터는 매일 업데이트됩니다. 즉, 응답의 데이터는 응답 시 Experience Platform 사용자 인터페이스에 표시될 수 있는 데이터보다 최대 24시간 더 오래될 수 있습니다.

액세스 제어에도 동일한 원칙이 적용됩니다. AI Assistant(기존)가 액세스 제어 및 권한 구성에서 변경된 사항을 반영하는 데 최대 24시간이 걸릴 수 있습니다. 자세한 내용은 [AI Assistant의 개인 정보 보호](./privacy.md)에 대한 안내서를 참조하십시오.
...
AI Assistant(기존)는 Adobe 제품 지식 쿼리를 처리할 수 있으며 데이터 객체의 운영 통찰력과 관련된 질문에 답변할 수 있습니다. (예: &quot;활성화된 대상이 몇 명입니까?&quot;)

## AI 어시스턴트가 프로필 데이터에 대한 정보를 제공할 수 있습니까?

아니요. AI Assistant(레거시)는 프로필 수준 데이터에 액세스할 수 없으므로 AI Assistant(레거시)에서 이를 보거나 사용하지 않습니다.

## AI Assistant의 교육 데이터에 내 개인 정보가 활용됩니까?

AI Assistant(기존)는 교육 목적으로 개인 정보를 사용하지 않습니다. 본인에 대한 개인 정보(이름 또는 연락처 정보 포함) 또는 AI Assistant(Legacy)의 다른 당사자에 대한 개인 정보를 제공하지 마십시오.

## 답변을 확인할 때 AI Assistant에서 반환한 SQL 쿼리를 실행할 수 있습니까?

아니요. 이 SQL 쿼리는 읽기 전용이며 AI Assistant(기존) 또는 Query Service에서 실행할 수 없습니다.