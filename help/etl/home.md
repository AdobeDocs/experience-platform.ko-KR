...
데이터가 [!DNL Experience Platform] 사용 [데이터 수집 API](https://www.adobe.io/experience-platform-apis/references/data-ingestion/).  데이터 쓰기는 비동기 프로세스입니다. Adobe Experience Platform에 데이터를 기록하면 데이터가 완전히 작성된 후에만 일괄 처리가 생성되고 성공으로 표시됩니다.
...
데이터가 처리되면 ETL 도구가 데이터를 다시 [!DNL Experience Platform] 사용 [배치 수집 API](https://www.adobe.io/experience-platform-apis/references/data-ingestion/). 데이터를 데이터 세트에 추가하려면 먼저 특정 데이터 세트에 업로드하는 일괄 처리에 연결해야 합니다.
