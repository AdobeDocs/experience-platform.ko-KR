...
데이터가 다음에 기록됩니다. [!DNL Experience Platform] 사용 [데이터 수집 API](https://www.adobe.io/experience-platform-apis/references/data-ingestion/).  데이터 쓰기는 비동기 프로세스입니다. 데이터가 Adobe Experience Platform에 기록되면 데이터가 완전히 기록된 후에만 배치가 만들어지고 성공으로 표시됩니다.
...
데이터가 처리되면 ETL 툴은 데이터를에 다시 기록합니다 [!DNL Experience Platform] 사용 [일괄 처리 수집 API](https://www.adobe.io/experience-platform-apis/references/data-ingestion/). 데이터를 데이터 세트에 추가하려면 먼저 나중에 특정 데이터 세트에 업로드되는 배치에 연결해야 합니다.
