...
* **범위**: 결정의 범위입니다.
   ...
   * 대상 [!DNL Offer Decisioning], offer decisioning 서비스에서 오퍼를 제안하는 데 사용할 활동 및 배치 ID가 포함된 JSON의 Base64 인코딩 문자열입니다.
...
| 속성 | 유형 | 설명 |
| --- | --- | --- |
| `schemas` | `String[]` | *선택 사항입니다*. 반환된 오퍼 유형을 선택하는 데 사용된 스키마 목록입니다. |
...
| `payload.scopeDetails.decisionProvider` | 문자열 | 다음 중 하나가 됩니다. `TGT` Adobe Target 사용 시 `ODE` 를 [!DNL Offer Decisioning]. |
