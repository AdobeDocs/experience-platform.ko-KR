...
CEE 믹스에는 유용한 인사이트를 도출하기 위해 활용해야 하는 몇 가지 주요 필드가 있습니다. 이 섹션에서는 이러한 필드의 사용 사례와 예상 데이터에 대해 설명하고 추가 예제에 대한 참조 설명서에 대한 링크를 제공합니다.

### 필수 필드

모든 키 필드를 사용하는 것이 좋지만, Intelligent Services가 작동하려면 두 가지 필드가 **필요합니다** .
...
스키마의 필드 중 하나를 기본 ID 필드로 설정해야 합니다. 이 필드를 통해 Intelligent Services는 시간 시리즈 데이터의 각 인스턴스를 개별 사용자에게 연결할 수 있습니다.
...
키 필드의 나머지 부분은 이 섹션에 요약되어 있습니다. 이러한 필드는 Intelligent Services가 작동하기 위해 반드시 필요한 것은 아니지만, 보다 풍부한 통찰력을 얻기 위해 가능한 한 많이 사용하는 것이 좋습니다.
...
데이터 세트를 만든 후 데이터 세트 작업 공간 내의 Platform UI에서 데이터 *[!UICONTROL 세트를]* 찾을 수 있습니다.
...
다음 요청은 Analytics에 대한 적절한 태그 값을 추가합니다.
...
CEE 스키마 및 데이터 세트를 만든 후 데이터 테이블을 스키마에 매핑하고 해당 데이터를 Platform에 인제스트할 수 있습니다. UI에서 CSV 파일을 [XDM 스키마에](../ingestion/tutorials/map-a-csv-file.md) 매핑하는 방법에 대한 자세한 내용은 자습서를 참조하십시오. 데이터 세트를 채운 후에는 동일한 데이터 세트를 사용하여 추가 데이터 파일을 인제스트할 수 있습니다.

데이터가 지원되는 타사 애플리케이션에 저장되어 있는 경우 [소스 커넥터를](../sources/home.md) 만들어 마케팅 이벤트 데이터를 실시간으로 Platform에 인제스트하도록 할 수도 있습니다.
