...
성공한 응답은 추출된 색상의 세부 정보를 반환합니다. 각 색상은 다음 정보가 포함된 `feature_value` 키로 표시됩니다.

- 색상 이름
- 이미지를 기준으로 이 색상이 표시되는 비율
- 색상의 RGB 값

`"White":{"coverage":0.5834,"rgb":{"red":254,"green":254,"blue":243}}`은(는) 이미지의 58.34%에 있는 흰색이며 평균 RGB 값이 254, 254, 243임을 의미합니다.
...
다음 예제 요청에서는 색상 태깅에 마스킹 방법을 사용합니다. 이 기능은 요청에서 `enable_mask` 매개 변수를 `true`(으)로 설정하여 사용할 수 있습니다.
...
>[!NOTE]
>
>또한 위의 요청에서 `retrieve_tone` 매개 변수도 `true`(으)로 설정되어 있습니다. 이를 통해 이미지의 전체, 전경 및 배경 영역에서 따뜻한 톤, 중간 톤 및 차가운 톤에 대한 톤 분포 히스토그램을 검색할 수 있습니다.
...
| `top_n` | 번호 | 아니요 | 0 | 음이 아닌 정수 | 반환할 결과 수. 0: 모든 결과를 반환합니다. 임계값과 함께 사용할 경우 반환되는 결과 수는 두 제한 중 더 적습니다. |
| `min_coverage` | 번호 | 아니요 | 0.05 | 실수 | 결과를 반환해야 하는 적용 범위의 임계값. 매개 변수를 제외하여 모든 결과를 반환합니다. |
| `resize_image` | 번호 | 아니요 | True | True/False | 입력 이미지의 크기를 조정할지 여부를 지정합니다. 기본적으로 이미지 크기는 색상 추출을 수행하기 전에 320*320픽셀로 조정됩니다. 디버깅의 목적으로도 이 설정을 `False`(으)로 설정하여 전체 이미지에서도 코드를 실행하도록 허용할 수 있습니다. |
| `enable_mask` | 번호 | 아니요 | False | True/False | 색상 추출 활성화/비활성화 |
| `retrieve_tone` | 번호 | 아니요 | False | True/False | 톤 추출 활성화/비활성화 |
