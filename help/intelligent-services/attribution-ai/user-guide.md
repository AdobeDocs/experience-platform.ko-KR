...
여러 데이터 세트가 서로 조인하려면 ID 유형(&quot;ID 네임스페이스&quot;라고도 함)과 해당 네임스페이스 내의 ID 값을 선택해야 합니다. 동일한 네임스페이스 아래에 두 개 이상의 필드를 스키마 내에 ID로 할당한 경우 네임스페이스가 앞에 추가하는 ID 드롭다운에 할당된 모든 ID 값이 표시됩니다 `EMAIL (personalEmail.address)` 또는 `EMAIL (workEmail.address)`.
...
![동일한 네임스페이스 선택](./images/user-guide/identity-type.png)
...
전환 이벤트를 정의하려면 이벤트에 이름을 지정하고 을(를) 선택하여 이벤트 유형을 선택해야 합니다. **필드 이름 입력** 드롭다운 메뉴 아래의 제품에서 사용할 수 있습니다.
