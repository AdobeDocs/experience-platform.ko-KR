...
각 데이터 세트가 ECID와 같은 동일한 ID 유형(네임스페이스)을 공유하는 경우 다른 소스에서 여러 데이터 세트를 추가할 수 있습니다. 여러 데이터 세트를 추가하는 방법에 대해 자세히 알아보려면 [Attribution AI 사용 안내서](./user-guide.md#identity).
...
>[!NOTE]
>
> 다음 9개 열은 필수 열이며, 추가 열은 선택 사항이지만, 와 같은 다른 Adobe 솔루션에 대해 동일한 데이터를 사용하려면 권장/필요합니다 [!DNL Customer AI] 및 [!DNL Journey AI].
