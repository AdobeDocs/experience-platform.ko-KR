...
고객 AI를 위한 가이드는 고객 AI 사용과 관련된 다양한 Platform 서비스에 대한 작업 이해를 필요로 합니다. 시작하기 전에 다음 문서를 검토하십시오.
...
- [실시간 고객 프로필 개요](../../rtcdp/overview.md): 실시간 CDP [!DNL Adobe Experience Platform]를 [!DNL Adobe Real-time Customer Data Platform] 통해 알려진 데이터와 알려지지 않은 데이터를 통합하여 고객 여정 전반에서 지능적인 의사 결정을 통해 고객 프로파일을 활성화할 수 있습니다. 실시간 CDP는 여러 엔터프라이즈 데이터 소스를 통합하여 실시간으로 통합 프로파일을 생성하므로 모든 채널과 디바이스에서 개인화된 고객 경험을 제공하는 데 사용할 수 있습니다.
...
- [세그먼트 빌더 사용 안내서](../../segmentation/tutorials/create-a-segment.md): Platform을 사용하면 세그먼트를 쉽게 만들고 액세스할 수 있을 뿐만 아니라, 여러 가지 기본 요소를 사용하여 세그먼트를 더 특정할 수 있습니다.
...
고객 AI 점수 다운로드는 API 호출 조합을 통해 이루어집니다. Platform API를 호출하려면 먼저 [인증 자습서를 완료해야 합니다](../../tutorials/authentication.md). 인증 자습서를 완료하면 아래와 같이 모든 Experience Platform API 호출에서 각 필수 헤더에 대한 값을 제공합니다.
