...
>[!NOTE] Raw 점수를 다운로드할 필요가 없는 경우 이 단계를 건너뛰고 [구성 안내서로 진행할 수 있습니다](./user-guide/configure.md).

고객 AI 점수 다운로드는 API 호출 조합을 통해 이루어집니다. 플랫폼 API를 호출하려면 먼저 [인증 자습서를 완료해야 합니다](../../tutorials/authentication.md). 인증 자습서를 완료하면 아래와 같이 모든 경험 플랫폼 API 호출에서 각 필수 헤더에 대한 값을 제공합니다.
