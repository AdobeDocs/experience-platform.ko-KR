...
이 사용 사례를 실행하려면 [Data Distiller](./overview.md)에 대해 Adobe Experience Platform 인스턴스에 라이선스를 부여해야 합니다. 더 많은 내용은 Adobe 담당자에게 문의하십시오.
...
Experience Platform UI의 왼쪽 탐색 레일에서 **[!UICONTROL Datasets]**&#x200B;을(를) 선택한 후 **[!UICONTROL Create dataset]**&#x200B;을(를) 선택합니다. 그런 다음 사용 가능한 옵션에서 **[!UICONTROL Create dataset from CSV file]**&#x200B;을(를) 선택합니다.

[!UICONTROL Configure Dataset] 패널이 나타납니다. **[!UICONTROL Name]** 필드에서 데이터 세트 이름을 &quot;luma_web_data&quot;로 입력하고 **[!UICONTROL Next]**&#x200B;을(를) 선택합니다.

[!UICONTROL Add data] 패널이 나타납니다. CSV 파일을 **[!UICONTROL Add data]** 상자로 끌어서 놓거나 **[!UICONTROL Choose File]**&#x200B;을(를) 선택하여 파일을 찾아 업로드하십시오.
...
파일이 업로드되면 UI 하단에 데이터 미리보기가 나타납니다. 업로드를 완료하려면 **[!UICONTROL Finish]**&#x200B;을(를) 선택하십시오.
...
은 배치로 수집되고 [!UICONTROL Batch ID]에 의해 식별됩니다. 오른쪽의 패널에 테이블 이름이 `luma_web_data`(으)로 표시됩니다.
...
데이터 처리가 완료되면 오른쪽 상단 모서리에서 [!UICONTROL Preview dataset]을(를) 선택하여 데이터 집합을 미리 봅니다. 데이터 세트 미리보기가 표시되는 방식은 다음과 같습니다.
...
Adobe Experience Platform UI의 왼쪽 탐색 레일에서 **[!UICONTROL Queries]**&#x200B;을(를) 선택한 다음 **[!UICONTROL Create Query]**&#x200B;을(를) 선택합니다. 쿼리 편집기가 나타납니다.
...
쿼리 결과는 **[!UICONTROL Results]** 탭의 쿼리 편집기 아래에 표시됩니다. 새 대화 상자에서 결과를 확장하려면 **[!UICONTROL View results]**&#x200B;을(를) 선택합니다. 결과는 아래 이미지와 유사합니다.
...
데이터 세트를 만든 후 **[!UICONTROL Datasets]** > **[!UICONTROL Browse]** > `adls_rfm_profile`(으)로 이동하여 데이터 세트가 비어 있는지 확인합니다.
...
**[!UICONTROL Schemas]** > **[!UICONTROL Browse]** > `adls_rfm_profile`(으)로 이동하여 새로 만든 데이터 집합의 XDM 개별 프로필 스키마 다이어그램과 해당 사용자 지정 필드 그룹을 볼 수도 있습니다.
...
그런 다음 실시간 고객 프로필에 대해 활성화된 `rfm_model_segment VIEW`에 `adls_rfm_profile`의 데이터를 삽입합니다.

`SELECT` 문의 `INSERT` 쿼리에 있는 필드 순서가 `rfm_model_segment`의 구조와 정확히 일치하는지 확인하십시오. 이렇게 정렬하면 `rfm_model_segment`의 값이 대상 테이블의 해당 필드에 올바르게 삽입됩니다. 원본 필드와 대상 필드 간의 정렬이 잘못되면 데이터가 일치하지 않을 수 있습니다.
...
SQL을 저장한 후 **[!UICONTROL Templates]** 탭으로 이동하여 저장된 쿼리를 보고 예약 프로세스를 시작합니다. 쿼리를 예약하는 방법에는 두 가지가 있습니다.

오른쪽 사이드바에서 **[!UICONTROL Add Schedule]**&#x200B;을(를) 선택합니다.

![일정 추가가 강조 표시된 쿼리 작업 영역 편집 탭입니다.](../images/data-distiller/top-tips-to-maximize-value/add-schedule-1.png)

또는 템플릿 이름 아래의 **[!UICONTROL Schedules]** 탭을 선택한 다음 **[!UICONTROL Add Schedule]**&#x200B;을(를) 선택합니다.
...
[!UICONTROL Schedule details] 보기가 나타납니다. 여기에서 다음 세부 정보를 입력하여 일정을 구성합니다.

- **[!UICONTROL Execution Frequency]**: **주별**
- **[!UICONTROL Day of Execution]**: **월요일 및 화요일**
- **[!UICONTROL Schedule Execution Time]**: 오전 **10:10(UTC)**
- **[!UICONTROL Schedule Period]**: **2025년 3월 17일 - 4월 30일**

**[!UICONTROL Save]**&#x200B;을(를) 선택하여 일정을 확인합니다.

![설정이 구성되어 있고 [저장]이 강조 표시된 예약 세부 정보.](../images/data-distiller/top-tips-to-maximize-value/set-schedule.png)

일정을 저장한 후 언제든지 **[!UICONTROL Scheduled Queries]** 탭으로 이동하여 예약된 데이터 Distiller 작업을 모니터링할 수 있습니다. [쿼리 실행 상태, 오류 메시지 및 경고 보기](../ui/monitor-queries.md)에 대한 자세한 내용은 예약된 쿼리 모니터링 문서를 참조하십시오.
...
`CREATE AUDIENCE AS SELECT` 명령을 사용하여 새 대상을 정의합니다. 만든 대상자는 데이터 집합에 저장되고 **[!UICONTROL Audiences]**&#x200B;의 **[!UICONTROL Data Distiller]** 작업 영역에 등록됩니다.

SQL 확장을 사용하여 만든 대상은 [!UICONTROL Data Distiller] 작업 영역의 [!UICONTROL Audiences] 원본 아래에 자동으로 등록됩니다. [대상 포털](../../segmentation/ui/audience-portal.md)에서 필요에 따라 대상을 보고, 관리하고, 활성화할 수 있습니다.
...
데이터가 실시간 고객 프로필에 로드되었는지 확인하려면 **[!UICONTROL Customers]> [!UICONTROL Profiles] >[!UICONTROL Browse]**(으)로 이동합니다. **[!UICONTROL Identity Namespace]**&#x200B;을(를) `Email`(으)로 선택하고 `user0076@example.com`을(를) 입력합니다. 프로필 세부 사항을 확인하여 예상 RFM 속성이 포함되어 있는지 확인합니다.
...
기존 대상자를 찾아보려면 왼쪽 탐색 패널에서 **[!UICONTROL Audiences]**&#x200B;을(를) 선택하고 **[!UICONTROL Browse]** 탭이 선택되어 있는지 확인하십시오. 샌드박스에서 사용 가능한 대상 목록이 나타납니다. 대상을 선택하면 해당 설명, 자격 규칙 및 포함된 프로필 수가 표시됩니다.

새 대상을 만들려면 오른쪽 상단 모서리에서 **[!UICONTROL Create Audience]**&#x200B;을(를) 선택하십시오. 대화 상자에 두 가지 옵션이 표시됩니다. **[!UICONTROL Build Rule]**&#x200B;을(를) 선택한 후 **[!UICONTROL Create]**&#x200B;을(를) 선택하십시오.

![빌드 규칙을 선택하고 [만들기]가 강조 표시된 대상 만들기 대화 상자.](../images/data-distiller/top-tips-to-maximize-value/create-audience-dialog.png)

대상 구성 UI는 프로필 속성에 대한 액세스를 제공합니다. 사용 가능한 특성을 보려면 **[!UICONTROL Attributes]>[!UICONTROL XDM Individual Profile]**(으)로 이동합니다.
...
대상자를 마무리하려면 오른쪽 상단 모서리에서 **[!UICONTROL Save and Publish]**&#x200B;을(를) 선택합니다. 저장한 후 새로 만든 대상자가 [!UICONTROL Audiences] 작업 영역에 표시되어 요약 및 자격 조건을 검토할 수 있습니다.
