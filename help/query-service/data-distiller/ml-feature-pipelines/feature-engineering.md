...
이 문서에서는 Adobe Experience Platform의 데이터를 **기능**&#x200B;머신 러닝 모델에서 사용할 수 있는 또는 변수입니다. 이 프로세스를 라고 합니다. **기능 엔지니어링**. Data Distiller을 사용하여 규모에 맞게 ML 기능을 계산하고 이러한 기능을 머신 러닝 환경에 공유할 수 있습니다. 여기에는 다음 항목이 포함됩니다.
...
다음 예제에서는 사용자의 뉴스레터 구독 성향을 예측하기 위해 모델에 대한 경험 이벤트 데이터 세트에서 교육 데이터를 도출하는 쿼리를 보여 줍니다. 구독 이벤트는 이벤트 유형으로 표시됩니다. `web.formFilledOut`, 및 데이터 세트의 기타 행동 이벤트는 구독을 예측하기 위한 프로필 수준 기능을 파생하는 데 사용됩니다.
...
이 경우 레이블은 이라는 변수입니다. `subscriptionOccurred` 사용자 프로필에 유형이 있는 이벤트가 있는 경우 1입니다 `web.formFilledOut` 및 그렇지 않으면 0입니다. 다음 쿼리는 양의 레이블( )이 있는 모든 사용자를 포함하여 이벤트 데이터 세트에서 50,000명의 사용자 집합을 반환합니다.`subscriptionOccurred = 1`)에 음수 레이블을 사용하여 임의로 선택한 사용자 세트를 추가하여 50,000개의 사용자 샘플 크기를 완료합니다. 이렇게 하면 학습 데이터에 모델이 학습할 긍정적인 사례와 부정적인 예가 모두 포함됩니다.
...
다음 쿼리는 데이터 세트의 이벤트를 성향 모델을 교육하는 데 사용할 수 있는 의미 있는 숫자 기능으로 집계합니다.

- **이메일 수** 마케팅 목적으로 전송되고 사용자가 수신했습니다.
- 이 이메일 중 **열림**.
- 사용자가 있는 이러한 이메일 일부 **선택됨** 링크를 클릭합니다.
- **제품 수** 조회되었습니다.
- 의 수 **상호 작용한 제안**.
- 의 수 **기각된 제안**.
- 의 수 **선택된 링크**.
...
- 존재하지 않는 경우 새 교육 데이터 세트를 만들고, 그렇지 않은 경우 기존 교육 데이터 세트에 새 레이블과 기능을 삽입하는 논리를 추가합니다. 이렇게 하려면 일련의 두 버전의 교육 집합 쿼리가 필요합니다.
   - 먼저, 를 사용합니다. `CREATE TABLE IF NOT EXISTS {table_name} AS` 구문
   - 다음, 사용 `INSERT INTO {table_name}` 교육 데이터 세트가 이미 있는 경우의 문
- 추가 `SNAPSHOT BETWEEN $from_snapshot_id AND $to_snapshot_id` 문을 사용하여 쿼리를 지정된 간격 내에 추가된 이벤트 데이터로 제한할 수 있습니다. 다음 `$` 스냅샷 ID의 접두사는 쿼리 템플릿 실행 시 전달될 변수임을 나타냅니다.
...
이 문서를 읽고 Adobe Experience Platform의 데이터를 기계 학습 모델에서 사용할 수 있는 기능 또는 변수로 변환하는 방법을 배웠습니다. 머신 러닝 환경에서 Experience Platform에서 피드 사용자 지정 모델로의 기능 파이프라인을 만드는 다음 단계는 다음과 같습니다 [기능 데이터 세트 내보내기](./export-data.md).
