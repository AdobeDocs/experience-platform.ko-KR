...
| 최대 실행 시간 | 10분 | 24시간 |
| 라이선스 지표 | **사용자 동시성 쿼리**: <ul><li>동시 사용자 1명(Real-Time CDP, Adobe Journey Optimizer&#x200B;)</li><li>동시 사용자 5명(Customer Journey Analytics&#x200B;)</li></ul> **쿼리 동시 실행**: <ul><li>동시 실행 쿼리 1개(모든 애플리케이션)&#x200B;</li></ul> **추가 임시 쿼리 사용자 팩 추가 기능** 을 구입하여 인증된 임시 쿼리 권한을 늘릴 수 있습니다. <ul><li>+팩당 추가 동시 사용자 5명</li><li>+1개의 추가 동시 실행 쿼리/팩</li></ul> | **시간 계산**: <ul><li>변수(애플리케이션 권한에 따라 범위 지정)</li></ul> **시간 계산** 는 일괄 처리 쿼리가 실행될 때 쿼리 서비스 엔진에서 데이터를 읽고 처리하고 데이터 레이크에 다시 쓰는 데 걸린 시간을 측정한 것입니다. <br>Data Distiller SKU를 사용하면 추가 사용자 및 쿼리 동시성을 얻을 수 있으며, 이는 임시 쿼리 실행에 사용할 수 있습니다.  Data Distiller SKU에는 다음이 포함됩니다.<br><ul><li>+5명의 추가 동시 사용자</li><li>+1개의 추가 동시 실행 쿼리</li></ul> |
