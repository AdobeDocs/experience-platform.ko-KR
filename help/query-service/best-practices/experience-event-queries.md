...
XDM 및 [!DNL Experience Events]에 대한 자세한 내용은 [[!DNL XDM System] overview](../../xdm/home.md)에서 확인할 수 있습니다. [!DNL Query Service]과 [!DNL Experience Events]을 결합하여 사용자 간의 행동 트렌드를 효과적으로 추적할 수 있습니다. 다음 문서에서는 [!DNL Experience Events]과 관련된 쿼리의 예를 제공합니다.

## 특정 날짜 범위에 대한 일별 이벤트의 트렌드 보고서 만들기

다음 예제에서는 지정된 날짜 범위 위에 날짜별로 그룹화된 이벤트의 트렌드 보고서를 만듭니다. 특히 A, B 및 C로 다양한 분석 값을 합한 다음 파카를 본 횟수를 합산합니다.

[!DNL Experience Event] 데이터 집합에 있는 타임스탬프 열이 UTC에 있습니다. 다음 예제에서는 `from_utc_timestamp()` 함수를 사용하여 타임스탬프를 UTC에서 EDT로 변환합니다. 그런 다음 `date_format()` 함수를 사용하여 나머지 타임스탬프에서 날짜를 분리합니다.
...
다음 예제에서는 가장 많은 페이지를 본 사용자의 ID를 나열하는 보고서를 만듭니다.
...
다음 예제에서는 지정된 사용자가 본 마지막 100개 페이지를 보여 줍니다.
