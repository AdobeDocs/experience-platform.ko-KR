...
>[!NOTE]
>
>자격 증명 패널은 모든 사용자가 자동으로 사용할 수 있는 것은 아닙니다. 필요한 경우 Adobe 계정 팀에 문의하여 [!UICONTROL Credentials] 탭을 쿼리 서비스 작업 영역에 포함하도록 요청하십시오. 요청할 경우 이러한 변경은 조직 전체에 적용되며 Adobe 엔지니어링 팀에 의해 수행됩니다. 사용자가 제어하는 설정이 아닙니다.
...
**[!UICONTROL Expiring credentials]** 섹션은 다음 정보를 제공합니다.

- **[!UICONTROL Host]**: 클라이언트를 연결할 호스트의 이름입니다. 여기에 Experience Platform UI의 상단 리본에 표시된 조직 이름이 통합됩니다.
- **[!UICONTROL Port]**: 연결할 호스트의 포트 번호입니다.
- **[!UICONTROL Database]**: 클라이언트를 연결할 데이터베이스의 이름입니다.
- **[!UICONTROL Username]**: 쿼리 서비스에 연결하는 데 사용되는 사용자 이름입니다.
- **[!UICONTROL Password]**: 쿼리 서비스에 연결하는 데 사용되는 암호입니다. 보안을 위해 UI의 암호가 해시되었습니다. 복사 아이콘(![복사 아이콘)을 선택합니다.](/help/images/icons/copy.png)) 해시되지 않은 전체 자격 증명을 클립보드에 복사합니다.
- **[!UICONTROL PSQL command]**: 명령줄에서 PSQL을 사용하여 쿼리 서비스에 연결할 수 있도록 모든 관련 정보를 자동으로 삽입한 명령입니다.
- **[!UICONTROL Expires]**: 만료되는 자격 증명의 만료 날짜 및 시간입니다. 토큰의 기본 유효 기간은 24시간이지만 Admin Console의 고급 설정에서 변경할 수 있습니다.
...
Power BI 또는 Tableau에서 Customer Journey Analytics 데이터에 액세스하려면 [!UICONTROL Database] 드롭다운 메뉴를 선택한 다음 사용 가능한 옵션에서 `prod:cja`을(를) 선택하십시오. 그런 다음 Power BI 또는 Tableau 구성에서 사용할 [!DNL Postgres] 자격 증명 매개 변수(호스트, 포트, 데이터베이스, 사용자 이름 등)를 복사합니다.
...
>[!IMPORTANT]
>
>만료되지 않는 자격 증명을 OAuth 서버 간 자격 증명으로 처음 만들거나 마이그레이션할 때는 시스템 관리 계정을 사용해야 합니다. 시스템 관리자만 조직에 대해 이 작업을 수행할 수 있습니다. 시스템 관리자가 아닌 사용자가 이 단계를 수행하면 프로세스가 실패하고 인증 오류가 발생합니다. 초기 설정 후 필요한 권한을 가진 사용자가 만료되지 않는 후속 자격 증명을 만들거나 마이그레이션할 수 있습니다.
...
만료되지 않는 자격 증명 집합을 만들려면 Experience Platform UI로 돌아가서 왼쪽 탐색에서 **[!UICONTROL Queries]**&#x200B;을(를) 선택하여 [!UICONTROL Queries] 작업 영역에 액세스합니다. **[!UICONTROL Credentials]** 탭 다음에 **[!UICONTROL Generate credentials]**&#x200B;을(를) 선택합니다.
...
- **[!UICONTROL Name]**: 생성 중인 자격 증명의 이름입니다.
- **[!UICONTROL Description]**: (선택 사항) 생성 중인 자격 증명에 대한 설명입니다.
- **[!UICONTROL Assigned to]**: 자격 증명을 할당할 사용자입니다. 이 값은 자격 증명을 만드는 사용자의 이메일 주소여야 합니다.
- **[!UICONTROL Password]**(선택 사항) 자격 증명의 선택적 암호입니다. 암호를 설정하지 않으면 Adobe에서 자동으로 암호를 생성합니다.

필요한 모든 세부 정보를 제공했으면 **[!UICONTROL Generate credentials]**&#x200B;을(를) 선택하여 자격 증명을 생성합니다.

![자격 증명 생성 대화 상자가 강조 표시됩니다.](../images/ui/credentials/create-account.png)

>[!IMPORTANT]
>
>**[!UICONTROL Generate credentials]**&#x200B;을(를) 선택하면 구성 JSON 파일이 로컬 컴퓨터에 다운로드됩니다. Adobe은 생성된 자격 증명을 **기록하지**&#x200B;하므로 다운로드한 파일을 안전하게 저장하고 자격 증명의 기록을 보관해야 합니다.
>
...
생성된 자격 증명을 저장한 후 **[!UICONTROL Close]**&#x200B;을(를) 선택합니다. 이제 만료되지 않는 모든 자격 증명 목록을 볼 수 있습니다.
...
필요한 모든 세부 정보를 제공했으면 **[!UICONTROL Update account]**&#x200B;을(를) 선택하여 자격 증명 업데이트를 완료합니다.
...
>[!NOTE]
>
>만료되지 않는 자격 증명을 사용하여 호스트에 연결할 때 암호와 사용자 이름을 제외하고 [!UICONTROL EXPIRING CREDENTIALS] 섹션에 나열된 모든 매개 변수를 사용해야 합니다.
>사용자 이름과 암호를 입력하는 형식은 이 예제 `username:{your_username}` 및 `password:{password_string}`과(와) 같이 콜론으로 구분된 값을 사용합니다.

| 매개변수 | 설명 | 예 |
|---|---|---|
| **서버/호스트** | 접속 중인 서버/호스트의 이름입니다. <ul><li>이 값은 만료 중인 자격 증명과 만료되지 않는 자격 증명 모두에 사용되며 `server.adobe.io` 형식을 사용합니다. **[!UICONTROL Host]** 섹션의 [!UICONTROL EXPIRING CREDENTIALS]에서 값을 찾았습니다.</ul></li> | `acme.platform.adobe.io` |
| **포트** | 연결 중인 서버/호스트의 포트입니다. <ul><li>이 값은 만료되는 자격 증명과 만료되지 않는 자격 증명 모두에 사용되며 **[!UICONTROL Port]** 섹션의 [!UICONTROL EXPIRING CREDENTIALS]에서 찾을 수 있습니다.</ul></li> | `80` |
| **데이터베이스** | 연결 중인 데이터베이스입니다. <ul><li>이 값은 만료 중인 자격 증명과 만료되지 않는 자격 증명 모두에 사용되며 **[!UICONTROL Database]** 섹션의 [!UICONTROL EXPIRING CREDENTIALS]에서 찾을 수 있습니다. </ul></li> | `prod:all` |
| **사용자 이름** | 외부 클라이언트에 연결하는 사용자의 사용자 이름입니다. <ul><li>이 값은 만료되는 자격 증명과 만료되지 않는 자격 증명 모두에 사용됩니다. `@AdobeOrg` 앞에 영숫자 문자열 형식을 사용합니다. 이 값은 **[!UICONTROL Username]**&#x200B;에 있습니다.</li></ul> | `ECBB80245ECFC73E8A095EC9@AdobeOrg` |
| **암호** | 외부 클라이언트에 연결하는 사용자의 암호입니다. <ul><li>만료되는 자격 증명을 사용하는 경우 **[!UICONTROL Password]** 섹션 내의 [!UICONTROL EXPIRING CREDENTIALS]에서 찾을 수 있습니다.</li><li>만료되지 않는 자격 증명을 사용하는 경우 이 값은 technicalAccountID의 연결된 인수와 구성 JSON 파일에서 가져온 자격 증명입니다. 암호 값은 `{technicalAccountId}:{credential}` 형식을 사용합니다.</li></ul> | <ul><li>만료될 자격 증명 암호는 1,000자 이상의 영숫자 문자열입니다. 예시는 제공되지 않을 것입니다.</li><li>만료되지 않는 자격 증명 암호는 다음과 같습니다. <br>`4F2611B8613DK3670V495N55:3d182fa9e0b54f33a7881305c06203ee`</li></ul> |
