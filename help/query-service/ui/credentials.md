...
1. [Adobe Admin Console](https://adminconsole.adobe.com/)에 로그인하고 위쪽 탐색 모음에서 관련 조직을 선택합니다.
2. [제품 프로필을 선택합니다.](../../access-control/ui/browse.md)
3. [제품 프로필에  **** 대한 Sandboxes 및  **Manage Query Service** ](../../access-control/ui/permissions.md) IntegrationPermissions를 모두 구성합니다.
4. [구성된 권한이 부여되도록 새 ](../../access-control/ui/users.md) 사용자를 제품 프로필에 추가합니다.
5. [사용자를 제품 프로필 관리자](https://helpx.adobe.com/enterprise/using/manage-product-profiles.html) 로 추가하여 활성 제품 프로필에 대해 계정을 만들 수 있습니다.
6. [통합을 만들려면 사용자를 제품 프로필 ](https://helpx.adobe.com/kr/enterprise/using/manage-developers.html) 개발자로 추가합니다.
...
만료되지 않은 자격 증명 세트를 만들려면 Platform UI로 돌아가서 왼쪽 탐색에서 **[!UICONTROL 쿼리]**&#x200B;를 선택하여 [!UICONTROL 쿼리] 작업 영역에 액세스합니다. 다음으로 **[!UICONTROL 자격 증명]** 탭 다음에 **[!UICONTROL 자격 증명 생성]**&#x200B;을 선택합니다.
...
- ****&#x200B;에 할당됨: 자격 증명을 할당할 사용자입니다. 이 값은 자격 증명을 만드는 사용자의 이메일 주소여야 합니다.
...
>[!IMPORTANT]
>
>**[!UICONTROL 자격 증명 생성]** 단추를 선택하면 구성 JSON 파일이 로컬 시스템에 다운로드됩니다. Adobe은 생성된 자격 증명을 기록하지 **하지 않으므로 다운로드한 파일을 안전하게 저장하고 자격 증명 레코드를 유지해야 합니다.**
>
...
생성된 자격 증명을 저장한 후 **[!UICONTROL 닫기]**&#x200B;를 선택합니다. 이제 만료되지 않은 자격 증명 목록을 볼 수 있습니다.
...
이 이미지는 만료되지 않은 자격 증명의 암호를 제외하고 UI에 있는 각 매개 변수의 위치를 나타냅니다. 만료되지 않은 자격 증명은 JSON 구성 파일에서 제공되지만, UI의 **자격 증명** 탭에서 만료되는 자격 증명을 볼 수 있습니다.
...
>[!NOTE]
>
>만료되지 않은 자격 증명을 사용하여 호스트에 연결할 때 암호 및 사용자 이름을 제외하고 [!UICONTROL EXPIRING CREDENTIALS] 섹션에 나열된 모든 매개 변수를 사용해야 합니다.

| 매개 변수 | 설명 |
|---|---|
| 서버/호스트 | 연결할 서버/호스트의 이름입니다. <ul><li>이 값은 만료 자격 증명과 만료 전 자격 증명 모두에 사용되며 `server.adobe.io` 형식을 갖습니다. 이 값은 [!UICONTROL 만료 자격 증명] 섹션의 **[!UICONTROL 호스트]** 아래에 있습니다.</ul></li> |
| 포트 | 연결할 서버/호스트의 포트입니다. <ul><li>이 값은 만료 자격 증명과 만료 전 자격 증명 모두에 사용되며 [!UICONTROL 만료 자격 증명] 섹션의 **[!UICONTROL 포트]**&#x200B;에서 찾을 수 있습니다. 포트의 예제 값은 `80`입니다.</ul></li> |
| 데이터베이스 | 연결할 데이터베이스입니다. <ul><li>이 값은 만료 자격 증명과 만료 전 자격 증명 모두에 사용되며 [!UICONTROL 만료 자격 증명] 섹션의 **[!UICONTROL 데이터베이스]**&#x200B;에 있습니다. 데이터베이스의 예제 값은 `prod:all`입니다.</ul></li> |
| 사용자 이름 | 외부 클라이언트에 연결하는 사용자의 사용자 이름입니다. <ul><li>만료 자격 증명을 사용하는 경우 `@AdobeOrg` 앞에 영숫자 문자열을 사용합니다. 이 값은 **[!UICONTROL 사용자 이름]**&#x200B;에 있습니다.</li><li>만료되지 않은 자격 증명을 사용 중인 경우, **a1/>은(는) 구성 JSON 파일에 있는 `technicalAccountID` 값과 동일할 수 없지만 선택하는 문자열입니다.**</li></ul> |
| 암호 | 외부 클라이언트에 연결하는 사용자의 암호입니다. <ul><li>만료 자격 증명을 사용하는 경우 [!UICONTROL 만료 자격 증명] 섹션 내에서 **[!UICONTROL 암호]**&#x200B;에서 찾을 수 있습니다.</li><li>만료되지 않은 자격 증명을 사용하는 경우 이 값은 technicalAccountID에서 연결된 인수와 구성 JSON 파일에서 가져온 자격 증명입니다. 암호 값은 다음 형식을 사용합니다. `{technicalAccountId}:{credential}`</li></ul> |
