...
>[!NOTE]
>
>경고 구독 아이콘(![경고 구독 아이콘.](../images/ui/monitor-queries/alert-subscription-icon.png))은(는) 제목 없는 열의 각 행에 포함되어 있습니다. 자세한 내용은 [경고 구독](#alert-subscription) 섹션을 참조하십시오.
...
필요에 따라 [!UICONTROL 예약된 쿼리] 탭의 열을 조정할 수 있습니다. [!UICONTROL 표 사용자 지정] 설정 대화 상자를 열고 사용 가능한 열을 편집하려면 설정 아이콘(![설정 아이콘)을 선택합니다.](../images/ui/monitor-queries/settings-icon.png))을(를) 클릭합니다.
