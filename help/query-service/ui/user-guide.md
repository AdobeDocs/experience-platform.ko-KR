...
다음에서 실행된 쿼리 [!DNL Query Editor] 대화식으로 실행. 즉, 브라우저를 닫거나 다른 곳으로 이동하면 쿼리가 취소됩니다. 이는 쿼리 출력에서 데이터 세트를 생성하기 위해 만들어진 쿼리에도 적용됩니다.
...
다음에서 [!DNL Experience Platform] UI, 선택 **[!UICONTROL 쿼리]** 을(를) 클릭하여 왼쪽 탐색 메뉴에서 [!DNL Query Service] 작업 영역. 그런 다음 을 선택합니다. **[!UICONTROL 쿼리 만들기]** 화면 오른쪽 상단에서 쿼리 작성을 시작합니다. 이 링크는 의 페이지에서 사용할 수 있습니다. [!DNL Query Service] 작업 영역.
...
토글을 사용하여 자동 완성 기능을 활성화하면 잠시 후에 테이블 및 열 이름과 SQL 키워드에 대한 권장 제안을 사용할 수 있습니다. 쿼리 편집기 아래에 있는 콘솔의 성공 메시지는 기능이 활성 상태임을 나타냅니다.
...
>[!IMPORTANT]
>
>이 기능을 사용하지 않도록 설정할 때 쿼리를 작성하거나 편집하는 경우 페이지를 새로 고치기 전에 쿼리에 대한 변경 사항을 저장해야 합니다. 그렇지 않으면 모든 진행률이 손실됩니다.

![자동 완성 기능을 비활성화하는 확인 대화 상자](../images/ui/query-editor/confirmation-dialog.png)

자동 완성 기능을 비활성화하려면 적절한 옵션을 선택합니다.
...
에서 저장된 템플릿 선택 [!UICONTROL 템플릿] 탭을 사용하여 쿼리 편집기에서 볼 수 있습니다. 쿼리 세부 정보 패널에서는 선택한 쿼리를 관리하기 위한 자세한 정보와 도구를 제공합니다.
...
템플릿으로 저장된 쿼리는 쿼리 편집기에서 예약할 수 있습니다. 이렇게 하면 사용자 지정 케이던스에서 실행되는 쿼리 실행을 자동화할 수 있습니다. 빈도, 날짜 및 시간에 따라 쿼리를 예약할 수 있으며, 필요한 경우 결과에 대한 출력 데이터 세트를 선택할 수도 있습니다. UI를 통해 쿼리 일정을 비활성화하거나 삭제할 수도 있습니다.

예약은 쿼리 편집기에서 설정됩니다. 다음은 쿼리 편집기를 사용할 때 예약된 쿼리에 대한 제한 사항 목록입니다. 이 변수는 다음에 적용되지 않습니다 [!DNL Query Service] API:

- 이미 생성, 저장 및 실행된 쿼리에만 일정을 추가할 수 있습니다.
- 본인 **할 수 없음** 매개 변수가 있는 쿼리에 일정을 추가합니다.
- 예약된 쿼리 **할 수 없음** 익명 블록을 포함합니다.

방법을 알아보려면 쿼리 일정 설명서 를 참조하십시오 [ui에서 쿼리 일정 만들기](./query-schedules.md). 또는 API를 사용하여 일정을 추가하는 방법을 알아보려면 다음을 참조하십시오. [예약된 쿼리 끝점 안내서](../api/scheduled-queries.md).
...
다음 [!DNL Query Editor] 는 쿼리를 저장하고 나중에 작업할 수 있도록 해주는 저장 기능을 제공합니다. 쿼리를 저장하려면 을 선택합니다. **[!UICONTROL 저장]** 의 오른쪽 위 모서리에서 [!DNL Query Editor]. 쿼리를 저장하려면 먼저 **[!UICONTROL 쿼리 세부 정보]** 패널.
...
>[!NOTE]
>
>콘솔에는 쿼리 실행으로 인해 발생한 오류만 표시됩니다. 쿼리가 실행되기 전의 쿼리 유효성 검사 오류는 표시되지 않습니다.
...
다음 비디오는 Adobe Experience Platform 인터페이스 및 PSQL 클라이언트에서 쿼리를 실행하는 방법을 보여 줍니다. 또한 XDM 개체에서 개별 속성 사용, Adobe 정의 함수 사용 및 CREATE TABLE AS SELECT(CTAS) 사용이 표시됩니다.
