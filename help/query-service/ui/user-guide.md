...
템플릿으로 저장된 쿼리는 쿼리 편집기에서 예약할 수 있습니다. 이를 통해 사용자 지정 케이던스에서 쿼리 실행을 자동화할 수 있습니다. 빈도, 날짜 및 시간을 기반으로 쿼리를 예약하고, 필요한 경우 결과에 대한 출력 데이터 세트를 선택할 수도 있습니다. UI를 통해 쿼리 일정을 비활성화하거나 삭제할 수도 있습니다.

예약은 쿼리 편집기에서 설정됩니다. 다음은 쿼리 편집기를 사용할 때 예약된 쿼리에 대한 제한 사항 목록입니다. 에는 적용되지 않습니다 [!DNL Query Service] API:

- 이미 생성, 저장 및 실행된 쿼리에 예약만 추가할 수 있습니다.
- 사용자 **사용할 수 없음** 매개 변수가 있는 쿼리에 일정을 추가합니다.
- 예약된 쿼리 **사용할 수 없음** 익명 블록을 포함합니다.

방법을 알려면 쿼리 예약 설명서 를 참조하십시오. [UI에서 쿼리 예약 만들기](./query-schedules.md). 또는 API를 사용하여 일정을 추가하는 방법을 알려면 다음을 참조하십시오. [예약된 쿼리 엔드포인트 가이드](../api/scheduled-queries.md).
