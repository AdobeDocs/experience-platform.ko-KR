...
아직 수행하지 않았다면 다음 단계를 수행합니다. [연결 [!DNL Jupyter Notebook] Adobe Experience Platform 쿼리 서비스 사용](../clients/jupyter-notebook.md) 이 안내서를 계속하기 전에.
...
성향 점수 모델을 생성하려면 Platform에 저장된 분석 데이터의 투영을 로 가져와야 합니다 [!DNL Jupyter Notebook]. 다음에서: [!DNL Python] 3 [!DNL Jupyter Notebook] 쿼리 서비스에 연결된 다음 명령은 가상 의류 매장인 Luma에서 고객 행동 데이터 세트를 가져옵니다. Platform 데이터가 XDM(Experience Data Model) 형식을 사용하여 저장되므로 스키마의 구조를 준수하는 샘플 JSON 개체를 생성해야 합니다. 방법에 대한 지침은 설명서 를 참조하십시오 [샘플 JSON 개체 생성](../../xdm/ui/sample.md).

![다음 [!DNL Jupyter Notebook] 여러 명령이 강조 표시된 대시보드.](../images/use-cases/jupyter-commands.png)

출력은 Luma 내의 동작 데이터 세트에서 모든 열의 테이블화된 보기를 표시합니다. [!DNL Jupyter Notebook] 대시보드입니다.

![내에서 가져온 Luma의 고객 행동 데이터 세트에 대한 표로 구분된 출력입니다. [!DNL Jupyter Notebook].](../images/use-cases/behavioural-dataset-results.png)

## 머신 러닝을 위한 데이터 준비 {#prepare-data-for-machine-learning}

머신 러닝 모델을 교육하려면 대상 열을 식별해야 합니다. 구매 성향이 이 사용 사례의 목표이므로, `analytic_action` 열은 Luma 결과에서 대상 열로 선택됩니다. 값 `productPurchase` 는 고객 구매를 나타내는 표시기입니다. 다음 `purchase_value` 및 `purchase_num` 열은 제품 구매 작업과 직접 관련이 있으므로 제거됩니다.
...
다음과 같은 기법 *1개의 핫 인코딩* 는 기계 및 딥 러닝 알고리즘과 함께 사용하기 위해 범주형 데이터 변수를 변환하는 데 사용됩니다. 이는 결과적으로 모델의 분류 정확도뿐만 아니라 예측도 향상시킵니다. 사용 `Sklearn` 라이브러리로 개별 열에 각 범주 값을 표시할 수 있습니다.
...
다음으로 정의된 데이터: `X` 은 표로 작성되며 다음과 같이 표시됩니다.

![다음 내에 있는 X의 표로 정리된 출력 [!DNL Jupyter Notebook].](../images/use-cases/x-output-table.png)


이제 머신 러닝에 필요한 데이터를 사용할 수 있으므로 사전 구성된 머신 러닝 모델에 맞출 수 있습니다. [!DNL Python]의 `sklearn` 라이브러리입니다. [!DNL Logistics Regression] 성향 모델을 교육하는 데 사용되며 테스트 데이터의 정확성을 확인할 수 있도록 해줍니다. 이 경우 약 85%입니다.

다음 [!DNL Logistic Regression] 머신 러닝 알고리즘의 성능을 추정하는 데 사용되는 알고리즘과 train-test split 방법은 아래 코드 블록으로 가져옵니다.
...
![구매 성향을 정의하는 상위 10개 기능의 시각화입니다.](../images/use-cases/visualized-results.png)
...
- 먼저 SQL에서 [!DNL Logistics Regression] 예측 레이블의 확률을 얻기 위한 모듈입니다. Logistics Regression에 의해 생성된 모형은 회귀 모형을 생성하였다 `y = wX + c`  가중치 `w` and intercept `c` 는 모델의 출력입니다. SQL 특징은 확률을 얻기 위해 가중치를 곱하는 데 사용될 수 있다.

- 둘째, 엔지니어링 프로세스는 [!DNL Python] 핫 인코딩을 사용하여 SQL에도 통합해야 합니다. 예를 들어 원본 데이터베이스에는 `geo_county` 군을 저장하는 열이지만 열은 로 변환됩니다. `geo_county=Bexar`, `geo_county=Dallas`, `geo_county=DeKalb`. 다음 SQL 문은 동일한 변환을 수행합니다. 여기서 `w1`, `w2`, 및 `w3` 의 모델에서 학습한 가중치로 대체할 수 있습니다. [!DNL Python]:
...
숫자가 얻어지면 Logistics Regression 알고리즘이 최종 예측을 생성하는 시그모이드 함수로 포팅될 수 있습니다. 아래 문장에서 `intercept` 는 회귀의 절편 수입니다.
        
...
열이 두 개인 경우(`c1` 및 `c2`), `c1` 에는 두 가지 범주가 있습니다. [!DNL Logistic Regression] 알고리즘은 다음 함수로 학습됩니다.
 
...
테이블화된 결과는 의 각 고객 세션에 대한 구매 성향을 표시합니다. `0` 즉, 구매 성향이 없으며 `1` 이는 구매 성향이 확인된 것을 의미합니다.

![SQL을 사용한 데이터베이스 추론의 표로 정리된 결과입니다.](../images/use-cases/inference-results.png)
...
위 그림은 모델을 교육하기 위해 1000개의 행만 사용하는 경우 정확도가 약 84%와 88% 사이에 떨어질 것으로 예상할 수 있음을 나타냅니다. 다음을 조정할 수 있습니다. `LIMIT` 모델의 성능을 보장하기 위해 필요에 따라 쿼리 서비스 의 조항입니다.
