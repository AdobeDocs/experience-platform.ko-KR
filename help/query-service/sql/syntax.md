...
다음 구문은 `SELECT`에서 지원하는 [!DNL Query Service] 쿼리를 정의합니다.
...
>[!NOTE]
>
>`HEAD` 절에 `TAIL` 또는 `SNAPSHOT`을(를) 사용하는 경우 작은 따옴표(예: &#39;HEAD&#39;, &#39;TAIL&#39;)로 묶어야 합니다. 따옴표 없이 이 기호를 사용하면 구문 오류가 발생합니다.
...
| `BETWEEN end_snapshot_id AND TAIL` | 지정된 `end_snapshot_id` 바로 뒤에서 데이터 집합 끝(스냅숏 ID 제외)까지 데이터를 읽습니다. 즉, `end_snapshot_id`이(가) 데이터 집합의 마지막 스냅숏인 경우 마지막 스냅숏 이후에 스냅숏이 없으므로 쿼리에서 0개의 행을 반환합니다. |
| `SINCE start_snapshot_id INNER JOIN table_to_be_joined AS OF your_chosen_snapshot_id ON table_to_be_queried.id = table_to_be_joined.id` | `table_to_be_queried`에서 지정한 스냅숏 ID로 시작하는 데이터를 읽고 `table_to_be_joined`에서와 같이 `your_chosen_snapshot_id`의 데이터와 결합합니다. 조인은 조인되는 두 테이블의 ID 열에서 일치하는 ID를 기반으로 합니다. |
...
>
>- 선택적 대체 동작 플래그를 설정하면 쿼리 서비스가 사용 가능한 가장 이른 스냅숏을 선택하고 시작 스냅숏으로 설정하며 사용 가능한 가장 이른 스냅숏과 지정된 끝 스냅숏 사이에 데이터를 반환합니다. 이 데이터는 사용 가능한 가장 빠른 스냅숏의 **포함**&#x200B;입니다.

### WHERE 절

기본적으로 `WHERE` 쿼리에서 `SELECT` 절에 의해 생성된 일치 항목은 대/소문자를 구분합니다. 일치하는 항목을 대/소문자를 구분하지 않게 하려면 `ILIKE` 대신 `LIKE` 키워드를 사용할 수 있습니다.
...
변형된 기능을 사용하여 모델을 교육할 준비가 되었으면 [모델 설명서](../advanced-statistics/models.md)에서 `CREATE MODEL` 절을 사용한 `TRANSFORM` 사용에 대한 지침을 참조하십시오.
...
`TRANSFORM` 또는 `CREATE TABLE`에 `CREATE TEMP TABLE` 절을 사용할 때는 다음 제한 사항을 염두에 두십시오.
...
- 변형은 테이블 생성 중에만 적용됩니다. `INSERT INTO`이(가) 있는 테이블에 삽입된 새 데이터는 **자동으로 변환되지 않습니다**. 새 데이터에 변환을 적용하려면 `CREATE TABLE AS SELECT` 절을 사용하여 `TRANSFORM`을(를) 사용하여 테이블을 다시 만들어야 합니다.
...
다음 코드 행은 메타데이터에서 사용 가능한 가장 빠른 `@from_snapshot_id`을(를) 사용하여 `snapshot_id`을(를) 재정의합니다.
...
`table_exists` SQL 명령을 사용하여 현재 시스템에 테이블이 있는지 확인합니다. 이 명령은 테이블 `true`does **이(가) 있는 경우**, 테이블이 `false`does **이(가) 없는 경우** 부울 값을 반환합니다.
...
모든 설정에 대한 값을 반환하려면 `SET [property key]` 없이 `property_value`을(를) 사용합니다.
...
다음은 `ANALYZE TABLE` 명령을 사용한 후에 사용할 수 있는 통계 계산 목록입니다:-
...
이제 [!DNL Azure Data Lake Storage] SQL 명령을 사용하여 `COMPUTE STATISTICS`(ADLS) 데이터 세트에 대한 열 수준 통계를 계산할 수 있습니다. 전체 데이터 세트, 데이터 세트의 하위 집합, 모든 열 또는 열의 하위 집합에 대한 열 통계를 계산합니다.
...
응답 형식을 정의하려면 `FORMAT` 명령과 함께 `EXPLAIN` 키워드를 사용합니다.
...
다음 쿼리는 `films_recent` 테이블의 최근 항목으로만 구성된 새 테이블 `films`을(를) 만듭니다.
