...
+++답변
아니요. 데이터 크기에는 제한이 없지만 대화형 세션에서 10분의 쿼리 시간 제한이 있습니다. 쿼리가 배치 CTAS로 실행되는 경우 10분 시간 초과는 적용되지 않습니다. 자세한 내용은 [대화형 쿼리 실행](./best-practices/writing-queries.md#interactive-query-execution)에 대한 지침을 참조하십시오.
+++
...
- Experience Platform에 로그인한 후 UI의 왼쪽 탐색에서 **[!UICONTROL Datasets]**&#x200B;을(를) 선택하여 [!UICONTROL Datasets] 대시보드로 이동합니다.
- 데이터 세트 [!UICONTROL Browse] 탭이 열립니다. 검색 창을 사용하여 사용 가능한 옵션을 구체화할 수 있습니다. 표시된 목록에서 데이터 세트를 선택합니다.

![검색 창과 데이터 세트가 강조 표시된 Experience Platform UI의 데이터 세트 대시보드.](./images/troubleshooting/dataset-selection.png)

- [!UICONTROL Datasets activity] 화면이 나타납니다. **[!UICONTROL Preview dataset]**&#x200B;을(를) 선택하여 XDM 스키마의 대화 상자와 선택한 데이터 집합에서 병합된 데이터의 테이블 보기를 엽니다. 자세한 내용은 [데이터 집합 문서 미리 보기](../catalog/datasets/user-guide.md#preview-a-dataset)에서 확인할 수 있습니다.
...
- 위쪽 탐색 모음 오른쪽에서 **[!UICONTROL Sign In]**&#x200B;을(를) 선택합니다.

![로그인이 강조 표시된 Adobe 웹 사이트입니다.](./images/troubleshooting/adobe-sign-in.png)

- Adobe 라이선스에 등록된 Adobe ID 및 암호를 사용합니다.
- 위쪽 탐색 모음에서 **[!UICONTROL Help & Support]**&#x200B;을(를) 선택합니다.

![도움말 및 지원, 엔터프라이즈 지원 및 문의하기 등이 강조 표시된 상단 탐색 모음 드롭다운 메뉴.](./images/troubleshooting/help-and-support.png)

[!UICONTROL Help and support] 섹션이 포함된 드롭다운 배너가 나타납니다. **[!UICONTROL Contact us]**&#x200B;을(를) 선택하여 Adobe Customer Care Virtual Assistant를 열거나 **[!UICONTROL Enterprise support]**&#x200B;을(를) 선택하여 대규모 조직을 위한 전용 도움말을 제공합니다.
+++
...
## TLS, 포트 액세스 및 암호화 {#tls-port-questions}

### 포트 80에서 수행된 연결이 여전히 HTTPS 및 TLS 암호화를 사용합니까?

+++답변
예. 포트 80의 연결은 TLS 암호화를 사용하여 보호되며, TLS 적용은 서비스에 필요합니다. 일반 HTTP 연결은 허용되지 않습니다. 포트 80 지원은 특정 고객 네트워크 정책을 수용하기 위해 존재합니다. 조직에서 포트 80을 차단하는 경우 포트 5432를 대신 사용하십시오. 두 포트 모두 TLS가 필요하며 동일한 보안 상태를 제공합니다.
+++

### Adobe의 쿼리 서비스가 암호화되지 않은 HTTP(포트 80)를 통해 데이터를 노출합니까?

+++답변
아니요. 포트 80의 연결은 TLS가 필요하며 모든 일반 텍스트 HTTP 요청은 서버측에서 거부됩니다. 포트 5432도 지원되며 TLS로 암호화됩니다.
+++

### 쿼리 서비스 및 Data Distiller에 포트 80을 사용하는 것이 기존 구성입니까?

+++답변
아니요. 필수 TLS가 있는 포트 80은 특정 네트워크 요구 사항이 있는 고객을 위해 설계된 지원되는 구성입니다. 레거시 또는 비보안 모드가 아닙니다. 사용자 환경에서 포트 80의 아웃바운드 연결을 제한하는 경우 포트 5432를 대신 사용하십시오. 두 포트 모두 TLS를 적용합니다.
+++

### Power BI 클라이언트에서 쿼리 서비스로의 모든 연결에 TLS 1.2를 사용합니까?

+++답변
예. 전송 중인 데이터는 항상 HTTPS를 사용하여 보호되며 현재 지원되는 버전은 TLS 1.2입니다. 쿼리 서비스에 대한 모든 Power BI 연결에는 암호화된 전송이 필요합니다.
+++

### Data Distiller과 함께 사용할 때 포트 80이 암호화되지 않습니까?

+++답변
아니요. 데이터 Distiller은 포트 80에서 TLS를 적용하고 일반 텍스트 HTTP 요청을 거부합니다. 포트 5432도 지원되며 TLS로 암호화됩니다.
+++

### Query Service 또는 Data Distiller에서 포트 80을 사용할 때 위험이나 제한이 있습니까?

+++답변
예. TLS는 포트 80에서 적용되며 암호화되지 않은 연결은 지원되지 않습니다. 일부 조직은 정책 제한으로 인해 포트 80의 아웃바운드 트래픽을 차단합니다. 이것이 네트워크에 적용되는 경우 포트 5432를 대신 사용하십시오. 모든 경우에 TLS가 필요하므로 두 포트는 동일한 수준의 보안을 제공합니다.
+++
...
+++답변
아니요. 시스템 데이터 세트에 대한 쓰기 권한이 제한되어 있으므로 샘플을 만들 수 없습니다.
+++
...
+++답변
`history_meta()` 메서드는 데이터 집합에서 스냅숏에 액세스하는 데 사용됩니다. 이전에는 Azure ADLS(데이터 레이크 저장소)의 빈 데이터 세트에 대해 쿼리를 실행하려고 하면 데이터 세트가 존재하지 않는다는 58000 오류 코드가 표시되었습니다. 이전 시스템 오류의 예가 아래에 표시됩니다.
