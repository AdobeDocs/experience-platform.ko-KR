...
**[!DNL PostgreSQL]** 목록에서 [!DNL Connections]을(를) 선택하면 새 [!DNL Object View] 연결에 대한 [!DNL PostgreSQL] 탭이 나타납니다.
...
>[!IMPORTANT]
>
>DBVisualizer를 Adobe Experience Platform과 연결하려면 SSL을 사용하도록 설정해야 합니다. Adobe Experience Platform 쿼리 서비스에 대한 서드파티 연결에 대한 SSL 지원 및 [&#x200B; SSL 모드를 사용하여 연결하는 방법에 대한 자세한 내용은 &#x200B;](./ssl-modes.md)SSL 모드 설명서`verify-full`를 참조하세요.

| 속성 | 설명 |
| ------ | ------ |
| `PGHOST` | [!DNL PostgreSQL] 서버의 호스트 이름입니다. 이 값은 Experience Platform **[!UICONTROL Host]자격 증명**&#x200B;입니다. |
...
| `user` | 데이터베이스에 연결된 사용자 이름은 조직 ID입니다. `@Adobe.Org`(으)로 끝나는 영숫자 문자열입니다. 이 값은 Experience Platform **[!UICONTROL Username]자격 증명**&#x200B;입니다. |
...
BBVisualizer를 쿼리 서비스에 연결하는 데 필요한 자격 증명을 찾으려면 Experience Platform UI에 로그인하고 왼쪽 탐색에서 **[!UICONTROL Queries]**&#x200B;을(를) 선택한 후 **[!UICONTROL Credentials]**&#x200B;을(를) 선택합니다. **호스트**, **포트**, **데이터베이스**, **사용자 이름** 및 **암호** 자격 증명을 찾는 방법에 대한 자세한 내용은 [자격 증명 안내서](../ui/credentials.md)를 참조하십시오.
...
연결을 설정할 때마다 사용자 ID 및 암호 기반 인증을 요구하려면 [!DNL Properties] 탭으로 이동하여 **[!DNL Authentication]** 아래의 탐색 사이드바에서 [!DNL PostgreSQL]을(를) 선택하십시오.
...
만료되거나 만료되지 않는 자격 증명을 사용하여 연결할 수 있습니다. 연결하려면 **[!DNL Connection]** 개체 보기 탭에서 [!DNL PostgreSQL] 탭을 선택하고 다음 설정에 대한 Experience Platform 자격 증명을 입력하십시오. [수동 연결 설정](https://confluence.dbvis.com/display/UG100/Setting+Up+a+Connection+Manually)에 대한 보완 지침은 공식 DBVisualizer 웹 사이트에서 사용할 수 있습니다.
...
| 연결 매개 변수 | 설명 |
|---|---|
| **[!UICONTROL Name]** | 연결의 이름을 만듭니다. 연결을 인식하기 위해 친숙한 이름을 제공하는 것이 좋습니다. |
| **[!UICONTROL Database Server]** | Experience Platform **[!UICONTROL Host]** 자격 증명입니다. |
| **[!UICONTROL Database Port]** | [!DNL Query Service]에 대한 포트입니다. **에 연결하려면** 80 **또는** 5432[!DNL Query Service] 포트를 사용해야 합니다. |
| **[!UICONTROL Database]** | Experience Platform **[!UICONTROL Database]** 자격 증명 값 사용: `prod:all`. |
| **[!UICONTROL Database Userid]** | Experience Platform 조직 ID입니다. Experience Platform **[!UICONTROL Username]** 자격 증명 값을 사용하십시오. ID는 `ORG_ID@AdobeOrg` 형식입니다. |
| **[!UICONTROL Database Password]** | 이 영숫자 문자열은 Experience Platform **[!UICONTROL Password]** 자격 증명입니다. 만료되지 않는 자격 증명을 사용하려면 이 값은 구성 JSON 파일에서 다운로드한 `technicalAccountID` 및 `credential`에서 연결된 인수입니다. 암호 값은 {technicalAccountId}:{credential} 형식을 사용합니다. 만료되지 않는 자격 증명에 대한 구성 JSON 파일은 초기화 중에 Adobe이 복사본을 보관하지 않는 한 번 다운로드됩니다. |
