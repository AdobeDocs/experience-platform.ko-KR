...
다음은 예약된 쿼리를 나열하기 위한 사용 가능한 쿼리 매개 변수 목록입니다. 이러한 매개 변수는 모두 선택 사항입니다. 매개 변수가 없는 이 끝점을 호출하면 조직에서 사용할 수 있는 모든 예약된 쿼리를 검색합니다.
...
| `property` | 필드를 기반으로 결과를 필터링합니다. 필터는 HTML을 **escape해야 합니다** . 쉼표는 여러 필터 집합을 결합하는 데 사용됩니다. 지원되는 필드는 `created`, `templateId`및 `userId`입니다. 지원되는 연산자 목록은 `>` (보다 큼), `<` (보다 작음) 및 `==` (같음)입니다. 예를 들어 사용자 ID가 지정된 모든 예약된 쿼리를 `userId==6ebd9c2d-494d-425a-aa91-24033f3abeec` 반환합니다. |
...
성공적인 응답은 지정된 IMS 조직에 대해 예약된 쿼리 목록이 있는 HTTP 상태 200을 반환합니다. 다음 응답은 IMS 조직에 대해 생성된 최신 예약 쿼리를 반환합니다.
...
| `schedule.schedule` | 쿼리의 크론 예약입니다. cron 일정에 대한 자세한 내용은 cron [expression format](http://www.quartz-scheduler.org/documentation/quartz-2.3.0/tutorials/crontrigger.html) 설명서를 참조하십시오. 이 예에서 &quot;30 * * * * * *&quot;는 30분 표시에서 매시간마다 쿼리가 실행됨을 의미합니다. |
| `schedule.startDate` | UTC 타임스탬프로 기록된 예약된 쿼리의 시작 날짜입니다. |

**응답**

성공적인 응답은 새로 만든 예약된 쿼리의 세부 정보와 함께 HTTP 상태 202(허용됨)를 반환합니다. 예약된 쿼리 활성화가 완료되면 `state` 는 에서 (으)로 `REGISTERING` 변경됩니다 `ENABLED`.
...
>[!NOTE]
>
>의 값을 사용하여 만든 예약된 쿼리 `_links.delete` 를 [삭제할 수 있습니다](#delete-a-specified-scheduled-query).
...
선택한 예약 쿼리 `/state` 의 상태(ENABLED 또는 DISABLED)를 업데이트하는 데 사용할 수 있습니다. 상태를 업데이트하려면 값을 `enable` 또는 로 설정해야 합니다 `disable`.
...
| 속성 | 설명 |
| -------- | ----------- |
| `path` | 패치할 값의 경로입니다. 이 경우 예약된 쿼리의 상태를 업데이트하기 때문에 값을 `path` 로 설정해야 합니다 `/state`. |
| `value` | 의 업데이트된 값 `/state`. 이 값은 예약된 쿼리를 활성화하거나 비활성화하기 `enable` 로 `disable` 설정할 수 있습니다. |
...
을 사용하여 예약된 쿼리 `/schedule/schedule` 의 크론 일정을 업데이트할 수 있습니다. cron 일정에 대한 자세한 내용은 cron [expression format](http://www.quartz-scheduler.org/documentation/quartz-2.3.0/tutorials/crontrigger.html) 설명서를 참조하십시오.
...
| 속성 | 설명 |
| -------- | ----------- |
| `path` | 패치할 값의 경로입니다. 이 경우 예약된 쿼리의 예약을 업데이트하므로 값을 `path` 다음으로 설정해야 합니다 `/schedule/schedule`. |
| `value` | 의 업데이트된 값 `/schedule`. 이 값은 cron 일정 형식이어야 합니다. 따라서 이 예에서는 예약된 쿼리가 45분 표시에서 매 시간마다 실행됩니다. |
...
종단점에 DELETE 요청을 만들고 요청 경로에서 삭제하려는 예약된 쿼리의 ID를 제공하여 지정된 예약 쿼리를 삭제할 수 있습니다. `/schedules`
