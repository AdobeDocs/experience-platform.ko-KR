...
#### 가속 데이터 세트에 대해 필드 기반 액세스 제한 만들기 {#create-field-based-access-restrictions-on-accelerated-datasets}

사용 [속성 기반 액세스 제어 기능](../../access-control/abac/overview.md) 의 팩트 및 차원 데이터 세트에 대해 조직 또는 데이터 사용 범위를 정의할 수 있습니다 [가속 저장](../data-distiller/query-accelerated-store/send-accelerated-queries.md). 이를 통해 관리자는 특정 세그먼트에 대한 액세스를 관리하고 사용자 또는 사용자 그룹에 부여된 액세스를 더 잘 관리할 수 있습니다.

가속화된 데이터 세트에 대한 필드 기반 액세스 제한을 만들려면 Query Service CTAS 쿼리를 사용하여 가속화된 데이터 세트를 만들고 기존 XDM 스키마나 애드혹 스키마를 기반으로 이러한 데이터 세트를 구조화할 수 있습니다. 그런 다음 관리자가 [스키마에 대한 데이터 사용 레이블 추가 및 편집](../../xdm/tutorials/labels.md#edit-the-labels-for-the-schema-or-field) 또는 [애드혹 스키마](./ad-hoc-schema-labels.md#edit-governance-labels). 스키마에서 레이블을 적용, 생성 및 편집할 수 있습니다 [!UICONTROL 레이블] 작업 영역 [!UICONTROL 스키마] UI.

데이터 사용 레이블은 [데이터 세트에 직접 적용되거나 편집된 경우](../../data-governance/labels/user-guide.md#add-labels) 데이터 세트 UI 사용 또는 액세스 제어에서 생성 [!UICONTROL 레이블] 작업 공간. 방법에 대한 안내서를 참조하십시오. [새 레이블 만들기](../../access-control/abac/ui/labels.md) 추가 정보.

그런 다음 개별 열에 대한 사용자 액세스를 첨부된 데이터 사용 레이블 및 사용자에게 할당된 역할에 적용된 권한 집합에 의해 제어할 수 있습니다.
