...
| `MINI_BATCH_FRACTION` | 교육 중 미니배치로 사용할 데이터의 비율입니다. `(0, 1]` 범위에 있어야 합니다. | 1.0 | `(0, 1]` |
...
| `MAX_MEMORY_IN_MB` | 히스토그램 집계에 할당된 최대 메모리(MB)입니다. 이 값이 너무 작으면 반복당 하나의 노드만 분할되고 해당 집계는 이 크기를 초과할 수 있습니다. | 256 |                                                                                                      |
| `PREDICTION_COL` | 예측 출력의 열 이름입니다. | &quot;prediction&quot; | 모든 문자열 |
| `VALIDATION_INDICATOR_COL` | 열 이름은 각 행이 교육에 사용되는지 또는 유효성 검사에 사용되는지를 나타냅니다. `false`은(는) 교육용, `true`은(는) 유효성 검사용 | 설정되지 않음 | 모든 문자열 |
...
| `FEATURE_SUBSET_STRATEGY` | 각 트리 노드에서 분할할 것으로 간주되는 기능의 수입니다. 지원되는 옵션: `auto`, `all`, `onethird`, `sqrt`, `log2` 및 `n`(일부 기능). | &quot;auto&quot; | `auto`, `all`, `onethird`, `sqrt`, `log2`, `n`(여기서 `n`은(는) 0과 1.0 사이의 분수임) |
| `WEIGHT_COL` | 열 이름(예: 가중치)입니다. 설정되지 않았거나 비어 있으면 모든 인스턴스 가중치는 `1.0`(으)로 처리됩니다. | 설정되지 않음 |                                                                                                      |
| `LOSS_TYPE` | [!DNL Gradient Boosted Tree] 모델이 최소화하려는 손실 함수입니다. 지원되는 옵션: `logistic`. | &quot;logistic&quot; | `logistic` |
...
| `AGGREGATION_DEPTH` | 트리 집계를 위한 제안된 깊이입니다. | 2 |                                                                                                      |
| `FIT_INTERCEPT` | 절편 용어에 맞는지 여부. | `true` | `true`, `false` |
| `TOL` | 최적화를 위한 수렴 허용 오차. | 1E-6 | (>= 0) |
| `MAX_BLOCK_SIZE_IN_MB` | 입력 데이터를 블록으로 스택하기 위한 최대 메모리(MB)입니다. 데이터는 파티션 내에 스택되며, 파티션의 나머지 데이터 크기가 더 작은 경우 이 값이 그에 따라 조정됩니다. | 0.0 | (>= 0) |
| `REG_PARAM` | 모델 복잡성을 제어하고 오버피팅을 방지하는 데 도움이 되는 정규화 매개변수입니다. | 0.0 | (>= 0) |
| `STANDARDIZATION` | 모델에 맞추기 전에 교육 기능을 표준화할지 여부입니다. | `true` | `true`, `false` |
...
[!DNL Logistic Regression]은(는) 이진 분류에 사용되는 감독되는 알고리즘입니다. 로지스틱 함수를 사용하여 인스턴스가 클래스에 속할 확률을 모델링하므로 인스턴스를 두 가지 카테고리 중 하나로 분류하는 것이 목표인 작업에 적합합니다.
...
[!DNL Multilayer Perceptron Classifier]은(는) 완전히 연결된 여러 노드 레이어로 구성된 피드포워드 인공 신경망 분류자입니다. 레이어의 각 노드는 입력 데이터의 가중 선형 조합을 사용하여 입력을 출력에 매핑합니다. 노드 가중치(`w`) 및 바이어스(`b`) 다음에 활성화 함수가 옵니다. 이 프로세스는 추가 지침을 위해 제공된 코드 예와 함께 주요 매개변수를 조정하여 고급 통계 모델을 구성하고 최적화하는 데 도움이 됩니다. —>
...
[!DNL Naive Bayes Classifier]은(는) 기능 간에 강력한(순진한) 독립성 가정을 가진 베이즈 정리에 기반한 간단한 확률론적 다중 클래스 분류자입니다. 각 레이블이 지정된 각 기능의 조건부 확률 분포를 계산하기 위해 교육 데이터에 대한 단일 패스만 요구하여 훈련에서 매우 효율적입니다. 예측의 경우, 관측치가 주어진 각 레이블의 조건부 확률 분포를 계산하기 위해 베이즈 정리를 사용한다.
...
| `SMOOTHING` | 범주형 데이터를 매끄럽게 하고 보이지 않는 피쳐를 처리하는 데 사용되는 매끄럽게 하기 매개 변수입니다. | 1.0 | (>= 0) |
| `PROBABILITY_COL` | 예측된 클래스 조건부 확률에 대한 열 이름입니다. 이러한 점수는 정확한 확률보다는 신뢰도 점수로 취급되어야 한다. | &quot;probability&quot; | 모든 문자열 |
