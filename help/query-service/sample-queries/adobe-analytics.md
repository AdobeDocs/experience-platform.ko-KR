...
description: 선택한 Adobe Analytics 보고서 세트의 데이터는 XDM ExperienceEvents로 변환되어 Adobe Experience Platform에 데이터 세트로 수집됩니다. 이 문서에서는 Adobe Experience Platform Query Service에서 이 데이터를 사용하는 많은 사용 사례를 간략하게 설명하고 포함된 샘플 쿼리는 Adobe Analytics 데이터 세트에서 작동해야 합니다.
...
이 문서에서는 Adobe Experience Platform이 [!DNL Query Service] 은 샘플 쿼리를 포함하여 이 데이터를 사용할 수 있도록 Adobe Analytics 데이터 세트에서 작동해야 합니다. 다음 문서를 참조하십시오. [Analytics 필드 매핑](../../sources/connectors/adobe-applications/mapping/analytics.md) 매핑 대상 [!DNL Experience Events].
...
다음 예제에서는 Adobe Analytics 데이터를 분석하는 데 일반적으로 사용되는 SQL 쿼리를 보여줍니다.
...
Adobe Experience Platform Query Service는 데이터 중복 제거를 지원합니다. 자세한 내용은 [Query Service 설명서의 데이터 중복 제거](../best-practices/deduplication.md) 을 클릭합니다. [!DNL Experience Event] 데이터 세트.
...
Adobe Analytics에서 머천다이징 변수라고 특별히 구성된 변수를 통해 사용자 지정 제품 수준 데이터를 수집할 수 있습니다. 이는 eVar 또는 사용자 지정 이벤트를 기반으로 합니다. 이러한 변수와 표준 사용 기준의 차이점은 히트에 대한 단일 값만 아니라 히트에 있는 각 제품에 대한 별도의 값을 나타낸다는 것입니다.
...
2. 사용자가 &quot;와플 비니&quot;를 클릭하고 제품 세부 사항 페이지에 도달합니다.\
   ...
   나. 왜냐면 `Product View` 는 바인딩 이벤트로 구성되며 제품 &quot;flumle beanie&quot;가 이제 &quot;internal search:winter hat&quot;이라는 eVar6 값에 바인딩됩니다. &quot;와플 비니&quot; 제품이 수집되면 언제든지 (1) 만료 설정이 도달되거나 (2) 새 eVar6 값이 설정되고 해당 제품에 결합 이벤트가 다시 발생할 때까지 &quot;internal search:winter hat&quot;과 연결됩니다.
