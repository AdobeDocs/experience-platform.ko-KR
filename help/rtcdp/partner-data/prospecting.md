...
브랜드는 서드파티 쿠키에 의존하지 않고, 제한된 예산, 투명성과 광고 지출 수익에 대한 높은 수요 없이 funnel 최고의 고객 확보 사용 사례를 책임감 있게 실행하는 어려운 문제에 동시에 직면하고 있습니다.
...
>[!VIDEO](https://video.tv.adobe.com/v/3452864/?captions=kor&learn=on)
...
* 파트너가 제공한 프로필에 대한 새 **[!UICONTROL Partner ID]** ID 유형
* 새 **[!UICONTROL XDM Individual Prospect Profile]** XDM 클래스
* **(설명서 준비 중)** 파트너 데이터 지원에 맞게 조정된 필드 그룹
* **(설명서 준비 중)** 파트너로부터 수신되는 속성에 추가할 서드파티 레이블

#### 파트너 ID 네임스페이스 만들기 {#create-partner-id-namespace}

파트너로부터 받을 프로필에 대한 새 ID 유형을 생성하여 시작합니다. 이렇게 하려면 ID 섹션에서 **[!UICONTROL Partner ID]** 유형의 새 ID 네임스페이스를 만들어야 합니다.
...
#### **[!UICONTROL XDM Individual Prospect Profile]** 클래스를 사용하여 새 스키마 만들기

그런 다음 **[!UICONTROL Data Management]** > **[!UICONTROL Schemas]**&#x200B;에서 새 스키마를 만들고 **[!UICONTROL XDM Individual Prospect Profile]** 클래스에 할당합니다.
...
**[!UICONTROL XDM Individual Prospect Profile]** 클래스는 아래 표시된 필드를 사용하여 사전 구성됩니다. 잠재 고객 프로필에 대해 파트너가 제공한 속성으로 스키마를 보강하려면 원하는 속성으로 새 필드 그룹을 만들어 스키마에 추가하거나 Adobe에서 제공하는 사전 구성된 필드 그룹 중 하나를 사용하면 됩니다.
...
프로필에 대해 스키마가 아직 활성화되지 않았습니다. 이 스키마를 프로필 서비스에 포함하려면 프로필 버튼을 토글합니다. 실시간 고객 프로필에서 사용할 스키마를 활성화하는 방법에 대한 자세한 내용은 [스키마 튜토리얼 만들기](/help/xdm/tutorials/create-schema-ui.md#profile)를 참조하십시오.
...
1. 만든 스키마로 이동하여 **[!UICONTROL Labels]** 탭을 선택합니다.
2. 맨 위에 있는 확인란 버튼을 사용하여 이 스키마의 모든 필드를 선택한 다음 오른쪽에 있는 연필 모양 아이콘을 클릭하여 이 스키마에 데이터 거버넌스 레이블을 적용합니다.
3. 왼쪽의 범주에서 **[!UICONTROL Partner Ecosystem]** 레이블을 선택합니다.
4. 이름이 **[!UICONTROL Third Party]**&#x200B;인 레이블을 선택하고 **[!UICONTROL Save]**&#x200B;을(를) 선택합니다.
...
1. **[!UICONTROL Data Management]** > **[!UICONTROL Datasets]**(으)로 이동하고 **[!UICONTROL Create dataset]**&#x200B;을(를) 선택합니다.
2. 스키마에서 데이터 세트 만들기를 선택합니다.
3. 이전 단계에서 생성한 스키마를 선택합니다.
4. 데이터 세트의 이름을 지정하고 필요한 경우 설명을 제공합니다.
5. **[!UICONTROL Finish]**&#x200B;를 선택합니다.
...
파트너로부터 받은 파일을 데이터 집합에 로드하려면 데이터 집합을 선택하고 오른쪽 레일에서 아래로 스크롤한 다음 **[!UICONTROL Add data]**&#x200B;을(를) 선택합니다. 파일을 끌어 놓거나 **[!UICONTROL Choose files]**&#x200B;을(를) 선택하여 파일 위치로 이동한 후 선택할 수 있습니다.
...
잠재 고객 프로필 목록을 보려면 왼쪽 레일에서 **[!UICONTROL Prospects]** > **[!UICONTROL Profiles]**(으)로 이동합니다.

Real-Time CDP에 방금 로드한 잠재 고객 프로필이 잠재 고객 프로필 화면의 **[!UICONTROL Browse]** 보기에 표시되는 데 최대 2시간이 걸릴 수 있습니다. 페이지에 “지금 검색할 수 있는 잠재 고객 프로필이 없습니다”라는 메시지가 표시되면 잠시 후 다시 시도하십시오. 잠시 기다린 후 잠재 고객 프로필이 **[!UICONTROL Browse]** 보기에 표시되기 시작해야 합니다.

>[!TIP]
>
>**[!UICONTROL Identity Namespace]** 열이 있는지 확인합니다. 여러 데이터 공급업체와 작업하는 경우 이 열을 사용하여 잠재 고객 프로필의 출처를 유추할 수 있습니다.
...
잠재 고객 프로필로 구성된 대상을 시작하고 만들려면 **[!UICONTROL Prospects]** > **[!UICONTROL Audiences]**(으)로 이동합니다.
