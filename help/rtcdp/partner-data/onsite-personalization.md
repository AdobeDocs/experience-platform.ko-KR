...
* 데이터 수집
   * [Adobe Experience Platform 웹 SDK](/help/collection/js/js-overview.md)
...
>[!VIDEO](https://video.tv.adobe.com/v/3449259/?captions=kor&learn=on)
...
먼저 파트너 ID 네임스페이스를 생성해야 합니다. 왼쪽 레일에서 **[!UICONTROL Customer]** > **[!UICONTROL Identities]**(으)로 이동한 다음 화면의 오른쪽 상단 모서리에서 **[!UICONTROL Create identity namespace]**&#x200B;을(를) 선택합니다.
...
그런 다음 [!UICONTROL Experience Event] 스키마를 만들어 나중에 웹 속성에서 수집할 시계열 데이터를 보유하고 [!UICONTROL XDM ExperienceEvent]을(를) 스키마의 기본 클래스로 사용합니다. [Experience Platform UI를 사용하여 스키마 만들기](/help/xdm/ui/resources/schemas.md#create)에 대해 자세히 알아보십시오.
...
스키마를 만드는 단계와 마찬가지로 데이터 집합을 [!UICONTROL Real-Time Customer Profile]에 포함할 수 있도록 설정해야 합니다. [!UICONTROL Real-Time Customer Profile]에서 사용할 데이터 집합을 활성화하는 방법에 대한 자세한 내용은 [스키마 만들기 자습서](/help/xdm/tutorials/create-schema-ui.md#profile)를 참조하십시오.

### 웹 속성에서 이벤트 데이터 수집 구현 {#implement-data-collection}

데이터 관리 구성을 설정하면 이제 웹 속성에 실시간 이벤트 [데이터 수집](/help/collection/home.md)을 구현해야 합니다. Adobe 데이터 수집 라이브러리([!UICONTROL Web SDK])로 속성을 계기하여 실시간 이벤트 호출을 수집하고 이를 Real-Time CDP으로 다시 보내야 합니다. 이 항목은 몇 가지 데이터 수집 구성 요소에 걸쳐 몇 가지 개별 작업으로 구성됩니다.

>[!IMPORTANT]
>
>파트너 제공 속성을 검색하려면 *웹 속성을 파트너 API 또는 다른 방법과 통합하여 데이터 파트너로부터 실시간으로 속성을 호출하고 검색*&#x200B;해야 합니다. 이 튜토리얼의 대상이 아니므로 선택한 파트너와 이에 대해 논의하십시오.

먼저 화면 오른쪽 위의 응용 프로그램 전환기를 사용하여 **[!UICONTROL Data Collection]** 섹션으로 이동합니다.

>[!TIP]
>
>응용 프로그램 전환기에서 [!UICONTROL Data Collection]을(를) 볼 수 없는 경우 시스템 관리자에게 문의하여 액세스 권한을 요청하십시오.

![데이터 수집 섹션으로 이동하는 앱 전환기.](/help/rtcdp/assets/partner-data/onsite-personalization/app-switcher-data-collection.png)

UI의 **[!UICONTROL Data Collection]** 섹션은 아래 이미지와 유사합니다.
...
데이터 스트림을 만들 때 **[!UICONTROL Event schema]** 필드에서 이전에 만든 스키마를 선택합니다.
...
드롭다운에서 이전에 만든 이벤트 데이터 집합을 [선택하십시오](/help/datastreams/configure.md#aep). **[!UICONTROL Edge Segmentation]** 및 **[!UICONTROL Personalization Destinations]** 옆의 확인란을 선택하고 **[!UICONTROL Save]**&#x200B;을(를) 선택하십시오.
...
**[!UICONTROL Tags]**(으)로 이동하여 **[!UICONTROL New property]**&#x200B;을(를) 선택합니다.

![새 태그 속성을 만듭니다.](/help/rtcdp/assets/partner-data/onsite-personalization/create-tag-property.png)

필수 필드를 입력하고 **[!UICONTROL Save]**&#x200B;을(를) 선택합니다.
...
다음으로 속성 내에 다양한 확장 기능을 설치해야 합니다. 태그 속성을 선택하고 [!UICONTROL Extensions] 섹션으로 이동합니다.

![새 태그 속성을 선택합니다.](/help/rtcdp/assets/partner-data/onsite-personalization/select-tag-property.png)

[!UICONTROL Core] 확장이 이미 설치되어 있습니다. 다음 섹션에 설명된 대로 두 개의 추가 확장 기능을 설치해야 합니다.
...
웹 SDK을 구성하는 화면에서 아래로 **[!UICONTROL Datastreams]** 섹션으로 이동하여 사용 중인 Experience Platform 샌드박스에 대한 정보를 제공합니다. 다음 드롭다운에서 이전 단계에 생성된 해당 샌드박스 및 데이터스트림을 선택합니다. 다른 모든 환경에 대해 동일한 샌드박스 및 데이터스트림 값을 선택할 수 있습니다. 다른 설정은 그대로 두고 **[!UICONTROL Save]**&#x200B;을(를) 선택하십시오.
...
[Experience Cloud ID 서비스 확장 기능](/help/tags/extensions/client/id-service/overview.md)을 사용하여 모든 Experience Cloud 솔루션에서 방문자를 위한 고유 장치 기반의 자사 ID를 만드십시오. 확장 카탈로그에서 **[!UICONTROL ID Service]**&#x200B;을(를) 검색하고 설치하십시오. 확장 기능을 설치할 때 모든 기본 설정을 유지하십시오.

![확장 기능 카탈로그의 ID 서비스 확장 기능을 확인하십시오.](/help/rtcdp/assets/partner-data/onsite-personalization/id-service-extension.png)

#### 환경 설정

그런 다음 왼쪽 탐색에서 **[!UICONTROL Environments]** 섹션으로 이동합니다. 이 단계에서는 웹 사이트를 Adobe Edge 네트워크에 연결하여 방문자 정보를 실시간으로 검색하고 전달해야 합니다.
...
먼저 `partnerData` 요소를 설정합니다. **[!UICONTROL Data Elements]** 섹션으로 이동하여 **[!UICONTROL Create New Data Element]**&#x200B;을(를) 선택합니다.

![새 데이터 요소를 만듭니다.](/help/rtcdp/assets/partner-data/onsite-personalization/create-data-element.gif)

데이터 요소 이름을 `partnerData`로 지정하고 [!UICONTROL extension] 값을 [!UICONTROL Core]&#x200B;(으)로 두고 **[!UICONTROL Data Element Type]**&#x200B;을(를) **[!UICONTROL JavaScript Variable]**(으)로 설정합니다. 제목이 `partnerData`인 필드에 **[!UICONTROL JavaScript variable name]**&#x200B;을(를) 입력하고 **[!UICONTROL Save]**&#x200B;을(를) 선택합니다.
...
두 번째 데이터 요소를 설정하려면 새 변수 이름을 `pageVisit`로 지정하고 **[!UICONTROL Extension]**&#x200B;을(를) **[!UICONTROL Adobe Experience Platform]**(으)로 설정하고 **[!UICONTROL XDM Object]**&#x200B;을(를) 데이터 형식으로 선택하십시오.
...
스키마에서 데이터 파트너에서 기대하는 값에 해당하는 서드파티 속성을 선택합니다. 그런 다음 제목이 **[!UICONTROL Provide entire object]**&#x200B;인 라디오 단추를 선택합니다. 데이터베이스처럼 보이는 아이콘을 선택하고 이전에 생성한 `partnerData` 데이터 요소를 선택합니다.

#### 규칙 설정

**[!UICONTROL Rules]** 섹션에서 방금 만든 데이터 요소에 로드된 특성을 사용하여 Adobe에 개인화 요청을 보내도록 웹 사이트를 구성할 수 있습니다. [규칙 생성](/help/tags/ui/managing-resources/rules.md)에 대해 자세히 알아보십시오.

**[!UICONTROL Create new Rule]**&#x200B;을(를) 선택합니다. 이 규칙의 이름을 **[!UICONTROL Personalize]**&#x200B;로 지정하고 **[!UICONTROL Events]** 옆에 있는 + 기호를 선택합니다. **[!UICONTROL Page Bottom]**&#x200B;을(를) 이벤트로 선택하고 저장합니다.

![규칙의 이벤트 유형 부분에 대한 선택 항목.](/help/rtcdp/assets/partner-data/onsite-personalization/add-events-rule.png)

**[!UICONTROL Actions]** 옆에 있는 + 기호를 선택합니다. 확장을 **[!UICONTROL Adobe Experience Platform Web SDK]**(으)로 업데이트하고 **[!UICONTROL Action Type]**&#x200B;을(를) **[!UICONTROL Send event]**(으)로 설정합니다.

![규칙의 작업 유형 부분에 대한 선택 항목.](/help/rtcdp/assets/partner-data/onsite-personalization/add-action-rule.png)

오른쪽의 **[!UICONTROL Type]** 드롭다운 선택기에서 이벤트 유형으로 `web.webpagedetails.pageViews`을(를) 선택합니다.
...
작업 구성 목록을 아래로 스크롤하고 **[!UICONTROL Render visual personalization decisions]** 상자를 선택해야 합니다. 이는 Adobe Target 또는 기타 유사한 제품을 통해 제공되는 경험을 웹 페이지에서 시각적으로 렌더링할 수 있도록 하는 데 중요합니다. **[!UICONTROL Keep Changes]**&#x200B;을(를) 선택한 다음 규칙을 **[!UICONTROL Save]**&#x200B;합니다.
...
**[!UICONTROL Publishing Flow]**&#x200B;을(를) 선택한 다음 **[!UICONTROL Add Library]**&#x200B;을(를) 선택합니다.

**[!UICONTROL Add all Changed Resources]**&#x200B;을(를) 선택하고 라이브러리에 이름을 지정한 다음 환경을 **[!UICONTROL Development]**(으)로 설정하고 **[!UICONTROL Save & Build to Development]**&#x200B;을(를) 선택합니다.
...
Experience Platform UI에서 **[!UICONTROL Customer]** > **[!UICONTROL Audiences]**(으)로 이동하여 웹 사이트 방문자를 캡처할 대상을 만듭니다.
