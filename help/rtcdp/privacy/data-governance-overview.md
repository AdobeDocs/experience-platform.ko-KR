...
실시간 고객 데이터 Platform(실시간 CDP)는 여러 엔터프라이즈 시스템의 데이터를 통합하므로 마케터는 고객을 보다 효과적으로 식별, 이해 및 참여시킬 수 있습니다. 이 데이터는 조직 또는 법률 규정에 의해 정의된 사용 제한 사항의 적용을 받을 수 있습니다. 따라서 데이터를 처리할 때 실시간 CDP가 사용 정책을 준수하도록 하는 것이 중요합니다.
...
실시간 CDP는 Adobe Experience Platform 위에 구축되므로 대부분의 데이터 거버넌스 기능은 Experience Platform 문서에 포함되어 있습니다. 이 문서는 Experience Platform에 대한 [데이터 거버넌스 개요를](../../data-governance/home.md) 보완하기 위해 작성되었으며 실시간 CDP에서 사용할 수 있는 거버넌스 기능에 대해 간략하게 설명합니다. 다음 주제를 다룹니다.
...
>[!NOTE] 마케팅 작업 및 데이터 사용 정책에 대한 자세한 내용은 Experience Platform 설명서의 [데이터 사용 정책 개요를](../../data-governance/policies/overview.md) 참조하십시오.
...
데이터 사용 레이블이 데이터 규정 준수를 효과적으로 지원하려면 데이터 사용 정책을 정의하고 활성화해야 합니다. 데이터 사용 정책은 실시간 CDP 내에서 데이터에 대해 수행하도록 허용되거나 제한된 마케팅 작업의 종류를 설명하는 규칙입니다. 자세한 내용은 Experience Platform [데이터 거버넌스 개요](../../data-governance/home.md) 의 &quot;데이터 사용 정책&quot; 섹션을 참조하십시오.
...
>[!NOTE] 데이터 세트 내의 특정 필드에만 적용된 데이터 사용 레이블이 있는 경우(전체 데이터 세트 아님) 활성화에서 이러한 필드 수준 레이블의 적용은 다음 조건에서만 발생합니다.
>* 세그먼트 정의에서 필드가 사용됩니다.
...
실시간 CDP에 대한 주요 데이터 거버넌스 기능과 Experience Platform을 통해 데이터를 구현하는 방법을 소개했으므로 Adobe Experience Platform에 대한 데이터 거버넌스 [설명서를 계속 읽어 보시기 바랍니다](../../data-governance/home.md). 이 설명서에서는 데이터 사용 레이블 및 정책 관리를 위한 단계별 워크플로우뿐만 아니라 필수 데이터 거버넌스 개념을 자세히 살펴봅니다.
