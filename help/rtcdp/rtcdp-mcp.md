...
>[!AVAILABILITY]
>
>Real-Time CDP MCP 서버는 사용자가 지원되는 MCP 클라이언트 및 앱 플랫폼(예: Claude, ChatGPT, Claude Code, Codex, Cursor 또는 VS Code)에 설치하고 구성하는 **원격 HTTP 전송 서버**(으)로 배포됩니다. 인증은 **브라우저 기반 로그인 흐름**&#x200B;을 통해 처리됩니다. — 클라이언트가 처음 서버에 연결되면 기본 브라우저가 열리고 Adobe 자격 증명으로 로그인하고 액세스 권한을 부여할 수 있습니다.
...
Real-Time CDP MCP 서버에 의해 노출된 도구는 다음과 같습니다.

| 도구 | 설명 |
| --- | --- |
| **기존 대상 검색** | 선택적 필터(이름, 엔티티 유형, 라이프사이클 상태, 네임스페이스, 원본)로 대상을 나열하거나 ID로 특정 대상을 가져옵니다. |
| **대상 멤버십 미리 보기** | 신뢰 구간 메타데이터를 포함하는 세그먼트 쿼리(프로필 대상의 경우 PQL, 관계형/계정 대상의 경우 SDD)의 크기를 예측합니다. |
| **대상 유형 나열** | 샌드박스에서 사용할 수 있는 대상 커넥터 유형의 카탈로그를 봅니다. |
| **구성된 계정 나열** | 구성된 대상 계정(기본 연결) 및 해당 인증 세부 정보를 찾아봅니다. |
| **구성된 대상 나열** | 이름, 상태, 흐름 사양 또는 소스/타겟 연결별로 필터링 가능한 대상 데이터 흐름을 찾아봅니다. |
| **Source 연결 나열** | 대상 데이터 흐름에 대한 데이터 세트 매핑 정보를 보유하는 소스 연결을 검사합니다. |
| **대상 연결 나열** | 대상에 대한 데이터 형식 및 경로 구성을 포함하는 대상 연결을 검사합니다. |
| **활성화 실행 검사** | 흐름 ID, 상태(성공, 실패, 진행 중) 및 완료 시간 범위별로 필터링 가능한 대상 데이터 흐름 실행 기록을 검토합니다. |
...
* MCP 서버 끝점 URL: `https://rtcdp-mcp.adobe.io/mcp`.
...
`claude.ai` 및 클라우드 데스크톱의 경우 **을(를) 사용하여 Real-Time CDP MCP 서버를**&#x200B;사용자 지정 커넥터`https://rtcdp-mcp.adobe.io/mcp`(으)로 추가하십시오. 개별 클라우드 계획에서 **사용자 지정 > 커넥터**&#x200B;에 추가하십시오. Team 및 Enterprise 계획에서 소유자는 **조직 설정 > 커넥터**&#x200B;에 먼저 추가한 후 각 사용자가 고유한 클라우드 설정으로 연결해야 할 수 있습니다. 구성하고 나면 대화에서 커넥터를 활성화하고 최초 사용 시 Adobe 브라우저 로그인을 완료합니다.

#### ChatGPT

ChatGPT에서 **을(를) 사용하여 Real-Time CDP MCP 서버를**&#x200B;사용자 지정 앱/커넥터`https://rtcdp-mcp.adobe.io/mcp`(으)로 추가합니다. ChatGPT 계획에 따라 **개발자 모드** 및 작업 영역 관리자 승인이 필요할 수 있습니다. 앱/커넥터를 만들거나 활성화한 후 **설정 > 앱** 또는 **설정 > 앱 및 커넥터**&#x200B;에서 연결한 다음 메시지가 표시되면 Adobe 브라우저 로그인을 통해 인증합니다.

#### 기타 UI 기반 클라이언트

Cursor, VS Code 또는 원격 MCP를 지원하는 기타 데스크톱 및 웹 응용 프로그램과 같은 클라이언트의 경우 Real-Time CDP MCP 서버를 **원격 HTTP** 서버로 추가하고 `https://rtcdp-mcp.adobe.io/mcp`을(를) 사용합니다. 클라이언트가 선택적 헤더 또는 전달자 토큰을 지원하는 경우 Adobe이 별도로 지시하지 않는 한 비워 두십시오. 인증은 처음 사용할 때 브라우저 기반 Adobe 로그인 흐름을 통해 처리됩니다.
...
| **활성화 실행 필터링은 시간을 기반으로 합니다** | `Inspect Activation Runs`은(는) 상태 및 완료 타임스탬프(epoch ms UTC)별 필터링을 지원하지만 오류 유형 또는 대상 플랫폼별로는 지원하지 않습니다. | `flowId`에서 가져온 `List Configured Destinations`을(를) 기준으로 특정 대상에 대한 범위 실행을 필터링합니다. |
