...
`/roles` 끝점에 대한 GET 요청을 수행하여 조직에 속한 기존 역할을 모두 나열할 수 있습니다.
...
| `subjectAttributes` | 주체와 주체가 액세스할 수 있는 Experience Platform 리소스 간의 상관 관계를 나타내는 특성입니다. |
...
적용할 작업에 해당하는 역할 ID와 값을 제공하는 동안 `/roles` 끝점에 PATCH 요청을 만들어 역할의 속성을 업데이트할 수 있습니다.
...
역할과 연결된 주체를 업데이트하려면 업데이트할 주체의 역할 ID를 제공하는 동안 `/roles` 끝점에 PATCH 요청을 만듭니다.
...
역할에 조회(GET) 요청을 시도하여 삭제를 확인할 수 있습니다. 역할이 관리에서 제거되었으므로 HTTP 상태 404(찾을 수 없음)를 받게 됩니다.
