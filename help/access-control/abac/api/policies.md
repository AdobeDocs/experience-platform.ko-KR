...
>[!IMPORTANT]
>
>이 끝점은 데이터 사용 정책을 관리하는 데 사용되는 [정책 서비스 API](../../../data-governance/api/policies.md)의 `/policies` 끝점과 혼동하지 않습니다.
...
| `createdAt` | 정책이 생성된 시간입니다. `createdAt` 속성이 unix epoch 타임스탬프에 표시됩니다. |
| `modifiedBy` | 정책을 마지막으로 업데이트한 사용자의 ID입니다. |
| `modifiedAt` | 정책이 마지막으로 업데이트된 시간입니다. `modifiedAt` 속성이 unix epoch 타임스탬프에 표시됩니다. |
| `name` | 정책의 이름입니다. |
| `description` | (선택 사항) 특정 정책에 대한 추가 정보를 제공하기 위해 추가할 수 있는 속성입니다. |
| `status` | 정책의 현재 상태. 이 속성은 정책이 현재 `active`인지 `inactive`인지를 정의합니다. |
| `subjectCondition` | 주제에 적용되는 조건. 주제는 작업을 수행하기 위해 리소스에 대한 액세스를 요청하는 특정 속성을 가진 사용자입니다. 이 경우 `subjectCondition`은(는) 제목 특성에 적용되는 쿼리와 유사한 조건입니다. |
| `rules` | 정책을 정의하는 규칙 세트입니다. 규칙은 주체가 리소스에 대한 작업을 성공적으로 수행하기 위해 인증되는 속성 조합을 정의합니다. |
| `rules.effect` | `action`, `condition` 및 `resource`의 값을 고려한 후 발생하는 효과입니다. 가능한 값은 `permit`, `deny` 또는 `indeterminate`입니다. |
...
| `rules.action` | 조회한 리소스에 대해 주체가 수행할 수 있는 작업입니다. 가능한 값은 `read`, `create`, `edit` 및 `delete`입니다. |
...
다음 요청은 이름이 `acme-integration-policy`인 새 정책을 만듭니다.
...
다음 요청은 정책 ID `c3863937-5d40-448d-a7be-416e538f955e`의 `/description` 값을 대체합니다.
...
다음 요청은 ID가 `c3863937-5d40-448d-a7be-416e538f955e`인 정책을 삭제합니다.
