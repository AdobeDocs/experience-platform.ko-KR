...
![데이터 액세스 가 어떻게 Experience Platform 내에서 수집된 데이터 세트의 검색 기능과 액세스 가능성을 용이하게 하는지 보여 주는 다이어그램입니다.](images/Data_Access_Experience_Platform.png)

## API 사양 참조

Swagger API 참조 설명서를 찾을 수 있습니다 [여기](https://developer.adobe.com/experience-platform-apis/references/data-access/).
...
다음 `"data"` 배열에는 지정된 일괄 처리 내의 모든 파일 목록이 포함됩니다. 반환된 각 파일의 고유 ID는 입니다(`{FILE_ID}`)에 포함되어 있습니다. `"dataSetFileId"` 필드. 이 고유 ID를 사용하여 파일에 액세스하거나 파일을 다운로드할 수 있습니다.
...
파일의 특정 세부 정보에 액세스하려면 파일 식별자(`{FILE_ID}`)를 사용할 수 있습니다. 이 API에는 이름, 크기(바이트) 및 다운로드 링크가 포함되어 있습니다.
...
다음을 사용할 수도 있습니다 [!DNL Data Access] 파일 콘텐츠에 액세스하기 위한 API입니다. 그런 다음 컨텐츠를 외부 소스에 다운로드할 수 있습니다.
...
다음을 통해 가치가 높은 특정 이벤트에 가입할 수 있습니다. [Adobe Developer 콘솔](https://developer.adobe.com/console/). 예를 들어 데이터 수집 이벤트에 가입하여 잠재적인 지연 및 실패를 알릴 수 있습니다. 다음 튜토리얼 참조: [데이터 수집 알림 구독](../ingestion/quality/subscribe-events.md) 추가 정보.
