...
성공적인 응답은 업데이트된 개체의 ID가 포함된 배열을 반환합니다. 이 ID는 PATCH 요청에 전송된 ID와 일치해야 합니다. 이 개체에 대한 GET 요청을 수행하면 이제 다른 모든 값이 변경되지 않은 상태로 유지되 `name` 는 동안 및 `description` 업데이트만 표시됩니다.
