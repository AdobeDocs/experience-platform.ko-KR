...
>[!NOTE]
>
>객체에 대한 패치 작업은 상호 관련 개체를 나타내는 확장 가능한 필드를 수정할 수 없습니다.  관련 개체를 직접 수정해야 합니다.
...
성공적인 응답은 업데이트된 데이터 세트의 ID가 포함된 배열을 반환합니다. 이 ID는 PATCH 요청에서 전송된 ID와 일치해야 합니다. 이제 이 데이터 세트에 대한 GET 요청을 수행하면 `name` 및 업데이트만 표시되고 다른 모든 값은 변경되지 `description` 않습니다.
...
성공적인 응답은 업데이트된 개체의 ID가 포함된 배열을 반환합니다. 이 ID는 PATCH 요청에서 전송된 ID와 일치해야 합니다. 이 개체에 대한 GET 요청을 수행하면 이제 다른 모든 값이 변경되지 않은 상태로 남아 있는 동안 `name` 및 `description` 업데이트만 표시됩니다.
