...
일부 [!DNL Catalog] 객체는 다른 객체와 상호 관련될 수 [!DNL Catalog] 있습니다. 응답 페이로드에서 미리 `@` 고정한 모든 필드는 관련 개체를 나타냅니다. 이러한 필드에 대한 값은 URI 형식을 사용하며, URI가 나타내는 관련 개체를 검색하기 위해 별도의 GET 요청에서 사용할 수 있습니다.
...
| `resource` | API의 루트에 대한 리소스 [!DNL Catalog] 경로입니다. 프로토콜과 도메인은 이 값의 일부가 되어서는 안 되며 &quot;/&quot;로 접두사를 붙여야 합니다. <br/><br/> 하위 요청의 일부로 PATCH 또는 DELETE을 사용할 때 리소스 경로 `method`에 개체 ID를 포함시키십시오. 사용자가 제공한 것과 혼동하지 않도록 리소스 경로 `id`는 개체 자체의 ID를 사용합니다(예: [!DNL Catalog] `resource: "/dataSets/1234567890"`). |
...
성공적인 응답은 각 요청에 할당한 개체, 개별 요청에 `id` 대한 HTTP 상태 코드 및 응답이 포함된 개체 배열을 반환합니다 `body`. 세 개의 샘플 요청이 모두 새 개체를 만들기 위한 것이었기 때문에 각 개체 `body` 의 배열은 새로 만든 개체의 ID만 포함하는 배열이며, 이는 POST 응답이 가장 성공한 표준입니다 [!DNL Catalog].
