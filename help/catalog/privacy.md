...
Adobe Experience Platform [!DNL Identity Service]은(는) 시스템 및 장치 간에 고객 id 데이터를 브리지합니다. [!DNL Identity Service]은(는) id 네임스페이스를 사용하여 id 값을 원본 시스템과 연결하여 컨텍스트를 제공합니다. 네임스페이스는 이메일 주소(&quot;이메일&quot;)와 같은 일반 개념을 나타내거나 Adobe Advertising ID 또는 Adobe Target ID와 같은 특정 애플리케이션과 ID를 연결할 수 있습니다.
...
[!DNL Experience Platform] 사용자 인터페이스에서 **[!UICONTROL Schemas]** 작업 영역을 사용하면 기존 XDM 스키마를 편집할 수 있습니다. 스키마에 ID 설명자를 추가하려면 목록에서 스키마를 선택하고 [ 자습서에서 ](../xdm/tutorials/create-schema-ui.md#identity-field)스키마 필드를 ID 필드로 설정[!DNL Schema Editor]하는 단계를 따릅니다.
...
`/descriptors` API의 [!DNL Schema Registry] 끝점에 POST 요청을 수행하여 데이터 세트의 XDM 스키마에 ID 설명자를 추가할 수 있습니다.
...
| 속성 | 설명 |
| --- | --- |
| `@type` | 생성 중인 설명자 유형. ID 설명자의 경우 값은 &quot;xdm:descriptorIdentity&quot;이어야 합니다. |
...
| `xdm:namespace` | [에서 인식하는 ](../privacy-service/api/appendix.md#standard-namespaces)표준 ID 네임스페이스[!DNL Privacy Service] 중 하나 또는 조직에서 정의한 사용자 지정 네임스페이스입니다. |
| `xdm:property` | :id에서 사용 중인 네임스페이스에 따라 &quot;xdm:code&quot; 또는 &quot;xdm`xdm:namespace`&quot;입니다. |
...
UI에서 작업 요청을 만들 때 데이터 레이크에 저장된 데이터에 대한 작업을 처리하려면 **[!UICONTROL AEP Data Lake]**&#x200B;에서 **[!UICONTROL Products]**&#x200B;을(를) 선택해야 합니다.

![개인 정보 보호 요청 만들기 대화 상자에서 선택한 데이터 레이크 제품을 보여 주는 이미지](./images/privacy/product-value.png)

### API 사용

API에서 작업 요청을 만들 때 제공된 모든 `userIDs`은(는) 적용되는 데이터 저장소에 따라 특정 `namespace` 및 `type`을(를) 사용해야 합니다. ID 서비스에서 인식하는 올바른 ID 네임스페이스가 네임스페이스 값에 제공되어야 합니다. 표준 네임스페이스에는 `standard`을(를) 사용하고 사용자 지정 네임스페이스에는 `custom`을(를) 사용합니다.

데이터 레이크의 ID는 해당 `unregistered` 값에 `type`을(를) 사용하고 해당 데이터 세트에 추가된 `namespace`개인 정보 보호 레이블[ 중 하나와 일치하는 ](#privacy-labels) 값을 사용해야 합니다.
...
[ 스토어에 대한 개인 정보 보호 요청을 처리하는 단계는 ](../profile/privacy.md)실시간 고객 프로필에 대한 개인 정보 보호 요청 처리[!DNL Profile]에 대한 문서를 참조하십시오.
