...
* [일괄 처리 수집](../../ingestion/batch-ingestion/overview.md): [!DNL Experience Platform]을(를) 사용하면 데이터를 일괄 처리 파일로 수집할 수 있습니다.
...
페이로드(POST, PUT, PATCH)가 포함된 모든 요청에는 추가 `Content-Type: application/json` 헤더가 필요합니다. JSON+PATCH 요청의 경우 `Content-Type`은(는) `application/json-patch+json`이어야 합니다.
...
이러한 표준 정의를 사용하면 출처에 관계없이 데이터를 일관되게 해석할 수 있으며 애플리케이션 간에 번역할 필요가 없습니다. 스키마 작성에 대한 자세한 내용은 스키마 작성의 [기본 사항](../../xdm/schema/composition.md)에 대한 안내서를 참조하십시오

## 데이터 세트 스키마 조회

이 자습서는 [스키마 레지스트리 API 자습서](../../xdm/tutorials/create-schema-api.md)가 끝나는 위치에서 시작되며, 해당 자습서 동안 만든 충성도 멤버 스키마를 사용합니다.

[!DNL Schema Registry] 자습서를 완료하지 않은 경우 필요한 스키마를 구성한 후에만 이 데이터 세트 자습서를 시작하고 계속하십시오.
...
>[!NOTE]
>
>이 자습서에서는 모든 예제에 대해 [Apache Parquet](https://parquet.apache.org/docs/) 파일 형식을 사용합니다. JSON 파일 형식을 사용하는 예는 [일괄 처리 수집 개발자 안내서](../../ingestion/batch-ingestion/api-overview.md)에서 확인할 수 있습니다
...
성공적인 응답은 HTTP 상태 201(생성됨) 및 응답 개체를 반환합니다. 응답 개체는 `"@/batches/{BATCH_ID}"` 형식의 새로 만든 일괄 처리 ID가 포함된 배열로 구성됩니다. 배치 ID는 API 호출에서 배치를 참조하는 데 사용되는 읽기 전용의 시스템 생성 문자열입니다.
...
| 매개변수 | 설명 |
| --- | --- |
| `{BATCH_ID}` | 업로드하고 있는 일괄 처리의 `id`입니다. |
| `{DATASET_ID}` | 배치가 유지될 데이터 집합 `id`입니다. |
...
모든 데이터 파일을 일괄 처리에 업로드한 후에는 일괄 처리가 완료되었음을 알릴 수 있습니다. 신호 처리가 완료되면 이 서비스는 업로드된 파일에 대해 [!DNL Catalog]개의 `DataSetFile` 항목을 만들고 이전에 생성된 일괄 처리와 연결합니다. [!DNL Catalog] 일괄 처리가 성공으로 표시되어 현재 사용 가능한 데이터에 대해 작업할 수 있는 모든 다운스트림 흐름을 트리거합니다.
...
| 매개변수 | 설명 |
| --- | --- |
| `{BATCH_ID}` | 완료로 표시 중인 일괄 처리의 `id`입니다. |
...
데이터 크기에 따라 일괄 처리에 다양한 시간이 소요됩니다. `GET /batches` 요청에 일괄 처리 ID를 추가하여 일괄 처리 상태를 모니터링할 수 있습니다.
...
양의 응답이 `success` 값을 포함하는 `status` 특성이 있는 개체를 반환합니다.
...
음수 응답은 `"status"` 특성에 값이 `"failed"`인 개체를 반환하며, 다음과 같은 관련 오류 메시지를 포함합니다.
...
데이터 액세스 API 작업에 대한 자세한 단계는 [데이터 액세스 개발자 안내서](../../data-access/home.md)에서 확인할 수 있습니다.

## 데이터 세트 스키마 업데이트

필드를 추가하고 만든 데이터 세트에 추가 데이터를 수집할 수 있습니다. 이렇게 하려면 먼저 새 데이터를 정의하는 속성을 추가하여 스키마를 업데이트해야 합니다. 이 작업은 PATCH 및/또는 PUT 작업을 사용하여 기존 스키마를 업데이트하여 수행할 수 있습니다.
...
스키마가 레지스트리에 저장되고 데이터 수집에 사용된 후에는 스키마에 심각한 변경 내용을 적용할 수 없음을 의미하는 스키마 진화는 순전히 부가적인 것임을 기억해야 합니다. Adobe Experience Platform에서 사용할 스키마를 작성하는 모범 사례에 대한 자세한 내용은 [스키마 작성의 기본 사항](../../xdm/schema/composition.md)에 대한 안내서를 참조하십시오.
