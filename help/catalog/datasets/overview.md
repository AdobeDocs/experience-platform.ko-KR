...
user-guide-description: 이 안내서를 통해 Experience Platform의 데이터 세트에 대한 높은 수준의 개요를 살펴보십시오. 여기에서 데이터 세트를 만들고, 데이터에 대한 제약 조건을 적용하고, 데이터 세트로 데이터를 수집하는 방법을 알아봅니다.
...
[!DNL Experience Data Model]&#x200B;(XDM)은 [!DNL Experience Platform]이(가) 고객 경험 데이터를 구성하는 표준화된 프레임워크입니다. [!DNL Experience Platform]에 수집되는 모든 데이터는 사전 정의된 XDM 스키마를 준수해야 [!DNL Data Lake]에서 데이터 세트로 유지할 수 있습니다.
...
Adobe Experience Platform 데이터 수집은 [!DNL Experience Platform]이(가) 다양한 소스에서 데이터를 수집하는 여러 방법을 나타냅니다. 수집 방법에 관계없이 성공적으로 수집된 모든 데이터는 배치 파일로 변환됩니다. 배치는 단일 단위로 수집할 하나 이상의 파일로 구성된 데이터 단위입니다. 그런 다음 이러한 배치 파일을 전용 데이터 세트에 추가하고 [!DNL Data Lake] 내에서 유지합니다.
...
서비스에 대한 자세한 내용은 [데이터 거버넌스 개요](../../data-governance/home.md)를 참조하세요. [!DNL Experience Platform]에서 사용 레이블을 사용하여 작업하는 방법에 대한 단계는 다음 안내서를 참조하십시오.

* [UI에서 레이블 관리](../../data-governance/labels/user-guide.md)
* [API에서 데이터 세트 레이블 관리](../../data-governance/labels/dataset-api.md)

## 다운스트림 [!DNL Experience Platform] 서비스의 데이터 세트

수집된 데이터를 저장하는 데 데이터 세트를 사용한 후에는 다운스트림 [!DNL Experience Platform] 서비스에서 해당 데이터 세트를 사용하여 고객 프로필을 업데이트하고 머신 러닝을 통해 통찰력을 얻는 등의 작업을 수행합니다.
...
이 문서를 읽으면 [!DNL Experience Platform]의 데이터 세트의 핵심 사용과 데이터 세트를 활용하는 다양한 [!DNL Experience Platform] 서비스에 대해 살펴볼 수 있습니다. [!DNL Experience Platform]에서 데이터 세트를 사용하는 다양한 방법에 대한 자세한 내용은 이 개요 전체에 연결된 서비스 설명서를 검토하십시오.
