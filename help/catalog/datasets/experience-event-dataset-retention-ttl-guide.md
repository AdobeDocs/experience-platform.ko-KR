...
+++답변
시계열 스키마를 사용하는 데이터 세트에 TTL 기반 보존 정책을 적용할 수 있습니다. 여기에는 표준 XDM ExperienceEvent 클래스를 기반으로 한 데이터 세트와 XDM 시계열 클래스를 확장하는 사용자 지정 스키마가 포함됩니다.

행 수준 만료를 사용하려면 다음 기술 조건이 필요합니다.

- 스키마는 XDM 시계열 기본 클래스를 확장해야 합니다.
