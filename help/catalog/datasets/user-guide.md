...
>title="오케스트레이션된 캠페인"
>abstract="이 토글을 활성화하면 선택한 데이터 세트를 Adobe Journey Optimizer 오케스트레이션된 캠페인에서 사용할 수 있습니다. 데이터 세트는 관계형 스키마를 사용해야 하며 스키마당 하나의 데이터 세트만 생성할 수 있습니다."
...
>title="조회를 위해 활성화"
>abstract="조회를 위해 이 데이터 세트를 활성화하면 개인화, 의사 결정 및 여정 오케스트레이션을 위해 Journey Optimizer에서 데이터를 사용할 수 있습니다."
>additional-url="https://experienceleague.adobe.com/ko/docs/journey-optimizer/using/data-management/lookup-aep-data" text="Journey Optimizer의 Adobe Experience Platform 데이터 사용"
...
[!UICONTROL Browse] 탭에서 데이터 집합 이름을 선택하여 **[!UICONTROL Dataset activity]** 화면에 액세스하고 선택한 데이터 집합의 세부 정보를 확인하십시오. 활동 탭에는 메시지 소비율을 시각화한 그래프와 성공한 배치 및 실패한 배치 목록이 표시됩니다
...
[!UICONTROL Delete] 세부 정보 보기에서 [!UICONTROL Enable a dataset for Profile] 또는 [!UICONTROL Dataset]할 수 있습니다. 사용 가능한 작업을 보려면 UI의 오른쪽 상단에서 **[!UICONTROL ... More]**&#x200B;을(를) 선택하십시오. 드롭다운 메뉴가 나타납니다.

![드롭다운 메뉴가 강조 표시된 데이터 세트 작업 영역입니다.[!UICONTROL ... More]](../images/datasets/user-guide/more-actions.png)

**[!UICONTROL Enable a dataset for Profile]**&#x200B;을(를) 선택하면 확인 대화 상자가 나타납니다. **[!UICONTROL Enable]**&#x200B;을(를) 선택하여 선택을 확인합니다.
...
**[!UICONTROL Delete]**&#x200B;을(를) 선택하면 [!UICONTROL Delete dataset] 확인 대화 상자가 나타납니다. **[!UICONTROL Delete]**&#x200B;을(를) 선택하여 선택을 확인합니다.

>[!NOTE]
>
>시스템 데이터 세트는 삭제할 수 없습니다.

[!UICONTROL Browse] 탭에 있는 인라인 작업에서 데이터 세트를 삭제하거나 실시간 고객 프로필에 사용할 데이터 세트를 추가할 수도 있습니다. 자세한 내용은 [인라인 작업 섹션](#inline-actions)을 참조하세요.
...
[!UICONTROL Browse] 탭의 인라인 옵션 또는 [!UICONTROL Dataset activity] 보기에서 모든 데이터 집합에 대해 최대 100개의 샘플 데이터 행을 미리 볼 수 있습니다.

[!UICONTROL Browse] 탭에서 데이터 세트 이름 옆의 생략 부호(...)를 선택하고 [!UICONTROL Preview dataset]을(를) 선택합니다. 데이터 세트가 비어 있으면 미리보기 옵션이 비활성화됩니다. 또는 **[!UICONTROL Dataset activity]** 화면에서 화면 오른쪽 상단 근처에 있는 **[!UICONTROL Preview dataset]**&#x200B;을(를) 선택합니다.
...
또는 **[!UICONTROL Dataset activity]** 화면에서 **[!UICONTROL Preview dataset]**&#x200B;을(를) 선택하여 미리 보기 창을 열고 데이터 집합의 구조 및 값의 샘플을 검토합니다.
...
조직에 데이터 Distiller 라이선스가 있는 경우 데이터 집합 미리 보기 창에서 직접 [!UICONTROL Advanced Query Editor]에 액세스할 수 있습니다. 이 바로 가기를 사용하여 샘플 데이터를 미리 보는 것에서 쿼리 서비스에서 쿼리를 실행하고 구체화하는 것으로 원활하게 이동할 수 있습니다.

>[!AVAILABILITY]
>
>[!UICONTROL Advanced Query Editor]에 대한 액세스는 Data Distiller SKU 라이선스가 있는 조직으로 제한됩니다. 조직에 필요한 라이센스가 없는 경우 데이터 세트 미리 보기 창에 이 옵션이 표시되지 않습니다.

미리 보기 창의 오른쪽 상단에서 [!UICONTROL Advanced Query Editor]을(를) 선택하여 현재 SQL 쿼리가 미리 로드되어 실행된 쿼리 서비스를 엽니다. 쿼리를 다시 입력하지 않고 SQL을 계속 분석하거나 수정할 수 있습니다.

![오른쪽 위에 고급 쿼리 편집기 단추를 표시하는 데이터 집합 미리 보기 창입니다.](../images/datasets/user-guide/dataset-preview-advanced-query-editor.png)

추가 분석을 위해 다운스트림 서비스(예: [!DNL Query Service] 및 [!DNL JupyterLab])를 사용하십시오. 자세한 내용은 다음 문서를 참조하십시오.

* [쿼리 서비스 개요](../../query-service/home.md)
* [JupyterLab 사용 안내서](../../data-science-workspace/jupyterlab/overview.md)

### 데이터 세트에 대한 데이터 거버넌스 관리 및 시행 {#manage-and-enforce-data-governance}

[!UICONTROL Browse] 탭의 인라인 옵션을 선택하여 데이터 집합에 대한 데이터 거버넌스 레이블을 관리할 수 있습니다. 관리할 데이터 세트 이름 옆의 말줄임표(...)를 선택한 후 드롭다운 메뉴에서 **[!UICONTROL Manage data and access labels]**&#x200B;을(를) 선택합니다.
...
[!UICONTROL Browse] 탭의 인라인 옵션과 [!UICONTROL Dataset activity] 보기 모두에서 프로필에 대한 데이터 세트를 활성화할 수 있습니다. [!UICONTROL Browse] 작업 영역의 [!UICONTROL Datasets] 탭에서 프로필에 사용할 데이터 집합의 줄임표를 선택합니다. 옵션 메뉴 목록이 나타납니다. 사용 가능한 옵션 목록에서 **[!UICONTROL Enable unified profile]**&#x200B;을(를) 선택합니다.

![줄임표 및 통합 프로필 사용이 강조 표시된 데이터 세트 작업 영역의 찾아보기 탭입니다.](../images/datasets/user-guide/enable-for-profile.png)

또는 데이터 집합의 **[!UICONTROL Dataset activity]** 화면에서 **[!UICONTROL Profile]** 열 내의 **[!UICONTROL Properties]** 전환을 선택합니다. 활성화되면 데이터 세트에 수집된 데이터는 고객 프로필을 채우는 데에도 사용됩니다.
...
사용자 지정 생성 태그를 추가하여 데이터 세트를 구성하고 검색, 필터링 및 정렬 기능을 개선합니다. [!UICONTROL Browse] 작업 영역의 [!UICONTROL Datasets] 탭에서 관리할 데이터 집합의 줄임표 다음에 드롭다운 메뉴에서 **[!UICONTROL Manage tags]**&#x200B;을(를) 선택합니다.

![선택한 데이터 세트에 대해 줄임표 및 태그 관리 옵션이 강조 표시된 데이터 세트 작업 영역의 찾아보기 탭입니다.](../images/datasets/user-guide/manage-tags.png)

[!UICONTROL Manage tags] 대화 상자가 나타납니다. 간단한 설명을 입력하여 사용자 지정 태그를 만들거나 기존 태그 중에서 선택하여 데이터 세트에 레이블을 지정합니다. 설정을 확인하려면 **[!UICONTROL Save]**&#x200B;을(를) 선택하세요.

![사용자 지정 태그가 강조 표시된 태그 관리 대화 상자입니다.](../images/datasets/user-guide/manage-tags-dialog.png)

[!UICONTROL Manage tags] 대화 상자는 데이터 집합에서 기존 태그를 제거할 수도 있습니다. 제거할 태그 옆에 있는 &#39;x&#39;를 선택하고 **[!UICONTROL Save]**&#x200B;을(를) 선택하면 됩니다.
...
[!UICONTROL Browse] 작업 영역의 [!UICONTROL Datasets] 탭에서 인라인 작업 메뉴를 사용하여 데이터 집합 만료 및 보존 설정을 관리합니다. 이 기능을 사용하여 데이터가 데이터 레이크 및 프로필 저장소에 유지되는 기간을 구성할 수 있습니다. 만료 날짜는 Experience Platform에 데이터가 수집된 시기 및 구성된 보존 기간을 기반으로 합니다.

>[!IMPORTANT]
>
>ExperienceEvent 데이터 세트에 대한 보존 규칙을 적용하거나 업데이트하려면 사용자 역할에 **[!UICONTROL Manage datasets]** 권한이 포함되어야 합니다. 이 역할 기반 액세스 제어를 사용하면 권한이 있는 사용자만 데이터 세트 보존 설정을 수정할 수 있습니다.
>
...
보존 기간을 구성하려면 드롭다운 메뉴에서 데이터 세트 옆에 있는 줄임표(**[!UICONTROL Set data retention policy]**)를 선택합니다.

![줄임표 및 데이터 보존 정책 설정 옵션이 강조 표시된 데이터 세트 작업 영역의 찾아보기 탭입니다.](../images/datasets/user-guide/set-data-retention-policy-dropdown.png)

[!UICONTROL Set dataset retention] 대화 상자가 나타납니다. 이 대화 상자에는 샌드박스 수준 라이선스 사용 지표, 데이터 세트 수준 세부 정보 및 현재 데이터 보존 설정이 표시됩니다. 이러한 지표는 사용 권한과 비교하여 사용량을 보여주며 데이터 세트별 스토리지 및 보존 구성을 평가하는 데 도움이 됩니다. 지표에는 데이터 세트 이름, 유형, 프로필 활성화 상태 및 데이터 레이크 및 프로필 스토어 사용이 포함됩니다.
...
다른 보존 정책의 저장소 영향에 대한 시각적 예측을 열려면 **[!UICONTROL View Experience Event Data distribution]**&#x200B;을(를) 선택하십시오.

차트에는 현재 선택한 데이터 세트에 대한 다양한 보존 기간에 대한 경험 이벤트 분포가 표시됩니다. 선택한 유지 기간이 적용되는 경우 제거될 레코드의 정확한 수를 보려면 각 막대 위로 마우스를 가져갑니다.

시각적 예측을 사용하여 다양한 보존 기간의 영향을 평가하고 정보에 입각한 비즈니스 결정을 내릴 수 있습니다. 예를 들어, 30일 보존 기간을 선택하고 차트에 60%의 데이터가 삭제되는 것으로 표시되면, 보존을 확장하여 더 많은 분석 데이터를 보존할 수 있습니다.

>[!NOTE]
>
>경험 이벤트 배포 차트는 선택한 데이터 세트에 해당되며 해당 데이터만 반영합니다. 데이터 레이크에 저장된 데이터에만 적용됩니다.

![경험 이벤트 배포 차트가 표시된 데이터 보존 설정 대화 상자입니다.](../images/datasets/user-guide/visual-forecast.png)

구성에 만족하면 **[!UICONTROL Save]**&#x200B;을(를) 선택하여 설정을 확인하십시오.
...
네 개의 새 열을 통해 데이터 관리에 대한 가시성을 높일 수 있습니다. **[!UICONTROL Data Lake Storage]**, **[!UICONTROL Data Lake Retention]**, **[!UICONTROL Profile Storage]** 및 **[!UICONTROL Profile Retention]**. 이 지표는 데이터가 사용하는 스토리지의 양과 데이터 레이크 및 프로필 서비스 모두에서 보존 기간을 보여 줍니다.
...
| 열 제목 | 설명 |
|---|---|
| [!UICONTROL Data Lake Retention] | 데이터 레이크의 각 데이터 세트에 대한 현재 보존 기간입니다. 이 값은 구성 가능하며 삭제하기 전에 데이터가 유지되는 기간을 결정합니다. |
| [!UICONTROL Data Lake Storage] | 데이터 레이크의 각 데이터 세트에 대한 현재 저장소 사용량. 이 지표를 사용하여 저장소 제한을 관리하고 사용량을 최적화합니다. |
| [!UICONTROL Profile Storage] | 프로필 서비스 내의 각 데이터 세트에 대한 현재 저장소 사용량. 스토리지 사용을 모니터링하고 데이터 관리 결정을 지원합니다. |
| [!UICONTROL Profile Retention] | 프로필 데이터 세트의 현재 보존 기간입니다. 이 값을 업데이트하여 프로필 데이터가 유지되는 기간을 제어할 수 있습니다. |
...
더 나은 데이터 세트 관리를 위해 폴더 내에 데이터 세트를 배치할 수 있습니다. 데이터 집합을 폴더로 이동하려면 관리하려는 데이터 집합 이름 옆의 생략 부호(...)를 선택한 다음 드롭다운 메뉴에서 **[!UICONTROL Move to folder]**&#x200B;을(를) 선택합니다.

![생략 부호 및 [!UICONTROL Datasets]이(가) 강조 표시된 [!UICONTROL Move to folder] 대시보드입니다.](../images/datasets/user-guide/move-to-folder.png)

[!UICONTROL Move] 데이터 집합과 폴더 간 대화 상자가 나타납니다. 대상자를 이동할 폴더를 선택한 다음 **[!UICONTROL Move]**&#x200B;을(를) 선택합니다. 팝업 알림은 데이터 세트 이동이 성공했음을 알려줍니다.

![강조 표시된 [!UICONTROL Move]이(가) 있는 [!UICONTROL Move] 데이터 세트 대화 상자.](../images/datasets/user-guide/move-dialog.png)
...
![데이터 세트 폴더 구조가 표시되고 폴더 표시 아이콘 및 선택한 폴더가 강조 표시된 [!UICONTROL Datasets] 대시보드입니다.](../images/datasets/user-guide/folder-structure.png)

### 데이터 세트 삭제 {#delete}

[!UICONTROL Browse] 탭의 데이터 세트 인라인 작업 또는 [!UICONTROL Dataset activity] 보기의 오른쪽 상단에서 데이터 세트를 삭제할 수 있습니다. [!UICONTROL Browse] 보기에서 삭제할 데이터 세트 이름 옆의 생략 부호(...)를 선택합니다. 옵션 메뉴 목록이 나타납니다. 드롭다운 메뉴에서 **[!UICONTROL Delete]**&#x200B;을(를) 선택합니다.

![선택한 데이터 세트에 대해 선택한 줄임표 및 삭제 옵션이 강조 표시된 데이터 세트 작업 영역의 찾아보기 탭입니다.](../images/datasets/user-guide/inline-delete-dataset.png)

확인 대화 상자가 나타납니다. **[!UICONTROL Delete]**&#x200B;을(를) 선택하여 확인하십시오.

또는 **[!UICONTROL Delete dataset]** 화면에서 **[!UICONTROL Dataset activity]**&#x200B;을(를) 선택합니다.
...
확인 상자가 나타납니다. 데이터 집합 삭제를 확인하려면 **[!UICONTROL Delete]**&#x200B;을(를) 선택하십시오.
...
사용 가능한 데이터 세트 목록을 검색하거나 필터링하려면 필터 아이콘(![필터 아이콘)을 선택합니다.](/help/images/icons/filter.png))(작업 영역의 왼쪽 상단). 왼쪽 레일에 필터 옵션 세트가 나타납니다. 사용 가능한 데이터 세트를 필터링하는 방법에는 몇 가지가 있습니다. 여기에는 [[!UICONTROL Show System Datasets]](#show-system-datasets), [[!UICONTROL Included in profile]](#filter-profile-enabled-datasets), [[!UICONTROL Tags]](#filter-by-tag), [[!UICONTROL Creation date]](#filter-by-creation-date), [[!UICONTROL Modified date], [!UICONTROL Created by]](#filter-by-creation-date) 및 [[!UICONTROL Schema]](#filter-by-schema)이(가) 포함됩니다.
...
프로필에 대해 데이터 세트가 활성화되었는지 여부를 기준으로 데이터 세트를 필터링하려면 필터 옵션에서 [!UICONTROL Yes] 확인란을 선택하십시오.
...
[!UICONTROL Tags] 입력에 사용자 지정 태그 이름을 입력한 다음 사용 가능한 옵션 목록에서 태그를 선택하여 해당 태그에 해당하는 데이터 세트를 검색하고 필터링합니다.

![[!UICONTROL Tags] 입력 및 필터 아이콘이 강조 표시된 데이터 세트 작업 영역의 필터 옵션입니다.](../images/datasets/user-guide/filter-tags.png)

### 만든 날짜별로 데이터 세트 필터링 {#filter-by-creation-date}

데이터 세트는 사용자 지정 기간 동안 만든 날짜별로 필터링할 수 있습니다. 이를 사용하여 내역 데이터를 제외하거나 특정 연대순 데이터 인사이트 및 보고를 생성할 수 있습니다. 각 필드의 달력 아이콘을 선택하여 [!UICONTROL Start date] 및 [!UICONTROL End date]을(를) 선택하십시오. 그런 다음 해당 기준을 준수하는 데이터 세트만 찾아보기 탭에 표시됩니다.

### 수정된 날짜별로 데이터 세트 필터링 {#filter-by-modified-date}

생성 날짜 필터와 마찬가지로 마지막으로 수정한 날짜를 기준으로 데이터 세트를 필터링할 수 있습니다. [!UICONTROL Modified date] 섹션에서 각 필드에 대한 달력 아이콘을 선택하여 [!UICONTROL Start date] 및 [!UICONTROL End date]을(를) 선택합니다. 이후 해당 기간 동안 수정된 데이터 세트만 찾아보기 탭에 표시됩니다.
...
[!UICONTROL Browse] 탭의 데이터 세트는 오름차순 또는 내림차순 날짜로 정렬할 수 있습니다. 오름차순과 내림차순 간에 번갈아 표시할 [!UICONTROL Created] 또는 [!UICONTROL Last updated] 열 머리글을 선택하십시오. 선택하면, 열 머리글의 옆에 위쪽 또는 아래쪽 화살표가 있는 이 항목이 열에 표시됩니다.

![만든 날짜 및 마지막 업데이트 열이 강조 표시된 데이터 세트 작업 영역의 찾아보기 탭입니다.](../images/datasets/user-guide/ascending-descending-columns.png)

## 데이터 세트 만들기 {#create}

새 데이터 집합을 만들려면 먼저 **[!UICONTROL Create dataset]** 대시보드에서 **[!UICONTROL Datasets]**&#x200B;을(를) 선택하십시오.
...
**[!UICONTROL Create dataset]** 화면에서 **[!UICONTROL Create dataset from schema]**&#x200B;을(를) 선택하여 새 빈 데이터 집합을 만듭니다.

![스키마에서 데이터 집합 만들기 단추가 강조 표시되어 있습니다.](../images/datasets/user-guide/create-dataset-schema.png)

**[!UICONTROL Select schema]** 단계가 나타납니다. **[!UICONTROL Next]**&#x200B;을(를) 선택하기 전에 스키마 목록을 탐색하고 데이터 집합이 준수할 스키마를 선택하십시오.

![스키마 목록이 표시됩니다. 데이터 집합을 만드는 데 사용할 스키마가 강조 표시됩니다.](../images/datasets/user-guide/select-schema.png)

**[!UICONTROL Configure dataset]** 단계가 나타납니다. 데이터 집합에 이름 및 선택적 설명을 제공한 다음 **[!UICONTROL Finish]**&#x200B;을(를) 선택하여 데이터 집합을 만드십시오.

![데이터 집합의 구성 세부 정보가 삽입됩니다. 여기에는 데이터 집합 이름 및 설명 등의 세부 정보가 포함됩니다.](../images/datasets/user-guide/configure-dataset-schema.png)
...
CSV 파일을 사용하여 데이터 세트를 만들면 제공된 CSV 파일과 일치하는 구조를 데이터 세트에 제공하는 임시 스키마가 만들어집니다. **[!UICONTROL Create dataset]** 화면에서 **[!UICONTROL Create dataset from CSV file]**&#x200B;을(를) 선택합니다.

![CSV 파일에서 데이터 집합 만들기 단추가 강조 표시됩니다.](../images/datasets/user-guide/create-dataset-csv.png)

**[!UICONTROL Configure]** 단계가 나타납니다. 데이터 집합에 이름과 선택적 설명을 입력한 다음 **[!UICONTROL Next]**&#x200B;을(를) 선택합니다.

![데이터 집합의 구성 세부 정보가 삽입됩니다. 여기에는 데이터 집합 이름 및 설명 등 자세한 정보가 포함됩니다.](../images/datasets/user-guide/configure-dataset-csv.png)

**[!UICONTROL Add data]** 단계가 나타납니다. CSV 파일을 끌어서 화면 중앙에 놓거나 **[!UICONTROL Browse]**&#x200B;을(를) 선택하여 파일 디렉터리를 탐색합니다. 파일 크기는 최대 10GB입니다. CSV 파일이 업로드되면 **[!UICONTROL Save]**&#x200B;을(를) 선택하여 데이터 집합을 만드십시오.
...
[!DNL Experience Platform] UI의 왼쪽 탐색에서 **[!UICONTROL Monitoring]**&#x200B;을(를) 선택합니다. **[!UICONTROL Monitoring]** 대시보드를 사용하면 일괄 처리 또는 스트리밍 수집에서 인바운드 데이터의 상태를 볼 수 있습니다. 개별 배치의 상태를 보려면 **[!UICONTROL Batch end-to-end]** 또는 **[!UICONTROL Streaming end-to-end]**&#x200B;을(를) 선택하십시오. 대시보드에는 성공, 실패 또는 계속 진행 중인 일괄 처리 또는 스트리밍 수집 실행이 모두 나열됩니다. 각 목록은 배치 ID, 대상 데이터 세트의 이름 및 수집된 레코드 수를 포함하여 배치의 세부 정보를 제공합니다. 대상 데이터 집합이 [!DNL Profile]에 대해 활성화된 경우 수집된 ID 및 프로필 레코드 수도 표시됩니다.

![모니터링 일괄 처리 전체 화면이 표시됩니다. 모니터링과 일괄 처리 작업이 모두 강조 표시됩니다.](../images/datasets/user-guide/batch-listing.png)

개별 **[!UICONTROL Batch ID]**&#x200B;에서 선택하여 **[!UICONTROL Batch overview]** 대시보드에 액세스하고 일괄 처리가 수집되지 않을 경우 오류 로그를 포함하여 일괄 처리에 대한 세부 정보를 볼 수 있습니다.
...
일괄 처리를 삭제하려면 대시보드 오른쪽 상단 근처에 있는 **[!UICONTROL Delete batch]**&#x200B;을(를) 선택하십시오. 배치를 삭제하면 원래 배치가 수집된 데이터 세트에서도 배치의 기록이 제거됩니다.
