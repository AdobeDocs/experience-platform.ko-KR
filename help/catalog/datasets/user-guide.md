...
[!UICONTROL 찾아보기] 탭의 인라인 옵션과 [!UICONTROL 데이터 집합 활동] 보기 모두에서 데이터 집합 샘플 데이터를 미리 볼 수 있습니다. 추가적인 탐색 및 컨텍스트 개선 사항으로 새 데이터 세트 미리보기 창을 사용할 수 있습니다.

[!UICONTROL 찾아보기] 탭에서 미리 보려는 데이터 세트 이름 옆에 있는 줄임표(...)를 선택합니다. 옵션 목록이 나타납니다. 그런 다음 사용 가능한 옵션에서 [!UICONTROL 데이터 집합 미리 보기]를 선택합니다. 데이터 세트가 비어 있으면 미리보기 링크가 비활성화되고 미리보기를 사용할 수 없음을 나타냅니다.
...
그러면 데이터 세트에 대한 계층적 스키마 보기가 왼쪽에 표시되는 미리보기 창이 열립니다.
...
데이터 세트 미리보기 창은 데이터 세트를 탐색하고 유효성을 검사하는 간소화된 인터페이스를 제공합니다.
...
* insight 데이터 세트 구조로 빠르게 전환하기 위해 각 열 이름 옆에 표시되는 데이터 유형 표시기입니다.
* 데이터 집합을 생성하는 데 사용된 쿼리를 보여 주는 SQL 쿼리가 창 맨 위에 표시됩니다.
* 효율적인 데이터 검토를 위해 오른쪽 하단에 최대 100개 행의 서식이 지정된 표 보기입니다.
* 추가 탐색 또는 수정을 위해 미리 채워진 SQL 쿼리를 사용하여 Data Distiller 사용자를 위한 쿼리 편집기로 직접 탐색합니다.

이러한 기능은 빠른 탐색, 스키마 이해 및 투명한 데이터 세트 유효성 검사를 지원합니다.

미리 보기 창을 열려면 인라인 작업 또는 **[!UICONTROL 데이터 세트 활동]** 화면에서 [!UICONTROL 데이터 세트 미리 보기]를 선택하십시오.

>[!NOTE]
>
>미리보기 창에는 최대 100개의 행 샘플이 표시됩니다. 데이터가 없는 필드는 보기에서 제외됩니다.

#### 고급 쿼리 편집기 바로 가기 {#query-editor-shortcut}

조직에 Data Distiller 라이센스가 있는 경우 데이터 세트 미리보기 창에서 직접 고급 쿼리 편집기에 액세스할 수 있습니다.

>[!AVAILABILITY]
>
>필요한 Data Distiller 라이선스가 있는 사용자만 이 기능에 액세스할 수 있습니다. 조직에 Data Distiller이 없는 경우 [!UICONTROL 고급 쿼리 편집기] 옵션이 표시되지 않습니다.

쿼리 편집기를 열려면 미리 보기 창의 오른쪽 상단에서 **[!UICONTROL 고급 쿼리 편집기]**&#x200B;를 선택하십시오. 현재 미리보기 쿼리가 미리 로드되어 있으며 실행 또는 추가 분석 준비가 되었습니다.

![오른쪽 상단의 고급 쿼리 편집기 단추를 표시하는 데이터 집합 미리 보기 창](../images/datasets/user-guide/dataset-preview-advanced-query-editor.png)

이 바로 가기를 사용하면 샘플 데이터 미리 보기에서 SQL 또는 컨텍스트를 다시 입력하지 않고 Query Service에서 쿼리를 실행 및 구체화하는 것으로 원활하게 이동할 수 있습니다.

추가 데이터 액세스 및 분석을 위해 [!DNL Query Service] 및 [!DNL JupyterLab]과(와) 같은 다운스트림 서비스를 사용하십시오. 자세한 내용은 다음 문서를 참조하십시오.
