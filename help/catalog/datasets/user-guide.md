...
title: 데이터 세트 사용 안내서
topic: datasets
description: 이 데이터 집합 사용자 안내서는 Adobe Experience Platform 사용자 인터페이스 내에서 데이터 집합을 사용할 때 일반적인 작업을 수행하는 방법에 대한 지침을 제공합니다.
...
[!DNL Experience Platform] UI에서 왼쪽 탐색 영역에서 **[!UICONTROL 데이터 집합]**&#x200B;을 클릭하여 **[!UICONTROL 데이터 집합]** 대시보드를 엽니다. 대시보드는 조직에 대해 사용 가능한 모든 데이터 세트를 나열합니다. 데이터 세트가 준수하는 스키마, 가장 최근의 통합 실행 상태를 비롯하여 나열된 각 데이터 세트에 대한 세부 사항이 표시됩니다.

![](../images/datasets/user-guide/browse_datasets.png)

데이터 집합 이름을 클릭하여 **[!UICONTROL 데이터 집합 활동]** 화면에 액세스하고 선택한 데이터 집합에 대한 세부 정보를 봅니다. 활동 탭에는 성공적인 배치와 실패한 배치의 목록뿐만 아니라 소비되는 메시지 비율을 시각화하는 그래프가 포함되어 있습니다.
...
**[!UICONTROL 데이터 집합 활동]** 화면에서 화면 오른쪽 위 모서리 근처에 있는 **[!UICONTROL 데이터 집합 미리 보기]**&#x200B;를 클릭하여 최대 100개의 데이터 행을 미리 봅니다. 데이터 세트가 비어 있으면 미리 보기 링크가 비활성화되고 미리 보기를 사용할 수 없다고 표시됩니다.
...
새 데이터 세트를 만들려면 **[!UICONTROL 데이터 집합]** 대시보드에서 **[!UICONTROL 데이터 집합 만들기]**&#x200B;를 클릭하여 시작합니다.
...
**[!UICONTROL 데이터 집합 만들기]** 화면에서 **[!UICONTROL 스키마]**&#x200B;에서 데이터 집합 만들기를 클릭하여 빈 새 데이터 집합을 만듭니다.

![](../images/datasets/user-guide/create_dataset_schema.png)

**[!UICONTROL 스키마 선택]** 단계가 나타납니다. 스키마 목록을 탐색하고 **[!UICONTROL 다음]**&#x200B;을 클릭하기 전에 데이터 세트에서 유지할 스키마를 선택합니다.

![](../images/datasets/user-guide/select_schema.png)

**[!UICONTROL 데이터 세트 구성]** 단계가 나타납니다. 데이터 세트에 이름과 선택적 설명을 제공한 다음 **[!UICONTROL 완료]**&#x200B;를 클릭하여 데이터 세트를 만듭니다.

![](../images/datasets/user-guide/configure_dataset_schema.png)

### CSV 파일 {#csv}을(를) 사용하여 데이터 세트 만들기

CSV 파일을 사용하여 데이터 세트를 만들 때 제공된 CSV 파일과 일치하는 구조를 데이터 세트에 제공하기 위해 임시 스키마가 만들어집니다. **[!UICONTROL 데이터 집합 만들기]** 화면에서 **[!UICONTROL CSV 파일에서 데이터 집합 만들기]**&#x200B;라는 상자를 클릭합니다.

![](../images/datasets/user-guide/create_dataset_csv.png)

**[!UICONTROL 구성]** 단계가 나타납니다. 데이터 세트에 이름과 선택적 설명을 입력한 다음 **[!UICONTROL 다음]**&#x200B;을 클릭합니다.

![](../images/datasets/user-guide/configure_dataset_csv.png)

**[!UICONTROL 데이터 추가]** 단계가 나타납니다. CSV 파일을 화면 가운데로 드래그하여 놓거나 **[!UICONTROL 찾아보기]**&#x200B;를 클릭하여 파일 디렉토리를 검색합니다. 파일 크기는 최대 10GB까지 될 수 있습니다. CSV 파일이 업로드되면 **[!UICONTROL 저장]**&#x200B;을 클릭하여 데이터 세트를 만듭니다.
...
프로필에 대한 데이터 세트를 활성화하려면 해당 **[!UICONTROL 데이터 세트 활동]** 화면에 액세스하고 **[!UICONTROL 속성]** 열 내에서 **[!UICONTROL 프로필]** 전환을 클릭합니다. 활성화되면 데이터 세트에 인제스트된 데이터도 고객 프로필을 채우는 데 사용됩니다.
...
먼저 **[!UICONTROL 데이터 집합 활동]** 화면에 액세스하여 데이터 집합을 삭제할 수 있습니다. 그런 다음 **[!UICONTROL 데이터 집합 삭제]**&#x200B;를 클릭하여 삭제합니다.
...
확인 상자가 나타납니다. **[!UICONTROL 삭제]**&#x200B;를 클릭하여 데이터 세트 삭제를 확인합니다.
...
[!DNL Experience Platform] UI에서 왼쪽 탐색 영역에서 **[!UICONTROL 모니터링]**&#x200B;을 클릭합니다. **[!UICONTROL 모니터링]** 대시보드를 사용하면 일괄 처리 또는 스트리밍 습득 중 하나에서 인바운드 데이터의 상태를 볼 수 있습니다. 개별 배치의 상태를 보려면 **[!UICONTROL 일괄 처리 끝]** 또는 **[!UICONTROL 스트리밍 종단 간]**&#x200B;을 클릭합니다. 대시보드에는 성공, 실패 또는 진행 중인 일괄 처리 또는 스트리밍 통합 실행이 모두 표시됩니다. 각 목록에서는 배치 ID, 대상 데이터 세트 이름, 인제스트된 레코드 수를 포함하여 일괄 처리에 대한 세부 사항을 제공합니다. 대상 데이터 세트가 [!DNL Profile]에 대해 활성화된 경우 인제스트된 ID 및 프로필 레코드 수도 표시됩니다.

![](../images/datasets/user-guide/batch_listing.png)

개별 **[!UICONTROL 일괄 처리 ID]**&#x200B;를 클릭하여 **[!UICONTROL 일괄 처리 개요]** 대시보드에 액세스하고 일괄 처리를 인제스트하지 못할 경우 오류 로그를 포함하여 일괄 처리에 대한 세부 정보를 볼 수 있습니다.

![](../images/datasets/user-guide/batch_overview.png)

배치를 삭제하려면 대시보드 오른쪽 상단에서 **[!UICONTROL 배치 삭제]**&#x200B;를 클릭하여 삭제할 수 있습니다. 이렇게 하면 배치가 원래 수집되었던 데이터 세트에서 해당 레코드도 제거됩니다.
