...
[!UICONTROL 찾아보기] 탭의 인라인 옵션과 [!UICONTROL 데이터 집합 활동] 보기 모두에서 데이터 집합 샘플 데이터를 미리 볼 수 있습니다. [!UICONTROL 찾아보기] 탭에서 미리 보려는 데이터 세트 이름 옆의 생략 부호(...)를 선택합니다. 옵션 메뉴 목록이 나타납니다. 사용 가능한 옵션 목록에서 **[!UICONTROL 데이터 집합 미리 보기]**&#x200B;를 선택합니다. 데이터 세트가 비어 있으면 미리보기 링크가 비활성화되고 대신 미리보기를 사용할 수 없다고 표시됩니다.
...
[!UICONTROL 찾아보기] 탭의 인라인 옵션을 선택하여 데이터 집합에 대한 데이터 거버넌스 레이블을 관리할 수 있습니다. 관리하려는 데이터 세트 이름 옆의 생략 부호(...)를 선택한 다음 드롭다운 메뉴에서 **[!UICONTROL 데이터 관리 및 레이블 액세스]**&#x200B;을(를) 선택합니다.
...
태그가 데이터 세트에 추가되면 해당 태그를 기반으로 데이터 세트를 필터링할 수 있습니다. 자세한 내용은 [태그로 데이터 세트를 필터링](#enable-profile)하는 방법에 대한 섹션을 참조하십시오.

쉽게 검색하고 분류할 수 있도록 비즈니스 개체를 분류하는 방법에 대한 자세한 내용은 [메타데이터 분류 관리](../../administrative-tags/ui/managing-tags.md)에 대한 안내서를 참조하십시오. 이 안내서에서는 적절한 권한이 있는 사용자가 사전 정의된 태그를 만들고, 카테고리를 태그에 할당하고, Platform UI의 태그 및 태그 카테고리에 대한 모든 관련 CRUD 작업을 수행하는 방법에 대해 자세히 설명합니다.
...
[!UICONTROL 데이터 세트] 작업 영역의 [!UICONTROL 찾아보기] 탭에서 데이터 세트 수준에서 데이터 세트 만료 및 보존 정책을 관리합니다. 이 기능을 사용하여 데이터 레이크 및 프로필 서비스에 이미 수집된 데이터에 대한 보존 정책을 구성할 수 있습니다. 만료 날짜는 데이터가 플랫폼에 수집된 시기 및 보존 규칙을 기반으로 합니다.

[!UICONTROL 데이터 보존 설정] 대화 상자를 열려면 데이터 집합 옆에 있는 줄임표를 선택한 다음 드롭다운 메뉴에서 **[!UICONTROL 데이터 보존 정책 설정]**&#x200B;을 선택합니다.

![줄임표 및 데이터 보존 정책 설정 옵션이 강조 표시된 데이터 세트 작업 영역의 찾아보기 탭입니다.](../images/datasets/user-guide/set-data-retention-policy-dropdown.png)

[!UICONTROL 데이터 보존 설정] 대화 상자가 나타납니다. 이 대화 상자에는 샌드박스 수준 라이선스 사용 지표 , 데이터 세트 수준 세부 정보 및 데이터 레이크 설정이 표시됩니다. 이러한 지표는 권한과 비교하여 사용량을 보여줍니다. 데이터 세트 세부 정보에는 데이터 세트 이름, 유형, 프로필 활성화 상태 및 현재 데이터 레이크 스토리지 사용이 포함됩니다.

>[!NOTE]
>
>샌드박스 수준의 사용이 허가된 데이터 레이크 스토리지 지표는 아직 개발 중이므로 사용할 수 없습니다.

![데이터 보존 설정 대화 상자](../images/datasets/user-guide/set-data-retention-dialog.png)

데이터 세트 보존 정책을 구성하기 전에 대화 상자에 권장 보존 설정이 표시됩니다. 1개월은 기본 권장 보존 기간입니다. 표준 보존 정책을 조정하려면 숫자를 선택하고 업데이트한 다음 원하는 기간(일, 월, 년)을 선택합니다. 데이터 레이크 및 프로필 서비스에 대한 보존 설정을 독립적으로 구성할 수 있습니다.

>[!NOTE]
> 
>데이터 레이크의 최소 데이터 보존 기간은 30일입니다. 프로필 서비스의 최소 데이터 보존 기간은 1일입니다.

![기간 드롭다운과 저장 이 강조 표시된 데이터 보존 설정 대화 상자.](../images/datasets/user-guide/time-unit-dropdown.png)

데이터 집합 만료 날짜 범위 및 데이터 보존 정책을 구성하는 모범 사례를 정의하는 규칙에 대한 자세한 내용은 [자주 묻는 질문 페이지](../catalog-faq.md)를 참조하세요.

#### (Beta) 보존 기간 및 스토리지 지표에 대한 가시성 향상 {#retention-and-storage-metrics}

데이터 관리에 대한 가시성을 높이는 Beta 사용자에게는 **[!UICONTROL 데이터 레이크 저장소]**, **[!UICONTROL 데이터 레이크 유지]**, **[!UICONTROL 프로필 저장소]** 및 **[!UICONTROL 프로필 유지]**&#x200B;의 네 가지 새 열이 제공됩니다. 이 지표는 데이터 레이크와 프로필 서비스 모두에서 데이터가 소비하는 스토리지 용량과 보존 기간을 보여 줍니다. 이러한 세부 정보를 통해 보존 정책을 최적화하고, 사용 권한에 따라 사용량을 추적하고, 조직 및 규정 표준을 준수할 수 있습니다. 가시성이 향상되어 정보에 입각한 결정을 내리고, 비용을 관리하고, 거버넌스를 간소화하며, 데이터 환경을 명확하게 파악할 수 있습니다.

![4개의 새 저장소 및 보존 열이 강조 표시된 데이터 세트 작업 영역의 찾아보기 탭입니다.](../images/datasets/user-guide/storage-and-retention-columns.png)

다음 표는 Beta 릴리스에서 사용할 수 있는 새로운 보존 및 스토리지 지표에 대한 개요를 제공합니다. 각 열의 목적과 Platform UI 내에서 데이터 보존 및 저장 관리를 지원하는 방법에 대해 자세히 설명합니다.

| 열 제목 | 설명 |
|---|---|
| [!UICONTROL 데이터 레이크 유지] | 각 데이터 세트에 대한 현재 보존 기간을 표시합니다. 이 값은 각 데이터 세트의 보존 설정에서 수정할 수 있습니다. 데이터 레이크 보존 정책은 데이터가 저장되는 기간 및 다른 서비스에서 삭제해야 하는 시기에 대한 규칙을 설정합니다. |
| [!UICONTROL 데이터 레이크 저장소] | 데이터 레이크의 각 데이터 세트에 대한 현재 저장소 사용량을 표시합니다. 이 지표는 각 데이터 세트가 차지하는 공간을 추적하여 스토리지 제한을 관리하고 사용을 최적화하는 데 도움이 됩니다. |
| [!UICONTROL 프로필 저장소] | 프로필 서비스 내의 각 데이터 세트에 대한 현재 저장소 사용량을 표시합니다. 이 정보를 사용하여 스토리지 사용을 모니터링하고 데이터 관리 목표에 부합하는지 확인하십시오. |
| [!UICONTROL 프로필 유지] | 각 프로필 데이터 세트에 대한 보존 기간을 나타냅니다. 이 값은 데이터 세트의 보존 설정에서 조정할 수 있으므로 삭제하기 전에 프로필 데이터가 저장되는 기간을 제어할 수 있습니다. |
...
![시스템 데이터 세트 표시] 섹션이 강조 표시된 데이터 세트 작업 영역의 필터 옵션입니다.](../images/datasets/user-guide/show-system-datasets.png)[!UICONTROL 
...
![프로필에 포함] 섹션이 강조 표시된 데이터 세트 작업 영역의 필터 옵션입니다.](../images/datasets/user-guide/included-in-profile.png)[!UICONTROL 
