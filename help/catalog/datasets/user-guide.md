...
[!UICONTROL Browse] 탭에서 데이터 세트 이름을 선택하여 해당 **[!UICONTROL Dataset activity]** 화면에 액세스하고 선택한 데이터 세트의 세부 정보를 확인합니다. 활동 탭에는 메시지 소비율을 시각화한 그래프와 성공한 배치 및 실패한 배치 목록이 표시됩니다
...
조직에 Data Distiller 라이선스가 있는 경우 데이터 세트 미리 보기 창에서 직접 [!UICONTROL Advanced Query Editor]에 액세스할 수 있습니다. 이 바로 가기를 사용하여 샘플 데이터 미리 보기에서 쿼리 서비스의 쿼리 실행 및 구체화로 원활하게 이동할 수 있습니다.

>[!AVAILABILITY]
>
>[!UICONTROL Advanced Query Editor]에 대한 액세스는 Data Distiller SKU 라이선스를 가진 조직으로 제한됩니다. 조직에 필요한 라이센스가 없는 경우 이 옵션이 데이터 세트 미리 보기 창에 표시되지 않습니다.

미리 보기 창의 오른쪽 상단에 있는 [!UICONTROL Advanced Query Editor]을(를) 선택하여 현재 SQL 쿼리가 미리 로드되고 실행된 상태에서 쿼리 서비스를 엽니다. 쿼리를 다시 입력하지 않고 SQL을 계속 분석하거나 수정할 수 있습니다.
...
[!UICONTROL Browse] 탭의 인라인 옵션을 선택하여 데이터 집합에 대한 데이터 거버넌스 레이블을 관리할 수 있습니다. 관리할 데이터 세트 이름 옆의 생략 부호(...)를 선택한 다음 드롭다운 메뉴에서 **[!UICONTROL Manage data and access labels]**&#x200B;을(를) 선택합니다.
...
다른 보존 정책의 저장 효과에 대한 시각적 예측을 열려면 **[!UICONTROL View Experience Event Data distribution]**&#x200B;을(를) 선택합니다.
...
구성에 만족하면 **[!UICONTROL Save]**&#x200B;을(를) 선택하여 설정을 확인합니다.
