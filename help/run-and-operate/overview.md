...
* **구현자**: 구현 효율성과 안정성을 확인하고 기술 문제를 해결하는 전문가.

## 전제 조건 {#prerequisites}

실행 및 운영 도구에 액세스하려면 **[!UICONTROL View Job Schedules]** 및 **[!UICONTROL View Profile Management]** [액세스 제어 권한](/help/access-control/home.md#permissions)이 필요합니다. 적절한 권한이 있는지 확인하려면 시스템 관리자에게 문의하십시오.
...
[상태 검사](health-checks.md)를 사용하면 스키마 및 ID 구성 문제가 비즈니스 운영에 영향을 미치기 전에 미리 감지할 수 있습니다. 현재 상태 검사는 스키마 및 ID 네임스페이스에 대해 매일 정적 검사를 실행하여 누락된 모범 사례, 잘못된 구성 및 다운스트림 실패로 이어지는 패턴을 표시합니다.

상태 검사는 현재 다섯 가지 기본 영역을 평가합니다.

* **[ID 필드 유효성 검사](health-checks.md#identity-field-validation)**: ID 필드에 적절한 길이 및 패턴 제약 조건이 있는지 확인하십시오.
* **[ID 그래프 연결 규칙](health-checks.md#identity-graph-linking-rules)**: 연결 규칙이 프로필 축소를 방지하도록 구성되어 있는지 확인하십시오.
* **[사용자 및 비사용자 ID 구성](health-checks.md#people-non-people-identity)**: 스키마 클래스 간 올바른 ID 유형 사용의 유효성을 검사합니다.
* **[사용자 지정 ID 네임스페이스 설명](health-checks.md#namespace-missing-description)**: 네임스페이스 메타데이터가 완전한지 확인하십시오.
* **[사용되지 않는 ID 네임스페이스](health-checks.md#deprecated-namespace)**: 정리할 사용되지 않는 네임스페이스를 검색합니다.

## 다음 단계 {#next-steps}

[!UICONTROL Run and Operate] 도구의 목적과 기능을 이해했으므로 다음 리소스를 탐색하여 지식을 심화하십시오.

* [상태 확인](health-checks.md)을 사용하여 스키마 및 ID 구성 문제를 감지하는 방법에 대해 알아봅니다
