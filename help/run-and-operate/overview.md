...
2. 검사 또는 문제 해결 요구 사항과 일치하는 도구를 선택합니다.
   ...
   >[!NOTE]
   >
   >현재 사용 가능한 기능은 [작업 일정](job-schedules.md) 및 [상태 검사](health-checks.md)입니다.
...
>[!IMPORTANT]
>
>[!UICONTROL Health checks]은(는) 현재 제한된 릴리스로 사용할 수 있습니다.

[상태 검사](health-checks.md)를 사용하면 스키마 및 ID 구성 문제가 비즈니스 운영에 영향을 미치기 전에 미리 감지할 수 있습니다. 현재 상태 검사는 스키마와 ID 네임스페이스에 대해 매일 정적 검사를 실행하며 누락된 모범 사례, 잘못된 구성 및 다운스트림 실패로 이어지는 패턴을 표시합니다.
...
* **[사용자 지정 ID 네임스페이스 설명](health-checks.md#namespace-missing-description)**: 네임스페이스 메타데이터가 완전한지 확인합니다.
