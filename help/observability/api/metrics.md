...
| `granularity` | 지표 데이터를 나누기 위한 시간 간격을 나타내는 선택적 필드입니다. 예를 들어 값 `DAY` 다음 사이의 각 날에 대한 지표를 반환합니다 `start` 및 `end` 날짜, 값 `MONTH` 대신 월별 지표 결과를 그룹화합니다. 이 필드를 사용하는 경우 해당 `downsample` 또한 데이터를 그룹화할 집계 함수를 나타내기 위해 속성을 제공해야 합니다. |
