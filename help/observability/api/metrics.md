...
쿼리 매개 변수 사용을 통해 지표를 지정하여 종단점에 대한 GET 요청을 `/metrics` 만들어 지표 데이터를 검색할 수 있습니다.

**API 형식**

매개 변수에 하나 이상의 지표를 제공해야 `metric` 합니다. 다른 쿼리 매개 변수는 결과 필터링을 위해 선택 사항입니다.
...
| `{ID}` | 지표를 노출하려는 특정 [!DNL Platform] 리소스의 식별자입니다. 이 ID는 사용 중인 지표에 따라 선택 사항이거나 필수 또는 적용되지 않을 수 있습니다. 각 지표에 대해 지원되는 ID(필수 및 선택 사항 모두)를 포함한 사용 가능한 지표 목록은 [부록을](#available-metrics) 참조하십시오. |
...
페이로드에서 검색할 지표를 지정하여 `/metrics` 끝점에 POST 요청을 만들어 지표 데이터를 검색할 수 있습니다.
...
| 속성 | 설명 |
| --- | --- |
| `start` | 지표 데이터를 검색하는 가장 빠른 날짜/시간입니다. |
| `end` | 지표 데이터를 검색할 최신 날짜/시간입니다. |
| `granularity` | 지표 데이터를 분류할 시간 간격을 나타내는 선택적 필드입니다. 예를 들어 값이 일 `DAY` 과 `start` 날짜 사이의 각 날짜에 대해 지표를 반환하는 반면, 값이 이 값을 사용하면 지표 결과를 월별로 그룹화할 `end` `MONTH` 수 있습니다. 이 필드를 사용할 때는 데이터를 그룹화할 집계 함수를 나타내도록 해당 `downsample` 속성을 제공해야 합니다. |
| `metrics` | 검색할 각 지표에 대해 하나씩, 개체의 배열. |
| `name` | 관측 가능한 통찰력으로 인식되는 지표의 이름입니다. 승인된 지표 이름의 전체 목록은 [부록을](#available-metrics) 참조하십시오. |
| `filters` | 특정 데이터 집합별로 지표를 필터링할 수 있는 선택적 필드입니다. 이 필드는 각 필터마다 하나씩, 다음 속성이 포함된 개체의 배열입니다. <ul><li>`name`:지표를 필터링할 엔티티 유형. 현재, 만 `dataSets` 지원됩니다.</li><li>`value`:하나 이상의 데이터 집합의 ID. 여러 데이터 집합 ID를 단일 문자열로 제공할 수 있으며 각 ID는 세로 막대 문자(`|`)로 구분됩니다.</li><li>`groupBy`:true로 설정되면 해당 데이터 세트가 지표 결과를 개별적으로 반환해야 하는 여러 데이터 세트를 `value` 나타낸다는 것을 나타냅니다. false로 설정하면 해당 데이터 세트에 대한 지표 결과가 함께 그룹화됩니다.</li></ul> |
| `aggregator` | 여러 시리즈 레코드를 단일 결과로 그룹화하는 데 사용할 집계 함수를 지정합니다. 사용 가능한 집계업체에 대한 자세한 내용은 [OpenTSDB 설명서를 참조하십시오](http://opentsdb.net/docs/build/html/user_guide/query/aggregators.html). |
| `downsample` | 필드를 간격(또는 &quot;버킷&quot;)으로 정렬하여 지표 데이터의 샘플링 비율을 줄이기 위해 집계 함수를 지정할 수 있는 선택적 필드입니다. 다운샘플링 간격은 속성에 의해 `granularity` 결정됩니다. 다운샘플링에 대한 자세한 내용은 [OpenTSDB 설명서를 참조하십시오](http://opentsdb.net/docs/build/html/user_guide/query/downsampling.html). |

**응답**

성공적인 응답으로 요청에 지정된 지표 및 필터에 대한 결과 데이터 포인트가 반환됩니다.
...
| 속성 | 설명 |
| --- | --- |
| `metricResponses` | 요청에 지정된 각 지표를 나타내는 개체의 배열입니다. 각 개체에는 필터 구성 및 반환된 지표 데이터에 대한 정보가 포함되어 있습니다. |
| `metric` | 요청에 제공된 지표 중 하나의 이름입니다. |
| `filters` | 지정된 지표에 대한 필터 구성. |
| `datapoints` | 지정된 지표 및 필터의 결과를 나타내는 개체의 배열입니다. 배열의 개체 수는 요청에 제공된 필터 옵션에 따라 달라집니다. 필터를 제공하지 않으면 배열에 모든 데이터 세트를 나타내는 단일 개체만 포함됩니다. |
| `groupBy` | 지표에 대한 `filter` 속성에 여러 데이터 세트가 지정되고 이 `groupBy` 옵션이 요청에 true로 설정된 경우 이 개체에는 해당 속성이 적용되는 데이터 세트의 ID가 `dps` 포함됩니다.<br><br>응답에서 이 개체가 비어 있는 것으로 나타나면 해당 `dps` 속성이 `filters` 배열에 제공된 모든 데이터 집합(또는 필터가 제공되지 [!DNL Platform] 않은 경우 모든 데이터 집합)에 적용됩니다. |
| `dps` | 지정된 지표, 필터 및 시간 범위에 대해 반환된 데이터입니다. 이 개체의 각 키는 지정된 지표에 해당하는 값을 가진 타임스탬프를 나타냅니다. 각 데이터 인쇄 사이의 기간은 요청에 지정된 `granularity` 값에 따라 달라집니다. |
...
끝점의 응답에서 `/metrics` 특정 조건에서 오류 메시지를 반환할 수 있습니다. 이러한 오류 메시지는 다음 형식으로 반환됩니다.
...
| 오류 코드 | Title | 설명 |
| --- | --- | --- |
| `INSGHT-1000-400` | 잘못된 요청 페이로드 | 요청 페이로드에 문제가 있습니다. 위에 표시된 대로 페이로드 형식을 정확하게 일치하는지 [확인합니다](#v2). 가능한 모든 이유로 이 오류가 발생할 수 있습니다.<ul><li>필수 필드 누락(예: `aggregator`</li><li>잘못된 지표</li><li>요청에 잘못된 수집기가 포함되어 있습니다.</li><li>종료 날짜 이후에 시작 날짜가 발생합니다.</li></ul> |
| `INSGHT-1001-400` | 지표 쿼리 실패 | 잘못된 요청이나 쿼리 자체가 구문 분석되지 않아 지표 데이터베이스를 쿼리하는 동안 오류가 발생했습니다. 다시 시도하기 전에 요청의 형식이 올바른지 확인하십시오. |
| `INSGHT-1001-500` | 지표 쿼리 실패 | 서버 오류로 인해 지표 데이터베이스를 쿼리하는 동안 오류가 발생했습니다. 요청을 다시 시도하십시오. 문제가 계속되면 Adobe 지원 센터에 문의하십시오. |
| `INSGHT-1002-500` | 서비스 오류 | 내부 오류로 인해 요청을 처리할 수 없습니다. 요청을 다시 시도하십시오. 문제가 계속되면 Adobe 지원 센터에 문의하십시오. |
| `INSGHT-1003-401` | 샌드박스 유효성 검사 오류 | 샌드박스 유효성 검사 오류로 인해 요청을 처리할 수 없습니다. 헤더를 통해 제공한 샌드박스 이름이 IMS 조직에 대해 유효하고 활성화된 샌드박스를 `x-sandbox-name` 나타내는지 확인한 후 요청을 다시 시도하십시오. |
