...
| 세그먼트 평가 작업 지연, 오류 및 오류 | 세그먼트 작업 지연 | 이 경고는 세그먼트 평가 작업을 완료하는 데 150분 이상 걸리는 경우 트리거됩니다. <br> 다음 상태 중 하나가 표시됩니다. <br>- 실행 - 실패 또는 지연 조건이 충족되었습니다(활성 상태로 고려). <br>- 비활성 - 조건이 충족되지 않았거나 해결되지 않았습니다(RESOLVED 상태에서 고려). |
