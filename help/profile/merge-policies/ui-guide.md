...
>abstract="이는 플랫폼이 요청된 병합 정책을 찾을 수 없음을 의미합니다. 이 오류를 해결하려면 다음 솔루션 중 하나를 시도해 보십시오.<ul><li>URL에 올바른 병합 정책 ID가 나열되어 있는지 확인합니다.</li><li>액세스하려는 병합 정책에 대한 올바른 조직 및 샌드박스 조합이 있는지 확인합니다.</li></ul>"
