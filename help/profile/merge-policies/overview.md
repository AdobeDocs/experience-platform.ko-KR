...
* **[!UICONTROL 없음]:** 이 옵션을 선택하면 ID가 함께 결합되지 않습니다. 세그먼테이션이 발생하면 동일한 사용자에게 속할 수 있는 ID는 함께 결합되지 않으며, 세그먼테이션은 고객이 대상 멤버십에 적합한지 여부를 결정할 때 각 개별 ID에 첨부된 속성만 고려합니다. 이렇게 하면 단일 고객이 여러 프로필을 보유할 수 있으며 각 프로필은 서로 다른 대상에 대해 자격이 부여되어 동일한 고객에게 여러 마케팅 메시지가 전송될 수 있습니다.
...
프로필 및 대상자 생성에 대한 ID 및 ID 역할에 대한 자세한 내용은 [ID 서비스 개요](../../identity-service/home.md).
...
>[!WARNING]
>
>기존의 연결된 기본 병합 정책이 있는 프로필 수 및 대상자가 영향을 받을 수 있습니다. 기본 병합 정책이 적용된 모든 대상자는 새 기본 병합 정책으로 업데이트됩니다.
