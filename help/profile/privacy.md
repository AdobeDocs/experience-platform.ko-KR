...
Adobe Experience Platform [!DNL Identity Service]은(는) 시스템 및 장치 간에 고객 id 데이터를 브리지합니다. [!DNL Identity Service]은(는) **id 네임스페이스**&#x200B;를 사용하여 ID 값을 원본 시스템에 연결하여 컨텍스트를 제공합니다. 네임스페이스는 이메일 주소(&quot;이메일&quot;)와 같은 일반 개념을 나타내거나 Adobe Advertising ID 또는 Adobe Target ID와 같은 특정 애플리케이션과 ID를 연결할 수 있습니다.
...
API에서 작업 요청을 만들 때 `userIDs` 내에 제공된 모든 ID는 특정 `namespace` 및 `type`을(를) 사용해야 합니다. 네임스페이스 값에 ID 서비스에서 인식하는 올바른 [ID 네임스페이스](#namespaces)을(를) 제공해야 합니다. 표준 네임스페이스에는 `standard`을(를) 사용하고 사용자 지정 네임스페이스에는 `custom`을(를) 사용합니다.
...
>[!NOTE]
>
>[ 배열에서 ](#profile-v-identity) 및 `ProfileService`을(를) 사용하는 영향에 대한 자세한 내용은 이 문서의 뒷부분에서 `identity`프로필 요청 및 ID 요청`include`에 대한 섹션을 참조하십시오.

다음 요청은 [!DNL Profile] 저장소에 있는 단일 고객 데이터에 대한 새 개인 정보 보호 작업을 만듭니다. `userIDs` 배열에서 고객에 대해 표준 `Email` ID 네임스페이스를 사용하는 ID 값과 사용자 지정 `Customer_ID` 네임스페이스를 사용하는 ID 값 두 개가 제공됩니다. [!DNL Profile] 배열의 `ProfileService`(`include`)에 대한 제품 값도 포함됩니다.
...
UI에서 작업 요청을 만들 때 데이터 레이크 또는 **[!UICONTROL AEP Data Lake]**&#x200B;에 저장된 데이터에 대한 작업을 처리하려면 **[!UICONTROL Profile]**&#x200B;에서 **[!UICONTROL Products]** 및/또는 [!DNL Real-Time Customer Profile]을(를) 각각 선택해야 합니다.
...
프로필(`identity`)에 대한 개인 정보 보호 요청에 ID 서비스(`aepDataLake`) 및 데이터 레이크(`ProfileService`)도 제품으로 포함했는지 여부에 따라 프로필과 관련된 다른 데이터 집합이 잠재적으로 다른 시간에 시스템에서 제거됩니다.
...
Privacy Service에서는 ID 결합을 수행하지 않는 병합 정책을 사용해야만 [!DNL Profile] 데이터를 처리할 수 있습니다. UI를 사용하여 개인 정보 보호 요청이 처리되고 있는지 확인하는 경우 **[!DNL None]**&#x200B;을(를) [!UICONTROL ID stitching] 유형으로 사용하는 정책을 사용하고 있는지 확인하십시오. 즉, [!UICONTROL ID stitching]이(가) [!UICONTROL Private graph]&#x200B;(으)로 설정된 병합 정책을 사용할 수 없습니다.
...
[!DNL Experience Platform]에서 사용하지 않는 [!DNL Profile] 리소스에 대한 개인 정보 보호 요청 처리에 대한 자세한 내용은 [데이터 레이크의 개인 정보 보호 요청 처리](../catalog/privacy.md)에 대한 문서를 참조하십시오.
