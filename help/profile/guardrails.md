...
title: 실시간 고객 프로필 데이터에 대한 보호 기능
...
# [!DNL Real-time Customer Profile] 데이터에 대한 보호 기능

[!DNL Real-time Customer Profile] 은 행동 인사이트 및 고객 속성을 기반으로 개인화된 크로스 채널 경험을 전달할 수 있도록 해주는 개별 프로필을 제공합니다. 이 타겟팅을 수행하기 위해 Adobe Experience Platform 내의 [!DNL Profile] 및 세그멘테이션 엔진은 고객 프로필을 개발하는 새로운 방법을 제공하는 고도로 탈화된 하이브리드 데이터 모델을 사용합니다. 이 하이브리드 데이터 모델을 사용하면 수집 중인 데이터가 올바르게 모델링되는 것이 중요합니다. 프로필 데이터를 유지 관리하는 [!DNL Profile] 데이터 저장소는 관계형 저장소가 아니지만 [!DNL Profile]에서 세그먼트를 간단하고 직관적으로 만들 수 있도록 작은 차원 엔터티와의 통합을 허용합니다. 이 통합을 다중 엔티티 세그먼테이션이라고 합니다.

Adobe Experience Platform에서는 [!DNL Real-time Customer Profile]에서 지원할 수 없는 데이터 모델을 만들지 않도록 하는 데 도움이 되는 일련의 보호 기능을 제공합니다. 이 문서에서는 세그멘테이션에 프로필 데이터를 사용할 때 이러한 보호 기능, 우수 사례 및 제한 사항에 대해 설명합니다.
...
* **기본 엔티티:** 기본 엔티티 또는 프로필 엔티티가 데이터를 함께 병합하여 개인에 대한 &quot;단일 데이터 소스&quot;를 형성합니다. 이 통합 데이터는 &quot;결합 보기&quot;라고 하는 것을 사용하여 표시됩니다. 결합 보기는 동일한 클래스를 구현하는 모든 스키마의 필드를 단일 결합 스키마에 집계합니다. [!DNL Real-time Customer Profile]에 대한 결합 스키마는 모든 프로필 속성 및 동작 이벤트에 대한 컨테이너 역할을 하는 비정규화된 하이브리드 데이터 모델입니다.

   시간 구분 속성(&quot;레코드 데이터&quot;라고도 함)은 [!DNL XDM Individual Profile]을 사용하여 모델링되는 반면, 시간 시리즈 데이터(&quot;이벤트 데이터&quot;라고도 함)는 [!DNL XDM ExperienceEvent]을 사용하여 모델링됩니다. 레코드 및 시계열 데이터가 Adobe Experience Platform에서 수집되면 [!DNL Real-time Customer Profile]을 트리거하여 사용할 수 있도록 활성화된 데이터의 섭취를 시작합니다. 수집된 상호 작용 및 세부 사항이 많을수록 더 강력한 개별 프로필이 생성됩니다.

   ![](images/guardrails/profile-entity.png)

* **Dimension 엔티티:** 조직은 저장소, 제품 또는 속성과 같이 개인 이외의 항목을 설명하는 XDM 클래스를 정의할 수도 있습니다. 이러한 비[!DNL XDM Individual Profile] 스키마는 &quot;차원 엔티티&quot;라고 하며 시계열 데이터를 포함하지 않습니다. Dimension 엔티티는 다중 엔티티 세그먼트 정의를 지원하고 단순화하는 조회 데이터를 제공하며, 세그먼테이션 엔진이 최적의 처리를 위해 전체 데이터 세트를 메모리에 로드할 수 있을 만큼 작아야 합니다(빠른 포인트 조회).
...
이 문서에서 제공하는 보호 기능에는 두 가지 제한 유형이 포함됩니다.

* **소프트 제한:** 소프트 제한은 최적 시스템 성능에 대한 권장 최대값을 제공합니다. 시스템을 중단하거나 오류 메시지를 받지 않고 소프트 한도를 넘을 수 있지만, 소프트 한도를 넘어서면 성능이 저하됩니다. 전체 성능이 저하되지 않도록 소프트 한도 내에 있는 것이 좋습니다.
...
| 가드레일 | 제한 | 제한 유형 | 설명 |
| --- | --- | --- | --- |
| 비[!DNL XDM Individual Profile] 엔터티에 대해 시계열 데이터가 허용되지 않습니다. | 0 | 하드 | **프로필 서비스의 비엔터티에 대해 [!DNL XDM Individual Profile] 시계열 데이터가 허용되지 않습니다.** 시계열 데이터 세트가 비[!DNL XDM Individual Profile] ID와 연결된 경우 데이터 세트에 대해 를 활성화해서는 안 됩니다 [!DNL Profile]. |
| 중첩 관계 없음 | 0 | 소프트 | **두 비스키마 간에 관계를 만들면 [!DNL XDM Individual Profile] 안 됩니다.** 관계를 만드는 기능은  [!DNL Profile] 결합 스키마에 속하지 않는 스키마에는 권장되지 않습니다. |
| 기본 ID 필드에 대한 최대 JSON 깊이 | 4 | 소프트 | **기본 ID 필드에 대한 권장 최대 JSON 깊이는 4입니다.** 즉, 중첩된 스키마에서 4개 수준 이상으로 중첩된 경우 필드를 기본 ID로 선택하면 안 됩니다. 중첩 수준이 4번째인 필드는 기본 ID로 사용할 수 있습니다. |

## 데이터 크기 보호 기능

데이터 크기는 다음의 보호 기능으로, 의도한 대로 데이터를 수집, 저장 및 쿼리할 수 있도록 하는 것이 좋습니다.
...
| 가드레일 | 제한 | 제한 유형 | 설명 |
| --- | --- | --- | --- |
| 모든 차원 엔터티에 대한 최대 총 크기 | 5GB | 소프트 | **모든 차원 엔터티에 대한 최대 권장 총 크기는 5GB입니다.** 큰 차원 엔터티를 섭취하면 시스템 성능이 저하됩니다. 예를 들어 10GB 제품 카탈로그를 차원 엔티티로 로드하려는 것은 권장되지 않습니다. |
...
| 일당 수집된 차원 엔티티 배치 수 | 개체당 4개 | 소프트 | **일당 수집된 최대 차원 엔티티 배치 수는 엔티티당 4개입니다.** 예를 들어 제품 카탈로그에 대한 업데이트를 하루에 최대 4회까지 수집할 수 있습니다. 동일한 엔티티에 대한 추가 차원 엔티티 배치를 섭취하면 시스템 성능에 영향을 줍니다. |
