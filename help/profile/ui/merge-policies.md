...
조직에서 사용할 수 있는 각 병합 정책에 대한 세부 사항은 [!UICONTROL 정책 이름], [!UICONTROL 기본 병합 정책]및 [!UICONTROL 스키마를 비롯한 랜딩 페이지에서 볼 수]있습니다.
...
* **[!UICONTROL 속성 병합]**:프로필 조각에는 개별 고객에 대해 존재하는 ID 목록 중 하나의 ID에 대한 정보가 포함되어 있습니다. ID 그래프 유형이 사용한 경우 둘 이상의 ID가 있을 경우 충돌하는 프로필 속성과 우선 순위를 지정해야 합니다. 속성 병합 [!UICONTROL 을] 사용하면 키-값(레코드 데이터) 형식 데이터 집합 간에 병합 충돌이 발생하는 경우 우선 순위를 지정할 데이터 집합 프로필 값을 지정할 수 있습니다. 두 가지 가능한 값이 있습니다.
...
프로필 레코드가 Experience Platform으로 수집되면 수집 시 시스템 타임스탬프를 획득하여 레코드에 추가됩니다. 병합 정책에 대한 [!UICONTROL 속성 병합]  유형으로 순차 타임스탬프가 선택되면 시스템 타임스탬프를 기반으로 프로필이 병합됩니다. 즉, 레코드를 플랫폼으로 인제스트한 때의 타임스탬프를 기반으로 병합이 수행됩니다.
...
사용자 지정 타임스탬프를 사용하려면 [!UICONTROL 외부 소스 시스템 감사 세부 정보] 혼합을 프로필 스키마에 추가해야 합니다. 사용자 지정 타임스탬프가 추가되면 `lastUpdatedDate` 필드를 사용하여 채울 수 있습니다.
...
다음 스크린샷은 [!UICONTROL 외부 소스 시스템 감사 세부 정보 믹싱의 필드를 표시합니다]. 스키마에 믹스를 추가하는 방법을 비롯하여 UI를 사용하는 스키마 작업에 대한 단계별 지침을 보려면 [튜토리얼을 참조하여 UI를 사용하여 스키마를 생성하십시오](../../xdm/tutorials/create-schema-ui.md).
...
이 알림에는 위반된 데이터 사용 정책 목록이 포함되어 있으며 목록에서 정책을 선택하여 위반 세부 사항을 볼 수 있습니다. 위반된 정책을 선택할 때 [!UICONTROL 데이터 연결] 탭은 위반 사유 및 [!UICONTROL 영향]받는 활동을 제공하며, 각 탭에서는 데이터 사용 정책 위반 방법에 대해 자세히 설명합니다.
