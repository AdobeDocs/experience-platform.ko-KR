...
이제 프로필 스토어에서 샘플 데이터를 미리 보고 데이터에 대한 여러 보고서를 실행하는 방법을 알았으므로, 세그멘테이션 서비스 API의 예상 및 미리 보기 끝점을 사용하여 세그먼트 정의와 관련된 요약 수준 정보를 볼 수도 있습니다. 다음은 예상 대상을 격리하는 데 유용한 정보입니다. Segmentation API를 사용하여 미리보기 및 예상 작업에 대해 자세히 알아보려면 다음을 방문하십시오. [끝점 미리 보기 및 예상 안내서](../../segmentation/api/previews-and-estimates.md).
