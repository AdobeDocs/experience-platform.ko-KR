...
| 네임스페이스 코드 | `code`은(는) 각 ID 네임스페이스 이름에 대한 약어입니다. `code`Adobe Experience Platform ID 서비스 API`name`를 사용하여 각 [과(와) 해당 ](../../identity-service/api/list-namespaces.md)의 매핑을 찾을 수 있습니다. `code`은(는) Experience Platform UI에서 [!UICONTROL Identity symbol]이라고도 합니다. 자세한 내용은 [ID 네임스페이스 개요](../../identity-service/features/namespaces.md)를 참조하세요. |
| `reportTimestamp` | 보고서의 타임스탬프. 요청 중에 `date` 매개 변수가 제공된 경우 반환된 보고서는 제공된 날짜에 대한 것입니다. `date` 매개 변수를 제공하지 않으면 가장 최근 보고서가 반환됩니다. |
...
* 사용자 지정 `AAID` ID 네임스페이스뿐만 아니라 `ECID`, `Email` 및 `crmid` 표준 ID로 구성된 142개의 프로필이 있습니다.
...
| `nsDistribution` | 각 네임스페이스에 대해 연결되지 않은 프로필 및 이벤트가 배포된 개별 ID 네임스페이스를 포함하는 개체입니다. 참고: `countOfProfiles` 개체의 각 ID 네임스페이스에 대한 총 `nsDistribution`을(를) 합하면 기간의 `countOfProfiles`과(와) 같습니다. 네임스페이스당 `eventsAssociated` 및 기간당 총 `eventsAssociated`에 대해서도 마찬가지입니다. |
