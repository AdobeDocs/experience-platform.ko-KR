...
종단점에 대한 GET 요청을 만들어 조직에 대해 이미 만들어진 에지 대상을 나열할 수 `/config/destinations` 있습니다.
...
투영 대상의 고유 ID를 알고 있는 경우 조회 요청을 수행하여 세부 사항을 볼 수 있습니다. 이것은 끝점에 GET 요청을 만들고 요청 경로에 대상의 ID를 포함하여 `/config/destinations` 수행합니다.
...
끝점에 PUT 요청을 만들고 요청 경로에서 업데이트할 대상의 ID를 포함하여 기존 대상을 업데이트할 수 있습니다. `/config/destinations` 이 작업은 기본적으로 대상을 _다시 작성하고_ 있으므로 새 대상을 만들 때 제공된 것처럼 요청의 본문에 동일한 속성을 제공해야 합니다.

>[!CAUTION]
>업데이트 요청에 대한 API 응답은 즉시 적용되지만 예측 변경 사항은 비동기적으로 적용됩니다. 즉, 대상에 대한 정의에 대한 업데이트를 수행한 시점과 대상을 적용한 시점은 서로 다릅니다.
...
종단점에 GET 요청을 만들어 조직에 대해 생성된 모든 프로젝션 구성을 나열할 수 `/config/projections` 있습니다. 요청 경로에 선택적 매개변수를 추가하여 특정 스키마의 프로젝션 구성에 액세스하거나 이름별로 개별 투영을 조회할 수도 있습니다.
