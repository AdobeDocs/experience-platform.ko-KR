...
- 계정 및 영업 기회는 시간 우선 순위 기반 병합으로 해결된 엔티티입니다.
   - 계정: `b2b_account` 네임스페이스를 사용하는 ID.
   - 영업 기회: `b2b_opportunity` 네임스페이스를 사용하는 ID입니다.
- 다른 모든 엔티티는 간단히 통합되며 기본 ID 중복만 시간 우선 순위 기반 병합에 병합됩니다.

>[!NOTE]
>
>엔터티 확인은 `b2b_account` 및 `b2b_opportunity`만 지원합니다. 다른 네임스페이스의 ID는 엔티티 해상도에서 사용되지 않습니다. 사용자 정의 네임스페이스를 사용하는 경우 계정 및 기회를 찾을 수 없습니다.
