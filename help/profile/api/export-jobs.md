...
>[!NOTE]
>
>이 안내서에서는 [!DNL Profile API]에서 내보내기 작업의 사용에 대해 다룹니다. Adobe Experience Platform 세그멘테이션 서비스의 내보내기 작업을 관리하는 방법에 대한 자세한 내용은 세그멘테이션 API](../../profile/api/export-jobs.md)에서 [작업 내보내기에 대한 안내서를 참조하십시오.
...
[!DNL Profile] 데이터를 내보내려면 먼저 데이터를 내보낼 데이터 세트를 만든 다음 새 내보내기 작업을 시작해야 합니다. 두 단계 모두 Experience Platform API를 사용하여 카탈로그 서비스 API를 사용하고 나머지 단계는 실시간 고객 프로필 API를 사용하여 수행할 수 있습니다. 각 단계를 완료하기 위한 자세한 지침은 다음에 나오는 섹션에 요약되어 있습니다.

### 대상 데이터 세트 만들기

[!DNL Profile] 데이터를 내보낼 때 먼저 대상 데이터 세트를 만들어야 합니다. 내보내기가 성공하도록 데이터 세트를 올바르게 구성해야 합니다.

주요 고려 사항 중 하나는 데이터 세트가 기반으로 하는 스키마( 아래 API 샘플 요청에서`schemaRef.id`)입니다. 프로필 데이터를 내보내려면 데이터 세트에 [!DNL XDM Individual Profile] 결합 스키마(`https://ns.adobe.com/xdm/context/profile__union`)를 기반으로 해야 합니다. 결합 스키마는 동일한 클래스를 공유하는 스키마의 필드를 집계하는 시스템에서 생성한 읽기 전용 스키마입니다. 이 경우 [!DNL XDM Individual Profile] 클래스입니다. 결합 보기 스키마에 대한 자세한 내용은 스키마 구성 안내서](../../xdm/schema/composition.md#union)의 기본 사항에서 [결합 섹션을 참조하십시오.

이 자습서에서 수행하는 단계는 [!DNL Catalog] API를 사용하여 [!DNL XDM Individual Profile] 결합 스키마를 참조하는 데이터 세트를 만드는 방법에 대해 설명합니다. 또한 [!DNL Platform] 사용자 인터페이스를 사용하여 결합 스키마를 참조하는 데이터 세트를 만들 수도 있습니다. UI 사용 단계는 세그먼트 내보내기](../../segmentation/tutorials/create-dataset-export-segment.md)에 대한 이 UI 자습서에 요약되어 있지만, 여기에 적용할 수도 있습니다. [ 완료되면 이 자습서로 돌아가서 [새 내보내기 작업](#initiate)을 시작하는 단계를 진행할 수 있습니다.
...
조합 유지 데이터 세트가 있는 경우 실시간 고객 프로필 API에서 `/export/jobs` 종단점에 POST 요청을 하고 요청 본문에 내보내려는 데이터의 세부 사항을 제공하여 프로필 데이터를 데이터 세트에 유지하는 내보내기 작업을 만들 수 있습니다.
...
| `additionalFields.eventList` | *(선택 사항)* 다음 설정 중 하나 이상을 제공하여 하위 객체 또는 관련 객체에 대해 내보낸 시계열 이벤트 필드를 제어합니다.<ul><li>`eventList.fields`: 내보낼 필드를 제어합니다.</li><li>`eventList.filter`: 연관된 객체에서 포함된 결과를 제한하는 기준을 지정합니다. 내보내기에 필요한 최소값(일반적으로 날짜)이 필요합니다.</li><li>`eventList.filter.fromIngestTimestamp`: 시계열 이벤트를 제공된 타임스탬프 이후에 수집된 이벤트로 필터링합니다. 이벤트 시간 자체가 아니라 이벤트의 수집 시간입니다.</li></ul> |
| `destination` | **(필수)** 내보낸 데이터에 대한 대상 정보:<ul><li>`destination.datasetId`:  **(필수)** 데이터를 내보낼 데이터 세트의 ID입니다.</li><li>`destination.segmentPerBatch`:  *(선택 사항)* 제공되지 않을 경우 기본값이 로 설정되는 부울 값입니다 `false`. `false` 값을 지정하면 모든 세그먼트 ID를 단일 배치 ID로 내보냅니다. `true` 값은 하나의 세그먼트 ID를 하나의 배치 ID로 내보냅니다. 값을 `true`으로 설정하면 배치 내보내기 성능에 영향을 줄 수 있습니다.</li></ul> |
...
`export/jobs` 종단점에 대한 GET 요청을 수행하여 특정 IMS 조직에 대한 모든 내보내기 작업 목록을 반환할 수 있습니다. 이 요청은 아래와 같이 쿼리 매개 변수 `limit` 및 `offset`도 지원합니다.
...
| `sort` | 결과를 오름차순( **`asc`** ) 또는 내림차순( **`desc`** )으로 특정 필드별로 정렬합니다. 여러 결과 페이지를 반환하는 경우 정렬 매개 변수가 작동하지 않습니다. 예: `sort=updateTime:asc` |
...
응답에는 IMS 조직에서 만든 내보내기 작업이 포함된 `records` 개체가 포함되어 있습니다.
...
특정 내보내기 작업의 세부 사항을 보거나 처리 시 해당 상태를 모니터링하려면 `/export/jobs` 종단점에 GET 요청을 수행하고 경로에 내보내기 작업의 `id` 을 포함할 수 있습니다. `status` 필드가 &quot;SUCCEEDED&quot; 값을 반환하면 내보내기 작업이 완료됩니다.
...
| 매개 변수 | 설명 |
| -------- | ----------- |
| `{EXPORT_JOB_ID}` | 액세스할 내보내기 작업의 `id` |
...
Experience Platform을 사용하면 기존 내보내기 작업을 취소할 수 있습니다. 내보내기 작업이 완료되지 않았거나 처리 단계에서 멈춘 경우를 포함하여 여러 가지 이유로 유용할 수 있습니다. 내보내기 작업을 취소하려면 `/export/jobs` 종단점에 대한 DELETE 요청을 수행하고 요청 경로에 취소할 내보내기 작업의 `id` 을 포함할 수 있습니다.
...
대상 데이터를 쿼리하는 방법에 대한 자세한 내용은 [쿼리 서비스 설명서](../../query-service/home.md)를 검토하십시오.
...
내보내기 작업을 시작하는 [의 섹션에 표시된 API 호출 예에서는 프로필(레코드)과 이벤트(시계열) 데이터를 모두 포함하는 작업을 만듭니다. ](#initiate) 이 섹션에서는 내보내기를 한 데이터 유형 또는 다른 데이터 유형을 포함하도록 제한하는 추가적인 요청 페이로드 예를 제공합니다.
...
내보내기 작업 종단점을 사용하여 [!DNL Profile] 데이터 대신 대상 세그먼트를 내보낼 수도 있습니다. 자세한 내용은 세그멘테이션 API](../../segmentation/api/export-jobs.md)에서 작업 내보내기 의 안내서를 참조하십시오.[
