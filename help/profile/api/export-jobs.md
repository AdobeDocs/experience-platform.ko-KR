...
[!DNL Real-time Customer Profile] 속성 데이터 및 행동 데이터를 포함하여 여러 소스에서 데이터를 취합하여 개별 고객에 대한 단일 뷰를 구축할 수 있습니다. 이후 처리를 위해 데이터 세트에 데이터를 내보낼 [!DNL Profile] 수 있습니다. 예를 들어, 데이터의 대상 세그먼트를 활성화하도록 내보낼 수 있으며, 프로필 속성을 보고용으로 내보낼 수 있습니다. [!DNL Profile]

이 문서에서는 [프로필 API를 사용하여 내보내기 작업을 만들고 관리하는 단계별 지침을 제공합니다](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/real-time-customer-profile.yaml).

>[!NOTE]
>
>이 안내서에서는 내보내기 작업의 사용에 대해 설명합니다 [!DNL Profile API]. Adobe Experience Platform 세그멘테이션 서비스의 내보내기 작업을 관리하는 방법에 대한 자세한 내용은 세그멘테이션 API의 [내보내기 작업 안내서를 참조하십시오](../../profile/api/export-jobs.md).

내보내기 작업을 만드는 것 외에도 &quot; [!DNL Profile] &quot;이라고도 하는 `/entities`[!DNL Profile Access]종점을 사용하여 데이터에 액세스할 수도 있습니다. See the [entities endpoint guide](./entities.md) for more information. UI를 사용하여 [!DNL Profile] 데이터에 액세스하는 방법에 대한 자세한 내용은 [사용 안내서를 참조하십시오](../ui/user-guide.md).

## 시작하기

이 안내서에서 사용되는 API 끝점은 [!DNL Real-time Customer Profile] API의 일부입니다. 계속하기 전에 [시작하기 가이드](getting-started.md) 에서 관련 문서 링크, 이 문서에서 샘플 API 호출 읽기 안내서, 모든 API를 성공적으로 호출하는 데 필요한 필수 헤더에 대한 중요 정보를 검토하십시오 [!DNL Experience Platform] .

## 내보내기 작업 만들기

데이터를 내보내려면 먼저 데이터를 내보낼 데이터 세트를 만든 다음 새 내보내기 작업을 시작해야 합니다. [!DNL Profile] 이러한 두 단계 모두 Experience Platform API를 사용하여 수행할 수 있으며, 이전 단계는 카탈로그 서비스 API를 사용하고 다른 단계는 실시간 고객 프로필 API를 사용합니다. 각 단계를 완료하기 위한 자세한 지침은 다음 섹션에 요약되어 있습니다.

### 대상 데이터 세트 만들기

데이터 [!DNL Profile] 를 내보낼 때 먼저 대상 데이터 세트를 만들어야 합니다. 내보내기가 성공하도록 데이터 세트를 올바르게 구성해야 합니다.

중요한 고려 사항 중 하나는 데이터 세트가 기반으로 하는 스키마입니다(아래`schemaRef.id` API 샘플 요청). 프로필 데이터를 내보내려면 데이터 집합은 [!DNL XDM Individual Profile] 조합 스키마(`https://ns.adobe.com/xdm/context/profile__union`)를 기반으로 해야 합니다. 공용 스키마는 동일한 클래스를 공유하는 스키마의 필드를 집계하는 시스템 생성 읽기 전용 스키마입니다. 이 경우에는 [!DNL XDM Individual Profile] 수업이다. 조합 보기 스키마에 대한 자세한 내용은 스키마 구성 안내서의 기본 사항에 있는 [조합 섹션을 참조하십시오](../../xdm/schema/composition.md#union).

이 튜토리얼에서 나오는 단계는 API를 사용하여 결합 스키마를 참조하는 데이터 세트를 [!DNL XDM Individual Profile] 만드는 방법에 대해 [!DNL Catalog] 설명합니다. 또한 [!DNL Platform] 사용자 인터페이스를 사용하여 조합 스키마를 참조하는 데이터 세트를 만들 수도 있습니다. UI 사용 단계는 세그먼트 내보내기를 위한 [이 UI 자습서에 요약되어](../../segmentation/tutorials/create-dataset-export-segment.md) 있지만 여기에도 적용됩니다. 완료되면 이 튜토리얼로 돌아가 새 내보내기 작업 [을 시작하는 단계를 진행할 수 있습니다](#initiate).

이미 호환되는 데이터 세트가 있고 ID를 알고 있는 경우 새 내보내기 작업을 [시작하는 단계로 직접 진행할 수 있습니다](#initiate).
...
특정 IMS에 대한 모든 내보내기 작업 목록을 `export/jobs` 끝점에 대한 GET 요청을 수행하여 반환할 수 있습니다. 또한 요청에서는 아래와 같이 쿼리 매개 변수 `limit` 와 `offset`를 지원합니다.
...
특정 내보내기 작업의 세부 사항을 보거나, 진행 중인 상태를 모니터링하려면 종단점에 GET을 요청하고 경로에 내보내기 작업 `/export/jobs` `id` 을 포함할 수 있습니다. 필드가 &quot;SUCCESS&quot; 값을 반환하면 내보내기 작업이 `status` 완료됩니다.
...
삭제 요청이 성공하면 HTTP Status 204(콘텐츠 없음) 및 빈 응답 본문을 반환하고 취소 작업이 성공적으로 수행되었음을 나타냅니다.
...
내보내기 작업 [](#initiate) 시작 섹션에 표시된 예제 API 호출은 프로필(레코드)과 이벤트(시간 시리즈) 데이터가 모두 포함된 작업을 만듭니다. 이 섹션에서는 내보내기를 하나의 데이터 유형이나 다른 데이터 유형으로 제한하는 추가 요청 페이로드 예를 제공합니다.

다음 페이로드에서는 프로필 데이터만 포함된 내보내기 작업을 만듭니다(이벤트 없음).
...
내보내기 작업 끝점을 사용하여 데이터 대신 대상 세그먼트를 내보낼 수도 [!DNL Profile] 있습니다. 자세한 내용은 세그멘테이션 API의 [내보내기 작업](../../segmentation/api/export-jobs.md) 안내서를 참조하십시오.