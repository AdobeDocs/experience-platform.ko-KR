...
를 사용하여 호출을 시작하려면 [!DNL Real-time Customer Profile] API에서 다음을 참조하십시오. [시작 안내서](getting-started.md) 그런 다음 엔드포인트 가이드 중 하나를 선택하여 특정 [!DNL Profile]- 관련 끝점입니다. 을 사용하여 작업하려면 [!DNL Profile] 데이터를 [!DNL Experience Platform] UI는 [실시간 고객 프로필 사용 안내서](../ui/user-guide.md).
