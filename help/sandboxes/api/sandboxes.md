...
Adobe Experience Platform의 샌드박스는 프로덕션 환경에 영향을 주지 않고 기능을 테스트하고 실험을 실행하며 사용자 지정 구성을 만들 수 있는 격리된 개발 환경을 제공합니다. `/sandboxes` API의 [!DNL Sandbox] 끝점을 사용하면 Experience Platform에서 샌드박스를 프로그래밍 방식으로 관리할 수 있습니다.
...
개발 샌드박스를 만들려면 요청 페이로드에 `type` 값이 있는 `development` 특성을 제공해야 합니다.
...
프로덕션 샌드박스를 만들려면 요청 페이로드에 `type` 값이 있는 `production` 특성을 제공해야 합니다.
...
[!DNL Audience Manager] 매개 변수를 요청에 추가하여 [!DNL Audience Core Service] 또는 `ignoreWarnings`과(와) 공유하는 양방향 세그먼트에 사용되는 프로덕션 샌드박스를 재설정할 수 있습니다.
