...
다음 `/packages` 샌드박스 도구 API의 끝점을 사용하면 패키지 게시 및 샌드박스로 패키지 가져오기를 포함하여 조직의 패키지를 프로그래밍 방식으로 관리할 수 있습니다.

## 패키지 만들기 {#create}

에 POST 요청을 하여 다중 아티팩트 패키지를 만들 수 있습니다. `/packages` 패키지 이름 및 패키지 유형에 대한 값을 제공하는 동안 끝점이 발생했습니다.
...
| `packageType` | 패키지 유형: **부분** 특정 가공물을 패키지에 포함하고 있음을 나타냅니다. | 문자열 | 예 |
...
| `artifacts` | 패키지로 내보낼 아티팩트 목록입니다. 다음 `artifacts` 값은 다음과 같아야 합니다. **null** 또는 **비어 있음**, 다음과 같은 경우 `packageType` 은(는) `FULL`. | 배열 | 아니요 |
...
에 PUT 요청을 하여 패키지를 업데이트할 수 있습니다. `/packages` 엔드포인트.

### 패키지에 아티팩트 추가 {#add-artifacts}

패키지에 객체를 추가하려면 다음을 제공해야 합니다. `id` 및 포함 **추가** 대상: `action`.
...
| `artifacts` | 패키지에 추가될 아티팩트 목록입니다. 목록이 인 경우 패키지에 변경 사항이 없습니다. **null** 또는 **비어 있음**. 아티팩트는 패키지에 추가되기 전에 중복 제거됩니다. | 배열 | 아니요 |
...
패키지에서 객체를 삭제하려면 다음을 제공해야 합니다. `id` 및 포함 **DELETE** 대상: `action`.
...
| `artifacts` | 패키지에서 삭제할 아티팩트 목록입니다. 목록이 인 경우 패키지에 변경 사항이 없습니다. **null** 또는 **비어 있음**. | 배열 | 아니요 |
...
>[!NOTE]
>
>다음 **업데이트** 작업은 패키지의 패키지 메타데이터 필드를 업데이트하는 데 사용되며 **할 수 없음** 패키지에 아티팩트를 추가/삭제하는 데 사용됩니다.

패키지의 메타데이터 필드를 업데이트하려면 다음을 제공해야 합니다. `id` 및 포함 **업데이트** 대상: `action`.
...
| `sourceSandbox` | 소스 샌드박스는 요청 헤더에 지정된 것과 동일한 조직에 속해야 합니다. | 문자열 | 예 |
...
패키지를 삭제하려면 DELETE에 `/packages` 끝점을 지정하고 삭제할 패키지의 ID를 지정합니다.
...
다음 요청은 ID가 인 패키지를 삭제합니다. {PACKAGE_ID}.
...
패키지를 샌드박스로 가져오려면 게시해야 합니다. 에 GET 요청 `/packages` 게시하려는 패키지의 ID를 지정하는 동안 끝점이 발생했습니다.
...
다음 요청은 ID가 인 패키지를 게시합니다. {PACKAGE_ID}.
...
에 GET 요청을 하여 개별 패키지를 조회할 수 있습니다. `/packages` 요청 경로에 패키지의 해당 ID를 포함하는 종단점입니다.
...
다음 요청은 다음에 대한 정보를 검색합니다. {PACKAGE_ID}.
...
에 GET 요청을 하여 조직의 모든 패키지를 나열할 수 있습니다. `/packages` 엔드포인트.
...
| 매개변수 | 설명 |
| --- | --- |
| {QUERY_PARAMS} | 결과를 필터링 기준으로 사용할 선택적 쿼리 매개 변수입니다. 의 섹션을 참조하십시오. [쿼리 매개 변수](./appendix.md) 추가 정보. |

**요청**

다음 요청은 다음을 기반으로 패키지의 정보를 검색합니다. {QUERY_PARAMS}.
...
충돌은 응답에서 반환됩니다. 응답은 원본 패키지와 `alternatives` 순위를 기준으로 정렬된 배열로서의 조각.
...
충돌을 검토하고 POST 요청을 통해 대체 항목을 제공한 후 패키지에 대한 가져오기를 제출할 수 있습니다. `/packages` 엔드포인트. 결과는 페이로드로 제공되며, 페이로드에 지정된 대로 대상 샌드박스에 대한 가져오기 작업을 시작합니다.
...
에 POST 요청을 하여 내보낸 개체의 모든 종속 개체를 패키지에 나열합니다. `/packages` 패키지의 ID를 지정하는 동안 끝점이 발생했습니다.
...
다음 요청은 다음에 대한 모든 종속 개체를 나열합니다. {PACKAGE_ID}.
...
에 GET 요청을 하여 패키지 아티팩트를 가져올 수 있는 권한이 있는지 확인할 수 있습니다. `/packages` 패키지의 ID와 대상 샌드박스 이름을 지정하는 동안 끝점이 발생했습니다.
...
다음 요청은 다음에 대한 권한을 확인합니다. {PACKAGE_ID} 샌드박스
...
에 GET 요청을 하여 현재 내보내기/가져오기 작업을 나열할 수 있습니다. `/packages` 엔드포인트.
