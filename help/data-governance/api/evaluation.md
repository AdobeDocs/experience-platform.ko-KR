...
하나 이상의 데이터 세트 ID를 제공하는 것 외에도 각 데이터 세트 내의 일부 필드도 지정하여 해당 필드의 데이터 사용 레이블만 평가되어야 함을 나타냅니다. 데이터 집합만 포함하는 POST 요청과 유사하게, 이 요청은 각 데이터 세트에 대한 특정 필드를 요청 본문에 추가합니다.
...
또한 [!DNL Policy Service] API를 사용하여 세그먼트 사용과 관련된 정책 위반을 확인할 수도 [!DNL Real-time Customer Profile] 있습니다. 자세한 내용은 대상 세그먼트에 대한 데이터 [사용 규정 준수를](../../segmentation/tutorials/governance.md) 적용하는 자습서를 참조하십시오.