...
리소스 `core` 는 Adobe에서 정의 및 유지 관리하는 반면, `custom` 리소스는 개별 고객이 생성 및 유지 관리하므로 이러한 리소스를 만든 IMS 조직에서만 고유하고 볼 수 있습니다. 따라서 목록 및 조회 작업(`GET`)은 `core` 자원에 대해 허용되는 유일한 작업이지만 목록 작성, 조회 및 업데이트 작업(`POST`, `PUT``PATCH`및 `DELETE`)은 `custom` 리소스에 사용할 수 있습니다.
...
마케팅 작업 이름은 마케팅 작업을 위한 고유한 식별자입니다. 각 `core` 마케팅 활동에는 모든 IMS 조직에 적용되는 고유한 이름이 있습니다. 이러한 이름은 Adobe에 의해 정의 및 유지됩니다. 한편, 모든 고객 정의 마케팅 작업(`custom` 리소스)은 개별 조직 내에서 고유하며 다른 IMS 조직과 보이거나 공유되지 않습니다.
