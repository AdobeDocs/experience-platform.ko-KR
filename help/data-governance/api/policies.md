...
정책 목록을 보려면 지정된 컨테이너에 대한 모든 정책을 반환하는 GET 요청 `/policies/core` 을 `/policies/custom` 수행할 수 있습니다.
...
데이터 사용 정책을 만든 후 업데이트해야 할 수 있습니다. 이는 업데이트된 정책 양식이 포함된 페이로드가 포함된 정책 `id` 에 대한 PUT 요청을 통해 수행됩니다. 즉, PUT 요청은 기본적으로 정책을 _재작성하는_ 것이므로 본체는 아래 예에서 보듯이 모든 필수 정보를 포함해야 합니다.
