...
데이터 거버넌스를 사용하면 데이터 관리자가 스키마 필드 수준 및 데이터 세트 수준에서 사용 레이블을 적용하여 적용되는 정책 유형에 따라 데이터를 분류할 수 있습니다.
...
마케팅 액션의 예로는 데이터 세트를 서드파티 서비스로 내보내는 작업이 있을 수 있습니다. PII(개인 식별 정보)를 내보낼 수 없다고 선언하는 정책이 있고, &quot;I&quot; 레이블(ID 데이터)이 필드 수준 또는 데이터 세트에 적용된 경우, [!DNL Policy Service] 은 이 데이터 세트를 서드파티 대상으로 내보내는 모든 작업을 방지합니다. 이러한 작업 시도 중 하나가 발생하면 Policy Service에서 데이터 사용 정책을 위반했다는 메시지를 보냅니다.
...
| **스키마 레이블** | 스키마 수준에서 데이터 거버넌스, 동의 및 액세스 제어에 대한 레이블을 관리합니다. 이렇게 하면 해당 스키마를 사용하는 모든 데이터 세트에 레이블을 전달할 수 있습니다. |
...
다음 비디오는 Experience Platform에서 스키마 및 데이터 세트에 데이터 사용 레이블을 적용하는 방법에 대한 지침을 제공합니다.
