...
마케팅 액션의 예로는 데이터 세트를 서드파티 서비스로 내보내는 작업이 있을 수 있습니다. PII(개인 식별 정보)를 내보낼 수 없다고 선언하는 정책이 있고 &quot;I&quot; 레이블(ID 데이터)이 데이터 세트에 적용된 경우 [!DNL Policy Service] 은 이 데이터 세트를 서드파티 대상으로 내보내는 모든 작업을 방지합니다. 이러한 작업 시도 중 하나가 발생하면 Policy Service에서 데이터 사용 정책을 위반했다는 메시지를 보냅니다.
