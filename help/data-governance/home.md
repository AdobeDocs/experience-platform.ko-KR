...
Adobe Experience Platform 데이터 거버넌스를 통해 고객 데이터를 관리하고 데이터 사용에 적용되는 규정, 제한 사항 및 정책을 준수하도록 합니다. 데이터 거버넌스는 카탈로그 작성, 데이터 계보, 데이터 사용 레이블 지정, 데이터 사용 정책, 마케팅 작업을 위한 데이터 사용 제어 등 다양한 수준에서 Experience Platform 내에서 중요한 역할을 합니다.
...
![4가지 데이터 거버넌스 역할을 전달하는 그래픽으로, 각 역할의 업무에 대한 따옴표를 포함합니다.](./images/overview/roles.png)
...
>[!NOTE]
>
>다음 안내서를 참조하십시오 [지원되는 데이터 사용 레이블](labels/reference.md) 사용 가능한 레이블의 전체 목록 및 각 레이블 유형에 대한 정의입니다.

레이블은 언제든지 적용할 수 있으므로 데이터를 제어하는 방법을 유연하게 선택할 수 있습니다. 우수 사례는에서 데이터를 사용할 수 있게되자마자 데이터가 Experience Platform에 수집될 때 데이터에 레이블을 지정하는 것을 권장합니다 [!DNL Platform].

의 개요 보기 [데이터 사용 레이블](./labels/overview.md) 데이터 거버넌스 규정 준수를 적용하는 데 데이터 사용 레이블을 사용하는 방법에 대한 자세한 내용을 확인하십시오.
...
데이터 사용 레이블이 적용되면 데이터 관리자는 정책 서비스 API 또는 Experience Platform 사용자 인터페이스를 사용하여 정책을 만들 수 있습니다. 데이터 사용 정책 및 마케팅 작업에 대한 자세한 내용은 [정책 개요](./policies/overview.md).
...
| **크로스 사이트 데이터** | 사이트 간 데이터는 여러 사이트의 데이터를 결합한 것입니다. 크로스 사이트 데이터에는 온사이트 및 오프사이트 데이터 또는 여러 오프사이트 소스의 데이터 조합이 포함됩니다. |
...
| **관심 기반 타기팅** | 다음 세 가지 조건이 충족되는 경우 관심사 기반 타깃팅(개인화)이 발생합니다.<br>사이트에서 수집된 데이터는 다음과 같습니다.<br><ul><li>사용자의 관심 사항을 추론하는 데 사용됩니다.</li><li>다른 사이트 또는 앱(오프사이트)과 같은 다른 컨텍스트에서 사용됩니다.</li><li>해당 추론을 기반으로 제공할 콘텐츠 또는 광고를 선택하는 데 사용됩니다.</li></ul> |
...
다음 비디오는 Experience Platform 또는 스키마의 데이터 세트 전체에 데이터 사용 레이블을 적용하는 방법에 대한 지침을 제공합니다.
