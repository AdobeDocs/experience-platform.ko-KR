...
**[!UICONTROL View Sources]** 계정을 Experience Platform에 연결하려면 계정에 대해 **[!UICONTROL Manage Sources]** 및 [!DNL Didomi] 권한이 모두 활성화되어야 합니다. 필요한 권한을 얻으려면 제품 관리자에게 문의하십시오. 자세한 내용은 [액세스 제어 UI 안내서](../../../access-control/ui/overview.md)를 참조하십시오.
...
스키마를 만들려면 Experience Platform UI의 왼쪽 탐색에서 [!UICONTROL Schemas]을(를) 선택하고 **[!UICONTROL Create schema]**&#x200B;을(를) 선택합니다. 그런 다음 스키마 유형으로 **[!UICONTROL Standard]**&#x200B;을(를) 선택한 다음 **[!UICONTROL Manual]**&#x200B;을(를) 선택하여 필드를 수동으로 만듭니다. 스키마에 대한 기본 클래스를 선택하고 스키마 이름을 제공합니다.

생성되면 필수 필드를 추가하여 스키마를 업데이트합니다. 기본 ID 값에 대해 Experience Platform에 알릴 수 있는 필드가 하나 이상 [!UICONTROL Identity] 필드인지 확인하십시오. 마지막으로 데이터를 성공적으로 저장하기 위해 [!UICONTROL Profile] 토글을 활성화해야 합니다.
...
데이터 집합을 만들려면 Experience Platform UI의 왼쪽 탐색에서 [!UICONTROL Datasets]을(를) 선택한 다음 **[!UICONTROL Create dataset]**&#x200B;을(를) 선택하십시오. **[!UICONTROL Create dataset from schema]**&#x200B;을(를) 선택한 다음 새 데이터 세트와 연결할 스키마를 선택합니다.
