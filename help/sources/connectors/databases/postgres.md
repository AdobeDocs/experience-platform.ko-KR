...
[!DNL Experience Platform]은(는) 타사 데이터베이스에서 데이터를 수집하는 기능을 지원합니다. [!DNL Platform]은(는) 관계형, NoSQL 또는 데이터 웨어하우스와 같은 다양한 유형의 데이터베이스에 연결할 수 있습니다. 데이터베이스 공급자에 대한 지원에는 [!DNL PostgreSQL]이(가) 포함됩니다.
...
아래 설명서는 API 또는 사용자 인터페이스를 사용하여 [!DNL PostgreSQL]을(를) [!DNL Platform]에 연결하는 방법에 대한 정보를 제공합니다.

## API를 사용하여 [!DNL PostgreSQL]을(를) [!DNL Platform]에 연결
...
## UI를 사용하여 [!DNL PostgreSQL]을(를) [!DNL Platform]에 연결
