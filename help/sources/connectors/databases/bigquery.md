...
| 자격 증명 | 설명 |
| --- | --- |
| `project` | 프로젝트는 [!DNL Google Cloud]을(를) 포함하여 [!DNL Google BigQuery] 리소스에 대한 기본 수준 조직 엔터티입니다. |
...
| `refreshToken` | 새로 고침 토큰을 사용하면 API에 대한 새 액세스 토큰을 가져올 수 있습니다. 액세스 토큰의 수명은 제한되어 있으며 프로젝트 과정 중에 만료될 수 있습니다. 필요한 경우 새로 고침 토큰을 사용하여 프로젝트에 대한 후속 액세스 토큰을 인증하고 요청할 수 있습니다. 새로 고침 토큰에 다음 [!DNL Google] OAuth 범위가 포함되어 있는지 확인하십시오. <ul><li>`https://www.googleapis.com/auth/bigquery`</li><li>`https://www.googleapis.com/auth/cloud-platform`</li></ul> 이러한 범위를 통해 Experience Platform은 BigQuery 작업을 제출하고 구성된 프로젝트에서 데이터를 읽을 수 있습니다. |
| `largeResultsDataSetId` | (선택 사항) 큰 결과 집합에 대한 지원을 사용하도록 설정하는 데 필요한 미리 만들어진 [!DNL Google BigQuery] 데이터 집합 ID입니다.<ul><li>`largeResultsDataSetId`은(는) 큰 결과 집합에 대한 임시 테이블을 저장하는 데 사용되는 미리 만들어진 [!DNL BigQuery] 데이터 집합을 참조해야 합니다.</li><li>값은 프로젝트 정규화된 이름이 아닌 데이터 세트 ID(예: `marketing_temp_results`)만 포함해야 합니다(`my-project.marketing_temp_results` 사용 안 함).</li><li>`largeResultsDataSetId`에 지정된 데이터 집합의 위치(지역)가 쿼리되는 테이블의 위치와 일치해야 합니다.</li><li>커넥터가 사용하는 계정에는 이 데이터 세트에 임시 결과를 읽고 쓸 수 있는 권한이 있어야 합니다. 최소한 [!DNL BigQuery Data Editor]에 지정된 데이터 집합에서 `largeResultsDataSetId` 역할을 할당하십시오.</li></ul> |

#### [!DNL Google] ID에 필요한 IAM 역할

OAuth 자격 증명(클라이언트 ID, 클라이언트 암호 및 refreshToken)을 생성하는 데 사용되는 [!DNL Google] ID는 대상 [!DNL Google Cloud] 프로젝트에서 다음 IAM 역할을 가져야 합니다.

- [!DNL BigQuery Job User]
- [!DNL BigQuery Data Viewer]
- [!DNL BigQuery Read Session User]

이러한 역할을 통해 Experience Platform은 [!DNL BigQuery] 작업을 만들고 실행하고, 구성된 테이블에서 데이터를 읽고, 커넥터의 필요에 따라 읽기 세션을 사용할 수 있습니다. 소스와 함께 사용할 [!DNL BigQuery] 데이터 세트가 포함된 동일한 프로젝트에서 이러한 역할이 부여되었는지 확인하십시오.
