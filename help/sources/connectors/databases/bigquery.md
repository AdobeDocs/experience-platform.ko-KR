...
[!DNL Experience Platform]은(는) 타사 데이터베이스에서 데이터를 수집하는 기능을 지원합니다. 플랫폼은 관계형, NoSQL 또는 데이터 웨어하우스와 같은 다양한 유형의 데이터베이스에 연결할 수 있습니다. 데이터베이스 공급자에 대한 지원에는 [!DNL Google BigQuery]이(가) 포함됩니다.
...
### [!DNL Google BigQuery] 자격 증명 생성

[!DNL Google BigQuery]을(를) 플랫폼에 연결하려면 다음 자격 증명에 대한 값을 생성해야 합니다.
