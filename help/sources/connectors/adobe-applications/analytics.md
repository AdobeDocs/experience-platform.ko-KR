...
| AACUSTOMID | AACUSTOMID는 `s.VisitorID` 구현에서의 [!DNL Analytics] 변수 사용에 따라 Adobe Analytics에 채워지는 별도의 식별자 필드입니다. AACUSTOMID는 `cust_visid`데이터 피드[[!DNL Analytics] 의 &#x200B;](https://experienceleague.adobe.com/docs/analytics/export/analytics-data-feed/data-feed-contents/datafeeds-reference.html?lang=ko) 열에 표시됩니다. AACUSTOMID가 있으면 AACUSTOMID가 [ID [!DNL Analytics] 에 대한 작업 순서 &#x200B;](https://experienceleague.adobe.com/docs/id-service/using/reference/analytics-reference/analytics-order-of-operations.html?lang=ko)로 정의된 다른 모든 식별자를 우선하므로 AAID는 AACUSTOMID를 기반으로 합니다. |
