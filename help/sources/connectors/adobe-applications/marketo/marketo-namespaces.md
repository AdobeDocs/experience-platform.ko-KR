...
>
>- 2026년 1월부터 Real-Time CDP B2B edition은 더 이상 B2B 엔터티 간의 **비표준** 관계를 지원하지 않습니다. 따라서 [B2B 네임스페이스 및 스키마 안내서](../../../../rtcdp/schemas/b2b.md)에 요약된 표준 관계를 사용하도록 B2B 엔터티를 업데이트하는 것이 좋습니다.
...
| `TECHNICAL_ACCOUNT_ID` | [Adobe Developer Console](https://developer.adobe.com/)에 통합하는 데 사용되는 자격 증명입니다. 기술 계정 ID를 검색하려면 아래 절차를 따르십시오.<ul><li>[Adobe Developer Console](https://developer.adobe.com/)&#x200B;(으)로 이동하여 프로젝트를 엽니다.</li><li>**서비스 계정(JWT)**&#x200B;을 사용하는 _자격 증명_&#x200B;을 여십시오. **참고**: 이전 UI에서 &quot;엔터프라이즈&quot;라고 할 수 있습니다.</li><li>자격 증명 세부 정보에 클라이언트 ID, 기술 계정 ID, 조직 ID 및 기타 JWT 메타데이터가 표시되어야 합니다.</li></ul>. | `D42AEVJZTTJC6LZADUBVPA15@techacct.adobe.com` |
...
| `PLATFORM_URL` | API 호출을 수행하는 URL 엔드포인트. 이 값은 고정되어 있으며 항상 `platform.adobe.io/`(으)로 설정됩니다. | `platform.adobe.io/` |
