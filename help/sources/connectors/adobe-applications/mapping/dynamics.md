...
이 시나리오에서 값은 키가 옵션에 있는 경우 키에 해당하고, `default`이(가) 있고 키를 찾을 수 없는 경우 `default`에 해당합니다. 옵션이 `null`이거나 `null`이(가) 없고 키를 찾을 수 없는 경우 값은 `default`에 해당합니다.
...
자세한 내용은 [의 논리 연산자에 대한  [!DNL Data Prep]](../../../../data-prep/functions.md##logical-operators)문서를 참조하십시오
