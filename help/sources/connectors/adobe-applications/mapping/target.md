...
Adobe Experience Platform을 사용하면 Target 소스 커넥터를 통해 Adobe Target 데이터를 인제스트할 수 있습니다. 커넥터를 사용하는 경우 Target 필드의 모든 데이터는 XDM ExperienceEvent 클래스와 연관된 [XDM(Experience Data Model)](../../../../xdm/home.md) 필드에 매핑해야 합니다.

다음 표에서는 경험 이벤트 스키마(*XDM ExperienceEvent 필드*)의 필드와 해당 Target 필드(*Target 요청 필드*)에 대해 간략히 설명합니다. 일부 매핑에 대한 추가 메모도 제공됩니다.
...
| `experience.target.activities[i].activityID` | 방문자가 자격이 있는 특정 활동의 ID |
...
| `placeContext.geo.city` | 요청의 IP 주소를 기반으로 해결된 도시 이름. |
| `placeContext.geo.countryCode` | 요청의 IP 주소를 기반으로 국가 코드가 해결되었습니다. |
| `placeContext.geo.dmaId` | 지정 시장 지역 코드는 요청의 IP 주소를 기반으로 해결되었습니다. |
