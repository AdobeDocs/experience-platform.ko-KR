...
| --------------- | --------- | -------- | ---------- |
| m_evar1 - m_evar250 | _experience.analytics.customDimensions.eVars.eVar1 - _experience.analytics.customDimensions.eVars.eVar250 | string | 1-250부터 범위를 지정할 수 있는 사용자 지정 변수입니다. 각 조직은 이러한 사용자 지정 eVar를 다르게 사용합니다. |
| m_prop1 - m_prop75 | _experience.analytics.customDimensions.prop1 - _experience.analytics.customDimensions.prop75 | string | 사용자 지정 트래픽 변수(1-75 범위)입니다. |
...
| m_campaign | marketing.trackingCode | string | 추적 코드 차원에 사용되는 변수입니다. |
| m_channel | web.webPageDetails.siteSection | string | 사이트 섹션 차원에 사용되는 변수입니다. |
| m_domain | environment.domain | string | 도메인 차원에 사용되는 변수입니다. 사용자의 인터넷 서비스 공급자(ISP)를 기반으로 합니다. |
| m_geo_city | placeContext.geo.city | string | 히트의 도시 이름입니다. 이것은 히트의 IP 주소를 기반으로 합니다. |
| m_geo_dma | placeContext.geo.dmaID | 정수 | 히트에 대한 인구 통계 영역의 숫자 ID입니다. 이것은 히트의 IP 주소를 기반으로 합니다. |
| m_geo_region | placeContext.geo.stateProvince | string | 히트의 주 또는 지역의 이름입니다. 이것은 히트의 IP 주소를 기반으로 합니다. |
| m_geo_zip | placeContext.geo.postalCode | string | 히트의 우편 번호입니다. 이것은 히트의 IP 주소를 기반으로 합니다. |
| m_keywords | search.keywords | string | 키워드 차원에 사용되는 변수입니다. |
| m_os | _experience.analytics.environment.operatingSystemID | 정수 | 방문자의 운영 체제를 나타내는 숫자 ID입니다. user_agent 열을 기반으로 합니다. |
| m_page_url | web.webPageDetails.URL | string | 페이지 히트의 URL입니다. |
| m_pagename_no_url | web.webPageDetails.</span>이름 | string | 페이지 차원을 채우는 데 사용되는 변수입니다. |
| m_referrer | web.webReferrer.URL | string | 이전 페이지의 페이지 URL입니다. |
| m_search_page_num | search.pageDepth | 정수 | 모든 검색 페이지 등급 차원에 사용됩니다. 사용자가 사이트에 클릭 스루하기 전에 사이트가 표시된 검색 결과 페이지를 나타냅니다. |
| m_state | _experience.analytics.customDimensions.statePromity | string | 상태 변수입니다. |
| m_user_server | web.webPageDetails.server | string | 서버 차원에 사용되는 변수입니다. |
| m_zip | _experience.analytics.customDimensions.postalCode | string | 우편 번호 차원을 채우는 데 사용되는 변수입니다. |
| accept_language | environment.browserDetails.acceptLanguage | string | Accept-Language HTTP 헤더에 표시된 대로 모든 수락된 언어를 나열합니다. |
| homepage | web.webPageDetails.isHomePage | 부울 | 더 이상 사용되지 않습니다. 현재 URL이 브라우저의 홈 페이지인지 나타냅니다. |
| ipv6 | environment.ipV6 | string |
| j_jscript | environment.browserDetails.javaScriptVersion | string | 브라우저가 지원하는 JavaScript 버전입니다. |
| user_agent | environment.browserDetails.userAgent | string | HTTP 헤더에서 전송된 사용자 에이전트 문자열입니다. |
| mobileappid | 응용 프로그램.</span>이름 | string | 모바일 앱 ID로서 다음 형식으로 저장됩니다. `[AppName][BundleVersion]`. |
| mobiledevice | device.model | string | 모바일 장치의 이름입니다. iOS에서는 쉼표로 구분된 2자리 문자열로 저장됩니다. 첫 번째 번호는 장치 생성을 나타내고, 두 번째 번호는 장치 제품군을 나타냅니다. |
| pointofinterest | placeContext.POIinteraction.POIDetail.</span>이름 | string | Mobile Services에서 사용됩니다. 관심 영역을 나타냅니다. |
| pointofinterestdistance | placeContext.POIinteraction.POIDetail.geoInteractionDetails.distanceToCenter | number | Mobile Services에서 사용됩니다. 관심 영역을 나타냅니다. |
...
| videocontenttype | media.mediaTimed.primaryAssetViewDetails.broadcastContentType | string | 비디오 컨텐츠 유형입니다. 모든 비디오 보기에 대해 자동으로 &quot;비디오&quot;로 설정됩니다. |
| videoadpod | advertising.adAssetViewDetails.adBreak._ID | string | 비디오 광고가 있는 Pod입니다. |
...
| videochapter | media.mediaTimed.mediaChapter.chapterAssetReference._ID | string | 비디오 장의 이름 |
...
| videoepisode | media.mediaTimed.primaryAssetReference._iptc4xmpExt.Episode._iptc4xmpExt.Name | string | 비디오 에피소드. |
...
| m_page_event_var1 | web.webInteraction.URL | string | 링크 추적 이미지 요청에만 사용되는 변수입니다. 이 변수에는 클릭한 다운로드 링크, 종료 링크 또는 사용자 지정 링크의 URL이 포함되어 있습니다. |
| m_page_event_var2 | web.webInteraction.name | string | 링크 추적 이미지 요청에만 사용되는 변수입니다. 지정된 경우 링크의 사용자 지정 이름이 표시됩니다. |
| m_page_type | web.webPageDetails.isErrorPage | 부울 | 페이지를 찾을 수 없음 차원을 채우는 데 사용되는 변수입니다. 이 변수는 비워 두거나 &quot;ErrorPage&quot;를 포함해야 합니다. |
| m_pagename_no_url | web.webPageDetails.pageViews.value | number | 페이지 이름(설정된 경우)입니다. 페이지가 지정되지 않은 경우 이 값은 비어 있습니다. |
...
| m_ref_type | web.webReferrer.type | string | 히트에 대한 참조 유형을 나타내는 숫자 ID입니다. 1 은 사이트 내부를 의미하고, 2 는 다른 웹 사이트를 의미하며, 3 은 검색 엔진을 의미하고, 4 는 하드 드라이브를 의미하며 5 는 USENET를 의미하며, 6 은 입력/책갈피 표시(레퍼러 없음), 7 은 이메일을 의미하고, 8 은 JavaScript 없음을 의미하며 9 는 소셜 네트워크를 의미합니다. |
| m_search_engine | search.searchEngine | string | 방문자를 사이트로 유도한 검색 엔진을 나타내는 숫자 ID입니다. |
| post_currency | commerce.order.currencyCode | string | 거래 중에 사용된 통화 코드입니다. |
| post_cust_hit_time_gmt | timestamp | string | 타임스탬프가 활성화된 데이터 세트에서만 사용됩니다. Unix 시간을 기반으로 이 타임스탬프와 함께 전송되는 타임스탬프입니다. |
| post_cust_visid | identityMap | 개체 | 고객 방문자 ID입니다. |
| post_cust_visid | endUserIDs._experience.aacustoid.primary | 부울 | 고객 방문자 ID입니다. |
| post_cust_visid | endUserIDs._experience.aacucustomer.namespace.code | string | 고객 방문자 ID입니다. |
| post_visid_high + visid_low | identityMap | 개체 | 방문에 대한 고유 식별자입니다. |
| post_visid_high + visid_low | endUserIDs._experience.aaid.id | string | 방문에 대한 고유 식별자입니다. |
| post_visid_high | endUserIDs._experience.aaid.primary | 부울 | 방문을 고유하게 식별하기 위해 visid_low와 함께 사용됩니다. |
| post_visid_high | endUserIDs._experience.aaid.namespace.code | string | 방문을 고유하게 식별하기 위해 visid_low와 함께 사용됩니다. |
| post_visid_low | identityMap | 개체 | 방문을 고유하게 식별하기 위해 visid_high와 함께 사용됩니다. |
| hit_time_gmt | receivedTimestamp | string | Unix 시간을 기반으로 하는 히트의 타임스탬프입니다. |
| hitid_high + hitid_low | _ID | string | 히트를 식별하는 고유 식별자입니다. |
| hitid_low | _ID | string | 히트를 고유하게 식별하기 위해 hitid_high와 함께 사용됩니다. |
...
| Analytics 필드 | XDM 필드 | XDM 유형 | 설명 |
| --------------- | --------- | -------- | ---------- |
| post_evar1 - post_evar250 | _experience.analytics.customDimensions.eVars.eVar1 - _experience.analytics.customDimensions.eVars.eVar250 | string | 1-250부터 범위를 지정할 수 있는 사용자 지정 변수입니다. 각 조직은 이러한 사용자 지정 eVar를 다르게 사용합니다. |
| post_prop1 - post_prop75 | _experience.analytics.customDimensions.prop1 - _experience.analytics.customDimensions.prop75 | string | 사용자 지정 트래픽 변수(1-75 범위)입니다. |
...
| post_campaign | marketing.trackingCode | string | 추적 코드 차원에 사용되는 변수입니다. |
| post_channel | web.webPageDetails.siteSection | string | 사이트 섹션 차원에 사용되는 변수입니다. |
| post_cust_visid | endUserIDs._experience.aacucustomer.id | string | 설정된 경우 사용자 지정 방문자 ID입니다. |
| post_first_hit_page_url | _experience.analytics.endUser.firstWeb.webPageDetails.URL | string | 방문자가 도달하는 첫 번째 페이지의 URL입니다. |
| post_first_hit_pagename | _experience.analytics.endUser.firstWeb.webPageDetails.name | string | 원래 시작 페이지 차원에 사용되는 변수입니다. 방문자의 시작 페이지의 페이지 이름입니다. |
| post_keywords | search.keywords | string | 히트에 대해 수집된 키워드입니다. |
| post_page_url | web.webPageDetails.URL | string | 페이지 히트의 URL입니다. |
| post_pagename_no_url | web.webPageDetails.name | string | 페이지 차원을 채우는 데 사용되는 변수입니다. |
| post_purchaseid | commerce.order.purchaseID | string | 구매를 고유하게 식별하는 데 사용되는 변수입니다. |
| post_referrer | web.webReferrer.URL | string | 이전 페이지의 URL입니다. |
| post_state | _experience.analytics.customDimensions.statePromity | string | 상태 변수입니다. |
| post_user_server | web.webPageDetails.server | string | 서버 차원에 사용되는 변수입니다. |
| post_zip | _experience.analytics.customDimensions.postalCode | string | 우편 번호 차원을 채우는 데 사용되는 변수입니다. |
| 브라우저 | _experience.analytics.environment.browserID | 정수 | 브라우저의 숫자 ID입니다. |
| 도메인 | environment.domain | string | 도메인 차원에 사용되는 변수입니다. 사용자의 인터넷 서비스 공급자(ISP)를 기반으로 합니다. |
| first_hit_referrer | _experience.analytics.endUser.firstWeb.webReferrer.URL | string | 방문자에 대한 첫 번째 참조 URL입니다. |
| geo_city | placeContext.geo.city | string | 히트의 도시 이름입니다. 이것은 히트의 IP 주소를 기반으로 합니다. |
| geo_dma | placeContext.geo.dmaID | 정수 | 히트에 대한 인구 통계 영역의 숫자 ID입니다. 이것은 히트의 IP 주소를 기반으로 합니다. |
| geo_region | placeContext.geo.stateProvince | string | 히트의 주 또는 지역의 이름입니다. 이것은 히트의 IP 주소를 기반으로 합니다. |
| geo_zip | placeContext.geo.postalCode | string | 히트의 우편 번호입니다. 이것은 히트의 IP 주소를 기반으로 합니다. |
...
| visit_keywords | _experience.analytics.session.search.keywords | string | 검색 키워드 차원에 사용되는 변수입니다. |
...
| post_page_event | web.webInteraction.type | string | 이미지 요청(표준 히트, 다운로드 링크, 종료 링크 또는 사용자 지정 링크 클릭됨)에서 전송된 히트 유형입니다. |
| post_page_event | web.webInteraction.linkClicks.value | number | 이미지 요청(표준 히트, 다운로드 링크, 종료 링크 또는 사용자 지정 링크 클릭됨)에서 전송된 히트 유형입니다. |
| post_page_event_var1 | web.webInteraction.URL | string | 이 변수는 링크 추적 이미지 요청에서만 사용됩니다. 클릭한 다운로드 링크, 종료 링크 또는 사용자 지정 링크의 URL입니다. |
| post_page_event_var2 | web.webInteraction.name | string | 이 변수는 링크 추적 이미지 요청에서만 사용됩니다. 링크의 사용자 지정 이름이 됩니다. |
...
| geo_country | placeContext.geo.countryCode | string | IP를 기반으로 한, 히트가 발생한 국가의 약어입니다. |
...
| visit_ref_type | _experience.analytics.session.web.webReferrer.type | string | 방문의 첫 번째 레퍼러 유형을 나타내는 숫자 ID입니다. |
| visit_search_engine | _experience.analytics.session.search.searchEngine | string | 방문의 첫 번째 검색 엔진에 대한 숫자 ID입니다. |
