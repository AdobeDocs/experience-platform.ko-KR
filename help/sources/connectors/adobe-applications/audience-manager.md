...
| 프로필 데이터 | &lt; 2일 | DCS/PCS Edge 데이터 및 온보드 데이터를 통해 캡처된 후 사용자 프로필로 처리되어 프로필에 나타나는 시간이 여기에 해당합니다. 이 데이터는 오늘날 Platform Data Lake에 직접 포함되지 않습니다. Audience Manager 프로필 데이터 세트에서 이 데이터를 프로필에 직접 인제스트하는 프로필 전환을 활성화할 수 있습니다. |