...
Experience Platform 시 XDM을 사용하는 방법에 대한 자세한 내용은 [XDM 시스템 개요](../../../xdm/home.md)를 참조하십시오. 프로필과 이벤트 간에 XDM 스키마를 구조화하는 방법에 대한 자세한 내용은 [스키마 구성 기본 사항](../../../xdm/schema/composition.md)을 참조하세요.
...
필드가 Audience Manager에서 XDM으로 매핑되는 방법에 대한 자세한 내용은 [Audience Manager 매핑 필드](./mapping/audience-manager.md)의 설명서를 참조하십시오.
...
>[!NOTE]
>
>AAM 실시간 기능은 데이터 레이크로 이동하는 유일한 데이터 세트입니다. 다른 모든 Audience Manager 데이터 세트가 [!DNL Profile]에 대해 활성화된 경우 [!DNL Profile](으)로 이동합니다. [!DNL Profile]에 대해 활성화되지 않은 경우 데이터가 수신되지 않으며 비어 있는 것으로 표시됩니다.
...
| AAM 인증된 프로필 | 이 데이터 세트에는 Audience Manager 인증 프로필이 포함되어 있습니다. 데이터는 데이터 세트에 배치로 표시되지 않습니다. **[!UICONTROL 프로필]** 토글을 활성화하여 데이터를 프로필로 직접 수집할 수 있습니다. | 레코드 |
