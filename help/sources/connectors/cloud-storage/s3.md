---
keywords: Experience Platform;홈;인기 항목;Amazon S3;amazon s3;S3;s3;s3
...
클라우드 스토리지 소스는 다운로드, 형식 지정 또는 업로드할 필요 없이 자신의 데이터를 [!DNL Platform]에 가져올 수 있습니다. 인제스트된 데이터는 XDM JSON, XDM 쪽모이 세공 마루 또는 구분된 형태로 포맷될 수 있습니다. 프로세스의 모든 단계가 소스 워크플로우에 통합됩니다. [!DNL Platform] S3에서 일괄로 데이터를 가져올 수 있습니다.
...
S3를 사용하여 단일 디렉토리를 인제스트하려면 S3 콘솔에서 플랫폼용 [!DNL Identity and Access Management](IAM) 사용자를 만들고 다음 작업에 대한 권한을 할당해야 합니다.

- `s3:GetObject`
- `s3:GetObjectVersion`

연결 확인 및 테스트에는 다음 권한도 필요합니다.

- `s3:ListAllMyBuckets`
- `s3:ListBucket`
- `s3:GetBucketLocation`

`myBucket/folder/subfolder/subsubfolder/abc.csv` 같은 파일 경로를 사용하면 `subsubfolder/abc.csv`만 액세스할 수 있습니다. 하위 폴더에 액세스하려면 S3 콘솔에 `bucket` 매개 변수를 `myBucket`, `folderPath`를 `folder/subfolder`으로 지정하여 `subsubfolder/abc.csv`와 대조적으로 `subfolder`에서 파일 탐색을 시작할 수 있습니다.

## S3을 [!DNL Platform]에 연결

아래 설명서는 API 또는 사용자 인터페이스를 사용하여 S3를 Adobe Experience Platform에 연결하는 방법에 대한 정보를 제공합니다.

### API 사용

- [Flow Service API를 사용하여 S3 소스 연결 만들기](../../tutorials/api/create/cloud-storage/s3.md)
