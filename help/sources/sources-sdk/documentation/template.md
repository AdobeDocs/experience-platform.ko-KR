...
| `{SOURCE_PARAMS}` | Experience Platform으로 가져올 소스 파일의 매개 변수를 정의합니다. `{SOURCE_PARAMS}`에 대해 허용되는 형식 유형을 검색하려면 base64에서 전체 `list_id` 문자열을 인코딩해야 합니다. 아래 예에서 base64로 인코딩된 `"list_id": "10c097ca71"`은(는) `eyJsaXN0SWQiOiIxMGMwOTdjYTcxIn0=`과(와) 같습니다. |
