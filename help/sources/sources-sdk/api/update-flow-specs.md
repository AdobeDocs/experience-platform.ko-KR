...
를 사용하여 만든 소스 `generic-rest-extension` 템플릿 모두 사용 `RestStorageToAEP` 흐름 사양. 이 흐름 사양은 `/flowSpecs/` 엔드포인트 및 제공 `flowSpec.id` 의 `6499120c-0b15-42dc-936e-847ea3c24d72`.
...
다음 요청은 를 검색합니다. `6499120c-0b15-42dc-936e-847ea3c24d72` 연결 사양입니다.
...
PUT 작업을 통해 연결 사양의 필드를 업데이트할 수 있습니다. PUT 요청을 통해 연결 사양을 업데이트할 때 본문에는 POST 요청에 새 연결 사양을 생성할 때 필요한 모든 필드가 포함되어야 합니다.

>[!IMPORTANT]
>
>다음 목록을 업데이트해야 합니다. `sourceConnectionSpecIds` 새 소스가 생성될 때마다 새 소스에 해당하는 흐름 사양입니다. 이렇게 하면 기존 흐름 사양이 새 소스를 지원하므로 새 소스로 데이터 흐름 만들기 프로세스를 완료할 수 있습니다.
...
다음 요청은 플로우 사양을 업데이트합니다. `6499120c-0b15-42dc-936e-847ea3c24d72` 연결 사양 ID 포함 `f6c0de0c-0a42-4cd9-9139-8768bf2f1b55`.
