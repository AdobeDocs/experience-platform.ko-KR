...
다음 문서에서는 다음을 사용하여 연결 사양을 생성하는 방법에 대해 설명합니다. [!DNL Flow Service] API를 실행하고 셀프서비스 소스(Streaming SDK)를 통해 새 소스를 통합합니다.
...
| {your_source}-category.txt | 텍스트 파일 형식의 소스가 속한 범주입니다. **참고**: 소스가 상기 범주 중 하나에 맞지 않는다고 생각되면 Adobe 담당자에게 문의하여 논의하십시오. | `medallia-category.txt` 파일 내부에 다음과 같이 소스의 카테고리를 지정하십시오. `streaming`. |
| {your_source}-description.txt | 소스에 대한 간략한 설명. | [!DNL Medallia] 는 가져올 때 사용할 수 있는 마케팅 자동화 소스입니다. [!DNL Medallia] Experience Platform 대상 데이터. |
