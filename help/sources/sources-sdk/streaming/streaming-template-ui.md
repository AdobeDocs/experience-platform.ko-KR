...
# 소스 연결 및 스트리밍할 데이터 흐름 만들기 *소스* UI를 사용하는 데이터
...
아래 **스트리밍** 범주, 선택 *소스*&#x200B;을 선택한 다음 을 선택합니다 **[!UICONTROL 데이터 추가]**.

>[!TIP]
>
>아래에 사용된 스크린샷은 예입니다. 설명서를 만들 때 이미지를 실제 소스의 스크린샷으로 바꾸십시오. 동일한 파일 이름뿐만 아니라 동일한 마크업 패턴 및 색상을 사용할 수 있습니다. 스크린샷에 전체 Platform UI 화면이 캡처되는지 확인하십시오. 스크린샷을 업로드하는 방법에 대한 자세한 내용은 의 안내서를 참조하십시오. [검토를 위해 설명서 제출](../documentation/github.md).
...
![소스 워크플로우의 미리보기 단계입니다.](../assets/streaming/preview.png)
...
![소스 워크플로우의 데이터 흐름 세부 단계.](../assets/streaming/dataflow-detail.png)
