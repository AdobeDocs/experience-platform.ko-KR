...
[[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/) 는 내의 다양한 개별 소스에서 고객 데이터를 수집하고 중앙 집중화하는 데 사용됩니다 [!DNL Platform]. 이 서비스는 지원되는 모든 소스가 연결 가능한 사용자 인터페이스와 RESTful API를 제공합니다.
...
이 자습서에서는 모니터링하려는 흐름이 실행되는 소스 연결을 이미 하나 이상 만들었다고 가정합니다. 아직 소스 연결을 구성하지 않은 경우 다음을 방문하여 시작하십시오. [소스 개요](./home.md) 이 안내서로 돌아가기 전에 선택한 소스를 구성합니다.

또한 이 문서에서는 웹후크 및 웹후크를 한 응용 프로그램에서 다른 응용 프로그램으로 연결하는 방법에 대해 작업해야 합니다. 다음을 참조하십시오. [[!DNL I/O Events] 설명서](https://www.adobe.io/apis/experienceplatform/events/docs.html#!adobedocs/adobeio-events/master/intro/webhook_docs_intro.md) webhooks를 소개합니다.

## 플로우 실행 알림에 대한 웹후크 등록

플로우 실행 알림을 수신하려면 Adobe Developer 콘솔을 사용하여 웹 후크를 [!DNL Experience Platform] 통합.

다음에 대한 자습서를 따르십시오 [[!DNL I/O Event] 알림 구독](../observability/alerts/subscribe.md) 을 수행하는 방법에 대한 자세한 단계입니다.

>[!IMPORTANT]
>
>가입 프로세스 중에 다음을 선택해야 합니다. **[!UICONTROL Platform 알림]** 를 이벤트 공급자로 설정하고 다음 이벤트 구독을 선택합니다.
>
...
알림은 실행된 수집 작업 수, 파일 크기 및 오류 등의 정보를 반환합니다. 알림은 플로우 실행과 연결된 페이로드도 JSON 형식으로 반환합니다. 응답 페이로드는 다음으로 분류될 수 있습니다. `sources_flow_run_success` 또는 `sources_flow_run_failure`.

>[!IMPORTANT]
>
>플로우 생성 프로세스 중에 부분 수집을 활성화하면 성공한 수집과 실패한 수집이 모두 포함된 플로우가 로 표시됩니다. `sources_flow_run_success` 오류 수가 플로우 생성 프로세스 중에 설정된 오류 임계값 퍼센트 미만인 경우에만 해당됩니다. 성공한 흐름 실행에 오류가 포함되어 있으면 이러한 오류는 여전히 반환 페이로드의 일부로 포함됩니다.

### 성공

성공적인 응답은 다음 집합을 반환합니다. `metrics` 특정 플로우 실행의 특성을 정의합니다. `activities` 데이터가 변환되는 방식을 간략하게 설명합니다.
...
>[!NOTE]
>
>다음을 참조하십시오. [부록](#errors) 오류 메시지에 대한 자세한 정보.

## 다음 단계

이제 플로우 실행 상태에 대한 실시간 알림을 받을 수 있는 이벤트를 구독할 수 있습니다. 흐름 실행 및 소스에 대한 자세한 내용은 [소스 개요](./home.md).
...
수집 오류는 소스에서 데이터를 복사할 때나 복사된 데이터를에 처리할 때 발생할 수 있습니다. [!DNL Platform]. 특정 오류에 대한 자세한 내용은 아래 표를 참조하십시오.

| 오류 | 설명 |
| ---------- | ----------- |
| `CONNECTOR-1001-500` | 소스에서 데이터를 복사하는 동안 오류가 발생했습니다. |
| `CONNECTOR-2001-500` | 복사된 데이터를에 처리하는 동안 오류가 발생했습니다. [!DNL Platform]. 이 오류는 구문 분석, 유효성 검사 또는 변환과 관련된 것일 수 있습니다. |
