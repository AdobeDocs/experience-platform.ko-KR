...
topic: overview
type: Tutorial
description: 이 자습서에서는 플랫폼 사용자 인터페이스를 사용하여 PayPal 소스 커넥터를 만드는 단계를 제공합니다.
...
이미 유효한 [!DNL PayPal] 연결이 있는 경우 이 문서의 나머지 부분을 건너뛰고 데이터 흐름 [구성에 대한 자습서로 진행할 수 있습니다](../../dataflow/payments.md)
...
시작하는 방법에 대한 자세한 내용은 이 [[!DNL PayPal] 문서를 참조하십시오](https://developer.paypal.com/docs/api/overview/#get-credentials)

## 계정 [!DNL PayPal] 연결

필요한 자격 증명을 수집했으면 아래 절차에 따라 [!DNL PayPal] 계정을 연결할 수 있습니다 [!DNL Platform].
...
지급 **[!UICONTROL 카테고리]** 아래에서 **[!UICONTROL PayPal을 선택합니다]**. 이 커넥터를 처음 사용하는 경우 구성을 **[!UICONTROL 선택합니다]**. 그렇지 않으면 데이터 **[!UICONTROL 추가]** 를 선택하여 새 [!DNL PayPal] 커넥터를 만듭니다.
...
새 자격 증명을 사용 중인 경우 **[!UICONTROL 새 계정을 선택합니다]**. 표시되는 입력 양식에서 이름, 선택적 설명 및 자격 증명을 [!DNL PayPal] 제공합니다. 완료되면 **[!UICONTROL Connect를]** 선택한 다음 새 연결이 설정될 때까지 잠시 기다립니다.
...
이 튜토리얼을 따라 계정에 대한 연결을 설정해 [!DNL PayPal] 드렸습니다. 이제 다음 튜토리얼로 계속 이동하여 데이터 흐름을 [구성하여 결제 데이터를 가져올 수 있습니다 [!DNL Platform]](../../dataflow/payments.md).