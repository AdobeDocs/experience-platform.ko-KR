...
Adobe Experience Platform의 소스 커넥터는 외부에서 가져온 데이터를 예약된 기준으로 인제스트하는 기능을 제공합니다. 이 자습서에서는 Platform 사용자 인터페이스를 사용하여 ServiceNow 소스 커넥터를 만드는 단계를 제공합니다.
...
이미 ServiceNow 연결이 있는 경우 이 문서의 나머지 부분은 건너뛰고 데이터 흐름 [구성에 대한 자습서로 진행할 수 있습니다](../../dataflow/customer-success.md)

### 필요한 자격 증명 수집

Platform에서 ServiceNow 계정에 액세스하려면 다음 값을 제공해야 합니다.
...
필요한 자격 증명을 수집했으면 아래 절차에 따라 Platform에 연결할 새 ServiceNow 계정을 만들 수 있습니다.

Adobe Experience Platform에 <a href="https://platform.adobe.com" target="_blank">로그인한</a> 다음 왼쪽 탐색 **모음에서** 소스 *를 선택하여* 소스 작업 영역에액세스합니다. [ *카탈로그* ] 화면에는 계정을 만들 수 있는 다양한 소스가 표시되고, 각 소스에는 계정과 연결된 기존 데이터 세트 흐름 수가 표시됩니다.

화면의 왼쪽에 있는 카탈로그에서 해당 범주를 선택할 수 있습니다. 또는 검색 옵션을 사용하여 작업할 특정 소스를 찾을 수 있습니다.

[ *고객 성공* ] 카테고리 아래에서 **ServiceNow** 를 선택하여 화면 오른쪽에 정보 막대를 표시합니다. 정보 표시줄에는 선택한 소스에 대한 간단한 설명과 소스와 연결하거나 설명서를 보는 옵션이 제공됩니다. 새 계정을 만들려면 **Connect 소스를 선택합니다**.
...
이 튜토리얼을 따라 ServiceNow 계정에 대한 연결을 설정했습니다. 이제 다음 튜토리얼로 계속 이동하여 데이터를 Platform으로 가져오도록 [데이터 흐름을 구성할 수 있습니다](../../dataflow/customer-success.md).
