...
>[!NOTE]
>ServiceNow 커넥터가 베타 버전입니다. 기능 및 설명서는 변경될 수 있습니다.

Adobe Experience Platform의 소스 커넥터는 외부에서 소스 데이터를 예약된 기준으로 인제스트하는 기능을 제공합니다. 이 자습서에서는 플랫폼 사용자 인터페이스를 사용하여 ServiceNow 소스 커넥터를 만드는 단계를 제공합니다.
...
필요한 자격 증명을 수집했으면 아래 절차에 따라 ServiceNow 계정을 만들어 Platform에 연결할 수 있습니다.

Adobe <a href="https://platform.adobe.com" target="_blank">Experience Platform에</a> 로그인한 다음 왼쪽 탐색 **모음에서** Sources를 *선택하여* Sources 작업 영역에액세스합니다. [ *카탈로그* ] 화면에는 계정을 만들 수 있는 다양한 소스가 표시되고, 각 소스에는 계정과 연결된 기존 데이터 세트 흐름 수가 표시됩니다.
...
이 튜토리얼을 따라 ServiceNow 계정에 대한 연결을 설정했습니다. 이제 다음 튜토리얼로 계속 진행하여 데이터 흐름을 [구성하여 Platform으로 데이터를 가져올 수 있습니다](../../dataflow/customer-success.md).
