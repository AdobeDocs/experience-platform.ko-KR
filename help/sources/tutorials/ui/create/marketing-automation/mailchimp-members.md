...
# 플랫폼 UI를 사용하여 [!DNL Mailchimp Members] 소스 연결 만들기

이 자습서에서는 사용자 인터페이스를 사용하여 [!DNL Mailchimp Members] 데이터를 Adobe Experience Platform으로 수집하기 위한 [!DNL Mailchimp] 소스 커넥터를 만드는 단계를 제공합니다.
...
[!DNL Mailchimp Members] 데이터를 플랫폼으로 가져오려면 먼저 [!DNL Mailchimp] 계정에 해당하는 적절한 인증 자격 증명을 제공해야 합니다.

[!DNL Mailchimp Members] 원본은 OAuth 2 새로 고침 코드와 기본 인증을 모두 지원합니다. 이러한 인증 유형에 대한 자세한 내용은 아래 표를 참조하십시오.
...
| 인증 테스트 URL | 인증 테스트 URL은 [!DNL Mailchimp]을(를) 플랫폼에 연결할 때 자격 증명의 유효성을 검사하는 데 사용됩니다. 제공되지 않으면 소스 연결 생성 단계에서 자격 증명을 자동으로 확인합니다. |
| 액세스 토큰 | 소스 인증에 사용되는 해당 액세스 토큰입니다. OAuth 기반 인증에 필요합니다. |

OAuth 2를 사용하여 [!DNL Mailchimp] 계정을 Platform에 인증하는 방법에 대한 자세한 내용은 이 [[!DNL Mailchimp] OAuth 2 사용에 대한 문서](https://mailchimp.com/developer/marketing/guides/access-user-data-oauth-2/)를 참조하십시오.
...
## [!DNL Mailchimp Members] 계정을 플랫폼에 연결
...
기존 계정을 사용하려면 새 데이터 흐름을 만들 [!DNL Mailchimp Members] 계정을 선택한 다음 **[!UICONTROL 다음]**&#x200B;을 선택하여 계속합니다.

![기존](../../../../images/tutorials/create/mailchimp-members/existing.png)

### 새 계정

새 계정을 만드는 경우 **[!UICONTROL 새 계정]**&#x200B;을(를) 선택한 다음 [!DNL Mailchimp Members] 원본 연결 세부 정보에 대한 이름과 설명을 입력하십시오.

![새로 만들기](../../../../images/tutorials/create/mailchimp-members/new.png)
...
### [!DNL Mailchimp Members]개 데이터 선택

소스가 인증되면 [!DNL Mailchimp Members] 계정에 해당하는 `listId`을(를) 제공해야 합니다.

[!UICONTROL 데이터 선택] 페이지에서 `listId`을(를) 입력한 다음 **[!UICONTROL 탐색]**&#x200B;을(를) 선택하십시오.
...
[!DNL Mailchimp] 계정이 인증되고 [!DNL Mailchimp Members] 데이터가 선택되면 이제 데이터 흐름을 만들어 데이터를 플랫폼으로 가져올 수 있습니다. 데이터 흐름을 만드는 방법에 대한 자세한 단계는 [마케팅 자동화 데이터를 플랫폼으로 가져오기 위한 데이터 흐름 만들기](../../dataflow/marketing-automation.md)에 대한 설명서를 참조하십시오.
