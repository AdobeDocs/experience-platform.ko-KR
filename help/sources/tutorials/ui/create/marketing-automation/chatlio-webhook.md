...
### 소스 스키마를 정의할 샘플 JSON [!DNL Chatlio] {#prerequisites-json-schema}

만들기 전 [!DNL Chatlio] 소스 연결에서 소스 스키마를 제공해야 합니다. 아래 JSON을 사용할 수 있습니다.
...
### 에 대한 플랫폼 스키마 만들기 [!DNL Chatlio] {#create-platform-schema}

또한 소스에 사용할 Platform 스키마를 생성해야 합니다. 다음에 대한 자습서 읽기: [플랫폼 스키마 생성](../../../../../xdm/schema/composition.md) 스키마를 만드는 방법에 대한 포괄적인 단계를 설명합니다.
...
로 이동 [!UICONTROL 마케팅 자동화] 범주 보기 [!DNL Chatlio] 소스 카드. 시작하려면 다음을 선택합니다. **[!UICONTROL 데이터 추가]**.

![Chatlio 카드가 포함된 Platform UI 카탈로그](../../../../images/tutorials/create/marketing-automation/chatlio-webhook/catalog.png)
...
![소스 워크플로우의 미리보기 단계입니다.](../../../../images/tutorials/create/marketing-automation/chatlio-webhook/preview.png)
...
![소스 워크플로우의 데이터 흐름 세부 단계.](../../../../images/tutorials/create/marketing-automation/chatlio-webhook/dataflow-detail.png)
...
웹후크를 구성하는 데 사용되는 URL을 구성하기 위해 [!DNL Chatlio] 다음을 검색해야 합니다.
...
![데이터 흐름 활동의 스트리밍 종단점입니다.](../../../../images/tutorials/create/marketing-automation/chatlio-webhook/endpoint-test.png)

스트리밍 끝점 및 데이터 흐름 ID를 검색했으면 다음 패턴을 기반으로 URL을 빌드합니다. ```{STREAMING_ENDPOINT}?x-adobe-flow-id={DATAFLOW_ID}```. 예를 들어 구성된 웹후크 URL은 다음과 같을 수 있습니다. ``https://dcs.adobedc.net/collection/d56b47ee3985104beaf724efcd78a3e1a863d720471a482bebac0acc1ab95983``

## 에서 웹후크 설정 [!DNL Chatlio] {#set-up-webhook}

웹후크 URL이 생성되면 이제 다음을 사용하여 웹후크를 설정할 수 있습니다. [!DNL Chatlio] 사용자 인터페이스.

에 로그인 [[!DNL Chatlio]](https://chatlio.com/) 계정 및 팔로우 [설치 및 설치에 대한 안내서](https://chatlio.com/docs/setup/) 위젯을 만듭니다.

위젯이 생성되면 위젯의 설정 페이지로 이동하여 해당 위젯에 Webhook URL을 추가합니다.

![Chatlio의 웹후크 설정 페이지.](../../../../images/tutorials/create/marketing-automation/chatlio-webhook/widget-settings.png)

그런 다음 **[!DNL Behavior]** 을(를) 탭하고에 웹후크 URL을 추가합니다. *[!DNL Webhook when a new conversation starts]* 필드 및 구독하려는 다른 웹후크 이벤트 필드.

![웹후크 엔드포인트 필드를 표시하는 Chatlio UI.](../../../../images/tutorials/create/marketing-automation/chatlio-webhook/webhook.png)

>[!TIP]
>
>에 대해 다양한 이벤트를 구독할 수 있습니다. [!DNL Chatlio] webhook. 다른 이벤트에 대한 자세한 내용은 [[!DNL Chatlio] 이벤트 설명서](https://chatlio.com/docs/webhooks/).

## 다음 단계 {#next-steps}

이 자습서에 따라 스트리밍 데이터 흐름을 구성하여 [!DNL Chatlio] Experience Platform 대상 데이터. 수집 중인 데이터를 모니터링하려면 의 안내서를 참조하십시오. [Platform UI를 사용하여 스트리밍 데이터 흐름 모니터링](../../monitor-streaming.md).

## 추가 리소스 {#additional-resources}

아래 섹션에서는 사용 시 참조할 수 있는 추가 리소스를 제공합니다. [!DNL Chatlio] 소스.

### 유효성 검사 {#validation}

소스 및 를 올바르게 설정했는지 확인하려면 [!DNL Chatlio] 메시지가 수집되는 경우 아래 단계를 따르십시오.

* 다음을 확인할 수 있습니다. [!DNL Chatlio] **[!UICONTROL 보고서]** > **[!UICONTROL 채팅 기록]** 캡처하고 있는 이벤트를 식별하는 페이지 [!DNL Chatlio].

![채팅 내역을 표시하는 Chatlio UI 스크린샷](../../../../images/tutorials/create/marketing-automation/chatlio-webhook/chatlio-chat-history.png)

* Platform UI에서 를 선택합니다. **[!UICONTROL 데이터 흐름 보기]** 의 옆에 [!DNL Chatlio] 소스 카탈로그의 카드 메뉴. 그런 다음 을 선택합니다. **[!UICONTROL 데이터 세트 미리 보기]** 내에서 구성한 웹후크에 대해 수집된 데이터를 확인합니다. [!DNL Chatlio].

![수집된 이벤트를 보여주는 플랫폼 UI 스크린샷](../../../../images/tutorials/create/marketing-automation/chatlio-webhook/platform-dataset.png)

에 대한 추가 정보를 위해 [!DNL Chatlio], 다음 방문: [[!DNL Chatlio] 설명서](https://chatlio.com/docs/) 및 [FAQ](https://chatlio.com/pricing/#FAQ).
