...
### [!DNL Customer.io]에 대한 플랫폼 스키마 만들기 {#create-platform-schema}

또한 소스에 사용할 Platform 스키마를 생성해야 합니다. 스키마를 만드는 방법에 대한 포괄적인 단계를 보려면 [플랫폼 스키마 만들기](../../../../../xdm/schema/composition.md)에 대한 자습서를 참조하십시오.

![Customer.io에 대한 예제 스키마를 보여 주는 Platform UI 스크린샷](../../../../images/tutorials/create/marketing-automation/customerio-webhook/schema.png)
...
>[!TIP]
>
>보고 웹후크에 대해 다양한 이벤트를 구독할 수 있습니다. [!DNL Customer.io] 작업 이벤트 트리거 기준이 충족되면 각 이벤트의 메시지가 플랫폼으로 수집됩니다. 다른 이벤트에 대한 자세한 내용은 [[!DNL Customer.io] 이벤트 설명서](https://customer.io/docs/webhooks/#events)를 참조하세요.

## 다음 단계 {#next-steps}

이 자습서를 따라 [!DNL Customer.io] 데이터를 Experience Platform 상태로 전환하기 위한 스트리밍 데이터 흐름을 구성했습니다. 수집 중인 데이터를 모니터링하려면 [플랫폼 UI를 사용하여 스트리밍 데이터 흐름 모니터링](../../monitor-streaming.md)에 대한 안내서를 참조하세요.
...
* Platform UI에서 소스 카탈로그의 [!DNL Customer.io] 카드 메뉴 옆에 있는 **[!UICONTROL 데이터 흐름 보기]**&#x200B;를 선택합니다. 그런 다음 **[!UICONTROL 데이터 집합 미리 보기]**&#x200B;를 선택하여 [!DNL Customer.io] 내에서 선택한 이벤트에 대해 수집된 데이터를 확인합니다.
