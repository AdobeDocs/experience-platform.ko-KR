...
# UI를 사용하여 [!DNL Talon.One] 데이터를 Experience Platform에 스트리밍

>[!AVAILABILITY]
>
>[!DNL Talon.One] 원본이 Beta 버전입니다. 베타 레이블 소스를 사용하는 방법에 대한 자세한 내용은 소스 개요에서 [약관](../../../../home.md#terms-and-conditions)을 참조하십시오.

UI의 소스 작업 영역을 사용하여 [!DNL Talon.One]에서 Adobe Experience Platform으로 데이터를 연결하고 스트리밍하는 방법에 대해 알아보려면 이 안내서를 참조하십시오.
...
[!DNL Talon.One]에서 데이터를 스트리밍하려면 **[!UICONTROL Talon.One Streaming Events]**&#x200B;에서 *[!UICONTROL Loyalty]* 원본 카드를 선택한 다음 **[!UICONTROL Add data]**&#x200B;을(를) 선택하십시오.

>[!TIP]
>
>지정된 소스에 아직 인증된 계정이 없는 경우 소스 카탈로그의 소스에 **[!UICONTROL Set up]** 옵션이 표시됩니다. 인증된 계정이 만들어지면 이 옵션이 **[!UICONTROL Add data]**(으)로 변경됩니다.

![Talon.One 스트리밍 이벤트 카드가 선택된 UI의 소스 카탈로그입니다.](../../../../images/tutorials/create/talon-one-streaming/catalog.png)

## 데이터 선택

그런 다음 *[!UICONTROL Select data]* 인터페이스를 사용하여 샘플 JSON 파일을 업로드하여 소스 스키마를 정의합니다. 이 단계에서 미리보기 인터페이스를 사용하여 페이로드의 파일 구조를 볼 수 있습니다. 완료되면 **[!UICONTROL Next]**&#x200B;을(를) 선택합니다.

![원본 워크플로의 데이터 선택 단계](../../../../images/tutorials/create/talon-one-streaming/select-data.png)
...
![데이터 흐름 세부 정보 인터페이스](../../../../images/tutorials/create/talon-one-streaming/dataflow-details.png)
...
![Talon.One 스트리밍에 대한 매핑 인터페이스입니다.](../../../../images/tutorials/create/talon-one-streaming/mapping.png)
...
![소스 워크플로의 검토 단계입니다.](../../../../images/tutorials/create/talon-one-streaming/review.png)

## 스트리밍 끝점 URL 검색

연결이 생성되면 소스 세부 정보 페이지가 나타납니다. 이 페이지에는 이전에 실행한 데이터 흐름, ID 및 스트리밍 끝점 URL을 포함하여 새로 생성된 연결의 세부 정보가 표시됩니다.

![스트리밍 끝점 URL입니다.](../../../../images/tutorials/create/talon-one-streaming/streaming-endpoint.png)

## 데이터 흐름 모니터링

데이터 흐름이 만들어지면 데이터 흐름을 통해 수집되는 데이터를 모니터링하여 수집 비율, 성공 및 오류에 대한 정보를 볼 수 있습니다. 데이터 흐름을 모니터링하는 방법에 대한 자세한 내용은 [UI에서 계정 및 데이터 흐름 모니터링](../../monitor-streaming.md)에 대한 자습서를 참조하십시오.

## 알려진 제한 사항

정확한 데이터 수집을 보장하려면 [!DNL Talon.One]의 충성도 포인트 변경, 계층 업그레이드 및 계층 다운그레이드 알림의 데이터를 커넥터로 보내야 합니다. 충성도 포인트 변경 알림에는 계층 정보가 포함되어 있지 않으므로 이러한 알림을 별도의 프로필 데이터 세트로 보내야 합니다. 포인트 변경 데이터를 동일한 데이터 세트에 계층 업그레이드 또는 다운그레이드 알림과 결합하면 계층 정보가 손실되거나 null 값으로 덮어쓰여집니다. 계층 업그레이드 및 다운그레이드 알림은 둘 다 계층 세부 사항을 포함하므로 동일한 데이터 세트를 사용할 수 있습니다. 수집 후 프로필 병합 규칙 은 가장 최근의 포인트 및 계층 정보를 반영하도록 병합된 프로필을 자동으로 업데이트합니다.