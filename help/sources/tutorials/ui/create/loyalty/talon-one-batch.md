...
일괄 처리 데이터의 [!DNL Talon.One] 원본이 [!DNL Talon.One] 응용 프로그램에 대해 기록된 이벤트를 받습니다. 일반적인 사용 사례는 획득한 충성도 포인트, 상환된 쿠폰, 할인된 할인 등을 포함하여 완료된 거래를 받도록 `talon_session_closed`을(를) 필터링하는 것입니다. API에 대한 자세한 내용은 [[!DNL Talon.One] 관리 API 참조](https://docs.talon.one/management-api#tag/Customer-data/operation/getApplicationEventsWithoutTotalCount)를 참조하십시오.

이 자습서를 따라 UI의 소스 작업 영역을 사용하여 [!DNL Talon.One] 계정의 일괄 처리 데이터를 Adobe Experience Platform으로 수집하는 방법을 알아보십시오.
...
>[!TIP]
>
>새 계정을 만들기 전에 [&#x200B; 개요에서 &#x200B;](../../../../connectors/loyalty/talon-one.md#prerequisites)필수 구성 요소[!DNL Talon.One]를 검토하여 필요한 인증 자격 증명을 알아보십시오.
...
>[!TIP]
>
>획득 또는 상환된 로열티 포인트, 리플레이팅된 쿠폰 및 할인된 할인 등 완료된 거래를 캡처하려면 `talon_session_closed` 이벤트 유형을 선택합니다. 유형별 이벤트 필터링에 대한 자세한 내용은[API 설명서 [!DNL Talon.One] 에서 &#x200B;](https://docs.talon.one/management-api#tag/Customer-data/operation/getApplicationEventsWithoutTotalCount)&quot;type&quot; 쿼리 매개 변수를 참조하십시오.
