...
title: Experience Platform UI를 사용하여 Adobe Campaign Managed Cloud Services 소스 연결 만들기
description: Experience Platform UI를 사용하여 Adobe Experience Platform을 Adobe Campaign Managed Cloud Services에 연결하는 방법을 알아봅니다.
...
## Adobe Campaign Managed Cloud Services을 Experience Platform에 연결

Experience Platform UI의 왼쪽 탐색에서 **[!UICONTROL Sources]**&#x200B;을(를) 선택하여 [!UICONTROL Sources] 작업 영역에 액세스합니다. [!UICONTROL Catalog] 화면에는 계정을 만들 수 있는 다양한 소스가 표시됩니다.

화면 왼쪽에 있는 카탈로그에서 적절한 카테고리를 선택할 수 있습니다. 검색 창을 사용하여 표시되는 소스의 범위를 좁힐 수도 있습니다.

**[!UICONTROL Adobe applications]** 범주 아래에서 **[!UICONTROL Adobe Campaign Managed Cloud Services]**&#x200B;을(를) 선택한 다음 **[!UICONTROL Add data]**&#x200B;을(를) 선택합니다.
...
[!UICONTROL Select data], [!UICONTROL Adobe Campaign instance] 및 [!UICONTROL Target mapping]을(를) 구성할 수 있는 인터페이스를 제공하는 [!UICONTROL Schema name] 단계가 나타납니다.
...
| 스키마 이름 | Experience Platform으로 가져오는 스키마 엔티티의 이름입니다. 옵션에는 게재 로그 및 추적 로그가 포함됩니다. |

![Adobe Campaign 인스턴스, 대상 매핑 및 스키마 이름을 구성할 수 있는 인터페이스입니다.](../../../../images/tutorials/create/campaign/select-data.png)

Campaign 인스턴스, 대상 매핑 및 스키마 이름에 대한 값을 제공하면 화면이 업데이트되어 스키마 미리보기와 샘플 데이터 세트가 표시됩니다. 완료되면 **[!UICONTROL Next]**&#x200B;을(를) 선택합니다.

![스키마 계층 구조 및 데이터 집합 샘플을 미리 봅니다](../../../../images/tutorials/create/campaign/preview.png)

### 기존 데이터 세트 사용

[!UICONTROL Dataflow detail] 페이지에서 기존 데이터 집합을 사용할지 또는 데이터 흐름에 대한 새 데이터 집합을 구성할지 여부를 선택할 수 있습니다.

기존 데이터 집합을 사용하려면 **[!UICONTROL Existing dataset]**&#x200B;을(를) 선택하십시오. [!UICONTROL Advanced search] 옵션을 사용하거나 드롭다운 메뉴에서 기존 데이터 세트 목록을 스크롤하여 기존 데이터 세트를 검색할 수 있습니다.
...
새 데이터 집합을 사용하려면 **[!UICONTROL New dataset]**&#x200B;을(를) 선택한 다음 출력 데이터 집합 이름과 선택적 설명을 입력하십시오. 그런 다음 [!UICONTROL Advanced search] 옵션을 사용하거나 드롭다운 메뉴에서 기존 스키마 목록을 스크롤하여 매핑할 스키마를 선택합니다. 완료되면 **[!UICONTROL Next]**&#x200B;을(를) 선택합니다.
...
데이터 집합 활동 보기를 시작하려면 소스 카탈로그에서 **[!UICONTROL Dataflows]**&#x200B;을(를) 선택하십시오.
...
이 자습서에 따라 Campaign v8 게재 로그 및 추적 로그 데이터를 Experience Platform으로 가져오는 데이터 흐름을 만들었습니다. 이제 [!DNL Real-Time Customer Profile] 및 [!DNL Data Science Workspace]과(와) 같은 다운스트림 Experience Platform 서비스에서 수신 데이터를 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
