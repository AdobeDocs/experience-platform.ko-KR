---
keywords: Experience Platform;홈;인기 항목;Analytics 소스 커넥터;Analytics 커넥터;Analytics 소스;분석
...
>title="필터 규칙 만들기"
>abstract="실시간 고객 프로필로 데이터를 전송할 때 행 및 열 수준 필터링 규칙을 정의합니다. 행 수준 필터링을 사용하여 조건 적용 및 대상 데이터 지시 **프로필 수집에 포함**. 열 수준 필터링을 사용하여 원하는 데이터 열을 선택합니다 **프로필 수집을 위한 제외**. 데이터 레이크로 전송된 데이터에는 필터링 규칙이 적용되지 않습니다."

>[!IMPORTANT]
>
>필터링 지원 [!DNL Analytics] 데이터는 현재 beta 버전이며 모든 사용자가 사용할 수 없습니다. 설명서 및 기능은 변경될 수 있습니다.

에 대한 매핑을 완료한 후 [!DNL Analytics] 보고서 세트 데이터에서 필터링 규칙 및 조건을 적용하여 수집에서 데이터를 선택적으로 포함하거나 제외할 수 있습니다. [!DNL Profile Service]. 필터링에 대한 지원은 다음에만 가능합니다. [!DNL Analytics] 데이터 및 데이터는 입력 전에 필터링됩니다. [!DNL Profile.] 모든 데이터는 데이터 레이크로 수집됩니다.

#### 행 수준 필터링

>[!IMPORTANT]
>
>행 수준 필터링을 사용하여 조건 적용 및 대상 데이터 지시 **프로필 수집에 포함**. 열 수준 필터링을 사용하여 원하는 데이터 열을 선택합니다 **프로필 수집을 위한 제외**.
...
![삭제](../../../../images/tutorials/create/analytics/delete.png)
