...
&lt;!—Analytics 보고서 세트는 한 번에 하나의 샌드박스에 대해 구성할 수 있습니다. 동일한 보고서 세트를 다른 샌드박스로 가져오려면 데이터 세트 흐름을 삭제하고 다른 샌드박스에 대한 구성을 통해 다시 인스턴스화해야 합니다.—>
...
### 실시간 고객 프로필 필터링(베타) {#filtering-for-profile}
...
다음에 대한 매핑을 완료하면 [!DNL Analytics] 보고서 세트 데이터에서 필터링 규칙 및 조건을 적용하여 섭취 데이터를 실시간 고객 프로필에 선택적으로 포함하거나 제외할 수 있습니다. 필터링 지원은 [!DNL Analytics] 데이터 및 데이터는 입력하기 전에만 필터링됩니다 [!DNL Profile.] 모든 데이터가 데이터 레이크에 수집됩니다.
