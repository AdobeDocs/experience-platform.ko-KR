...
* **사용자 지정 속성**: 사용자 지정 속성은 의 사용자 지정 차원 계층에 있는 모든 속성입니다  [!DNL Analytics]. 또한 Adobe 정의 스키마 중이지만 서로 다른 고객별로 다르게 해석할 수 있습니다. 사용자 지정 속성에는 eVar, prop 및 목록이 포함됩니다. eVar에 대한 자세한 내용은 전환 변수](https://experienceleague.adobe.com/docs/analytics/admin/admin-tools/conversion-variables/conversion-var-admin.html?lang=en)에 대한 다음 [[!DNL Analytics] 설명서를 참조하십시오.
...
**[!UICONTROL Analytics 소스 데이터 추가]** 단계가 나타납니다. **[!UICONTROL 보고서 세트]**&#x200B;를 선택하여 Analytics 보고서 세트 데이터에 대한 소스 연결 만들기를 시작한 다음 수집할 보고서 세트를 선택합니다. 계속하려면 **[!UICONTROL 다음]**&#x200B;을 선택하십시오.

>[!NOTE]
>
>서로 다른 데이터를 가져오기 위해 소스에 대한 여러 인바운드 연결을 만들 수 있습니다.
...
>[!TIP]
>
>[!DNL Analytics] 템플릿 필드 그룹이 있는 스키마만 스키마 선택 메뉴에 표시됩니다. 다른 스키마는 생략됩니다. 보고서 세트 데이터에 사용할 수 있는 적절한 스키마가 없는 경우 새 스키마를 만들어야 합니다. 스키마를 만드는 방법에 대한 자세한 단계는 UI](../../../../../xdm/ui/resources/schemas.md)에서 스키마를 만들고 편집하는 방법에 대한 안내서를 참조하십시오.[
...
| 표준 필드 매핑 | 설명 |
| --- | --- |
| [!UICONTROL 표준 매핑이 적용됨] | 적용된 [!UICONTROL 표준 매핑] 패널에는 매핑된 표준 속성의 총 수가 표시됩니다. 표준 매핑은 소스 [!DNL Analytics] 데이터의 표준 속성과 [!DNL Analytics] 필드 그룹의 표준 속성 간의 매핑 세트를 참조합니다. 미리 매핑되어 있으므로 편집할 수 없습니다. |
| [!UICONTROL 일치하지 않는 표준 매핑] | [!UICONTROL 일치하지 않는 표준 매핑] 패널은 친숙한 이름 충돌이 포함된 매핑된 표준 속성의 수를 나타냅니다. 이러한 충돌은 이미 채워진 필드 설명자 세트가 있는 스키마를 다시 사용할 때 나타납니다. 친숙한 이름이 충돌하는 경우에도 [!DNL Analytics] 데이터 흐름을 계속 진행할 수 있습니다. |
| [!UICONTROL 사용자 지정 매핑] | [!UICONTROL 사용자 지정 매핑] 패널에는 eVar, prop 및 목록을 포함하여 매핑된 사용자 지정 속성의 수가 표시됩니다. 사용자 지정 매핑은 소스 [!DNL Analytics] 데이터의 사용자 지정 속성과 [!DNL Analytics] 필드 그룹의 사용자 지정 속성 간의 매핑 세트를 참조합니다. 사용자 지정 속성은 표준 속성뿐만 아니라 다른 사용자 지정 속성에 매핑할 수 있습니다. |
...
매핑 세트에 친숙한 이름 충돌이 있는 경우 [!DNL Analytics] 데이터 플로우를 계속 사용할 수 있으므로 필드 설명자가 동일한지 확인할 수 있습니다. 또는 빈 설명자 세트가 있는 새 스키마를 만들도록 선택할 수 있습니다.
...
연결이 만들어지면 들어오는 데이터를 포함하도록 대상 스키마와 데이터 흐름이 자동으로 생성됩니다. 또한 데이터 다시 채우기가 발생하고 최대 13개월 동안의 이전 데이터가 수집됩니다. 초기 수집이 완료되면 [!DNL Analytics] 데이터를 사용하고 [!DNL Real-time Customer Profile] 및 세그멘테이션 서비스와 같은 다운스트림 Platform 서비스에서 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
