...
Adobe Experience Platform의 소스 커넥터는 외부에서 가져온 데이터를 예약된 기준으로 인제스트하는 기능을 제공합니다. 이 자습서에서는 Platform 사용자 인터페이스를 사용하여 IBM DB2(이하 &quot;DB2&quot;라 한다) 소스 커넥터를 만드는 단계를 제공합니다.
...
다음 섹션에서는 Flow Service API를 사용하여 DB2에 성공적으로 연결하려면 알아야 할 추가 정보를 제공합니다.
...
필요한 자격 증명을 수집했으면 아래 절차에 따라 Platform에 연결할 새 DB2 계정을 만들 수 있습니다.
...
데이터베이스 *[!UICONTROL 카테고리 아래에서]* IBM **[!UICONTROL DB2]** 를 선택하여 + 아이콘(+) **을** 클릭하여 새 DB2 커넥터를 만듭니다.
...
새 자격 증명을 사용 중인 경우 **[!UICONTROL 새 계정을 선택합니다]**. 표시되는 입력 양식에서 이름, 선택적 설명 및 DB2 자격 증명으로 연결을 제공합니다. 완료되면 **[!UICONTROL Connect를]** 선택한 다음 새 계정이 설정되기까지 약간의 시간이 소요됩니다.
...
이 튜토리얼을 따라 DB2 계정에 대한 연결을 설정했습니다. 이제 다음 튜토리얼로 계속 이동하여 데이터를 Platform으로 가져오도록 [데이터 흐름을 구성할 수 있습니다](../../dataflow/databases.md).