...
topic: overview
type: Tutorial
description: 이 자습서에서는 플랫폼 사용자 인터페이스를 사용하여 IBM DB2(이하 "DB2") 소스 커넥터를 만드는 단계를 제공합니다.
...
Adobe Experience Platform의 소스 커넥터는 예약된 기준으로 외부 소스 데이터를 인제스트하는 기능을 제공합니다. 이 자습서에서는 [!DNL Platform] 사용자 인터페이스를 사용하여 IBM DB2(이하 &quot;DB2&quot;) 소스 커넥터를 만드는 단계를 제공합니다.
...
이미 유효한 DB2 연결이 있는 경우 이 문서의 나머지 부분을 건너뛰고 데이터 흐름 [구성에 대한 자습서로 진행할 수 있습니다](../../dataflow/databases.md).
...
필요한 자격 증명을 수집했으면 아래 절차에 따라 DB2 계정을 연결할 수 있습니다 [!DNL Platform].
...
데이터베이스 **[!UICONTROL 카테고리]** 아래에서 **[!UICONTROL IBM DB2를 선택합니다]**. 이 커넥터를 처음 사용하는 경우 구성을 **[!UICONTROL 선택합니다]**. 그렇지 않은 경우 데이터 **[!UICONTROL 추가]** 를 선택하여 새 DB2 커넥터를 만듭니다.

![카탈로그](../../../../images/tutorials/create/ibm-db2/catalog.png)

IBM **[!UICONTROL DB2에]** 연결 페이지가 나타납니다. 이 페이지에서 새 자격 증명이나 기존 자격 증명을 사용할 수 있습니다.

### 새 계정

새 자격 증명을 사용 중인 경우 **[!UICONTROL 새 계정을 선택합니다]**. 표시되는 입력 양식에서 이름, 선택적 설명 및 DB2 자격 증명을 입력합니다. 완료되면 **[!UICONTROL Connect를]** 선택한 다음 새 연결이 설정될 때까지 잠시 기다립니다.

![connect](../../../../images/tutorials/create/ibm-db2/new.png)

### 기존 계정

기존 계정을 연결하려면 연결할 DB2 계정을 선택한 다음 **[!UICONTROL 다음을]** 선택하여 진행합니다.

![기존](../../../../images/tutorials/create/ibm-db2/existing.png)

## 다음 단계

이 튜토리얼을 따라 DB2 계정에 대한 연결을 설정했습니다. 이제 다음 튜토리얼로 계속 이동하여 데이터를 [데이터 흐름을 구성하여 데이터를 가져올 수 있습니다 [!DNL Platform]](../../dataflow/databases.md).