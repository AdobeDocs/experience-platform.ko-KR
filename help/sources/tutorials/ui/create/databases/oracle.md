...
# UI에서 Oracle 소스 커넥터 생성

> [!NOTE]
> Oracle 커넥터가 베타 상태입니다. 베타 [레이블이 지정된 커넥터 사용에 대한 자세한 내용은 소스 개요를](../../../../home.md#terms-and-conditions) 참조하십시오.

Adobe Experience Platform의 소스 커넥터는 외부에서 가져온 데이터를 예약된 기준으로 인제스트하는 기능을 제공합니다. 이 자습서에서는 Platform 사용자 인터페이스를 사용하여 Oracle 소스 커넥터를 생성하는 단계를 제공합니다.
...
Platform에서 Oracle 계정에 액세스하려면 다음 값을 제공해야 합니다.
...
필요한 인증서를 수집했으면 아래 절차에 따라 Platform에 접속할 새 Oracle 계정을 생성할 수 있습니다.
...
[ *데이터베이스* ] 범주 아래에서 **[!UICONTROL Oracle DB를]** 선택하고 + 아이콘(+) **을 클릭하여 새 Oracle 커넥터를 생성합니다** .
...
새 자격 증명을 사용 중인 경우 **[!UICONTROL 새 계정을 선택합니다]**. 표시되는 입력 양식에서 이름, 선택적 설명 및 Oracle 자격 증명으로 연결을 제공합니다. 완료되면 **[!UICONTROL Connect를]** 선택한 다음 새 계정이 설정되기까지 약간의 시간이 소요됩니다.

![connect](../../../../images/tutorials/create/oracle/new.png)

### 기존 계정

기존 계정을 연결하려면 연결할 Oracle 계정을 선택한 다음 **[!UICONTROL 다음을]** 선택하여 진행합니다.

![기존](../../../../images/tutorials/create/oracle/existing.png)

## 다음 단계

이 자습서를 따라 Oracle 계정에 대한 연결을 설정했습니다. 이제 다음 튜토리얼로 계속 이동하여 데이터를 Platform으로 가져오도록 [데이터 흐름을 구성할 수 있습니다](../../dataflow/databases.md).