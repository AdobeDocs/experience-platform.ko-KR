...
> [!NOTE]
> Azure 데이터 탐색기 커넥터가 베타 버전입니다. 베타 [레이블이 지정된 커넥터 사용에 대한 자세한 내용은 소스 개요를](../../../../home.md#terms-and-conditions) 참조하십시오.

Adobe Experience Platform의 소스 커넥터는 외부에서 가져온 데이터를 예약된 기준으로 인제스트하는 기능을 제공합니다. 이 자습서에서는 Platform 사용자 인터페이스를 사용하여 Azure 데이터 탐색기(이하 &quot;데이터 탐색기&quot;) 소스 커넥터를 만드는 단계를 제공합니다.
...
Platform에서 데이터 탐색기 계정에 액세스하려면 다음 값을 제공해야 합니다.
...
필요한 자격 증명을 수집했으면 아래 절차에 따라 Platform에 연결할 새 데이터 탐색기 계정을 만들 수 있습니다.
...
이 튜토리얼을 따라 데이터 탐색기 계정에 대한 연결을 설정했습니다. 이제 다음 튜토리얼로 계속 이동하여 데이터를 Platform으로 가져오도록 [데이터 흐름을 구성할 수 있습니다](../../dataflow/databases.md).