...
Adobe Experience Platform의 소스 커넥터는 일정에 따라 외부 소스 데이터를 수집하는 기능을 제공합니다. 이 자습서에서는 다음을 만드는 단계를 제공합니다 [!DNL Amazon Redshift] (이하 &quot;라고 한다)[!DNL Redshift]&quot;) 를 사용하는 소스 커넥터 [!DNL Platform] 사용자 인터페이스.
...
에 액세스하려면 [!DNL Redshift] 다음에 대한 계정 [!DNL Platform], 다음 값을 제공해야 합니다.

| **자격 증명** | **설명** |
| -------------- | --------------- |
| `server` | 와(과) 연계된 서버 [!DNL Redshift] 계정입니다. |
| `username` | 와(과) 연계된 사용자 이름 [!DNL Redshift] 계정입니다. |
| `password` | 과(와) 연계된 암호 [!DNL Redshift] 계정입니다. |
| `database` | 다음 [!DNL Redshift] 액세스 중인 데이터베이스입니다. |
...
필요한 자격 증명을 수집했으면 아래 단계에 따라 를 연결할 수 있습니다. [!DNL Redshift] 계정 위치: [!DNL Platform].
...
이 자습서를 따라 [!DNL Redshift] 계정입니다. 이제 다음 튜토리얼을 계속 진행하여 [데이터 흐름을 구성하여 데이터 가져오기 [!DNL Platform]](../../dataflow/databases.md).
