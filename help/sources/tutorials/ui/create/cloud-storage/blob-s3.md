...
필요한 자격 증명을 수집했으면 아래 절차에 따라 플랫폼에 연결할 새 Blob 또는 S3 계정을 만들 수 있습니다.
...
데이터베이스 *[!UICONTROL 카테고리 아래에서]* Azure Blob 저장소를 **[!UICONTROL 선택하거나]** Amazon S3 **[!UICONTROL 을 클릭하여 + 아이콘(+)을]** **** 클릭하여 새 Blob S3 커넥터를 만듭니다.
...
새 자격 증명을 사용 중인 경우 **[!UICONTROL 새 계정을 선택합니다]**. 표시되는 입력 양식에서 이름, 선택적 설명 및 Blob 또는 S3 자격 증명을 사용하여 연결을 제공합니다. 완료되면 **[!UICONTROL Connect를]** 선택한 다음 새 계정이 설정되기까지 약간의 시간이 소요됩니다.

![connect](../../../../images/tutorials/create/blob/new.png)

### 기존 계정

기존 계정을 연결하려면 연결할 Blob 또는 S3 계정을 선택한 다음 **[!UICONTROL 다음을]** 선택하여 진행하십시오.

![기존](../../../../images/tutorials/create/blob/existing.png)

## 다음 단계

이 튜토리얼을 따라 Blob 또는 S3 계정에 대한 연결을 설정했습니다. 이제 다음 튜토리얼로 계속 진행하여 데이터 흐름을 [구성하여 클라우드 스토리지의 데이터를 Platform으로 가져올 수 있습니다](../../dataflow/batch/cloud-storage.md).