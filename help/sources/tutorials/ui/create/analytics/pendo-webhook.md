...
# 만들기 [!DNL Pendo] 소스 연결 데이터 흐름 및 UI

>[!NOTE]
>
>다음 [!DNL Pendo] 소스가 베타 버전입니다. 자세한 내용은 [소스 개요](../../../../home.md#terms-and-conditions) 베타 레이블이 지정된 소스 사용에 대한 자세한 정보.

이 자습서에서는 을(를) 만드는 단계를 제공합니다 [!DNL Pendo] Adobe Experience Platform 사용자 인터페이스를 사용한 소스 연결 및 데이터 흐름.
...
다음 섹션에서는 만들기 전에 완료해야 하는 사전 요구 사항에 대해 설명합니다 [!DNL Pendo] 소스 연결.

### 소스 스키마를 정의하기 위한 샘플 JSON [!DNL Pendo] {#prerequisites-json-schema}

만들기 전 [!DNL Pendo] 소스 연결을 사용하려면 소스 스키마를 제공해야 합니다. 아래에서 JSON을 사용할 수 있습니다.
...
### 용 플랫폼 스키마 만들기 [!DNL Pendo] {#create-platform-schema}

또한 소스에 사용할 플랫폼 스키마를 먼저 만들어야 합니다. 다음에서 자습서를 참조하십시오. [플랫폼 스키마 만들기](../../../../../xdm/schema/composition.md) 를 참조하십시오.

![Pendo의 예제 스키마를 보여주는 Platform UI .](../../../../images/tutorials/create/analytics-pendo-webhook/schema.png)
...
로 이동합니다. [!UICONTROL Analytics] 카테고리를 클릭하여 [!DNL Pendo] 소스 카드. 시작하려면 다음을 선택합니다 **[!UICONTROL 데이터 추가]**.
...
![소스 워크플로우의 미리 보기 단계입니다.](../../../../images/tutorials/create/analytics-pendo-webhook/preview.png)
...
| Target 필드 | 설명 |
| --- | --- |
| `uniqueId` | 다음 [!DNL Pendo] 이벤트의 식별자입니다. |
...
웹 후크를 구성하는 데 사용되는 URL을 구성하려면 [!DNL Pendo] 다음을 검색해야 합니다.
...
스트리밍 종단점 및 데이터 흐름 ID를 검색한 후에는 다음 패턴을 기반으로 URL을 빌드합니다. ```{STREAMING_ENDPOINT}?x-adobe-flow-id={DATAFLOW_ID}```. 예를 들어 만들어진 웹 후크 URL은 다음과 같을 수 있습니다. ```https://dcs.adobedc.net/collection/0c61859cc71939a0caf01123f91b2fc52589018800ad46b6c76c2dff3595ee95```

## 에서 웹 후크 설정 [!DNL Pendo] {#set-up-webhook}

다음으로, [[!DNL Pendo]](https://pendo.io/) 웹 후크를 만듭니다. 를 사용하여 웹 후크를 만드는 방법에 대한 절차를 설명합니다. [!DNL Pendo] 사용자 인터페이스를 참조하십시오. [[!DNL Pendo] 웹 후크 만들기 안내서](https://support.pendo.io/hc/en-us/articles/360032285012-Webhooks#create-a-webhook-0-4).

웹 후크가 만들어지면 의 설정 페이지로 이동합니다 [!DNL Pendo] webhook 및 입력 [!DNL URL] 필드.

![Pendo UI 스크린샷에서는 웹 후크 끝점 필드를 보여줍니다](../../../../images/tutorials/create/analytics-pendo-webhook/webhook.png)

>[!TIP]
>
>다양한 이벤트 카테고리에 가입하여 이벤트에서 보낼 이벤트를 결정할 수 있습니다 [!DNL Pendo] 인스턴스를 Platform에 추가합니다. 다른 이벤트에 대한 자세한 내용은 [[!DNL Pendo] 설명서](https://support.pendo.io/hc/en-us/articles/360032285012-Webhooks#create-a-webhook-0-4).

## 다음 단계 {#next-steps}

이 자습서를 따라 스트리밍 데이터 흐름을 구성하여 [!DNL Pendo] Experience Platform 데이터. 수집되는 데이터를 모니터링하려면 다음 안내서를 참조하십시오 [플랫폼 UI를 사용하여 스트리밍 데이터 흐름 모니터링](../../monitor-streaming.md).

## 추가 리소스 {#additional-resources}

아래 섹션에서는 를 사용할 때 참조할 수 있는 추가 리소스를 제공합니다. [!DNL Pendo] 소스.

### 유효성 검사 {#validation}

소스 및 [!DNL Pendo] 메시지를 수집하는 중입니다. 아래 단계를 수행하십시오.

* 확인할 수 있습니다 [!DNL Pendo] **[!UICONTROL 보고서]** > **[!UICONTROL 채팅 내역]** 페이지에 캡처되는 이벤트를 식별합니다. [!DNL Pendo].

![채팅 내역을 보여주는 Pendo UI 스크린샷](../../../../images/tutorials/create/analytics-pendo-webhook/pendo-events.png)

* 플랫폼 UI에서 **[!UICONTROL 데이터 흐름 보기]** 옆에 [!DNL Pendo] 소스 카탈로그의 카드 메뉴. 다음 을 선택합니다. **[!UICONTROL 데이터 세트 미리 보기]** 에서 구성한 웹 후크에 대해 수집된 데이터를 확인하려면 [!DNL Pendo].

![수집된 이벤트를 보여주는 Platform UI 스크린샷](../../../../images/tutorials/create/analytics-pendo-webhook/platform-dataset.png)

### 오류 및 문제 해결 {#errors-and-troubleshooting}

데이터 흐름 실행을 확인할 때 다음 오류 메시지가 표시될 수 있습니다. `The message can't be validated ... uniqueID:expected minLength:1, actual 0].`

![오류를 보여주는 Platform UI 스크린샷입니다.](../../../../images/tutorials/create/analytics-pendo-webhook/error.png)

이 오류를 수정하려면 *uniqueID* 매핑이 설정되었습니다. 추가 지침은 [Mmpping](#mapping) 섹션을 참조하십시오.
