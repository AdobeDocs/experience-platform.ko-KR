---
keywords: Experience Platform;홈;인기 항목;스트리밍 연결;스트리밍 연결 만들기;ui 안내서;자습서;스트리밍 연결 만들기;스트리밍 수집;수집;
...
Parquet 파일은 XDM을 준수해야 하며 매핑을 수동으로 구성할 필요가 없습니다. 반면 CSV 파일은 매핑을 명시적으로 구성해야 하지만 매핑할 소스 데이터 필드를 선택할 수 있습니다. JSON 파일은 XDM 컴플레인으로 표시되는 경우 수동 구성이 필요하지 않습니다. 그러나 XDM 호환으로 표시되지 않으면 매핑을 명시적으로 구성해야 합니다.
