...
| 소스 데이터 흐름 실행 성공 | 이 경고는 소스의 데이터가 Platform으로 성공적으로 수집되면 메시지를 보냅니다. |
| 소스 데이터 흐름 실행 실패 | 이 경고는 데이터 흐름에 오류가 발생하는 경우 메시지를 보냅니다. |
| ~~소스 데이터 흐름 수집 부족~~ | ~~이 경고는 수집이 7시간 이상 지연되고 플랫폼으로 데이터가 수집되지 않는 경우 메시지를 보냅니다.~~ <br>**참고:** 이 경고는 더 이상 사용되지 않으므로 더 이상 경고를 받지 않습니다. |
