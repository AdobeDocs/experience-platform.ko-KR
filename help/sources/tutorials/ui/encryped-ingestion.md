...
* 암호화된 데이터를 클라우드 스토리지에 저장합니다. 이 단계에서는 소스 데이터를 XDM(Experience Data Model) 스키마에 매핑하는 참조로 사용할 수 있는 데이터 샘플 파일도 클라우드 스토리지에 있는지 확인해야 합니다.
...
Experience Platform UI에서 소스 작업 영역으로 이동한 다음 상단 헤더에서 [!UICONTROL Key Pairs]을(를) 선택합니다.

![&quot;키 쌍&quot; 헤더가 선택된 소스 카탈로그입니다.](../../images/tutorials/edi/catalog.png)

조직의 기존 암호화 키 쌍 목록을 표시하는 페이지로 이동합니다. 이 페이지에서는 해당 키의 제목, ID, 유형, 암호화 알고리즘, 만료 및 상태에 대한 정보를 제공합니다. 새 키 쌍을 만들려면 **[!UICONTROL Create Key]**&#x200B;을(를) 선택합니다.

![키 쌍에 &quot;암호화 키&quot;를 키 유형으로 선택하고 &quot;키 만들기&quot; 단추를 선택했습니다.](../../images/tutorials/edi/encryption_key_page.png)

그런 다음 만들려는 키 유형을 선택합니다. 암호화 키를 만들려면 **[!UICONTROL Encryption Key]**&#x200B;을(를) 선택한 다음 **[!UICONTROL Continue]**&#x200B;을(를) 선택하십시오.

![암호화 키가 선택된 키 생성 창입니다.](../../images/tutorials/edi/choose_encryption_key_type.png)

암호화 키의 제목과 암호를 입력합니다. 암호는 암호화 키에 대한 추가 보호 계층입니다. 생성 시 Experience Platform은 암호를 공개 키와 다른 보안 저장소에 저장합니다. 비어 있지 않은 문자열을 암호로 제공해야 합니다. 완료되면 **[!UICONTROL Create]**&#x200B;을(를) 선택합니다.
...
기존 암호화 키에 대한 정보를 보려면 키 제목 옆에 있는 줄임표(`...`)를 선택하십시오. 공개 키와 키 ID를 보려면 **[!UICONTROL Key details]**&#x200B;을(를) 선택하십시오. 또는 암호화 키를 삭제하려면 **[!UICONTROL Delete]**&#x200B;을(를) 선택하십시오.
...
서명 확인 키는 개인 키와 공개 키를 포함하는 또 다른 암호화 메커니즘입니다. 이 경우 서명 확인 키 쌍을 만들고 개인 키를 사용하여 서명하고 데이터에 추가 암호화 계층을 제공할 수 있습니다. 그런 다음 해당 공개 키를 Experience Platform에 공유합니다. 수집 중에 Experience Platform은 공개 키를 사용하여 개인 키와 연결된 서명을 확인합니다.

>[!ENDSHADEBOX]

서명 확인 키를 만들려면 키 유형 선택 창에서 **[!UICONTROL Sign Verification Key]**&#x200B;을(를) 선택한 다음 **[!UICONTROL Continue]**&#x200B;을(를) 선택하십시오.

![서명 확인 키를 선택하는 키 형식 선택 창입니다.](../../images/tutorials/edi/choose_sign_verification_key_type.png)

그런 다음 제목과 [!DNL Base64] 인코딩 PGP 키를 공개 키로 제공한 다음 **[!UICONTROL Create]**&#x200B;을(를) 선택합니다.
...
선택한 클라우드 스토리지 소스로 인증합니다. 워크플로우의 데이터 선택 단계에서 수집할 암호화된 파일 또는 폴더를 선택한 다음 **[!UICONTROL Is the file encrypted]** 전환을 활성화합니다.
...
![이 파일이 암호화되어 있습니까? 토글이 활성화되었으며 &quot;샘플 파일 선택&quot; 단추가 선택되었습니다](../../images/tutorials/edi/select_sample_file.png).

샘플 파일을 선택한 후에는 해당 데이터 형식, 구분 기호 및 압축 유형과 같은 데이터 설정을 구성합니다. 미리 보기 인터페이스가 완전히 렌더링될 때까지 잠시 기다렸다가 **[!UICONTROL Save]**&#x200B;을(를) 선택하십시오.
