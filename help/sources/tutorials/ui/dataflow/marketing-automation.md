...
# UI에서 마케팅 자동화 커넥터에 대한 데이터 흐름 구성

데이터 흐름(Dataflow)은 소스에서 데이터 세트로 데이터를 검색하고 인제스트하는 예약된 [!DNL Platform] 작업입니다. 이 자습서에서는 마케팅 자동화 계정을 사용하여 새 데이터 흐름을 구성하는 절차를 제공합니다.
...
또한 이 자습서에서는 이미 마케팅 자동화 계정을 만들어야 합니다. UI에서 다른 마케팅 자동화 커넥터를 만들기 위한 자습서 목록은 [소스 커넥터 개요에 있습니다](../../../home.md).

## 데이터 선택

마케팅 자동화 계정을 만든 후 *데이터* 선택 단계가 나타나며, 파일 계층 구조를 탐색하는 데 필요한 대화형 인터페이스를 제공합니다.
...
![예약](../../../images/tutorials/dataflow/marketing-automation/scheduling.png)
...
인증 ** 화면에서 비활성화할 데이터 프롤과 연결된 기본 연결의 이름을 선택합니다.
