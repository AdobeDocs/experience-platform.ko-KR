---
keywords: Experience Platform;홈;인기 항목;데이터 흐름 구성;advertising connector
solution: Experience Platform
title: UI에서 광고 소스를 사용하여 데이터 흐름 만들기
type: Tutorial
description: 데이터 흐름은 소스에서 Platform 데이터 세트로 데이터를 검색하고 수집하는 예약된 작업입니다. 이 자습서에서는 Platform UI를 사용하여 광고 소스에 대한 데이터 흐름을 만드는 방법에 대한 단계를 제공합니다.
...
데이터 흐름은 소스에서 Adobe Experience Platform의 데이터 세트로 데이터를 검색하고 수집하는 예약된 작업입니다. 이 자습서에서는 Platform UI를 사용하여 광고 소스에 대한 데이터 흐름을 만드는 방법에 대한 단계를 제공합니다.

>[!NOTE]
>
>데이터 흐름을 만들려면 광고 소스가 있는 인증된 계정이 이미 있어야 합니다. UI에서 다양한 광고 소스 계정을 만들기 위한 자습서 목록은 [소스 개요](../../../home.md#advertising).
...
광고 소스 계정을 만든 후 **[!UICONTROL 데이터 추가]** 광고 소스 계정의 테이블 계층 구조를 살펴볼 수 있는 인터페이스를 제공하는 단계가 나타납니다.
...
이 자습서를 따라 광고 소스에서 Platform으로 데이터를 가져오는 데이터 흐름을 만들었습니다. 이제 들어오는 데이터를 다운스트림에서 사용할 수 있습니다. [!DNL Platform] 다음과 같은 서비스 [!DNL Real-Time Customer Profile] 및 [!DNL Data Science Workspace]. 자세한 내용은 다음 문서를 참조하십시오.
