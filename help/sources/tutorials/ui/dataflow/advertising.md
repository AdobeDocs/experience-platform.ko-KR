...
또한 이 튜토리얼을 사용하려면 이미 광고 계정을 만들어야 합니다. UI에서 서로 다른 지불 커넥터를 만들기 위한 자습서 목록은 [소스 커넥터 개요에](../../../home.md)있습니다.

## 데이터 선택

광고 계정을 만든 *후 데이터* 선택 단계가 나타나 파일 계층 구조를 탐색할 수 있는 대화형 인터페이스를 제공합니다.

- 인터페이스의 왼쪽 절반은 서버의 파일과 디렉토리를 표시하는 디렉토리 브라우저입니다.
- 인터페이스 오른쪽의 절반은 호환 파일에서 최대 100개의 데이터 행을 미리 볼 수 있도록 해줍니다.

사용할 디렉토리를 선택한 다음 다음을 **선택합니다**.
...
매핑 *화면에서는* 델타 열을 *설정할 수도*&#x200B;있습니다. 데이터 세트 흐름이 만들어지면 타임스탬프 필드를 기반으로 설정하여 예약된 증분 인제션에서 어느 레코드를 인제스트할지 결정할 수 있습니다.
...
데이터 *집합 흐름 세부* 사항 단계가 나타나고, 여기에서 데이터 집합 흐름에 대한 이름과 선택적 설명을 제공해야 합니다. 완료되면 **[다음** ]을 선택합니다.
