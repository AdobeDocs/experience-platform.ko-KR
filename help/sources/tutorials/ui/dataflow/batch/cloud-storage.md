...
클라우드 스토리지 커넥터를 만든 후 *[!UICONTROL 데이터]* 선택 단계가 나타나며, 클라우드 스토리지 계층 구조를 탐색하는 데 필요한 대화형 인터페이스를 제공합니다.

* 인터페이스의 왼쪽 절반은 서버의 파일과 디렉토리를 표시하는 디렉토리 브라우저입니다.
* 인터페이스의 오른쪽 절반을 사용하면 호환 파일에서 최대 100개의 데이터 행을 미리 볼 수 있습니다.

나열된 폴더를 클릭하면 폴더 계층 구조를 더 깊은 폴더로 이동할 수 있습니다. 호환되는 파일 또는 폴더를 선택하면 **[!UICONTROL 데이터 형식]** 선택 드롭다운이 나타나며, 이 드롭다운에서 데이터를 미리 보기 창에 표시할 형식을 선택할 수 있습니다.
...
이름 *[!UICONTROL 흐름]* 단계가 나타나면서 새 데이터 흐름 이름을 지정하고 간단한 설명을 제공할 수 있습니다.
...
* *[!UICONTROL 소스 세부 정보]*: 소스 유형, 선택한 소스 파일의 관련 경로 및 해당 소스 파일 내의 열 양을 표시합니다.
...
소스 커넥터의 인바운드 데이터를 [!DNL Real-time Customer Profile] 데이터 누락을 위해 사용할 수 있습니다. 실제 고객 [!DNL Profile] 데이터 채우기에 대한 자세한 내용은 프로필 모집단 [에 대한 자습서를 참조하십시오](../../profile.md).
