...
type: Tutorial
description: 데이터 흐름은 소스에서 플랫폼 데이터 집합으로 데이터를 검색하고 수집하는 예약된 작업입니다. 이 자습서에서는 데이터베이스 계정을 사용하여 새 데이터 흐름을 구성하는 단계를 제공합니다.
...
데이터 흐름은 소스에서 플랫폼 데이터 집합으로 데이터를 검색하고 수집하는 예약된 작업입니다. 이 자습서에서는 데이터베이스 계정을 사용하여 새 데이터 흐름을 구성하는 단계를 제공합니다.
...
또한 이 자습서에서는 이미 데이터베이스 계정을 만들어야 합니다. UI에서 다른 데이터베이스 커넥터를 만드는 자습서 목록은 [소스 커넥터 개요](../../../home.md)에 있습니다.

## 데이터 추가

데이터베이스 계정을 만든 후 **[!UICONTROL 데이터 추가]** 단계가 나타나고 데이터베이스 계층을 탐색하기 위한 대화형 인터페이스를 제공합니다.

- 인터페이스 왼쪽 절반 부분은 브라우저의 브라우저로 계정의 데이터 테이블 목록을 표시합니다.
- 인터페이스의 오른쪽 절반 섹션에서는 최대 100개의 데이터 행을 미리 볼 수 있습니다.
...
소스 데이터를 찾으면 테이블을 선택한 다음 **[!UICONTROL 다음]**&#x200B;을 선택합니다.
...
*매핑* 단계가 나타나고 소스 데이터를 Platform 데이터 세트에 매핑하기 위한 대화형 인터페이스를 제공합니다.

수집할 인바운드 데이터에 대한 데이터 세트를 선택합니다. 기존 데이터 세트를 사용하거나 새 데이터 세트를 만들 수 있습니다.

### 기존 데이터 세트 사용

기존 데이터 세트에 데이터를 수집하려면 **[!UICONTROL 기존 데이터 세트]**&#x200B;를 선택한 다음 데이터 세트 아이콘을 클릭합니다.
...
이 자습서를 따라 외부 데이터베이스에서 데이터를 가져오는 데이터 흐름을 성공적으로 만들어 데이터 세트 모니터링에 대한 통찰력을 얻을 수 있습니다. 이제 [!DNL Real-time Customer Profile] 및 [!DNL Data Science Workspace] 등의 다운스트림 [!DNL Platform] 서비스에서 들어오는 데이터를 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
...
**[!UICONTROL 소스]** 작업 영역에서 **[!UICONTROL 데이터 흐름]** 탭을 선택합니다. 다음으로 비활성화할 데이터 흐름을 선택합니다.

![](../../../images/tutorials/dataflow/databases/list-of-dataflows.png)

**[!UICONTROL 속성]** 열은 **[!UICONTROL 활성화됨]** 전환 단추를 포함하여 화면 오른쪽에 나타납니다. 데이터 흐름을 비활성화하려면 전환을 선택합니다. 동일한 전환을 사용하여 데이터 흐름을 비활성화한 후 다시 활성화할 수 있습니다.
