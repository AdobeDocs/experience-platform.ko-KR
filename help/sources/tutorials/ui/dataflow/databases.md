...
데이터베이스 원본 계정을 만든 후 **[!UICONTROL 데이터 추가]** 단계가 표시되어 데이터베이스 원본 계정의 테이블 계층 구조를 살펴볼 수 있는 인터페이스를 제공합니다.
...
>[!NOTE]
>
>모델 기반 스키마에 매핑할 때 소스 데이터에 기본 키 및 버전 식별자 또는 시계열 스키마에 대한 타임스탬프 식별자 와 같은 필수 필드가 포함되어 있는지 확인합니다.

변경 데이터 캡처에 사용되는 `_change_request_type`과(와) 같은 제어 열은 수집 중에 읽히지만 대상 스키마에 저장되지 않습니다.

모델 기반 스키마는 기본 및 외래 키 매핑을 사용하여 데이터 세트 간의 관계를 지원합니다.
...
* **[!UICONTROL 데이터 집합 및 맵 필드 할당]**: 소스 데이터가 수집될 데이터 집합과 관련 스키마를 표시합니다. 모델 기반 스키마를 사용하는 경우 기본 키 및 버전 식별자와 같은 필수 필드가 올바르게 매핑되었는지 확인합니다. 또한 모든 변경 데이터 캡처 제어 열이 올바르게 구성되어 있는지 확인합니다. 모델 기반 스키마를 사용하는 데이터 세트는 여러 데이터 모델을 지원하며 [데이터 캡처 워크플로 변경](../../api/change-data-capture.md)을 사용하도록 설정합니다.
