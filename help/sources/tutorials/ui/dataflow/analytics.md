...
description: 이 자습서에서는 플랫폼 UI를 사용하여 Analytics 소스에 대한 데이터 흐름을 만드는 방법에 대해 설명합니다.
...
# UI에서 분석 소스를 사용하여 데이터 흐름 만들기

데이터 흐름은 소스에서 Adobe Experience Platform의 데이터 집합으로 데이터를 검색하고 수집하는 예약된 작업입니다. 이 자습서에서는 플랫폼 UI를 사용하여 Analytics 소스에 대한 데이터 흐름을 만드는 방법에 대해 설명합니다.

>[!NOTE]
>
>데이터 흐름을 만들려면 이미 [!DNL Mixpanel] 소스. 다음에서 자습서를 참조하십시오. [만들기 [!DNL Mixpanel] UI의 소스 연결](../../ui/create/analytics/mixpanel.md) 추가 정보.
...
>[!IMPORTANT]
>
>동적 키 쌍 값을 개체로 매핑할 수 없습니다 [!DNL OneTrust] 대상 및 대상 스키마에서 해당 키를 지정해야 수집 중에 데이터를 매핑할 수 있습니다.
...
이 자습서를 따라 분석 소스에서 플랫폼으로 데이터를 가져오는 데이터 흐름을 성공적으로 만들었습니다. 이제 다운스트림에서 들어오는 데이터를 사용할 수 있습니다 [!DNL Platform] 와 같은 서비스 [!DNL Real-Time Customer Profile] 및 [!DNL Data Science Workspace]. 자세한 내용은 다음 문서를 참조하십시오.
