...
데이터 흐름은 소스에서 Adobe Experience Platform의 데이터 집합으로 데이터를 검색하고 수집하는 예약된 작업입니다. 이 자습서에서는 플랫폼 UI를 사용하여 CRM(고객 관계 관리) 소스용 데이터 흐름을 만드는 방법에 대해 설명합니다.

>[!NOTE]
>
>데이터 흐름을 만들려면 CRM 소스가 있는 인증된 계정이 이미 있어야 합니다. UI에서 다른 CRM 소스 계정을 만드는 자습서 목록은 [소스 개요](../../../home.md#crm).
...
CRM 소스 계정을 만든 후 **[!UICONTROL 데이터 추가]** CRM 계정의 테이블 계층 구조를 탐색할 수 있는 인터페이스를 제공하는 단계가 나타납니다.
