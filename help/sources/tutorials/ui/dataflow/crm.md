...
title: UI에서 CRM Source을 사용하여 데이터 흐름 만들기
type: Tutorial
...
>[!NOTE]
>
>* 데이터 흐름을 만들려면 CRM 소스가 있는 인증된 계정이 이미 있어야 합니다. UI에서 다양한 CRM 소스 계정을 만들기 위한 자습서 목록은 [소스 개요](../../../home.md#crm)에서 찾을 수 있습니다.
...
CRM 소스 계정을 만든 후 **[!UICONTROL 데이터 추가]** 단계가 표시되어 CRM 계정의 테이블 계층 구조를 살펴볼 수 있는 인터페이스를 제공합니다.
...
>[!NOTE]
>
>[!DNL Salesforce]을(를) Adobe Real-time Customer Data Platform B2B 에디션의 일부로 사용하는 경우 [!DNL Salesforce] 소스 필드를 해당 XDM 대상 필드에 매핑하는 방법에 대한 지침은 [[!DNL Salesforce] 필드 매핑 문서](../../../connectors/adobe-applications/mapping/salesforce.md)를 참조하십시오.
...
| 필드 | 설명 |
| --- | --- |
| 빈도 | 수집이 발생하는 빈도입니다. 선택 가능한 빈도는 `Once`, `Minute`, `Hour`, `Day` 및 `Week`입니다. |
...
| 증분 데이터 로드 기준 | 유형, 날짜 또는 시간의 소스 스키마 필드 세트가 필터링되어 있는 옵션. 이 필드는 새 데이터와 기존 데이터를 구분하는 데 사용됩니다. 선택한 열의 타임스탬프를 기준으로 증분 데이터가 수집됩니다. |
...
이 자습서에 따라 CRM 소스의 데이터를 플랫폼으로 가져오는 데이터 흐름을 만들었습니다. 이제 [!DNL Real-Time Customer Profile] 및 [!DNL Data Science Workspace] 등의 다운스트림 [!DNL Platform] 서비스에서 수신 데이터를 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
