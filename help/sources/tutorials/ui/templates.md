...
Experience Platform UI에서 왼쪽 탐색에서 **[!UICONTROL Sources]**&#x200B;을(를) 선택하여 [!UICONTROL Sources] 작업 영역에 액세스하고 Experience Platform에서 사용할 수 있는 소스 카탈로그를 확인합니다.
...
자동 생성된 에셋을 사용하려면 **[!UICONTROL Browse templates]**&#x200B;을(를) 선택한 다음 **[!UICONTROL Select]**&#x200B;을(를) 선택하십시오.
...
| 캠페인 | 캠페인 템플릿은 회사 소개 정보, 위치 및 청구 정보와 같은 비즈니스 계정 세부 정보를 캡처합니다. |
...
| --- | --- |
| 연락처 | 연락처 템플릿은 인구 통계 정보, 연락처 정보, 관련 비즈니스 엔터티와 같은 연락처의 속성을 캡처합니다. |
| 리드 | 잠재 고객 템플릿은 인구 통계 정보, 연락처 정보, 관련 비즈니스 엔터티와 같은 잠재 고객의 속성을 캡처합니다. |

{style="table-layout:auto"}

+++

+++B2B 템플릿 [!DNL Microsoft Dynamics]개
다음 표에는 [!DNL Microsoft Dynamics] 소스에서 사용할 수 있는 B2B 템플릿이 요약되어 있습니다.
...
| 마케팅 목록 멤버 | 마케팅 목록 멤버는 마케팅 목록에 있는 잠재 고객, 계정 또는 연락처와 같은 한 가지 유형의 고객 레코드 세부 정보를 캡처합니다. |
| 기회 | 영업 기회 템플릿은 유형, 영업 단계 및 관련 계정 등 비즈니스 영업 기회 세부 정보를 캡처합니다. |
| 영업 기회 연락처 역할 | Opportunity Contact Roles 템플릿은 특정 Opportunity 와 연관된 Lead 의 역할에 대한 세부 정보를 캡처합니다. |
...
예약 인터페이스를 사용하여 데이터 흐름에 대한 수집 일정을 구성합니다. 수집 빈도를 **Once**(으)로 설정하여 일회성 수집을 만듭니다.

![Dynamics 및 Salesforce 템플릿의 일정 인터페이스](../../images/tutorials/templates/schedule.png)

또는 수집 빈도를 **분**, **시간**, **일** 또는 **주**&#x200B;로 설정할 수 있습니다. 여러 인제스트에 대해 데이터 플로우를 예약하는 경우 모든 인제스트 사이의 시간 프레임을 설정하는 간격을 설정해야 합니다. 예를 들어, 수집 빈도가 **시간**&#x200B;으로 설정되고 간격이 **15**&#x200B;으로 설정된 경우 데이터 흐름이 **15시간**&#x200B;마다 데이터를 수집하도록 예약되어 있습니다.

이 단계에서 **백필**&#x200B;을 사용하도록 설정하고 데이터의 증분 수집에 대한 열을 정의할 수도 있습니다. 채우기 기능은 내역 데이터를 수집하는 데 사용되며 증분 수집을 위해 정의하는 열은 새 데이터를 기존 데이터와 구별할 수 있도록 합니다.
...
![미리 보기 매핑 옵션이 선택된 드롭다운 창.](../../images/tutorials/templates/preview.png)

소스 데이터 필드와 대상 스키마 필드 간의 매핑 관계를 검사할 수 있는 미리 보기 페이지가 나타납니다. 데이터 플로우의 매핑을 본 후 **[!UICONTROL Got it.]** 선택

![매핑 미리 보기 창.](../../images/tutorials/templates/preview-mappings.png)

실행 후 언제든지 데이터 플로우를 업데이트할 수 있습니다. 데이터 흐름 이름 옆의 생략 부호(`...`)를 선택한 다음 **[!UICONTROL Update dataflow]**&#x200B;을(를) 선택합니다. 부분 수집, 오류 진단 및 경고 알림에 대한 설정과 데이터 흐름의 매핑을 포함하여 데이터 흐름 세부 정보를 업데이트할 수 있는 소스 워크플로우 페이지로 이동합니다.
...
>[!NOTE]
>
>언제든지 **[!UICONTROL Save as draft]**&#x200B;을(를) 선택하여 중지하고 데이터 흐름에 대한 변경 내용을 저장할 수 있습니다.
