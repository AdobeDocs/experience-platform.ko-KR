...
다음 요청은 개인 끝점을 사용하면서 [!DNL Azure Blob Storage]에 대해 인증된 기본 연결을 만듭니다.
...
| `auth.params.connectionString` | [!DNL Azure Blob Storage] 연결 문자열입니다. 자세한 내용은 [[!DNL Azure Blob Storage] API 인증 안내서](../api/create/cloud-storage/blob.md)를 참조하십시오. |
| `auth.params.usePrivateLink` | 개인 끝점을 사용하는지 여부를 결정하는 부울 값입니다. 개인 끝점을 사용하는 경우 이 값을 `true`(으)로 설정하십시오. |
| `connectionSpec.id` | 연결 사양 ID [!DNL Azure Blob Storage]입니다. |
| `connectionSpec.version` | [!DNL Azure Blob Storage] 연결 사양 ID의 버전입니다. |
