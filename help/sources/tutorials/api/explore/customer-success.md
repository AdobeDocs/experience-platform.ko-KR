...
# 를 사용하여 고객 성공 시스템 살펴보기 [!DNL Flow Service] API
...
이 자습서에서는 [!DNL Flow Service] CS(Customer Success) 시스템을 살펴보기 위한 API입니다.
...
다음 섹션에서는 를 사용하여 CS 시스템에 성공적으로 연결하기 위해 알고 있어야 하는 추가 정보를 제공합니다. [!DNL Flow Service] API.

### 기본 연결 받기

CS 시스템을 살펴보려면 [!DNL Platform] API에 올바른 기본 연결 ID가 있어야 합니다. 작업하려는 CS 시스템에 대한 기본 연결이 아직 없는 경우 다음 자습서를 통해 기본 연결을 만들 수 있습니다.
...
성공적인 응답은 CS 시스템의 테이블 배열을 반환합니다. 가져올 테이블을 찾으십시오 [!DNL Platform] 그리고 그것을 주목하세요 `path` 속성을 확인하기 위해 다음 단계에서 제공해야 하므로 속성을 검사합니다.
...
CS 시스템에서 테이블의 구조를 검사하려면 테이블의 경로를 쿼리 매개 변수로 지정하는 동안 GET 요청을 수행합니다.
...
이 자습서를 통해 CS 시스템을 살펴보고 수집할 테이블의 경로를 찾았습니다 [!DNL Platform], 및에서 해당 구조에 대한 정보를 얻습니다. 이 정보는 다음 자습서에서 사용할 수 있습니다 [CS 시스템에서 데이터를 수집하여 플랫폼으로 가져오기](../collect/customer-success.md).
