...
이 자습서는 [!DNL Flow Service] API를 사용하여 고객 성공(CS) 시스템을 탐색합니다.
...
다음 섹션에서는 [!DNL Flow Service] API를 사용하여 CS 시스템에 성공적으로 연결하려면 알아야 할 추가 정보를 제공합니다.

### 기본 연결 받기

[!DNL Platform] API를 사용하여 CS 시스템을 탐색하려면 유효한 기본 연결 ID를 보유하고 있어야 합니다. 작업할 CS 시스템에 대한 기본 연결이 아직 없는 경우 다음 자습서를 통해 기본 연결을 생성할 수 있습니다.
...
CS 시스템의 기본 연결을 사용하여 GET 요청을 수행하여 데이터 테이블을 탐색할 수 있습니다. 다음 호출을 사용하여 [!DNL Platform]에 검사 또는 인제스트할 테이블의 경로를 찾습니다.
...
성공적인 응답으로 CS 시스템에서 표 배열을 반환합니다. [!DNL Platform]에 가져올 테이블을 찾고 해당 `path` 속성을 적어 구조를 검사하기 위해 다음 단계에서 제공해야 합니다.
...
이 튜토리얼을 따라 CS 시스템을 살펴보고, [!DNL Platform]에 인제스트할 테이블의 경로를 찾은 후 해당 구조에 대한 정보를 얻습니다. 다음 튜토리얼에서 이 정보를 사용하여 CS 시스템에서 데이터를 수집하고 Platform](../collect/customer-success.md)로 가져올 수 있습니다.[
