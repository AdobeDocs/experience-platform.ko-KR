...
# Flow Service API를 사용하여 결제 시스템 살펴보기
...
이 자습서는 Flow Service API를 사용하여 결제 애플리케이션을 탐색합니다.
...
이 자습서에서는 데이터를 인제스트할 서드파티 결제 애플리케이션과 유효한 연결이 필요합니다. 유효한 연결에는 응용 프로그램의 연결 사양 ID 및 연결 ID가 포함됩니다. 결제 연결 만들기 및 이러한 값 검색에 대한 자세한 내용은 결제 소스를 플랫폼에 [연결 자습서를 참조하십시오](../../api/create/payments/paypal.md) .
...
결제 시스템에 대한 연결 ID를 사용하여 GET 요청을 수행하여 데이터 테이블을 탐색할 수 있습니다. 다음 호출을 사용하여 Platform에서 검사하거나 인제스트할 테이블의 경로를 찾습니다.
...
성공적인 응답은 결제 시스템에서 테이블 배열을 반환합니다. Platform(플랫폼)으로 가져올 표를 찾아 다음 단계에서 제공해야 하므로 해당 `path` 속성을 기록해 두십시오.
...
이 튜토리얼을 따라 결제 시스템을 살펴보고 플랫폼에 인제스트할 테이블의 경로를 확인했으며 해당 구조에 대한 정보를 얻었습니다. 다음 튜토리얼에서 이 정보를 사용하여 결제 시스템에서 데이터를 [수집하고 Platform으로 가져올 수 있습니다](../collect/payments.md).