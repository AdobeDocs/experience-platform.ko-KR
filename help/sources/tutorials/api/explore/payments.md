...
결제 시스템의 연결 ID를 사용하여 GET 요청을 수행하여 데이터 테이블을 탐색할 수 있습니다. 다음 호출을 사용하여 검사하거나 인제스트할 테이블의 경로를 찾습니다 [!DNL Platform].
...
성공적인 응답은 결제 시스템에서 일련의 테이블을 반환합니다. 가져오려는 테이블을 찾아 다음 단계에서 제공해야 [!DNL Platform] 하므로 해당 `path` 속성을 확인합니다.
