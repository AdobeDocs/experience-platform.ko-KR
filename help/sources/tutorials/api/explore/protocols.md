...
# 다음을 사용하여 프로토콜 시스템 탐색 [!DNL Flow Service] API
...
이 튜토리얼에서는 [!DNL Flow Service] 프로토콜 응용 프로그램을 탐색하기 위한 API입니다.
...
다음 섹션에서는 다음을 사용하여 프로토콜 응용 프로그램에 성공적으로 연결하기 위해 알아야 할 추가 정보를 제공합니다 [!DNL Flow Service] API.

### 기본 연결 얻기

를 사용하여 프로토콜 시스템을 탐색하려면 [!DNL Platform] API를 사용하려면 올바른 기본 연결 ID를 보유해야 합니다. 작업할 프로토콜 시스템에 대한 기본 연결이 없는 경우 다음 자습서를 통해 기본 연결을 만들 수 있습니다.
...
프로토콜 응용 프로그램의 연결 ID를 사용하여 GET 요청을 수행하여 데이터 테이블을 탐색할 수 있습니다. 다음 호출을 사용하여 검사하거나 수집할 테이블의 경로를 찾습니다 [!DNL Platform].
...
성공적인 응답은 프로토콜 응용 프로그램에서 테이블 배열을 반환합니다. 가져올 테이블을 찾습니다. [!DNL Platform] 참고하시기 바랍니다. `path` 속성을 다음 단계에서 제공해야 구조를 검사할 수 있습니다.
...
이 자습서에 따라 프로토콜 애플리케이션을 살펴보고 수집하려는 테이블의 경로를 찾았습니다 [!DNL Platform]및 해당 구조에 대한 정보를 얻었습니다. 다음 자습서에서 이 정보를 사용하여 다음을 수행할 수 있습니다. [프로토콜 응용 프로그램에서 데이터를 수집하고 플랫폼으로 가져옵니다.](../collect/protocols.md).
