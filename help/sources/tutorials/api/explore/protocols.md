...
title: Flow Service API를 사용하여 프로토콜 시스템 살펴보기
topic: overview
...
# Flow Service API를 사용하여 프로토콜 시스템 살펴보기
...
이 자습서는 Flow Service API를 사용하여 프로토콜 애플리케이션을 탐색합니다.
...
다음 섹션에서는 Flow Service API를 사용하여 프로토콜 애플리케이션에 성공적으로 연결하기 위해 알아야 하는 추가 정보를 제공합니다.

### 기본 연결 받기

플랫폼 API를 사용하여 프로토콜 시스템을 탐색하려면 유효한 기본 연결 ID를 보유해야 합니다. 작업할 프로토콜 시스템에 대한 기본 연결이 아직 없는 경우 다음 자습서를 통해 기본 연결을 만들 수 있습니다.
...
프로토콜 응용 프로그램에 대한 연결 ID를 사용하여 GET 요청을 수행하여 데이터 테이블을 탐색할 수 있습니다. 다음 호출을 사용하여 Platform에서 검사하거나 인제스트할 테이블의 경로를 찾습니다.
...
성공적인 응답은 프로토콜 애플리케이션에서 테이블 배열을 반환합니다. Platform(플랫폼)으로 가져올 표를 찾아 다음 단계에서 제공해야 하므로 해당 `path` 속성을 기록해 두십시오.
...
| `{TABLE_PATH}` | 프로토콜 애플리케이션 내의 표 경로입니다. |
...
이 튜토리얼을 따라 프로토콜 애플리케이션을 살펴보고 플랫폼에 인제스트할 테이블의 경로를 확인하고 해당 구조에 대한 정보를 얻었습니다. 다음 자습서에서 이 정보를 사용하여 프로토콜 애플리케이션에서 데이터를 [수집하고 Platform으로 가져올 수 있습니다](../collect/protocols.md).
