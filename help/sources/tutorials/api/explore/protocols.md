...
# API를 사용하여 프로토콜 시스템 [!DNL Flow Service] 살펴보기
...
이 자습서에서는 API를 사용하여 [!DNL Flow Service] 프로토콜 애플리케이션을 살펴봅니다.
...
API를 사용하여 프로토콜 시스템을 [!DNL Platform] 탐색하려면 유효한 기본 연결 ID를 보유해야 합니다. 작업할 프로토콜 시스템에 대한 기본 연결이 아직 없는 경우 다음 자습서를 통해 기본 연결을 생성할 수 있습니다.
...
프로토콜 응용 프로그램에 대한 연결 ID를 사용하여 GET 요청을 수행하여 데이터 테이블을 탐색할 수 있습니다. 다음 호출을 사용하여 검사하거나 인제스트할 테이블의 경로를 찾습니다 [!DNL Platform].
...
| 매개 변수 | 설명 |
| --- | --- |
| `{BASE_CONNECTION_ID}` | 프로토콜 기반 연결의 ID입니다. |
...
성공적인 응답은 프로토콜 애플리케이션에서 테이블 배열을 반환합니다. 가져오려는 테이블을 찾아 다음 단계에서 제공해야 [!DNL Platform] 하므로 해당 `path` 속성을 확인합니다.
...
프로토콜 애플리케이션에서 테이블 구조를 검사하려면 테이블의 경로를 쿼리 매개 변수로 지정하는 동안 GET 요청을 수행합니다.
...
이 튜토리얼을 따라 프로토콜 애플리케이션을 살펴보고 인제스트할 테이블의 경로를 찾은 [!DNL Platform]후 해당 구조에 대한 정보를 얻습니다. 다음 자습서에서 이 정보를 사용하여 프로토콜 애플리케이션에서 데이터를 [수집하고 Platform으로 가져올 수 있습니다](../collect/protocols.md).
