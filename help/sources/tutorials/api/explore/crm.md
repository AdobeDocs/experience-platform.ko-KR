...
# 다음을 사용하여 CRM 시스템 탐색 [!DNL Flow Service] API
...
이 튜토리얼에서는 [!DNL Flow Service] CRM 시스템을 탐색하기 위한 API입니다.
...
다음 섹션에서는 를 사용하여 CRM 시스템에 성공적으로 연결하기 위해 알아야 할 추가 정보를 제공합니다 [!DNL Flow Service] API.

### 연결 ID 만들기

을 사용하여 CRM 시스템을 탐색하려면 [!DNL Platform] API를 사용하려면 올바른 연결 ID를 보유해야 합니다. 함께 작업할 CRM 시스템에 대한 연결이 없는 경우 다음 튜토리얼을 통해 연결을 만들 수 있습니다.
...
성공적인 응답은 CRM 시스템에 대한 표의 배열입니다. 가져올 테이블을 찾습니다. [!DNL Platform] 참고하시기 바랍니다. `path` 속성을 다음 단계에서 제공해야 구조를 검사할 수 있습니다.
...
이 자습서에 따라 CRM 시스템을 탐색하고 가져올 테이블의 경로를 찾았습니다 [!DNL Platform]및 해당 구조에 대한 정보를 얻었습니다. 다음 자습서에서 이 정보를 사용하여 다음을 수행할 수 있습니다. [crm 시스템에서 데이터를 수집하고 플랫폼으로 가져옵니다.](../collect/crm.md).
