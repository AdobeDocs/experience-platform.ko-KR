...
title: Flow Service API를 사용하여 CRM 시스템 살펴보기
topic: overview
...
# Flow Service API를 사용하여 CRM 시스템 살펴보기
...
이 자습서는 Flow Service API를 사용하여 CRM 시스템을 탐색합니다.
...
플랫폼 API를 사용하여 CRM 시스템을 탐색하려면 유효한 기본 연결 ID를 보유해야 합니다. 작업할 CRM 시스템에 대한 기본 연결이 아직 없는 경우 다음 자습서를 통해 기본 연결을 만들 수 있습니다.
...
CRM 시스템의 기본 연결을 사용하여 GET 요청을 수행하여 데이터 테이블을 탐색할 수 있습니다. 다음 호출을 사용하여 Platform에서 검사하거나 인제스트할 테이블의 경로를 찾습니다.
...
성공적인 응답은 CRM 시스템에 대한 일련의 테이블입니다. Platform(플랫폼)으로 가져올 표를 찾아 다음 단계에서 제공해야 하므로 해당 `path` 속성을 기록해 두십시오.
...
이 튜토리얼을 따라 CRM 시스템을 살펴보고 플랫폼으로 가져올 테이블의 경로를 확인했으며 해당 구조에 대한 정보를 얻을 수 있습니다. 다음 튜토리얼에서 이 정보를 사용하여 CRM 시스템에서 데이터를 [수집하여 플랫폼으로 가져올 수 있습니다](../collect/crm.md).