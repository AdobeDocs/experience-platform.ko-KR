...
* [[!DNL Experience Data Model (XDM) System]](../../../../xdm/home.md):Experience Platform이 고객 경험 데이터를 구성하는 표준화된 프레임워크입니다.
   ...
   * [스키마 레지스트리 API](../../../../xdm/api/getting-started.md):스키마 레지스트리 API에 대한 호출을 성공적으로 수행하는 방법을 알아봅니다. 여기에는 `{TENANT_ID}`, &quot;컨테이너&quot; 개념 및 요청 수행에 필요한 머리글이 포함됩니다(수락 헤더와 가능한 값에 특히 유의함).
...
다음 섹션에서는 [!DNL Flow Service] API를 사용하여 **[!UICONTROL eCommerce]** 시스템에 성공적으로 연결하려면 알아야 할 추가 정보를 제공합니다.
...
| `connectionSpec.id` | **[!UICONTROL eCommerce]** 소스의 연결 사양 ID. |
...
데이터 플로우는 소스에서 데이터를 수집하고 [!DNL Platform]으로 가져와야 합니다. 데이터 흐름을 만들려면 먼저 [!DNL Flow Service] API에 대한 GET 요청을 수행하여 데이터 흐름 사양을 구해야 합니다. 데이터 흐름 사양은 **[!UICONTROL eCommerce]** 소스에서 데이터를 수집하는 책임을 집니다.
