...
# 다음을 사용하여 전자 상거래 소스에 대한 데이터 흐름 만들기 [!DNL Flow Service] API

이 튜토리얼에서는 전자 상거래 소스에서 데이터를 검색하고 이를 사용하여 플랫폼으로 가져오는 단계를 설명합니다. [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
* [[!DNL Experience Data Model (XDM) System]](../../../../xdm/home.md): Experience Platform이 고객 경험 데이터를 구성하는 표준화된 프레임워크입니다.
   ...
   * [스키마 레지스트리 API](../../../../xdm/api/getting-started.md): 스키마 레지스트리 API에 대한 호출을 성공적으로 수행하는 방법을 알아봅니다. 여기에는 다음 항목이 포함됩니다. `{TENANT_ID}`, &quot;컨테이너&quot;의 개념 및 요청을 하는 데 필요한 헤더(Accept 헤더 및 가능한 값에 특별한 주의 필요).
* [[!DNL Catalog Service]](../../../../catalog/home.md): 카탈로그는 데이터 위치 및 내부 계보에 대한 레코드 시스템입니다 [!DNL Experience Platform].
* [[!DNL Batch ingestion]](../../../../ingestion/batch-ingestion/overview.md): 일괄 처리 수집 API를 사용하여 데이터를으로 수집할 수 있습니다 [!DNL Experience Platform] 배치 파일로.
...
성공적인 응답은 고유 식별자()를 반환합니다.`id`)을 참조하십시오. 이 ID는 대상 연결을 만드는 이후 단계에서 필요합니다.
...
성공적인 응답은 새 타겟 연결의 고유 식별자( )를 반환합니다.`id`). 이 값은 데이터 흐름을 만들기 위해 이후 단계에서 필요합니다.
...
데이터 흐름은 소스에서 데이터를 수집하고 가져옵니다. [!DNL Platform]. 데이터 흐름을 만들려면 먼저 다음에 대한 GET 요청을 수행하여 데이터 흐름 사양을 얻어야 합니다 [!DNL Flow Service] API. 데이터 흐름 사양은 전자 상거래 소스에서 데이터를 수집하는 역할을 합니다.
...
| `scheduleParams.frequency` | 다음 `frequency` 데이터 흐름에서 데이터를 수집합니다. 허용되는 값은 다음과 같습니다. `once`, `minute`, `hour`, `day`, 또는 `week`. |
| `scheduleParams.interval` | 간격은 두 개의 연속 흐름 실행 사이의 기간을 지정합니다. 간격 값은 0이 아닌 정수여야 합니다. 다음과 같은 경우에는 간격이 필요하지 않습니다. `frequency` 다음으로 설정됨 `once` 다음보다 크거나 같아야 합니다. `15` 기타 `frequency` 값. |

**응답**

성공적인 응답은 ID를 반환합니다 `id` 새로 생성된 데이터 흐름의
...
이 자습서에 따라 일정에 따라 데이터 전자 상거래를 수집하는 소스 커넥터를 만들었습니다. 이제 들어오는 데이터를 다운스트림에서 사용할 수 있습니다. [!DNL Platform] 다음과 같은 서비스 [!DNL Real-Time Customer Profile] 및 [!DNL Data Science Workspace]. 자세한 내용은 다음 문서를 참조하십시오.
