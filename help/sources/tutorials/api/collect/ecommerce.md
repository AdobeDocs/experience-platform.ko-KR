...
| `scheduleParams.interval` | 간격은 두 개의 연속 흐름 실행 사이의 기간을 지정합니다. 간격 값은 0이 아닌 정수여야 합니다. `frequency`이(가) `once`(으)로 설정된 경우 간격이 필요하지 않으며 다른 `frequency` 값에 대해 `15`보다 크거나 같아야 합니다. |
