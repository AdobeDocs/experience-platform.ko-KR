---
keywords: Experience Platform;홈;자주 찾는 항목;전자 상거래 데이터 수집;전자 상거래 데이터
solution: Experience Platform
title: 흐름 서비스 API를 사용하여 전자 상거래 소스에 대한 데이터 흐름 만들기
type: Tutorial
description: 이 튜토리얼에서는 타사 전자 상거래 시스템에서 데이터를 검색하고 소스 커넥터 및 API를 사용하여 Experience Platform으로 수집하는 단계를 설명합니다.
...
이 자습서에서는 전자 상거래 소스에서 데이터를 검색하고 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)를 사용하여 Experience Platform으로 가져오는 단계를 다룹니다.
...
데이터 흐름은 원본에서 데이터를 수집하여 [!DNL Experience Platform]&#x200B;(으)로 가져오는 역할을 합니다. 데이터 흐름을 만들려면 먼저 [!DNL Flow Service] API에 대한 GET 요청을 수행하여 데이터 흐름 사양을 얻어야 합니다. 데이터 흐름 사양은 전자 상거래 소스에서 데이터를 수집하는 역할을 합니다.
...
이 자습서에 따라 일정에 따라 데이터 전자 상거래를 수집하는 소스 커넥터를 만들었습니다. 이제 [!DNL Real-Time Customer Profile] 및 [!DNL Data Science Workspace] 등의 다운스트림 [!DNL Experience Platform] 서비스에서 수신 데이터를 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
