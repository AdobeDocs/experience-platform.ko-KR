...
이 자습서에서는 마케팅 자동화 시스템에서 데이터를 검색하고 소스 커넥터 및 API를 통해 플랫폼에 가져오는 단계를 다룹니다.

## 시작하기

이 자습서에서는 파일의 경로 및 구조를 비롯하여 Platform으로 가져오려는 파일에 대한 정보를 필요로 합니다. 이 정보가 없는 경우 이 자습서를 시작하기 전에 Flow Service API를 사용하여 마케팅 자동화 애플리케이션을 [탐색하는](../../api/create/marketing-automation/hubspot.md) 방법에 대한 자습서를 참조하십시오.
...
| 속성 | 설명 |
| -------- | ----------- |
| `baseConnectionId` | 마케팅 자동화 애플리케이션의 연결 ID |
| `data.schema.id` | 임시 `$id` XDM 스키마 |
| `params.path` | 소스 파일의 경로입니다. |
| `connectionSpec.id` | 마케팅 자동화 애플리케이션의 연결 사양 ID. |
...
| `connectionSpec.id` | 마케팅 자동화를 위한 연결 사양 ID. |
...
데이터 흐름(Dataflow)은 소스에서 데이터를 수집한 다음 Platform으로 가져와야 합니다. 데이터 흐름을 만들려면 먼저 마케팅 자동화 데이터 수집을 담당하는 데이터 흐름 사양을 얻어야 합니다.
...
성공적인 응답은 마케팅 자동화 시스템에서 Platform으로 데이터를 가져오는 작업을 담당하는 데이터 흐름 사양의 세부 정보를 반환합니다. 새 데이터 흐름 만들기를 위해 다음 단계에서 필요한 대로 `id` 필드 값을 저장합니다.
...
마케팅 자동화 데이터를 수집하는 마지막 단계는 데이터 흐름 생성을 위한 것입니다. 현재 다음과 같은 필수 값이 준비되었습니다.
...
이 튜토리얼을 따라 예약된 기준으로 마케팅 자동화 시스템에서 데이터를 수집하는 소스 커넥터를 만들었습니다. 이제 실시간 고객 프로필 및 데이터 과학 작업 공간과 같은 다운스트림 플랫폼 서비스에서 들어오는 데이터를 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
