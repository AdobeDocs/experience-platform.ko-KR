...
이 자습서에서는 파일의 경로 및 구조를 포함하여 Platform으로 가져오려는 파일에 대한 정보와 올바른 연결을 통해 서드파티 마케팅 자동화 시스템에 액세스해야 합니다. 이 정보가 없는 경우 이 자습서를 시도하기 전에 [Flow Service API](../explore/marketing-automation.md)를 사용하여 타사 마케팅 자동화 시스템 탐색 자습서를 참조하십시오.
...
다음 섹션에서는 [!DNL Flow Service] API를 사용하여 마케팅 자동화 시스템에 성공적으로 연결하기 위해 알고 있어야 하는 추가 정보를 제공합니다.
...
성공적인 응답은 해당 고유 식별자(`id`)를 포함하여 새로 생성된 매핑의 세부 정보를 반환합니다. 이 값은 데이터 흐름을 만드는 이후 단계에서 필요하므로 저장합니다.
...
이 자습서에 따라 예약된 대로 마케팅 자동화 시스템에서 데이터를 수집하기 위한 소스 커넥터를 만들었습니다. 이제 들어오는 데이터를 [!DNL Real-time Customer Profile] 및 [!DNL Data Science Workspace] 등의 다운스트림 Platform 서비스에서 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
