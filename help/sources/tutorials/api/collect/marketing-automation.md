...
이 자습서에서는 마케팅 자동화 시스템에서 데이터를 검색하고 소스 커넥터 및 API를 통해 플랫폼으로 가져오는 절차를 다룹니다.

## 시작하기

이 자습서에서는 파일의 경로 및 구조를 포함하여 플랫폼에 가져오려는 파일에 대한 정보와 유효한 연결을 통해 타사 마케팅 자동화 시스템에 액세스해야 합니다. 이 정보가 없는 경우 이 튜토리얼을 시작하기 전에 Flow Service API를 사용하여 타사 마케팅 자동화 시스템 [을 탐색하는 자습서를](../explore/marketing-automation.md) 참조하십시오.
...
다음 섹션에서는 Flow Service API를 사용하여 마케팅 자동화 시스템에 성공적으로 연결하기 위해 알아야 하는 추가 정보를 제공합니다.
...
소스 데이터를 대상 데이터 세트에 수집하려면 먼저 대상 데이터 세트가 준수하는 대상 스키마에 매핑해야 합니다. 이것은 요청 페이로드 내에 정의된 데이터 매핑과 함께 전환 서비스 API에 대한 POST 요청을 수행하여 얻습니다.
...
데이터 프롤은 소스에서 데이터를 수집하고 플랫폼으로 가져와야 합니다. 데이터 흐름을 만들려면 먼저 마케팅 자동화 데이터 수집을 담당하는 데이터 흐름 사양을 얻어야 합니다.
...
성공적인 응답은 마케팅 자동화 시스템에서 Platform으로 데이터를 가져오는 작업을 담당하는 데이터 흐름 세부 항목의 세부 정보를 반환합니다. 새 데이터 흐름 `id` 을 만들려면 다음 단계에서 필요한 대로 필드 값을 저장합니다.
...
이 자습서에 따라, 예약된 기준에 따라 마케팅 자동화 시스템에서 데이터를 수집하는 소스 커넥터를 만들었습니다. 이제 실시간 고객 프로필 및 데이터 과학 작업 공간과 같은 다운스트림 플랫폼 서비스에서 들어오는 데이터를 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
