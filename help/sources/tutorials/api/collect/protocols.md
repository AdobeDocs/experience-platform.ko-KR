...
이 자습서에서는 테이블의 경로 및 구조를 비롯하여 Platform으로 가져오려는 파일에 대한 정보와 유효한 기본 연결을 통해 프로토콜 시스템에 액세스해야 합니다. 이 정보가 없는 경우 이 튜토리얼을 시작하기 전에 [Flow Service API](../explore/protocols.md)를 사용하여 프로토콜 시스템 탐색을 참조하십시오.
...
다음 섹션에서는 [!DNL Flow Service] API를 사용하여 프로토콜 애플리케이션에 성공적으로 연결하려면 알아야 할 추가 정보를 제공합니다.
...
데이터 플로우는 소스에서 데이터를 수집하고 플랫폼으로 가져오는 작업을 수행합니다. 데이터 흐름을 만들려면 먼저 프로토콜 애플리케이션에서 데이터를 수집하는 작업을 담당하는 데이터 흐름 사양을 입수해야 합니다.
...
| `transformations.params.deltaColum` | 새 데이터와 기존 데이터를 구분하는 데 사용되는 지정된 열. 선택한 열의 타임스탬프를 기반으로 증분 데이터를 인제스트합니다. 제네릭 OData를 사용할 때 `deltaColumn`에 지원되는 형식은 `yyyy-MM-ddTHH:mm:ssZ`입니다. |
...
이 튜토리얼을 따라 예약된 기준에 따라 프로토콜 애플리케이션에서 데이터를 수집하는 소스 커넥터를 만들었습니다. 이제 수신 데이터를 [!DNL Real-time Customer Profile] 및 [!DNL Data Science Workspace] 등의 다운스트림 플랫폼 서비스에서 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
