...
# 프로토콜 소스에 대한 데이터 흐름 만들기 [!DNL Flow Service] API

이 튜토리얼에서는 프로토콜 소스에서 데이터를 검색하고 다음을 사용하여 플랫폼에 가져오는 단계를 다룹니다. [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
| `transformations.params.deltaColum` | 새 데이터와 기존 데이터를 구분하는 데 사용되는 지정된 열입니다. 증분 데이터는 선택한 열의 타임스탬프를 기반으로 수집됩니다. 에 지원되는 형식 `deltaColumn` 일반 OData 사용 시 `yyyy-MM-ddTHH:mm:ssZ`. |
...
이 자습서에 따라 일정에 따라 프로토콜 응용 프로그램에서 데이터를 수집하는 소스 커넥터를 만들었습니다. 이제 다음과 같은 다운스트림 플랫폼 서비스에서 수신 데이터를 사용할 수 있습니다. [!DNL Real-Time Customer Profile] 및 [!DNL Data Science Workspace]. 자세한 내용은 다음 문서를 참조하십시오.
