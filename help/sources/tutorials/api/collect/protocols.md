...
이 자습서에서는 타사 프로토콜 애플리케이션에서 데이터를 검색하고 소스 커넥터 및 [[!DNL Flow Service]](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/flow-service.yaml) API를 통해 Adobe Experience Platform으로 수집하는 단계를 설명합니다.

## 시작하기

이 자습서에서는 올바른 기본 연결을 통해 프로토콜 시스템에 액세스해야 하며 테이블의 경로 및 구조를 포함하여 플랫폼에 가져올 파일에 대한 정보를 입력해야 합니다. 이 정보가 없는 경우 이 자습서를 시도하기 전에 [Flow Service API](../explore/protocols.md)를 사용하여 프로토콜 시스템 탐색 자습서를 참조하십시오.
...
다음 섹션에서는 [!DNL Flow Service] API를 사용하여 프로토콜 애플리케이션에 성공적으로 연결하기 위해 알고 있어야 하는 추가 정보를 제공합니다.
...
| `connectionSpec.id` | 프로토콜 응용 프로그램의 연결 사양 ID입니다. |
...
데이터 흐름은 소스에서 데이터를 수집하여 플랫폼으로 가져와야 합니다. 데이터 흐름을 만들려면 먼저 프로토콜 애플리케이션에서 데이터를 수집하는 책임이 있는 데이터 흐름 사양을 가져와야 합니다.
...
| `transformations.params.deltaColum` | 새 데이터와 기존 데이터를 구분하는 데 사용되는 지정된 열입니다. 증분 데이터는 선택한 열의 타임스탬프를 기반으로 수집됩니다. 일반 OData를 사용할 때 `deltaColumn`에 지원되는 형식은 `yyyy-MM-ddTHH:mm:ssZ`입니다. |
...
이 자습서에 따라 예약된 대로 프로토콜 애플리케이션에서 데이터를 수집하기 위한 소스 커넥터를 만들었습니다. 이제 들어오는 데이터를 [!DNL Real-time Customer Profile] 및 [!DNL Data Science Workspace] 등의 다운스트림 Platform 서비스에서 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
