...
이 자습서에서는 클라우드 저장소 원본에서 데이터를 검색하고 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)를 사용하여 플랫폼으로 가져오는 단계를 다룹니다.
...
기본 연결 ID, 수집하려는 원본 파일의 경로 및 원본의 해당 연결 사양 ID를 제공하는 동안 [!DNL Flow Service] API의 `sourceConnections` 끝점에 대한 POST 요청을 수행하여 원본 연결을 만들 수 있습니다.
...
| `data.format` | 플랫폼으로 가져올 데이터의 형식입니다. 지원되는 값은 `delimited`, `JSON` 및 `parquet`입니다. |
...
| `data.properties.encoding` | (선택 사항) 데이터를 Platform에 수집할 때 사용할 인코딩 유형을 정의하는 속성입니다. 지원되는 인코딩 유형은 `UTF-8` 및 `ISO-8859-1`입니다. **참고**: `encoding` 매개 변수는 구분된 CSV 파일을 수집할 때만 사용할 수 있습니다. 다른 파일 형식은 기본 인코딩 `UTF-8`을(를) 사용하여 수집됩니다. |
...
이 자습서에 따라 일정에 따라 클라우드 저장소에서 데이터를 수집하는 소스 커넥터를 만들었습니다. 이제 [!DNL Real-Time Customer Profile] 및 [!DNL Data Science Workspace]과(와) 같은 다운스트림 플랫폼 서비스에서 수신 데이터를 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
