...
이 자습서에서는 타사 광고 응용 프로그램에서 데이터를 검색하고 소스 커넥터 및 API를 통해 플랫폼으로 가져오는 단계를 다룹니다.

## 시작하기

이 자습서에서는 파일의 경로 및 구조를 비롯하여 Platform으로 가져오려는 파일에 대한 정보를 필요로 합니다. 이 정보가 없는 경우 이 튜토리얼을 시작하기 전에 Flow Service API를 [사용하여 광고 애플리케이션을 둘러보는](../../api/create/advertising/ads.md) 방법에 대한 자습서를 참조하십시오.
...
다음 섹션에서는 Flow Service API를 사용하여 광고 시스템에 성공적으로 연결하기 위해 알아야 하는 추가 정보를 제공합니다.
...
데이터 프롤은 소스에서 데이터를 수집하고 플랫폼으로 가져와야 합니다. 데이터 흐름을 만들려면 먼저 광고 데이터 수집을 담당하는 데이터 흐름 사양을 입수해야 합니다.
...
성공적인 응답으로 광고 시스템에서 Platform으로 데이터를 가져오는 책임을 지는 데이터 흐름 세부 정보의 세부 정보가 반환됩니다. 새 데이터 흐름 `id` 을 만들려면 다음 단계에서 필요한 대로 필드 값을 저장합니다.
