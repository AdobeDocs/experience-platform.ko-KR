...
type: Tutorial
description: 이 자습서에서는 타사 광고 애플리케이션에서 데이터를 검색하고 소스 커넥터 및 플로우 서비스 API를 사용하여 플랫폼으로 수집하는 단계를 설명합니다.
...
이 자습서에서는 타사 광고 애플리케이션에서 데이터를 검색하고 소스 커넥터 및 [[!DNL Flow Service]](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/flow-service.yaml) API를 통해 Adobe Experience Platform으로 수집하는 단계를 설명합니다.

## 시작하기

이 자습서에서는 파일의 경로 및 구조를 포함하여 Platform으로 가져오려는 파일에 대한 정보를 가지고 있어야 합니다. 이 정보가 없는 경우 이 자습서를 시도하기 전에 [Flow Service API](../../api/create/advertising/ads.md)를 사용하여 광고 애플리케이션을 탐색하는 자습서를 참조하십시오.
...
성공적으로 응답하면 고유 식별자(`$id`)를 포함하여 새로 만든 스키마의 세부 정보가 반환됩니다. 이 ID를 이후 단계에서 필요에 따라 저장하여 대상 데이터 세트, 매핑 및 데이터 흐름을 만듭니다.
...
이 자습서에 따라 예약된 대로 광고 시스템에서 데이터를 수집하기 위한 소스 커넥터를 만들었습니다. 이제 들어오는 데이터를 [!DNL Real-time Customer Profile] 및 [!DNL Data Science Workspace] 등의 다운스트림 Platform 서비스에서 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
