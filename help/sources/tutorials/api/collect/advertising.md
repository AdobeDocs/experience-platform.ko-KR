---
keywords: Experience Platform;홈;인기 항목;흐름 서비스;광고;google adwords;광고
solution: Experience Platform
title: 흐름 서비스 API를 사용하여 Advertising 소스에 대한 데이터 흐름 만들기
type: Tutorial
description: 이 자습서에서는 소스 커넥터 및 플로우 서비스 API를 사용하여 서드파티 광고 애플리케이션에서 데이터를 검색하고 Experience Platform으로 수집하는 단계를 다룹니다.
...
# [!DNL Flow Service] API를 사용하여 광고 소스에 대한 데이터 흐름 만들기

이 자습서에서는 타사 광고 응용 프로그램에서 데이터를 검색하고 소스 커넥터 및 [[!DNL Flow Service]](https://www.adobe.io/experience-platform-apis/references/flow-service/) API를 통해 Adobe Experience Platform으로 수집하는 단계를 다룹니다.

>[!NOTE]
>
>* 데이터 흐름을 만들려면 광고 소스가 있는 유효한 기본 연결 ID가 이미 있어야 합니다. 이 ID가 없는 경우 [소스 개요](../../../home.md#advertising)에서 기본 연결을 만들 수 있는 광고 소스 목록을 참조하십시오.
...
| 속성 | 설명 |
| -------- | ----------- |
| `baseConnectionId` | 액세스 중인 광고 애플리케이션의 고유 연결 ID. |
| `params.path` | 소스 파일의 경로입니다. |
| `connectionSpec.id` | 광고 애플리케이션과 연계된 연결 사양 ID. |
...
데이터 흐름은 소스에서 데이터를 수집하고 이를 Experience Platform으로 가져오는 역할을 합니다. 데이터 흐름을 만들려면 먼저 광고 데이터 수집을 담당하는 데이터 흐름 사양을 구해야 합니다.
...
이 자습서에 따라 일정에 따라 광고 시스템에서 데이터를 수집하는 소스 커넥터를 만들었습니다. 이제 [!DNL Real-Time Customer Profile] 및 [!DNL Data Science Workspace]과(와) 같은 다운스트림 Experience Platform 서비스에서 수신 데이터를 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
