...
이 자습서에서는 CRM 시스템에서 데이터를 검색하고 소스 커넥터 및 API를 통해 플랫폼에 가져오는 단계를 다룹니다.

## 시작하기

이 자습서에서는 테이블의 경로 및 구조를 포함하여 플랫폼에 가져오려는 표에 대한 정보와 유효한 기본 연결을 통해 CRM 시스템에 액세스해야 합니다. 이 정보가 없는 경우 이 튜토리얼을 시작하기 전에 Flow Service API를 사용하여 CRM 시스템을 [살펴보는](../explore/crm.md) 자습서를 참조하십시오.
...
다음 섹션에서는 Flow Service API를 사용하여 CRM 시스템에 성공적으로 연결하기 위해 알아야 하는 추가 정보를 제공합니다.
...
| 속성 | 설명 |
| --- | --- |
| `baseConnectionId` | CRM 시스템에 대한 기본 연결의 ID입니다. |
...
| `connectionSpec.id` | CRM의 연결 사양 ID입니다. |
...
데이터 흐름(Dataflow)은 소스에서 데이터를 수집한 다음 Platform으로 가져와야 합니다. 데이터 흐름을 만들려면 먼저 CRM 데이터 수집을 담당하는 데이터 흐름 사양을 얻어야 합니다.
...
성공적인 응답은 CRM 시스템에서 Platform으로 데이터를 가져오는 작업을 담당하는 데이터 흐름 세부 정보를 반환합니다. 새 데이터 흐름 만들기를 위해 다음 단계에서 필요한 대로 `id` 필드 값을 저장합니다.
...
CRM 데이터 수집을 위한 마지막 단계는 데이터 흐름을 만드는 것입니다. 현재 다음과 같은 필수 값이 준비되었습니다.
...
이 튜토리얼을 따라 CRM 시스템에서 데이터를 수집하기 위한 소스 커넥터를 만들었습니다. 이제 실시간 고객 프로필 및 데이터 과학 작업 공간과 같은 다운스트림 플랫폼 서비스에서 들어오는 데이터를 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
