...
title: 흐름 서비스 API를 사용하여 CRM 소스의 데이터 흐름 만들기
type: Tutorial
...
# 다음을 사용하여 CRM 소스의 데이터 흐름 만들기 [!DNL Flow Service] API

이 튜토리얼에서는 CRM 소스에서 데이터를 검색하고 다음을 사용하여 이를 플랫폼으로 가져오는 단계를 설명합니다. [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

>[!NOTE]
>
>* 데이터 흐름을 만들려면 CRM 원본과의 유효한 기본 연결 ID가 이미 있어야 합니다. 이 ID가 없는 경우 [소스 개요](../../../home.md#customer-relationship-management) 기본 연결을 만들 수 있는 CRM 소스 목록의 경우.
...
| 속성 | 설명 |
| --- | --- |
| `baseConnectionId` | 액세스하고 있는 타사 CRM 시스템의 고유 연결 ID입니다. |
| `params.path` | 소스 파일의 경로입니다. |
| `connectionSpec.id` | 특정 타사 CRM 시스템과 연결된 연결 사양 ID입니다. 다음을 참조하십시오. [부록](#appendix) 연결 사양 ID 목록. |
...
데이터 흐름은 소스에서 데이터를 수집하고 플랫폼으로 가져오는 역할을 합니다. 데이터 흐름을 만들려면 먼저 CRM 데이터 수집을 담당하는 데이터 흐름 사양을 얻어야 합니다.
...
CRM 데이터를 수집하는 마지막 단계는 데이터 흐름을 만드는 것입니다. 이제 다음 필수 값이 준비되었습니다.
...
| `transformations.params.deltaColum` | 새 데이터와 기존 데이터를 구분하는 데 사용되는 지정된 열입니다. 증분 데이터는 선택한 열의 타임스탬프를 기반으로 수집됩니다. 에 지원되는 형식 `deltaColumn` 은(는) `yyyy-MM-dd HH:mm:ss`. Microsoft Dynamics를 사용하는 경우 지원되는 형식 `deltaColumn` 은(는) `yyyy-MM-ddTHH:mm:ssZ`. |
...
이 자습서에 따라 일정에 따라 CRM 시스템에서 데이터를 수집하는 소스 커넥터를 만들었습니다. 이제 다음과 같은 다운스트림 플랫폼 서비스에서 수신 데이터를 사용할 수 있습니다. [!DNL Real-Time Customer Profile] 및 [!DNL Data Science Workspace]. 자세한 내용은 다음 문서를 참조하십시오.
...
다음 섹션에서는 다양한 CRM 소스 커넥터와 해당 연결 사양을 나열합니다.
