...
이 자습서에서는 플랫폼에 가져올 파일(파일의 경로 및 구조 포함)에 대한 정보는 물론 유효한 연결을 통해 결제 응용 프로그램에 액세스해야 합니다. 이 정보가 없는 경우 이 튜토리얼을 시작하기 전에 [Flow Service API](../explore/payments.md)를 사용하여 결제 응용 프로그램 탐색을 참조하십시오.
...
다음 섹션에서는 [!DNL Flow Service] API를 사용하여 결제 애플리케이션에 성공적으로 접속하기 위해 알아야 할 추가 정보를 제공합니다.
...
| 속성 | 설명 |
| -------- | ----------- |
| `baseConnectionId` | 액세스하는 제3자 결제 애플리케이션의 고유한 연결 ID. |
...
소스 데이터를 대상 데이터 세트에 수집하려면 먼저 대상 데이터 세트가 준수하는 대상 스키마에 매핑해야 합니다. 이것은 요청 페이로드 내에 정의된 데이터 매핑과 함께 [전환 서비스 API](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/mapping-service-api.yaml)에 대한 POST 요청을 수행하여 수행됩니다.
...
데이터 플로우는 소스에서 데이터를 수집하고 플랫폼으로 가져오는 작업을 수행합니다. 데이터 흐름을 만들려면 먼저 [!DNL Flow Service] API에 대한 GET 요청을 수행하여 데이터 흐름 사양을 구해야 합니다. 데이터 흐름 사양은 외부 데이터베이스 또는 NoSQL 시스템에서 데이터를 수집하는 책임을 집니다.
...
| `transformations.params.deltaColum` | 새 데이터와 기존 데이터를 구분하는 데 사용되는 지정된 열. 선택한 열의 타임스탬프를 기반으로 증분 데이터를 인제스트합니다. `deltaColumn`에 대해 지원되는 날짜 형식은 `yyyy-MM-dd HH:mm:ss`입니다. |
...
성공적인 응답은 새로 만든 데이터 흐름 중 ID `id`을 반환합니다.
...
이 튜토리얼을 따라 예약된 기준에 따라 결제 애플리케이션에서 데이터를 수집하는 소스 커넥터를 만들었습니다. 이제 수신 데이터를 [!DNL Real-time Customer Profile] 및 [!DNL Data Science Workspace] 등의 다운스트림 플랫폼 서비스에서 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
