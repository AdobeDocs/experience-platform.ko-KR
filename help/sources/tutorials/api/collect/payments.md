...
이 자습서에서는 타사 결제 애플리케이션에서 데이터를 검색하고 소스 커넥터 및 [[!DNL Flow Service]](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/flow-service.yaml) API를 통해 플랫폼으로 수집하는 단계를 다룹니다.

## 시작하기

이 자습서에서는 플랫폼으로 가져올 파일에 대한 정보(파일의 경로 및 구조 포함)와 유효한 연결을 통해 결제 애플리케이션에 액세스해야 합니다. 이 정보가 없는 경우 이 자습서를 시작하기 전에 [Flow Service API](../explore/payments.md)를 사용하여 결제 애플리케이션 탐색을 참조하십시오.
...
다음 섹션에서는 [!DNL Flow Service] API를 사용하여 결제 애플리케이션에 성공적으로 접속하기 위해 알아야 하는 추가 정보를 제공합니다.
...
| 속성 | 설명 |
| -------- | ----------- |
| `baseConnectionId` | 액세스하는 타사 지급 애플리케이션의 고유 연결 ID입니다. |
| `params.path` | 소스 파일의 경로입니다. |
| `connectionSpec.id` | 결제 애플리케이션의 연결 사양 ID입니다. |
...
이 자습서에 따라 스케줄링된 기준에 따라 지급 애플리케이션에서 데이터를 수집하기 위한 소스 커넥터를 만들었습니다. 이제 들어오는 데이터를 [!DNL Real-time Customer Profile] 및 [!DNL Data Science Workspace] 등의 다운스트림 Platform 서비스에서 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
