...
title: 흐름 서비스 API를 사용하여 결제 소스에 대한 데이터 흐름 만들기
type: Tutorial
description: 이 자습서에서는 결제 애플리케이션에서 데이터를 검색하고 소스 커넥터 및 API를 사용하여 플랫폼으로 수집하는 단계를 다룹니다.
...
# 다음을 사용하여 결제 소스에 대한 데이터 흐름 만들기 [!DNL Flow Service] API

이 자습서에서는 결제 소스에서 데이터를 검색하고 다음을 사용하여 플랫폼으로 가져오는 단계를 다룹니다. [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

>[!NOTE]
>
>데이터 흐름을 만들려면 이미 결제 출처가 있는 유효한 기본 연결 ID가 있어야 합니다. 이 ID가 없는 경우 [소스 개요](../../../home.md#payments) 기본 연결을 생성할 수 있는 결제 소스 목록에 대해 설명합니다.
...
| 속성 | 설명 |
| -------- | ----------- |
| `baseConnectionId` | 액세스 중인 서드파티 결제 애플리케이션의 고유 연결 ID. |
...
이 자습서에 따라 일정에 따라 결제 애플리케이션에서 데이터를 수집하는 소스 커넥터를 만들었습니다. 이제 다음과 같은 다운스트림 플랫폼 서비스에서 수신 데이터를 사용할 수 있습니다. [!DNL Real-Time Customer Profile] 및 [!DNL Data Science Workspace]. 자세한 내용은 다음 문서를 참조하십시오.
