...
# 소스 커넥터 및 API를 사용하여 고객 성공 시스템에서 데이터를 수집합니다

이 자습서에서는 타사 고객 성공 시스템에서 데이터를 검색하고 소스 커넥터 및 [[!DNL Flow Service]](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/flow-service.yaml) API를 통해 [!DNL Platform]로 수집하는 단계를 설명합니다.

## 시작하기

이 자습서에서는 파일의 경로 및 구조를 포함하여 [!DNL Platform]에 가져올 파일에 대한 정보와 올바른 연결을 통해 타사 고객 성공 시스템에 액세스해야 합니다. 이 정보가 없는 경우 이 자습서를 시도하기 전에 Flow Service API](../explore/customer-success.md)를 사용하여 데이터베이스 또는 NoSQL 시스템 탐색에 대한 자습서를 참조하십시오.[
...
다음 섹션에서는 [!DNL Flow Service] API를 사용하여 고객 성공 시스템에 성공적으로 연결하기 위해 알고 있어야 하는 추가 정보를 제공합니다.
...
모든 테이블 기반 커넥터의 경우 값 `tabular`을 설정합니다.
...
| `connectionSpec.id` | 특정 타사 고객 성공 시스템과 연결된 연결 사양 ID입니다. 연결 사양 ID 목록은 [부록](#appendix)을 참조하십시오. |
...
소스 데이터를 대상 데이터 세트에 수집하려면 먼저 대상 데이터 세트가 준수하는 대상 스키마에 매핑해야 합니다. 이 작업은 요청 페이로드 내에 정의된 데이터 매핑으로 [!DNL Conversion Service] API에 대한 POST 요청을 수행하여 수행됩니다.
...
| `transformations.params.deltaColum` | 새 데이터와 기존 데이터를 구분하는 데 사용되는 지정된 열입니다. 증분 데이터는 선택한 열의 타임스탬프를 기반으로 수집됩니다. `deltaColumn`에 대해 지원되는 날짜 형식은 `yyyy-MM-dd HH:mm:ss`입니다. |
...
이 자습서에 따라 예약된 대로 고객 성공 시스템에서 데이터를 수집하기 위한 소스 커넥터를 만들었습니다. 이제 [!DNL Real-time Customer Profile] 및 [!DNL Data Science Workspace] 등의 다운스트림 [!DNL Platform] 서비스에서 들어오는 데이터를 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
