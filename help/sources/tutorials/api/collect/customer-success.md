...
이 자습서에서는 고객 성공(&quot;CS&quot;) 시스템에서 데이터를 검색하고 이를 소스 커넥터 및 API를 통해 플랫폼으로 인제스트하는 절차를 다룹니다.

## 시작하기

이 자습서를 사용하려면 유효한 기본 연결을 통해 타사 CS 시스템에 액세스해야 하며 파일의 경로 및 구조를 포함하여 플랫폼에 가져오려는 파일에 대한 정보가 필요합니다. 이 정보가 없는 경우 이 자습서를 시작하기 전에 Flow Service API를 사용하여 데이터베이스 또는 NoSQL 시스템을 [탐색하는](../explore/customer-success.md) 자습서를 참조하십시오.
...
| 속성 | 설명 |
| -------- | ----------- |
| `baseConnectionId` | CS 시스템의 기본 연결 ID |
| `data.schema.id` | 임시 `$id` XDM 스키마 |
| `params.path` | 소스 파일의 경로입니다. |
| `connectionSpec.id` | CS 시스템의 연결 사양 ID. |
...
| `connectionSpec.id` | 고객 성공 시스템의 연결 사양 ID. |
...
성공적인 응답은 CS 시스템에서 Platform으로 데이터를 가져오는 작업을 담당하는 데이터 흐름 세부 항목의 세부 정보를 반환합니다. 이 ID는 새 데이터 흐름을 만들려면 다음 단계에서 필요합니다.
...
이 튜토리얼을 따라 소스 커넥터를 만들어 CS 시스템에서 데이터를 정기적으로 수집합니다. 이제 실시간 고객 프로필 및 데이터 과학 작업 공간과 같은 다운스트림 플랫폼 서비스에서 들어오는 데이터를 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
