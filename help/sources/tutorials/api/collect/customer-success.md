---
keywords: Experience Platform;홈;인기 있는 주제;고객 성공 수집;고객 성공
solution: Experience Platform
title: 흐름 서비스 API를 사용하여 고객 성공 소스에 대한 데이터 흐름 만들기
type: Tutorial
description: 이 자습서에서는 고객 성공 시스템에서 데이터를 검색하고 소스 커넥터 및 API를 사용하여 Experience Platform으로 수집하는 단계를 다룹니다.
...
# [!DNL Flow Service] API를 사용하여 고객 성공 소스에 대한 데이터 흐름 만들기

이 자습서에서는 고객 성공 소스에서 데이터를 검색하고 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)를 사용하여 Experience Platform으로 가져오는 단계를 다룹니다.

>[!NOTE]
>
>* 데이터 흐름을 만들려면 고객 성공 소스에 대한 유효한 기본 연결 ID가 이미 있어야 합니다. 이 ID가 없는 경우 [소스 개요](../../../home.md#customer-success)에서 기본 연결을 만들 수 있는 고객 성공 소스 목록을 확인하십시오.
...
모든 테이블 기반 커넥터의 경우 `tabular` 값을 설정하십시오.
...
| 속성 | 설명 |
| -------- | ----------- |
| `baseConnectionId` | 액세스 중인 서드파티 고객 성공 시스템의 고유 연결 ID. |
...
데이터 흐름은 소스에서 데이터를 수집하고 Experience Platform으로 가져오는 역할을 합니다. 데이터 흐름을 만들려면 먼저 흐름 서비스 API에 대한 GET 요청을 수행하여 데이터 흐름 사양을 얻어야 합니다. 데이터 흐름 사양은 서드파티 고객 성공 시스템에서 데이터를 수집하는 역할을 합니다.
...
| `transformations.params.deltaColum` | 새 데이터와 기존 데이터를 구분하는 데 사용되는 지정된 열입니다. 증분 데이터는 선택한 열의 타임스탬프를 기반으로 수집됩니다. `deltaColumn`에 지원되는 날짜 형식은 `yyyy-MM-dd HH:mm:ss`입니다. |
...
이 자습서에 따라 일정에 따라 고객 성공 시스템에서 데이터를 수집하기 위한 소스 커넥터를 만들었습니다. 이제 [!DNL Real-Time Customer Profile] 및 [!DNL Data Science Workspace] 등의 다운스트림 [!DNL Experience Platform] 서비스에서 수신 데이터를 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
