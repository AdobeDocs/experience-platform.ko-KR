...
이 자습서에서는 파일의 경로와 구조를 포함하여 [!DNL Platform]에 가져오려는 파일에 대한 정보와 유효한 연결을 통해 타사 고객 성공 시스템에 액세스해야 합니다. 이 정보가 없는 경우 이 자습서를 시작하기 전에 [Flow Service API](../explore/customer-success.md)를 사용하여 데이터베이스 또는 NoSQL 시스템 탐색에 대한 자습서를 참조하십시오.
...
다음 섹션에서는 [!DNL Flow Service] API를 사용하여 고객 성공 시스템에 성공적으로 연결하기 위해 알아야 하는 추가 정보를 제공합니다.
...
| 속성 | 설명 |
| -------- | ----------- |
| `baseConnectionId` | 액세스하는 제3자 고객 성공 시스템의 고유 연결 ID. |
| `params.path` | 소스 파일의 경로입니다. |
| `connectionSpec.id` | 특정 타사 고객 성공 시스템과 연결된 연결 사양 ID. 연결 사양 ID 목록은 [부록](#appendix)을 참조하십시오. |
...
데이터 플로우는 소스에서 데이터를 수집하고 플랫폼으로 가져오는 작업을 수행합니다. 데이터 흐름을 만들려면 먼저 Flow Service API에 대한 GET 요청을 수행하여 데이터 흐름 사양을 구해야 합니다. 데이터 흐름 사양은 제3자 고객 성공 시스템에서 데이터를 수집하는 책임을 집니다.
...
이 튜토리얼을 따라 소스 커넥터를 만들어 고객 성공 시스템의 데이터를 예약된 시간에 수집할 수 있습니다. 이제 [!DNL Real-time Customer Profile] 및 [!DNL Data Science Workspace] 등의 다운스트림 [!DNL Platform] 서비스에서 들어오는 데이터를 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
