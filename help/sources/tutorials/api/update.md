...
연결을 업데이트하는 첫 번째 단계는 연결 ID를 사용하여 세부 사항을 검색하는 것입니다. 연결의 현재 세부 정보를 검색하려면 업데이트할 연결의 연결 ID를 제공하는 동안 [!DNL Flow Service] API에 GET 요청을 합니다.
...
>[!IMPORTANT]
>
>PATCH 요청을 할 때 `If-Match` 헤더가 필요합니다. 이 헤더의 값은 업데이트할 연결의 고유한 버전입니다.
...
성공적인 응답은 연결 ID와 업데이트된 etag를 반환합니다. 연결 ID를 제공하는 동안 [!DNL Flow Service] API에 대한 GET 요청을 하여 업데이트를 확인할 수 있습니다.
