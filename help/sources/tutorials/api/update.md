...
이 자습서에서는 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)를 사용하여 연결의 세부 사항 및 자격 증명을 업데이트하는 단계를 설명합니다.

## 시작하기

이 자습서에서는 기존 연결과 유효한 연결 ID가 있어야 합니다. 기존 연결이 없는 경우 [소스 개요](../../home.md)에서 선택한 소스를 선택하고 이 자습서를 수행하기 전에 설명된 단계를 따릅니다.
...
연결을 업데이트하는 첫 번째 단계는 연결 ID를 사용하여 세부 사항을 검색하는 것입니다. 연결의 현재 세부 사항을 검색하려면 업데이트할 연결의 연결 ID를 제공하는 동안 [!DNL Flow Service] API에 GET 요청을 하십시오.
...
성공적인 응답은 해당 자격 증명, 고유 식별자(`id`) 및 버전을 포함하여 연결의 현재 세부 정보를 반환합니다. 연결을 업데이트하려면 버전 값이 필요합니다.
...
| 매개 변수 | 설명 |
| --------- | ----------- |
| `{CONNECTION_ID}` | 업데이트할 연결의 고유한 `id` 값입니다. |
...
성공적인 응답은 연결 ID와 업데이트된 태그를 반환합니다. 연결 ID를 제공하는 동안 [!DNL Flow Service] API에 GET 요청을 수행하여 업데이트를 확인할 수 있습니다.
...
이 자습서에 따르면 [!DNL Flow Service] API를 사용하여 연결과 관련된 자격 증명과 정보를 업데이트했습니다. 소스 커넥터 사용에 대한 자세한 내용은 [소스 개요](../../home.md)를 참조하십시오.
