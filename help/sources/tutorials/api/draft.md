...
# 의 초안 만들기 [!DNL Flow Service] api를 사용하는 엔티티

다음을 사용할 수 있습니다. `mode=draft` 의 쿼리 매개 변수 [[!DNL Flow Service] API](<https://www.adobe.io/experience-platform-apis/references/flow-service/>) 을(를) 설정하려면 [!DNL Flow Service] 기본 연결, 소스 연결, 타겟 연결 및 데이터 흐름과 같은 엔티티가 도면 상태로 이동합니다.

초안은 나중에 새 정보로 업데이트한 다음 준비가 되면 다음을 사용하여 게시할 수 있습니다. `op=publish` 쿼리 매개 변수.

이 자습서에서는 를 설정하는 방법에 대한 단계를 제공합니다. [!DNL Flow Service] 엔티티를 도면 상태로 전환하고 나중에 완료할 수 있도록 워크플로우를 일시 중지하고 저장할 수 있습니다.
...
+++Request 다음 요청은 연결 사양 정보를 검색합니다. [!DNL Azure File Storage]:
...
성공한 응답은 소스에 대한 연결 사양 정보를 반환합니다. 소스에 대해 초안 모드가 지원되는지 확인하려면 `items[0].attributes.isDraftModeSupported` 의 값이 `true`.
...
+++요청 다음 요청은 클라우드 스토리지 소스에 대한 플로우 사양 세부 정보를 검색합니다.
...
성공한 응답은 소스에 대한 흐름 사양 정보를 반환합니다. 소스에 대해 초안 모드가 지원되는지 확인하려면 `items[0].attributes.isDraftModeSupported` 의 값이 `true`.
...
POST 초안 기본 연결을 만들려면 `/connections` 의 엔드포인트 [!DNL Flow Service] API 및 제공 `mode=draft` 를 쿼리 매개 변수로 사용하십시오.
...
| 매개변수 | 설명 |
| --- | --- |
| `mode` | 기본 연결의 상태를 결정하는 사용자 제공 쿼리 매개 변수입니다. 기본 연결을 초안으로 설정하려면 를 설정합니다 `mode` 끝 `draft`. |

**요청**

다음 요청은 다음에 대한 기본 연결을 만듭니다. [!DNL Azure File Storage] 소스:
...
초안을 게시할 준비가 되면에 POST 요청을 `/connections` 게시하려는 초안 기본 연결의 ID를 끝점에 추가하고 게시를 위한 작업 작업을 제공합니다.
...
| 매개변수 | 설명 |
| --- | --- |
| `op` | 쿼리된 기본 연결의 상태를 업데이트하는 작업 작업입니다. 구배 베이스 연결을 게시하려면 를 설정합니다. `op` 끝 `publish`. |

**요청**

다음 요청은 다음에 대한 기본 연결 초안을 게시합니다. [!DNL Azure File Storage] 이전 단계에서 만들어졌습니다.
...
POST 초안 소스 연결을 만들려면 `/sourceConnections` 의 엔드포인트 [!DNL Flow Service] API 및 제공 `mode=draft` 를 쿼리 매개 변수로 사용하십시오.
...
| 매개변수 | 설명 |
| --- | --- |
| `mode` | 소스 연결의 상태를 결정하는 사용자 제공 쿼리 매개 변수입니다. 소스 연결을 초안으로 설정하려면 를 설정합니다 `mode` 끝 `draft`. |
...
초안을 게시할 준비가 되면에 POST 요청을 `/sourceConnections` 게시하려는 초안 소스 연결의 ID를 끝점으로 지정하고 입력한 다음 게시하기 위한 작업 작업을 제공합니다.
...
| 매개변수 | 설명 |
| --- | --- |
| `op` | 쿼리된 소스 연결의 상태를 업데이트하는 작업 작업입니다. 초안 소스 연결을 게시하려면 을 설정합니다. `op` 끝 `publish`. |

**요청**

다음 요청은 다음에 대한 초안 소스 연결을 게시합니다. [!DNL Azure File Storage] 이전 단계에서 만들어졌습니다.
...
POST 초안 대상 연결을 만들려면 `/targetConnections` 의 엔드포인트 [!DNL Flow Service] API 및 제공 `mode=draft` 를 쿼리 매개 변수로 사용하십시오.
...
| 매개변수 | 설명 |
| --- | --- |
| `mode` | 대상 연결의 상태를 결정하는 사용자 제공 쿼리 매개 변수입니다. 대상 연결을 초안으로 설정하려면 를 설정합니다 `mode` 끝 `draft`. |
...
초안을 게시할 준비가 되면에 POST 요청을 `/targetConnections` 을(를) 종단하여 게시하려는 초안 타겟 연결의 ID와 함께 게시를 위한 작업 작업을 제공합니다.
...
| 매개변수 | 설명 |
| --- | --- |
| `op` | 쿼리된 대상 연결의 상태를 업데이트하는 작업 작업입니다. 초안 대상 연결을 게시하려면 다음을 설정하십시오. `op` 끝 `publish`. |

**요청**

다음 요청은 다음에 대한 초안 대상 연결을 게시합니다. [!DNL Azure File Storage] 이전 단계에서 만들어졌습니다.
