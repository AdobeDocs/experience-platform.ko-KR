...
topic: overview
type: Tutorial
description: 이 자습서에서는 Flow Service API를 사용하여 PayPal을 Experience Platform에 연결하는 단계를 단계별로 안내합니다.
...
# [!DNL Flow Service] API를 사용하여 [!DNL PayPal] 커넥터를 만듭니다.
...
이 자습서에서는 [!DNL Flow Service] API를 사용하여 [!DNL PayPal]을(를) Experience Platform에 연결하는 단계를 안내합니다.
...
다음 섹션에서는 [!DNL Flow Service] API를 사용하여 [!DNL PayPal]에 성공적으로 연결하려면 알아야 할 추가 정보를 제공합니다.

### 필요한 자격 증명 수집

[!DNL Flow Service]이(가) [!DNL PayPal]에 연결하려면 다음 연결 속성에 대한 값을 제공해야 합니다.

| 자격 증명 | 설명 |
| ---------- | ----------- |
| `host` | [!DNL PayPal] 인스턴스의 URL입니다. (기본값:api.sandbox.paypal.com)에서 다운로드할 수 있습니다. |
| `clientId` | [!DNL PayPal] 응용 프로그램에 연결된 클라이언트 ID. |
| `clientSecret` | [!DNL PayPal] 응용 프로그램과 연결된 클라이언트 암호입니다. |
| `connectionSpec.id` | 연결을 만드는 데 필요한 고유 식별자입니다. [!DNL PayPal]에 대한 연결 사양 ID는 다음과 같습니다.`221c7626-58f6-4eec-8ee2-042b0226f03b` |
...
연결은 소스를 지정하며 해당 소스의 자격 증명을 포함합니다. 다른 데이터를 가져오기 위해 여러 소스 커넥터를 만드는 데 사용할 수 있으므로 [!DNL PayPal] 계정에는 하나의 연결만 필요합니다.
...
[!DNL PayPal] 연결을 만들려면 고유한 연결 사양 ID를 POST 요청의 일부로 제공해야 합니다. [!DNL PayPal]에 대한 연결 사양 ID는 `221c7626-58f6-4eec-8ee2-042b0226f03b`입니다.
...
| `auth.params.clientId` | [!DNL PayPal] 인스턴스와 연결된 클라이언트 ID. |
...
이 자습서를 따라 [!DNL Flow Service] API를 사용하여 [!DNL PayPal] 연결을 만들고 연결의 고유 ID 값을 받았습니다. 이 ID는 다음 튜토리얼에서 Flow Service API](../../explore/payments.md)를 사용하여 [결제 애플리케이션을 탐색하는 방법을 학습할 때 사용할 수 있습니다.
