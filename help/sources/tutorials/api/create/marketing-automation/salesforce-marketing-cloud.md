...
# [!DNL Salesforce Marketing Cloud] API를 사용하여 [!DNL Flow Service]을(를) Experience Platform에 연결
...
[!DNL Salesforce Marketing Cloud]API[[!DNL Flow Service] 를 사용하여 &#x200B;](https://developer.adobe.com/experience-platform-apis/references/flow-service/) 계정을 Adobe Experience Platform에 연결하는 방법을 알아보려면 이 안내서를 참조하십시오.
...
다음 섹션에서는 [!DNL Azure Synapse Analytics] API를 사용하여 [!DNL Flow Service]에 성공적으로 연결하기 위해 알아야 할 추가 정보를 제공합니다.
...
다음 섹션에서는 [!DNL Salesforce Marketing Cloud] API를 사용하여 [!DNL Flow Service]에 성공적으로 연결하기 위해 알아야 할 추가 정보를 제공합니다.
...
## [!DNL Salesforce Marketing Cloud]에서 [!DNL Azure]을(를) Experience Platform에 연결
...
기본 연결 ID를 만들려면 요청 매개 변수에 `/connections` 인증 자격 증명을 포함하여 [!DNL Salesforce Marketing Cloud] 끝점에 POST 요청을 보냅니다.
