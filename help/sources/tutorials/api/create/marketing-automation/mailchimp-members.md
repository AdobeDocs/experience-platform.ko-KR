...
# Flow Service API를 사용하여 [!DNL MailChimp Members]에 대한 데이터 흐름 만들기

다음 자습서에서는 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)를 사용하여 [!DNL MailChimp Members] 데이터를 Platform으로 가져오도록 소스 연결 및 데이터 흐름을 만드는 단계를 안내합니다.
...
이제 [!DNL MailChimp] 인증 자격 증명을 검색한 후 데이터 흐름을 만드는 프로세스를 시작하여 [!DNL MailChimp Members] 데이터를 플랫폼으로 가져올 수 있습니다. 데이터 흐름을 만드는 첫 번째 단계는 기본 연결을 만드는 것입니다.
...
>[!TIP]
>
>`{SOURCE_PARAMS}`에 대해 허용되는 형식 유형을 검색하려면 base64에서 전체 `list_id` 문자열을 인코딩해야 합니다. 예를 들어 base64에서 인코딩된 `"list_id": "10c097ca71"`은 `eyJsaXN0SWQiOiIxMGMwOTdjYTcxIn0=`과 같습니다.
...
| `{SOURCE_PARAMS}` | `list_id`의 base64로 인코딩된 문자열입니다. |
...
소스 데이터를 대상 데이터 세트에 수집하려면 먼저 대상 데이터 세트가 준수하는 대상 스키마에 매핑해야 합니다. 이 작업은 요청 페이로드 내에 정의된 데이터 매핑과 함께 [[!DNL Data Prep] API](https://www.adobe.io/experience-platform-apis/references/data-prep/)에 대한 POST 요청을 수행하여 수행됩니다.
...
| `mappings.identity` | 매핑 세트가 [!DNL Identity Service]에 대해 표시될지 여부를 지정하는 부울 값입니다. |
...
| `transformations.params.mappingVersion` | 매핑 ID의 해당 버전입니다. 이 값의 기본값은 `0`입니다. |
