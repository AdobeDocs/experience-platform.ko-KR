...
## 연결 [!DNL Chatlio] 를 사용하여 플랫폼으로 [!DNL Flow Service] API {#connect-platform-to-flow-api}

다음은 소스 연결 및 데이터 흐름을 만들기 위해 수행해야 하는 단계를 간략하게 설명합니다. [!DNL Chatlio] Experience Platform에 대한 이벤트 데이터.
...
| `data.format` | 형식 [!DNL Chatlio] 수집할 데이터. 현재 지원되는 데이터 형식은 `json`. |
...
| `data.format` | 형식 [!DNL Chatlio] 수집할 데이터. |
...
데이터 가져오기를 위한 마지막 단계 [!DNL Chatlio] 를 플랫폼으로 가져와서 데이터 흐름을 만듭니다. 이제 다음 필수 값이 준비되었습니다.
...
성공적인 응답은 엔드포인트 URL을 포함하여 다음과 같이 표시된 데이터 흐름에 대한 정보를 반환합니다. `inletUrl`. 다음을 참조하십시오. [Webhook 설정](../../../ui/create/marketing-automation/chatlio-webhook.md#get-streaming-endpoint-url) 필요한 값을 얻기 위한 페이지입니다.
