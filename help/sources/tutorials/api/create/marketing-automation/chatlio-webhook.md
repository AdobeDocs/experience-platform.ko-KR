...
## [!DNL Flow Service] API를 사용하여 [!DNL Chatlio]을(를) 플랫폼에 연결 {#connect-platform-to-flow-api}

다음은 소스 연결 및 데이터 흐름을 만들어 [!DNL Chatlio] 이벤트 데이터를 Experience Platform 상태로 만들기 위해 수행해야 하는 단계를 간략하게 설명합니다.
...
[!DNL Chatlio]에서 플랫폼으로 데이터를 가져오는 마지막 단계는 데이터 흐름을 만드는 것입니다. 이제 다음 필수 값이 준비되었습니다.
