---
title: 흐름 서비스 API를 사용하여 Chatlio용 Source 연결 및 데이터 흐름 만들기
...
# 흐름 서비스 API를 사용하여 [!DNL Chatlio]에 대한 소스 연결 및 데이터 흐름을 만듭니다.

>[!NOTE]
>
>[!DNL Chatlio] 원본이 Beta 버전입니다. 베타 레이블 소스를 사용하는 방법에 대한 자세한 내용은 [소스 개요](../../../../home.md#terms-and-conditions)를 참조하십시오.

다음 자습서에서는 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)를 사용하여 [[!DNL Chatlio]](https://chatlio.com/) 이벤트 데이터를 Adobe Experience Platform으로 가져오기 위한 소스 연결 및 데이터 흐름을 만드는 단계를 안내합니다.
...
## [!DNL Flow Service] API를 사용하여 [!DNL Chatlio]을(를) Experience Platform에 연결 {#connect-platform-to-flow-api}

다음은 소스 연결 및 데이터 흐름을 만들어 [!DNL Chatlio] 이벤트 데이터를 Experience Platform으로 가져오기 위해 수행해야 하는 단계를 간략하게 설명합니다.
...
다음 요청은 [!DNL Chatlio]에 대한 소스 연결을 만듭니다.
...
| `data.format` | 수집할 [!DNL Chatlio] 데이터의 형식입니다. 현재 지원되는 데이터 형식은 `json`뿐입니다. |
...
다음 요청은 [!DNL Chatlio]에 대한 대상 연결을 만듭니다.
...
| `data.format` | 수집할 [!DNL Chatlio] 데이터의 형식입니다. |
...
[!DNL Chatlio]에서 Experience Platform으로 데이터를 가져오는 마지막 단계는 데이터 흐름을 만드는 것입니다. 이제 다음 필수 값이 준비되었습니다.
...
응답이 `inletUrl`(으)로 표시된 엔드포인트 URL을 포함하여 데이터 흐름에 대한 정보를 반환합니다. [Webhook 설정](../../../ui/create/marketing-automation/chatlio-webhook.md#get-streaming-endpoint-url) 페이지를 참조하여 필요한 값을 얻으십시오.
