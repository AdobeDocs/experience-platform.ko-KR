...
| 자격 증명 | 설명 |
| ---------- | ----------- |
| 클라이언트 ID | HubSpot 응용 프로그램과 연결된 클라이언트 ID입니다. |
| 클라이언트 암호 | HubSpot 응용 프로그램과 연결된 클라이언트 암호입니다. |
| 액세스 토큰 | 처음에 OAuth 통합을 인증할 때 얻은 액세스 토큰입니다. |
...
연결은 소스를 지정하고 해당 소스에 대한 자격 증명을 포함합니다. 여러 소스 커넥터를 만들어 다른 데이터를 가져오는 데 사용할 수 있으므로 HubSpot 계정당 하나의 연결만 필요합니다.
...
HubSpot 연결을 만들려면 고유한 연결 사양 ID를 POST 요청의 일부로 제공해야 합니다. HubSpot의 연결 사양 ID는 `cc6a4487-9e91-433e-a3a3-9cf6626c1806`입니다.
...
이 자습서를 따라 Flow Service API를 사용하여 HubSpot 연결을 만들고 연결의 고유 ID 값을 받았습니다. 다음 자습서에서 Flow Service API를 사용하여 마케팅 자동화 시스템을 [탐색하는 방법을 배울 때 이 연결 ID를 사용할 수 있습니다](../../explore/marketing-automation.md).