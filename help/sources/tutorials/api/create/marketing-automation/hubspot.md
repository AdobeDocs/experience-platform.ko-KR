...
| 자격 증명 | 설명 |
| ---------- | ----------- |
| 클라이언트 ID | 응용 프로그램과 연결된 클라이언트 [!DNL HubSpot] ID. |
| 클라이언트 암호 | 응용 프로그램과 연결된 클라이언트 [!DNL HubSpot] 암호입니다. |
| 액세스 토큰 | OAuth 통합을 처음 인증할 때 얻은 액세스 토큰입니다. |
| 토큰 새로 고침 | OAuth 통합을 처음 인증할 때 얻은 새로 고침 토큰입니다. |
| 연결 사양 ID | 연결을 만드는 데 필요한 고유 식별자입니다. 에 대한 연결 사양 ID [!DNL HubSpot] 는 다음과 같습니다. `cc6a4487-9e91-433e-a3a3-9cf6626c1806` |
...
연결은 소스를 지정하고 해당 소스에 대한 자격 증명을 포함합니다. 다른 데이터를 가져오기 위해 여러 소스 커넥터를 만드는 데 사용할 수 있으므로 계정당 하나의 연결만 필요합니다. [!DNL HubSpot]
...
연결을 만들려면 [!DNL HubSpot] 고유한 연결 사양 ID를 POST 요청의 일부로 제공해야 합니다. 에 대한 연결 사양 ID [!DNL HubSpot] 는 `cc6a4487-9e91-433e-a3a3-9cf6626c1806`입니다.
...
이 튜토리얼을 따라 API를 사용하여 [!DNL HubSpot] 연결을 만들고 연결 [!DNL Flow Service] 의 고유 ID 값을 얻게 되었습니다. Flow Service API를 사용하여 마케팅 자동화 시스템을 [탐색하는 방법을 배울 때 다음 자습서에서 이 연결 ID를 사용할 수 있습니다](../../explore/marketing-automation.md).