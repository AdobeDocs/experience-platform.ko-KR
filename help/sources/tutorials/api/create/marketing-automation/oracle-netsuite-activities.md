---
title: 흐름 서비스 API를 사용하여 NetSuite 활동 Oracle에 대한 소스 연결 및 데이터 흐름 만들기
...
# 소스 연결 및 데이터 흐름 만들기 [!DNL Oracle NetSuite Activities] 흐름 서비스 API 사용
...
에서 이벤트 데이터를 가져오는 방법을 알아보려면 다음 튜토리얼을 참조하십시오 [!DNL Oracle NetSuite Activities] 계정을 Adobe Experience Platform에 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
다음 섹션에서는 에 성공적으로 연결하기 위해 알아야 하는 추가 정보를 제공합니다 [!DNL Oracle NetSuite Activities] 사용 [!DNL Flow Service] API.
...
## 연결 [!DNL Oracle NetSuite Activities] 를 사용하여 플랫폼으로 [!DNL Flow Service] API

아래 안내서에 따라 다음을 인증하는 방법을 알아보십시오. [!DNL Oracle NetSuite Activities] 소스, 소스 연결 만들기 및 데이터 흐름을 만들어 이벤트 데이터를 Experience Platform 상태로 만듭니다.
...
POST 기본 연결 ID를 만들려면 `/connections` 을(를) 제공하는 동안 엔드포인트 [!DNL Oracle NetSuite Activities] 요청 본문의 일부인 인증 자격 증명입니다.
...
다음 요청은에 대한 기본 연결을 만듭니다. [!DNL Oracle NetSuite Activities]:
...
| `{SOURCE_PARAMS}` | Platform으로 가져올 소스 파일의 매개 변수를 정의합니다. 에 대해 허용된 형식 유형을 검색하려면 `{SOURCE_PARAMS}`, 전체 문자열을 base64에서 인코딩해야 합니다. <br> 다음에 대한 값이 비어 있습니다. [!DNL Oracle NetSuite Activities].</li></ul> |
...
다음 요청은에 대한 소스 연결을 만듭니다. [!DNL Oracle NetSuite Activities].
...
| `baseConnectionId` | 의 기본 연결 ID [!DNL Oracle NetSuite Activities]. 이 ID는 이전 단계에서 생성되었습니다. |
| `connectionSpec.id` | 소스에 해당하는 연결 사양 ID. |
| `data.format` | 형식 [!DNL Oracle NetSuite Activities] 수집할 데이터. 현재 지원되는 데이터 형식은 `json`. |
...
다음 요청은에 대한 대상 연결을 만듭니다. [!DNL Oracle NetSuite Activities]:
...
| `data.format` | 형식 [!DNL Oracle NetSuite Activities] 수집할 데이터. |
...
다음 요청은에 대한 매핑을 만듭니다. [!DNL DNL NetSuite Activities]
...
데이터 가져오기를 위한 마지막 단계 [!DNL Oracle NetSuite Activities] 를 플랫폼으로 가져와서 데이터 흐름을 만듭니다. 이제 다음 필수 값이 준비되었습니다.
