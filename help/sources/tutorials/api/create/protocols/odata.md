...
topic: overview
type: Tutorial
description: 이 자습서에서는 Flow Service API를 사용하여 일반 OData를 Experience Platform에 연결하는 단계를 단계별로 안내합니다.
...
# API를 [!DNL Generic OData] 사용하여 커넥터 [!DNL Flow Service] 만들기
...
이 자습서에서는 [!DNL Flow Service] API를 사용하여 연결 단계 [!DNL Generic OData] 를 안내합니다 [!DNL Experience Platform].
...
OData [!DNL Flow Service] 에 연결하려면 다음 연결 속성에 값을 제공해야 합니다.

| 자격 증명 | 설명 |
| ---------- | ----------- |
| url | 서비스의 루트 URL입니다 [!DNL OData] . |
| connectionSpec.id | 연결을 만드는 데 필요한 고유 식별자입니다. 에 대한 연결 사양 ID [!DNL OData] 는 다음과 같습니다. `8e6b41a8-d998-4545-ad7d-c6a9fff406c3` |
...
연결은 소스를 지정하고 해당 소스에 대한 자격 증명을 포함합니다. 다른 데이터를 가져오기 위해 여러 소스 커넥터를 만드는 데 사용할 수 있으므로 계정당 하나의 연결만 필요합니다. [!DNL OData]
...
연결을 만들려면 [!DNL OData] 고유한 연결 사양 ID를 POST 요청의 일부로 제공해야 합니다. 에 대한 연결 사양 ID [!DNL OData] 는 `8e6b41a8-d998-4545-ad7d-c6a9fff406c3`입니다.
...
| `connectionSpec.id` | 연결 [!DNL OData] 사양 ID: `8e6b41a8-d998-4545-ad7d-c6a9fff406c3`. |
...
이 튜토리얼을 따라 API를 사용하여 [!DNL OData] 연결을 만들고 연결 [!DNL Flow Service] 의 고유 ID 값을 얻게 되었습니다. 다음 자습서에서는 Flow Service API를 사용하여 프로토콜 애플리케이션을 [탐색하는 방법을 학습하면서 이 ID를 사용할 수 있습니다](../../explore/protocols.md).