...
이 자습서는 Flow Service API를 사용하여 일반 OData를 경험 플랫폼에 연결하는 단계를 안내합니다.
...
| connectionSpec.id | 연결을 만드는 데 필요한 고유 식별자입니다. OData에 대한 연결 사양 ID는 다음과 같습니다. `8e6b41a8-d998-4545-ad7d-c6a9fff406c3` |
...
연결은 소스를 지정하고 해당 소스에 대한 자격 증명을 포함합니다. 여러 소스 커넥터를 만들어 다른 데이터를 가져오는 데 사용할 수 있으므로 OData 계정당 하나의 연결만 필요합니다.
...
OData 연결을 만들려면 고유한 연결 사양 ID를 POST 요청의 일부로 제공해야 합니다. OData에 대한 연결 사양 ID는 `8e6b41a8-d998-4545-ad7d-c6a9fff406c3`입니다.
...
| `connectionSpec.id` | OData 연결 사양 ID: `8e6b41a8-d998-4545-ad7d-c6a9fff406c3`Adobe |
...
이 자습서를 따라 Flow Service API를 사용하여 OData 연결을 만들고 연결의 고유 ID 값을 얻게 됩니다. 다음 자습서에서 이 ID를 사용하여 Flow Service API를 사용하여 프로토콜 애플리케이션을 [탐색하는 방법을 배울 수 있습니다](../../explore/protocols.md).