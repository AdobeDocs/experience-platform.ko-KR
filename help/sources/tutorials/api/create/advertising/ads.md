...
| `loginCustomerId` | 로그인 고객 ID는 Google 광고 관리자 계정에 해당하는 계정 번호이며 특정 운영 고객으로부터 보고서 데이터를 가져오는 데 사용됩니다. 로그인 고객 ID에 대한 자세한 내용은 [Google 광고 API 설명서](https://developers.google.com/google-ads/api/docs/migration/login-customer-id). |
...
| `auth.params.loginCustomerID` | Google 광고 관리자 계정에 해당하는 로그인 고객 ID입니다. |
