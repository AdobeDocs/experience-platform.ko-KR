---
title: Flow Service API를 사용하여 Pinterest Ads의 소스 연결 및 데이터 흐름 만들기
description: Flow Service API를 사용하여 Adobe Experience Platform을 Pinterest Ads에 연결하는 방법을 알아봅니다.
badge: "Beta"
hide: true
hidefromtoc: true
...
# 에 대한 소스 연결 및 데이터 흐름 만들기 [!DNL Pinterest Ads] 사용 [!DNL Flow Service] API

>[!NOTE]
>
>다음 [!DNL Pinterest Ads] 소스가 베타 버전입니다. 다음 문서를 참조하십시오. [소스 개요](../../../../home.md#terms-and-conditions) 베타 레이블이 지정된 소스 사용에 대한 자세한 정보.

다음 자습서에서는 다음을 만드는 단계를 안내합니다 [!DNL Pinterest Ads] 가져올 소스 연결 및 데이터 흐름 [[!DNL Pinterest Ads]](https://ads.pinterest.com/) 데이터를 사용하여 Adobe Experience Platform에 데이터 보내기 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
다음 섹션에서는에 성공적으로 연결하기 위해 알고 있어야 하는 추가 정보를 제공합니다 [!DNL Pinterest Ads] 사용 [!DNL Flow Service] API.

### 사전 요구 사항 {#prerequisites}

연결하려면 [!DNL Pinterest Ads] Experience Platform을 수행하려면 다음 연결 속성에 값을 제공해야 합니다.

* 다음 [!DNL Pinterest] `accessToken`.
* 다음 [!DNL Pinterest] `adAccountId`.
* 다음 중 하나 [!DNL Pinterest] `campaign`, `adGroup` 또는 `ad` ID는 필수입니다.

이러한 연결 속성에 대한 자세한 내용은 [[!DNL Pinterest Ads] 개요](../../../../connectors/advertising/pinterest-ads.md#prerequisites).

## Connect [!DNL Pinterest Ads] 를 사용하여 플랫폼 구현 [!DNL Flow Service] API {#connect-platform-to-flow-api}

다음은 연결하기 위해 수행할 단계에 대해 설명합니다 [!DNL Pinterest Ads] Experience Platform에 연결할 수도 있습니다.

### 기본 연결 만들기 {#base-connection}

기본 연결은 소스의 인증 자격 증명, 현재 연결 상태 및 고유한 기본 연결 ID를 포함하여 소스와 플랫폼 간의 정보를 유지합니다. 기본 연결 ID를 사용하면 소스 내에서 파일을 탐색 및 탐색하고 해당 데이터 유형 및 형식에 대한 정보를 포함하여 수집할 특정 항목을 식별할 수 있습니다.

기본 연결 ID를 만들려면 `/connections` 제공하는 동안 엔드포인트 [!DNL Pinterest Ads] 요청 본문의 일부로 인증 자격 증명.
...
| `auth.params.accessToken` | 다음 포함 [!DNL Pinterest] 소스를 인증하는 데 필요한 액세스 토큰 값입니다. |
...
플랫폼으로 가져올 파일의 경로를 찾으려면 다음 호출을 사용하십시오.
...
| `{SOURCE_PARAMS}` | Platform으로 가져올 소스 파일의 매개 변수를 정의합니다. 에 대해 허용되는 형식 유형을 검색하려면 `{SOURCE_PARAMS}`를 인코딩해야 전체 `{"ad_account_id":"{PINTEREST_AD_ACCOUNT_ID}","object_ids":"{COMMA_SEPERATED_OBJECT_IDS}","object_type":"{OBJECT_TYPE}}"}` base64의 문자열입니다. |

[!DNL Pinterest Ads] 다중 지원 [!DNL Pinterest] Analytics API 엔드포인트. 전송 요청을 활용하는 개체 유형에 따라 다음과 같습니다.
...
대상 [!DNL Pinterest Ads]Campaign Analytics API를 활용할 때 `{SOURCE_PARAMS}` 이 `{"ad_account_id":"123456789000","object_ids":"000123456789","object_type":"campaigns"}`. base64에서 인코딩되면 다음과 같습니다 `YHsiYWRfYWNjb3VudF9pZCI6IjEyMzQ1Njc4OTAwMCIsIm9iamVjdF9pZHMiOiIwMDAxMjM0NTY3ODkiLCJvYmplY3RfdHlwZSI6ImNhbXBhaWducyJ9` 아래와 같이 표시됩니다.
...
대상 [!DNL Pinterest Ads]광고 그룹 Analytics API를 활용할 때 `{SOURCE_PARAMS}` 이 `{"ad_account_id":"123456789000","object_ids":"000123456789,100123456789","object_type":"ad_groups"}`. base64에서 인코딩하면 다음과 같습니다. `eyJhZF9hY2NvdW50X2lkIjoiMTIzNDU2Nzg5MDAwIiwib2JqZWN0X2lkcyI6IjAwMDEyMzQ1Njc4OSwxMDAxMjM0NTY3ODkiLCJvYmplY3RfdHlwZSI6ImFkX2dyb3VwcyJ9` 아래와 같이 표시됩니다.
...
대상 [!DNL Pinterest Ads]광고 Analytics API를 활용할 때 `{SOURCE_PARAMS}` 이 `{"ad_account_id":"123456789000","object_ids":"687247811001,687247811002,687247815005,687247834765","object_type":"ads"}`. base64에서 인코딩하면 다음과 같습니다. `eyJhZF9hY2NvdW50X2lkIjoiMTIzNDU2Nzg5MDAwIiwib2JqZWN0X2lkcyI6IjY4NzI0NzgxMTAwMSw2ODcyNDc4MTEwMDIsNjg3MjQ3ODE1MDA1LDY4NzI0NzgzNDc2NSIsIm9iamVjdF90eXBlIjoiYWRzIn0=` 아래와 같이 표시됩니다.
...
>[!NOTE]
>
>일부 레코드는 더 나은 프레젠테이션을 위해 잘렸습니다.

>[!BEGINTABS]

>[!TAB 캠페인]

성공적인 응답은 해당 [!DNL Pinterest Ads] 호출한 API.
...
에 POST 요청을 수행하여 소스 연결을 만들 수 있습니다 [!DNL Flow Service] API. 소스 연결은 기본 연결 ID, 소스 데이터 파일의 경로 및 연결 사양 ID로 구성됩니다.
...
다음 [!DNL Pinterest Ads] 소스 지원 [!DNL Pinterest] Analytics API 엔드포인트. 다음 요청을 활용하는 객체 유형에 따라 소스 연결이 만들어집니다.
...
| `baseConnectionId` | 의 기본 연결 ID입니다. [!DNL Pinterest Ads]. 이 ID는 이전 단계에서 생성되었습니다. |
| `connectionSpec.id` | 소스에 해당하는 연결 사양 ID입니다. |
| `data.format` | 의 형식 [!DNL Pinterest Ads] 수집할 데이터입니다. 현재 지원되는 데이터 형식은 `json`. |
| `params.ad_account_id` | 다음 [!DNL Pinterest] `Ad account ID`. |
| `params.object_type` | 로서의 [!DNL Pinterest] Campaign Analytics API 엔드포인트는 다음과 같습니다. `campaigns`. |
| `params.object_ids` | 쉼표로 구분된 [!DNL Pinterest] 캠페인 ID. |
...
| `data.format` | 의 형식 [!DNL Pinterest Ads] 수집할 데이터입니다. 현재 지원되는 데이터 형식은 `json`. |
| `params.ad_account_id` | 다음 [!DNL Pinterest] `Ad account ID`. |
| `params.object_type` | 로서의 [!DNL Pinterest] Ad Groups Analytics API 엔드포인트는 다음과 같습니다. `ad_groups`. |
| `params.object_ids` | 쉼표로 구분된 [!DNL Pinterest] 광고 그룹 ID. |
...
| `data.format` | 의 형식 [!DNL Pinterest Ads] 수집할 데이터입니다. 현재 지원되는 데이터 형식은 `json`. |
| `params.ad_account_id` | 다음 [!DNL Pinterest] `Ad account ID`. |
| `params.object_type` | 로서의 [!DNL Pinterest] Ad Analytics API 엔드포인트는 다음과 같습니다. `ads`. |
| `params.object_ids` | 쉼표로 구분된 [!DNL Pinterest] 광고 ID. |
...
| `data.format` | 의 형식 [!DNL Pinterest Ads] Platform으로 가져올 데이터입니다. |
...
에서 데이터를 가져오는 마지막 단계 [!DNL Pinterest Ads] 플랫폼 목표는 데이터 흐름을 만드는 것입니다. 현재까지는 다음 필수 값이 준비되었습니다.
