...
## [!DNL Flow Service] API를 사용하여 [!DNL Pinterest Ads]을(를) 플랫폼에 연결 {#connect-platform-to-flow-api}

다음은 [!DNL Pinterest Ads]을(를) Experience Platform에 연결하기 위해 수행해야 하는 단계를 간략히 설명합니다.
...
| `{SOURCE_PARAMS}` | Platform으로 가져올 소스 파일의 매개 변수를 정의합니다. `{SOURCE_PARAMS}`에 대해 허용되는 형식 유형을 검색하려면 base64에서 전체 `{"ad_account_id":"{PINTEREST_AD_ACCOUNT_ID}","object_ids":"{COMMA_SEPERATED_OBJECT_IDS}","object_type":"{OBJECT_TYPE}}"}` 문자열을 인코딩해야 합니다. |
...
| `data.format` | 플랫폼으로 가져올 [!DNL Pinterest Ads] 데이터의 형식입니다. |
...
[!DNL Pinterest Ads]에서 플랫폼으로 데이터를 가져오는 마지막 단계는 데이터 흐름을 만드는 것입니다. 이제 다음 필수 값이 준비되었습니다.
