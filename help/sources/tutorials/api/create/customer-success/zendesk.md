...
# [!DNL Flow Service] API를 사용하여 [!DNL Zendesk]에 대한 데이터 흐름 만들기

다음 자습서에서는 [[!DNL Flow Service] API](<https://www.adobe.io/experience-platform-apis/references/flow-service/>)를 사용하여 [!DNL Zendesk] 데이터를 Experience Platform으로 가져오기 위한 소스 연결 및 데이터 흐름을 만드는 단계를 안내합니다.
...
* [샌드박스](../../../../../sandboxes/home.md): [!DNL Experience Platform]은(는) 디지털 경험 응용 프로그램을 개발하고 발전시키는 데 도움이 되는 단일 Experience Platform 인스턴스를 별도의 가상 환경으로 분할하는 가상 샌드박스를 제공합니다.

다음 섹션에서는 [!DNL Flow Service] API를 사용하여 [!DNL Zendesk]에 성공적으로 연결하기 위해 알아야 할 추가 정보를 제공합니다.

### 필요한 자격 증명 수집

Experience Platform에서 [!DNL Zendesk] 계정에 액세스하려면 다음 자격 증명에 대한 값을 제공해야 합니다.
...
## [!DNL Flow Service] API를 사용하여 [!DNL Zendesk]을(를) Experience Platform에 연결

다음 자습서에서는 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)를 사용하여 [!DNL Zendesk] 데이터를 Experience Platform으로 가져오기 위해 [!DNL Zendesk] 소스 연결을 만들고 데이터 흐름을 만드는 단계를 안내합니다.
...
기본 연결 ID를 만들려면 [!DNL Zendesk] 인증 자격 증명을 요청 본문의 일부로 제공하는 동안 `/connections` 끝점에 대한 POST 요청을 만듭니다.
...
다음 요청은 [!DNL Zendesk]에 대한 기본 연결을 만듭니다.
...
| `auth.params.subdomain` | 계정과 연결된 고유 도메인. 하위 도메인의 형식은 `https://yoursubdomain.zendesk.com`입니다. |
...
| `{SOURCE_PARAMS}` | Experience Platform으로 가져올 소스 파일의 매개 변수를 정의합니다. `{SOURCE_PARAMS}`에 대해 허용되는 형식 유형을 검색하려면 base64에서 전체 `parameter` 문자열을 인코딩해야 합니다. 아래 예에서 base64로 인코딩된 `"{}"`은(는) `e30`과(와) 같습니다. |
...
성공한 응답은 쿼리된 파일의 구조를 반환합니다. ``data[]`` 페이로드 내의 아래 예제에서는 단일 레코드만 표시되지만 여러 레코드가 있을 수 있습니다.
...
다음 요청은 [!DNL Zendesk]에 대한 소스 연결을 만듭니다.
...
| `baseConnectionId` | [!DNL Zendesk]의 기본 연결 ID. 이 ID는 이전 단계에서 생성되었습니다. |
| `connectionSpec.id` | 소스에 해당하는 연결 사양 ID. |
| `data.format` | 수집할 [!DNL Zendesk] 데이터의 형식입니다. 현재 지원되는 데이터 형식은 `json`뿐입니다. |
...
| `data.format` | Experience Platform으로 가져올 [!DNL Zendesk] 데이터의 형식입니다. |
...
Zendesk에서 Experience Platform으로 데이터를 가져오는 마지막 단계는 데이터 흐름을 만드는 것입니다. 이제 다음 필수 값이 준비되었습니다.
