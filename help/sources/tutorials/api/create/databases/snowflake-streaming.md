...
| `auth.params.warehouse` | [!DNL Snowflake] 웨어하우스 이름. [!DNL Snowflake] 웨어하우스에서 응용 프로그램의 쿼리 실행 프로세스를 관리합니다. 각 웨어하우스는 서로 독립적이므로 Experience Platform으로 데이터를 가져올 때 개별적으로 액세스해야 합니다. |
...
| `params.tableName` | Experience Platform으로 가져올 [!DNL Snowflake] 데이터베이스의 테이블 이름입니다. |
