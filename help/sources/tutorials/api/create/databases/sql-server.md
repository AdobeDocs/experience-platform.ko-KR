---
keywords: Experience Platform;홈;자주 찾는 항목;Microsoft SQL;microsoft sql;sql server;SQL Server
solution: Experience Platform
title: 흐름 서비스 API를 사용하여 SQL Server 기본 연결 만들기
type: Tutorial
...
이 자습서에서는 기본 연결을 만드는 단계를 안내합니다. [!DNL Microsoft SQL Server] 사용 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
| 자격 증명 | 설명 |
| ---------- | ----------- |
| `connectionString` | 와(과) 연결된 연결 문자열 [!DNL Microsoft SQL Server] 계정입니다. 다음 [!DNL Microsoft SQL Server] 연결 문자열 패턴: `Data Source={SERVER_NAME}\\<{INSTANCE_NAME} if using named instance>;Initial Catalog={DATABASE};Integrated Security=False;User ID={USERNAME};Password={PASSWORD};`. |
...
| 속성 | 설명 |
| --------- | ----------- |
| `auth.params.connectionString` | 와(과) 연결된 연결 문자열 [!DNL Microsoft SQL Server] 계정입니다. 다음 [!DNL Microsoft SQL Server] 연결 문자열 패턴: `Data Source={SERVER_NAME}\\<{INSTANCE_NAME} if using named instance>;Initial Catalog={DATABASE};Integrated Security=False;User ID={USERNAME};Password={PASSWORD};`. |
