---
keywords: Experience Platform;홈;인기 항목;아파치 하이브;하이브
solution: Experience Platform
title: Flow Service API를 사용하여 Azure HDInsights 커넥터에서 Apache 하이브를 만듭니다.
topic: overview
type: Tutorial
description: 이 자습서에서는 흐름 서비스 API를 사용하여 Azure HDInsights의 Apache 하이브를 Experience Platform에 연결하는 단계를 안내합니다.
...
# [!DNL Flow Service] API를 사용하여 [!DNL Azure HDInsights] 커넥터에 [!DNL Apache Hive]을 만듭니다.
...
이 자습서는 [!DNL Flow Service] API를 사용하여 [!DNL Azure HDInsights]의 [!DNL Apache Hive]을(를) [!DNL Experience Platform]에 연결하는 단계를 안내합니다.
...
다음 섹션에서는 [!DNL Flow Service] API를 사용하여 [!DNL Hive]에 성공적으로 연결하려면 알아야 할 추가 정보를 제공합니다.

### 필요한 자격 증명 수집

[!DNL Flow Service]이(가) [!DNL Hive]에 연결하려면 다음 연결 속성에 대한 값을 제공해야 합니다.
...
| `connectionSpec.id` | 연결을 만드는 데 필요한 고유 식별자입니다. [!DNL Hive]에 대한 연결 사양 ID는 다음과 같습니다.`aac9bbd4-6c01-46ce-b47e-51c6f0f6db3f` |

시작하는 방법에 대한 자세한 내용은 [이 하이브 문서](https://cwiki.apache.org/confluence/display/Hive/Tutorial#Tutorial-GettingStarted)를 참조하십시오.
...
연결은 소스를 지정하며 해당 소스의 자격 증명을 포함합니다. 다른 데이터를 가져오기 위해 여러 소스 커넥터를 만드는 데 사용할 수 있으므로 하이브 계정당 하나의 연결만 필요합니다.
...
[!DNL Hive] 연결을 만들려면 고유한 연결 사양 ID를 POST 요청의 일부로 제공해야 합니다. [!DNL Hive]에 대한 연결 사양 ID는 `aac9bbd4-6c01-46ce-b47e-51c6f0f6db3f`입니다.
