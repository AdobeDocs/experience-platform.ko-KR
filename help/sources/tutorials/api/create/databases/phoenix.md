...
# [!DNL Flow Service] API를 사용하여 [!DNL Phoenix] 커넥터를 만듭니다.
...
다음 섹션에서는 [!DNL Flow Service] API를 사용하여 [!DNL Phoenix]에 성공적으로 연결하려면 알아야 할 추가 정보를 제공합니다.

### 필요한 자격 증명 수집

[!DNL Flow Service]이(가) [!DNL Phoenix]에 연결하려면 다음 연결 속성에 대한 값을 제공해야 합니다.
...
| `port` | [!DNL Phoenix] 서버가 클라이언트 연결을 수신하는 데 사용하는 TCP 포트입니다. [!DNL Azure] HDInsights에 연결하는 경우 포트를 443으로 지정합니다. |
| `httpPath` | [!DNL Phoenix] 서버에 해당하는 부분 URL. [!DNL Azure] HDInsights 클러스터를 사용하는 경우 /hbasephoenix0을 지정합니다. |
| `enableSsl` | 부울 값입니다. 서버에 대한 연결을 SSL을 사용하여 암호화할지 여부를 지정합니다. |
| `connectionSpec.id` | 연결을 만드는 데 필요한 고유 식별자입니다. [!DNL Phoenix]에 대한 연결 사양 ID는 다음과 같습니다.`102706fb-a5cd-42ee-afe0-bc42f017ff43` |
...
연결은 소스를 지정하며 해당 소스의 자격 증명을 포함합니다. 다른 데이터를 가져오기 위해 여러 소스 커넥터를 만드는 데 사용할 수 있으므로 [!DNL Phoenix] 계정에는 하나의 연결만 필요합니다.
...
[!DNL Phoenix] 연결을 만들려면 고유한 연결 사양 ID를 POST 요청의 일부로 제공해야 합니다. [!DNL Phoenix]에 대한 연결 사양 ID는 `102706fb-a5cd-42ee-afe0-bc42f017ff43`입니다.
