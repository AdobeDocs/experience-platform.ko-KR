...
title: Flow Service API를 사용하여 HP Vertica Source Connection 생성
...
# [!DNL Flow Service] API를 사용하여 HP [!DNL Vertica] 소스 연결을 만듭니다

>[!NOTE]
>
>HP [!DNL Vertica] 커넥터가 베타 버전입니다. 베타 레이블이 지정된 커넥터 사용에 대한 자세한 내용은 [소스 개요](../../../../home.md#terms-and-conditions) 를 참조하십시오.
...
[!DNL Flow Service]이 HP [!DNL Vertica]에 연결하려면 다음 연결 속성에 대한 값을 제공해야 합니다.
...
연결은 소스를 지정하고 해당 소스에 대한 자격 증명을 포함합니다. HP [!DNL Vertica] 계정당 하나의 연결만 있으면 다른 데이터를 가져오기 위해 여러 소스 커넥터를 만드는 데 사용할 수 있습니다.
...
| 매개 변수 | 설명 |
| --------- | ----------- |
| `auth.params.connectionString` | HP [!DNL Vertica] 계정에 연결된 연결 문자열입니다. HP [!DNL Vertica]의 연결 문자열 패턴은 다음과 같습니다.`Server={SERVER};Port={PORT};Database={DATABASE};UID={USERNAME};PWD={PASSWORD}`. |
