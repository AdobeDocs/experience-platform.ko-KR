...
>[!NOTE]
>CouchBase 커넥터가 베타 버전입니다. 기능 및 설명서는 변경될 수 있습니다.
...
이 자습서에서는 Flow Service API를 사용하여 CouchBase를 Experience Platform에 연결하는 단계를 단계별로 안내합니다.
...
| 자격 증명 | 설명 |
| ---------- | ----------- |
| `connectionString` | CoachBase 인스턴스에 연결하는 데 사용되는 연결 문자열입니다. CouchBase의 연결 문자열 패턴은 입니다 `Server={SERVER}; Port={PORT};AuthMech=1;CredString=[{\"user\": \"{USER}\", \"pass\":\"{PASS}\"}];`. 연결 문자열 가져오기에 대한 자세한 내용은 [이 CouchBase 문서를 참조하십시오](https://docs.couchbase.com/c-sdk/2.10/client-settings.html#configuring-overview). |
| `connectionSpec.id` | 연결을 만드는 데 필요한 식별자입니다. CouchBase에 대한 고정 연결 사양 ID가 입니다 `1fe283f6-9bec-11ea-bb37-0242ac130002`. |
...
| 속성 | 설명 |
| --------- | ----------- |
| `auth.params.connectionString` | CoachBase 계정에 연결하는 데 사용되는 연결 문자열입니다. 연결 문자열 패턴은 다음과 같습니다. `Server={SERVER}; Port={PORT};AuthMech=1;CredString=[{\"user\": \"{USER}\", \"pass\":\"{PASS}\"}];`. |
