...
topic: overview
type: Tutorial
description: 이 자습서에서는 Flow Service API를 사용하여 Experience Platform에 Oracle을 연결하는 단계를 안내합니다.
...
# API를 사용하여 [!DNL Oracle] 커넥터 [!DNL Flow Service] 만들기
...
이 자습서에서는 [!DNL Flow Service] API를 사용하여 연결 단계 [!DNL Oracle] 를 안내합니다 [!DNL Experience Platform].
...
다음 섹션에서는 API를 [!DNL Oracle] 사용하기 위해 연결하기 위해 알아야 할 추가 정보를 [!DNL Flow Service] 제공합니다.
...
연결은 소스를 지정하고 해당 소스에 대한 자격 증명을 포함합니다. 다른 데이터를 가져오기 위해 여러 소스 커넥터를 만드는 데 사용할 수 있으므로 [!DNL Oracle] 계정당 하나의 커넥터만 필요합니다.
...
연결을 만들려면 [!DNL Oracle] 고유한 연결 사양 ID를 POST 요청의 일부로 제공해야 합니다. 에 대한 연결 사양 ID [!DNL Oracle] 는 `d6b52d86-f0f8-475f-89d4-ce54c8527328`입니다.
...
| 매개 변수 | 설명 |
| --------- | ----------- |
| `auth.params.connectionString` | 데이터베이스에 연결하는 데 사용되는 연결 [!DNL Oracle] 문자열입니다. 연결 [!DNL Oracle] 문자열 패턴은 다음과 같습니다. `Host={HOST};Port={PORT};Sid={SID};User Id={USERNAME};Password={PASSWORD}`. |
| `connectionSpec.id` | 연결 [!DNL Oracle] 사양 ID: `d6b52d86-f0f8-475f-89d4-ce54c8527328`. |
...
이 튜토리얼을 따라 API를 사용하여 [!DNL Oracle] 연결을 만들고 연결 [!DNL Flow Service] 의 고유 ID 값을 얻게 되었습니다. 다음 자습서에서는 Flow Service API를 사용하여 데이터베이스를 [탐색하는 방법을 배울 때 이 ID를 사용할 수 있습니다](../../explore/database-nosql.md).
