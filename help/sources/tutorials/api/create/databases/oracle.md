...
title: Flow Service API를 사용하여 Oracle 커넥터 만들기
topic: overview
type: Tutorial
description: 이 자습서에서는 Flow Service API를 사용하여 Oracle을 Experience Platform에 연결하는 단계를 안내합니다.
...
# [!DNL Flow Service] API를 사용하여 [!DNL Oracle] 커넥터를 만듭니다.
...
이 자습서에서는 [!DNL Flow Service] API를 사용하여 [!DNL Oracle]을(를) [!DNL Experience Platform]에 연결하는 단계를 안내합니다.
...
다음 섹션에서는 [!DNL Flow Service] API를 사용하여 [!DNL Oracle]에 성공적으로 연결하려면 알아야 할 추가 정보를 제공합니다.
...
| `connectionSpec.id` | 연결을 만드는 데 필요한 고유 식별자입니다. [!DNL Oracle]에 대한 연결 사양 ID는 `d6b52d86-f0f8-475f-89d4-ce54c8527328`입니다. |
...
연결은 소스를 지정하며 해당 소스의 자격 증명을 포함합니다. 다른 데이터를 가져오기 위해 여러 소스 커넥터를 만드는 데 사용할 수 있으므로 [!DNL Oracle] 계정에는 커넥터가 하나만 필요합니다.
...
[!DNL Oracle] 연결을 만들려면 고유한 연결 사양 ID를 POST 요청의 일부로 제공해야 합니다. [!DNL Oracle]에 대한 연결 사양 ID는 `d6b52d86-f0f8-475f-89d4-ce54c8527328`입니다.
