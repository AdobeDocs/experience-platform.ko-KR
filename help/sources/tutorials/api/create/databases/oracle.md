...
# Flow Service API를 사용하여 Oracle 커넥터 생성

>[!NOTE]
>Oracle 커넥터가 베타 상태입니다. 베타 [레이블이 지정된 커넥터 사용에 대한 자세한 내용은 소스 개요를](../../../../home.md#terms-and-conditions) 참조하십시오.
...
이 자습서에서는 Flow Service API를 사용하여 Oracle을 Experience Platform에 연결하는 단계를 단계별로 안내합니다.
...
다음 섹션에서는 Flow Service API를 사용하여 Oracle에 성공적으로 연결하기 위해 알아야 하는 추가 정보를 제공합니다.
...
연결은 소스를 지정하고 해당 소스에 대한 자격 증명을 포함합니다. 다른 데이터를 가져오기 위해 여러 소스 커넥터를 생성하는 데 사용할 수 있으므로 Oracle 계정당 하나의 커넥터만 필요합니다.
...
Oracle 연결을 만들려면 고유한 연결 사양 ID를 POST 요청의 일부로 제공해야 합니다. Oracle에 대한 연결 사양 ID입니다 `d6b52d86-f0f8-475f-89d4-ce54c8527328`.
...
| 매개 변수 | 설명 |
| --------- | ----------- |
| `auth.params.connectionString` | Oracle 데이터베이스에 연결하는 데 사용되는 연결 문자열. Oracle 연결 문자열 패턴은 다음과 같습니다. `Host={HOST};Port={PORT};Sid={SID};User Id={USERNAME};Password={PASSWORD}`. |
| `connectionSpec.id` | Oracle 연결 사양 ID: `d6b52d86-f0f8-475f-89d4-ce54c8527328`. |
...
이 튜토리얼을 따라 Flow Service API를 사용하여 Oracle 연결을 만들고 연결의 고유 ID 값을 받았습니다. 다음 자습서에서는 Flow Service API를 사용하여 데이터베이스를 [탐색하는 방법을 배울 때 이 ID를 사용할 수 있습니다](../../explore/database-nosql.md).
