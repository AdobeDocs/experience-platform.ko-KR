...
title: 흐름 서비스 API를 사용하여  [!DNL MySQL] 기본 연결 만들기
type: Tutorial
...
# [!DNL Flow Service] API를 사용하여 [!DNL MySQL] 기본 연결 만들기
...
[!DNL Flow Service]이(가) [!DNL MySQL] 저장소에 연결하려면 다음 연결 속성에 대한 값을 제공해야 합니다.
...
연결 문자열을 가져오는 방법에 대한 자세한 내용은 이 [[!DNL MySQL] 문서](https://dev.mysql.com/doc/connector-net/en/connector-net-connections-string.html)를 참조하세요.
...
기본 연결 ID를 만들려면 [!DNL MySQL] 인증 자격 증명을 요청 매개 변수의 일부로 제공하는 동안 `/connections` 끝점에 대한 POST 요청을 만듭니다.
...
다음 요청은 [!DNL MySQL]에 대한 기본 연결을 만듭니다.
...
성공한 응답은 고유 식별자(`id`)를 포함하여 새로 만든 기본 연결의 세부 정보를 반환합니다. 이 ID는 다음 자습서에서 데이터베이스를 탐색하는 데 필요합니다.
...
이 자습서에 따라 [!DNL Flow Service] API를 사용하여 [!DNL MySQL]기본 연결을 만들었습니다. 다음 자습서에서 이 기본 연결 ID를 사용할 수 있습니다.
