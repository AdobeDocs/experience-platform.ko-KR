...
이 자습서는 Flow Service API를 사용하여 Experience Platform을 MySQL에 연결하는 단계를 안내합니다.
...
Flow Service가 MySQL 저장소에 연결하려면 다음 연결 속성 값을 제공해야 합니다.

| 자격 증명 | 설명 |
| ---------- | ----------- |
| `connectionString` | 계정과 연결된 MySQL 연결 문자열. |

연결 문자열에 대한 자세한 내용과 MySQL 문서를 [읽고 연결 문자열을 얻는 방법에 대해 자세히 알아볼](https://dev.mysql.com/doc/connector-net/en/connector-net-connections-string.html)수 있습니다.
...
사용 가능한 각 소스에는 인증 요구 사항과 같은 커넥터 속성을 설명하는 고유한 연결 사양 세트가 있습니다. GET 요청을 `/connectionSpecs` 끝점으로 보내면 사용 가능한 모든 소스에 대한 연결 사양이 반환됩니다. 또한 쿼리를 포함하여 MySQL에 대한 정보를 얻을 `property=name=="mysql"` 수도 있습니다.
...
성공적인 응답은 고유 식별자(`id`)를 포함하여 MySQL에 대한 연결 사양을 반환합니다. 이 ID는 기본 연결을 만들려면 다음 단계에서 필요합니다.
...
기본 연결은 소스를 지정하고 해당 소스에 대한 자격 증명을 포함합니다. 여러 소스 커넥터를 만들어 다른 데이터를 가져오는 데 사용할 수 있으므로 MySQL 계정당 하나의 기본 연결만 필요합니다.
...
이 자습서를 따라 Flow Service API를 사용하여 MySQL 기본 연결을 만들고 연결의 고유 ID 값을 받았습니다. 다음 자습서에서 Flow Service API를 사용하여 데이터베이스 또는 NoSQL 시스템을 [탐색하는 방법을 배울 때 이 기본 연결 ID를 사용할 수 있습니다](../../explore/database-nosql.md).