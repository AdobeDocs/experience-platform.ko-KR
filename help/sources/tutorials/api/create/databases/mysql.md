...
title: Flow Service API를 사용하여 [!DNL MySQL] 기본 연결 만들기
...
# [!DNL Flow Service] API를 사용하여 [!DNL MySQL] 기본 연결을 만듭니다
...
[!DNL Flow Service]이 [!DNL MySQL] 스토리지와 연결하려면 다음 연결 속성 값을 제공해야 합니다.
...
기본 연결 ID를 만들려면 요청 매개 변수의 일부로 [!DNL MySQL] 인증 자격 증명을 제공하는 동안 `/connections` 끝점에 POST 요청을 하십시오.
...
성공적인 응답은 고유 식별자(`id`)를 포함하여 새로 생성된 기본 연결의 세부 정보를 반환합니다. 이 ID는 다음 자습서에서 데이터베이스를 탐색하는 데 필요합니다.
...
이 자습서에 따르면 [!DNL Flow Service] API를 사용하여 [!DNL MySQL] 연결을 만들고 연결의 고유 ID 값을 받았습니다. 이 연결 ID는 다음 자습서에서 [Flow Service API](../../explore/database-nosql.md)를 사용하여 데이터베이스 또는 NoSQL 시스템을 탐색하는 방법을 배울 때 사용할 수 있습니다.
