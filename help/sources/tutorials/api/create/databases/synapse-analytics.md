...
# [!DNL Flow Service] API를 사용하여 [!DNL Azure Synapse Analytics] 기본 연결 만들기
...
이 자습서에서는 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)를 사용하여 [!DNL Azure Synapse Analytics]&#x200B;(이하 &quot;[!DNL Synapse]&quot;)에 대한 기본 연결을 만드는 단계를 안내합니다.
...
다음 섹션에서는 [!DNL Flow Service] API를 사용하여 [!DNL Synapse]에 성공적으로 연결하기 위해 알아야 할 추가 정보를 제공합니다.

### 필요한 자격 증명 수집

[!DNL Flow Service]이(가) [!DNL Synapse]과(와) 연결하려면 다음 연결 속성에 대한 값을 제공해야 합니다.
...
| `connectionSpec.id` | 연결 사양은 기본 및 소스 연결 만들기와 관련된 인증 사양을 포함하여 소스의 커넥터 속성을 반환합니다. [!DNL Synapse]의 연결 사양 ID는 `a49bcc7d-8038-43af-b1e4-5a7a089a7d79`입니다. |

연결 문자열을 얻는 방법에 대한 자세한 내용은 [이 Synapse 문서](https://docs.microsoft.com/en-us/azure/sql-database/sql-database-aad-authentication-configure?toc=%2Fazure%2Fsynapse-analytics%2Fsql-data-warehouse%2Ftoc.json&amp;bc=%2Fazure%2Fsynapse-analytics%2Fsql-data-warehouse%2Fbreadcrumb%2Ftoc.json&amp;tabs=azure-powershell)를 참조하세요.
...
기본 연결 ID를 만들려면 [!DNL Synapse] 인증 자격 증명을 요청 매개 변수의 일부로 제공하는 동안 `/connections` 끝점에 대한 POST 요청을 만듭니다.
...
다음 요청은 [!DNL Synapse]에 대한 기본 연결을 만듭니다.
...
이 자습서에 따라 [!DNL Flow Service] API를 사용하여 [!DNL Synapse] 기본 연결을 만들었습니다. 다음 자습서에서 이 기본 연결 ID를 사용할 수 있습니다.
