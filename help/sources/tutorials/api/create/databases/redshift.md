...
다음 섹션에서는 Flow Service API를 사용하여 Redshift에 성공적으로 연결하기 위해 알아야 할 추가 정보를 제공합니다.
...
시작하는 방법에 대한 자세한 내용은 [이 Redshift 문서를](https://docs.aws.amazon.com/redshift/latest/gsg/getting-started.html)참조하십시오.
...
Redshift 연결을 만들려면 Redshift 연결 사양 세트가 Flow Service 내에 있어야 합니다. Platform과 Redshift를 연결하는 첫 번째 단계는 이러한 사양을 가져오는 것입니다.

**API 형식**

사용 가능한 각 소스에는 인증 요구 사항과 같은 커넥터 속성을 설명하는 고유한 연결 사양 세트가 있습니다. GET 요청을 수행하고 쿼리 매개 변수를 사용하여 Redshift에 대한 연결 사양을 조회할 수 있습니다.

쿼리 매개 변수 없이 GET 요청을 전송하면 사용 가능한 모든 소스에 대한 연결 사양이 반환됩니다. 쿼리를 포함하여 Redshift에 대한 정보를 얻을 `property=name=="amazon-redshift"` 수 있습니다.
...
성공적인 응답은 고유 식별자(`id`)를 포함하여 Redshift에 대한 연결 사양을 반환합니다. 이 ID는 기본 연결을 만들려면 다음 단계에서 필요합니다.
...
기본 연결은 소스를 지정하고 해당 소스에 대한 자격 증명을 포함합니다. 여러 소스 커넥터를 만들어 다른 데이터를 가져오는 데 사용할 수 있으므로 Redshift 계정당 하나의 기본 연결만 필요합니다.
...
| `connectionSpec.id` | 이전 단계에서 검색한 Redshift `id` 계정의 연결 사양 |
...
이 자습서를 따라 Flow Service API를 사용하여 Redshift 기본 연결을 만들고 연결의 고유 ID 값을 받았습니다. 다음 자습서에서 Flow Service API를 사용하여 데이터베이스 또는 NoSQL 시스템을 [탐색하는 방법을 배울 때 이 기본 연결 ID를 사용할 수 있습니다](../../explore/database-nosql.md).
