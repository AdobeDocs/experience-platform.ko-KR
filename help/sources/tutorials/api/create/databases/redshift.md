...
# [!DNL Flow Service] API를 사용하여 [!DNL Amazon Redshift] 커넥터를 만듭니다.
...
이 자습서는 [!DNL Flow Service] API를 사용하여 [!DNL Experience Platform]을 [!DNL Amazon Redshift](이하 &quot;a3/>&quot;라 한다)에 연결하는 단계를 안내합니다.[!DNL Redshift]
...
다음 섹션에서는 [!DNL Flow Service] API를 사용하여 [!DNL Redshift]에 성공적으로 연결하려면 알아야 할 추가 정보를 제공합니다.

### 필요한 자격 증명 수집

[!DNL Flow Service]이(가) [!DNL Redshift]에 연결하려면 다음 연결 속성을 제공해야 합니다.
...
시작하는 방법에 대한 자세한 내용은 [이 Redshift 문서](https://docs.aws.amazon.com/redshift/latest/gsg/getting-started.html)를 참조하십시오.
...
연결은 소스를 지정하며 해당 소스의 자격 증명을 포함합니다. 다른 데이터를 가져오기 위해 여러 소스 커넥터를 만드는 데 사용할 수 있으므로 [!DNL Redshift] 계정에는 하나의 연결만 필요합니다.
...
| 속성 | 설명 |
| ------------- | --------------- |
| `auth.params.server` | [!DNL Redshift] 서버. |
| `auth.params.database` | [!DNL Redshift] 계정과 연결된 데이터베이스입니다. |
| `auth.params.password` | [!DNL Redshift] 계정과 연결된 암호입니다. |
| `auth.params.username` | [!DNL Redshift] 계정과 연결된 사용자 이름입니다. |
| `connectionSpec.id` | 이전 단계에서 검색된 [!DNL Redshift] 계정의 연결 사양 `id`입니다. |

**응답**

성공적인 응답은 고유 식별자(`id`)를 포함하여 새로 만든 연결을 반환합니다. 이 ID는 다음 튜토리얼에서 데이터를 탐색하는 데 필요합니다.
