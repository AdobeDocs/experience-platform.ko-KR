...
# 만들기 [!DNL Apache Spark] 날짜 [!DNL Azure] 를 사용하는 HDInsights 기본 연결 [!DNL Flow Service] API
...
이 자습서에서는 기본 연결을 만드는 단계를 안내합니다. [!DNL Apache Spark] 날짜 [!DNL Azure HDInsights] (이하 &quot;라고 한다)[!DNL Spark]&quot;) 사용 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
다음 섹션에서는 에 성공적으로 연결하기 위해 알아야 하는 추가 정보를 제공합니다 [!DNL Spark] 사용 [!DNL Flow Service] API.

### 필요한 자격 증명 수집

주문 [!DNL Flow Service] 연결 대상 [!DNL Spark], 다음 연결 속성에 대한 값을 제공해야 합니다.
...
| `connectionSpec.id` | 연결 사양은 기본 및 소스 연결 만들기와 관련된 인증 사양을 포함하여 소스의 커넥터 속성을 반환합니다. 에 대한 연결 사양 ID [!DNL Spark] 은(는) `6a8d82bc-1caf-45d1-908d-cadabc9d63a6` |

시작에 대한 자세한 내용은 다음을 참조하십시오. [이 Spark 문서](https://docs.microsoft.com/en-us/azure/hdinsight/spark/apache-spark-overview).
...
POST 기본 연결 ID를 만들려면 `/connections` 을(를) 제공하는 동안 엔드포인트 [!DNL Spark] 요청 매개 변수의 일부로 인증 자격 증명을 사용합니다.
...
| `auth.params.password` | 과(와) 연계된 암호 [!DNL Spark] 연결. |
...
이 자습서를 따라 [!DNL Spark] 를 사용한 기본 연결 [!DNL Flow Service] API. 다음 자습서에서 이 기본 연결 ID를 사용할 수 있습니다.
