...
이 자습서에서는 Flow Service API를 사용하여 Azure HDInsights(이하 &quot;Spark&quot;라 한다)의 Apache Spark를 Experience Platform에 연결하는 단계를 안내합니다.
...
Flow Service가 Spark와 연결되려면 다음 연결 속성에 대한 값을 제공해야 합니다.
...
연결은 소스를 지정하고 해당 소스에 대한 자격 증명을 포함합니다. 여러 소스 커넥터를 만들어 다른 데이터를 가져올 수 있으므로 Spark 계정당 하나의 연결만 필요합니다.
...
Spark 연결을 만들려면 고유한 연결 사양 ID를 POST 요청의 일부로 제공해야 합니다. Spark의 연결 사양 ID입니다 `6a8d82bc-1caf-45d1-908d-cadabc9d63a6`.
...
| `auth.params.password` | Spark 연결과 관련된 암호입니다. |
| `connectionSpec.id` | Spark 연결 사양 ID: `6a8d82bc-1caf-45d1-908d-cadabc9d63a6`. |
...
이 튜토리얼을 따라 Flow Service API를 사용하여 Spark 연결을 만들고 연결 고유 ID 값을 받았습니다. 다음 자습서에서는 Flow Service API를 사용하여 데이터베이스를 [탐색하는 방법을 배울 때 이 ID를 사용할 수 있습니다](../../explore/database-nosql.md).