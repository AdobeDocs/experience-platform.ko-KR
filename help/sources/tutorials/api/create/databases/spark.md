...
title: Flow Service API를 사용하여 Azure HDInsights 소스 연결에서 Apache Spark 만들기
...
# [!DNL Flow Service] API를 사용하여 [!DNL Azure] HDInsights 소스 연결에 [!DNL Apache Spark]을 만듭니다.
...
이 자습서는 [!DNL Flow Service] API를 사용하여 [!DNL Azure HDInsights]의 [!DNL Apache Spark](이하 &quot;[!DNL Spark]&quot;라 함)을 [!DNL Experience Platform]에 연결하는 단계를 안내합니다.
...
다음 섹션에서는 [!DNL Flow Service] API를 사용하여 [!DNL Spark]에 성공적으로 연결하려면 알아야 할 추가 정보를 제공합니다.

### 필요한 자격 증명 수집

[!DNL Flow Service]이(가) [!DNL Spark]에 연결하려면 다음 연결 속성에 대한 값을 제공해야 합니다.
...
| `connectionSpec.id` | 연결을 만드는 데 필요한 고유 식별자입니다. [!DNL Spark]에 대한 연결 사양 ID는 다음과 같습니다.`6a8d82bc-1caf-45d1-908d-cadabc9d63a6` |
...
연결은 소스를 지정하며 해당 소스의 자격 증명을 포함합니다. 다른 데이터를 가져오기 위해 여러 소스 커넥터를 만드는 데 사용할 수 있으므로 [!DNL Spark] 계정에는 하나의 연결만 필요합니다.
...
[!DNL Spark] 연결을 만들려면 고유한 연결 사양 ID를 POST 요청의 일부로 제공해야 합니다. [!DNL Spark]에 대한 연결 사양 ID는 `6a8d82bc-1caf-45d1-908d-cadabc9d63a6`입니다.
...
| `auth.params.username` | [!DNL Spark] 연결에 연결된 사용자 이름입니다. |
| `auth.params.password` | [!DNL Spark] 연결에 연결된 암호입니다. |
...
이 자습서를 따라 [!DNL Flow Service] API를 사용하여 [!DNL Spark] 연결을 만들고 연결의 고유 ID 값을 받았습니다. 다음 자습서에서는 Flow Service API](../../explore/database-nosql.md)를 사용하여 [데이터베이스를 탐색하는 방법을 배울 때 이 ID를 사용할 수 있습니다.
