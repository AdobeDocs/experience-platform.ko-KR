...
# 만들기 [!DNL PostgreSQL] 를 사용한 기본 연결 [!DNL Flow Service] API

기본 연결은 소스와 Adobe Experience Platform 간의 인증된 연결을 나타냅니다.

이 자습서에서는 기본 연결을 만드는 단계를 안내합니다. [!DNL PostgreSQL] 사용 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
다음 섹션에서는 에 성공적으로 연결하기 위해 알아야 하는 추가 정보를 제공합니다 [!DNL PostgreSQL] 사용 [!DNL Flow Service] API.

### 필요한 자격 증명 수집

주문 [!DNL Flow Service] 연결 대상 [!DNL PostgreSQL], 다음 연결 속성을 제공해야 합니다.
...
| `connectionSpec.id` | 연결 사양은 기본 및 소스 연결 만들기와 관련된 인증 사양을 포함하여 소스의 커넥터 속성을 반환합니다. 에 대한 연결 사양 ID [!DNL PostgreSQL] 은(는) `74a1c565-4e59-48d7-9d67-7c03b8a13137`. |

연결 문자열을 얻는 방법에 대한 자세한 내용은 다음을 참조하십시오 [[!DNL PostgreSQL] 문서](https://www.postgresql.org/docs/9.2/app-psql.html).
...
POST 기본 연결 ID를 만들려면 `/connections` 을(를) 제공하는 동안 엔드포인트 [!DNL PostgreSQL] 요청 매개 변수의 일부로 인증 자격 증명을 사용합니다.
...
| 속성 | 설명 |
| ------------- | --------------- |
| `auth.params.connectionString` | 와(과) 연결된 연결 문자열 [!DNL PostgreSQL] 계정입니다. 다음 [!DNL PostgreSQL] 연결 문자열 패턴: `Server={SERVER};Database={DATABASE};Port={PORT};UID={USERNAME};Password={PASSWORD}`. |
| `connectionSpec.id` | 다음 [!DNL PostgreSQL] 연결 사양 ID: `74a1c565-4e59-48d7-9d67-7c03b8a13137`. |

**응답**

성공적인 응답은 고유 식별자()를 반환합니다.`id`)을 참조하십시오. 이 ID는 다음을 살펴보는 데 필요합니다. [!DNL PostgreSQL] 다음 자습서에서 데이터베이스.
...
이 자습서를 따라 [!DNL PostgreSQL] 를 사용한 연결 기본 연결 [!DNL Flow Service] API. 다음 자습서에서 이 기본 연결 ID를 사용할 수 있습니다.
