...
이 자습서는 Flow Service API를 사용하여 경험 플랫폼을 SFTP(Secure File Transfer Protocol) 서버에 연결하는 단계를 안내합니다.

경험 플랫폼에서 사용자 인터페이스를 사용하려는 경우 UI 튜토리얼에서는 [](../../../ui/create/cloud-storage/ftp-sftp.md) 유사한 작업을 수행하기 위한 단계별 지침을 제공합니다.
...
다음 섹션은 Flow Service API를 사용하여 SFTP 서버에 성공적으로 연결하기 위해 알아야 할 추가 정보를 제공합니다.

### 필요한 자격 증명 수집

Flow Service가 SFTP에 연결하려면 다음 연결 속성에 대한 값을 제공해야 합니다.
...
SFTP 연결을 만들려면 Flow Service 내에 SFTP 연결 사양 세트가 있어야 합니다. 플랫폼을 SFTP에 연결하는 첫 번째 단계는 이러한 사양을 가져오는 것입니다.

**API 형식**

사용 가능한 각 소스에는 인증 요구 사항과 같은 커넥터 속성을 설명하는 고유한 연결 사양 세트가 있습니다. GET 요청을 수행하고 쿼리 매개 변수를 사용하여 SFTP에 대한 연결 사양을 조회할 수 있습니다.

쿼리 매개 변수 없이 GET 요청을 전송하면 사용 가능한 모든 소스에 대한 연결 사양이 반환됩니다. 쿼리를 포함하여 SFTP에 대한 정보를 얻을 `property=name=="sftp"` 수 있습니다.
...
다음 요청은 SFTP 서버에 대한 연결 사양을 검색합니다.
...
성공적인 응답은 고유 식별자(`id`)를 포함하여 SFTP 서버에 대한 연결 사양을 반환합니다. 이 ID는 기본 연결을 만들려면 다음 단계에서 필요합니다.
...
기본 연결은 소스를 지정하고 해당 소스에 대한 자격 증명을 포함합니다. 여러 소스 커넥터를 만들어 다른 데이터를 가져오는 데 사용할 수 있으므로 SFTP 계정당 하나의 기본 연결만 필요합니다.
...
| `connectionSpec.id` | 이전 단계에서 검색한 SFTP `id` 서버의 연결 사양 |

**응답**

성공적인 응답은 새로 만든 기본 연결의 고유 식별자(`id`)를 반환합니다. 이 ID는 다음 자습서에서 SFTP 서버를 탐색하는 데 필요합니다.
...
이 자습서를 따라 Flow Service API를 사용하여 SFTP 연결을 만들고 연결의 고유 ID 값을 받았습니다. 이 연결 ID를 사용하여 Flow Service API를 사용하여 클라우드 [스토리지를](../../explore/cloud-storage.md) 탐색하거나 Flow Service API를 사용하여 [인제스트](../../cloud-storage-parquet.md)쪽모이 세공 데이터를 생성할 수 있습니다.
