...
| 자격 증명 | 설명 |
| ---------- | ----------- |
| `accessKeyId` | [!DNL Google Cloud Storage] 계정을 Platform에 인증하는 데 사용되는 61자의 영숫자 문자열입니다. |
| `secretAccessKey` | [!DNL Google Cloud Storage] 계정을 Platform에 인증하는 데 사용되는 40자의 Base64로 인코딩된 문자열입니다. |
...
기본 연결 ID를 만들려면 [!DNL Google Cloud Storage] 인증 자격 증명을 요청 매개 변수의 일부로 제공하는 동안 `/connections` 끝점에 POST 요청을 하십시오.
