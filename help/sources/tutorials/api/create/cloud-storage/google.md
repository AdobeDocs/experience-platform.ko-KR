...
# Flow Service API를 사용하여 Google 클라우드 스토리지 커넥터 만들기
...
이 자습서에서는 Flow Service API를 사용하여 Experience Platform을 Google Cloud 스토리지 계정에 연결하는 단계를 안내합니다.
...
| 자격 증명 | 설명 |
| ---------- | ----------- |
| `accessKeyId` | Google 클라우드 스토리지 계정의 액세스 키 ID. |
...
연결은 소스를 지정하고 해당 소스에 대한 자격 증명을 포함합니다. 여러 소스 커넥터를 만들어 다른 데이터를 가져올 수 있으므로 Google 클라우드 스토리지 계정당 하나의 연결만 필요합니다.
...
| 속성 | 설명 |
| -------- | ----------- |
| `auth.params.accessKeyId` | Google 클라우드 스토리지 계정과 연결된 액세스 키 ID. |
| `auth.params.secretAccessKey` | Google 클라우드 스토리지 계정과 연결된 비밀 액세스 키 |
| `connectionSpec.id` | Google 클라우드 스토리지 연결 사양 ID: `32e8f412-cdf7-464c-9885-78184cb113fd` |
...
이 튜토리얼을 따라 API를 사용하여 Google 클라우드 스토리지 연결을 만들고 응답 본문의 일부로 고유 ID를 받았습니다. 이 연결 ID를 사용하여 Flow Service API를 사용하여 클라우드 스토리지 [를](../../explore/cloud-storage.md) 탐색하거나 Flow Service API를 사용하여 [인제스트 쪽모이 세공 데이터를 생성할 수 있습니다](../../cloud-storage-parquet.md).