...
# 흐름 서비스 API를 사용하여 Azure 이벤트 허브 커넥터를 만들기
...
이 자습서에서는 Flow Service API를 사용하여 Experience Platform을 Azure 이벤트 허브 계정에 연결하는 단계를 안내합니다.
...
다음 섹션에서는 Flow Service API를 사용하여 Azure 이벤트 허브 계정에 성공적으로 연결하려면 알아야 할 추가 정보를 제공합니다.

### 필요한 자격 증명 수집

흐름 서비스가 Azure 이벤트 허브 계정과 연결하려면 다음 연결 속성에 대한 값을 제공해야 합니다.
...
연결은 소스를 지정하고 해당 소스에 대한 자격 증명을 포함합니다. 여러 소스 커넥터를 만들어 다른 데이터를 가져올 수 있으므로 Azure 이벤트 허브 계정당 하나의 연결만 필요합니다.
...
이 튜토리얼을 따라 API를 사용하여 Azure 이벤트 허브 연결을 만들고 응답 본체의 일부로 고유 ID를 받았습니다. 이 연결 ID를 사용하여 Flow Service API를 [사용하여 클라우드 스토리지를 탐색할 수 있습니다](../../explore/cloud-storage.md).