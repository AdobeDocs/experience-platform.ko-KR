...
| `connectionSpec.id` | 이전 단계에서 검색되는 ADLS Gen2 `id` 계정의 연결 사양입니다. |
...
이 자습서를 따라 API를 사용하여 ADLS Gen2 연결을 만들고 응답 본문의 일부로 고유 ID를 받았습니다. 이 연결 ID를 사용하여 Flow Service API를 사용하여 클라우드 [스토리지를](../../explore/cloud-storage.md) 탐색하거나 Flow Service API를 사용하여 [인제스트](../../cloud-storage-parquet.md)쪽모이 세공 데이터를 생성할 수 있습니다.
