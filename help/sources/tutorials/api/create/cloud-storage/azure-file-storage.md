...
이 자습서에서는 Flow Service API를 사용하여 Azure 파일 저장소를 Experience Platform에 연결하는 단계를 안내합니다.
...
다음 섹션에서는 Flow Service API를 사용하여 Azure 파일 저장소에 성공적으로 연결하려면 알아야 할 추가 정보를 제공합니다.

### 필요한 자격 증명 수집

Flow Service가 Azure 파일 저장소에 연결하려면 다음 연결 속성에 대한 값을 제공해야 합니다.

| 자격 증명 | 설명 |
| ---------- | ----------- |
| `host` | 액세스하는 Azure 파일 저장소 인스턴스의 끝점입니다. |
| `userId` | Azure 파일 저장소 종단점에 액세스할 수 있는 사용자입니다. |
| `password` | Azure 파일 저장소 인스턴스의 암호 |
| 연결 사양 ID | 연결을 만드는 데 필요한 고유 식별자입니다. Azure 파일 저장소의 연결 사양 ID: `be5ec48c-5b78-49d5-b8fa-7c89ec4569b8` |
...
| 속성 | 설명 |
| --------- | ----------- |
| `auth.params.host` | 액세스하는 Azure 파일 저장소 인스턴스의 끝점.. |
| `auth.params.userId` | Azure 파일 저장소 종단점에 액세스할 수 있는 사용자입니다. |
| `auth.params.password` | Azure 파일 저장소 액세스 키 |
| `connectionSpec.id` | Azure 파일 저장소 연결 사양 ID: `be5ec48c-5b78-49d5-b8fa-7c89ec4569b8`. |
...
이 튜토리얼을 따라 Flow Service API를 사용하여 Azure 파일 저장소 연결을 만들고 연결 고유 ID 값을 받았습니다. 다음 자습서에서는 Flow Service API를 사용하여 타사 클라우드 스토리지를 [탐색하는 방법을 학습하면서 이 ID를 사용할 수 있습니다](../../explore/cloud-storage.md).
