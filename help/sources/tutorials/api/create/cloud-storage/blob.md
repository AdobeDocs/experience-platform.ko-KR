...
title: 흐름 서비스 API를 사용하여 Azure Blob 커넥터 만들기
topic: overview
...
# 흐름 서비스 API를 사용하여 Azure Blob 커넥터 만들기
...
이 자습서에서는 Flow Service API를 사용하여 경험 플랫폼을 Azure Blob(이하 &quot;Blob&quot;) 저장소에 연결하는 단계를 안내합니다.
...
Flow Service가 Blob 저장소에 연결하려면 다음 연결 속성 값을 제공해야 합니다.

| 자격 증명 | 설명 |
| ---------- | ----------- |
| `connectionString` | Blob 저장소의 데이터에 액세스하는 데 필요한 연결 문자열입니다. |

시작하는 방법에 대한 자세한 내용은 [이 Azure Blob 문서를](https://docs.microsoft.com/en-us/azure/storage/common/storage-configure-connection-string)참조하십시오.
...
Blob 저장소에 플랫폼을 연결하기 전에 Blob에 대한 연결 사양이 있는지 확인해야 합니다. 연결 사양이 없으면 연결할 수 없습니다.

사용 가능한 각 소스에는 인증 요구 사항과 같은 커넥터 속성을 설명하는 고유한 연결 사양 세트가 있습니다. GET 요청을 수행하고 쿼리 매개 변수를 사용하여 Blob에 대한 연결 사양을 조회할 수 있습니다.

**API 형식**

쿼리 매개 변수 없이 GET 요청을 전송하면 사용 가능한 모든 소스에 대한 연결 사양이 반환됩니다. 쿼리를 포함하여 Blob에 `property=name=="azure-blob"` 대한 정보를 얻을 수 있습니다.
...
성공적인 응답은 고유 식별자(`id`)를 포함하여 Blob에 대한 연결 사양을 반환합니다. 이 ID는 기본 연결을 만들려면 다음 단계에서 필요합니다.
...
기본 연결은 소스를 지정하고 해당 소스에 대한 자격 증명을 포함합니다. 다른 데이터를 가져오기 위해 여러 소스 커넥터를 만드는 데 사용할 수 있으므로 Blob 계정당 하나의 기본 연결만 필요합니다.
...
| 속성 | 설명 |
| -------- | ----------- |
| `auth.params.connectionString` | Blob 저장소에 대한 연결 문자열입니다. |
| `connectionSpec.id` | 이전 단계에서 검색된 Blob `id` 저장소의 연결 사양 |
...
이 자습서를 따라 API를 사용하여 Blob 연결을 만들고 응답 본문의 일부로 고유한 ID를 얻었습니다. 이 연결 ID를 사용하여 Flow Service API를 사용하여 클라우드 [스토리지를](../../explore/cloud-storage.md) 탐색하거나 Flow Service API를 사용하여 [인제스트](../../cloud-storage-parquet.md)쪽모이 세공 데이터를 생성할 수 있습니다.