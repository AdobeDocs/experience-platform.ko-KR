...
다음 섹션에서는 [!DNL Flow Service] API를 사용하여 [!DNL Kinesis]을(를) 플랫폼에 성공적으로 연결하기 위해 알아야 할 추가 정보를 제공합니다.
...
| 자격 증명 | 설명 |
| ---------- | ----------- |
| `accessKeyId` | 액세스 키 ID는 [!DNL Kinesis] 계정을 Platform에 인증하는 데 사용되는 액세스 키 쌍의 절반입니다. |
| `secretKey` | 보안 액세스 키는 [!DNL Kinesis] 계정을 Platform에 인증하는 데 사용되는 액세스 키 쌍의 나머지 절반입니다. |
...
기본 연결 ID를 만들려면 [!DNL Kinesis] 인증 자격 증명을 요청 매개 변수의 일부로 제공하는 동안 `/connections` 끝점에 POST 요청을 하십시오.
