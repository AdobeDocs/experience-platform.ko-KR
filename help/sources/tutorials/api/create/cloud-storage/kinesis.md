...
# Flow Service API를 사용하여 [!DNL Amazon Kinesis] 소스 연결 만들기

>[!NOTE]
>
>[!DNL Amazon Kineses] 커넥터가 베타에 있습니다. 베타 레이블이 지정된 커넥터 사용에 대한 자세한 내용은 [소스 개요](../../../../home.md#terms-and-conditions)를 참조하십시오.
...
이 자습서에서는 [!DNL Flow Service] API를 사용하여 [!DNL Experience Platform] 계정을 연결하는 단계를 안내합니다.[!DNL Amazon Kinesis]
...
다음 섹션에서는 [!DNL Flow Service] API를 사용하여 [!DNL Amazon Kinesis] 계정에 성공적으로 연결하려면 알아야 할 추가 정보를 제공합니다.

### 필요한 자격 증명 수집

[!DNL Flow Service]이(가) [!DNL Amazon Kinesis] 계정과 연결하려면 다음 연결 속성에 대한 값을 제공해야 합니다.

| 자격 증명 | 설명 |
| ---------- | ----------- |
| `accessKeyId` | [!DNL Kinesis] 계정의 액세스 키 ID입니다. |
| `secretKey` | [!DNL Kinesis] 계정의 비밀 액세스 키. |
| `region` | [!DNL Kinesis] 계정의 영역입니다. |
| `connectionSpec.id` | [!DNL Kinesis] 연결 사양 ID:`86043421-563b-46ec-8e6c-e23184711bf6` |

이러한 값에 대한 자세한 내용은 [이 Kinesis 문서](https://docs.aws.amazon.com/streams/latest/dev/getting-started.html)를 참조하십시오.
...
연결은 소스를 지정하며 해당 소스의 자격 증명을 포함합니다. 다른 데이터를 가져오기 위해 여러 소스 커넥터를 만드는 데 사용할 수 있으므로 [!DNL Amazon Kinesis] 계정에는 하나의 연결만 필요합니다.
...
이 자습서를 따라 API를 사용하여 [!DNL Amazon Kinesis] 연결을 만들고 고유한 ID를 응답 본문의 일부로 받았습니다. 이 연결 ID를 사용하여 Flow Service API](../../collect/streaming.md)를 사용하여 스트리밍 데이터를 수집할 수 있습니다.[