...
# 만들기 [!DNL Oracle Object Storage] 를 사용한 기본 연결 [!DNL Flow Service] API

기본 연결은 소스와 Adobe Experience Platform 간의 인증된 연결을 나타냅니다.

이 자습서에서는 기본 연결을 만드는 단계를 안내합니다. [!DNL Oracle Object Storage] 사용 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
다음 섹션에서는 에 성공적으로 연결하기 위해 알아야 하는 추가 정보를 제공합니다 [!DNL Oracle Object Storage] 사용 [!DNL Flow Service] API.

### 필요한 자격 증명 수집

주문 [!DNL Flow Service] 에 연결하려면 [!DNL Oracle Object Storage], 다음 연결 속성에 대한 값을 제공해야 합니다.
...
| `connectionSpec.id` | 연결 사양은 기본 및 소스 연결 만들기와 관련된 인증 사양을 포함하여 소스의 커넥터 속성을 반환합니다. 에 대한 연결 사양 ID [!DNL Oracle Object Storage] 은(는) `c85f9425-fb21-426c-ad0b-405e9bd8a46c`. |
...
POST 기본 연결 ID를 만들려면 `/connections` 을(를) 제공하는 동안 엔드포인트 [!DNL Oracle Object Storage] 요청 매개 변수의 일부로 인증 자격 증명을 사용합니다.
...
| `auth.params.accessKey` | 다음 [!DNL Oracle Object Storage] 인증에 필요한 액세스 키 ID. |
...
이 자습서를 따라 [!DNL Oracle Object Storage] 을 사용한 연결 [!DNL Flow Service] API 및 고유한 연결 ID를 가져왔습니다. 이 연결 ID를 사용하여 다음을 수행할 수 있습니다. [흐름 서비스 API를 사용하여 클라우드 저장소 살펴보기](../../explore/cloud-storage.md).
