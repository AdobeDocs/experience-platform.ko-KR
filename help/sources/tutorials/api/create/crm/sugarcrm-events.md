---
title: Flow Service API를 사용하여 SugarCRM 이벤트에 대한 소스 연결 및 데이터 흐름을 만듭니다
description: Flow Service API를 사용하여 Adobe Experience Platform을 SugarCRM 이벤트에 연결하는 방법을 알아봅니다.
...
# (베타) 다음에 대한 소스 연결 및 데이터 흐름 만들기 [!DNL SugarCRM Events] 흐름 서비스 API 사용

>[!NOTE]
>
>다음 [!DNL SugarCRM Events] 소스가 베타 버전입니다. 자세한 내용은 [소스 개요](../../../../home.md#terms-and-conditions) 베타 레이블이 지정된 소스 사용에 대한 자세한 정보.

다음 자습서에서는 다음을 만드는 단계를 안내합니다 [!DNL SugarCRM Events] 소스 연결 및 가져올 데이터 흐름 만들기 [[!DNL SugarCRM]](https://www.sugarcrm.com/) 이벤트를 사용하여 Adobe Experience Platform에 데이터 보내기 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
다음 섹션에서는에 성공적으로 연결하기 위해 알고 있어야 하는 추가 정보를 제공합니다 [!DNL SugarCRM] 사용 [!DNL Flow Service] API.
...
## Connect [!DNL SugarCRM Events] 를 사용하여 플랫폼 구현 [!DNL Flow Service] API

다음은 인증을 위해 수행해야 하는 단계에 대해 설명합니다 [!DNL SugarCRM] 소스, 소스 연결 만들기 및 데이터 흐름 을 만들어 이벤트 데이터를 Experience Platform으로 가져옵니다.

### 기본 연결 만들기 {#base-connection}

기본 연결은 소스의 인증 자격 증명, 현재 연결 상태 및 고유한 기본 연결 ID를 포함하여 소스와 플랫폼 간의 정보를 유지합니다. 기본 연결 ID를 사용하면 소스 내에서 파일을 탐색 및 탐색하고 해당 데이터 유형 및 형식에 대한 정보를 포함하여 수집할 특정 항목을 식별할 수 있습니다.

기본 연결 ID를 만들려면 `/connections` 제공하는 동안 엔드포인트 [!DNL SugarCRM Events] 요청 본문의 일부로 인증 자격 증명.
...
| `{SOURCE_PARAMS}` | Platform으로 가져올 소스 파일의 매개 변수를 정의합니다. 에 대해 허용되는 형식 유형을 검색하려면 `{SOURCE_PARAMS}`를 설정하는 경우 base64에서 전체 문자열을 인코딩해야 합니다. <br> [!DNL SugarCRM Events] 페이로드가 필요하지 않습니다. 에 대한 값 `{SOURCE_PARAMS}` 이 `{}`, base64에서 인코딩되었으며 다음과 같습니다. `e30=` 아래와 같이 표시됩니다. |
...
성공적인 응답은 쿼리된 파일의 구조를 반환합니다. *더 나은 프레젠테이션을 위해 일부 레코드가 제거되었습니다.*
...
| `baseConnectionId` | 의 기본 연결 ID입니다. [!DNL SugarCRM Events]. 이 ID는 이전 단계에서 생성되었습니다. |
| `connectionSpec.id` | 소스에 해당하는 연결 사양 ID입니다. |
| `data.format` | 의 형식 [!DNL SugarCRM Events] 수집할 데이터입니다. 현재 지원되는 데이터 형식은 `json`. |
...
| `connectionSpec.id` | 데이터 레이크에 해당하는 연결 사양 ID입니다. 이 고정 ID는 다음과 같습니다. `6b137bf6-d2a0-48c8-914b-d50f4942eb85`. |
| `data.format` | 의 형식 [!DNL SugarCRM Events] 수집할 데이터입니다. |
...
에서 데이터를 가져오는 마지막 단계 [!DNL SugarCRM Events] 플랫폼 목표는 데이터 흐름을 만드는 것입니다. 현재까지는 다음 필수 값이 준비되었습니다.
...
수집을 예약하려면 먼저 시작 시간 값을 초 단위로 설정해야 합니다. 그런 다음 빈도 값을 다음 중 하나로 설정해야 합니다 `hour` 또는 `day`. 간격 값은 두 개의 연속 인스턴스 사이의 기간을 지정합니다. 간격 값은 `1` 또는 `24` 에 따라 `scheduleParams.frequency` 선택 `hour` 또는 `day`.
...
| `scheduleParams.frequency` | 데이터 흐름에서 데이터를 수집하는 빈도입니다. 허용되는 값은 다음과 같습니다. `hour` 또는 `day`. |
| `scheduleParams.interval` | 간격은 두 개의 연속 흐름 실행 사이의 기간을 지정합니다. 간격 값은 0이 아닌 정수여야 합니다. 간격 값은 `1` 또는 `24` 에 따라 `scheduleParams.frequency` 선택 `hour` 또는 `day`. |
