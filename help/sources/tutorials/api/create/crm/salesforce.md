...
# Flow Service API를 사용하여 Salesforce 커넥터 만들기
...
이 자습서에서는 Flow Service API를 사용하여 CRM 데이터 수집을 위해 Salesforce 계정에 플랫폼을 연결하는 단계를 안내합니다.
...
다음 섹션에서는 Flow Service API를 사용하여 성공적으로 Salesforce 계정에 플랫폼을 연결하기 위해 알아야 하는 추가 정보를 제공합니다.

### 필요한 자격 증명 수집

Flow Service가 Salesforce에 연결하려면 다음 연결 속성에 대한 값을 제공해야 합니다.

| 자격 증명 | 설명 |
| ---------- | ----------- |
| `environmentUrl` | Salesforce 소스 인스턴스의 URL입니다. |
| `username` | Salesforce 사용자 계정의 사용자 이름입니다. |
| `password` | Salesforce 사용자 계정의 암호입니다. |
| `securityToken` | Salesforce 사용자 계정의 보안 토큰입니다. |

시작하는 방법에 대한 자세한 내용은 [이 Salesforce 문서를](https://developer.salesforce.com/docs/atlas.en-us.api_rest.meta/api_rest/intro_understanding_authentication.htm)참조하십시오.
...
Salesforce 계정에 플랫폼을 연결하기 전에 Salesforce에 대한 연결 사양이 있는지 확인해야 합니다. 연결 사양이 없으면 연결할 수 없습니다.

사용 가능한 각 소스에는 인증 요구 사항과 같은 커넥터 속성을 설명하는 고유한 연결 사양 세트가 있습니다. GET 요청을 수행하고 쿼리 매개 변수를 사용하여 Salesforce의 연결 사양을 조회할 수 있습니다.

**API 형식**

쿼리 매개 변수 없이 GET 요청을 전송하면 사용 가능한 모든 소스에 대한 연결 사양이 반환됩니다. 쿼리를 포함하여 Salesforce에 대한 정보를 얻을 `property=name=="salesforce"` 수 있습니다.
...
다음 요청은 Salesforce의 연결 사양을 검색합니다.
...
성공적인 응답은 고유 식별자(`id`)를 포함하여 Salesforce에 대한 연결 사양을 반환합니다. 이 ID는 기본 연결을 만들려면 다음 단계에서 필요합니다.
...
기본 연결은 소스를 지정하고 해당 소스에 대한 자격 증명을 포함합니다. 여러 소스 커넥터를 만들어 다른 데이터를 가져오는 데 사용할 수 있으므로 Salesforce 계정당 하나의 기본 연결만 필요합니다.
...
| 속성 | 설명 |
| -------- | ----------- |
| `auth.params.username` | Salesforce 계정과 연결된 사용자 이름입니다. |
| `auth.params.password` | Salesforce 계정과 연결된 암호입니다. |
| `auth.params.securityToken` | Salesforce 계정과 연결된 보안 토큰입니다. |
| `connectionSpec.id` | 이전 단계에서 검색한 Salesforce `id` 계정의 연결 사양 |
...
이 튜토리얼을 따라 API를 사용하여 Salesforce 계정에 대한 기본 연결을 만들고 응답 본문의 일부로 고유 ID를 받았습니다. 다음 자습서에서 Flow Service API를 사용하여 CRM 시스템을 [탐색하는 방법을 배울 때 이 기본 연결 ID를 사용할 수 있습니다](../../explore/crm.md).