...
# 흐름 서비스 API를 사용하여 [!DNL SugarCRM Accounts & Contacts]에 대한 소스 연결 및 데이터 흐름을 만듭니다.

다음 자습서에서는 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)를 사용하여 [[!DNL SugarCRM]](https://www.sugarcrm.com/) 계정 및 연락처 데이터를 Adobe Experience Platform으로 가져오기 위해 [!DNL SugarCRM Accounts & Contacts] 소스 연결을 만들고 데이터 흐름을 만드는 단계를 안내합니다.
...
## [!DNL Flow Service] API를 사용하여 [!DNL SugarCRM Accounts & Contacts]을(를) Experience Platform에 연결

다음은 [!DNL SugarCRM] 원본을 인증하고, 원본 연결을 만들고, 데이터 흐름을 만들어 계정과 연락처 데이터를 Experience Platform으로 가져오기 위해 수행해야 하는 단계를 간략하게 설명합니다.
...
기본 연결 ID를 만들려면 [!DNL SugarCRM Accounts & Contacts] 인증 자격 증명을 요청 본문의 일부로 제공하는 동안 `/connections` 끝점에 대한 POST 요청을 만듭니다.
...
다음 요청은 [!DNL SugarCRM Accounts & Contacts]에 대한 기본 연결을 만듭니다.
...
| `{SOURCE_PARAMS}` | Experience Platform으로 가져올 소스 파일의 매개 변수를 정의합니다. `{SOURCE_PARAMS}`에 대해 허용되는 형식 유형을 검색하려면 전체 문자열을 base64로 인코딩해야 합니다. <br> [!DNL SugarCRM Accounts & Contacts]이(가) 여러 API를 지원합니다. 활용하고 있는 오브젝트 유형에 따라 다음 중 하나를 전달하십시오. <ul><li>`accounts` : 귀사와 관계를 맺고 있는 회사입니다.</li><li>`contacts` : 조직이 관계를 설정한 개별 사용자입니다.</li></ul> |

[!DNL SugarCRM Accounts & Contacts]은(는) 여러 API를 지원합니다. 보낼 요청을 활용하는 오브젝트 유형에 따라 다음과 같습니다.
...
[!DNL SugarCRM] 계정 API의 경우 `{SOURCE_PARAMS}`의 값이 `{"object_type":"accounts"}`(으)로 전달됩니다. Base64로 인코딩하면 아래와 같이 `eyJvYmplY3RfdHlwZSI6ImFjY291bnRzIn0=`과(와) 같습니다.
...
[!DNL SugarCRM] 연락처 API의 경우 `{SOURCE_PARAMS}`의 값이 `{"object_type":"contacts"}`(으)로 전달됩니다. Base64로 인코딩하면 아래와 같이 `eyJvYmplY3RfdHlwZSI6ImNvbnRhY3RzIn0=`과(와) 같습니다.
...
다음 요청은 [!DNL SugarCRM Accounts & Contacts]에 대한 소스 연결을 만듭니다.
...
[!DNL SugarCRM] 계정 API의 경우 `object_type` 속성 값은 `accounts`이어야 합니다.
...
[!DNL SugarCRM] 연락처 API의 경우 `object_type` 속성 값은 `contacts`이어야 합니다.
...
| `baseConnectionId` | [!DNL SugarCRM Accounts & Contacts]의 기본 연결 ID. 이 ID는 이전 단계에서 생성되었습니다. |
| `connectionSpec.id` | 소스에 해당하는 연결 사양 ID. |
| `data.format` | 수집할 [!DNL SugarCRM Accounts & Contacts] 데이터의 형식입니다. 현재 지원되는 데이터 형식은 `json`뿐입니다. |
| `object_type` | [!DNL SugarCRM Accounts & Contacts]이(가) 여러 API를 지원합니다. 활용하고 있는 오브젝트 유형에 따라 다음 중 하나를 전달하십시오. <ul><li>`accounts` : 귀사와 관계를 맺고 있는 회사입니다.</li><li>`contacts` : 조직이 관계를 설정한 개별 사용자입니다.</li></ul> |
...
다음 요청은 [!DNL SugarCRM Accounts & Contacts]에 대한 대상 연결을 만듭니다.
...
| `data.format` | 수집할 [!DNL SugarCRM Accounts & Contacts] 데이터의 형식입니다. |
...
[!DNL SugarCRM Accounts & Contacts]에서 Experience Platform으로 데이터를 가져오는 마지막 단계는 데이터 흐름을 만드는 것입니다. 이제 다음 필수 값이 준비되었습니다.
...
기본 연결 ID를 쿼리 매개 변수로 제공하면서 [!DNL Flow Service] API에 대한 PATCH 요청을 수행하여 소스 계정의 이름, 설명 및 자격 증명을 업데이트합니다. PATCH을 요청할 때 `If-Match` 헤더에 소스 계정의 고유 `etag`을(를) 제공해야 합니다. 전체 API 예제는 [API를 사용하여 소스 계정을 업데이트하는 방법](https://experienceleague.adobe.com/docs/experience-platform/sources/api-tutorials/update.html)에 대한 안내서를 참조하십시오.
