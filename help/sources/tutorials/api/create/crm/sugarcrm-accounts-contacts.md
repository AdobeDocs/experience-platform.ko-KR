...
## 연결 [!DNL SugarCRM Accounts & Contacts] 를 사용하여 플랫폼으로 [!DNL Flow Service] API

다음은 를 인증하기 위해 수행해야 하는 단계입니다. [!DNL SugarCRM] 소스, 소스 연결 만들기 및 데이터 흐름을 만들어 계정과 연락처 데이터를 Experience Platform 상태로 만듭니다.

### 기본 연결 만들기 {#base-connection}

기본 연결은 소스의 인증 자격 증명, 연결의 현재 상태 및 고유한 기본 연결 ID를 포함하여 소스와 플랫폼 간에 정보를 유지합니다. 기본 연결 ID를 사용하면 소스 내에서 파일을 탐색 및 탐색하고 데이터 유형 및 형식에 대한 정보를 포함하여 수집할 특정 항목을 식별할 수 있습니다.

POST 기본 연결 ID를 만들려면 `/connections` 을(를) 제공하는 동안 엔드포인트 [!DNL SugarCRM Accounts & Contacts] 요청 본문의 일부인 인증 자격 증명입니다.
...
다음 요청은에 대한 기본 연결을 만듭니다. [!DNL SugarCRM Accounts & Contacts]:
...
| `{SOURCE_PARAMS}` | Platform으로 가져올 소스 파일의 매개 변수를 정의합니다. 에 대해 허용된 형식 유형을 검색하려면 `{SOURCE_PARAMS}`, 전체 문자열을 base64에서 인코딩해야 합니다. <br> [!DNL SugarCRM Accounts & Contacts] 는 여러 API를 지원합니다. 활용하고 있는 오브젝트 유형에 따라 다음 중 하나를 전달하십시오. <ul><li>`accounts` : 조직이 관계를 맺고 있는 회사입니다.</li><li>`contacts` : 조직이 관계를 설정한 개별 사용자입니다.</li></ul> |

다음 [!DNL SugarCRM Accounts & Contacts] 는 여러 API를 지원합니다. 보낼 요청을 활용하는 오브젝트 유형에 따라 다음과 같습니다.
...
대상 [!DNL SugarCRM] 다음 값 계정 API: `{SOURCE_PARAMS}` 다음으로 전달됨 `{"object_type":"accounts"}`. Base64로 인코딩하면 `eyJvYmplY3RfdHlwZSI6ImFjY291bnRzIn0=` 아래와 같이 표시됩니다.
...
대상 [!DNL SugarCRM] 다음에 대한 값을 가진 연락처 API `{SOURCE_PARAMS}` 다음으로 전달됨 `{"object_type":"contacts"}`. base64로 인코딩하면 다음과 같습니다. `eyJvYmplY3RfdHlwZSI6ImNvbnRhY3RzIn0=` 아래와 같이 표시됩니다.
...
다음 요청은에 대한 소스 연결을 만듭니다. [!DNL SugarCRM Accounts & Contacts]:
...
대상 [!DNL SugarCRM] 계정 API `object_type` 속성 값은 다음과 같아야 합니다. `accounts`.
...
대상 [!DNL SugarCRM] 연락처 API `object_type` 속성 값은 다음과 같아야 합니다. `contacts`.
...
| `baseConnectionId` | 의 기본 연결 ID [!DNL SugarCRM Accounts & Contacts]. 이 ID는 이전 단계에서 생성되었습니다. |
| `connectionSpec.id` | 소스에 해당하는 연결 사양 ID. |
| `data.format` | 형식 [!DNL SugarCRM Accounts & Contacts] 수집할 데이터. 현재 지원되는 데이터 형식은 `json`. |
| `object_type` | [!DNL SugarCRM Accounts & Contacts] 는 여러 API를 지원합니다. 활용하고 있는 오브젝트 유형에 따라 다음 중 하나를 전달하십시오. <ul><li>`accounts` : 조직이 관계를 맺고 있는 회사입니다.</li><li>`contacts` : 조직이 관계를 설정한 개별 사용자입니다.</li></ul> |
...
다음 요청은에 대한 대상 연결을 만듭니다. [!DNL SugarCRM Accounts & Contacts]:
...
| `data.format` | 형식 [!DNL SugarCRM Accounts & Contacts] 수집할 데이터. |
