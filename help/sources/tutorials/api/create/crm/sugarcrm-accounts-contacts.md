---
title: Flow Service API를 사용하여 SugarCRM 계정 및 연락처에 대한 소스 연결 및 데이터 흐름 만들기
description: Flow Service API를 사용하여 Adobe Experience Platform을 SugarCRM 계정 및 연락처에 연결하는 방법을 알아봅니다.
...
# (베타) 다음에 대한 소스 연결 및 데이터 흐름 만들기 [!DNL SugarCRM Accounts & Contacts] 흐름 서비스 API 사용

>[!NOTE]
>
>다음 [!DNL SugarCRM Accounts & Contacts] 소스가 베타 버전입니다. 자세한 내용은 [소스 개요](../../../../home.md#terms-and-conditions) 베타 레이블이 지정된 소스 사용에 대한 자세한 정보.

다음 자습서에서는 다음을 만드는 단계를 안내합니다 [!DNL SugarCRM Accounts & Contacts] 소스 연결 및 가져올 데이터 흐름 만들기 [[!DNL SugarCRM]](https://www.sugarcrm.com/) 계정 및 연락처 데이터를 사용하여 Adobe Experience Platform에 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
## Connect [!DNL SugarCRM Accounts & Contacts] 를 사용하여 플랫폼 구현 [!DNL Flow Service] API

다음은 인증을 위해 수행해야 하는 단계에 대해 설명합니다 [!DNL SugarCRM] 소스, 소스 연결 만들기, 데이터 흐름 만들기 등의 작업을 수행하여 계정 및 연락처 데이터를 Experience Platform에 가져옵니다.

### 기본 연결 만들기 {#base-connection}

기본 연결은 소스의 인증 자격 증명, 현재 연결 상태 및 고유한 기본 연결 ID를 포함하여 소스와 플랫폼 간의 정보를 유지합니다. 기본 연결 ID를 사용하면 소스 내에서 파일을 탐색 및 탐색하고 해당 데이터 유형 및 형식에 대한 정보를 포함하여 수집할 특정 항목을 식별할 수 있습니다.

기본 연결 ID를 만들려면 `/connections` 제공하는 동안 엔드포인트 [!DNL SugarCRM Accounts & Contacts] 요청 본문의 일부로 인증 자격 증명.
...
| `{SOURCE_PARAMS}` | Platform으로 가져올 소스 파일의 매개 변수를 정의합니다. 에 대해 허용되는 형식 유형을 검색하려면 `{SOURCE_PARAMS}`를 설정하는 경우 base64에서 전체 문자열을 인코딩해야 합니다. <br> [!DNL SugarCRM Accounts & Contacts] 은 여러 API를 지원합니다. 활용하는 개체 유형에 따라 아래 중 하나를 전달합니다. <ul><li>`accounts` : 조직과 관계가 있는 회사.</li><li>`contacts` : 조직에 관계가 설정된 개별 사용자입니다.</li></ul> |

다음 [!DNL SugarCRM Accounts & Contacts] 은 여러 API를 지원합니다. 전송 요청을 활용하는 개체 유형에 따라 다음과 같습니다.
...
대상 [!DNL SugarCRM] 계정 API에 대한 값 `{SOURCE_PARAMS}` 이 `{"object_type":"accounts"}`. base64에서 인코딩되면 다음과 같습니다 `eyJvYmplY3RfdHlwZSI6ImFjY291bnRzIn0=` 아래와 같이 표시됩니다.
...
대상 [!DNL SugarCRM] Contact API에 대한 값 `{SOURCE_PARAMS}` 이 `{"object_type":"contacts"}`. base64에서 인코딩하면 다음과 같습니다. `eyJvYmplY3RfdHlwZSI6ImNvbnRhY3RzIn0=` 아래와 같이 표시됩니다.
...
마찬가지로, 수신한 응답을 활용하는 객체 유형에 따라 다음과 같이 달라집니다.
...
활용하는 객체 유형에 따라 아래 탭에서 을 선택합니다.

>[!BEGINTABS]

>[!TAB 계정]

대상 [!DNL SugarCRM] 계정 API `object_type` 속성 값은 `accounts`.
...
대상 [!DNL SugarCRM] API에 문의 `object_type` 속성 값은 `contacts`.
...
| `baseConnectionId` | 의 기본 연결 ID입니다. [!DNL SugarCRM Accounts & Contacts]. 이 ID는 이전 단계에서 생성되었습니다. |
| `connectionSpec.id` | 소스에 해당하는 연결 사양 ID입니다. |
| `data.format` | 의 형식 [!DNL SugarCRM Accounts & Contacts] 수집할 데이터입니다. 현재 지원되는 데이터 형식은 `json`. |
| `object_type` | [!DNL SugarCRM Accounts & Contacts] 은 여러 API를 지원합니다. 활용하는 개체 유형에 따라 아래 중 하나를 전달합니다. <ul><li>`accounts` : 조직과 관계가 있는 회사.</li><li>`contacts` : 조직에 관계가 설정된 개별 사용자입니다.</li></ul> |
| `path` | 이 값에는 선택한 값이 동일합니다 *`object_type`*. |
...
| `data.format` | 의 형식 [!DNL SugarCRM Accounts & Contacts] 수집할 데이터입니다. |
...
에서 데이터를 가져오는 마지막 단계 [!DNL SugarCRM Accounts & Contacts] 플랫폼 목표는 데이터 흐름을 만드는 것입니다. 현재까지는 다음 필수 값이 준비되었습니다.
