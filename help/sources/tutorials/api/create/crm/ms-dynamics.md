...
topic: overview
type: Tutorial
description: 이 자습서에서는 Flow Service API를 사용하여 CRM 데이터를 수집하기 위해 플랫폼을 Microsoft Dynamics(이하 "Dynamics") 계정에 연결하는 단계를 안내합니다.
...
# [!DNL Flow Service] API를 사용하여 [!DNL Microsoft Dynamics] 커넥터를 만듭니다.
...
이 자습서는 [!DNL Flow Service] API를 사용하여 [!DNL Platform]을(를) CRM 데이터 수집을 위해 [!DNL Microsoft Dynamics](이하 &quot;Dynamics&quot;) 계정에 연결하는 단계를 안내합니다.
...
* [샌드박스](../../../../../sandboxes/home.md):디지털 경험 [!DNL xperience Platform] 애플리케이션을 개발 및 발전시키는 데 도움이 되도록 단일  [!DNL Platform] 인스턴스를 별도의 가상 환경으로 분할하는 가상 샌드박스를 제공합니다.

다음 섹션에서는 [!DNL Flow Service] API를 사용하여 [!DNL Platform]을(를) Dynamics 계정에 성공적으로 연결하려면 알아야 할 추가 정보를 제공합니다.
...
시작하는 방법에 대한 자세한 내용은 [이 Dynamics 문서](https://docs.microsoft.com/en-us/powerapps/developer/common-data-service/authenticate-oauth)를 참조하십시오.
...
연결은 소스를 지정하며 해당 소스의 자격 증명을 포함합니다. 다른 데이터를 가져오기 위해 여러 소스 커넥터를 만드는 데 사용할 수 있으므로 [!DNL Dynamics] 계정에는 하나의 연결만 필요합니다.
