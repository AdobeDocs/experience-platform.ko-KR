...
사용 가능한 각 소스에는 인증 요구 사항과 같은 커넥터 속성을 설명하는 고유한 연결 사양이 있습니다. GET 요청을 `/connectionSpecs` 끝점으로 보내면 사용 가능한 모든 소스에 대한 연결 사양이 반환됩니다. 또한 쿼리를 포함하여 특정 `property=name=="hubspot"` 의 정보를 얻을 수도 있습니다 [!DNL HubSpot].
...
성공적인 응답은 고유 식별자( [!DNL HubSpot])를`id`비롯한 연결 사양을 반환합니다. API에 대한 연결을 만들려면 다음 단계에서 이 ID가 필요합니다.
