...
이 자습서에서는 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)를 사용하여 데이터 흐름을 업데이트하는 단계, 설명 및 일정을 다룹니다.
...
데이터 흐름을 업데이트하는 첫 번째 단계는 흐름 ID를 사용하여 데이터 흐름 세부 사항을 검색하는 것입니다. `/flows` 종단점에 GET 요청을 수행하여 기존 데이터 흐름의 현재 세부 사항을 볼 수 있습니다.
...
이 자습서에 따라 [!DNL Flow Service] API를 사용하여 데이터 흐름의 실행 일정, 이름 및 설명을 업데이트했습니다. 소스 커넥터 사용에 대한 자세한 내용은 [소스 개요](../../home.md)를 참조하십시오.
