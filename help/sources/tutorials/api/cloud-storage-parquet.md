...
# [!DNL Flow Service] API를 사용하여 타사 클라우드 스토리지 시스템에서 Parquet 데이터를 수집
...
[!DNL Platform] API를 사용하여 Parquet 데이터를 수집하려면, 액세스 중인 타사 클라우드 스토리지 소스에 대해 유효한 연결을 보유해야 합니다. 작업할 스토리지에 대한 연결이 아직 없는 경우 다음 자습서를 통해 연결을 만들 수 있습니다.
...
연결의 고유 식별자(`$id`)를 가져와 저장한 다음 이 자습서의 다음 단계를 진행합니다.

## 대상 스키마 만들기

소스 데이터를 [!DNL Platform]에 사용하려면 필요에 따라 소스 데이터를 구조화하기 위해 대상 스키마도 만들어야 합니다. 그런 다음 대상 스키마를 사용하여 소스 데이터가 포함된 [!DNL Platform] 데이터 세트를 만듭니다.

[!DNL Experience Platform]에서 사용자 인터페이스를 사용하려는 경우 [스키마 편집기 자습서](../../../xdm/tutorials/create-schema-ui.md)에서는 스키마 편집기에서 유사한 작업을 수행하는 단계별 지침을 제공합니다.
...
성공적으로 응답하면 고유 식별자(`$id`)를 포함하여 새로 만든 스키마의 세부 정보가 반환됩니다. 이 ID는 소스 연결을 만들려면 다음 단계에서 필요합니다.
...
이제 대상 XDM 스키마가 만들어지면 [!DNL Flow Service] API에 대한 POST 요청을 사용하여 소스 연결을 만들 수 있습니다. 소스 연결은 API에 대한 연결, 소스 데이터 형식 및 이전 단계에서 검색된 대상 XDM 스키마에 대한 참조로 구성됩니다.
...
성공적으로 응답하면 새로 만든 소스 연결의 고유 식별자(`id`)가 반환됩니다. 이 값은 타겟 연결을 만드는 이후의 단계에서 필요하므로 저장합니다.
...
외부 데이터를 [!DNL Platform]에 수집하려면 먼저 [!DNL Experience Platform] 데이터 집합 기본 연결을 획득해야 합니다.

데이터 집합 기본 연결을 만들려면 [데이터 집합 기본 연결 자습서](./create-dataset-base-connection.md)에 설명된 단계를 따르십시오.

데이터 세트 기본 연결을 만들 때까지 개발자 안내서에 설명된 단계를 계속 수행합니다. 고유 식별자(`$id`)를 가져와 저장하고 다음 단계에서 기본 연결 ID로 사용하여 대상 연결을 만듭니다.
...
이제 데이터 세트 기본 연결에 대한 고유 식별자, 대상 스키마 및 대상 데이터 세트가 있습니다. 이러한 식별자를 사용하여 [!DNL Flow Service] API를 사용하여 대상 연결을 만들어 인바운드 소스 데이터가 포함될 데이터 세트를 지정할 수 있습니다.
...
이 자습서에 따라 서드파티 클라우드 스토리지 시스템에서 Parquet 데이터를 수집하기 위한 소스 커넥터를 만들었습니다. 이제 [!DNL Real-time Customer Profile] 및 [!DNL Data Science Workspace] 등의 다운스트림 [!DNL Platform] 서비스에서 들어오는 데이터를 사용할 수 있습니다. 자세한 내용은 다음 문서를 참조하십시오.
