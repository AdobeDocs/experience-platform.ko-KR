...
이 자습서에서는 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)를 사용하여 완료, 오류 및 지표에 대한 흐름 실행 데이터를 모니터링하는 단계를 설명합니다.

## 시작하기

이 자습서에서는 유효한 데이터 흐름의 ID 값을 지정해야 합니다. 유효한 데이터 흐름 ID가 없는 경우 [소스 개요](../../home.md)에서 선택한 커넥터를 선택하고 이 자습서를 시작하기 전에 설명된 단계를 수행하십시오.
...
이 자습서를 따라 [!DNL Flow Service] API를 사용하여 데이터 흐름에서 지표 및 오류 정보를 검색했습니다. 이제 데이터 흐름을 수집 스케줄에 따라 계속 모니터링하여 상태 및 수집 비율을 추적할 수 있습니다. 사용자 인터페이스를 사용하여 동일한 작업을 수행하는 방법에 대한 자세한 내용은 사용자 인터페이스를 사용하여 데이터 흐름 모니터링 [에서 자습서를 참조하십시오](../ui/monitor.md)
