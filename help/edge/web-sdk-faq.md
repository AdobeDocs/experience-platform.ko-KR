...
**제어:** 모든 데이터가 하나의 연결된 데이터 스트림을 사용하고 있으므로 데이터 여정의 매 밀리초마다, 응용 프로그램 간 또는 모든 밀리초마다 데이터가 어떻게 표시되는지를 논리적으로 추적하고 제어할 수 있습니다.
...
## 고객이 웹 SDK를 사용하려면 Adobe Experience Platform을 구입해야 합니까?

아니요. 모든 Adobe Digital Experience 고객이 사용할 수 있습니다. 완전히 자유로워요 웹 SDK를 사용하려는 모든 고객은 Adobe Experience Platform UI에서 스키마 및 데이터 세트를 만들 수 있는 액세스 권한을 받습니다.
...
웹 SDK가 빠르게 진화하고 있습니다. 더 많은 사용 사례가 진행 중입니다. 현재 지원되는 사용 사례 목록은 [에 있습니다.](https://github.com/adobe/alloy/projects/5)
...
즉, 솔루션 이외의 사용 사례에 Adobe Experience Platform Web SDK를 사용하려는 경우 `alloy.js`으로 사이트에 태그를 지정하고 새 솔루션인 것처럼 계속 이동할 수 있습니다. Adobe Analytics, Target, Audience Manager 또는 애플리케이션 사용 사례에 사용하려는 경우 페이지에서 이전 코드를 제거해야 할 수 있습니다.
...
CNAME에 대한 자세한 내용은 [Adobe 설명서](https://experienceleague.adobe.com/docs/id-service/using/reference/analytics-reference/cname.html)에서 확인할 수 있습니다
...
**kndct_orgid_identity:**  id 쿠키는 ECID 및 ECID와 관련된 기타 정보를 저장하는 데 사용됩니다.

**kndctr_orgid_consent:**  이 쿠키는 웹 사이트에 대한 사용자의 동의 기본 설정을 저장합니다.

**kndctr_orgid_personalization:** 이 쿠키에는 Adobe Target이 웹 페이지를 개인화하는 데 사용하는 세션 정보가 포함되어 있습니다.

**kndctr_orgid_consencheck:** 이 세션 기반 쿠키는 서버에서 동의 환경 설정 서버 측을 조회하도록 신호를 보냅니다.
