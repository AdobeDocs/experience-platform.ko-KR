...
1. 먼저, [데이터 스트림 구성 페이지](../datastreams/configure.md).
2. 그런 다음 웹 SDK 명령 또는 웹 SDK 태그 확장을 사용하여 Edge Network에 무시를 보내야 합니다.

데이터 스트림 보기 [구성 무시 설명서](../datastreams/overrides.md) 데이터 스트림 구성을 재정의하는 방법에 대한 자세한 지침

Web SDK 명령을 통해 무시를 전달하는 대신 아래 표시된 태그 확장 화면에서 무시를 구성할 수 있습니다.

![Web SDK 태그 확장 페이지에서 데이터 스트림 구성 재정의가 표시되는 이미지입니다.](../assets/extension/overview/datastream-overrides.png)
