...
모든 링크 클릭 추적 이벤트에 대해 호출되기 바로 전에 콜백을 구성합니다. 콜백은 `xdm`, `clickedElement`, 및 `data` 필드.

DOM 요소 구조를 사용하여 링크 추적을 필터링할 때 `clickElement` 명령입니다. `clickedElement` 는 클릭되었으며 상위 노드 트리를 캡슐화한 DOM 요소 노드입니다.
