...
데이터 저장소는 데이터 스트림 구성 도구를 사용하여 데이터 수집 UI에서 만들 수 있습니다.

![데이터 세트 도구 탐색](../images/datastreams/config.png)

>[!NOTE]
>
>허용 목록에서 고객이 Platform을 태그 관리자로 사용하는지에 관계없이 데이터 스트림 구성 도구를 사용할 수 있습니다. 또한 사용자에게 개발 권한이 필요합니다. 자세한 내용은 태그 설명서의 [사용자 권한](../../tags/ui/administration/user-permissions.md) 문서를 참조하십시오.
...
데이터 스트림을 만들 때 동일한 설정으로 세 개의 환경이 자동으로 만들어집니다. 이러한 세 환경은 *dev*, *stage* 및 *prod*&#x200B;입니다. 태그의 세 가지 기본 환경과 일치합니다. 개발 환경에 태그 라이브러리를 빌드하면 라이브러리는 구성의 개발 환경을 자동으로 사용합니다. 개별 환경에서 원하는 만큼 설정을 편집할 수 있습니다.
