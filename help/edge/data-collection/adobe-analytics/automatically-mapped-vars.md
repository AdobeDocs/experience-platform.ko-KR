---
title: Adobe Experience Platform 웹 SDK에서 자동으로 매핑된 Adobe Analytics 변수
...
다음은 Adobe Experience Platform Edge Network가 자동으로 Adobe Analytics에 매핑하는 변수 목록입니다.
...
| `commerce.checkouts.id` | `events` | `scCheckout` 이벤트 정리. 이 필드를 제외하는 경우(즉, 일련화되지 않은 이벤트의 경우), 시스템은 자체 ID 값을 생성하여 엔티티에 할당합니다. |
| `commerce.checkouts.value` | `events` | 구분 기호 `,`을(를) 사용하여 전환 COMMERCE_SC_CHECKOUT을 사용하는 AppMeasurement 쿼리 매개 변수 EVENT_LIST_FULL 매핑을 참조하십시오. |
...
| `commerce.productListAdds.id` | `events` | `scAdd` 이벤트 정리. 이 필드를 제외하는 경우(즉, 일련화되지 않은 이벤트의 경우), 시스템은 자체 ID 값을 생성하여 엔티티에 할당합니다. |
| `commerce.productListAdds.value` | `events` | 구분 기호 `,`을(를) 사용하여 전환 COMMERCE_SC_ADD를 사용하는 AppMeasurement 쿼리 매개 변수 EVENT_LIST_FULL 매핑을 참조하십시오. |
| `commerce.productListOpens.id` | `events` | `scOpen` 이벤트 정리. 이 필드를 제외하는 경우(즉, 일련화되지 않은 이벤트의 경우), 시스템은 자체 ID 값을 생성하여 엔티티에 할당합니다. |
| `commerce.productListOpens.value` | `events` | 구분 기호 `,`을(를) 사용하여 전환 COMMERCE_SC_OPEN을 사용하는 AppMeasurement 쿼리 매개 변수 EVENT_LIST_FULL 매핑을 참조하십시오. |
| `commerce.productListRemovals.id` | `events` | `scRemove` 이벤트 정리. 이 필드를 제외하는 경우(즉, 일련화되지 않은 이벤트의 경우), 시스템은 자체 ID 값을 생성하여 엔티티에 할당합니다. |
| `commerce.productListRemovals.value` | `events` | 구분 기호 `,`을(를) 사용하여 전환 COMMERCE_SC_REMOVE를 사용하여 AppMeasurement 쿼리 매개 변수 EVENT_LIST_FULL 매핑을 참조하십시오. |
| `commerce.productListViews.id` | `events` | `scView` 이벤트 정리. 이 필드를 제외하는 경우(즉, 일련화되지 않은 이벤트의 경우), 시스템은 자체 ID 값을 생성하여 엔티티에 할당합니다. |
| `commerce.productListViews.value` | `events` | 구분 기호 `,`을(를) 사용하여 전환 COMMERCE_SC_VIEW를 사용하는 AppMeasurement 쿼리 매개 변수 EVENT_LIST_FULL 매핑을 참조하십시오. |
| `commerce.productViews.id` | `events` | `prodView` 이벤트 정리. 이 필드를 제외하는 경우(즉, 일련화되지 않은 이벤트의 경우), 시스템은 자체 ID 값을 생성하여 엔티티에 할당합니다. |
| `commerce.productViews.value` | `events` | 구분 기호 `,`을(를) 사용하여 전환 COMMERCE_PROD_VIEW를 사용한 AppMeasurement 쿼리 매개 변수 EVENT_LIST_FULL 매핑을 참조하십시오. |
| `commerce.purchases.value` | `events` | 구분 기호 `,`을(를) 사용하여 전환 COMMERCE_PURCHASE를 사용하는 AppMeasurement 쿼리 매개 변수 EVENT_LIST_FULL 매핑을 참조하십시오. |
...
| `environment.browserDetails.cookiesEnabled` | `k` | AppMeasurement 쿼리 매개 변수 전환 BOOLEAN_TO_YN과 함께 쿠키 매핑을 참조하십시오. |
...
| `media.mediaTimed.primaryAssetReference.showType` | `c.a.media.type` | AppMeasurement 컨텍스트 데이터 `c.a.media.type` 매핑과 전환 AUDIO_SHOW_TYPE을 사용합니다. |
...
| `web.webPageDetails.errorPage` | `pageType` | 전환 ERROR_PAGE_TYPE과 함께 AppMeasurement 쿼리 매개 변수 PAGE_TYPE_FULL 매핑. |
| `web.webPageDetails.homePage` | `hp` | AppMeasurement 쿼리 매개 변수 HOMEPAGE 매핑과 전환 BOOLEAN_TO_YN을 함께 합니다. |
