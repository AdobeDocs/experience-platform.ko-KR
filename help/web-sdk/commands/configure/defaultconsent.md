...
다음 `defaultConsent` 속성은 다음 세 가지 값을 지원합니다.

* **`in`**: 사용자가 옵트아웃할 때까지 데이터 수집은 정상적으로 진행됩니다.
* **`out`**: 사용자가 옵트인할 때까지 데이터가 영구적으로 삭제됩니다.
* **`pending`**: 사용자가 를 사용하여 옵트인할 때까지 데이터가 로컬에 저장됩니다. [`setConsent`](../setconsent.md) 명령입니다. 일반적인 목적에 대한 기본 동의가 로 설정된 경우 `pending`, 사용자 옵트인 환경 설정에 따라 달라지는 모든 명령 실행 시도(예: [`sendEvent`](../sendevent/overview.md) command)를 사용하면 웹 SDK에서 대기 중인 명령이 생성됩니다. 대기열에 추가된 명령은 사용자의 옵트인 환경 설정을 웹 SDK에 전달할 때까지 처리되지 않습니다.
...
사용자의 옵트인 환경 설정이 지정되기 전에 발생한 이벤트를 수집하지 않으려면 을 전달할 수 있습니다. `"defaultConsent": "out"` 웹 SDK 구성 중에. 사용자 옵트인 환경 설정에 의존하는 명령을 실행하려고 시도해도 사용자의 옵트인 환경 설정을 웹 SDK에 전달할 때까지 효과가 없습니다.

>[!NOTE]
>
>현재 Web SDK는 단일 전체 또는 무목적 기능만 지원합니다. 다양한 Adobe 기능 및 제품 오퍼링에 해당하는 보다 강력한 목적 또는 카테고리 세트를 구축할 계획이지만 현재 구현은 옵트인에 대한 양자택일의 접근 방식입니다.  이는 에만 적용됩니다. [!DNL Web SDK] 다른 Adobe JavaScript 라이브러리는 아닙니다.
