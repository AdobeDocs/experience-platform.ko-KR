...
새 사용자가 웹 사이트에 도달하면 Adobe Experience Cloud Identity Service에서 해당 사용자에 대한 장치 ID 쿠키 설정을 시도합니다. 처음 방문하는 사용자의 경우 ECID가 생성되고 Adobe Experience Platform Edge Network의 첫 번째 응답에서 반환됩니다. 반복 방문자의 경우 ECID가에서 검색됩니다. `kndctr_{YOUR-ORG-ID}_AdobeOrg_identity` 쿠키에 추가되고 Edge Network에 의해 페이로드에 추가됩니다.
...
사용 사례에 따라 다음에 액세스할 수 있는 두 가지 방법이 있습니다 [!DNL ECID]:

* [검색 [!DNL ECID] 데이터 수집을 위한 데이터 준비를 통해](#retrieve-ecid-data-prep): 이 방법이 권장되는 방법입니다.
* [검색 [!DNL ECID] 다음을 통해 `getIdentity()` 명령](#retrieve-ecid-getidentity): 다음이 필요한 경우에만 이 메서드를 사용하십시오. [!DNL ECID] 클라이언트측 정보입니다.

### 검색 [!DNL ECID] 데이터 수집을 위한 데이터 준비를 통해 {#retrieve-ecid-data-prep}

사용 [데이터 수집을 위한 데이터 준비](../../datastreams/data-prep.md) 를 매핑하려면 다음을 수행하십시오. [!DNL ECID] (으)로 [!DNL XDM] 필드. 다음에 액세스하는 데 권장되는 방법입니다. [!DNL ECID].
...
그런 다음 타겟 필드를 필드가 유형인 XDM 경로로 설정합니다 `string`.

![](../../tags/extensions/client/web-sdk/assets/access-ecid-data-prep.png)


### 검색 [!DNL ECID] 다음을 통해 `getIdentity()` 명령 {#retrieve-ecid-getidentity}


>[!IMPORTANT]
>
>를 통해서만 ECID를 검색해야 합니다. `getIdentity()` 필요한 경우 명령 [!DNL ECID] 클라이언트 측. ECID를 XDM 필드에만 매핑하려면 [데이터 수집을 위한 데이터 준비](#retrieve-ecid-data-prep) 대신,

현재 방문자에 대한 고유한 ECID를 검색하려면 `getIdentity` 명령입니다. 권한이 없는 처음 방문자의 경우 [!DNL ECID] 하지만 이 명령은 새 [!DNL ECID]. `getIdentity` 은 방문자에 대한 지역 ID도 반환합니다.
