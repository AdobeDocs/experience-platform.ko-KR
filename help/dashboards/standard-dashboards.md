...
사용자 지정 대시보드를 만들려면 먼저 대시보드 인벤토리로 이동합니다. Experience Platform UI의 왼쪽 탐색에서 **[!UICONTROL Dashboards]**&#x200B;을(를) 선택한 후 **[!UICONTROL Create dashboard]**&#x200B;을(를) 선택합니다.
...
[!UICONTROL Create dashboard] 대화 상자가 나타납니다. 만들려는 위젯 컬렉션에 대해 사용자에게 친숙한 설명 이름을 입력하고 **[!UICONTROL Save]**&#x200B;을(를) 선택합니다.
...
>title="최대 위젯 수"
>abstract="대시보드 서비스는 최대 10개의 위젯을 지원합니다. 10개의 위젯을 대시보드에 추가한 후에는 [!UICONTROL Add new widget] 옵션이 비활성화되고 회색으로 표시됩니다."

새 대시보드 보기에서 **[!UICONTROL Add new widget]**&#x200B;을(를) 선택하여 위젯 만들기 프로세스를 시작합니다.

>[!IMPORTANT]
>
>각 대시보드는 최대 10개의 위젯을 지원합니다. 10개의 위젯을 대시보드에 추가한 후에는 [!UICONTROL Add new widget] 옵션이 비활성화되고 회색으로 표시됩니다.

![새 위젯 추가 가 강조 표시된 빈 대시보드입니다.](./images/standard-dashboards/add-new-widget.png)

### 위젯 작성기

위젯 작성기 작업 영역이 나타납니다. **[!UICONTROL Select data]**&#x200B;을(를) 선택하여 위젯에 특성을 추가할 데이터 모델을 선택합니다.

![위젯 작성기 작업 영역입니다.](./images/standard-dashboards/widget-composer.png)

#### 데이터 모델 선택 {#select-data-model}

[!UICONTROL Select data model] 대화 상자가 나타납니다. 사용 가능한 모든 테이블의 미리보기 목록을 표시하려면 왼쪽 열에서 데이터 모델을 선택합니다. Real-Time Customer Data Platform에 대해 미리 구성된 데이터 모델의 이름은 [!UICONTROL CDPInsights]입니다.
...
| [!UICONTROL Table type] | 테이블 유형. 가능한 형식은 `fact`, `dimension` 및 `none`입니다. |
| [!UICONTROL Records] | 선택한 테이블과 연결된 레코드 수입니다. |
| [!UICONTROL Lookups] | 선택한 테이블에 연결된 테이블 수입니다. |
| [!UICONTROL Attributes] | 선택한 테이블에 대한 속성의 수입니다. |

**[!UICONTROL Next]**&#x200B;을(를) 선택하여 데이터 모델 선택을 확인합니다. 다음 보기는 왼쪽 레일에 사용 가능한 테이블 목록을 표시합니다. 선택한 테이블에 포함된 데이터에 대한 포괄적인 분류를 보려면 테이블을 선택합니다.

### 위젯 채우기 {#populate-widget}

[!UICONTROL Preview] 패널에는 [!UICONTROL Sample records] 및 [!UICONTROL Attributes]에 대한 탭이 있습니다. [!UICONTROL Sample records] 탭은 테이블 보기에서 선택한 테이블의 레코드 하위 집합을 제공합니다. [!UICONTROL Attributes] 탭은 선택한 테이블과 연결된 모든 특성에 대한 특성 이름, 데이터 형식 및 원본 테이블을 제공합니다.

위젯에 데이터를 제공할 왼쪽 레일의 사용 가능한 목록에서 테이블을 선택하고 **[!UICONTROL Select]**&#x200B;을(를) 선택하여 위젯 작성기로 돌아갑니다.

![선택 항목이 강조 표시된 데이터 선택 대화 상자입니다.](./images/standard-dashboards/select-a-table.png)

이제 위젯 작성기가 선택한 테이블의 데이터로 채워집니다.

데이터 모델 및 현재 선택한 테이블이 왼쪽 레일의 맨 위에 표시되고 위젯을 만드는 데 사용할 수 있는 특성이 [!UICONTROL Attributes] 열에 나열됩니다. 목록을 스크롤하는 대신 검색 창을 사용하여 특성을 찾거나 연필 아이콘(![연필 아이콘)을 선택하여 선택한 데이터 모델을 변경할 수 있습니다.](/help/images/icons/edit.png))을(를) 왼쪽 레일에 둡니다.
...
위젯의 그래프 또는 차트 유형을 변경하려면 [!UICONTROL Marks] 드롭다운을 선택하고 사용 가능한 옵션 중에서 선택합니다. 옵션에는 막대, 점, 틱, 선 또는 영역이 포함됩니다. 선택하면 위젯의 현재 설정에 대한 미리보기 시각화가 생성됩니다.

![표시 드롭다운이 강조 표시된 위젯 작성기입니다.](./images/standard-dashboards/marks-dropdown.png)

속성을 필터로 추가하여 위젯에서 포함하거나 제외할 값을 선택할 수 있습니다. 특성 목록에서 필터를 추가하면 해당 확인란을 사용하여 값을 선택하거나 선택 취소할 수 있는 [!UICONTROL Filter] 대화 상자가 나타납니다.

![위젯에서 값을 필터링하는 필터 대화 상자입니다.](./images/standard-dashboards/filter-dialog.png)

#### 내역 데이터 필터링 {#filter-historical-data}

위젯에서 생성된 인사이트에서 내역 데이터를 필터링하려면 `date_key` 특성을 필터로 추가하고 **[!UICONTROL Recent date]**, **[!UICONTROL Apply]**&#x200B;을(를) 선택합니다. 이 필터는 인사이트를 도출하는 데 사용되는 데이터를 최신 시스템 스냅샷에서 가져옵니다.

![강조 표시된 [!UICONTROL Filter: date_key] 및 [!UICONTROL Recent date]의 [!UICONTROL Apply] 대화 상자.](./images/standard-dashboards/recent-date.png)

또는 사용자 지정 기간을 만들어 데이터를 필터링 기준으로 사용할 수 있습니다. 사용 가능한 날짜 목록으로 대화 상자를 확장하려면 **[!UICONTROL Select dates]**&#x200B;을(를) 선택하십시오. **[!UICONTROL Select all]** 확인란을 사용하여 사용 가능한 모든 옵션을 활성화하거나 비활성화하거나 각 날짜의 확인란을 개별적으로 선택하십시오. 마지막으로 **[!UICONTROL Apply]**&#x200B;을(를) 선택하여 선택 항목을 확인합니다.

>[!NOTE]
>
>`date_key` 특성이 이미 필터로 추가된 경우 드롭다운 옵션에서 줄임표 다음에 **[!UICONTROL Edit]**&#x200B;을(를) 선택하여 필터 기간을 변경합니다.

![일별 확인란이 있는 [!UICONTROL Filter: date_key] 대화 상자를 선택하고 선택하지 않습니다.](./images/standard-dashboards/select-dates.png)

### 위젯 속성

속성 아이콘(![속성 아이콘)을 선택합니다.](/help/images/icons/properties.png))을(를) 오른쪽 레일에서 클릭하여 속성 패널을 엽니다. [!UICONTROL Properties] 패널에서 [!UICONTROL Widget title] 텍스트 필드에 위젯의 이름을 입력합니다.

![속성 아이콘과 위젯 제목 필드가 강조 표시된 속성 패널입니다.](./images/standard-dashboards/properties-panel.png)

위젯 속성 패널에서 위젯의 여러 측면을 편집할 수 있습니다. 위젯 범례의 위치를 편집할 수 있는 완전한 제어 권한이 있습니다. 범례를 이동하려면 [!UICONTROL Legend placement] 드롭다운을 선택하고 사용 가능한 옵션 목록에서 원하는 위치를 선택합니다. [!UICONTROL Legend title] 텍스트 필드 또는 [!UICONTROL Axis label] 텍스트 필드에 각각 새 이름을 입력하여 범례와 연결된 레이블, X 또는 Y 축의 이름을 변경할 수도 있습니다.

#### 위젯 저장 {#save-widget}

위젯 작성기에 저장하면 위젯이 대시보드에 로컬로 저장됩니다. 작업을 저장하고 나중에 다시 시작하려면 **[!UICONTROL Save]**&#x200B;을(를) 선택하십시오. 위젯 이름 아래에 있는 확인 표시 아이콘은 위젯이 저장되었음을 나타냅니다. 또는 위젯에 만족하면 **[!UICONTROL Save and close]**&#x200B;을(를) 선택하여 대시보드에 액세스할 수 있는 다른 모든 사용자가 위젯을 사용할 수 있도록 합니다. 작업을 포기하고 사용자 지정 대시보드로 돌아가려면 **[!UICONTROL Cancel]**&#x200B;을(를) 선택하십시오.
...
이 작업 영역에 있는 동안 위젯을 다시 정렬하고 크기를 조정할 수 있습니다. 대시보드 이름과 구성된 레이아웃을 유지하려면 **[!UICONTROL Save]**&#x200B;을(를) 선택하십시오.
...
>[!NOTE]
>
>복제를 통해 처음부터 시작하지 않고도 insight의 속성을 사용자 정의하여 고유한 위젯을 만들 수 있습니다. 위젯을 복제하면 사용자 정의 대시보드에 표시됩니다. 그런 다음 새 위젯의 생략 부호를 선택한 후 **[!UICONTROL Edit]**&#x200B;을(를) 선택하여 insight을 사용자 지정할 수 있습니다.
...
>[!VIDEO](https://video.tv.adobe.com/v/3413784?captions=kor&quality=12&learn=on)
