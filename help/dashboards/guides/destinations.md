...
* **세그먼트 정의:** 세그먼트 정의는 **규칙 세트** 타겟 대상의 주요 특성 또는 동작을 설명하는 데 사용됩니다. 이러한 규칙에는 프로필을 대상의 일부로 규정하는 속성 및 이벤트 데이터가 포함됩니다.
...
* **활성화**: 활성화 는 사용자가 대상자 또는 프로필을 Oracle Eloqua, Google 또는 Salesforce Marketing Cloud과 같은 대상에 매핑하기 위해 수행하는 작업입니다.
...
* [[!UICONTROL 일반 대상자]](#common-audiences)
...
에 표시되는 매핑된 대상자 수 [!UICONTROL 대상자 수] 열은 마지막 일별 스냅샷을 기준으로 정확합니다. 새 대상을 대상에 매핑하면 다음 스냅샷을 가져올 때까지 카운트가 업데이트되지 않습니다.

특정 대상에 대한 대상 세부 정보로 이동하려면 위젯에 표시된 목록에서 대상 이름을 선택합니다. 다음을 선택할 수도 있습니다. **[!UICONTROL 모두 보기]** 을 클릭하여 다음 위치로 이동합니다. **[!UICONTROL 찾아보기]** 을 탭한 다음 대상의 이름을 선택하여 세부 정보를 확인합니다.
...
위젯에 표시된 목록에서 대상 이름을 선택하면 대상 세부 사항으로 이동합니다. 다음을 선택할 수도 있습니다. **[!UICONTROL 모두 보기]** 을 클릭하여 다음 위치로 이동합니다. [!UICONTROL 대상] [!UICONTROL 찾아보기] 을 탭한 다음 대상자의 이름을 선택하여 세부 정보를 확인합니다.

Experience Platform에서 대상자 작업에 대한 자세한 내용은 다음을 참조하십시오. [세그먼테이션 서비스 개요](../../segmentation/home.md).
...
다음 **[!UICONTROL 대상자 크기 트렌드]** 위젯에서 다음을 제공합니다. [!UICONTROL 캡션] 위젯의 오른쪽 상단에 있는 단추입니다. 선택 **[!UICONTROL 캡션]** 자동 캡션 대화 상자를 엽니다. 머신 러닝 모델은 차트 및 대상 데이터를 분석하여 주요 트렌드 및 중요한 이벤트를 설명하는 캡션을 자동으로 생성합니다.
...
![ID별 매핑되지 않은 대상 위젯.](../images/destinations/unmapped-audiences-by-identity.png)
...
다음 **[!UICONTROL ID별로 매핑된 대상자]** 위젯은 선택한 대상 내에서 캠페인에 대한 프로필 기회를 성공적으로 타겟팅할 가능성을 한눈에 강조 표시합니다. 효율적인 타겟팅된 캠페인은 대상으로 전송된 프로필 수가 아니라 대상 ID와 일치하여 유용하고 실행 가능한 데이터를 제공할 수 있는 소스 ID 수에 따라 달라집니다.
...
선택 **[!UICONTROL 대상]** 대상자 대시보드로 이동하려면 [!UICONTROL 찾아보기] 탭. 이 작업 공간에는 조직의 모든 세그먼트 정의 목록이 표시됩니다.
...
대상 내의 [!UICONTROL 매핑된 대상자 상태] 위젯이 큰 폭으로 벗어나고 있습니다. 대상자 크기 트렌드 차트를 참조하여 이례적인 대상자를 찾아야 합니다. 이러한 경향은 대상자의 건강에 대한 추가적인 통찰력을 제공할 수 있습니다.

>[!NOTE]
>
>매핑된 대상자 상태 위젯의 기본 크기로 인해 테이블 정보가 방해를 받을 수 있습니다. 매핑된 대상자 이름 및 열 제목의 가독성을 향상시키려면 위젯 크기를 수정하십시오. 에 대한 지침은 대시보드 수정 설명서 를 참조하십시오. [위젯 크기를 조정하는 방법](../customize/modify.md).
...
다음 [!UICONTROL 모든 대상에 대해 활성화된 대상자] 위젯은 단일 지표의 모든 대상에 대해 활성화된 총 대상자 수를 제공합니다. 이 숫자는 가장 최근 스냅샷에 정확합니다.
...
선택 **[!UICONTROL 대상]** 대상 대시보드의 세부 정보 페이지로 이동합니다. 다음 [!UICONTROL 활성화 데이터] 탭에는 시작 날짜와 종료 날짜(해당하는 경우)를 포함하여 대상에 매핑된 대상자의 목록과 데이터 내보내기를 위한 기타 관련 정보(내보내기 유형, 일정 및 빈도)가 표시됩니다. 특정 대상에 대한 세부 정보를 보려면 다음에서 해당 이름을 선택합니다 [!UICONTROL 대상자 이름] 열.
