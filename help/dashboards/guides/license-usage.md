...
**[!UICONTROL License usage]** 대시보드를 통해 조직의 라이선스 사용에 대한 중요한 정보를 볼 수 있습니다. 대시보드는 Adobe Experience Platform에 라이선스를 부여하는 조직과 라이선스를 부여하지 않는 조직을 포함하여 자격이 있는 Experience Cloud 조직에서 사용할 수 있습니다. 표시되는 정보는 조직 환경의 일일 스냅샷 중에 캡처되며 실시간으로 업데이트되지 않습니다.

라이선스 사용 보고서는 높은 수준의 세부기간을 제공합니다. 대부분의 지표는 여러 제품에서 공유되며 제품당 합계가 아닌 해당 지표를 사용하는 모든 제품에서 집계된 사용을 반영합니다.
...
## [!UICONTROL License usage] 대시보드 데이터

[!UICONTROL License usage] 대시보드에는 구입한 모든 Experience Platform 제품 목록과 해당 제품에 대한 추가 기능이 표시됩니다. 이 대시보드에서 연결된 모든 샌드박스에서 Experience Platform에 대한 조직의 라이선스 관련 데이터의 스냅샷을 찾을 수 있습니다.

이 대시보드의 데이터는 스냅샷이 생성된 특정 시점에 표시된 것과 동일하게 표시됩니다. 근사치나 샘플이 아니라 대시보드가 실시간으로 업데이트되지 않는다.

Adobe Experience Platform 애플리케이션이 없는 조직(예: Real-time Customer Data Platform, Adobe Journey Optimizer, Customer Journey Analytics 등)의 경우 대시보드에는 AI 신용 사용 지표만 표시됩니다.

>[!NOTE]
>
>대시보드의 지표 대부분은 Experience Platform 인스턴스의 스냅샷을 기반으로 매일 업데이트됩니다. [!UICONTROL CJA Rows Available]은(는) 예외이며 매월 업데이트됩니다. [!UICONTROL Adhoc Query Service Users Packs], [!UICONTROL Profile Richness No of Packs] 및 [!UICONTROL Streaming Segmentation No of Packs]과(와) 같이 &quot;팩&quot;으로 레이블이 지정된 지표는 추가 기능 오퍼링에 대한 라이선스 권한을 반영하며 진행 중인 사용을 추적하지 않습니다. 스냅샷 이후에 변경한 내용은 다음 스냅샷을 생성할 때까지 표시되지 않습니다.

## 라이선스 사용 대시보드 살펴보기 {#explore}

Experience Platform UI 내의 라이선스 사용 대시보드로 이동하려면 왼쪽 레일에서 **[!UICONTROL License usage]**&#x200B;을(를) 선택합니다. 대시보드에는 **[!UICONTROL Metrics]** 및 **[!UICONTROL Products]** 탭이 있습니다.

>[!NOTE]
>
>라이선스 사용 대시보드는 기본적으로 활성화되어 있지 않습니다. 액세스하려면 **&quot;[!UICONTROL View License Usage Dashboard]&quot;** 권한이 있어야 합니다.
>
>조직에서 Adobe Experience Platform 애플리케이션에 라이선스를 부여하는 경우 해당 제품 프로필 및 샌드박스에서 이 권한을 부여합니다.
>
>Adobe Experience Platform 애플리케이션이 없는 조직(예: AEM 전용 또는 Workflow 전용 조직)의 경우, 이 권한은 Adobe Experience Platform 제품 카드(조직에 대해 규정된 경우) 아래의 Adobe Admin Console에서 사용할 수 있습니다. 사용자가 대시보드를 볼 수 있으려면 제품 프로필에 권한을 추가해야 합니다.

## [!UICONTROL Metrics] 탭 {#metrics-tab}

**[!UICONTROL Metrics]** 탭에서는 조직의 모든 라이선스 사용 지표를 중앙에서 볼 수 있습니다. 대부분의 지표는 제품에서 공유되므로 이러한 지표에 대한 제품별 분류는 별도로 없습니다.

지표 테이블에는 다음 열이 포함되어 있습니다.

| 열 이름 | 설명 |
|---|---|
| **[!UICONTROL Metric Name]** | 라이선스 사용 지표의 이름입니다. 각 항목에는 설명 및 관련 제품 목록을 표시하는 정보 아이콘(`ⓘ`)이 포함되어 있습니다. |
| **[!UICONTROL Licensed]** | 계약에 정의된 대로 조직에서 사용할 수 있는 단위 수. 이 지표는 제품 탭의 **라이선스 양**&#x200B;과 같은 값입니다. |
| **[!UICONTROL Measured]** | 조직에서 현재 사용하고 있는 지표의 양입니다. |
| **[!UICONTROL Usage %]** | 현재 사용 중인 라이선스 값의 백분율입니다. |
| **[!UICONTROL Predicted Usage %]** | 향후 6주 동안 예상되는 지표 사용량 범위. |

**[!UICONTROL Production]** 또는 **[!UICONTROL Development]** 샌드박스 토글을 사용하여 샌드박스로 표시되는 지표를 필터링합니다.

>[!NOTE]
>
>소비 보고는 샌드박스 유형별로 누적됩니다. [!UICONTROL Production] 또는 [!UICONTROL Development]을(를) 선택하면 해당 형식의 모든 샌드박스에서 결합된 사용량이 표시됩니다.
...
**[!UICONTROL Custom date]**&#x200B;을(를) 선택하여 표시되는 기간을 선택할 수도 있습니다.

![사용자 지정 날짜 범위 옵션이 강조 표시된 라이선스 사용 대시보드 개요 탭](../images/license-usage/custom-date-range.png)

### CSV 내보내기 {#export-metric-usage-data}

지표 세부 사항 보기에서 선택한 지표 및 샌드박스에 대한 사용 내역 데이터를 CSV 파일로 직접 내보낼 수 있습니다. 차트 데이터를 표 형식으로 다운로드하려면 **[!UICONTROL Export]** 아이콘을 선택하십시오. 내보낸 CSV를 통해 오프라인에서 트렌드를 쉽게 분석하거나 팀 간에 사용 인사이트를 공유할 수 있습니다.

## [!UICONTROL Products] 탭 {#products-tab}

**[!UICONTROL Products]** 탭에는 구매한 제품 및 관련 추가 기능별로 그룹화된 라이선스 사용 데이터가 표시됩니다. [!UICONTROL Products] 탭에는 두 개의 테이블이 있습니다.

- **[!UICONTROL Core products]테이블**: 이 테이블에는 조직에서 라이선스를 부여한 기본 Adobe Experience Platform 제품이 나열됩니다. 각 제품은 기본 지표, 사용 추적 및 예측된 사용을 나열합니다.
- **[!UICONTROL Add-ons]테이블**: 라이선스 금액이 핵심 제품 지표에 기여하는 보조 항목을 나열합니다. 추가 기능에는 별도의 지표가 없지만 연결된 핵심 제품의 사용 추적을 개선합니다.

| 열 이름 | 설명 |
|---|---|
| **[!UICONTROL Product]** | 조직에서 라이선스를 부여한 Adobe 솔루션. |
| **[!UICONTROL Primary Metric]** | 해당 제품 내에서 추적하는 데 사용되는 기본 지표입니다. |
| **[!UICONTROL License Amount]** | 기본 지표의 최대 양에 대한 계약된 값입니다. |
| **[!UICONTROL Usage]** | 사용된 기본 지표의 양입니다. |
| **[!UICONTROL Usage %]** | 라이선스 금액에 따라 사용되는 기본 지표의 비율입니다. |
| **[!UICONTROL Predicted Usage]** | 기본 지표의 예상 사용 백분율입니다. |

>[!NOTE]
>
>추가 기능의 [!UICONTROL License Amount]은(는) 핵심 제품의 총 라이선스 금액에 포함됩니다. 추가 기능은 별도로 추적되지 않지만 관련 제품의 기능을 향상시킵니다. 예를 들어, 추가 기능으로 샌드박스 5개 팩 1개를 구매하면 기본 제품 금액에 금액이 추가됩니다. 추가 기능 테이블에는 추가 기능과 관련된 [!UICONTROL License Amount]이(가) 표시되지만 실제 사용량은 기본 제품을 통해 추적됩니다.
...
정확한 최신 사용 예측을 통해 라이센스 리소스를 사전 예방적으로 관리하고 최적화할 수 있습니다. [!UICONTROL Predicted Usage] 열은 구입한 모든 제품에 대한 모든 프로덕션 및 개발 샌드박스에서 샌드박스 수준에서 향후 라이선스 사용을 예측합니다. 이제 예측은 매주 업데이트되며, 최신 사용 데이터를 기반으로 6주 간의 예측을 제공합니다. 각 예측은 정보에 입각한 계획을 지원하기 위한 하한 및 상한을 모두 포함한다.

>[!IMPORTANT]
>
>예측은 매주 금요일에 갱신된다. 새로 고침 날짜가 정보 아이콘(![이 정보 아이콘)에 포함되어 있습니다.열 제목 위에 &#x200B;](../images/license-usage/info-icon.png)) 있습니다.

[!UICONTROL Product] 테이블 아래의 [!UICONTROL Core products] 탭에서 제품의 권한 사용 요약을 봅니다.

![제품 및 예상 사용 열이 강조 표시된 [!UICONTROL License usage] [!UICONTROL Product] 탭입니다.](../images/license-usage/product-predicted-usage.png)
...
>[!IMPORTANT]
>
>8월 20일부터 &#39;[!UICONTROL Average Profile Richness]&#39; 및 &#39;[!UICONTROL Total Storage]&#39;에 대한 권한이 있는 고객이 라이선스 사용 대시보드에서 &#39;[!UICONTROL Total Data Volume]&#39;을(를) 대신 보았습니다. 고객 권한은 변경되지 않았으며 추적 지표의 단순화만 있었습니다. [!UICONTROL Total Data Volume]은(는) 참여 및 개인화 워크플로에 대한 실시간 고객 프로필에서 사용할 수 있는 데이터를 나타냅니다. 이 간소화된 지표는 실시간 고객 프로필 사용에 대한 관리 및 측정을 개선했습니다. 고객은 이 변경에 대한 자세한 내용을 확인하기 위해 Adobe 담당자에게 문의하는 것이 좋습니다.

대시보드에 표시되는 지표는 조직과 연결된 제품 및 권한에 따라 다릅니다. 조직이 [Adobe Experience Platform 에이전트 사용 제한 평가판](https://experienceleague.adobe.com/ko/docs/experience-cloud-ai/experience-cloud-ai/agents/trial)에 참여하거나 Adobe Experience Platform 에이전트에 라이선스를 부여하는 경우 대시보드에 [!UICONTROL AI credits] 지표가 포함됩니다. 조직에서 Adobe Experience Platform에 라이선스를 부여하지 않는 경우 AI 신용 사용이 기본 지표로 표시됩니다.

| 지표 | 설명 |
|---|---|
| [!UICONTROL AI credits] | Adobe Experience Platform 에이전트를 사용할 때 조직에서 사용한 AI 크레딧 수입니다. AI 크레딧은 Adobe Experience Platform Agent 사용 제한 평가판 진행 중 및 유료 에이전트 사용에 대한 라이선스가 부여된 경우 사용됩니다. 이 지표를 사용하면 사용 가능한 자격에 대해 AI 크레딧 소비를 모니터링할 수 있습니다. |
| [!UICONTROL Audience Activation Size] | 1년 동안 파일 기반 대상에 대해 활성화된 프로필의 총 크기입니다. 참고: 여기에는 스트리밍 대상을 통해 전송된 프로필이 포함되지 않습니다. |
| [!UICONTROL Addressable Audience] | 직접 식별 가능한 프로필과 익명 프로필을 포함하여 조직에서 참여할 수 있는 실시간 고객 프로필의 개인 프로필 집합입니다. 이러한 프로필에는 속성, 동작 및 세그먼트 멤버십 데이터가 포함될 수 있습니다. 프로필 볼륨은 Adobe Experience Platform의 기본 결정론적 ID 그래프를 사용하여 계산되며 공유 기능으로 간주됩니다. |
| [!UICONTROL Adhoc Query Service Users Packs] | 허가된 동시 쿼리 서비스 사용자 권한을 5명의 추가 동시 쿼리 서비스 사용자와 팩당 하나의 추가 동시 실행 임시 쿼리로 늘리는 추가 기능입니다. 여러 개의 추가 임시 쿼리 사용자 팩에 라이선스가 부여될 수 있습니다. |
| [!UICONTROL Average profile richness] | **사용되지 않음** - 특정 시점에 Hub Profile Service에 저장된 모든 프로덕션 데이터의 합계를 인증된 비즈니스 사용자 프로필의 5배로 나눈 값입니다. [!UICONTROL Average profile richness]은(는) 공유 기능입니다. |
| [!UICONTROL CJA Rows Available] | Customer Journey Analytics에서 분석에 사용할 수 있는 일일 평균 데이터 행의 수입니다. |
| [!UICONTROL Computed Attributes] | 프로필 속성으로 변환되고 개인 프로필에 포함될 수 있는 경험 이벤트를 기반으로 집계된 프로필 행동 데이터입니다. |
| [!UICONTROL Consumer Audience] | 판매 주문에서 &quot;소비자 대상&quot;으로 식별된 개인 프로필 수입니다. |
| [!UICONTROL Data Export Size] | 1년 동안 데이터 세트 활성화를 통해 전송된 데이터의 양입니다. |
| [!UICONTROL Data Exports] | 1년 동안 비 Adobe 솔루션으로 (직접 또는 간접적으로) 내보낼 수 있는 데이터 세트의 총 크기입니다. |
| [!UICONTROL Data Lake Storage] | Adobe Experience Platform 내 분석 데이터 저장소에 사용된 수량입니다. |
| [!UICONTROL Engageable Audience] | Journey Optimizer의 작성, 의사 결정, 게재, 실험 또는 오케스트레이션 기능을 사용하여 지난 12개월 내에 참여하려고 한 실시간 고객 프로필의 사용자 프로필 그룹입니다. |
| [!UICONTROL Look-alike Audiences] | 유사 소비자 대상은 기존 소비자 대상을 모델링하여 유사한 속성이나 행동이 있는 사용자 프로필을 식별하여 생성된 대상입니다. |
| [!UICONTROL Number of AMM Models] | 귀하의 투자에 따라 지정된 결과를 측정 및/또는 예측하는 데 사용되는 머신 러닝 모델(Adobe Mix Modeler에서 기본 제공)의 수입니다. |
| [!UICONTROL Number of Sandboxes] | Adobe Experience Platform에 액세스하여 데이터와 작업을 분리하는 모든 Adobe 주문형 서비스 인스턴스 내 논리적 분리의 수입니다. |
| [!UICONTROL Profile Richness No of Packs] | 추가 프로필 풍부도 팩당 프로필당 승인된 총 데이터 용량이 25KB씩 증가합니다. |
| [!UICONTROL Query Service Compute Hours] | 배치 쿼리 실행 시 쿼리 서비스 엔진이 데이터를 읽고, 처리하고, 데이터 레이크로 다시 쓰는 데 걸리는 시간을 측정한 값입니다. |
| [!UICONTROL Streaming Segmentation No of Packs] | 팩은 스트리밍 흐름을 통해 새 데이터가 세분화 서비스에 입력됨에 따라 사용자 프로필에 대한 세그먼트 멤버십을 업데이트해 줍니다. 세그먼트 멤버십은 이전 행동을 고려하지 않으면서 현재 사용자 프로필 속성과 현재의 이벤트의 값을 기준으로 평가됩니다. 스트리밍 세분화는 공유 기능입니다. |
| [!UICONTROL Total Data Volume] | 실시간 고객 프로필이 참여 워크플로우에서 사용할 수 있는 총 데이터 양입니다. 총 데이터 볼륨은 **총 데이터 볼륨 = 대응 가능 대상 × 평균 프로필 풍부도** 공식을 사용하여 계산됩니다. 이 지표는 프로필 저장소에만 저장된 데이터를 반영하며 데이터 레이크 저장소는 제외합니다. 프로필 기반 참여와 관련된 데이터에 대해 보다 집중적인 보기를 제공합니다. 자세한 내용은 총 데이터 볼륨에 대한 [자주 묻는 질문](../../landing/license-usage-and-guardrails/total-data-volume.md)을 참조하세요. |
| [!UICONTROL Total Volume of Data Egress] | Adobe Experience Platform에서 서드파티 데이터 웨어하우스로 내보낸 연간 누적 데이터 양입니다. |
...
| <ul><li>RT 고객 데이터 플랫폼:OD</li><li>RT 고객 데이터 플랫폼 :OD PRFL ~ 10M</li><li>RT 고객 데이터 플랫폼 :OD PRFL ~ 50M</li></ul> | [Adobe Real-Time Customer Data Platform](https://helpx.adobe.com/kr/legal/product-descriptions/real-time-customer-data-platform.html) |
| <ul><li>AEP:OD 활성화</li><li>AEP:OD 활성화 PRFL ~ 10M</li><li>최대 50M의 AEP:OD 활성화 PRFL</li></ul> | [Adobe Experience Platform 활성화](https://helpx.adobe.com/kr/legal/product-descriptions/adobe-experience-platform0.html) |
...
| <ul><li>JOURNEY OPTIMIZER SELECT:OD</li><li>JOURNEY OPTIMIZER PRIME:OD</li><li>JOURNEY OPTIMIZER ULTIMATE:OD</li><li>AJO PRIME STARTER:OD 실행</li><li>AJO ULTIMATE STARTER:OD 실행</li><li>Real-Time CDP:OD 프로필 오케스트레이션 실행</li></ul> | [Adobe Journey Optimizer](https://helpx.adobe.com/kr/legal/product-descriptions/adobe-journey-optimizer.html) |
