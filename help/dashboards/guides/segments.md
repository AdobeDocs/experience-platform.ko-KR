...
>title="대상자 크기"
>abstract="이 위젯은 선택한 세그먼트 내에서 병합된 프로필의 총 수를 표시합니다. 이 숫자는 데이터에 적용된 병합 정책에 따라 다르며 최신 스냅샷 시점에 정확합니다."
...
>title="대상자 크기 트렌드"
>abstract="이 위젯은 의 기준을 충족하는 총 프로필 수에 대한 정보를 제공합니다. **임의** 지난 30일, 90일 또는 12개월 동안 일별 스냅샷 중에 캡처된 세그먼트 정의."
...
>title="ID 중복"
>abstract="이 위젯은 선택한 두 ID를 모두 포함하는 세그먼트의 프로필 중첩을 표시합니다. 원은 각 ID의 상대적 크기를 표시합니다. 두 네임스페이스가 모두 포함된 프로필 수는 원 간의 겹침으로 표시됩니다."
...
>title="ID별 프로필"
>abstract="이 위젯은 선택한 세그먼트에 있는 모든 병합된 프로필에 대한 ID 분류를 표시합니다."
