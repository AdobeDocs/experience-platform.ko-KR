...
이 문서를 사용하려면 태그 내의 확장과 구성 방법을 비롯한 확장 작업에 대한 이해가 필요합니다. 자세한 내용은 [확장 개발 개요](./overview.md)를 참조하십시오.
...
다음 JavaScript은 [Pinterest 전환 태그](https://developers.pinterest.com/docs/ad-tools/conversion-tag/?)에 대해 축소되지 않은 기본 코드의 예제입니다. 이 태그는 태그를 사용하는 다른 구현 전략에 맞게 기본 코드가 어떻게 조정되는지 보여주기 위해 이 문서의 후반부에 참조됩니다.
...
아래 섹션에서는 이전에 예제로 표시된 Pinterest 기본 코드를 사용하여 확장에서 공급업체 라이브러리를 로드할 수 있는 다양한 방법을 보여줍니다. 이러한 각 예제는 웹 사이트에 라이브러리를 로드하는 웹 확장에 대한 [작업 유형](./web/action-types.md)을 만드는 것과 관련되어 있습니다.
...
또한 태그는 모든 확장에서 사용할 수 있는 유틸리티인 여러 [코어 모듈](./web/core.md)을 제공합니다. 특히 `@adobe/reactor-load-script` 모듈은 스크립트 요소를 만들어 문서에 추가하여 원격 위치에서 스크립트를 로드합니다. 스크립트 로드 프로세스에 이 모듈을 사용하면 작업 코드를 더 세부적으로 재평가할 수 있습니다.
...
라이브러리 파일이 프로젝트에 있으면 [확장 매니페스트](./manifest.md)(`extension.json`)를 업데이트하여 공급업체 라이브러리가 기본 태그 라이브러리와 함께 제공되어야 함을 표시해야 합니다. 이 작업은 `hostedLibFiles` 배열 내에 라이브러리 파일에 대한 경로를 추가하여 수행합니다.
