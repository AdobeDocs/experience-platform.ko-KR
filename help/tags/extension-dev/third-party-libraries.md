...
확장 내에서 타사(공급업체) 라이브러리를 호스팅하는 방법에는 몇 가지가 있습니다. 이 문서에서는 각 솔루션의 장단점을 포함하여 다양한 구현 방법에 대한 개요를 제공합니다.

## 전제 조건

이 문서를 사용하려면 태그 내의 확장 기능 및 작성 방법을 숙지해야 합니다. 자세한 내용은 [확장 개발 개요](./overview.md)를 참조하십시오.

## 기본 코드 로드 프로세스

태그 컨텍스트 외부에서 마케팅 기술이 일반적으로 웹 사이트에서 로드되는 방식을 이해하는 것이 중요합니다. 타사 라이브러리 공급업체는 라이브러리의 기능을 로드하기 위해 웹 사이트의 HTML 내에 포함되어야 하는 코드 조각(기본 코드라고 함)을 제공합니다.
...
전역 함수가 처음 설정되면 라이브러리 로드가 완료되기 전에 함수를 호출할 수 있습니다. 수행하는 모든 호출은 기본 코드의 큐 메커니즘에 추가된 다음 라이브러리가 로드되면 순차적 순서로 실행됩니다.

라이브러리 로드가 완료되면 전역 함수가 큐를 건너뛰고 대신 함수에 대한 향후 호출을 즉시 처리하는 새 함수로 대체됩니다.

### 기본 코드 예

다음 JavaScript는 [Pinterest 변환 태그](https://developers.pinterest.com/docs/ad-tools/conversion-tag/?)에 대한 축소되지 않은 기본 코드의 예이며, 이 문서의 뒷부분에서 참조되어 태그가 있는 다른 구현 전략에 기본 코드가 어떻게 적합한지 보여 줍니다.
...
아래 섹션에서는 이전에 예제로 표시된 Pinterest 기본 코드를 사용하여 확장에서 공급업체 라이브러리를 로드할 수 있는 다양한 방법을 보여줍니다. 이러한 각 예제는 웹 사이트에 라이브러리를 로드하는 웹 확장](./web/action-types.md)에 대한 [작업 유형을 만드는 작업이 포함됩니다.

>[!NOTE]
>
>아래 예제는 데모용으로 작업 유형을 사용하지만, 사이트에 태그 라이브러리를 로드하는 모든 함수에 동일한 원칙을 적용할 수 있습니다.
...
- [타사 라이브러리 구현](#implementing-third-party-libraries)
   ...
   - [태그 구현 옵션](#tags-implementation-options)
      ...
      - [태그 라이브러리 호스트에서 런타임 시 로드](#load-at-runtime-from-the-tag-library-host)
...
이 방법은 CDN의 파일에 수행된 모든 업데이트가 확장에 의해 자동으로 로드되므로 일반적으로 유지 관리하는 가장 쉽습니다.
...
또한 태그는 모든 확장에서 사용할 수 있는 유틸리티인 여러 [핵심 모듈](./web/core.md)을 제공합니다. 특히 `@adobe/reactor-load-script` 모듈은 스크립트 요소를 만들어 문서에 추가하여 원격 위치에서 스크립트를 로드합니다. 스크립트 로드 프로세스에 이 모듈을 사용하면 작업 코드를 더 세부적으로 재평가할 수 있습니다.
...
### 태그 라이브러리 호스트에서 런타임 시 로드

라이브러리 호스팅에 공급업체 CDN을 사용하면 몇 가지 위험이 발생합니다. CDN이 실패하거나, 파일을 언제든 중요한 버그로 업데이트하거나, 파일을 다양한 목적으로 훼손할 수 있습니다.

이러한 문제를 해결하기 위해 공급업체 라이브러리를 확장 내에 별도의 파일로 포함할 수 있습니다. 그런 다음 파일이 기본 태그 라이브러리와 함께 호스팅되도록 확장을 구성할 수 있습니다. 런타임 시 확장 프로그램은 기본 라이브러리를 웹 사이트로 전달한 동일한 서버의 공급업체 라이브러리를 로드합니다.
...
라이브러리 파일이 프로젝트에 있으면 [확장 매니페스트](./manifest.md)(`extension.json`)를 업데이트하여 공급업체 라이브러리가 기본 태그 라이브러리와 함께 전달되어야 함을 나타냅니다. 이 작업은 `hostedLibFiles` 배열 내에 라이브러리 파일에 대한 경로를 추가하여 수행합니다.
...
이 방법을 사용할 때는 라이브러리가 CDN에서 업데이트될 때마다 다운로드한 공급업체 파일을 수동으로 업데이트하고 변경 내용을 새 버전의 확장에 릴리스해야 한다는 점에 유의해야 합니다.

### 라이브러리를 직접 포함

라이브러리 코드를 작업 코드 자체에 직접 포함함으로써 공급업체 라이브러리를 완전히 로드할 필요가 없으므로 기본 태그 라이브러리의 일부로 유용하게 사용할 수 있습니다. 이 방법을 사용하면 기본 라이브러리의 크기가 증가하지만 별도의 파일을 검색하기 위해 추가 HTTP 요청을 수행할 필요가 없습니다.
...
이 문서에서는 태그 확장에서 타사 라이브러리를 호스팅하기 위한 다양한 방법에 대한 개요를 제공합니다. 제공된 예는 라이브러리에 중점을 두었지만 이러한 기술은 확장에서 활용할 수 있는 모든 코드에 적용됩니다.
