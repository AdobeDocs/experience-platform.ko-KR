...
Adobe Experience Platform에서 태그 확장을 테스트하려면 태그 API 및/또는 명령줄 도구를 사용하여 확장 패키지를 업로드합니다. 다음으로, Platform UI 또는 데이터 수집 UI를 사용하여 확장 패키지를 속성에 설치하고 태그 라이브러리 및 빌드에서 해당 기능을 수행합니다.
...
Adobe Experience Platform에서 태그와 함께 사용할 기술 계정을 만드는 방법에 대한 자세한 내용은 [Reactor API 시작](../../api/getting-started.md) 안내서.
...
>[!NOTE]
>
>업로드 또는 패치할 때 확장 패키지는 보류 중인 상태로 전환되고 시스템은 비동기적으로 패키지를 추출하여 배포합니다. 이 프로세스가 진행되는 동안 을(를) 폴링할 수 있습니다 `extension_package` API를 사용하고 UI 내에서 해당 상태에 대한 ID입니다. 보류 중으로 표시된 카탈로그에서 확장 카드가 표시됩니다.
...
UI에 로그인하고 를 선택한 후 **[!UICONTROL 태그]** 왼쪽 탐색에서 [!UICONTROL 속성] 화면이 표시됩니다. 속성은 배포하려는 태그의 컨테이너이며 하나 이상의 사이트에서 사용할 수 있습니다.
...
이전 단계를 따랐는데 카탈로그에 보류 중 또는 실패한 확장 패키지가 표시되지 않는 경우 API를 사용하여 직접 확장 패키지의 상태를 확인해야 합니다. 적절한 API 호출을 수행하는 방법에 대한 자세한 내용은 [ExtensionPackage 가져오기](../../api/endpoints/extension-packages.md#lookup) 를 클릭합니다.
...
다음 `Window Loaded` **이벤트 유형** 는 페이지가 테스트 사이트에 로드될 때마다 이 규칙이 트리거되도록 합니다. **변경사항 유지**&#x200B;를 선택합니다. 이 예제에서는 를 무시합니다 **조건** 테스트 사이트의 모든 페이지에 대해 규칙을 트리거해야 합니다.
