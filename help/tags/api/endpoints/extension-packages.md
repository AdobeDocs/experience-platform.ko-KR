...
>[!WARNING]
>
>기능이 추가, 제거 및 다시 만들어짐에 따라 `/extension_packages` 끝점의 구현이 유동적입니다.

확장 패키지는 확장 개발자가 작성한 [확장](./extensions.md)을 나타냅니다. 확장 패키지는 사용자를 태깅하는 데 사용할 수 있는 추가 기능을 정의합니다. 가장 일반적으로 이러한 기능은 [규칙 구성 요소](./rule-components.md)(이벤트, 조건 및 작업) 및 [데이터 요소](./data-elements.md) 형태로 제공되지만 기본 모듈 및 공유 모듈도 포함할 수 있습니다.
...
확장 패키지는 만든 개발자의 [회사](./companies.md)에 속합니다.
...
Reactor API를 호출하는 방법을 이해하는 것 외에도 확장 패키지의 `status` 및 `availability` 특성이 수행할 수 있는 작업에 어떤 영향을 미치는지 이해하는 것도 중요합니다. 아래 섹션에 설명되어 있습니다.

### 상태

확장 패키지에는 다음과 같은 세 가지 가능한 상태가 있습니다. `pending`, `succeeded` 및 `failed`.

| 상태 | 설명 |
| --- | --- |
| `pending` | 확장 패키지를 만들 때 해당 `status`이(가) `pending`(으)로 설정됩니다. 시스템이 확장 패키지에 대한 정보를 수신하고 처리를 시작함을 나타냅니다. 상태가 `pending`인 확장 패키지를 사용할 수 없습니다. |
| `succeeded` | 처리가 완료되면 확장 패키지의 상태가 `succeeded`(으)로 업데이트됩니다. |
| `failed` | 처리가 완료되지 않으면 확장 패키지의 상태가 `failed`(으)로 업데이트됩니다. 처리가 성공할 때까지 상태가 `failed`인 확장 패키지를 업데이트할 수 있습니다. 상태가 `failed`인 확장 패키지를 사용할 수 없습니다. |

### 가용성

확장 패키지에 대한 가용성 수준이 있습니다. `development`, `private` 및 `public`.

| 가용성 | 설명 |
| --- | --- |
| `development` | `development`의 확장 패키지는 해당 패키지를 소유한 회사에서만 볼 수 있으며 해당 패키지에서 사용할 수 있습니다. 또한 확장 개발에 대해 구성된 속성에만 사용할 수 있습니다. |
| `private` | `private` 확장 패키지는 이 패키지를 소유한 회사에만 표시되며, 회사가 소유한 속성에만 설치할 수 있습니다. |
| `public` | `public` 확장 패키지가 표시되며 모든 회사 및 속성에서 사용할 수 있습니다. |

>[!NOTE]
>
>확장 패키지를 만들 때 `availability`이(가) `development`(으)로 설정됩니다. 테스트가 완료되면 확장 패키지를 `private` 또는 `public`(으)로 전환할 수 있습니다.

## 확장 패키지 목록 검색 {#list}

`/extension_packages`에 GET 요청을 하여 확장 패키지 목록을 검색할 수 있습니다.
...
>[!NOTE]
>
>쿼리 매개 변수를 사용하여 나열된 확장 패키지를 다음 속성에 따라 필터링할 수 있습니다.<ul><li>`archive`</li><li>`created_at`</li><li>`name`</li><li>`stage`</li><li>`token`</li><li>`updated_at`</li></ul>자세한 내용은 [응답 필터링](../guides/filtering.md)에 대한 안내서를 참조하세요.
...
| 매개변수 | 설명 |
| --- | --- |
| `EXTENSION_PACKAGE_ID` | 조회할 확장 패키지의 `id`입니다. |
...
확장 패키지는 Node.js 스캐폴딩 툴을 사용하여 생성되며 Reactor API에 제출되기 전에 로컬 시스템에 저장됩니다. 확장 패키지 구성에 대한 자세한 내용은 [확장 개발 시작](../../extension-dev/getting-started.md)에 대한 안내서를 참조하십시오.
...
다음 요청은 새 확장 패키지를 만듭니다. 업로드 중인 패키지 파일의 로컬 경로가 양식 데이터(`package`)로 참조되므로 이 끝점에는 `multipart/form-data`의 `Content-Type` 헤더가 필요합니다.
...
| 매개변수 | 설명 |
| --- | --- |
| `EXTENSION_PACKAGE_ID` | 업데이트할 확장 패키지의 `id`입니다. |

{style="table-layout:auto"}

**요청**

[확장 패키지 만들기](#create)와 마찬가지로 업데이트된 패키지의 로컬 버전은 양식 데이터를 통해 업로드해야 합니다.
...
비공개로 릴리스한 후에는 [공개 릴리스 요청 양식](https://www.feedbackprogram.adobe.com/c/r/DCExtensionReleaseRequest)을 작성하여 공개 릴리스 프로세스를 시작할 수 있습니다.
...
| 매개변수 | 설명 |
| --- | --- |
| `EXTENSION_PACKAGE_ID` | 비공개 릴리스할 확장 패키지의 `id`. |

{style="table-layout:auto"}

**요청**

개인 릴리스는 요청 데이터의 `meta`에서 `action`의 값을 `release_private`로 제공하여 수행됩니다.
...
조회 요청의 경로에 `/versions`을(를) 추가하여 확장 패키지의 버전을 나열할 수 있습니다.
...
| 매개변수 | 설명 |
| --- | --- |
| `EXTENSION_PACKAGE_ID` | 나열할 버전의 확장 패키지의 `id`입니다. |
