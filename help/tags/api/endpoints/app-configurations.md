...
>[!WARNING]
>
>기능이 추가, 제거 및 재작동하므로 `/app_configurations` 종단점의 구현은 유동적입니다.

앱 구성을 사용하면 자격 증명을 저장하고 나중에 사용할 수 있습니다. Reactor API의 `/app_configurations` 종단점을 사용하면 경험 애플리케이션 내에서 앱 구성을 프로그래밍 방식으로 관리할 수 있습니다.
...
| 매개 변수 | 설명 |
| --- | --- |
| `COMPANY_ID` | 앱 구성을 소유하는 [company](./companies.md)의 `id`. |

{style=&quot;table-layout:auto&quot;}

>[!NOTE]
>
>쿼리 매개 변수를 사용하여 나열된 앱 구성을 다음 속성에 따라 필터링할 수 있습니다.<ul><li>`app_id`</li><li>`created_at`</li><li>`key_type`</li><li>`messaging_service`</li><li>`name`</li><li>`platform`</li><li>`updated_at`</li></ul>자세한 내용은 [응답 필터링](../guides/filtering.md)에 대한 안내서를 참조하십시오.
...
| 매개 변수 | 설명 |
| --- | --- |
| `APP_CONFIGURATION_ID` | 조회할 앱 구성의 `id` |
...
| 매개 변수 | 설명 |
| --- | --- |
| `COMPANY_ID` | 에서 앱 구성을 정의하는 [company](./companies.md)의 `id` |
...
| `messaging_service` | [Apple APNs(Push Notification Service)](https://developer.apple.com/library/archive/documentation/NetworkingInternet/Conceptual/RemoteNotificationsPG/APNSOverview.html) 및 [Firebase Cloud Messaging(FCM)](https://firebase.google.com/docs/cloud-messaging)과 같이 앱과 연결된 메시징 서비스입니다. 이것은 사용할 수 있는 키 유형을 결정합니다. |
| `key_type` | 푸시 서비스 공급업체가 지원하는 프로토콜을 나타내며 `push_credential` 개체의 형식을 결정합니다. 메시징 서비스에 대한 프로토콜이 발전함에 따라 업데이트된 프로토콜을 지원하기 위해 새로운 `key_type` 값이 만들어집니다. |
| `push_credential` | 실제 자격 증명 값(나머지 위치에서 암호화됨)입니다. 이 필드는 일반적으로 해독되거나 API 응답에 포함되지 않습니다. 특정 Adobe 서비스만 해독된 푸시 자격 증명이 포함된 응답을 가져올 수 있습니다. |
...
PUT 요청 경로에 해당 ID를 포함하여 앱 구성을 업데이트할 수 있습니다.
...
| 매개 변수 | 설명 |
| --- | --- |
| `APP_CONFIGURATION_ID` | 업데이트할 앱 구성의 `id` |
...
| `id` | 업데이트할 앱 구성의 `id` 이 값은 요청 경로에 제공된 `{APP_CONFIGURATION_ID}` 값과 일치해야 합니다. |
| `type` | 업데이트할 리소스 유형입니다. 이 끝점의 경우 값은 `app_configurations`이어야 합니다. |
...
| 매개 변수 | 설명 |
| --- | --- |
| `APP_CONFIGURATION_ID` | 삭제할 앱 구성의 `id` |
