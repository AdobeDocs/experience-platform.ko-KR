...
콜백은 Reactor API에서 활동을 추적하기 위해 [감사 이벤트](./audit-events.md)와 함께 사용됩니다. 특정 유형의 감사 이벤트가 생성될 때마다 콜백은 지정된 URL에 일치하는 메시지를 보낼 수 있습니다.
...
콜백은 정확히 하나의 [속성](./properties.md)에 속합니다. 속성에는 여러 콜백이 있을 수 있습니다.
...
>[!NOTE]
>
>쿼리 매개 변수를 사용하여 나열된 콜백을 다음 속성을 기준으로 필터링할 수 있습니다.<ul><li>`created_at`</li><li>`updated_at`</li></ul>자세한 내용은 [응답 필터링](../guides/filtering.md)에 대한 안내서를 참조하세요.
...
| 매개변수 | 설명 |
| --- | --- |
| `PROPERTY_ID` | 아래 콜백을 정의하는 [property](./properties.md)의 `id`입니다. |
...
| `subscriptions` | 콜백을 트리거할 감사 이벤트 유형을 나타내는 문자열 배열입니다. 가능한 이벤트 유형 목록은 [감사 이벤트 끝점 안내서](./audit-events.md)를 참조하십시오. |
...
| 매개변수 | 설명 |
| --- | --- |
| `CALLBACK_ID` | 업데이트할 콜백의 `id`입니다. |

{style="table-layout:auto"}

**요청**

다음 요청은 기존 콜백에 대한 `subscriptions` 배열을 업데이트합니다.
...
| 속성 | 설명 |
| --- | --- |
| `attributes` | 콜백에 대해 업데이트될 속성을 나타내는 속성을 갖는 객체입니다. 각 키는 업데이트해야 하는 해당 값과 함께 업데이트할 특정 콜백 속성을 나타냅니다.<br><br>콜백에 대해 다음 특성을 업데이트할 수 있습니다.<ul><li>`subscriptions`</li><li>`url`</li></ul> |
| `id` | 업데이트할 콜백의 `id`입니다. 요청 경로에 제공된 `{CALLBACK_ID}` 값과 일치해야 합니다. |
| `type` | 업데이트 중인 리소스 유형. 이 끝점의 경우 값은 `callbacks`이어야 합니다. |
