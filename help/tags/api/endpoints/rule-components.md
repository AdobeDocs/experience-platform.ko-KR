...
데이터 수집 태그에서 [규칙](./rules.md)은(는) 배포된 [라이브러리](./libraries.md)에 있는 리소스의 동작을 제어합니다. **규칙** 구성 요소는 규칙을 구성하는 개별 부품입니다. 규칙이 배합식인 경우 규칙 구성요소는 재료 중 하나입니다. Reactor API의 `/rule_components` 종단점을 사용하면 규칙 구성 요소를 프로그래밍 방식으로 관리할 수 있습니다.

>[!NOTE]
>
>이 문서에서는 Reactor API에서 규칙 구성 요소를 관리하는 방법을 다룹니다. 데이터 수집 UI에서 규칙 및 규칙 구성 요소와 상호 작용하는 방법에 대한 자세한 내용은 [UI 안내서](../../ui/managing-resources/rules.md)를 참조하십시오.

규칙 구성 요소에는 세 가지 기본 유형이 있습니다.

| 규칙 구성 요소 유형 | 설명 |
| --- | --- |
| 이벤트 | 이벤트는 규칙의 트리거입니다. 규칙은 클라이언트 장치에서 런타임 시 이벤트가 발생할 때 시작됩니다. &quot;[!UICONTROL 라이브러리 로드]&quot;, &quot;[!UICONTROL 페이지 상단]&quot; 및 &quot;[!UICONTROL 클릭]&quot;은 이벤트의 예입니다. |
...
규칙 구성 요소는 정확히 하나의 [확장](./extensions.md)에서 제공됩니다. 확장은 다양한 규칙 구성 요소 유형을 제공할 수 있습니다.
...
| 매개 변수 | 설명 |
| --- | --- |
| `RULE_ID` | 구성 요소를 나열할 규칙의 `id` |

{style=&quot;table-layout:auto&quot;}

>[!NOTE]
>
>쿼리 매개 변수를 사용하여 나열된 규칙 구성 요소를 다음 속성에 따라 필터링할 수 있습니다.<ul><li>`created_at`</li><li>`dirty`</li><li>`enabled`</li><li>`name`</li><li>`negate`</li><li>`origin_id`</li><li>`published`</li><li>`published_at`</li><li>`revision_number`</li><li>`updated_at`</li></ul>자세한 내용은 [응답 필터링](../guides/filtering.md)에 대한 안내서를 참조하십시오.
...
| 매개 변수 | 설명 |
| --- | --- |
| `RULE_COMPONENT_ID` | 조회할 규칙 구성 요소의 `id` |
...
| 매개 변수 | 설명 |
| --- | --- |
| `RULE_ID` | 규칙 구성 요소를 정의하는 규칙의 `id` |

{style=&quot;table-layout:auto&quot;}

**요청**

다음 요청은 지정된 규칙에 대한 새 규칙 구성 요소를 만듭니다. 또한 호출은 `relationships` 속성을 통해 규칙 구성 요소를 기존 확장과 연결합니다. 자세한 내용은 [관계](../guides/relationships.md)의 안내서를 참조하십시오.
...
| 속성 | 설명 |
| --- | --- |
| `attributes.delegate_descriptor_id` | **(필수)** 정의할 수 있는 규칙 구성 요소 유형은  [확장 패키지로 제공됩니다](./extension-packages.md). 새 규칙 구성 요소를 만들 때 이 규칙 구성 요소가 기반으로 하는 확장 패키지, 구성 요소의 유형(이벤트, 조건 또는 작업) 및 확장에서 정의한 특정 구성 요소의 이름(예: 코어 확장의 &quot;클릭&quot; 이벤트)을 나타내는 위임 설명자 ID를 제공해야 합니다.<br><br>자세한 내용은  [위임 설명자 ](../guides/delegate-descriptor-ids.md) ID에 대한 안내서를 참조하십시오. |
...
| `relationships` | 규칙 구성 요소에 필요한 관계를 설정하는 객체입니다. 두 가지 관계를 설정해야 합니다. <ol><li>`extension`: 이 규칙 구성 요소를 정의하는 확장입니다. 확장 패키지가 `delegate_descriptor_id`에 표시된 것과 동일한 확장이어야 합니다.</li><li>`rules`: 이 구성 요소가 정의되는 규칙입니다. 요청 경로에 제공된 것과 동일한 규칙 ID여야 합니다.</li></ol>관계에 대한 일반적인 정보는 [관계 안내서](../guides/relationships.md)를 참조하십시오. |
| `type` | 만들어지는 리소스 유형입니다. 이 끝점의 경우 값은 `rule_components`이어야 합니다. |
...
| 매개 변수 | 설명 |
| --- | --- |
| `RULE_COMPONENT_ID` | 업데이트할 규칙 구성 요소의 `id` |

{style=&quot;table-layout:auto&quot;}

**요청**

다음 요청은 기존 규칙 구성 요소에 대한 `order` 및 `settings` 속성을 업데이트합니다.
...
| `id` | 업데이트할 규칙 구성 요소의 `id` 이 값은 요청 경로에 제공된 `{RULE_COMPONENT_ID}` 값과 일치해야 합니다. |
| `type` | 업데이트할 리소스 유형입니다. 이 끝점의 경우 값은 `rule_components`이어야 합니다. |
...
| 매개 변수 | 설명 |
| --- | --- |
| `RULE_COMPONENT_ID` | 삭제할 규칙 구성 요소의 `id` |
...
규칙 구성 요소는 &quot;주목할 만한&quot; 리소스입니다. 즉, 각 개별 리소스에 대해 텍스트 기반 메모를 만들고 검색할 수 있습니다. 규칙 구성 요소 및 기타 호환 리소스에 대한 메모를 관리하는 방법에 대한 자세한 내용은 [참고 엔드포인트 안내서](./notes.md)를 참조하십시오.

## 규칙 구성 요소에 대한 관련 리소스 검색 {#related}

다음 호출에서는 규칙 구성 요소에 대한 관련 리소스를 검색하는 방법을 보여 줍니다. [규칙 구성 요소를 찾으면 이러한 관계가 `relationships` 규칙 구성 요소 아래에 나열됩니다.](#lookup)

Reactor API의 관계에 대한 자세한 내용은 [관계 안내서](../guides/relationships.md)를 참조하십시오.

### 규칙 구성 요소에 대한 관련 규칙 나열 {#rules}

조회 요청의 경로에 `/rules`을 추가하여 특정 규칙 구성 요소를 활용하는 규칙을 나열할 수 있습니다.
...
| 매개 변수 | 설명 |
| --- | --- |
| `{RULE_COMPONENT_ID}` | 규칙을 나열할 규칙 구성 요소의 `id` |
...
조회 요청의 경로에 `/extension`을 추가하여 규칙 구성 요소를 제공하는 확장을 조회할 수 있습니다.
...
| 매개 변수 | 설명 |
| --- | --- |
| `{RULE_COMPONENT_ID}` | 확장을 조회할 규칙 구성 요소의 `id` |
...
조회 요청의 경로에 `/origin`을 추가하여 규칙 구성 요소에 대한 원본(이전 개정)을 조회할 수 있습니다.
...
| 매개 변수 | 설명 |
| --- | --- |
| `{RULE_COMPONENT_ID}` | 원본을 조회하려는 규칙 구성 요소의 `id` |
