...
각 리소스 유형에 대한 호환 관계 목록 및 해당하는 경우 이러한 관계를 설정하는 데 필요한 방법은 [관계 요구 사항](#requirements)의 섹션을 참조하십시오.
...
* [필요한 ](../endpoints/data-elements.md#create) 확장을 사용하여 데이터 요소 만들기
* [필요한 호스트 ](../endpoints/environments.md#create) 관계를 사용하여 환경 만들기
...
다음 요청은 `rules` 및 `extension`와의 관계를 설정하는 새 `rule_component`을 만듭니다.
...
| 속성 | 설명 |
| --- | --- |
| `relationships` | 페이로드별로 관계를 만들 때 제공해야 하는 개체입니다. 이 개체의 각 키는 특정 관계 유형을 나타냅니다. 위의 예에서 `extension` 및 `rules` 관계는 특히 `rule_components`에 대해 설정됩니다. 다른 리소스의 호환 관계 유형에 대한 자세한 내용은 [리소스](#relationship-requirements-by-resource)에 의한 관계 요구 사항 섹션을 참조하십시오. |
| `data` | `relationship` 개체 아래에 제공된 각 관계 유형에는 관계가 설정되는 리소스의 `id` 및 `type`을 참조하는 `data` 속성이 포함되어야 합니다. 해당 리소스의 `id` 및 `type`를 포함하는 각 개체를 사용하여 `data` 속성을 객체 배열로 지정하여 같은 유형의 여러 리소스와 관계를 만들 수 있습니다. |
| `id` | 리소스의 고유 ID입니다. 각 `id`에는 해당 리소스의 유형을 나타내는 동위 `type` 속성이 포함되어야 합니다. |
| `type` | 동위 `id` 필드에서 참조하는 리소스 유형입니다. 허용되는 값에는 `data_elements`, `rules`, `extensions` 및 `environments`이 포함됩니다. |
...
| `{RESOURCE_TYPE}` | 관계가 타깃팅하는 리소스의 유형입니다. 사용 가능한 값에는 `environment`, `data_elements`, `extensions` 및 `rules`이 포함됩니다. |
...
| 속성 | 설명 |
| --- | --- |
| `data` | 관계에 대한 대상 리소스의 `id` 및 `type`을 참조하는 개체입니다. 동일한 유형의 여러 리소스(예: `extensions` 및 `rules`)와 관계를 만드는 경우, `data` 속성은 각 객체가 적용 가능한 리소스의 `id` 및 `type`를 포함하는 객체 배열로서 형식을 지정해야 합니다. |
| `id` | 리소스의 고유 ID입니다. 각 `id`에는 해당 리소스의 유형을 나타내는 동위 `type` 속성이 포함되어야 합니다. |
| `type` | 동위 `id` 필드에서 참조하는 리소스 유형입니다. 허용되는 값에는 `data_elements`, `rules`, `extensions` 및 `environments`이 포함됩니다. |
