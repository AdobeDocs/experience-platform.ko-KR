...
>[!IMPORTANT]
>
>확장 프로그램은 Adobe Experience Platform Web SDK 확장 프로그램과 함께 사용됩니다. AppMeasurement에서 사용할 버전에 대한 정보를 보려면 [일반적인 Analytics 플러그인 확장 프로그램](../plugins/overview.md)의 개요를 참조하십시오.

이 문서에서는 Web SDK 플러그인 태그 확장을 구성하고 이 확장을 사용하여 [Adobe Experience Platform Web SDK 확장](../web-sdk/overview.md)을 확장하는 방법을 다룹니다.
...
>[!IMPORTANT]
>
>일반적인 웹 SDK 플러그인 확장 프로그램은 Adobe Experience Platform 웹 SDK 확장 프로그램을 늘리기 위한 것이지만 확장 프로그램이 예상대로 작동하도록 설치할 필요는 없습니다.

## Adobe Experience Platform Web SDK 확장 기능에 플러그인 추가

일반적인 웹 SDK 플러그인 확장에서 제공하는 다음 기본 데이터 요소를 사용하는 외에 플러그인을 초기화하거나 라이브러리에 추가하는 구성은 필요하지 않습니다.
...
* `"latitude=[LATITUDE] | longtitude=[LONGITUDE]"`: 여기서 `[LATITUDE]/[LONGITUDE]`은(는) 각각 위도와 경도입니다.
...
* `ep`(선택 사항, 문자열): `et` 인수도 설정된 경우에만 이 인수를 설정하십시오. `"m"` 인수가 일 대신 분 단위로 만료되도록 하려면 이 인수를 `et`(으)로 설정하십시오. 기본값은 `"d"`이고, 이 값은 `et` 인수를 일 단위로 설정합니다.
