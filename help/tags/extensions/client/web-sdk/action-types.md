...
Experience Platform에서 전송하는 데이터를 수집하고 해당 정보에 대해 조치를 취할 수 있도록 이벤트를 Experience Platform에 보냅니다. 전송하려는 모든 데이터는 **[!UICONTROL XDM 데이터]** 필드에서 전송할 수 있습니다. [!DNL JSON] 스키마의 구조를 준수하는 [!DNL XDM] 개체를 사용합니다. 이 개체는 페이지 또는 **[!UICONTROL 사용자 지정 코드]** **[!UICONTROL 데이터 요소]**&#x200B;를 통해 만들 수 있습니다.
...
* **[!UICONTROL 유형]**: 이 필드에서는 XDM 스키마에 기록될 이벤트 유형을 지정할 수 있습니다. 자세한 내용은 [`type`](/help/web-sdk/commands/sendevent/type.md) 명령의 `sendEvent`을(를) 참조하십시오.
* **[!UICONTROL XDM]**
* **[!UICONTROL 데이터]**: 이 필드를 사용하여 XDM 스키마와 일치하지 않는 데이터를 보냅니다. 이 필드는 Adobe Target 프로필을 업데이트하거나 Target Recommendations 속성을 보내려는 경우에 유용합니다. 자세한 내용은 [`data`](/help/web-sdk/commands/sendevent/data.md) 명령의 `sendEvent`을(를) 참조하십시오.
...
* **[!UICONTROL 표면]**: 페이지에서 개인화를 사용할 수 있는 웹 표면을 설정합니다. 자세한 내용은 [Adobe Journey Optimizer 설명서](https://experienceleague.adobe.com/docs/journey-optimizer/using/web/create-web.html?lang=ko)를 참조하세요.
...
* **[!UICONTROL 기본 개인화 요청]**: 이 섹션을 사용하여 페이지 범위(글로벌 mbox)와 기본 표면(현재 URL을 기반으로 하는 웹 표면)을 요청할지 여부를 제어합니다. 기본적으로 페이지 로드 중 처음 `sendEvent` 호출 중에 자동으로 요청됩니다. 다음 옵션 중에서 선택할 수 있습니다.
   ...
   * **[!UICONTROL 사용]**: 페이지 범위 및 기본 표면을 명시적으로 요청합니다. SPA 보기 캐시를 업데이트합니다. `requestDefaultPersonalization`(으)로 설정된 `true`에 해당합니다.
   * **[!UICONTROL 사용 안 함]**: 페이지 범위 및 기본 표면에 대한 요청을 명시적으로 표시하지 않습니다. `requestDefaultPersonalization`(으)로 설정된 `false`에 해당합니다.
* **[!UICONTROL 의사 결정 컨텍스트]**: 온디바이스 의사 결정에 대한 Adobe Journey Optimizer 규칙 세트를 평가할 때 사용되는 키-값 맵입니다. 결정 컨텍스트를 수동으로 또는 데이터 요소를 통해 제공할 수 있습니다.

### Advertising {#advertising}

Web SDK 사용자 지정 빌드 구성 요소에 대해 **[!UICONTROL Advertising]** 구성 요소를 선택하면 `sendEvent` 작업에 대한 규칙 설정에 속성 측정에 광고 데이터를 사용하는 방법을 정의하는 [!UICONTROL Advertising] 섹션이 포함됩니다. 이 설정은 규칙에 여러 작업의 시퀀스가 포함된 경우 유용합니다.

![이벤트 보내기 작업 유형에 대한 Advertising 설정을 보여 주는 Experience Platform 태그 UI 이미지입니다.](assets/send-event-advertising.png)

**[!UICONTROL 기본 Advertising 데이터 요청]** 섹션에서 다음 옵션을 제공합니다.

* **[!UICONTROL 자동]**: 이 이벤트 시 사용할 수 있는 모든 광고 데이터가 자동으로 XDM에 추가됩니다.
* **[!UICONTROL 대기]**: 광고 데이터를 검색하고 해결할 때까지 이 호출의 실행을 지연합니다. 그런 다음 데이터를 XDM에 추가합니다.
* **[!UICONTROL 사용 안 함]**: XDM에 광고 데이터를 추가하지 마십시오. Customer Journey Analytics 또는 Adobe Analytics에 적합하지 않은 모든 요청에 이 옵션을 사용합니다.
* **[!UICONTROL 데이터 요소를 제공합니다]**: 페이지를 로드하는 동안 광고 데이터를 포함하거나 제외하려면 데이터 요소를 사용하십시오. 데이터 요소에 대해 확인된 값에는 `automatic`, `wait` 및 `disabled`이(가) 포함될 수 있습니다.
