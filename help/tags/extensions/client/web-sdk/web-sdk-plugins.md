...
>[!IMPORTANT]
>
>확장 프로그램은 Adobe Experience Platform 웹 SDK 확장 프로그램과 함께 사용됩니다. AppMeasurement에 사용할 버전에 대한 정보를 보려면 [일반 Analytics 플러그인 확장 프로그램](../plugins/overview.md).

이 문서에서는 Web SDK 플러그인 태그 확장 기능을 구성하고 이를 사용하여 다음을 확장하는 방법을 다룹니다. [Adobe Experience Platform 웹 SDK 확장](../web-sdk/overview.md).
...
다음 `getPageName` 데이터 요소는 다음 인수를 사용합니다.
...
`t` (선택 사항이지만 권장됨, 문자열): 방문자의 현지 시간을 변환하여 구할 시간대의 이름입니다.  기본값은 UTC/GMT 시간으로 설정됩니다. 다음을 참조하십시오. [TZ 데이터베이스 시간대 목록](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones) 위키백과에서 유효한 가치의 전체 목록을 확인하십시오.
