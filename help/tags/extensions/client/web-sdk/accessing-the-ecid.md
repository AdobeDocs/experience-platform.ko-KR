...
[데이터 수집을 위한 데이터 준비](/help/datastreams/data-prep.md)(권장) 또는 태그를 통해 ECID에 액세스할 수 있습니다.

## 데이터 준비를 통해 ECID에 액세스(기본 방법) {#accessing-ecid-data-prep}

이 메서드는 [데이터 수집을 위한 데이터 준비](/help/datastreams/data-prep.md)를 사용하여 `ECID`에 대한 사용자 지정 매핑을 구성합니다.

이 기능을 사용하는 방법에 대해 알아보려면 [데이터 수집을 위한 데이터 준비](/help/datastreams/data-prep.md) 설명서를 참조하십시오.
...
1. 속성이 [규칙 구성 요소 시퀀싱](/help/tags/ui/managing-resources/rules.md#sequencing)을 사용하도록 구성되어 있는지 확인하십시오.
1. 새 규칙을 만듭니다. 이 규칙은 다른 중요한 작업 없이 [!DNL ECID]을(를) 캡처하는 데만 사용해야 합니다.
1. 규칙에 [!UICONTROL Library Loaded] 이벤트를 추가합니다.
1. 다음 코드를 사용하여 규칙에 [!UICONTROL Custom Code] 작업을 추가합니다(SDK 인스턴스에 대해 구성한 이름이 `alloy`이고 같은 이름의 데이터 요소가 아직 없다고 가정).
...
그런 다음 다른 데이터 요소에 액세스하는 것처럼 [!DNL ECID] 또는 `%ECID%`을(를) 사용하여 후속 규칙의 `_satellite.getVar("ECID")`에 액세스할 수 있습니다.
