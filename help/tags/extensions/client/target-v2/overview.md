...
* **Body 숨김 스타일:** 기본값은 body{opacity:0}입니다. 이 값은 body{display:none}과(와) 같은 다른 값으로 변경될 수 있습니다.
