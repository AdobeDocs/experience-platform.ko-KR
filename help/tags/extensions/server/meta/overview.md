...
>[!IMPORTANT]
>
>* Real-Time CDP Prime 및 Ultimate 패키지를 구입한 고객이 빠른 시작 기능을 사용할 수 있습니다. 자세한 내용은 Adobe 담당자에게 문의하십시오.
>* 이 기능은 순 새로운 구현을 위한 것으로서, 현재 기존 태그 및 이벤트 전달 속성에 확장 및 구성 자동 설치를 지원하지 않습니다.


빠른 시작 기능은 메타 전환 API 및 메타 픽셀 확장 기능을 사용하여 쉽고 효율적으로 설정할 수 있도록 도와줍니다. 이 도구는 Adobe 태그와 이벤트 전달에서 수행되는 여러 단계를 자동화하여 설정 시간을 크게 줄입니다.

이 기능은 필요한 규칙 및 데이터 요소를 사용하여 새로 생성된 태그 및 이벤트 전달 속성에 메타 전환 API와 메타 픽셀 확장 프로그램을 모두 자동으로 설치 및 구성합니다. 또한 Experience Platform Web SDK 및 데이터 스트림을 자동으로 설치 및 구성합니다. 마지막으로 빠른 시작 기능은 개발 환경의 지정된 URL에 라이브러리를 자동으로 게시하여 이벤트 전달 및 Experience Edge를 통해 클라이언트측 데이터 수집 및 서버측 이벤트 전달을 실시간으로 가능하게 합니다.

다음 비디오에서는 빠른 시작 기능을 소개합니다.
...
>[!NOTE]
>
>이 기능은 이벤트 전달 구현을 시작하는 데 도움이 되도록 설계되었습니다. 모든 사용 사례를 포용하는 완벽한 기능 구현은 종단간 전달되지 않습니다.

이 설정은 메타 변환 API와 메타 픽셀 확장 프로그램을 모두 자동으로 설치합니다. 이 하이브리드 구현은 이벤트 전환 서버 측을 수집하고 전달하는 데 Meta에서 권장합니다.
빠른 설정 기능은 고객이 이벤트 전달 구현을 시작할 수 있도록 고안되었으며, 모든 사용 사례를 수용하는 완전히 기능적인 구현을 처음부터 끝까지 전달하려는 것이 아닙니다.

기능을 설치하려면 **[!UICONTROL 시작하기]** 대상 **[!DNL Send Conversions Data to Meta]** Adobe Experience Platform 데이터 수집 **[!UICONTROL 홈]** 페이지.

![메타에 대한 전환 데이터를 보여주는 데이터 수집 홈 페이지](../../../images/extensions/server/meta/conversion-data-to-meta.png)

을(를) 입력합니다. **[!UICONTROL 도메인]**&#x200B;를 선택하고 을 선택합니다. **[!UICONTROL 다음]**. 이 도메인은 자동 생성된 태그 및 이벤트 전달 속성, 규칙, 데이터 요소, 데이터 세트 등에 대한 이름 지정 규칙으로 사용됩니다.

![도메인 이름을 요청하는 시작 화면](../../../images/extensions/server/meta/welcome.png)

에서 **[!UICONTROL 초기 설정]** 대화 상자 입력 **[!UICONTROL 메타 픽셀 ID]**, **[!UICONTROL 메타 전환 API 액세스 토큰]**, 및 **[!UICONTROL 데이터 레이어 경로]**&#x200B;를 선택하고 을 선택합니다. **[!UICONTROL 다음]**.

![초기 설정 대화 상자](../../../images/extensions/server/meta/initial-setup.png)

초기 설정 프로세스를 완료하기 위해 몇 분 정도 기다렸다가 **[!UICONTROL 다음]**.

![초기 설정 완료 확인 화면](../../../images/extensions/server/meta/setup-complete.png)

에서 **[!UICONTROL 사이트에 코드 추가]** 대화 상자에서 복사를 사용하여 제공된 코드를 복사합니다. ![복사](../../../images/extensions/server/meta/copy-icon.png) 함수에 붙여 넣습니다. `<head>` 소스 웹 사이트에 액세스할 수 있습니다. 구현되면 을 선택합니다 **[!UICONTROL 유효성 검사 시작]**

![사이트 대화 상자에 코드 추가](../../../images/extensions/server/meta/add-code-on-your-site.png)

다음 [!UICONTROL 유효성 검사 결과] 대화 상자에 Meta 확장 구현 결과가 표시됩니다. **[!UICONTROL 다음]**&#x200B;을 선택합니다. 을(를) 선택하여 추가 유효성 검사 결과를 볼 수도 있습니다 **[!UICONTROL 보증]** 링크를 클릭합니다.

![구현 결과를 표시하는 테스트 결과 대화 상자](../../../images/extensions/server/meta/test-results.png)

다음 **[!UICONTROL 다음 단계]** 화면 디스플레이에서 설정 완료를 확인합니다. 여기에서는 다음 섹션에 표시되는 새 이벤트를 추가하여 구현을 최적화하는 옵션이 있습니다.

이벤트를 추가하지 않으려면 을(를) 선택합니다. **[!UICONTROL 닫기]**.
...
![태그 웹 속성 편집을 보여주는 다음 단계 대화 상자](../../../images/extensions/server/meta/edit-your-tags-web-property.png)

편집할 메타 이벤트에 해당하는 규칙을 선택합니다. 예, **MetaConversion_AddToCart**.

>[!NOTE]
>
>이벤트가 없으면 이 규칙이 실행되지 않습니다. 이는 **MetaConversion_PageView** 규칙이 예외입니다.

이벤트를 추가하려면 를 선택합니다 **[!UICONTROL 추가]** 아래에 [!UICONTROL 이벤트] 제목.

![이벤트가 표시되지 않는 태그 속성 페이지](../../../images/extensions/server/meta/edit-rule.png)

을(를) 선택합니다 [!UICONTROL 이벤트 유형]. 이 예제에서는 [!UICONTROL 클릭] 이벤트를 트리거하고 **.add-to-cart-button** 이 선택되어 있습니다. **[!UICONTROL 변경사항 유지]**&#x200B;를 선택합니다.
...
그런 다음 옆의 드롭다운을 선택합니다 **[!UICONTROL 라이브러리에 저장]** 을(를) 선택합니다. **[!UICONTROL 라이브러리 및 빌드에 저장]**. 그러면 변경 사항이 라이브러리에 게시됩니다.

![라이브러리에 저장 및 빌드를 선택합니다.](../../../images/extensions/server/meta/save-and-build.png)
...
>[!IMPORTANT]
>
>이 글로벌 데이터 계층을 업데이트하는 방법은 웹 사이트 아키텍처에 따라 다릅니다. 단일 페이지 애플리케이션은 서버측 렌더링 앱과 다릅니다. 또한 태그 제품 내에서 이 데이터를 만들고 업데이트하는 데에 전적으로 관여할 수도 있습니다. 모든 인스턴스에서 각 `MetaConversion_* rules`. 규칙 간에 데이터를 업데이트하지 않으면 마지막으로 오래된 데이터를 전송하는 경우에 발생할 수도 있습니다 `MetaConversion_* rule` 현재 `MetaConversion_* rule`.

구성 중에 데이터 계층이 있는 위치를 묻는 메시지가 표시되었습니다. 기본적으로 다음과 같습니다 `window.dataLayer.meta`, 및 내부 `meta` 개체 아래에 표시된 대로 데이터가 예상됩니다.

![데이터 계층 메타 정보](../../../images/extensions/server/meta/data-layer-meta.png)

이것은 모든 사람이 `MetaConversion_*` 규칙은 이 데이터 구조를 사용하여 관련 데이터를 [!DNL Meta Pixel] 확장 및 [!DNL Meta Conversions API]. 다음 문서를 참조하십시오. [표준 이벤트](https://developers.facebook.com/docs/meta-pixel/reference#standard-events) 다양한 메타 이벤트에 필요한 데이터에 대한 자세한 정보.

예를 들어 `MetaConversion_Subscribe` 규칙을 업데이트해야 합니다. `window.dataLayer.meta.currency`, `window.dataLayer.meta.predicted_ltv`, 및 `window.dataLayer.meta.value` 설명서에서 설명하는 개체 속성에 따라 [표준 이벤트](https://developers.facebook.com/docs/meta-pixel/reference#standard-events).

다음은 규칙이 실행되기 전에 데이터 레이어를 업데이트하기 위해 웹 사이트에서 실행해야 하는 작업의 예입니다.

![데이터 계층 메타 정보 업데이트](../../../images/extensions/server/meta/update-data-layer-meta.png)

기본적으로 `<datalayerpath>.conversionData.eventId` 는 다음 중 하나에서 &quot;새 이벤트 ID 생성&quot; 작업에 의해 임의로 생성됩니다 `MetaConversion_* rules`.

데이터 레이어가 표시되는 방식에 대한 로컬 참조의 경우, `MetaConversion_DataLayer` 속성에 대한 데이터 요소입니다.
