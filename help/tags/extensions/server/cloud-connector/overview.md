...
| 쿠키 | 이전에 서버에서 [`Set-Cookie`](https://developer.mozilla.org/ko-KR/docs/Web/HTTP/Headers/Set-Cookie) 헤더를 사용해 전송한 저장된 [HTTP 쿠키](https://developer.mozilla.org/ko-KR/docs/Web/HTTP/Cookies)가 포함됩니다. |
| 일자 | 일반 HTTP 헤더에는 메시지가 시작된 날짜와 시간이 포함됩니다. |
...
이제 새 규칙이 준비되었습니다. **[!UICONTROL Save to Library]**&#x200B;을(를) 선택한 다음 **[!UICONTROL Build]**&#x200B;을(를) 선택하여 배포합니다. 이제 [!DNL mTLS] 요청이 활성 상태이며 라이브러리에서 사용할 수 있습니다.
