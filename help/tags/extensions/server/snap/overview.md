...
확장을 설치하려면 **[!UICONTROL Data Collection]**>**[!UICONTROL Event Forwarding]**(으)로 이동합니다. 확장을 설치할 속성을 선택합니다.

원하는 속성을 선택하고 다음 단계를 수행합니다.

1. 왼쪽 탐색 패널에서 **[!UICONTROL Extensions]**&#x200B;을(를) 선택합니다.
2. **[!UICONTROL Snap Conversion API Extension]**&#x200B;을(를) 검색하고 **[!UICONTROL Install]**&#x200B;을(를) 선택합니다.
...
3. **[!UICONTROL Core]**&#x200B;을(를) 확장으로 선택하고 **[!UICONTROL Path]**&#x200B;을(를) 데이터 요소 유형으로 선택합니다.

4. 드롭다운 메뉴에서 적절한 항목을 선택하고 오른쪽 패널의 [!UICONTROL Path] 필드에 내용을 입력하여 스키마에서 원하는 데이터를 참조합니다.
...
![스키마를 표시하는 이미지](../../../images/extensions/server/snap/schema.png)
...
1. 이벤트 전달 속성으로 이동하고 작성 메뉴에서 **[!UICONTROL Rules]**&#x200B;을(를) 선택합니다. **[!UICONTROL Create New Rule]**&#x200B;을(를) 클릭합니다.
...
3. 조건을 저장한 후 스냅 변환 API를 트리거하는 작업을 추가합니다. 왼쪽 패널에서 다음을 수행합니다.
   ...
   * [!UICONTROL Extension] 드롭다운 메뉴를 [!UICONTROL Snap Conversions API Extension]&#x200B;(으)로 설정합니다.

   * [!UICONTROL Action Type] 드롭다운 메뉴를 [!UICONTROL Report Web Conversions]&#x200B;(으)로 설정합니다.
...
| 가격 | `value` |
| 통화 | `currency` |
| 거래 ID | `order_id`(`event_id` 대신 `client dedup idD`에 대해 전송됨) |
...
각 이벤트에는 항상 `event_source`(으)로 설정된 `WEB.`이(가) 필요합니다. 일치시키려면 다음 필드 또는 조합 중 하나 이상도 필요합니다.
...
* **[!UICONTROL Test Mode]** 확인란을 활성화하면 표준 보고 대신 테스트 이벤트 도구에 표시되는 테스트 이벤트로 이벤트가 전송됩니다. 자세한 내용은 이 [비즈니스 도움말 센터 문서](https://businesshelp.snapchat.com/s/article/capi-event-testing?language=en_US#:~:text=Snap's%20Conversions%20API%20(CAPI)%20Test,being%20processed%20as%20production%20results입니다.)를 참조하십시오.
...
이 안내서에서는 **[!DNL Snap]** 확장을 사용하여 서버측 이벤트 데이터를 **[!DNL Snap Conversions API]**&#x200B;에 보내는 방법을 다룹니다. Experience Platform의 이벤트 전달 기능에 대한 자세한 내용은 [이벤트 전달 개요](../../../ui/event-forwarding/overview.md)를 참조하세요.
