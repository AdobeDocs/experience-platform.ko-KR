---
title: Adobe Snapchat 전환 API 확장 통합
description: 이 Adobe Experience Platform 웹 이벤트 API를 사용하면 웹 사이트 상호 작용을 Snapchat과 직접 공유할 수 있습니다.
...
[!DNL Snapchat] 전환 API를 사용하려면 Adobe Experience Platform에 [이벤트 전달 속성](https://experienceleague.adobe.com/en/docs/experience-platform/tags/event-forwarding/getting-started)을 설정하고 속성을 편집하려면 [필수 권한](https://experienceleague.adobe.com/en/docs/experience-platform/collection/permissions)을 설정해야 합니다.

[데이터 스트림](/help/tags/ui/event-forwarding/getting-started.md)을(를) 만들고 [이벤트 전달 서비스](/help/tags/ui/event-forwarding/getting-started#enable-event-forwarding)를 추가합니다.
...
데이터 포인트를 [!DNL Snapchat] 전환 API 확장에 매개 변수로 전달하려면 각 데이터 포인트에 대해 [데이터 요소](https://experienceleague.adobe.com/en/docs/platform-learn/implement-web-sdk/event-forwarding/setup-event-forwarding#create-an-event-forwarding-data-element)를 만들어야 합니다. 다음 단계를 수행하십시오.

1. 속성의 **[!UICONTROL 속성 정보]** 화면에서 **[!UICONTROL 작성]**>**[!UICONTROL 데이터 요소]**(으)로 이동한 다음 **[!UICONTROL 데이터 요소 추가]**&#x200B;를 선택합니다.

   ![데이터 요소 추가 단추](../../../images/extensions/server/snap/add_data_element.png)를 보여 주는 이미지입니다.
...
데이터 요소 만들기에 대한 자세한 내용은 [이벤트 전달 속성 설명서](/help/tags/ui/event-forwarding/overview#data-elements.md)를 참조하세요.

## 스냅으로 전환 이벤트를 전송하는 규칙 만들기 {#create-snap-rules}
...
1. 이벤트 전달 속성으로 이동하고 작성 메뉴에서 **[!UICONTROL 규칙]**&#x200B;을(를) 선택합니다. 그런 다음 **[!UICONTROL 새 규칙 만들기]**&#x200B;를 클릭합니다.
   ...
   ![왼쪽 탐색에 규칙을 표시하는 이미지](../../../images/extensions/server/snap/create_new_rule.png).
...
* 필수 필드:
   ...
   * 모든 이벤트에는 `event_source`이(가) `WEB`(으)로 설정됩니다.

   * 일치에는 다음 필드 또는 조합 중 하나 이상이 필요합니다.
...
[사용자 지정 전환 값 및 ROAS 보고](https://businesshelp.snapchat.com/s/article/custom-conversions-value-roas?language=en_US)를 사용하려면 `contents` 필드에 관련 매개 변수를 포함하십시오. 예: `brand`, `item_price`, `id`.

`Purchase` 이벤트에 대한 구성 예:

[데이터 바인딩을 보여 주는 이미지](../../../images/extensions/server/snap/data_bindings.png)

옵션 필드는 다음과 같이 설정할 수 있습니다.

[선택적 필드를 보여주는 이미지](../../../images/extensions/server/snap/optional_fields.png)
...
이제 이러한 변경 사항을 속성에 게시할 수 있습니다. 자세한 내용은 [게시 흐름](https://experienceleague.adobe.com/en/docs/experience-platform/tags/publish/overview)에 대한 설명서를 참조하십시오.
