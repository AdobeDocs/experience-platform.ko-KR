...
>[!NOTE]
>
>Adobe Experience Platform Launch은 Adobe Experience Platform의 데이터 수집 기술군으로 새롭게 브랜딩되었습니다. 그 결과로 제품 설명서 전반에서 몇 가지 용어 변경이 있었습니다. 용어 변경에 대한 통합 참조는 [기간 업데이트](../../term-updates.md) 문서.
...
Adobe Experience Platform의 태그는 웹 사이트에서 스크립트를 동적으로 로드하도록 설계된 태그 관리 시스템입니다. TLS는 Adobe 호스트 간의 통신을 보호합니다 `assets.adobedtm.com` 그리고 이러한 스크립트가 로드될 때 웹 사이트를 방문합니다.
...
다음과 같은 경우 [자체 호스팅](../publishing/hosts/self-hosting-libraries.md) 그러면 라이브러리에서 지원되는 TLS 버전이 자체 호스팅 서비스에 의해 결정됩니다.

## 제거할 TLS 암호 2024년 5월 1일
