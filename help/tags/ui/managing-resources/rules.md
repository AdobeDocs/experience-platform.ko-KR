...
1. 규칙 이름을 지정합니다.
1. 이벤트 **[!UICONTROL Add]** 아이콘을 선택합니다.
1. 확장을 선택하고 해당 확장에 사용할 수 있는 이벤트 유형 중 하나를 선택한 다음 이벤트에 대한 설정을 구성합니다.

   ![규칙 이벤트 구성 페이지입니다.](../../images/rule-event-config.png)
...
1. 순서 매개 변수를 설정한 다음 **[!UICONTROL Keep Changes]**&#x200B;를 선택합니다.
...
1. Conditions **[!UICONTROL Add]** 아이콘을 선택한 다음 논리 유형, 확장, 조건 유형을 선택하고 조건에 대한 설정을 구성합니다. 그 다음 **[!UICONTROL Keep Changes]**&#x200B;를 선택합니다.
...
1. 작업 **[!UICONTROL Add]** 아이콘을 선택한 다음, 확장 및 해당 확장에 사용할 수 있는 작업 유형 중 하나를 선택하고, 작업에 대한 설정을 구성한 다음 **[!UICONTROL Keep Changes]**&#x200B;을(를) 선택합니다.

   ![규칙 작업 구성 페이지입니다.](../../images/action-settings.png)

   사용 가능한 작업 유형은 선택한 확장에 따라 다릅니다. 작업 설정은 작업 유형에 따라 다릅니다.

   (고급) 다음 작업 실행 대기: 이 옵션은 속성에 규칙 구성 요소 순서가 활성화되어 있을 때 사용할 수 있습니다. 이 옵션을 선택하면 이 작업이 완료될 때까지 태그가 다음 작업을 호출하지 않습니다. 이 옵션을 선택하지 않으면 다음 작업이 즉시 실행되기 시작합니다. 기본값은 **[!UICONTROL Checked]**&#x200B;입니다.
...
1. 규칙을 검토하고 **[!UICONTROL Save Rule]**&#x200B;를 선택합니다.
...
이러한 이벤트는 거의 항상(조건이 false로 평가되지 않는 경우) 실행해야 하므로 효율성을 위해 임베드 코드에서 참조하는 파일인 기본 라이브러리에 번들로 제공됩니다.

* **Javascript:** JavaScript이 기본 태그 라이브러리에 포함되어 있습니다. 사용자 지정 스크립트는 스크립트 태그에 래핑되고 `document.write`를 사용하여 문서에 작성됩니다. 규칙에 사용자 지정 스크립트가 여러 개 있는 경우 순서대로 작성됩니다.
...
런타임 환경의 동작은 속성에 대해 **[!UICONTROL Run rule components in sequence]**&#x200B;이(가) 설정되었는지 여부에 따라 다릅니다. 이 설정은 규칙 구성 요소를 동시에(비동기적으로) 평가할 수 있는지 또는 순차적으로 평가해야 하는지 여부를 결정합니다.
