...
1. 확장을 선택하고 해당 확장에 사용할 수 있는 이벤트 유형 중 하나를 선택한 다음, 이벤트에 대한 속성을 구성합니다.
   ...
   >[!IMPORTANT]
   >
   >클라이언트측 규칙에서 데이터 요소는 데이터 요소 이름의 시작 및 끝에 `%`으로 토큰화됩니다. 예, `%viewportHeight%`. 이벤트 전달 규칙에서 데이터 요소는 `{{` 시작 및 `}}` at the end of the data element name. 예: `{{viewportHeight}}`.
...
1. 순서 매개 변수를 설정한 다음 **[!UICONTROL 변경 내용 유지]**.
...
1. 조건 선택 **[!UICONTROL 추가]** 아이콘을 선택한 다음 논리 유형, 확장, 조건 유형을 선택하고 조건에 대한 설정을 구성합니다. 다음 을 선택합니다. **[!UICONTROL 변경 내용 유지]**.
...
1. 작업 선택 **[!UICONTROL 추가]** 아이콘을 클릭하고, 확장 및 해당 확장에 사용할 수 있는 작업 유형 중 하나를 선택하고, 작업에 대한 설정을 구성한 다음, 를 선택합니다 **[!UICONTROL 변경 내용 유지]**.
   ...
   (고급) Wait to run next action: 이 옵션은 속성에 규칙 구성 요소 순서가 활성화되어 있을 때 사용할 수 있습니다. 이 옵션을 선택하면 이 작업이 완료될 때까지 태그가 다음 작업을 호출하지 않습니다. 이 옵션을 선택하지 않으면 다음 작업이 바로 실행되기 시작합니다. 기본값은 입니다. **[!UICONTROL 선택됨]**.
...
1. 규칙을 검토하고 를 선택합니다 **[!UICONTROL 규칙 저장]**.
...
규칙 순서 지정을 사용하면 이벤트를 공유하는 규칙의 실행 순서를 제어할 수 있습니다. 각 규칙에는 순서 우선 순위를 결정하는 정수가 포함되어 있습니다(기본값은 50임). 주문에 대해 더 낮은 값을 포함하는 규칙은 값이 더 높은 규칙보다 먼저 실행됩니다.
...
>[!NOTE]
>
>궁극적으로, 작업을 순서대로 실행하는 책임은 사용 중인 이벤트 유형의 확장 개발자에게 있습니다. Adobe 확장 개발자는 확장이 의도한 대로 작동하는지 확인합니다. Adobe은 타사 확장 개발자에게 이를 제대로 수행하도록 지침을 제공하지만 이러한 지침이 어떻게 이행되는지 보장할 수 없습니다.

1과 100 사이의 양수로 규칙 순서를 지정하는 것이 좋습니다(기본값 50). 규칙 순서를 수동으로 유지 관리해야 하므로 가능한 한 간단하게 주문 구성을 유지하는 것이 좋습니다. 이 제한이 너무 제한되는 경계 사례가 있는 경우 태그는 +/- 2,147,483,648 사이의 규칙 순서 번호를 지원합니다.
...
## 규칙 구성 요소 순서 지정 {#sequencing}

런타임 환경의 동작은 **[!UICONTROL 규칙 구성 요소를 순서대로 실행]** 이 속성에 대해 설정 또는 해제 상태입니다. 이 설정은 규칙의 구성 요소를 병렬로 평가할 수 있는지(비동기적으로) 또는 순서대로 평가해야 하는지 여부를 결정합니다.

>[!IMPORTANT]
>
>이 설정은 각 규칙 내에서 조건 및 작업을 평가하는 방법만 결정하고, 속성에서 규칙 자체가 실행되는 시퀀스에는 영향을 주지 않습니다. 의 이전 섹션을 참조하십시오. [규칙 순서 지정](#rule-ordering) 를 참조하십시오.
>
>in [이벤트 전달](../event-forwarding/overview.md) 속성, 규칙 작업은 항상 순차적으로 실행되며 이 설정은 사용할 수 없습니다. 규칙을 만들 때 순서가 올바른지 확인하십시오.

### 활성화됨

런타임 시 이벤트가 트리거될 때 이 설정이 활성화되면 규칙의 조건 및 작업이 처리 큐에 추가되고(정의한 순서에 따라) &quot;첫 번째 로그인, 첫 번째 종료&quot;(FIFO) 기준으로 한 번에 하나씩 처리됩니다. 이 규칙은 구성 요소가 완료될 때까지 기다렸다가 다음 구성 요소로 이동합니다.
