...
마크업에서 JavaScript 변수 또는 개체 속성을 수집하여 확장이나 규칙에서 사용하려는 경우 데이터 요소를 사용하여 이러한 값을 캡처할 수 있습니다. 그렇게 하면 규칙 전체에서 데이터 요소를 참조할 수 있으며 데이터 소스가 변경되더라도 데이터 수집 UI 내의 한 위치에서 소스(데이터 요소)에 대한 참조만 변경하면 됩니다.
...
이전에 다음 데이터 요소를 사용한 경우 데이터 수집 UI에서 사용자 지정 데이터 요소를 만들어야 합니다.
