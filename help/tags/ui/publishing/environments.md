...
라이브러리 빌드를 만들면 이를 환경에 지정해야 합니다. 그런 다음 빌드의 확장, 규칙 및 데이터 요소를 컴파일하여 지정된 환경에 배치합니다. 각 환경에서는 지정된 빌드를 사이트에 통합할 수 있는 고유한 포함 코드를 제공합니다.
...
태그는 각각 의 다른 상태에 해당하는 세 가지 다른 환경 유형을 지원합니다. [게시 작업 과정](./publishing-flow.md):
...
환경에 대한 설치 지침에 액세스하려면 다음으로 이동합니다. **[!UICONTROL 환경]** 속성을 탭한 다음 **[!UICONTROL 설치]** 아이콘 을 클릭하여 액세스합니다.
...
다음에서 [!UICONTROL 환경] 탭에서 나열된 환경을 선택하여 해당 구성 컨트롤을 표시합니다.
...
빌드가 만들어지면 해당 빌드가 할당된 호스트에 지정한 위치로 전달됩니다. 태그 호스트를 만들고 구성하는 방법에 대한 자세한 내용은 [호스트 개요](./hosts/hosts-overview.md).
...
포함 코드는 `<script>` 에 배치해야 하는 태그 `<head>` 코드를 로드하고 실행하기 위한 웹 사이트 페이지의 섹션입니다. 각 환경 구성은 자체 포함 코드를 자동으로 생성하므로 태그를 실행할 페이지의 사이트에 복사하여 붙여넣으면 됩니다.
...
다음에서 [!UICONTROL 환경] 속성의 탭에서 다음을 선택합니다. **[!UICONTROL 환경 추가]**.

![](./images/environments/create-new.png)

다음 화면에서 **[!UICONTROL 개발]** 옵션을 선택합니다.

![](./images/environments/create-development.png)

다음 화면에서는 새 환경의 이름을 지정하고, 호스트를 선택하고, 아카이브 설정을 선택할 수 있습니다. 완료되면 다음을 선택합니다. **[!UICONTROL 저장]** 을 클릭하여 환경을 만듭니다.
