...
Adobe 관리 호스트를 사용하는 라이브러리 빌드는 Adobe와 계약한 타사 컨텐츠 전달 네트워크(CDN)로 전달됩니다. 이러한 CDN은 Adobe과 독립적으로 작동하므로 Experience Platform이 유지 보수를 받고 있거나 다운되었을 경우 배포된 코드가 사이트 및 애플리케이션에서 정상적으로 계속 작동합니다. Adobe 관리 호스트의 임베드 코드는 CDN의 기본 라이브러리 파일을 참조하므로 클라이언트 장치가 런타임 시 이 파일을 검색할 수 있습니다.
...
Akamai에 호스팅된 파일의 도메인은 `assets.adobedtm.com`입니다. 이는 임베드된 `<script>` 코드 내에서 호출되는 방법에 따라 안전하게 참조되거나 그렇지 않을 수 있습니다(`http://` 또는 `https://`).
...
위의 테이블에서 알 수 있듯이 개발 및 스테이징 환경에서 브라우저 캐싱은 지원되지 않습니다. 따라서 높은 트래픽 또는 프로덕션 컨텍스트에서는 개발 또는 스테이징 임베드 코드를 사용할 수 없습니다.
...
>
>1. 속성에서 **[!UICONTROL Hosts]** 탭을 선택한 다음 **[!UICONTROL Add Host]**&#x200B;을 선택합니다.
>1. 호스트의 이름을 입력하고 호스트 유형으로 **[!UICONTROL Managed by Adobe]**&#x200B;을 선택한 다음 **[!UICONTROL Save]**&#x200B;을 선택합니다.
>
