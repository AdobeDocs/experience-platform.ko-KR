...
* 속성
* 데이터 세트
* 대상(계정에 대한 질문과 데이터 흐름에 대한 일부 질문은 현재 답변할 수 없음)
* 스키마(필드 그룹에 대한 질문에 지금은 답변할 수 없습니다.)
* 세그먼트
* 소스 (현재 계정에 대한 질문에 답변할 수 없습니다.)
...
* 이 세그먼트에 사용된 속성을 나열하시겠습니까?
* 총 몇 개의 세그먼트가 있습니까?
* 지난 달에 마지막으로 수정된 세그먼트 목록 표시
* 지난 주에 수정된 세그먼트는 무엇입니까?
* 다음에 대한 프로필 수는 얼마입니까? {SEGMENT_NAME} 세그먼트?
* 모든 중복 세그먼트를 나열합니다.
* 지난 7일 동안 생성되거나 업데이트된 세그먼트를 표시합니다.
* 세그먼트 간 프로필 수 분포는 어떻게 됩니까?
* 세분화에는 몇 개의 필드가 사용됩니까?
* 활성화된 세그먼트의 총 수는 얼마입니까?
* 어떤 세그먼트가 활성화됩니까?
* 얼마나 많은 중복 세그먼트가 활성화됩니까?
* 지난 해에 생성된 세그먼트를 나열합니다.
* 이전에 마지막으로 수정한 세그먼트 표시 {DATE}.
* 와(과) 연결된 고유한 세그먼트 이름 수 {SCHEMA_NAME} 스키마?
* 세그먼트 간에 가장 일반적으로 사용되는 스키마는 무엇입니까?
* 스키마가 몇 개나 있습니까?
* 사용 데이터 세트 {SCHEMA_NAME} 스키마?
* 지난 주에 수정된 모든 스키마를 나열합니다.
* 프로필이 활성화된 스키마는 몇 개입니까?
* 모든 경험 이벤트 클래스 스키마를 나열하시겠습니까?
* 수집할 데이터 세트 {SCHEMA_NAME} 스키마?
* 동일한 스키마를 사용하여 수집된 데이터 세트 수는 얼마입니까?
* 보유한 데이터 세트는 몇 개입니까?
* 각 세그먼트에서 사용되는 데이터 세트는 무엇입니까?
* 어떤 세그먼트를 사용합니까 {ATTRIBUTE_NAME} 속성?
* 스키마에 포함되는 항목 {ATTRIBUTE_NAME} 속성?
* 세그먼트에서 사용되지 않는 XDM 스키마 속성 수는 몇 개입니까?
* 데이터 세트 {ATTRIBUTE_NAME} XDM 필드가 채워졌습니까?
* 데이터가 있는 데이터 세트 {ATTRIBUTE_NAME} 속성?
