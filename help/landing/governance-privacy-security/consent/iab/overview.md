...
고객 동의 데이터는 스키마에 TCF 동의 필드가 포함된 데이터 세트로 전송해야 합니다. 이 안내서를 계속 진행하기 전에 [TCF 2.0 동의](./dataset.md)를 캡처하기 위한 데이터 세트 생성 자습서를 참조하십시오.
...
| [!UICONTROL 이벤트 데이터 세트] | [이전 단계](#datasets)에서 만든 [!DNL XDM ExperienceEvent] 데이터 세트를 선택합니다. |
| [!UICONTROL 프로필 데이터 세트] | [이전 단계](#datasets)에서 만든 [!DNL XDM Individual Profile] 데이터 세트를 선택합니다. |
