...
1. 페이지를 로드할 때마다(또는 CMP에서 동의 환경 설정이 변경될 때) 사이트의 사용자 지정 스크립트는 현재 환경 설정을 표준 XDM 개체에 매핑합니다. 그런 다음 이 개체가 Platform Web SDK `setConsent` 명령에 전달됩니다.
1. `setConsent`이(가) 호출되면 Platform Web SDK는 동의 값이 마지막으로 받은 값과 다른지 확인합니다. 값이 다르거나 이전 값이 없는 경우 구조화된 동의/환경 설정 데이터가 Adobe Experience Platform으로 전송됩니다.
...
Platform의 동의 처리 지원에 대한 현재 릴리스에서는 데이터 수집 권한(`collect.val`)만 Platform Web SDK에 의해 자동으로 적용됩니다. 보다 세분화된 동의와 환경 설정을 수집하여 고객 프로필에서 지속할 수 있지만, 이러한 추가 신호는 자체 다운스트림 프로세스에서 수동으로 적용되어야 합니다.
...
웹 사이트에서 동의 변경 이벤트를 수신하도록 CMP를 구성했으면 Experience Platform 웹 SDK를 통합하여 업데이트된 동의 설정을 수신하고 모든 페이지 로드 및 동의 변경 이벤트가 발생할 때마다 플랫폼으로 전송할 수 있습니다. 자세한 내용은 [고객 동의 데이터를 처리하기 위해 Web SDK를 구성](../sdk.md)하는 방법에 대한 안내서를 참조하십시오.
...
[동의 모바일 확장 구성](https://developer.adobe.com/client-sdks/documentation/consent-for-edge-network/) 및 [동의 API 사용](https://developer.adobe.com/client-sdks/documentation/consent-for-edge-network/api-reference/)에 대한 Mobile SDK 설명서를 참조하십시오. Mobile SDK를 사용하여 개인 정보 문제를 처리하는 방법에 대한 자세한 내용은 [개인 정보 및 GDPR](https://developer.adobe.com/client-sdks/resources/privacy-and-gdpr/) 섹션을 참조하십시오.
...
[XDM에 CSV 파일 매핑](../../../../ingestion/tutorials/map-csv/overview.md)에 대한 자습서를 따라 데이터 필드를 XDM으로 변환하고 플랫폼으로 수집하는 방법을 알아보십시오. 매핑을 위해 [!UICONTROL 대상]을(를) 선택할 때는 **[!UICONTROL 기존 데이터 세트 사용]** 옵션을 선택하고 이전에 만든 [!DNL Profile] 사용 동의 데이터 세트를 선택하십시오.
...
Experience Platform의 개인 정보 보호 관련 기능에 대한 자세한 내용은 [Platform의 거버넌스, 개인 정보 보호 및 보안에 대한 개요](../../overview.md)를 참조하십시오.
