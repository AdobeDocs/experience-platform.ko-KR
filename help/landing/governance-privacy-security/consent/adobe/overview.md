...
>[!NOTE]
>
>위에 언급된 XDM 동의 필드의 구조에 대한 자세한 내용은 [[!UICONTROL Consents and Preferences] 데이터 형식](/help/xdm/data-types/consents.md)에 대한 안내서를 참조하세요.
...
이 안내서를 계속하기 전에 [&#x200B; 사용 데이터 집합에 이러한 필수 필드를 추가하는 방법에 대한 자세한 단계는 &#x200B;](./dataset.md)동의 데이터를 캡처하기 위한 데이터 집합 구성[!DNL Profile]에 대한 자습서를 참조하십시오.
...
병합 정책으로 작업하는 방법에 대한 자세한 내용은 [병합 정책 개요](../../../../profile/merge-policies/overview.md)를 읽는 것부터 시작하십시오. 병합 정책을 설정할 때는 [!UICONTROL Consents and Preferences]데이터 세트 준비[에 대한 안내서에 설명된 대로 &#x200B;](./dataset.md) 스키마 필드 그룹에서 제공하는 모든 필수 동의 특성을 프로필에 포함해야 합니다.
...
[XDM에 CSV 파일 매핑](../../../../ingestion/tutorials/map-csv/overview.md)에 대한 자습서를 따라 데이터 필드를 XDM으로 변환하고 Experience Platform으로 수집하는 방법을 알아보십시오. 매핑을 위해 [!UICONTROL Destination]을(를) 선택할 때는 **[!UICONTROL Use existing dataset]** 옵션을 선택하고 이전에 만든 [!DNL Profile] 사용 동의 데이터 세트를 선택하십시오.
...
프로필의 세부 정보를 찾는 방법에 대한 특정 단계는 [&#x200B; UI 안내서의 &#x200B;](../../../../profile/ui/user-guide.md#browse)ID별 프로필 찾아보기[!DNL Profile] 섹션을 참조하십시오.

새 동의 속성은 기본적으로 프로필의 대시보드에 표시되지 않습니다. 따라서 프로필의 세부 정보 페이지에서 **[!UICONTROL Attributes]** 탭으로 이동하여 예상대로 수집되었는지 확인해야 합니다. 필요에 따라 대시보드를 사용자 지정하는 방법에 대해 알아보려면 [프로필 대시보드](../../../../profile/ui/profile-dashboard.md)의 안내서를 참조하십시오.
