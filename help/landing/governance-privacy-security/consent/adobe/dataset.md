...
`consents` 개체가 스키마 구조에 추가되었음을 나타내는 캔버스가 다시 나타납니다. 표준 필드 그룹으로 캡처되지 않은 추가 동의 및 기본 설정 필드가 필요한 경우 [사용자 지정 동의 및 기본 설정 필드를 스키마](#custom-consent)에 추가하는 부록 섹션을 참조하십시오. 그렇지 않으면 **[!UICONTROL 저장]**&#x200B;을 선택하여 스키마 변경 사항을 완료합니다.
