...
일반 조회 목적으로 사용할 뿐만 아니라 실시간 고객 프로필에 대해 조회 데이터 세트를 활성화할지 여부도 결정해야 합니다. 라이선스 한도를 초과하지 않도록 하려면 아래 지침을 따르십시오.

#### 조회 데이터 세트에 대한 프로필 지원 {#profile-enablement-lookup-datasets}

조회 데이터 세트는 응용 프로그램이 런타임 시 참조할 수 있도록 Experience Platform에서 활성화하는 데이터 세트입니다. 조회 데이터 세트를 사용하여 프로필 속성(예: 이름, 이메일 또는 충성도 계층)이나 경험 이벤트(예: 페이지 보기 수 또는 구매)를 제공하는 것이 주요 목적인 데이터 세트가 아니라 제품 세부 사항, 스토어 메타데이터 또는 오퍼 구성과 같은 상대적으로 정적이고 핵심적인 정보를 저장합니다.

[!DNL Journey Optimizer] 및 기타 의사 결정 애플리케이션과 같은 Experience Platform 애플리케이션은 이러한 데이터 세트를 사용하여 키(예: 제품 ID 또는 스토어 ID)를 기반으로 추가 필드를 검색하고 개인화, 의사 결정 및 오케스트레이션 워크플로우를 보강합니다. 실시간 고객 프로필에 대한 조회 데이터 세트를 활성화하면 프로필 데이터 볼륨에 영향을 주므로 다음 지침을 사용하여 라이선스 권한을 유지합니다.

조회 목적으로 데이터 세트를 구성할 때는 Experience Platform에서 데이터 세트가 수행할 수 있는 두 가지 역할을 고려하십시오.

* **데이터 집합 조회**: 응용 프로그램에서 [!DNL Journey Optimizer]의 개인화 및 의사 결정과 같은 서비스에 대한 참조 데이터를 검색할 수 있도록 허용합니다.
* **프로필 사용 데이터 세트**: 실시간 고객 프로필의 통합 고객 프로필에 특성 및 이벤트를 제공하십시오. 이러한 데이터 세트는 세분화 및 활성화 사용 사례에 해당 필드를 사용할 수 있도록 합니다.

>[!IMPORTANT]
>
>실시간 고객 프로필에서 해당 데이터 세트의 필드를 사용해야 하는 경우(예: 대상 정의, 활성화 또는 다중 엔티티 세그멘테이션의 경우)에만 실시간 고객 프로필에 대한 조회 데이터 세트를 활성화합니다. 실시간 고객 프로필에 대한 조회 데이터 세트를 활성화하면 프로필 데이터 볼륨이 증가합니다. 자세한 내용은 [다중 엔터티 세분화](../../segmentation/tutorials/multi-entity-segmentation.md)에 대한 자습서를 참조하십시오.

**실시간 고객 프로필에 데이터 세트를 사용하도록 설정하는 시기:**

다음과 같은 경우 실시간 고객 프로필에 대한 데이터 세트를 활성화합니다.

* 데이터 세트에는 고객 프로필로 통합해야 하는 고객 속성(예: 충성도 계층, 환경 설정, 계정 정보)이 포함되어 있습니다.
* 데이터 세트에는 고객 행동 분석 및 세그멘테이션에 기여하는 경험 이벤트가 포함되어 있습니다.
* 데이터 세트에는 다중 엔티티 세그멘테이션 또는 다운스트림 활성화를 포함하여 대상 정의에 사용해야 하는 참조 또는 데이터 보강 속성(예: 제품, 스토어 또는 계정 속성)이 포함되어 있습니다.

**실시간 고객 프로필에 대한 데이터 세트를 활성화하지 않는 경우:**

다음과 같은 경우 실시간 고객 프로필에 대해 데이터 세트를 활성화하지 마십시오.

* 데이터 세트에는 제품 카탈로그, SKU 세부 정보, 스토어 위치 또는 기타 비고객 데이터와 같은 참조 데이터가 포함되어 있으며 다중 엔티티 세그먼테이션을 비롯한 세분화 또는 활성화를 위해 실시간 고객 프로필에 이러한 속성이 필요하지 않습니다.
* 데이터 세트에는 런타임 시 조회에만 사용되고 고객 ID의 일부 또는 대상 정의로 필요하지 않은 데이터 보강 데이터가 포함되어 있습니다.
