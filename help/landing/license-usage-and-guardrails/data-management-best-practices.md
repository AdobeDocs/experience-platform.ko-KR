---
keywords: Experience Platform;홈;인기 항목;데이터 관리;라이선스 권한;라이선스;우수 사례
...
* 특정 시점에 Adobe Real-time Customer Data Platform(즉, Profile Service 및 Identity Service) 내에 저장된 모든 프로덕션 데이터의 합을 [!DNL Addressable Audience];
* 플랫폼 내에 저장된 모든 데이터의 합계(다음을 포함하지만 이에 국한되지 않음) [!DNL Data Lake], Profile Service 및 Identity Service) 를 기준으로 지난 12개월 동안 플랫폼으로 스트리밍한 모든 데이터(내부에 저장하지 않고)를 [!DNL Addressable Audience].
...
플랫폼의 하나 또는 여러 시스템으로 데이터를 수집할 수 있습니다. 즉, [!DNL Data Lake] 및/또는 프로필 스토어 이것은 다양한 다른 사용 사례에 대해 두 시스템 모두에 다른 데이터가 존재할 수 있음을 의미한다. 예를 들어 내역 데이터를 [!DNL Data Lake], 프로필 스토어에서는 사용할 수 없습니다. 프로필 수집을 위한 데이터 세트를 활성화하여 프로필 스토어에 전송할 데이터를 선택할 수 있습니다.
...
| 수집 필터 | 설명 |
| --- | --- |
| Adobe Audience Manager 소스 필터링 | Adobe Audience Manager 소스 연결을 만들 때 로 가져올 세그먼트 및 트레이트를 선택하고 선택할 수 있습니다. [!DNL Data Lake] Audience Manager 및 Profile Service를 사용하십시오. 다음 안내서를 참조하십시오 [Audience Manager 소스 연결 만들기](../../sources/tutorials/ui/create/adobe-applications/audience-manager.md) 추가 정보. |
...
| ID 네임스페이스(ID) | 고객 데이터가 합쳐지면에서 를 사용하여 단일 프로필로 병합됩니다. **id 네임스페이스**&#x200B;및 더 많은 정보가 사용자에 대해 알려짐에 따라 이러한 id를 함께 연결하는 기능이 제공됩니다. 다음을 참조하십시오. [id 네임스페이스 개요](../../identity-service/namespaces.md) 추가 정보. |
...
프로필 스토어의 구성을 이해하는 데 도움이 되는 다양한 보고서가 제공됩니다. 이러한 보고서는 라이선스 사용을 보다 잘 최적화하기 위해 경험 이벤트 만료를 설정하는 방법과 위치에 대해 정보에 입각한 결정을 내리는 데 도움이 됩니다.
...
* 구성 [경험 이벤트 만료](#event-expirations) 웹 데이터와 같은 고주파 데이터의 경우.
* 주기적으로 다음을 확인합니다. [프로필 구성 보고서](#profile-store-composition-reports) 를 사용하여 프로필 저장소 구성을 이해할 수 있습니다. 이를 통해 라이선스 사용 소비에 가장 많이 기여하는 데이터 소스를 이해할 수 있습니다.
...
| 기능 | 설명 |
| --- | --- |
| [프로필에 대한 데이터 세트 활성화/비활성화](../../catalog/datasets/user-guide.md) | 프로필 서비스에 데이터 세트 수집 활성화 또는 비활성화 |
| [경험 이벤트 만료](../../profile/event-expirations.md) | 프로필 활성화 데이터 세트에 수집된 모든 이벤트에 만료 시간을 적용합니다. 이 기능을 활성화하려면 Adobe 지원 담당자에게 문의하십시오. |
