...
IMS 조직은 고객의 Adobe 표현입니다. 라이센스가 부여된 모든 Adobe 솔루션은 이 고객 조직과 통합됩니다. IMS 조직에 권한이 부여된 경우 [!DNL Experience Platform], 개발자에게 액세스 권한을 할당할 수 있습니다. IMS 조직 ID(`x-gw-ims-org-id`)는 API 호출을 실행해야 하는 조직을 나타내며 모든 API 요청에서 헤더로 필요합니다. 이 ID는 [Adobe Developer 콘솔](https://www.adobe.com/go/devs_console_ui): **통합** 탭에서 다음 위치로 이동 **개요** 섹션에서 ID를 찾기 위한 특정 통합에 대한 섹션 **클라이언트 자격 증명**. 에서 인증하는 방법에 대한 단계별 연습 [!DNL Platform], 다음을 참조하십시오. [인증 자습서](https://www.adobe.com/go/platform-api-authentication-en).
...
모든 API 호출의 인증 헤더에 액세스 토큰이 필요합니다. 다음을 사용하여 생성할 수 있습니다. `curl` ims 조직의 통합에 액세스할 수 있는 경우 명령. 액세스 토큰은 24시간 동안만 유효하며, 그 이후에는 API를 계속 사용하려면 새 토큰을 생성해야 합니다. 액세스 토큰 생성에 대한 자세한 내용은 [인증 자습서](https://www.adobe.com/go/platform-api-authentication-en).
...
이 오류 메시지는 IMS 조직 헤더(`x-gw-ims-org-id`)이 API 요청에서 누락되었습니다. 헤더가 IMS 조직의 ID에 포함되어 있는지 확인한 후 다시 시도하십시오.
...
이 오류 메시지는 사용자 또는 Adobe I/O 통합( [액세스 토큰](#how-do-i-get-an-access-token) 다음에서 `Authorization` 헤더)는 (으)로 호출할 권한이 없습니다. [!DNL Experience Platform] 에 제공된 IMS 조직의 API `x-gw-ims-org-id` 머리글입니다. 헤더에서 IMS 조직에 대한 올바른 ID를 입력했는지 확인한 후 다시 시도하십시오. 조직 ID를 모를 경우 [Adobe I/O 콘솔](https://console.adobe.io): **통합** 탭에서 다음 위치로 이동 **개요** 섹션에서 ID를 찾을 수 있습니다. **클라이언트 자격 증명**.
...
- IMS 조직 헤더가 잘못되거나 잘못된 경우(`x-gw-ims-org-id`)가 API 요청에서 전달됩니다. IMS 조직의 올바른 ID가 포함되어 있는지 확인한 후 다시 시도하십시오.
...
| Adobe Experience Platform 데이터 수집 | [[!DNL Data Ingestion API]](https://www.adobe.io/experience-platform-apis/references/data-ingestion/) | [일괄 처리 수집 문제 해결 안내서](../ingestion/batch-ingestion/troubleshooting.md)<br><br>[스트리밍 수집 문제 해결 안내서](../ingestion/streaming-ingestion/troubleshooting.md) |
