...
다음 **[!UICONTROL 데이터 세트]** 섹션에는 IMS 조직 내의 데이터 세트 수가 표시됩니다. 이 번호는 새 데이터 세트를 만들 때 업데이트됩니다. 데이터 세트에 대한 자세한 내용은 [데이터 세트 개요](../catalog/datasets/overview.md).

다음 **[!UICONTROL 프로필]** 섹션에는 IMS 조직 내에서 프로필 조각을 제외한 프로필 사용자의 총 수가 표시됩니다. 이 총 인원 수는 총 대응 가능 대상을 나타내며 24시간마다 한 번씩 업데이트됩니다. 프로필에 대한 자세한 내용은 [실시간 고객 프로필 개요](../profile/home.md).

다음 **[!UICONTROL 세그먼트]** 섹션에는 IMS 조직 내에서 생성된 총 세그먼트 수가 표시됩니다. 이 번호는 새 세그먼트를 만들 때 업데이트됩니다. 세그먼트에 대한 자세한 내용은 [세그먼테이션 서비스 개요](../segmentation/home.md).

다음 **[!UICONTROL 대상]** 섹션에는 IMS 조직에 대해 생성된 총 대상 수가 표시됩니다. 이 번호는 새 대상을 만들 때 업데이트됩니다. 대상에 대한 자세한 내용은 [대상 개요](../destinations/home.md).
...
다음 **[!UICONTROL 최근 데이터 세트]** 섹션에는 IMS 조직 내에서 가장 최근에 만들어진 5개의 데이터 세트가 나열됩니다. 이 목록은 새 데이터 세트가 생성될 때마다 업데이트됩니다. 목록에서 데이터 세트를 선택하여 지정된 데이터 세트에 대한 자세한 정보를 보거나 선택할 수 있습니다 **[!UICONTROL 모두 보기]** 생성된 모든 데이터 세트 목록을 봅니다. 데이터 세트에 대한 자세한 내용은 [데이터 세트 개요](../catalog/datasets/overview.md).

다음 **[!UICONTROL 최근 소스]** 섹션에는 IMS 조직 내에서 가장 최근에 만들어진 5개의 소스 커넥터가 나열됩니다. 이 목록은 새 소스 커넥터를 만들 때마다 업데이트됩니다. 목록에서 소스 연결을 선택하여 지정된 커넥터에 대한 자세한 정보를 보거나 를 선택할 수 있습니다 **[!UICONTROL 모두 보기]** 생성된 모든 소스 연결 목록을 확인합니다. 소스에 대한 자세한 내용은 [소스 개요](../sources/home.md).

다음 **[!UICONTROL 최근 세그먼트]** 섹션에는 IMS 조직 내에서 가장 최근에 만들어진 5개의 세그먼트 정의가 나열됩니다. 이 목록은 새 세그먼트 정의가 만들어질 때마다 업데이트됩니다. 목록에서 세그먼트 정의를 선택하여 지정된 세그먼트 정의에 대한 자세한 내용을 보거나 를 선택할 수 있습니다 **[!UICONTROL 모두 보기]** 생성된 모든 세그먼트 정의 목록을 봅니다. 세그먼트에 대한 자세한 내용은 [세그먼테이션 서비스 개요](../segmentation/home.md).

다음 **[!UICONTROL 최근 대상]** 섹션에는 IMS 조직 내에서 가장 최근에 만들어진 5개의 대상이 나열됩니다. 이 목록은 새 대상을 만들 때마다 업데이트됩니다. 목록에서 대상을 선택하여 지정된 대상에 대한 자세한 정보를 보거나 선택할 수 있습니다 **[!UICONTROL 모두 보기]** 생성된 모든 대상 목록을 봅니다. 대상에 대한 자세한 내용은 [대상 개요](../destinations/home.md).
...
Platform UI의 상단 탐색 표시줄에는 현재 로그인한 IMS 조직이 표시되며 몇 가지 중요한 컨트롤이 제공됩니다.
...
상단 탐색 막대의 오른쪽에 있는 첫 번째 항목은 입니다. **IMS 조직 전환기**.

![](./images/user-guide/homepage-ims-org-switcher.png)

전환기를 선택하면 액세스 권한이 있는 IMS 조직 드롭다운 메뉴가 열립니다(사용 가능한 경우). 다른 IMS 조직으로 전환하려면 나열된 옵션을 선택합니다.

### 애플리케이션 전환

위쪽 탐색의 오른쪽에 있는 다음 항목은 입니다. **응용 프로그램 전환기**, 로 표시 ![응용 프로그램 전환기](./images/user-guide/app-switcher-icon.png) 아이콘. 이 아이콘을 선택하면 IMS 조직이 액세스할 수 있는 Adobe 애플리케이션(예: Experience Platform, Analytics, 에셋 등) 간에 전환할 수 있습니다.
