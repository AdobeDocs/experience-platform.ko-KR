...
**[!UICONTROL 최근 데이터 세트]** 섹션에는 조직 내에서 가장 최근에 만들어진 5개의 데이터 세트가 나열됩니다. 이 목록은 새 데이터 세트가 생성될 때마다 업데이트됩니다. 목록에서 데이터 세트를 선택하여 볼 수 있습니다. 지정된 데이터 세트에 대한 자세한 정보를 찾거나 **[!UICONTROL 모두 보기]**&#x200B;를 선택하여 만든 모든 데이터 세트의 목록을 볼 수 있습니다. [데이터 세트 개요](../catalog/datasets/overview.md)에서 데이터 세트에 대한 자세한 정보를 찾을 수 있습니다.
...
Experience Platform UI의 상단 탐색 모음에는 현재 로그인한 조직이 표시되며 몇 가지 중요한 컨트롤이 제공됩니다.

탐색 막대의 왼쪽에 Adobe Experience Platform 로고가 있습니다. 언제든지 이 로고를 선택하면 Experience Platform UI 홈 화면으로 돌아갑니다.
...
응용 프로그램 전환기 오른쪽에는 ![물음표/도움말](/help/images/icons/help.png) 아이콘으로 표시되는 **도움말 및 지원 메뉴**&#x200B;가 있습니다. 이 아이콘을 선택하면 몇 가지 도움말 및 지원 리소스가 포함된 팝오버 메뉴가 나타납니다. **[!UICONTROL 도움말]** 탭에는 현재 사용 중인 페이지에 대한 관련 문서 목록이 표시됩니다. **[!UICONTROL 지원]** 탭에서 Adobe 지원 팀과 함께 지원 티켓을 만들 수 있습니다. **[!UICONTROL 피드백]** 탭에서 Experience Platform에 대한 피드백을 Adobe에 제출할 수 있습니다.
...
이름과 이메일 바로 아래에 위치한 스위치를 사용하여 Experience Platform 인터페이스의 밝은 테마와 어두운 테마 사이를 전환할 수 있습니다. 원하는 테마를 선택합니다.

![](images/theme.png)

### 샌드박스

상단 탐색 모음 바로 아래에 샌드박스 막대가 있습니다. 이 표시줄에는 현재 Experience Platform에 사용 중인 샌드박스가 표시됩니다. 샌드박스에 대한 자세한 내용은 [샌드박스 개요](../sandboxes/home.md)를 참조하세요.

## 왼쪽 탐색 {#left-nav}

화면 왼쪽의 탐색에는 Experience Platform UI에서 지원되는 다양한 서비스가 모두 나열됩니다.
...
**[!UICONTROL 홈]** 섹션을 사용하면 Experience Platform UI 홈 페이지로 돌아갈 수 있습니다.

**[!UICONTROL 워크플로]** 섹션에는 Experience Platform 내에서 작업을 수행하기 위한 여러 단계 워크플로 목록이 표시됩니다. 워크플로우에 대한 자세한 내용은 [워크플로우 개요](./workflows.md)를 참조하세요.

### [!UICONTROL 연결]

**[!UICONTROL 소스]** 섹션을 사용하면 소스 연결을 만들고 업데이트하고 삭제할 수 있으므로 외부 소스의 데이터를 Experience Platform으로 수집할 수 있습니다. 소스에 대한 자세한 내용은 [소스 개요](../sources/home.md)를 참조하세요.

**[!UICONTROL 대상]** 섹션을 사용하면 대상을 만들고, 업데이트하고, 삭제할 수 있으므로 Experience Platform에서 여러 외부 대상으로 데이터를 내보낼 수 있습니다. 대상에 대한 자세한 내용은 [대상 개요](../destinations/home.md)를 참조하세요.

### [!UICONTROL 고객]

**[!UICONTROL 프로필]** 섹션을 통해 고객 프로필을 찾아보고, 프로필 지표를 보고, 병합 정책을 만들고 관리하고, 결합 스키마를 볼 수 있습니다. [!UICONTROL 프로필] 섹션 사용에 대한 자세한 내용은 [[!DNL Profile] 사용 안내서](../profile/ui/user-guide.md)를 참조하십시오. 실시간 고객 프로필에 대한 자세한 내용은 [실시간 고객 프로필 개요](../profile/home.md)를 참조하세요.
...
**[!UICONTROL ID]** 섹션에서 ID 네임스페이스를 만들고 관리할 수 있습니다. ID 네임스페이스 및 Experience Platform UI에서 ID를 사용하는 방법에 대한 정보를 포함하여 [!UICONTROL ID] 섹션에 대한 자세한 내용은 [ID 네임스페이스 개요](../identity-service/features/namespaces.md)를 참조하십시오.
...
Experience Platform UI(사용자 인터페이스)는 일별 스냅샷 중에 캡처한 대로 조직의 라이선스 사용에 대한 중요한 정보를 볼 수 있는 대시보드를 제공합니다. 탐색에서 **[!UICONTROL 라이선스 사용량]**&#x200B;을 선택하여 이 대시보드에 액세스합니다. 라이선스 사용 대시보드에 대한 자세한 내용은 [라이선스 사용 대시보드 안내서](./license-usage-and-guardrails/license-usage-dashboard.md)를 참조하세요.
...
이 안내서를 읽으면 이제 Experience Platform UI의 홈 페이지 및 주요 탐색 요소에 대해 소개합니다. 사용자 인터페이스 작업에 대한 자세한 내용은 각 개별 Experience Platform 서비스에 대한 설명서를 참조하십시오. 이 설명서에 대한 링크는 이 문서의 앞부분에 있는 [왼쪽 탐색](#left-nav) 섹션에 제공됩니다.
