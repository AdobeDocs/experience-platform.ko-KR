...
Adobe Experience Platform은 [!DNL Platform] 서비스를 사용하여 들어오는 데이터를 구조화, 레이블 지정 및 향상시키는 기능을 제공하면서 외부 소스에서 데이터를 수집할 수 있도록 허용합니다. Adobe 애플리케이션, 클라우드 기반 스토리지, 데이터베이스 등 다양한 소스의 데이터를 인제스트할 수 있습니다. 또한 Experience Platform을 통해 외부 파트너가 데이터를 활성화할 수 있습니다.

[!DNL Flow Service] 는 Adobe Experience Platform 내의 다양한 소스에서 수집된 고객 데이터를 수집하고 중앙에서 관리하는 데 사용됩니다. 이 서비스는 지원되는 모든 소스 및 대상을 연결할 수 있는 사용자 인터페이스 및 RESTful API를 제공합니다.
...
이 자습서에서는 올바른 데이터 흐름 ID 값을 필요로 합니다. 유효한 데이터 흐름 ID가 없는 경우 [소스 개요](../../sources/home.md) 또는 [대상 개요에서](../../destinations/catalog/overview.md) 선택한 커넥터를 선택하고 이 튜토리얼을 시작하기 전에 설명한 단계를 따르십시오.
...
데이터 흐름을 만들었으면 API에 GET 요청을 [!DNL Flow Service] 수행합니다.
...
| 매개 변수 | 설명 |
| --------- | ----------- |
| `{FLOW_ID}` | 모니터링할 데이터 프롤에 대한 고유한 `id` 값입니다. |
...
이 튜토리얼을 따라 [!DNL Flow Service] API를 사용하여 데이터 흐름에 대한 지표 및 오류 정보를 검색했습니다. 이제 통합 일정에 따라 데이터 흐름을 계속 모니터링하여 상태 및 섭취 비율을 추적할 수 있습니다. 소스의 데이터 흐름 모니터링 방법에 대한 자세한 내용은 사용자 인터페이스 [자습서를 사용하여 소스의 데이터 흐름](../ui/monitor-sources.md) 모니터링을 참조하십시오. 대상에 대한 데이터 흐름 모니터링 방법에 대한 자세한 내용은 사용자 인터페이스 [자습서를 사용하여 대상에 대한 데이터 흐름](../ui/monitor-destinations.md) 모니터링을 참조하십시오.