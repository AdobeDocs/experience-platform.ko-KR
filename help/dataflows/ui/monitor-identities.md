...
모니터링 대시보드는 데이터의 ID 상태를 포함하여 ID 내에서 데이터의 활동을 시각적으로 표시합니다. 이 자습서에서는 Experience Platform 사용자 인터페이스를 사용하여 모니터링 대시보드를 사용하여 데이터의 ID를 모니터링하여 ID 처리 상태를 추적할 수 있는 방법에 대한 지침을 제공합니다.
...
**[!UICONTROL Identities]** 대시보드에 액세스하려면 왼쪽 탐색에서 **[!UICONTROL Monitoring]**&#x200B;을(를) 선택하십시오. **[!UICONTROL Monitoring]** 페이지에서 **[!UICONTROL Identities]** 카드를 선택하십시오.

![ID 카드입니다. 받은 레코드 수, 수집한 레코드 수 및 성공률에 대한 정보가 표시됩니다.](../assets/ui/monitor-identities/focus-card.png)

기본 **[!UICONTROL Identities]** 대시보드에서 **[!UICONTROL Identities]** 카드에 받은 총 레코드 수, 수집된 레코드 수 및 레코드 수집 성공률에 대한 정보가 표시됩니다.
...
[!UICONTROL Identity processing] 페이지에는 추가된 ID 수, 만들어진 그래프 및 업데이트된 그래프를 포함하여 [!DNL Identity Service]에 수집된 레코드에 대한 정보가 포함되어 있습니다.
...
| **[!UICONTROL Records failed]** | 데이터 오류로 인해 Experience Platform에 수집되지 않은 레코드 수입니다. |
| **[!UICONTROL Records skipped]** | 레코드 행에 식별자가 하나만 있으므로 [!DNL Identity Service]에 수집되지 않고 수집된 레코드 수입니다. |
| **[!UICONTROL Records ingested]** | [!DNL Identity Service]에 수집된 레코드 수입니다. |
| **[!UICONTROL Identities added]** | [!DNL Identity Service]에 추가된 순 새 식별자 수입니다. |
| **[!UICONTROL Graphs created]** | [!DNL Identity Service]에 만들어진 순 새 ID 그래프 수입니다. |
| **[!UICONTROL Graphs updated]** | 새 에지로 업데이트된 기존 ID 그래프 수입니다. |
| **[!UICONTROL Total failed dataflows]** | 실패한 데이터 흐름 실행 수입니다. |
...
또는 토글을 통해 **[!UICONTROL Dataflows]**&#x200B;을(를) 선택하여 지난 24시간 동안 조직의 데이터 흐름에 대한 ID 처리 세부 정보를 볼 수 있습니다.
...
| **[!UICONTROL Records skipped]** | 레코드 행에 식별자가 하나만 있으므로 [!DNL Identity Service]에 수집되지 않고 수집된 레코드 수입니다. |
| **[!UICONTROL Records ingested]** | [!DNL Identity Service]에 수집된 레코드 수입니다. |
| **[!UICONTROL Total records]** | 실패한 레코드, 건너뛴 레코드, 추가된 ID 및 복제된 레코드를 포함한 모든 레코드의 총 개수입니다. |
| **[!UICONTROL Identities added]** | [!DNL Identity Service]에 추가된 순 새 식별자 수입니다. |
| **[!UICONTROL Graphs created]** | [!DNL Identity Service]에 만들어진 순 새 ID 그래프 수입니다. |
| **[!UICONTROL Graphs updated]** | 새 에지로 업데이트된 기존 ID 그래프 수입니다. |
| **[!UICONTROL Total failed dataflows]** | 실패한 데이터 흐름 실행 수입니다. |

![&#x200B; 데이터 흐름 실행에 대한 자세한 내용을 보려면 데이터 흐름 실행 시작 시간 옆에 있는 필터 아이콘 &#x200B;](/help/images/icons/filter.png)filter[!DNL Identity]을(를) 선택하십시오.

![필터 아이콘이 강조 표시되어 있습니다. 이 아이콘을 선택하면 선택한 데이터 흐름에 대한 세부 정보를 볼 수 있습니다.](../assets/ui/monitor-identities/dataflows-filter.png)

[!UICONTROL Dataflow run details] 페이지에는 조직 ID 및 데이터 흐름 실행 ID를 포함하여 [!DNL Identity] 데이터 흐름 실행에 대한 자세한 정보가 표시됩니다. 수집 프로세스에서 오류가 발생하는 경우 해당 오류 코드와 [!DNL Identity Service]에서 제공한 오류 메시지도 이 페이지에 표시됩니다.
...
| **[!UICONTROL Graphs created]** | [!DNL Identity Service]에 만들어진 순 새 ID 그래프 수입니다. |
| **[!UICONTROL Graphs updated]** | 새 에지로 업데이트된 기존 ID 그래프 수입니다. |
| **[!UICONTROL Status]** | 데이터 흐름의 전체 상태를 정의합니다. 가능한 상태 값은 다음과 같습니다. <ul><li>`Success`: 데이터 흐름이 활성 상태이며 제공된 일정에 따라 데이터를 수집 중임을 나타냅니다.</li><li>`Failed`: 오류로 인해 데이터 흐름의 활성화 프로세스가 중단되었음을 나타냅니다. </li><li>`Processing`: 데이터 흐름이 아직 활성화되지 않았음을 나타냅니다. 이 상태는 종종 새 데이터 흐름이 생성된 직후에 발생합니다.</li></ul> |
| **[!UICONTROL Dataflow run start]** | 데이터 흐름이 실행된 날짜 및 시간입니다. |
| **[!UICONTROL Last updated]** | 데이터 흐름이 마지막으로 업데이트된 날짜 및 시간입니다. |
| **[!UICONTROL Error summary]** | 데이터 흐름 실행이 실패한 경우 데이터 흐름 실행이 실패한 이유에 대한 오류 코드와 요약이 표시됩니다. |
| **[!UICONTROL Dataflow run ID]** | 데이터 흐름이 실행되는 ID입니다. |
| **[!UICONTROL IMS org ID]** | 데이터 흐름 실행이 속한 조직 ID입니다. |
