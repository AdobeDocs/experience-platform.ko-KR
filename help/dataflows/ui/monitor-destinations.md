...
- **[!UICONTROL ID 실패]** 오류로 인해 대상에 활성화되지 않은 총 프로필 ID 수입니다.
- **[!UICONTROL 활성화 속도]**: 활성화 또는 건너뛴 수신 ID의 비율입니다. 다음 수식은 이 값이 계산되는 방법을 보여 줍니다.
...
고객, 계정(Adobe Real-Time CDP B2B 에디션만 해당), 잠재 고객 및 계정 보강과 같은 데이터 유형별로 표시된 정보를 필터링할 수 있습니다. [모니터링 대시보드 가이드](/help/dataflows/ui/monitor.md#monitoring-dashboard-overview)에서 이러한 옵션에 대해 자세히 알아보세요.
