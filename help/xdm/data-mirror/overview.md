---
keywords: Experience Platform;데이터 미러;관계형 스키마;데이터 캡처 변경;데이터베이스 동기화;기본 키;관계
...
Data Mirror을 사용하여 관계형 스키마를 구현할 때의 삭제 및 데이터 위생 요구 사항을 계획합니다. 모든 애플리케이션은 배포 전에 삭제가 관련 데이터 세트, 규정 준수 워크플로우 및 다운스트림 프로세스에 미치는 영향을 고려해야 합니다.
...
* [standard](../schema/composition.md)와(과) [관계형 스키마](../schema/relational.md) 구분
* [설명자와 구조적 관계 정의](../api/descriptors.md)

### 구현 요구 사항

Data Mirror이 제대로 작동하려면 플랫폼 인스턴스 및 소스 데이터가 특정 요구 사항을 충족해야 합니다. Data Mirror에는 강제 제약 조건이 있는 유연한 데이터 구조인 **관계형 스키마**&#x200B;가 필요합니다.
...
필수 설명자(스키마 동작 및 제약 조건을 정의하는 메타데이터)를 사용하여 [관계형 스키마](../schema/relational.md)를 만듭니다. UI를 통해 또는 API를 통해 직접 팀의 워크플로에 맞는 방법을 선택하십시오.

* **UI 접근 방식**: [스키마 편집기에서 관계형 스키마 만들기](../ui/resources/schemas.md#create-relational-schema)
* **API 접근 방식**: [스키마 레지스트리 API를 통해 스키마 만들기](../api/schemas.md#create-relational-schema)
...
* **레코드 위생**: [관계형 스키마를 기준으로 데이터 세트에 대한 전체 자릿수 레코드 삭제 관리](../../hygiene/ui/record-delete.md#relational-record-delete)
...
Data Mirror의 [관계형 스키마](../schema/relational.md)를 사용하여 엔터티를 표시하고, 행 수준에서 삽입, 업데이트 및 삭제를 처리하고, 데이터 원본에 있는 기본 및 외래 키 관계를 유지할 수 있습니다. 이 접근 방식은 관계형 데이터 모델링 원칙을 Experience Platform에 도입하고 데이터 세트 간에 구조적 일관성을 보장합니다.
...
관계형 스키마 및 Data Mirror을 사용하는 모든 애플리케이션은 데이터 삭제 의미를 이해해야 합니다. 관계형 스키마를 사용하면 연결된 데이터 세트 간에 관련 데이터에 영향을 줄 수 있는 정확한 레코드 수준 삭제가 가능합니다. 이러한 삭제 기능은 특정 사용 사례에 관계없이 데이터 무결성, 규정 준수 및 다운스트림 애플리케이션 동작에 영향을 줍니다. 관계형 스키마를 기반으로 하는 데이터 세트에 대한 [데이터 위생 요구 사항](../../hygiene/ui/record-delete.md#relational-record-delete)을 검토하고 구현 전에 삭제 시나리오를 계획하십시오.

### 스키마 동작 선택

관계형 스키마는 기본적으로 엔터티 상태(고객, 계정 등)를 캡처하는 **레코드 동작**&#x200B;입니다. 이벤트 추적에 **시계열 동작**&#x200B;이 필요한 경우 명시적으로 구성해야 합니다.
...
2. **비즈니스 이해 당사자**&#x200B;는 라이선스에 관계형 스키마 지원 및 필요한 Experience Platform 버전이 포함되어 있는지 확인해야 합니다.
...
구현 세부 정보는 [관계형 스키마 설명서](../schema/relational.md)를 참조하십시오.
