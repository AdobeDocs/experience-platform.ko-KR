...
description: 이 문서에서는 구독 XDM 데이터 유형을 사용하는 일반 마케팅 환경 설정 필드에 대한 개요를 제공합니다.
...
# [!UICONTROL 구독이 있는 일반 마케팅 환경 설정 필드] 데이터 유형

[!UICONTROL 구독이 있는 일반 마케팅 환경 설정 필드] 는 특정 마케팅 환경 설정에 대한 고객의 선택을 설명하는 표준 XDM 데이터 유형입니다.
...
>
>이 필요하지 않은 경우 `subscriptions` 특정 마케팅 환경 설정 필드에 대한 맵에서 [기본 마케팅 필드 데이터 유형](./marketing-field.md) 대신,
...
| `subscriptions` | 맵 | 특정 구독에 대한 고객 마케팅 환경 설정 맵입니다. 의 섹션을 참조하십시오. [구독](#subscriptions) 추가 정보. |
| `time` | DateTime | 마케팅 환경 설정이 변경된 경우의 ISO 8601 타임스탬프(해당하는 경우). |
| `val` | 문자열 | 이 마케팅 사용 사례에 대해 고객이 제공한 환경 설정 선택. 다음을 참조하십시오. [다음 섹션](#val) 허용되는 값 및 정의용. |
...
다음 JSON은 가 포함된 전화 통화 마케팅 채널에 대한 예제 마케팅 필드를 나타냅니다. `subscriptions` 맵. 의 각 키 `subscriptions` 객체는 마케팅 채널에 대한 개별 구독을 나타냅니다. 각 구독에는 차례로 옵트인 값(`val`).
...
| `subscribers` | 특정 구독을 구독한 식별자 세트(예: 이메일 주소 또는 전화번호)를 나타내는 선택적 맵 유형 필드입니다. 이 개체의 각 키는 해당 식별자를 나타내며 두 개의 하위 속성을 포함합니다. <ul><li>`time`: ID를 구독한 경우의 ISO 8601 타임스탬프(해당하는 경우).</li><li>`source`: 구독자가 시작된 소스. 15자 이하인 경우 모든 설명 문자열이 될 수 있습니다.</li></ul> |
