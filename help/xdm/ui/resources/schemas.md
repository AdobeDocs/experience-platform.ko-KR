...
버전 추적을 활성화하고 변경 데이터 캡처를 지원하려면 스키마에 버전 식별자 필드를 지정해야 합니다. 스키마 편집기에서 더하기(![A 더하기 아이콘)를 선택합니다.](/help/images/icons/plus.png)) 아이콘을 사용하여 스키마 이름 옆에 새 필드를 추가할 수 있습니다.
...
>[!IMPORTANT]
>
>테이블 구조에는 **기본 키** 및 **버전 식별자**&#x200B;이(가) 있어야 합니다(예: datetime 또는 number 유형의 `updateSequence` 필드).
>
...
그런 다음 필요에 따라 [추가 필드를 추가](#add-field-groups)하고 [스키마 수준 관계를 추가](../../tutorials/relationship-ui.md#relationship-field)합니다.

Experience Platform 소스에서 변경 데이터 캡처를 활성화하는 방법에 대한 지침은 [변경 데이터 캡처 수집 안내서](../../../sources/tutorials/api/change-data-capture.md)를 참조하십시오.
...
>[!NOTE]
>
>스키마 편집기 내에서 표준(Adobe에서 생성한) 클래스와 필드 그룹은 자물쇠 아이콘 ![자물쇠 아이콘으로 표시됩니다.](/help/images/icons/lock-closed.png) 구문을 사용하는 키-값 쌍으로 전달됩니다. 자물쇠는 클래스 또는 필드 그룹 이름 옆의 왼쪽 레일과 시스템 생성 리소스의 일부인 스키마 다이어그램의 필드 옆에 나타납니다.
>
...
| 작업 | 현재 스키마에만 적용됩니다. | 필드 그룹 수정 | 다른 스키마에 영향 | 설명 |
|--------------------------|--------------------------------|----------------------|-----------------------|-------------|
| **필드 제거** | 아니요 | 예 | 예 | 필드 그룹에서 필드를 삭제합니다. 이렇게 하면 해당 그룹을 사용하는 모든 스키마에서 제거됩니다. |
| **관련 필드 관리** | 예 | 아니요 | 아니요 | 현재 스키마에서만 필드를 숨깁니다. 필드 그룹은 변경되지 않습니다. |
| **필드 사용 안 함** | 아니요 | 예 | 예 | 필드를 필드 그룹에서 더 이상 사용되지 않는 것으로 표시합니다. 스키마에서 더 이상 사용할 수 없습니다. |

>[!NOTE]
>
>이 동작은 레코드 기반 및 이벤트 기반 스키마 모두에서 일관됩니다.
...
필드에 대한 표시 이름 및 데이터 유형을 제공한 후 다음 단계는 상위 XDM 리소스에 필드를 할당하는 것입니다. 스키마에서 사용자 지정 클래스를 사용하는 경우 대신 [필드를 할당된 클래스에 추가](#add-to-class) 또는 [필드 그룹](#add-to-field-group)하도록 선택할 수 있습니다. 그러나 스키마에서 표준 클래스를 사용하는 경우 사용자 지정 필드만 필드 그룹에 할당할 수 있습니다.
