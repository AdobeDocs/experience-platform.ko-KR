...
일단 이 [!DNL Schema Editor] 필드를 추가할 컨트롤이 캔버스에 나타납니다. 이러한 컨트롤은 스키마 이름 옆에 나타나고 선택한 클래스나 필드 그룹에 정의된 개체 형식 필드도 나타납니다.
...
| [!UICONTROL 표시 이름] | 필드의 표시 이름입니다. 스키마 편집기 캔버스 내의 필드를 나타내는 데 사용할 이름입니다. 필드 이름을 [표시 이름 전환](../resources/schemas.md#display-name-toggle). |
