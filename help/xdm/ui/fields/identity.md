...
[UI에서 새 필드를 정의](./overview.md#define)할 때 오른쪽 레일에서 **[!UICONTROL Identity]** 확인란을 선택하여 ID 필드로 설정할 수 있습니다.

![](../../images/ui/fields/special/identity.png)

확인란을 선택하면 추가 컨트롤이 나타납니다. 이 필드를 스키마의 기본 ID로 설정하려면 **[!UICONTROL Primary identity]** 확인란을 선택하십시오.
...
**[!UICONTROL Identity namespace]**&#x200B;에서 드롭다운 메뉴를 사용하여 ID 필드에 적합한 네임스페이스를 선택합니다. 조직에서 정의한 사용자 정의 네임스페이스와 함께 Adobe에서 제공하는 표준 네임스페이스가 나열됩니다.

완료되면 **[!UICONTROL Apply]**&#x200B;을(를) 선택하여 스키마에 변경 내용을 적용합니다.

>[!IMPORTANT]
>
>실시간 고객 프로필에서 사용할 수 있도록 스키마를 사용하도록 설정하고 데이터를 수집하면 **기본 ID 필드를 변경할 수 없습니다**. 그렇게 하면 유효성 검사 오류가 발생합니다. 다른 기본 ID를 사용해야 하는 경우 업데이트된 ID 구성으로 새 스키마 및 새 데이터 세트를 만들어야 합니다.
...
필드도 기본 ID로 설정된 경우 왼쪽 레일의 **[!UICONTROL Required fields]** 아래에 나열됩니다. 스키마 구조 내에 ID 필드가 중첩되면 모든 상위 필드도 필요에 따라 나열됩니다.
