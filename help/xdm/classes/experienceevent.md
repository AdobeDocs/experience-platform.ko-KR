...
| `eventMergeId` | 을 사용하는 경우 [Adobe Experience Platform 웹 SDK](/help/web-sdk/home.md) 데이터를 수집하기 위해 레코드를 만들게 한 수집된 일괄 처리의 ID를 나타냅니다. 이 필드는 데이터 수집 시 시스템에서 자동으로 채워집니다. 웹 SDK 구현의 컨텍스트 외부에서 이 필드를 사용할 수 없습니다. |
...
| `timestamp`<br>**(필수)** | 다음 형식의 이벤트 발생 시점의 ISO 8601 타임스탬프 [RFC 3339 섹션 5.6](https://datatracker.ietf.org/doc/html/rfc3339). 이 타임스탬프 **필수** 과거에 발생했지만 **필수** 1970년부터 진행됩니다. 에 대한 아래 섹션 참조 [타임스탬프](#timestamps) 이 필드의 사용에 대한 모범 사례입니다. |
...
루트 `timestamp` 이벤트 스키마의 필드는 **전용** 사건 자체의 관찰을 나타내며 과거에 발생해야 합니다. 그러나 이벤트는 **필수** 1970년부터 진행됩니다. 세분화 사용 사례에서 향후 발생할 수 있는 타임스탬프를 사용해야 하는 경우 이러한 값은 경험 이벤트 스키마의 다른 곳에서 제한되어야 합니다.
