...
포함된 개체를 사용하여 XDM 스키마는 복잡한 데이터를 직접 표시하고 계층적 구조의 독립된 문서에 저장할 수 있습니다. 이 구조에 대한 주요 이점 중 하나는 많은 비용이 드는 결합 표에 연결하여 엔티티를 재구성하지 않고도 데이터를 쿼리할 수 있다는 것입니다. 스키마 계층 구조의 수준을 크게 제한할 수 없습니다.
...
스키마 구축 및 활용에는 몇 가지 주요 이점이 있습니다 [!DNL Experience Platform]. 첫째, 스키마는 데이터 거버넌스와 데이터 최소화를 향상시키도록 하며 이는 개인 정보 보호 규정에 특히 중요합니다. 둘째, Adobe의 표준 구성 요소를 사용하여 스키마를 구축하면 즉시 사용 가능한 통찰력과 최소한의 사용자 요구에 맞게 AI/ML 서비스를 사용할 수 있습니다. 마지막으로 스키마는 데이터 공유 인사이트와 효율적인 통합 관리를 위한 인프라를 제공합니다.
...
스키마의 클래스는 해당 스키마에 사용할 수 있는 혼합을 결정합니다. 이 내용은 [다음 섹션에서 자세히 설명합니다](#mixin).

Adobe은 두 개의 표준(&quot;코어&quot;) XDM 클래스를 제공합니다. [!DNL XDM Individual Profile] 및 [!DNL XDM ExperienceEvent]. 또한 이러한 핵심 클래스 외에도 고유한 사용자 지정 클래스를 만들어 조직의 특정 사용 사례를 설명할 수도 있습니다. 사용자 지정 클래스는 고유한 사용 사례를 설명하는 데 사용할 수 있는 Adobe으로 정의된 핵심 클래스가 없을 때 조직에서 정의합니다.
...
[!DNL Experience Platform] 다양한 표준 Adobe 혼합을 포함하며 공급업체는 사용자에 대한 믹스를 정의하고 개별 사용자가 고유한 개념을 위해 믹스를 정의할 수 있도록 합니다.
...
모든 현재 표준 믹스의 목록은 [공식 XDM 저장소를 참조하십시오](https://github.com/adobe/xdm/tree/master/components/mixins).
...
>[!NOTE]
>
>혼합과 데이터 유형 간의 차이점과 유사한 사용 사례를 위해 서로 사용하는 것에 대한 장단점에 대한 자세한 내용은 [부록을](#mixins-v-datatypes) 참조하십시오.
...
>[!TIP]
>
>개체 [유형 필드 대신 자유 형식 필드를 사용하는 방법에 대한 장단점에 대한 자세한 내용은 부록을](#objects-v-freeform) 참조하십시오.
...
스키마를 디자인할 때 자유 형식 필드 위에 있는 개체를 선택할 때 고려해야 할 몇 가지 주요 요소가 있습니다.

| 개체 | 자유 형식 필드 |
| --- | --- |
| 중첩 증가 | 중첩 안 함 |
| 논리 필드 그룹 생성 | 임시 위치에 필드가 배치됨 |

#### 개체

자유 형식 필드 위에 개체를 사용하는 경우의 장단점은 아래에 나와 있습니다.

**전문가**:

* 특정 필드의 논리 그룹을 만들려면 개체가 가장 잘 사용됩니다.
* 개체는 보다 구조화된 방식으로 스키마를 구성합니다.
* 개체는 세그먼트 빌더 UI에서 좋은 메뉴 구조를 만드는 데 간접적으로 도움이 됩니다. 스키마 내의 그룹화된 필드는 세그먼트 빌더 UI에서 제공하는 폴더 구조에 직접 반영됩니다.

**반대**:

* 필드가 더 중첩됩니다.
* Adobe Experience Platform 쿼리 서비스 [](../../query-service/home.md)사용 시 개체에 중첩된 쿼리 필드에 더 긴 참조 문자열을 제공해야 합니다.

#### 자유 형식 필드

개체 위에 자유 형식 필드를 사용하는 경우의 장점과 단점은 아래에 나와 있습니다.

**전문가**:

* 자유 형식 필드는 스키마의 루트 개체(`_tenantId`)에서 직접 만들어지므로 가시성이 높아집니다.
* 쿼리 서비스를 사용할 때 자유 형식 필드의 참조 문자열이 짧아지는 경우가 있습니다.

**반대**:

* 스키마 내의 자유 형식 필드의 위치는 애드혹, 즉 스키마 편집기 내에 알파벳 순서로 표시됩니다. 따라서 스키마를 구조화하지 않고 유사한 자유 형식 필드는 이름에 따라 크게 구분될 수 있습니다.