...
| 1234567 | 이벤트가 복제되지 않도록 하면서 현재 이벤트 변수에 | 910482 | 5 | 오후 2:15, 10월 3일 |
...
예를 들어 [[!UICONTROL Adobe Analytics ExperienceEvent Template] 필드 그룹](https://github.com/adobe/xdm/blob/master/extensions/adobe/experience/analytics/experienceevent-all.schema.json)을(를) 사용하여 [!DNL Analytics]별 필드를 XDM 스키마에 매핑할 수 있습니다. 작업 중인 Adobe 애플리케이션에 따라 스키마에서 이러한 Adobe 제공 필드 그룹을 사용해야 합니다.

![[!UICONTROL Adobe Analytics ExperienceEvent Template]의 스키마 다이어그램입니다.](../images/best-practices/analytics-field-group.png)
...
데이터를 데이터 레이크로 수집하면 제한된 필드에 대해서만 데이터 유효성 검사가 실행됩니다. 일괄 처리 수집 중에 특정 필드의 유효성을 검사하려면 필드를 XDM 스키마에서 제한된 것으로 표시해야 합니다. 잘못된 데이터가 Experience Platform으로 들어가지 않도록 하려면 스키마를 만들 때 유효성 검사 요구 사항을 정의하십시오.

>[!IMPORTANT]
>
>중첩된 열에는 유효성 검사가 적용되지 않습니다. 필드 형식이 배열 열 내에 있는 경우 데이터의 유효성이 검사되지 않습니다.

필드에 대한 제약 조건을 설정하려면 스키마 편집기에서 필드를 선택하여 **[!UICONTROL Field properties]** 사이드바를 엽니다. 사용 가능한 필드에 대한 정확한 설명은 [유형별 필드 속성](../ui/fields/overview.md#type-specific-properties)에 대한 설명서를 참조하십시오.

![제약 조건 필드가 [!UICONTROL Field properties] 사이드바에서 강조 표시된 스키마 편집기.](../images/best-practices/data-validation-fields.png)

### 데이터 무결성을 유지하기 위한 팁 {#data-integrity-tips}

다음 제안 사항은 스키마를 생성할 때 데이터 무결성을 유지하는 데 도움이 됩니다.
...
* **ID로 `_id`을(를) 사용하지 않는지 확인**: 경험 이벤트 스키마의 `_id` 필드는 레코드 고유성을 위한 것이므로 ID로 사용할 수 없습니다.
...
* **일관된 값에 패턴을 적용합니다**: ID 값이 특정 패턴을 따르는 경우 **[!UICONTROL Pattern]** 설정을 사용하여 제한을 적용하세요. 이 설정에는 숫자만, 대문자, 소문자 또는 특정 문자 조합과 같은 규칙이 포함될 수 있습니다. 정규 표현식을 사용하여 문자열의 패턴을 일치시킵니다.
...
## 프로필 활성화 스키마 관리 {#managing-profile-enabled-schemas}

이 섹션에서는 실시간 고객 프로필에 대해 이미 활성화된 스키마를 관리하는 방법을 설명합니다. 스키마를 활성화한 후에는 비활성화하거나 삭제할 수 없습니다. 추가 사용을 방지하는 방법과 삭제할 수 없는 데이터 세트를 관리하는 방법을 결정해야 합니다.

프로필에 대해 스키마를 활성화하면 구성을 되돌릴 수 없습니다. 더 이상 스키마를 사용하지 말아야 하는 경우 스키마 이름을 변경하여 상태를 명확히 하고 올바른 구조 및 ID 구성으로 대체 스키마를 만드십시오. 이를 통해 사용자가 새 데이터 세트를 만들거나 수집 워크플로우를 구성할 때 더 이상 사용되지 않는 스키마를 실수로 재사용하지 않도록 방지할 수 있습니다.

시스템 데이터 세트는 실시간 고객 프로필 지원 스키마와 함께 표시되는 경우가 있습니다. 연결된 스키마가 더 이상 사용되지 않는 경우에도 시스템 데이터 세트를 삭제할 수 없습니다. 의도하지 않은 사용을 방지하려면 더 이상 사용되지 않는 프로필 활성화 스키마의 이름을 바꾸고 수집 워크플로우가 제자리에 있는 시스템 데이터 세트를 가리키는지 확인하십시오.

더 이상 사용되지 않는 프로필 활성화 스키마가 실수로 재사용되지 않도록 하려면 다음 모범 사례를 사용하십시오.

* 스키마를 사용하지 않을 때에는 명확한 명명 규칙을 사용하십시오. &quot;사용되지 않음&quot;, &quot;사용하지 않음&quot; 또는 버전 태그와 같은 레이블을 포함합니다.
* 중단하려는 스키마를 기반으로 하는 데이터 세트로의 데이터 수집을 중단합니다.
* 올바른 구조, ID 구성 및 이름 지정 패턴을 사용하여 새 스키마를 만듭니다.
* 삭제할 수 없는 시스템 데이터 세트를 검토하고 이를 참조하는 수집 워크플로우가 없는지 확인합니다.
* 다른 사용자가 스키마가 더 이상 사용되지 않는 이유를 이해할 수 있도록 내부적으로 변경 사항을 문서화합니다.

>[!TIP]
>
>프로필 사용 스키마 및 관련 제한 사항에 대한 자세한 내용은 [XDM 문제 해결 안내서](../troubleshooting-guide.md#delete-profile-enabled)를 참조하십시오.

## 다음 단계

이 문서에서는 Experience Platform용 데이터 모델을 설계하기 위한 일반 지침 및 모범 사례를 다룹니다. 요약하려면:

* 스키마를 구성하기 전에 데이터 테이블을 프로필, 조회 및 이벤트 카테고리로 정렬하십시오.
* 다양한 사용 사례에 대해 스키마를 디자인할 때 여러 접근 방식을 평가합니다.
* 데이터 모델이 세분화 또는 고객 여정 목표를 지원하는지 확인하십시오.
* 스키마를 가능한 한 단순하게 유지합니다. 필요한 경우에만 새 필드를 추가합니다.

준비가 되면 스키마 만들기, 클래스 할당 및 필드 매핑에 대한 단계별 지침을 보려면 [UI에서 스키마 만들기](../tutorials/create-schema-ui.md)에 대한 자습서를 참조하십시오.
