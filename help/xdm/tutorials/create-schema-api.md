...
스키마 레지스트리는 Adobe Experience Platform 내의 스키마 라이브러리에 액세스하는 데 사용됩니다. 스키마 라이브러리에는 Adobe, Experience Platform 파트너 및 사용자가 애플리케이션을 사용하는 공급업체에서 제공하는 리소스가 포함되어 있습니다. 레지스트리는 사용 가능한 모든 라이브러리 리소스에 액세스할 수 있는 사용자 인터페이스와 RESTful API를 제공합니다.

이 자습서는 스키마 레지스트리 API를 사용하여 표준 클래스를 사용하여 스키마를 구성하는 단계를 안내합니다. Experience Platform에서 사용자 인터페이스를 사용하려는 경우 스키마 편집기 [자습서에서는](create-schema-ui.md) 스키마 편집기에서 유사한 작업을 수행하기 위한 단계별 지침을 제공합니다.
...
* [XDM(Experience Data Model) 시스템](../home.md):Adobe Experience Platform을 통해 고객 경험 데이터를 구성하는 표준화된 프레임워크입니다.
   * [스키마 컴포지션의](../schema/composition.md)기본 사항:스키마 컴포지션의 주요 원칙 및 모범 사례 등 XDM 스키마의 기본 구성 요소에 대해 알아봅니다.
...
요청에는 클래스의 속성을 참조하는 `allOf` 특성이 포함되어야 `$id` 합니다. 이 속성은 스키마가 구현할 &quot;기본 클래스&quot;를 정의합니다. 이 예에서 기본 클래스는 XDM 개별 프로필 클래스입니다. XDM `$id` 개별 프로필 클래스의 값은 아래 `$ref` 배열에서 `allOf` 필드 값으로 사용됩니다.
...
요청이 성공하면 HTTP 응답 상태 201(Created)이 반환되고, `$id``meta:altIt`및 `version`를 포함하여 새로 만든 스키마의 세부 정보가 포함된 응답 본문이 반환됩니다. 이러한 값은 읽기 전용이며 스키마 레지스트리에 지정됩니다.
...
새로 만든 스키마를 보려면 스키마에 대해 `meta:altId` 또는 URL 인코딩 URI를 사용하여 조회(GET) 요청을 `$id` 수행합니다.
...
응답 형식은 요청과 함께 전송된 수락 헤더에 따라 달라집니다. 요구 사항에 가장 적합한 헤더를 확인하기 위해 다른 승인 헤더를 사용해 보십시오.
...
선택한 스키마 클래스에 따라 사용할 수 있는 표준 혼합이 다릅니다. 각 혼합에는 믹싱과 호환되는 클래스를 정의하는 `intendedToExtend` 필드가 포함되어 있습니다.

Mixins는 동일한 정보를 캡처해야 하는 모든 스키마에서 다시 사용할 수 있는 &quot;이름&quot; 또는 &quot;주소&quot;와 같은 개념을 정의합니다.
...
이 요청 업데이트(PATCH)는 충성도 멤버 스키마를 업데이트하여 &quot;프로필-개인-세부 사항&quot; 믹싱 내에 필드를 포함합니다.

&quot;프로필-사람-세부 사항&quot; 혼합을 추가하면 충성도 멤버 스키마는 이제 이름, 성 및 생일과 같은 충성도 프로그램 멤버에 대한 정보를 캡처합니다.
...
이 응답은 배열에 새로 추가된 믹싱을 보여주며 `meta:extends` 특성의 혼합에 `$ref` `allOf` 대한 를 포함합니다.
...
>[!TIP] 사용 가능한 모든 혼합물을 검토하여 각 영역에 포함된 필드에 익숙해지도록 하는 것은 가치가 있다. 각 &quot;global&quot; 및 &quot;tenant&quot; 컨테이너에 대한 요청을 수행하여 특정 클래스에서 사용할 수 있는 모든 믹스를 나열하고 &quot;meta:intendToExtend&quot; 필드가 사용 중인 클래스와 일치하는 믹스만 반환할 수 있습니다. 이 경우 XDM 개인 프로필 클래스이므로 XDM 개별 프로필이 `$id` 사용됩니다.
...
이 요청 업데이트(PATCH)는 &quot;profile-personal-details&quot; mixin 내의 필드를 포함하고, 스키마에 &quot;home address&quot;, &quot;email address&quot; 및 &quot;home phone&quot; 필드를 추가하는 충성도 멤버 스키마를 업데이트합니다.
...
충성도 멤버 스키마는 이제 배열에 세 개의 `$ref` `allOf` 값을 포함해야 합니다.&quot;profile&quot;, &quot;profile-person-details&quot; 및 &quot;profile-personal-details&quot;를 참조하십시오.
...
충성도 멤버 스키마는 충성도 프로그램에 고유한 정보를 캡처해야 합니다. 이 정보는 표준 혼합에 포함되지 않습니다.

테넌트 컨테이너 내에 고유한 혼합을 정의할 수 있도록 하여 이에 대한 스키마 레지스트리 계정을 만듭니다. 이러한 혼합은 조직에 고유하며 IMS 조직 외부의 어떤 사람도 보거나 편집할 수 없습니다.

새 혼합을 만들기(POST)하려면 요청에는 믹싱이 포함될 속성과 함께 믹신이 호환되는 기본 클래스의 `meta:intendedToExtend` `$id` 필드가 포함되어 있어야 합니다.

사용자 정의 속성은 다른 믹스나 필드와 충돌하지 `TENANT_ID` 않도록 사용자 아래에 중첩되어야 합니다.
...
이 요청은 네 개의 충성도 프로그램 전용 필드를 포함하는 &quot;충성도&quot; 개체가 있는 새 혼합을 만듭니다.&quot;loyaltyId&quot;, &quot;loyaltyLevel&quot;, &quot;loyaltyPoints&quot; 및 &quot;memberSince&quot;
...
요청이 성공하면 HTTP 응답 상태 201(Created)이 반환되고, `$id``meta:altIt`및 `version`를 포함하여 새로 만든 믹스의 세부 정보가 포함된 응답 본문이 반환됩니다. 이러한 값은 읽기 전용이며 스키마 레지스트리에 지정됩니다.
...
이제 표준 믹스를 [추가하는 것과 동일한 단계를 수행하여 새로 만든 믹싱을](#add-a-mixin) 스키마에 추가할 수 있습니다.
...
이 요청 업데이트(PATCH)는 충성도 멤버 스키마를 업데이트하여 새로운 &quot;충성도 멤버 세부 사항&quot; 믹스에 필드를 포함합니다.
...
이제 응답에 배열에 새로 추가된 믹스가 표시되고 `meta:extends` `$ref` `allOf` 특성의 혼합에 대한 내용이 포함되므로 혼합이 성공적으로 추가되었음을 확인할 수 있습니다.
...
수락 `application/vnd.adobe.xed-full+json; version=1` 헤더를 사용하면 모든 속성을 표시하는 전체 스키마를 볼 수 있습니다. 이러한 속성은 스키마를 구성하는 데 사용된 클래스와 혼합에서 만든 필드입니다. 이 예에서 개별 속성 속성은 공간에 대해 최소화되었습니다. 이 문서의 끝 부분에 있는 [부록에서](#appendix) 모든 속성과 해당 속성을 포함한 전체 스키마를 볼 수 있습니다.

아래에서 `"properties"`사용자 지정 혼합을 추가할 때 만들어진 `_{TENANT_ID}` 네임스페이스를 볼 수 있습니다. 이 네임스페이스 안에는 &quot;충성도&quot; 개체와 혼합이 생성될 때 정의된 필드가 있습니다.
...
만든 충성도 혼합에는 다른 스키마에서 유용할 수 있는 특정 충성도 속성이 포함됩니다. 예를 들어, 데이터는 경험 이벤트의 일부로 인제스트되거나 다른 클래스를 구현하는 스키마에서 사용될 수 있습니다. 이 경우 객체 계층을 데이터 유형으로 저장하여 다른 곳에서 정의를 쉽게 재사용할 수 있습니다.

데이터 유형을 사용하면 객체 계층 구조를 한 번 정의할 수 있으며 다른 스칼라 유형에 대해 언급하는 것처럼 필드에서 참조할 수 있습니다.

즉, 데이터 유형은 필드의 &quot;유형&quot;으로 추가하여 스키마에서 어디에나 포함할 수 있으므로 혼합보다 유연하므로 다중 필드 구조를 일관되게 사용할 수 있습니다.
...
요청이 성공하면 HTTP 응답 상태 201(작성됨)이 반환되고, `$id``meta:altIt`및 `version`를 포함하여 새로 만든 데이터 유형의 세부 정보가 들어 있습니다. 이러한 값은 읽기 전용이며 스키마 레지스트리에 지정됩니다.
...
URL 인코딩 URI를 사용하여 조회(GET) `$id` 요청을 수행하여 새 데이터 형식을 직접 볼 수 있습니다. 조회 요청에 대한 수락 `version` 헤더에 를 포함해야 합니다.

### 스키마에 데이터 유형 사용

충성도 세부 정보 데이터 유형이 만들어졌으므로 이전에 있었던 필드 대신 데이터 유형을 참조하도록 만든 믹스의 &quot;충성도&quot; 필드를 업데이트(PATCH)할 수 있습니다.
...
스키마는 Experience Platform에 데이터를 인제스트하는 데 사용됩니다. 이 데이터는 궁극적으로 여러 서비스에서 사용되어 한 개인의 단일 뷰를 생성합니다. 이 프로세스를 돕기 위해 키 필드를 &quot;ID&quot;로 표시할 수 있으며 데이터 수집 시 해당 필드의 데이터가 해당 개인에 대한 &quot;ID 그래프&quot;에 삽입됩니다. 그러면 실시간 고객 프로필 및 [기타 Experience](../../profile/home.md) Platform 서비스를 통해 그래프 데이터에 액세스하여 각 개별 고객에 대한 통합된 보기를 제공할 수 있습니다.

일반적으로 &quot;ID&quot;로 표시된 필드에는 다음이 포함됩니다.이메일 주소, 전화 번호, [Experience Cloud ID(ECID)](https://marketing.adobe.com/resources/help/en_US/mcvid/), CRM ID 또는 기타 고유 ID 필드

좋은 ID 필드일 수 있으므로 조직 고유의 식별자를 고려합니다.

ID 설명자는 &quot;sourceSchema&quot;의 &quot;sourceProperty&quot;가 &quot;Identity&quot;로 간주되어야 하는 고유한 식별자임을 알립니다.

설명자 작업에 대한 자세한 내용은 스키마 레지스트리 [개발자 안내서를](../api/getting-started.md)참조하십시오.
...
다음 요청은 &quot;loyaltyId&quot; 필드에 ID 설명자를 정의합니다. 이렇게 하면 경험 플랫폼이 고유한 충성도 프로그램 멤버 식별자(이 경우 멤버의 이메일 주소)를 사용하여 개인 정보를 연결하는 데 도움이 됩니다.
...
>[!NOTE] Identity Service API를 사용하여 사용 가능한 &quot;xdm:namespace&quot; 값을 나열하거나 새 값을 [만들 수 있습니다](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/id-service-api.yaml). &quot;xdm:property&quot;의 값은 사용된 &quot;xdm:namespace&quot;에 따라 &quot;xdm:code&quot; 또는 &quot;xdm:id&quot;일 수 있습니다.

**응답**

성공적으로 응답하면 새로 만든 설명자를 포함하여 새로 만든 설명자의 세부 사항을 포함하는 응답 본문을 포함하는 HTTP Status 201(Created)이 반환됩니다 `@id`. 는 `@id` 스키마 레지스트리에서 할당한 읽기 전용 필드이며 API의 설명자를 참조하는 데 사용됩니다.
...
속성에 &quot;union&quot; 태그를 추가하면 실시간 고객 프로필에서 사용할 충성도 멤버 스키마를 활성화할 수 있습니다. `meta:immutableTags`
...
이 응답에는 작업이 성공적으로 수행되었으며 이제 스키마에는 값 &quot;union&quot;이 `meta:immutableTags`포함된 배열인 최상위 수준 속성이 포함됩니다.
...
이제 스키마를 XDM 개인 프로필 결합에 추가했습니다. 동일한 결합에 속하는 모든 스키마 목록을 보려면 쿼리 매개 변수를 사용하여 응답을 필터링할 수 있습니다.

쿼리 매개 변수를 사용하여 XDM 개별 프로필 클래스 `property` 같은 `meta:immutableTags` 필드를 포함하는 스키마만 반환되도록 지정할 `meta:class` `$id` 수 있습니다.
...
아래 예제 요청은 XDM 개별 프로필 조합의 일부인 모든 스키마를 반환합니다.
...
이 튜토리얼을 따라 표준 믹스와 정의한 믹스를 사용하여 스키마를 성공적으로 구성했습니다. 이제 이 스키마를 사용하여 데이터 집합을 만들고 Adobe Experience Platform에 레코드 데이터를 인제스트할 수 있습니다.

이 자습서 전체에서 생성된 전체 충성도 멤버 스키마는 다음의 부록에서 사용할 수 있습니다. 스키마를 살펴보면 믹싱이 전체 구조에 기여하는 방법과 데이터 수집에 사용할 수 있는 필드를 확인할 수 있습니다.

두 개 이상의 스키마를 만들었으면 관계 설명자의 사용을 통해 스키마 간의 관계를 정의할 수 있습니다. 자세한 내용은 두 스키마 [간의 관계를](relationship-api.md) 정의하는 자습서를 참조하십시오. 레지스트리에서 모든 작업(GET, POST, PUT, PATCH 및 DELETE)을 수행하는 방법에 대한 자세한 예는 API를 [사용하는 동안 스키마 레지스트리 개발자 안내서를](../api/getting-started.md) 참조하십시오.
...
이 자습서 전반에서 소매 충성도 프로그램의 구성원을 설명하기 위해 스키마가 구성됩니다.

스키마는 XDM 개별 프로필 클래스를 구현하고 여러 혼합을 결합합니다.튜토리얼 중에 정의된 &quot;충성도 세부 사항&quot; 혼합뿐만 아니라 표준 &quot;개인 세부 사항&quot; 및 &quot;개인 세부 사항&quot; 혼합을 사용하여 충성도 구성원에 대한 정보를 제공합니다.
