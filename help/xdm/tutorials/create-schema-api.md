...
이 요청은 를 포함하는 새 필드 그룹을 만듭니다 `loyaltyTier` 회사의 특정 충성도 프로그램과 관련된 네 개의 필드를 포함하는 객체입니다. `id`, `effectiveDate`, `currentThreshold`, 및 `nextThreshold`.
...
아래 `"properties"`를 클릭하면 `_{TENANT_ID}` 사용자 지정 필드 그룹을 추가할 때 만든 네임스페이스입니다. 해당 네임스페이스 내에서는 `loyaltyTier` 개체 및 필드 그룹을 만들 때 정의한 필드입니다.
...
이제 응답에 참조( )가 포함됩니다.`$ref`)을 클릭하여 `loyaltyTier` 이전에 정의한 필드 대신 개체를 작성합니다.
