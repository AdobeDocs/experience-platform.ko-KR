...
Adobe Experience Platform 사용자 인터페이스를 사용하면 [!DNL Experience Data Model]이라는 인터랙티브한 시각적 캔버스에서 [!DNL Schema Editor]&#x200B;(XDM) 스키마를 만들고 관리할 수 있습니다. 이 자습서에서는 [!DNL Schema Editor]을(를) 사용하여 스키마를 만드는 방법을 다룹니다.
...
>
>[!DNL Schema Registry] API를 사용하여 스키마를 작성하려면 [[!DNL Schema Registry] API를 사용하여 스키마 만들기](../api/getting-started.md)에 대한 자습서를 시도하기 전에 [개발자 안내서](create-schema-api.md)를 읽는 것부터 시작하십시오.
...
## [!UICONTROL Schemas] 작업 영역 열기 {#browse}

[!UICONTROL Schemas] UI의 [!DNL Experience Platform] 작업 영역은 [!DNL Schema Library]의 시각화를 제공하므로 조직에서 사용할 수 있는 스키마 관리를 볼 수 있습니다. 작업 영역에는 이 자습서 전체에서 스키마를 구성할 수 있는 캔버스인 [!DNL Schema Editor]도 포함됩니다.

[!DNL Experience Platform]에 로그인한 후 왼쪽 탐색에서 **[!UICONTROL Schemas]**&#x200B;을(를) 선택하여 **[!UICONTROL Schemas]** 작업 영역을 엽니다. **[!UICONTROL Browse]** 탭에는 사용자가 보고 사용자 지정할 스키마 목록([!DNL Schema Library] 표시)이 표시됩니다. 이 목록에는 스키마의 기반이 되는 이름, 유형, 클래스 및 동작(레코드 또는 시계열)과 스키마가 마지막으로 수정된 날짜 및 시간이 포함됩니다.

자세한 내용은 [UI에서 기존 XDM 리소스 탐색](../ui/explore.md)에 대한 안내서를 참조하십시오.

## 스키마 만들기 및 이름 지정 {#create}

스키마 작성을 시작하려면 **[!UICONTROL Create schema]** 작업 영역의 오른쪽 상단 모서리에서 **[!UICONTROL Schemas]**&#x200B;을(를) 선택합니다.

![[!UICONTROL Schemas]이(가) 강조 표시된 [!UICONTROL Browse] 작업 영역 [!UICONTROL Create schema] 탭입니다.](../images/tutorials/create-schema/create-schema-button.png)

[!UICONTROL Create a schema] 대화 상자가 나타납니다. 이 대화 상자에서 필드와 필드 그룹을 추가하여 스키마를 수동으로 만들도록 선택하거나 CSV 파일을 업로드하고 ML 알고리즘을 사용하여 스키마를 생성할 수 있습니다. 대화 상자에서 스키마 생성 워크플로우를 선택합니다.
...
[!UICONTROL Create schema] 워크플로가 나타납니다. 그런 다음 스키마의 기본 클래스를 선택합니다. [!UICONTROL XDM Individual Profile]과(와) [!UICONTROL XDM ExperienceEvent]의 핵심 클래스 또는 [!UICONTROL Other]이(가) 목적에 맞지 않으면 선택할 수 있습니다. [!UICONTROL Other] 클래스 옵션을 사용하면 [새 클래스를 만들거나](#create-new-class) 기존 다른 클래스에서 선택할 수 있습니다.

이러한 클래스에 대한 자세한 내용은 [[!UICONTROL XDM individual profile]](../classes/individual-profile.md) 및 [[!UICONTROL XDM ExperienceEvent]](../classes/experienceevent.md) 설명서를 참조하십시오. 이 자습서에서는 **[!UICONTROL XDM Individual Profile]**&#x200B;을(를) 선택한 후 **[!UICONTROL Next]**&#x200B;을(를) 선택하십시오.

![[!UICONTROL Create schema] 옵션과 [!UICONTROL XDM individual profile]이(가) 강조 표시된 [!UICONTROL Next] 워크플로우입니다.](../images/tutorials/create-schema/individual-profile-base-class.png)

### 이름 및 검토 {#name-and-review}

클래스를 선택하면 [!UICONTROL Name and review] 섹션이 나타납니다. 이 섹션에서는 스키마를 식별하기 위한 이름과 설명을 제공합니다. 스키마의 이름을 결정할 때 고려해야 할 몇 가지 중요한 사항이 있습니다.
...
[!DNL Schema Editor]이(가) 나타납니다. 스키마를 구성할 캔버스입니다. 편집기에 도착하면 캔버스의 **[!UICONTROL Structure]** 섹션에 선택한 기본 클래스에 포함된 표준 필드와 함께 자체 제목이 지정된 스키마가 자동으로 만들어집니다. 스키마에 할당된 클래스도 **[!UICONTROL Class]** 섹션의 **[!UICONTROL Composition]** 아래에 나열됩니다.

>[!NOTE]
>
>**[!UICONTROL Schema properties]** 사이드바에서 스키마의 표시 이름과 선택적 설명을 업데이트할 수 있습니다. 새 이름을 입력하면 스키마의 새 이름을 반영하도록 캔버스가 자동으로 업데이트됩니다.
...
필드 그룹을 추가하려면 **[!UICONTROL Add]** 하위 섹션에서 **[!UICONTROL Field groups]**&#x200B;을(를) 선택하십시오.
...
![대화 상자 [!UICONTROL Add field groups]개](../images/tutorials/create-schema/field-group-popularity.png)

왼쪽 레일에서 필터 중 하나를 선택하여 표준 필드 그룹 목록을 소매, 금융 서비스, 의료 등의 특정 [산업](../schema/industries/overview.md)(으)로 좁힐 수 있습니다.

![산업 필드 그룹이 강조 표시된 [!UICONTROL Add field groups] 대화 상자.](../images/tutorials/create-schema/industry-field-groups.png)
...
![선택한 필드 그룹 미리 보기 아이콘이 강조 표시된 [!UICONTROL Add field groups] 대화 상자입니다.](../images/tutorials/create-schema/preview-field-group-button.png)

필드 그룹을 미리 볼 때 필드 그룹의 스키마에 대한 자세한 설명이 오른쪽 레일에 제공됩니다. 제공된 캔버스에서 필드 그룹의 필드를 탐색할 수도 있습니다. 다른 필드를 선택하면 오른쪽 레일이 업데이트되어 해당 필드에 대한 세부 정보가 표시됩니다. 미리 보기를 마치면 **[!UICONTROL Back]**&#x200B;을(를) 선택하여 필드 그룹 선택 대화 상자로 돌아갑니다.

![인구 통계 세부 정보 필드 그룹이 미리 표시된 [!UICONTROL Preview field group] 대화 상자입니다.](../images/tutorials/create-schema/preview-field-group.png)

이 자습서에서는 **[!UICONTROL Demographic Details]** 필드 그룹을 선택한 다음 **[!UICONTROL Add field group]**&#x200B;을(를) 선택하십시오.

![인구 통계학적 세부 정보 필드 그룹이 선택되어 있고 [!UICONTROL Add field groups]이(가) 강조 표시된 [!UICONTROL Add field groups] 대화 상자.](../images/tutorials/create-schema/demographic-details.png)

스키마 캔버스가 다시 나타납니다. 이제 **[!UICONTROL Field groups]** 섹션에 &quot;[!UICONTROL Demographic Details]&quot;이(가) 나열되고 **[!UICONTROL Structure]** 섹션에는 필드 그룹에서 제공한 필드가 포함됩니다. **[!UICONTROL Field groups]** 섹션 아래에서 필드 그룹의 이름을 선택하여 캔버스 내에서 제공하는 특정 필드를 강조 표시할 수 있습니다.
...
이 필드 그룹은 데이터 형식이 &quot;`person`&quot;인 최상위 이름 [!UICONTROL Person] 아래에 여러 필드를 기여합니다. 이 필드 그룹은 이름, 생년월일 및 성별을 포함하여 개인에 대한 정보를 설명합니다.

>[!NOTE]
>
>필드는 [!DNL Schema Registry] 내에 정의된 데이터 형식(일반적인 개념을 나타내는 필드 그룹)뿐만 아니라 스칼라 형식(예: 문자열, 정수, 배열 또는 날짜)을 사용할 수 있습니다.

`name` 필드의 데이터 형식은 &quot;[!UICONTROL Full name]&quot;입니다. 이는 일반적인 개념을 설명하고 이름, 성, 호칭 및 접미사와 같은 이름 관련 하위 필드를 포함함을 의미합니다.

캔버스 내에서 다른 필드를 선택하여 스키마 구조에 기여하는 추가 필드를 표시합니다.

## 더 많은 필드 그룹 추가 {#field-group-2}

이제 동일한 단계를 반복하여 다른 필드 그룹을 추가할 수 있습니다. 이번에 **[!UICONTROL Add field group]** 대화 상자를 볼 때 &quot;[!UICONTROL Demographic Details]&quot; 필드 그룹이 회색으로 표시되어 옆에 있는 확인란을 선택할 수 없습니다. 이렇게 하면 이미 현재 스키마에 포함된 필드 그룹을 실수로 복제할 수 없습니다.

이 자습서의 경우 목록에서 표준 필드 그룹 **[!UICONTROL Personal Contact Details]** 및 **[!UICONTROL Loyalty Details]**&#x200B;을(를) 선택한 다음 **[!UICONTROL Add field groups]**&#x200B;을(를) 선택하여 스키마에 추가합니다.

![두 개의 새 필드 그룹이 선택되고 [!UICONTROL Add field groups]이(가) 강조 표시된 [!UICONTROL Add field groups] 대화 상자.](../images/tutorials/create-schema/more-field-groups.png)

추가된 필드 그룹이 **[!UICONTROL Field groups]** 섹션의 **[!UICONTROL Composition]** 아래에 나열되고 합성 필드가 스키마 구조에 추가된 상태로 캔버스가 다시 나타납니다.

![새 복합 스키마 구조가 강조 표시된 스키마 편집기입니다.](../images/tutorials/create-schema/updated-structure.png)

## 사용자 정의 필드 그룹 정의 {#define-field-group}

[!UICONTROL Loyalty Members] 스키마는 충성도 프로그램의 구성원과 관련된 데이터를 캡처하기 위한 것이며, 스키마에 추가한 표준 [!UICONTROL Loyalty Details] 필드 그룹은 프로그램 유형, 포인트, 조인 날짜 등을 포함하여 이러한 기능 대부분을 제공합니다.
...
![강조 표시된 [!UICONTROL Add field groups], [!UICONTROL Create new field group] 및 [!UICONTROL Display name]의 [!UICONTROL Description] 대화 상자.](../images/tutorials/create-schema/create-new-field-group.png)

>[!NOTE]
>
>클래스 이름과 마찬가지로 필드 그룹 이름은 필드 그룹이 스키마에 기여할 내용을 설명하는 짧고 간단해야 합니다. 이러한 속성도 고유하므로 이름을 다시 사용할 수 없으므로 이름이 충분히 구체적인지 확인해야 합니다.

이제 캔버스의 왼쪽 [!DNL Custom Loyalty Details] 아래에 &quot;**[!UICONTROL Field groups]**&quot;이(가) 표시되어야 하지만 아직 연결된 필드가 없으므로 **[!UICONTROL Structure]** 아래에 새 필드가 표시되지 않습니다.
...
캔버스에 &quot;[!UICONTROL Untitled Field]&quot; 자리 표시자가 나타나고 오른쪽 레일이 업데이트되어 필드에 대한 구성 옵션이 표시됩니다.

![스키마 편집기([!UICONTROL Untitled Field] 및 [!UICONTROL Field properties])가 강조 표시되었습니다.](../images/tutorials/create-schema/untitled-field.png)

이 시나리오에서는 스키마에 개인의 현재 충성도 계층을 자세히 설명하는 오브젝트 유형 필드가 있어야 합니다. 오른쪽 레일의 컨트롤을 사용하여 관련 필드를 보관하는 데 사용할 &quot;`loyaltyTier`&quot; 형식의 [!UICONTROL Object] 필드를 만들기 시작합니다.

**[!UICONTROL Assign to]**&#x200B;에서 필드를 할당할 필드 그룹을 선택해야 합니다. 모든 스키마 필드는 클래스 또는 필드 그룹에 속하며 이 스키마는 표준 클래스를 사용하므로 필드 그룹을 선택하는 것이 유일한 옵션입니다. &quot;[!DNL Custom Loyalty Details]&quot; 이름을 입력한 다음 목록에서 필드 그룹을 선택합니다.

완료되면 **[!UICONTROL Apply]**&#x200B;을(를) 선택합니다.

![충성도 계층 개체가 강조 표시된 스키마 [!UICONTROL Field properties]에 추가된 스키마 편집기.](../images/tutorials/create-schema/loyalty-tier-object.png)
...
하위 필드 추가를 시작하려면 **개체 옆에 있는**&#x200B;더하기(+)`loyaltyTier` 아이콘을 선택하십시오. 새 필드 자리 표시자가 나타나고 캔버스의 오른쪽에 **[!UICONTROL Field properties]** 섹션이 표시됩니다.
...
* **[!UICONTROL Field Name]:** 필드의 이름으로, camelCase로 작성되는 것이 좋습니다. 공백 문자는 허용되지 않습니다. 코드 및 기타 다운스트림 애플리케이션에서 필드를 참조하는 데 사용되는 이름입니다.
   * 예: loyaltyLevel
* **[!UICONTROL Display Name]:** 제목 대/소문자로 작성된 필드 이름입니다. 스키마를 보거나 편집할 때 캔버스에 표시되는 이름입니다.
   * 예: 충성도 수준
* **[!UICONTROL Type]:** 필드의 데이터 형식입니다. 여기에는 기본 스칼라 형식과 [!DNL Schema Registry]에 정의된 모든 데이터 형식이 포함됩니다. 예: [!UICONTROL String], [!UICONTROL Integer], [!UICONTROL Boolean], [!UICONTROL Person], [!UICONTROL Address], [!UICONTROL Phone number] 등
* **[!UICONTROL Description]:** 필드에 대한 선택적 설명은 최대 200자로 포함해야 합니다.

`loyaltyTier` 개체의 첫 번째 필드는 충성도 멤버의 현재 계층의 ID를 나타내는 `id`(이)라는 문자열입니다. 이 회사는 서로 다른 요인에 따라 각 고객에 대해 서로 다른 충성도 계층 포인트 임계값을 설정하므로 계층 ID는 각 충성도 멤버에 대해 고유합니다. 새 필드의 유형을 &quot;[!UICONTROL String]&quot;(으)로 설정하면 **[!UICONTROL Field properties]** 섹션이 기본값, 형식 및 최대 길이 등 제약 조건을 적용할 수 있는 여러 옵션으로 채워집니다. 자세한 내용은 [데이터 유효성 검사 필드 모범 사례](../schema/best-practices.md#data-validation-fields)에 대한 설명서를 참조하세요.

![새 ID 필드의 필드 속성 값이 강조 표시된 스키마 편집기.](../images/tutorials/create-schema/string-constraints.png)

`id`은(는) 임의로 생성된 자유 형식 문자열이므로 추가 제약 조건이 필요하지 않습니다. 변경 내용을 적용하려면 **[!UICONTROL Apply]**&#x200B;을(를) 선택하십시오.
...
스키마에 각 필드를 추가하려면 **개체 옆의**&#x200B;더하기(+)`loyalty` 아이콘을 선택하고 필요한 정보를 입력하십시오.
...
| 제한 | 설명 |
| --- | --- |
| [!UICONTROL Required] | 데이터 수집에 필드가 필요함을 나타냅니다. 이 필드를 포함하지 않는 이 스키마를 기반으로 한 데이터 세트에 업로드된 모든 데이터는 수집 시 실패합니다. |
| [!UICONTROL Array] | 필드에 데이터 형식이 지정된 값의 배열이 포함되어 있음을 나타냅니다. 예를들어 데이터 형식이 &quot;[!UICONTROL String]&quot;인 필드에 이 제약 조건을 사용하면 필드에 문자열 배열이 포함되도록 지정할 수 있습니다. |
| [!UICONTROL Enum & Suggested Values] | 열거형은 이 필드에 가능한 값의 열거형 목록에 있는 값 중 하나를 포함해야 함을 나타냅니다. 또는 이 옵션을 사용하여 필드를 해당 값으로 제한하지 않고 문자열 필드에 대해 제안된 값 목록을 제공할 수도 있습니다. |
| [!UICONTROL Identity] | 이 필드가 ID 필드임을 나타냅니다. ID 필드에 대한 자세한 정보는 [나중에 이 자습서에서](#identity-field)에 제공됩니다. |
| [!UICONTROL Relationship] | 유니온 스키마와 [!DNL Real-Time Customer Profile]을(를) 사용하여 스키마 관계를 유추할 수 있지만 이는 동일한 클래스를 공유하는 스키마에만 적용됩니다. [!UICONTROL Relationship] 제약 조건은 이 필드가 다른 클래스를 기반으로 하는 스키마의 기본 ID를 참조하며 두 스키마 간의 관계를 암시함을 나타냅니다. 자세한 내용은 [관계 정의](./relationship-ui.md)에 대한 자습서를 참조하십시오. |
...
이 자습서에서 스키마의 `loyaltyTier` 개체에는 계층 클래스를 설명하는 새 열거형 필드가 필요합니다. 여기서 값은 네 가지 옵션 중 하나일 수 있습니다. 이 필드를 스키마에 추가하려면 **개체 옆의**&#x200B;더하기(+)`loyaltyTier` 아이콘을 선택하고 **[!UICONTROL Field name]** 및 **[!UICONTROL Display name]**&#x200B;의 필수 필드를 입력하십시오. **[!UICONTROL Type]**&#x200B;에 대해 &quot;[!UICONTROL String]&quot;을(를) 선택합니다.

![계층 클래스 개체가 있는 스키마 편집기가 [!UICONTROL Field properties]에 추가되고 강조 표시되었습니다.](../images/tutorials/create-schema/tier-class-type.png)

해당 유형을 선택한 후 필드에 대해 **[!UICONTROL Array]**, **[!UICONTROL Enum & Suggested Values]**, **[!UICONTROL Identity]** 및 **[!UICONTROL Relationship]**&#x200B;에 대한 확인란을 포함하여 추가 확인란이 표시됩니다.

**[!UICONTROL Enum & Suggested Values]** 확인란을 선택한 다음 **[!UICONTROL Enum]**&#x200B;을(를) 선택합니다. 허용되는 각 충성도 계층 클래스에 대해 **[!UICONTROL Value]**(camelCase의 경우) 및 **[!UICONTROL Display Name]**(Title Case의 경우 읽기 쉬운 선택적 이름)을 입력할 수 있습니다.

모든 필드 속성을 완료했으면 **[!UICONTROL Apply]**&#x200B;을(를) 선택하여 `tierClass` 필드를 `loyaltyTier` 개체에 추가합니다.

![열거형 및 제안 값 필드 속성이 완료되었으며 [!UICONTROL Apply]이(가) 강조 표시되었습니다.](../images/tutorials/create-schema/tier-class-enum.png)
...
`loyaltyTier` 개체를 데이터 형식으로 변환하려면 캔버스에서 `loyaltyTier` 필드를 선택한 다음 **[!UICONTROL Convert to new data type]** 아래 편집기 오른쪽의 **[!UICONTROL Field properties]**&#x200B;을(를) 선택하십시오.

![loyaltyTier 개체와 [!UICONTROL Convert to new data type]이(가) 강조 표시된 스키마 편집기입니다.](../images/tutorials/create-schema/convert-data-type.png)
...
스키마가 제공하는 표준 데이터 구조를 활용하여 여러 소스에서 동일한 개인에 속하는 데이터를 식별할 수 있으며, 세그멘테이션, 보고, 데이터 과학 분석 등과 같은 다양한 다운스트림 사용 사례를 허용할 수 있습니다. 개별 ID를 기반으로 데이터를 연결하려면 키 필드를 적용 가능한 스키마 내의 [!UICONTROL Identity] 필드로 표시해야 합니다.

[!DNL Experience Platform]을(를) 사용하면 **[!UICONTROL Identity]**&#x200B;에서 [!DNL Schema Editor] 확인란을 사용하여 id 필드를 쉽게 표시할 수 있습니다. 그러나 데이터의 특성에 따라 ID로 사용할 가장 적합한 필드를 결정해야 합니다.

예를 들어, 동일한 충성도 수준에 속하는 수천 명의 충성도 프로그램 구성원과 동일한 실제 주소를 공유하는 여러 명의 충성도 프로그램 구성원이 있을 수 있습니다. 그러나 이 시나리오에서는 등록 시 고객 충성도 프로그램의 각 구성원이 자신의 개인 이메일 주소를 제공합니다. 개인 전자 메일 주소는 일반적으로 한 사람이 관리하므로 `personalEmail.address` 필드([!UICONTROL Personal Contact Details] 필드 그룹에서 제공)는 ID 필드에 적합한 후보입니다.
...
그런 다음 드롭다운의 미리 정의된 네임스페이스 목록에서 **[!UICONTROL Identity namespace]**&#x200B;을(를) 제공해야 합니다. 이 필드는 고객의 전자 메일 주소이므로 드롭다운에서 &quot;[!UICONTROL Email]&quot;을(를) 선택합니다. **[!UICONTROL Apply]** 필드에 대한 업데이트를 확인하려면 `personalEmail.address`을(를) 선택하십시오.
...
변경 내용을 적용한 후 `personalEmail.address`의 아이콘에 지문 기호가 표시되어 현재 ID 필드임을 나타냅니다. 필드는 **[!UICONTROL Identities]** 아래의 왼쪽 레일에도 나열됩니다.
...
편집기의 오른쪽에는 표시 이름, 설명 및 유형을 포함한 스키마에 대한 정보가 표시됩니다. 이 정보 외에도 **[!UICONTROL Profile]** 전환 단추가 있습니다.

![스키마 루트와 프로필 사용 토글이 강조 표시된 스키마 편집기.](../images/tutorials/create-schema/profile-toggle.png)

**[!UICONTROL Profile]**&#x200B;을(를) 선택하면 [!DNL Profile]에 대한 스키마를 사용할지 확인하는 팝오버가 나타납니다.
...
**[!UICONTROL Enable]**&#x200B;을(를) 선택하여 선택을 확인합니다. 원하는 경우 **[!UICONTROL Profile]** 전환을 다시 선택하여 스키마를 비활성화할 수 있지만 [!DNL Profile]을(를) 사용하는 동안 스키마를 저장한 후에는 더 이상 비활성화할 수 없습니다.

## 기타 액션 {#more}

>[!NOTE]
>
>XDM 리소스로 작업하는 경우 인벤토리 테이블(행 메뉴)과 리소스 세부 사항 보기(**[!UICONTROL More]**) 모두에서 작업을 사용할 수 있습니다. **삭제**, **JSON 구조 복사** 및 **패키지에 추가**&#x200B;를 포함한 전체 작업 집합에 액세스하려면 사용자 지정(테넌트 정의) 리소스를 선택해야 합니다. 표준(Adobe 제공) 리소스에는 제한된 작업이 있습니다. 작업, 제약 조건 및 사용 권한에 대한 전체 개요는 [스키마, 클래스, 필드 그룹 및 데이터 형식 관리(작업 및 삭제)를 참조하십시오](../ui/explore.md#xdm-resource-actions).

스키마 편집기 내에서 빠른 작업을 수행하여 스키마의 JSON 구조를 복사하거나 스키마를 삭제할 수도 있습니다. 빠른 작업과 함께 드롭다운을 표시하려면 보기 맨 위에서 [!UICONTROL More]을(를) 선택하십시오.
...
[!UICONTROL More] 작업을 사용하여 스키마 편집기의 UI 내에서 스키마를 삭제할 수 있으며 [!UICONTROL Browse] 탭의 스키마 세부 정보에서 스키마를 삭제할 수도 있습니다. 스키마가 삭제되지 않도록 하는 특정 조건이 있습니다. 다음과 같은 경우에는 스키마를 삭제할 수 없습니다.
...
스키마 라이브러리의 모든 스키마에 대한 내보내기 페이로드를 생성하려면 **[!UICONTROL Copy JSON structure]**&#x200B;을(를) 선택하십시오. 이 작업은 JSON 구조를 클립보드에 복사합니다. 그런 다음 내보낸 JSON을 사용하여 스키마 및 관련 리소스를 다른 샌드박스 또는 조직으로 가져올 수 있습니다. 이를 통해 서로 다른 환경 간에 스키마를 간단하고 효율적인 방식으로 공유하고 재사용할 수 있습니다.

## 다음 단계 및 추가 리소스

스키마 작성을 완료했으므로 이제 캔버스에서 전체 스키마를 볼 수 있습니다. **[!UICONTROL Save]**&#x200B;을(를) 선택하면 스키마가 [!DNL Schema Library]에 저장되므로 [!DNL Schema Registry]에서 액세스할 수 있습니다.
...
>[!VIDEO](https://video.tv.adobe.com/v/3430221?captions=kor&quality=12&learn=on)

다음 비디오는 현장 그룹 및 수업에 대한 이해를 강화하기 위한 것입니다.

>[!VIDEO](https://video.tv.adobe.com/v/3413604?captions=kor&quality=12&learn=on)
