...
관계에 정의될 두 개의 스키마를 이미 생성한 것으로 예상됩니다. 이 자습서는 조직의 현재 로열티 프로그램(&quot;[!DNL Loyalty Members]&quot; 스키마에 정의됨)의 멤버와 자주 사용하는 호텔(&quot;[!DNL Hotels]&quot; 스키마에 정의됨) 간의 관계를 만듭니다.
...
>[!NOTE]
>
>헤더는 결과 스키마의 제목, ID 및 버전만 [!DNL Accept] `application/vnd.adobe.xed-id+json` 반환합니다.
...
관계 설명자는 [!DNL Schema Registry]관계형 데이터베이스 테이블의 외래 키와 비슷하게 작동합니다.소스 스키마의 필드는 대상 스키마의 기본 ID 필드에 대한 참조 역할을 합니다. 소스 스키마에 이 용도로 사용할 필드가 없으면 새 필드와 혼합을 만들어 스키마에 추가해야 할 수 있습니다. 이 새 필드에는 &quot; `type` 값&quot;이 있어야[!DNL string]합니다.
...
다음 요청은 &quot;[!DNL Favorite Hotel]&quot; 스키마에 &quot;[!DNL Loyalty Members]&quot; 혼합을 추가합니다.
...
| `xdm:identityNamespace` | 참조 필드의 ID 네임스페이스입니다. 이 네임스페이스는 스키마의 기본 ID로 필드를 정의할 때 사용되는 네임스페이스와 동일해야 합니다. 자세한 내용은 [ID 네임스페이스 개요를](../../identity-service/home.md) 참조하십시오. |
...
다음 요청은 &quot;[!DNL Loyalty Members]&quot;를 소스 스키마로, &quot;[!DNL Legacy Loyalty Members]&quot;를 대상 스키마로 사용하여 새 관계 설명자를 만듭니다.
...
| `xdm:sourceProperty` | 소스 스키마의 참조 필드 경로입니다. |
| `xdm:destinationSchema` | 대상 스키마의 `$id` URL. |
| `xdm:destinationVersion` | 대상 스키마의 버전 번호입니다. |
| `xdm:destinationProperty` | 대상 스키마의 참조 필드 경로입니다. |
...
이 튜토리얼을 따라 두 스키마 간의 1:1 관계를 성공적으로 만들었습니다. API를 사용한 설명자 작업에 대한 자세한 내용은 [!DNL Schema Registry] 스키마 레지스트리 개발자 안내서를 참조하십시오 [](../api/descriptors.md). UI에서 스키마 관계를 정의하는 방법에 대한 단계는 스키마 편집기를 사용하여 스키마 관계를 [정의하는 자습서를 참조하십시오](relationship-ui.md).
