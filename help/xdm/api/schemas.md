...
스키마는 Adobe Experience Platform에 인제스트할 데이터의 청사진이라고 생각할 수 있습니다. 각 스키마는 클래스와 스키마 필드 그룹이 0개 이상 있는 그룹으로 구성됩니다. [!DNL Schema Registry] API의 `/schemas` 종단점을 사용하면 경험 애플리케이션 내의 스키마를 프로그래밍 방식으로 관리할 수 있습니다.
...
>[!NOTE]
>
>아래 예제 호출은 클래스의 최소 구성 요구 사항과 필드 그룹이 없는 API에서 스키마를 만드는 방법에 대한 기본 예제일 뿐입니다. 필드 그룹 및 데이터 유형을 사용하여 필드를 할당하는 방법을 포함하여 API에서 스키마를 만드는 방법에 대한 전체 단계는 [스키마 만들기 자습서](../tutorials/create-schema-api.md)를 참조하십시오.
...
| 속성 | 설명 |
| --- | --- |
| `allOf` | 각 객체가 스키마가 구현하는 필드나 클래스가 참조하는 클래스 또는 필드 그룹을 참조하는 객체의 배열입니다. 각 객체에는 새 스키마가 구현할 클래스 또는 필드 그룹의 `$id`을 나타내는 값이 있는 단일 속성(`$ref`)이 포함되어 있습니다. 0개 이상의 추가 필드 그룹과 함께 하나의 클래스를 제공해야 합니다. 위의 예에서 `allOf` 배열의 단일 객체는 스키마의 클래스입니다. |
...
가장 일반적인 PATCH 작업 중 하나는 아래 예와 같이 이전에 정의한 필드 그룹을 스키마에 추가하는 것입니다.
...
아래 예제 요청은 `meta:extends` 배열과 `allOf` 배열 모두에 해당 필드 그룹의 `$id` 값을 추가하여 스키마에 새 필드 그룹을 추가합니다.
...
이 응답에는 두 작업이 성공적으로 수행되었음을 보여줍니다. 필드 그룹 `$id`이(가) `meta:extends` 배열에 추가되었고 이제 필드 그룹 `$id`에 대한 참조(`$ref`)가 `allOf` 배열에 표시됩니다.
