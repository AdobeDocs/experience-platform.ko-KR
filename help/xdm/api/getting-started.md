...
이 [!DNL Schema Registry] 는 Adobe Experience Platform 내의 스키마 라이브러리에 액세스하여 사용 가능한 모든 라이브러리 리소스에 액세스할 수 있는 사용자 인터페이스와 RESTful API를 제공합니다.

스키마 레지스트리 API를 사용하여 Adobe Experience Platform 내에서 사용 가능한 모든 스키마 및 관련 리소스를 보고 관리하기 위해 기본 CRUD 작업을 수행할 수 있습니다. 여기에는 애플리케이션을 사용하는 Adobe, 파트너 [!DNL Experience Platform] 및 공급업체에서 정의한 애플리케이션이 포함됩니다. 또한 API 호출을 사용하여 조직의 새 스키마 및 리소스를 만들고, 이미 정의한 리소스를 보고 편집할 수도 있습니다.
...
사용자에 대한 모든 조회(GET) 요청에는 추가 수락 헤더가 [!DNL Schema Registry] 필요합니다. 이 헤더 값은 API에서 반환되는 정보의 형식을 결정합니다. 자세한 내용은 [아래의 헤더](#accept) 수락 섹션을 참조하십시오.
...
글로벌 컨테이너에는 모든 표준 Adobe 및 [!DNL Experience Platform] 파트너가 제공하는 클래스, 믹싱, 데이터 유형 및 스키마가 들어 있습니다. 글로벌 컨테이너에 대해서만 목록 및 조회(GET) 요청만 수행할 수 있습니다.
