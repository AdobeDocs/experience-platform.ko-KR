...
테넌트 컨테이너의 모든 스키마를 나열하기 위한 GET 요청을 수행하면 이제 속성 정보 스키마가 포함되거나 URL 인코딩 URI를 사용하여 조회(GET) 요청을 수행하여 새 스키마를 직접 볼 수 있습니다 `$id` . 모든 조회 요청에 대한 수락 헤더 `version` 에 해당 항목을 포함해야 합니다.
