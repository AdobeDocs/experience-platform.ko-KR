...
>[!NOTE]
>
>개별 필드를 업데이트하지 않고 전체 리소스를 새 값으로 대체하려면 PUT 작업을 사용하여 리소스를 [대체하는 방법을 참조하십시오](replace-resource.md).
...
PATCH 작업을 사용하여 스키마를 업데이트하여 이전에 만든 믹싱에 정의된 필드를 포함할 수 있습니다. 이 작업을 수행하려면 스키마 `meta:altId` 또는 URL 인코딩 URI를 사용하여 PATCH 요청을 `$id` 수행해야 합니다.
...
스키마 레지스트리 리소스 내의 개별 필드에 여러 항목을 변경하는 PATCH 요청을 보낼 수도 있습니다.
