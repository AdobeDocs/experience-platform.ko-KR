...
title: 개인 정보/개인화/마케팅 기본 설정(동의) 스키마 필드 그룹
topic-legacy: overview
description: 이 문서에서는 개인 정보/개인화/마케팅 기본 설정(동의) 스키마 필드 그룹의 개요를 제공합니다.
...
# [!UICONTROL Privacy/Personalization/Marketing Preferences (Consents)] 필드 그룹

[!UICONTROL Privacy/Personalization/Marketing Preferences (Consents)] (이하  [!DNL Privacy & Consents] 필드 그룹&quot;이라 한다)은 고객의 동의 및  [[!DNL XDM Individual Profile] 선호도 정보를 캡처하는 데 사용되는 ](../../classes/individual-profile.md)클래스의 표준필드 그룹입니다.

>[!NOTE]
>
>이 필드 그룹은 [!DNL XDM Individual Profile]와만 호환되므로 [!DNL XDM ExperienceEvent] 스키마에 사용할 수 없습니다. 경험 이벤트 스키마에 동의 및 기본 설정 데이터를 포함하려면 [사용자 지정 필드 그룹](../../ui/resources/field-groups.md#create)을 대신 사용하여 [[!UICONTROL Consent for Privacy, Personalization and Marketing Preferences] 데이터 유형](../../data-types/consents.md)을 스키마에 추가합니다.

## 필드 그룹 구조 {#structure}

>[!IMPORTANT]
>
>[!DNL Consents & Preferences] 필드 그룹은 동의 및 기본 설정 관리 사용 사례 범위를 포함하도록 설계되었습니다. 그 결과, 이 문서에서는 필드 그룹의 필드를 일반 용어로 사용하는 방법에 대해 설명하고 이러한 필드의 사용을 해석하는 방법에 대한 제안만 합니다. 귀하의 조직이 이러한 동의 및 선호 사항을 어떻게 해석하고 고객에게 제시할 것인지에 따라 필드 그룹의 구조를 정렬하려면 개인 정보 법률 팀과 상의하십시오.

[!DNL Consents & Preferences] 필드 그룹은 **consent** 및 **preference** 정보를 캡처하는 데 사용되는 여러 필드를 제공합니다.
...
다음 스크린샷은 플랫폼 UI에서 필드 그룹의 구조가 표시되는 방식을 보여줍니다.
...
다음 JSON은 [!DNL Consents & Preferences] 필드 그룹이 처리할 수 있는 데이터 유형의 예를 보여줍니다. 이러한 각 필드의 특정 사용에 대한 정보는 다음 섹션에 제공됩니다.
...
`adID` 동의는 광고주 ID(IDFA 또는 GAID)를 사용하여 이 장치의 앱 간에 고객을 연결할 수 있는지 여부에 대한 고객의 동의를 나타냅니다. 이 값은 `idSpecific` 섹션의 `ECID` ID 네임스페이스 아래에서만 구성할 수 있으며 다른 네임스페이스나 이 필드 그룹의 사용자 수준에 대해 설정할 수 없습니다.
...
## 필드 그룹 {#ingest}을(를) 사용하여 데이터 인제스트

[!DNL Consents & Preferences] 필드 그룹을 사용하여 고객의 동의 데이터를 인제스트하려면 해당 필드 그룹을 포함하는 스키마를 기반으로 데이터 세트를 만들어야 합니다.

필드 그룹을 필드에 할당하는 방법에 대한 자세한 내용은 [UI](http://www.adobe.com/go/xdm-schema-editor-tutorial-en)에서 스키마 만들기에 대한 자습서를 참조하십시오. 필드가 포함된 [!DNL Consents & Preferences] 필드 그룹을 만들고 나면 데이터 세트 사용자 안내서의 [데이터 세트](../../../catalog/datasets/user-guide.md#create)를 만드는 섹션에 있는 섹션을 참조하고 기존 스키마를 사용하여 데이터 세트를 만드는 단계를 따릅니다.

>[!IMPORTANT]
>
>동의 데이터를 [!DNL Real-time Customer Profile]에 보내려면 [!DNL Consents & Preferences] 필드 그룹이 포함된 [!DNL XDM Individual Profile] 클래스를 기반으로 [!DNL Profile] 사용 스키마를 만들어야 합니다. 해당 스키마를 기반으로 만드는 데이터 집합도 [!DNL Profile]에 대해 활성화되어 있어야 합니다. 스키마 및 데이터 집합에 대한 [!DNL Real-time Customer Profile] 요구 사항과 관련된 특정 단계는 위에 연결된 자습서를 참조하십시오.
>
...
[!DNL Consents & Preferences] 필드 그룹에 대한 전체 스키마를 보려면 [공식 XDM 리포지토리](https://github.com/adobe/xdm/blob/master/components/datatypes/consent-preferences.schema.json)를 참조하십시오.
