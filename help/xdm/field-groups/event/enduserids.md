...
[!UICONTROL End User ID Details]은(는) 여러 Adobe 응용 프로그램에서 개인의 ID 정보를 설명하는 데 사용되는 [[!DNL XDM ExperienceEvent] class](../../classes/experienceevent.md)에 대한 표준 스키마 필드 그룹입니다. 필드 그룹은 루트 수준 `endUserIDs` 개체를 제공합니다. 이 개체에는 데이터가 수집될 때 값이 자동으로 업데이트되는 읽기 전용 `_experience` 필드가 포함되어 있습니다.
...
| `adcloud` | [ID](../../data-types/identity.md) | Adobe Advertising의 최종 사용자 ID. |
