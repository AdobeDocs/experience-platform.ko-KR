...
[데이터 유형은](./schema/composition.md#data-type) 스키마에 대해 하나 이상의 필드를 제공할 수도 있습니다. 그러나 믹스와는 달리 데이터 유형은 특정 클래스에 한정되지 않습니다. 따라서 데이터 유형이 보다 유연한 옵션을 사용하여 여러 개의 스키마에서 재사용 가능한 일반적인 데이터 구조를 설명할 수 있습니다(잠재적으로 다른 클래스).
