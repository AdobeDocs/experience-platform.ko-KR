...
조직에서 정의한 리소스는 다른 업계 및 공급업체 리소스와 충돌을 방지하기 위해 테넌트 ID 아래에 해당 필드를 네임스페이스를 지정해야 합니다. 표준 필드 그룹을 사용하여 스키마를 작성할 때, 해당 필드 그룹의 구조 내에 추가하는 사용자 지정 필드도 테넌트 ID 아래에 지정해야 합니다.
