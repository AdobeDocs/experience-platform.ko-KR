...
SDK을 사용하면 기본 코드의 배열에 다른 이름을 추가하여 각 속성에 대해 별도의 인스턴스를 만들 수 있습니다. 다음 예제에서는 `titanium`과(와) `copper`의 두 이름을 제공합니다.
...
따라서 스크립트는 SDK의 두 인스턴스를 생성합니다. 첫 번째 인스턴스와 상호 작용하기 위한 전역 함수의 이름은 `titanium`이고 두 번째 인스턴스와 상호 작용하기 위한 전역 함수의 이름은 `copper`입니다.
