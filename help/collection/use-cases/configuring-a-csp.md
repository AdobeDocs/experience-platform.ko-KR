...
위의 예에서 `EDGE-DOMAIN`은(는) 자사 도메인으로 대체해야 합니다. [edgeDomain](../js/commands/configure/edgedomain.md) 설정에 대해 자사 도메인이 구성되어 있습니다. 구성된 퍼스트 파티 도메인이 없으면 `EDGE-DOMAIN`을(를) `*.adobedc.net`(으)로 바꿔야 합니다. [idMigrationEnabled](../js/commands/configure/idmigrationenabled.md)을(를) 사용하여 방문자 마이그레이션이 설정된 경우 `connect-src` 지시문에도 `*.demdex.net`이(가) 포함되어야 합니다.
