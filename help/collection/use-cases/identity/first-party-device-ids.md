...
>[!IMPORTANT]
>
>자사 장치 ID는 Web SDK의 [타사 쿠키](/help/tags/extensions/client/web-sdk/configure/identity.md) 기능과 호환되지 않습니다. 자사 디바이스 ID 또는 타사 쿠키를 동시에 사용할 수는 없습니다.

## 전제 조건 {#prerequisites}

시작하기 전에 ECID 및 `identityMap`을(를) 포함하여 웹 SDK에서 ID 데이터가 작동하는 방식을 잘 알고 있는지 확인하십시오. 자세한 내용은 웹 SDK의 [ID 데이터에 대한 개요](id-overview.md)를 참조하십시오.
...
`HTTPOnly` 플래그를 사용하여 설정된 쿠키는 클라이언트측 스크립트를 사용하여 액세스할 수 없습니다. 즉, `HTTPOnly`을(를) 설정할 때 [!DNL FPID] 플래그를 설정하는 경우 `identityMap`에 포함할 쿠키 값을 읽으려면 서버측 스크립팅 언어를 사용해야 합니다.
...
[!DNL ECID]과(와) [!DNL FPID]이(가) 모두 있으면 [!DNL ECID]이(가) 사용자를 식별하는 우선 순위를 갖습니다. 이렇게 하면 기존 [!DNL ECID]이(가) 브라우저 쿠키 저장소에 있는 경우 기본 식별자로 유지되며 기존 방문자 수가 영향을 받지 않습니다. 기존 사용자의 경우 [!DNL FPID]이(가) 만료되거나 브라우저 정책 또는 수동 프로세스의 결과로 삭제될 때까지 [!DNL ECID]이(가) 기본 ID가 되지 않습니다.

ID는 다음 순서로 우선 순위가 지정됩니다.

1. [!DNL ECID]에 포함된 `identityMap`
1. [!DNL ECID]이(가) 쿠키에 저장됨
1. [!DNL FPID]에 포함된 `identityMap`
...
| 방문 | 설명 |
| --- | --- |
| 첫 방문 | [!DNL FPID] 쿠키 설정을 아직 시작하지 않았다고 가정해 보십시오. [!DNL ECID]AMCV 쿠키[에 포함된 &#x200B;](https://experienceleague.adobe.com/docs/id-service/using/intro/cookies.html?lang=ko#section-c55af54828dc4cce89f6118655d694c8)은(는) 방문자를 식별하는 데 사용되는 식별자입니다. |
...
* **[메서드 2](#identityMap)**: ID 맵에 [!DNL FPID]을(를) 포함합니다. 자세한 내용은 이 문서의 아래쪽에서 [에서 FPID 사용 `identityMap`](#identityMap)에 대한 섹션을 참조하십시오.
...
웹 사이트 `example.com`에서 Web SDK을 구현하는 것이 좋습니다. 웹 SDK이 데이터를 Edge Network으로 `edge.adobedc.net` 도메인으로 보냅니다.

| [!DNL CNAME] 없이 | [!DNL CNAME] 사용 |
|---------|----------|
| <ul><li>웹 사이트 `example.com`은(는) 웹 SDK 도메인 `edge.adobedc.net`을(를) 사용하여 Edge Network에 데이터를 보냅니다.</li><li>`edge.adobedc.net`에서 설정한 쿠키는 `example.com` 도메인에서 가져오지 않으므로 타사 쿠키로 간주됩니다. 사용자 브라우저에 따라 서드파티 쿠키가 차단될 수 있으며, 귀하의 데이터가 Edge Network에 도달하지 않습니다.</li></ul> | <ul><li>`metrics.example.com`과(와) 같이 웹 SDK을 배포하는 하위 도메인을 만듭니다.</li><li>[!DNL CNAME]이(가) `metrics.example.com`을(를) 가리키도록 DNS 시스템에서 `edge.adobedc.net` 레코드를 설정했습니다.</li><li>웹 사이트에서 `metrics.example.com`을(를) 통해 브라우저에 쿠키를 설정하면 `example.com`(타사) 대신 `edge.adobedc.net`(자사)에서 쿠키가 제공되는 것으로 표시됩니다. 이렇게 하면 자사 ID 쿠키가 차단될 가능성이 줄어들어 보다 정확한 데이터 수집이 보장됩니다.</li></ul> |
...
데이터스트림을 설정하는 방법에 대한 자세한 내용은 [데이터스트림 구성 설명서](/help/datastreams/configure.md#advanced-options)를 참조하세요.
...
><br> 자사 ID를 사용하는 경우 Audience Manager 파트너 ID 동기화가 대부분 [&#x200B; 또는 &#x200B;](https://experienceleague.adobe.com/ko/docs/audience-manager)을(를) 기반으로 하는 경우 파트너 플랫폼에서 활성화되도록 타깃팅된 `UUIDs`Audience Manager`DIDs` 기능이 지원되지 않습니다. 자사 ID에서 파생된 [!DNL ECID]이(가) `UUID`에 연결되어 있지 않으므로 주소를 지정할 수 없습니다.

## 방법 2: `identityMap`에서 FPID 사용 {#identityMap}

[!DNL FPID]을(를) 자신의 쿠키에 저장하는 대신 ID 맵을 통해 [!DNL FPID]을(를) Edge Network으로 보낼 수 있습니다.

다음은 [!DNL FPID]에서 `identityMap`을(를) 설정하는 방법의 예입니다.
...
다른 ID 형식과 마찬가지로 [!DNL FPID] 내에 다른 ID와 함께 `identityMap`을(를) 포함할 수 있습니다. 다음은 인증된 [!DNL FPID]에 포함된 [!DNL CRM ID]의 예입니다.
...
다음 `identityMap`은(는) `primary`에 대한 [!DNL FPID] 표시기가 없으므로 Edge Network에서 오류 응답을 생성합니다. `identityMap`에 있는 ID 중 적어도 하나는 `primary`(으)로 표시되어야 합니다.
