---
title: 디버깅 메서드
description: 웹 SDK에서 디버깅 기능을 전환하는 방법에 대해 알아봅니다.
...
디버깅이 활성화되면 웹 SDK은 구현 디버깅에 도움이 될 수 있는 메시지를 브라우저 콘솔에 출력합니다. 또한 태그를 구현 방법으로 사용하는 경우 [`_satellite.logger`](../tags/logger.md)개의 메시지를 볼 수 있습니다. 디버깅은 설정한 규칙 및 데이터 요소에 따라 SDK이 어떻게 동작하는지 이해하려는 경우 유용합니다.

디버깅은 기본적으로 비활성화되어 있지만 네 가지 방법으로 전환할 수 있습니다. 이러한 메서드를 조합하여 사용하면 개발 워크플로에서 가장 편리한 디버깅을 활성화하거나 비활성화할 수 있습니다.

## `debugEnabled` 명령에 `configure` 사용
...
자세한 내용은 [`debugEnabled`](../js/commands/configure/debugenabled.md)을(를) 참조하십시오.
...
자세한 내용은 [`setDebug`](../js/commands/setdebug.md) 명령을 참조하십시오.
...
Adobe Experience Platform Debugger은 웹 페이지를 검사하고 Experience Cloud 제품 구현을 디버깅하는 데 도움이 되는 강력한 도구입니다. AEP Web SDK 섹션의 구성 탭에서 디버깅을 활성화할 수 있습니다.

![디버거 사용](../js/assets/enable-debugging.png)
