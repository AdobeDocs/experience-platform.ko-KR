...
이 안내서에 언급된 다양한 데이터 수집 제품을 사용하려면 데이터스트림에 대한 액세스 권한과 스키마를 만들고 관리할 수 있는 기능도 있어야 합니다. 이러한 기능 중 하나에 액세스해야 하는 경우 CSM에 연락하여 필요한 액세스 권한을 받으십시오. Adobe Experience Platform을 구매하지 않은 경우 Adobe에서 추가 비용 없이 SDK를 사용하는 데 필요한 액세스 권한을 프로비저닝합니다.
