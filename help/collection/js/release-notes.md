...
- URL에 광고 관련 `s_kwcid` 또는 `ef_id` 매개 변수가 여러 개 있는 경우 웹 SDK이 충돌하는 문제를 해결했습니다.
- 동의하기 전에 Advertising 데이터가 전송되고 쿠키가 생성되던 문제를 수정했습니다.
- Safari에서 Brand Concierge 스트림을 올바르게 구문 분석하지 못하는 문제가 해결되었습니다.

## 버전 2.31.0 - 2026년 2월 9일

**새로운 기능**

- `"oneTimeAnalyticsReferrer"`의 가용성을 [`context`](commands/configure/context.md) 문자열 배열에 추가했습니다.
- Brand Concierge 구성 요소를 추가했습니다.
- 이벤트 생성과 보낸 시간 사이의 시간을 기록하기 위해 네트워크 요청에 `meta.queueTimeMillis`을(를) 추가했습니다.
- 후속 호출로 채울 수 있도록 ID 맵을 유지하는 기능.

**수정 사항 및 개선 사항**

- `aria-label` 및 `name` 특성은 이제 [자동 링크 컬렉션](commands/configure/clickcollectionenabled.md)에서 고려됩니다.
- ID 맵 통합으로 가능한 경합 조건을 수정했습니다.
- 타임스탬프가 `streamingMedia`에 포함되지 않던 문제를 수정했습니다.
- 사용자 지정 코드 작업이 한 번만 실행되던 문제를 수정했습니다.

## 버전 2.30.0 - 2025년 9월 24일
...
- 미디어 세션을 시작할 때 [데이터 스트림 무시](/help/datastreams/overrides.md)가 지원됩니다.
...
- [getIdentity](/help/collection/use-cases/identity/id-overview.md) 명령에서 [CORE ID](commands/getidentity.md) 요청에 대한 지원을 추가했습니다.
...
- [스트리밍 미디어 컬렉션](commands/configure/streamingmedia.md)에 대한 지원을 추가했습니다.
...
- [페이지 이벤트의 상단 및 하단](../use-cases/personalization/top-bottom-page-events.md)에 대한 지원을 추가했습니다.
...
- [`onBeforeLinkClickSend`](commands/configure/onbeforelinkclicksend.md) 링크 클릭 콜백에 대한 지원을 추가했습니다.
...
- [사용자 에이전트 클라이언트 힌트](../use-cases/client-hints.md) 속성을 잘못된 유형으로 설정할 수 있는 문제를 해결했습니다.
...
- 페이지 전체 마이그레이션별로 페이지에 대한 지원을 추가했습니다. 이제 Adobe Target 프로필은 방문자가 at.js와 웹 SDK 페이지 간을 이동할 때 보존됩니다.
- [높은 엔트로피 사용자 에이전트 클라이언트 힌트](../use-cases/client-hints.md)에 대한 구성 가능한 지원이 추가되었습니다.
- [`applyResponse`](commands/applyresponse.md) 명령에 대한 지원을 추가했습니다. 이렇게 하면 [Edge Network API](https://developer.adobe.com/data-collection-apis/docs/api/)를 통해 하이브리드 개인화가 가능합니다.
...
- Adobe Target 응답 토큰을 포함하여 `sendEvent` 확인된 약속에 더 많은 개인화 콘텐츠를 제공합니다. `sendEvent` 명령이 실행되면 약속이 반환되고 서버에서 받은 정보가 포함된 `result` 개체로 해결됩니다. 이전에는 이 결과 개체에 이름이 `decisions`인 속성이 포함되었습니다. 이 `decisions` 속성은 더 이상 사용되지 않습니다. 새 속성 `propositions`이(가) 추가되었습니다. 이 새 속성은 고객이 응답 토큰을 포함한 더 많은 개인화 콘텐츠에 액세스할 수 있도록 합니다.
...
- [`setConsent`](commands/setconsent.md) 명령에 대한 Adobe 동의 2.0 표준에 대한 지원이 추가되었습니다.
