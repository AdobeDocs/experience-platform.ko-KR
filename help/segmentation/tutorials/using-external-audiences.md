...
Adobe Experience Platform은 나중에 새 대상의 구성 요소로 사용할 수 있는 외부 대상을 가져오는 기능을 지원합니다. 이 문서에서는 외부 대상을 가져와 사용하도록 Experience Platform을 설정하는 방법에 대한 자습서를 제공합니다.
...
- [XDM(경험 데이터 모델)](../../xdm/home.md): Experience Platform에서 고객 경험 데이터를 구성하는 표준화된 프레임워크입니다. 세그먼테이션을 최대한 활용하려면 [데이터 모델링 모범 사례](../../xdm/schema/best-practices.md)에 따라 데이터가 프로필 및 이벤트로 수집되는지 확인하십시오.
- [데이터 세트](../../catalog/datasets/overview.md): Experience Platform의 데이터 지속성을 위한 저장소 및 관리 구성입니다.
- [스트리밍 수집](../../ingestion/streaming-ingestion/overview.md): Experience Platform에서 클라이언트 및 서버측 장치에서 실시간으로 데이터를 수집하고 저장하는 방법
...
외부 대상을 사용하는 첫 번째 단계는 ID 네임스페이스를 만드는 것입니다. ID 네임스페이스를 사용하면 Experience Platform에서 대상이 시작되는 위치를 연결할 수 있습니다.
...
이제 이 스키마가 프로필에 대해 활성화되며, 사용자가 만든 비개인 ID 네임스페이스에 기본 ID가 할당됩니다. 따라서 이 스키마를 사용하여 Experience Platform으로 가져온 세그먼트 메타데이터는 다른 사용자 관련 프로필 데이터와 병합되지 않고 프로필로 수집됩니다.
...
이제 데이터 세트가 활성화되면 UI를 통해 또는 Experience Platform API를 사용하여 Experience Platform으로 데이터를 전송할 수 있습니다. 일괄 처리 또는 스트리밍 연결을 통해 이 데이터를 수집할 수 있습니다.
...
이제 연결을 만든 후 데이터를 Experience Platform으로 수집할 수 있습니다.
...
가져온 외부 대상 메타데이터를 사용하고 세그먼트를 만드는 데 사용할 수 있을 뿐만 아니라, 외부 세그먼트 멤버십을 Experience Platform으로 가져올 수도 있습니다.
