...
에 GET 요청을 작성하여 조직의 모든 세그먼트 정의 목록을 검색할 수 있습니다 `/segment/definitions` 엔드포인트.
...
다음 요청은 조직 내에 게시된 마지막 두 세그먼트 정의를 검색합니다.
...
성공적인 응답은 지정된 조직의 세그먼트 정의 목록이 JSON인 HTTP 상태 200을 반환합니다.
...
| `description` | 만들고 있는 세그먼트 정의에 대한 설명입니다. |
| `evaluationInfo` | 만들려는 세그먼트 유형입니다. 배치 세그먼트를 만들려면 을 설정합니다 `evaluationInfo.batch.enabled` 참이라고 스트리밍 세그먼트를 만들려면 다음을 설정합니다 `evaluationInfo.continuous.enabled` 참이라고 에지 세그먼트를 만들려면 을 설정합니다. `evaluationInfo.synchronous.enabled` 참이라고 비워 두면 세그먼트가 **배치** 세그먼트. |
...
| `evaluationInfo` | 세그먼트 정의가 수행할 평가 유형을 나타내는 객체입니다. 일괄 처리, 스트리밍(연속이라고도 함) 또는 에지(동기식이라고도 함) 세그먼테이션일 수 있습니다. |
...
| `evaluationInfo` | 세그먼트 정의가 수행되는 평가, 일괄 처리, 스트리밍(연속이라고도 함) 또는 에지(동기어라고도 함)의 유형을 나타내는 개체입니다. |
