...
| `children.schedule` | 작업 일정을 포함하는 문자열입니다. 작업은 하루에 한 번만 실행되도록 예약할 수 있습니다. 즉, 24시간 동안 작업을 두 번 이상 실행하도록 예약할 수 없습니다. cron 일정에 대한 자세한 내용은 [cron 식 형식](#appendix)의 부록을 참조하십시오. 이 예에서 &quot;`0 0 1 * *`&quot;은(는) 이 일정이 매일 오전 1시에 실행됨을 의미합니다. |
...
| `schedule` | *선택 사항입니다.* 작업 일정이 포함된 문자열입니다. 작업은 하루에 한 번만 실행되도록 예약할 수 있습니다. 즉, 24시간 동안 작업을 두 번 이상 실행하도록 예약할 수 없습니다. cron 일정에 대한 자세한 내용은 [cron 식 형식](#appendix)의 부록을 참조하십시오. 이 예에서 &quot;`0 0 1 * *`&quot;은(는) 이 일정이 매일 오전 1시에 실행됨을 의미합니다. <br><br>이 문자열을 제공하지 않으면 시스템에서 생성한 일정이 자동으로 생성됩니다. |
...
`/config/schedules` 끝점에 GET을 요청하고 요청 경로에 검색할 일정의 ID를 제공하여 특정 일정에 대한 자세한 정보를 검색할 수 있습니다.
...
| `schedule` | 작업 일정을 포함하는 문자열입니다. 작업은 하루에 한 번만 실행되도록 예약할 수 있습니다. 즉, 24시간 동안 작업을 두 번 이상 실행하도록 예약할 수 없습니다. cron 일정에 대한 자세한 내용은 [cron 식 형식](#appendix)의 부록을 참조하십시오. 이 예에서 &quot;`0 0 1 * *`&quot;은(는) 이 일정이 매일 오전 1시에 실행됨을 의미합니다. |
...
`/config/schedules` 끝점에 대한 PATCH 요청을 만들고 요청 경로에 업데이트하려는 일정의 ID를 제공하여 특정 일정을 업데이트할 수 있습니다.
...
`/config/schedules` 끝점에 DELETE을 요청하고 요청 경로에 삭제하려는 일정의 ID를 제공하여 특정 일정의 삭제를 요청할 수 있습니다.
...
| `0 30 9 * * ? 2022` | 이벤트는 2022년에 매일 9:30AM에 실행됩니다. |
| `0 * 18 * * ?` | 이 이벤트는 매일 오후 6시부터 시작하여 6:59PM까지 매분마다 실행됩니다. |
| `0 0/10 17 * * ?` | 이 행사는 매일 오후 5시부터 오후 6시까지 10분 간격으로 시작됩니다. |
| `0 13,38 5 ? 6 WED` | 이벤트는 6월 매주 수요일 5:13AM 및 5:38AM에 실행됩니다. |
| `0 30 12 ? * 4#3` | 이벤트는 매월 세 번째 수요일에 12:30PM에 실행됩니다. |
| `0 30 12 ? * 6L` | 이벤트는 매월 마지막 금요일 12:30PM에 실행됩니다. |
| `0 45 11 ? * MON-THU` | 이 이벤트는 매주 월요일, 화요일, 수요일, 목요일에 11:45AM에 실행됩니다. |