...
>[!NOTE]
>
>스트리밍 세그먼테이션은 플랫폼으로 스트리밍되는 데이터를 평가하는 데에만 사용할 수 있습니다. 다시 말해, 일괄 처리를 통해 수집된 데이터는 스트리밍 세그먼테이션을 통해 평가되지 않고, 매일 밤 예약된 세그먼트화된 작업과 함께 평가됩니다.
...
### 스트리밍 세그먼테이션이 활성화된 쿼리 유형 {#streaming-segmentation-query-types}
...
세그먼트가 다음 중 하나와 일치하는 경우 자동으로 스트리밍이 활성화됩니다 [위에 나열된 스트리밍 세분화 유형](#streaming-segmentation-query-types).
...
| `schedule` | **(필수)** 작업 일정을 포함하는 문자열입니다. 작업은 하루에 한 번만 실행되도록 예약할 수 있습니다. 즉, 24시간 기간 동안 두 번 이상 실행하도록 작업을 예약할 수 없습니다. 예 (`0 0 1 * * ?`)은 작업이 매일 1에 트리거됨을 의미합니다:00:00 UTC입니다. 자세한 내용은 [cron 식 형식](http://www.quartz-scheduler.org/documentation/quartz-2.3.0/tutorials/crontrigger.html) 설명서. |
...
다음 요청에서는 을 사용합니다 [JSON 패치 형식](http://jsonpatch.com/) 업데이트하기 위해 `state` 일정 기준 `active`.
