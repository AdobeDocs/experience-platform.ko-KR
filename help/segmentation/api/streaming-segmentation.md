...
에 GET 요청을 하여 조직 내에서 스트리밍 세분화를 위해 활성화된 모든 세그먼트 목록을 검색할 수 있습니다. `/segment/definitions` 엔드포인트.
...
성공적인 응답은 스트리밍 세그먼테이션이 활성화된 조직의 세그먼트 배열을 반환합니다.
...
스트리밍 평가가 활성화되면 기준선을 만들어야 합니다. 그 후에는 항상 세그먼트가 최신 상태가 됩니다. 시스템에서 베이스라인을 자동으로 수행하려면 먼저 예약된 평가(예약된 세그먼테이션이라고도 함)를 활성화해야 합니다. 예약된 세그먼테이션을 사용하면 조직에서 반복 일정을 준수하여 세그먼트를 평가하기 위해 내보내기 작업을 자동으로 실행할 수 있습니다.
