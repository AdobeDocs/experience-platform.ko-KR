...
description: Adobe Experience Platform UI의 Segment Builder는 프로필 데이터 요소와 상호 작용할 수 있는 풍부한 작업 영역을 제공합니다. 작업 영역에서는 데이터 속성을 나타내는 데 사용되는 드래그 앤 드롭 타일과 같은 규칙을 작성하고 편집하기 위한 직관적인 컨트롤을 제공합니다.
...
| 같지 않음 | 지정한 값 제외 | 상태 **같지 않음** &quot;취소됨&quot; |
| 다음을 포함 | 텍스트에 지정된 값이 포함되어 있습니다 | 제품 이름 **포함** &quot;iPhone&quot; |
| 다음을 포함하지 않음 | 텍스트에 지정한 값이 없습니다. | 설명 **은(는) &quot;중단됨&quot;을 포함하지 않습니다** |
...
| 간격: | 지정한 값의 수는 **between**&#x200B;입니다. 이러한 값은 지정된 숫자의 **포함**&#x200B;입니다. | **25세에서 45세 사이** |
...
| 연산자 | 설명 | 예 |
| -------- | ----------- | ------- |
| 다음과 같음 | 부울 값을 지정된 값(True 또는 False)으로 설정합니다. | 전자 메일 옵트인 **True** |
...
| 연산자 | 설명 | 예 |
| -------- | ----------- | ------- |
| 다음과 같음 | 값이 지정된 열거형 값과 같습니다. | 성별 **같음** 여성 |
...
| 기간 | 선택한 월 또는 연도 내에서 값이 발생했습니다. | 3월 동안 **is** 할인 |
| (+/-) 이내 | 값은 선택한 날짜로부터 일, 주, 월 또는 년 이내에 발생했습니다. 이 기간은 두 날짜의 **포함**&#x200B;입니다. | 장바구니 포기는 **3일 이내** |
| 다음 이전 | 값이 선택한 날짜 이전에 발생했습니다. | 2025년 1월 3일 회원가입일은 **이전**&#x200B;입니다. |
| 후 | 값이 선택한 날짜 이후에 발생했습니다. | 구매 날짜는 2024년 3월 14일 **이후**&#x200B;입니다. |
| 롤링 범위 | 값이 두 상대적 날짜 사이에 발생했습니다. | 마지막 구매 날짜는 7일 전부터 3일 전까지입니다. |
| 다음 간격 | 값이 선택한 다음 기간 내에 발생했습니다. | 2일 후 장바구니 포기 |
...
계산된 속성은 집계나 공식을 사용하여 다른 속성에서 계산된 필드입니다. 여러 이벤트에서 합계, 개수 또는 평균과 같은 합산 데이터가 필요하거나 복잡한 계산이 필요한 자주 사용하는 대상자를 빌드하는 경우 계산된 속성을 사용할 수 있습니다.

계산 특성을 만드는 방법, 특성 내에서 사용할 수 있는 함수 및 특성을 관리하는 방법을 포함하여 계산 특성에 대한 자세한 내용은 [계산 특성 개요](/help/profile/computed-attributes/overview.md)를 참조하십시오.

### 이벤트

**[!UICONTROL Events]** 탭을 사용하면 [!DNL XDM ExperienceEvent] 데이터 요소를 사용하여 발생한 이벤트나 동작을 기반으로 대상을 만들 수 있습니다. **[!UICONTROL Events]** 탭에서도 이벤트 유형을 찾을 수 있습니다. 이벤트 유형은 세그먼트 정의를 더 빨리 만들 수 있도록 일반적으로 사용되는 이벤트 모음입니다.
...
[Lucene의 검색 구문](https://docs.microsoft.com/en-us/azure/search/query-lucene-syntax)을 사용하는 검색 창에 구성 요소의 이름을 입력하여 모든 유형의 구성 요소를 검색할 수 있습니다. 전체 단어를 입력하면 검색 결과가 채워지기 시작합니다. 예를 들어 XDM 필드 `ExperienceEvent.commerce.productViews`을(를) 기반으로 규칙을 작성하려면 검색 필드에 &quot;product views&quot;를 입력합니다. &quot;product&quot; 단어를 입력하면 검색 결과가 나타납니다. 각 결과에는 해당 결과가 속한 객체 계층 구조가 포함됩니다.

>[!NOTE]
>
>조직에서 정의한 사용자 정의 스키마 필드가 나타나고 규칙을 작성하는 데 사용할 수 있게 되기까지 최대 24시간이 걸릴 수 있습니다.
...
| -------- | ----------------------- | ----------------------------- | ---------------------------- |
| 단일 보고서 세트 | 친숙한 이름 설명자는 일반 변수에 포함됩니다. <br><br>**예:** 페이지 이름(eVar2) | <ul><li>일반 변수에 포함된 이름 설명자</li><li>쿼리는 특정 데이터 집합의 데이터만 사용하므로</li></ul> | 쿼리는 Adobe Analytics 데이터 및 잠재적으로 다른 소스를 사용할 수 있습니다. |
...
![특성이 될 수 있는 열거형 값의 목록을 표시하는 이미지입니다.](../images/ui/segment-builder/enum-list.png)

이 열거형 목록에서 값을 선택하면 값이 실선 테두리로 윤곽선으로 표시됩니다. 그러나 `meta:enum`(소프트) 열거형을 사용하는 필드의 경우 열거형 목록에서 **아님** 값을 선택할 수도 있습니다. 사용자 고유의 값을 만들면 이 값이 열거형 목록에 없다는 경고와 함께 점선 테두리가 표시됩니다.
...
### 대상 그룹 유효성 검사 {#audience-validation}

세그먼트 빌더는 대상 정의를 자동으로 분석 및 확인하여 대상 정의 모범 사례를 준수하도록 합니다. 이러한 Best Practice는 중요한 검증 및 성능 최적화 두 가지 범주로 설정할 수 있습니다.
...
이제 합계가 추가되었습니다. 이제 합산 함수를 선택하고 합산할 속성, 같음 함수 및 값을 선택할 수 있습니다. 아래 예의 경우, 이 세그먼트 정의는 각 개별 구매가 $100 미만이더라도 $100보다 큰 구매 값의 합계가 있는 모든 프로필에 자격을 부여합니다.

![집계 함수를 표시하는 이벤트 규칙](../images/ui/segment-builder/filled-aggregation.png)
...
| 다음 이전 | 비교 중인 특성 또는 이벤트는 선택한 날짜 이전에 발생합니다&#x200B;**필수**. 선택한 날짜는 선택한 사용자 정의 날짜이거나 일, 주, 월 또는 년 전의 선택 날짜일 수 있습니다. | 예 | ![사용 중인 &quot;Before&quot; 시간 제약 조건의 예입니다.](../images/ui/segment-builder/time-constraints/before.png){width="100" zoomable="yes"} |
...
관객은 자신이 나열되는 순서대로 평가된다. 컨테이너를 사용하면 중첩 쿼리를 사용하여 실행 순서를 제어할 수 있습니다.
...
**[!UICONTROL Unwrap container]**&#x200B;을(를) 선택하면 자식 컨테이너가 제거되고 조건이 인라인으로 표시됩니다.
