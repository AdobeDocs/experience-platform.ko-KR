...
Audience Portal은 Adobe Experience Platform 내의 중앙 허브로, 대상을 보고, 관리하고, 만들 수 있습니다.

Audience Portal 내에서 다음 작업을 수행할 수 있습니다.
...
- [대상자에 대한 세부 사항 보기](#audience-details)
...
기본적으로 Audience Portal에는 프로필 수, 원본, 만든 날짜, 마지막 수정 날짜, 태그 및 분류 등 조직 및 샌드박스의 모든 대상 목록이 표시됩니다.

![찾아보기 화면이 표시됩니다. 조직에 속하는 모든 대상의 목록이 표시됩니다.](../images/ui/audience-portal/audience-browse.png)
...
>[!IMPORTANT]
>
>대상자 삭제하기 전에 대상자 **가 계정 기반 대상자에서 구성 요소로 사용되거나 Journey Optimizer에서 사용되지 않** 는지 확인하십시오Adobe Systems.
...
팝오버가 **[!UICONTROL Manage tags]** 나타납니다. 이 팝오버에서 분류된 태그 또는 분류되지 않은 태그를 선택할 수 있습니다.

| 태그 유형 | 설명 |
| -------- | ----------- |
| 분류 | 조직의 관리자가 만들고 관리하는 태그. |
| 미분류 | [!UICONTROL Manage tags] 팝오버 내에 만들어진 태그입니다. 누구나 이러한 유형의 태그를 만들거나 관리할 수 있습니다. |

![팝오버가 [!UICONTROL Manage tags] 표시됩니다. 분류됨 또는 분류되지 않음을 선택하는 옵션이 강조 표시됩니다.](../images/ui/audience-portal/create-tag.png)

대상자 목록에 첨부할 태그를 모두 추가한 후 을 선택합니다 **[!UICONTROL Save]**.
...
| [!UICONTROL Created by] | 대상자 작성자를 기준으로 필터링할 수 있습니다. |
| [!UICONTROL Creation date] | 대상자 작성 날짜를 기준으로 필터링할 수 있습니다. 날짜 범위를 선택하여 대상자를 만든 시점을 필터링할 수 있습니다. |
| [!UICONTROL Modified date] | 대상자 중 마지막으로 수정한 날짜를 기준으로 필터링할 수 있습니다. 날짜 범위를 선택하여 대상자가 마지막으로 수정된 시기를 필터링할 수 있습니다. |

![사용 가능한 필터가 대상 찾아보기 페이지 에 표시되고 강조 표시됩니다.](../images/ui/audience-portal/filter-audiences.png)
...
![대상자 세부 사항 페이지 페이지가 표시됩니다. 대상자 요약, 대상자 합계 및 활성화된 대상 카드가 강조 표시됩니다.](../images/ui/audience-portal/audience-details-summary.png)

### 대상자 요약 {#audience-summary}

이 **[!UICONTROL Audience summary]** 섹션에서는 속성의 ID, 이름, 설명, 출처 및 세부 정보와 같은 정보를 제공합니다.

또한 대상에 대한 대상자를 활성화하거나, 액세스 레이블을 적용하거나, 대상자를 편집/업데이트할 수 있는 옵션이 제공됩니다.

선택하면 **[!UICONTROL Activate to destination]** 대상에 대한 대상자 활동을 활성화할 수 있습니다. 대상을 대상으로 활성화하는 방법에 대한 자세한 내용은 [활성화 개요](../../destinations/ui/activation-overview.md)를 참조하십시오.
...
이 섹션에는 **[!UICONTROL Activated destinations]** 이 대상자 활동이 활성화되는 대상이 표시됩니다.
...
그 아래에는 세그먼트 자격이 있는 프로필의 샘플링이 있으며 ID, 이름, 성 및 개인 이메일을 포함한 [!DNL Profile] 세부 정보가 있습니다.
...
스캔의 샘플 크기는 프로필 스토어 의 전체 엔티티 수에 따라 다릅니다. 이러한 샘플 크기는 다음 표에 나와 있습니다.
...
각각 [!DNL Profile] 에 대한 자세한 정보는 ID를 [!DNL Profile] 선택하여 볼 수 있습니다. 프로필의 세부 정보에 대한 자세한 내용은 [[!DNL Real-Time Customer Profile] 사용 안내서](../../profile/ui/user-guide.md#profile-detail)를 참조하세요.
...
![모든 대상 예약 토글이 대상 포털에서 강조 표시됩니다.](../images/ui/audience-portal/browse-audiences-scheduled.png)

## 대상자 만들기 {#create-audience}

대상자 생성을 선택할 **[!UICONTROL Create audience]** 수 있습니다.

![대상 찾아보기 페이지에서 대상자 만들기 버튼이 강조 표시됩니다.](../images/ui/audience-portal/browse-create-audience.png)

팝오버가 나타나 대상자 작성 또는 규칙 작성 중에서 선택할 수 있습니다.
...
선택하면 **[!UICONTROL Build rule]** 세그먼트 빌더로 이동합니다. 이 작업 영역은 데이터 속성을 나타내는 데 사용되는 드래그 앤 드롭 타일과 같은 세그먼트 정의를 작성하고 편집하기 위한 직관적인 컨트롤을 제공합니다. 세그먼트 정의 생성에 [대한 자세한 내용은 세그먼트 빌더 안내서](./segment-builder.md)

![세그먼트 빌더 작업 영역 이 표시됩니다.](../images/ui/audience-portal/segment-builder.png)
...
>title="데이터 만료"
>abstract="대상자 멤버십이 만료되는 기간(일)입니다. 이 값은 1일에서 90일까지 설정할 수 있습니다. 대상자 멤버십이 만료되면 평가가 중지되고 모든 프로필이 대상자 멤버십을 종료합니다. 멤버십 기간이 7일 미만 남았대상자 경우 만료 창을 새로 고칠 수 있습니다. 그러나 0일이 지나면 만료 창을 새로 고칠 수 없습니다."
...
![대상 찾아보기 페이지에서 가져오기 대상자 버튼이 강조 표시됩니다.](../images/ui/audience-portal/browse-import-audience.png)
...
외부에서 생성된 대상자 항목을 가져올 때 열 중 하나를 기본 ID 필드로 선택하고 네임스페이스 값을 지정해야 합니다. 나머지 모든 필드는 페이로드 속성으로&#x200B;**간주됩니다**. 이러한 특성은 개인화 **목적으로만 이 대상자와 연결되고 프로파일에 연결되지 않으므로** 비내구성&#x200B;**으로 간주됩니다**.

![[!UICONTROL Audience details] 페이지 표시됩니다.](../images/ui/audience-portal/import-audience-audience-details.png)

필요에 따라 외부 ID 제공, 병합 정책 정의, 열 데이터 형식 편집 또는 사용자 지정 데이터 만료 적용을 포함하여 외부에서 생성된 대상자에 몇 가지 세부 정보를 추가할 수도 있습니다.
