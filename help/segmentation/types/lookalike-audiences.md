...
- 기본 대상 그룹 **must**&#x200B;이(가) 기본 병합 정책에 있습니다.
- 기본 대상 **필수**&#x200B;은(는) 데이터 거버넌스에 의해 제한된 필드를 사용하지 않습니다.
...
이때 유사 모델은 24시간마다 실행되며, 기본 대상에 대해 영향력 있는 요인 및 유사성 그래프를 만들고 다시 생성합니다. 닮은꼴 관객을 위한 득점 역시 빈번하게 진행된다.
...
>[!NOTE]
>
>영향요인은 중요도 순으로 정렬되어 서로 독립적이다.

| 필드 | 설명 |
| ----- | ----------- |
| 유형 | 영향력 있는 요인이 파생되는 데이터의 유형입니다. 프로필 속성, 경험 이벤트 또는 사용자 멤버십일 수 있습니다. |
| 키 | 데이터 필드의 이름입니다. 대상 그룹 멤버 자격 유형의 키에서 이 값은 데이터를 받는 대상의 **네임스페이스**&#x200B;을(를) 나타냅니다. 가능한 값은 `ups`(세그먼테이션 서비스) 및 `AO`(Audience Orchestration)입니다. 다른 유형의 키의 경우 이 값은 XDM 필드 경로를 나타냅니다. 예를 들어 회사 Luma에 income이라는 사용자 지정 필드가 있는 경우 키는 `_luma.income`입니다. |
...
>[!IMPORTANT]
>
>**다른 유사 대상을 위한 기본 대상으로 유사 대상을 사용할 수 없습니다**. 즉, **연결된 닮은 외모를 만들 수 없습니다**.

닮음 청중을 만들려면 닮음 청중의 기반이 될 청중을 선택해야 합니다. 사용 가능한 대상 그룹 목록에 액세스하려면 왼쪽 탐색 막대에서 **[!UICONTROL Audiences]**&#x200B;을(를) 선택한 다음 **[!UICONTROL Browse]**&#x200B;을(를) 선택합니다. 대상 그룹 목록이 나타납니다. 이 페이지에서 기본 오디언스로 사용할 오디언스를 선택할 수 있습니다.

![동일시 모델링에 사용되는 기본 대상자뿐만 아니라 Audience 버튼이 강조 표시됩니다.](../images/types/lookalike/browse.png)

대상 그룹 세부 정보 페이지에서 **[!UICONTROL Create look-alike audience]**&#x200B;을(를) 선택하여 유사 대상 그룹을 만드는 프로세스를 시작합니다.

![[!UICONTROL Create look-alike audience] 단추가 강조 표시되어 있습니다.](../images/types/lookalike/create-look-alike-audience.png)

**[!UICONTROL Create a look-alike audience]** 팝오버가 나타납니다. 이 페이지에서는 비슷한 모양 사용자에 대한 유사도 비율을 설정할 수 있습니다.

![[!UICONTROL Create a look-alike audience] 팝오버가 표시됩니다.](../images/types/lookalike/create-audience.png)

다음과 같은 세 가지 방법으로 이 유사도 비율을 설정할 수 있습니다.

- 슬라이더를 움직여 유사도 비율을 설정합니다
